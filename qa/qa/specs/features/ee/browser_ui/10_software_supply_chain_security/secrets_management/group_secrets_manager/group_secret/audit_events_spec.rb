...
module QA
  RSpec.describe(
    ...
    describe 'Group Secret audit events' do
      let(:secret_name) { "audit_secret_#{Faker::Alphanumeric.alphanumeric(number: 8)}" }

      it 'records create, update (value and description-only), and delete audit events',
        testcase: 'https://gitlab.com/gitlab-org/gitlab/-/quality/test_cases/604114' do
        Flow::Login.while_signed_in(as: owner) do
          ...
          QA::Support::Retrier.retry_until(
            ...
            message: 'Waiting for group secret audit events to appear'
          ) do
            Page::Group::Menu.perform(&:go_to_audit_events)
            page.has_text?('Created group secret') &&
              page.text.scan('Updated group secret').size == 2 &&
              page.has_text?('Deleted group secret')
          end
        end
      end
    end
