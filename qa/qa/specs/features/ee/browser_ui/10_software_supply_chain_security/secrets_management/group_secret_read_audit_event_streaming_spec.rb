...
module QA
  RSpec.describe(
    ...
    # events table or REST API - they can only be observed at a streaming destination. An
    # instance-level destination is used because group-level destinations reject non-top-level
    # groups, and the base context's group is created under the QA sandbox.
    describe 'Group Secret read audit event streaming' do
      let(:executor) { "qa-runner-#{Faker::Alphanumeric.alphanumeric(number: 8)}" }
      let(:secret_name) { 'Test' }
      let(:secret_value) { 'my-secret-value-for-ci' }
      let(:project) { create(:project, :with_readme, group: group, name: 'group-secret-read-audit-project') }
        ...
        Flow::Login.while_signed_in(as: owner) do
          ...
          EE::Page::Group::Secure::SecretsManager.perform do |secrets_page|
            ...
            secrets_page.create_secret(
              name: secret_name,
              value: secret_value,
              description: 'Group secret for read audit streaming test',
              environment: '*'
            )
          end
        end
      ...
      before do
        ...
        stream_destination.add_filters(%w[secrets_manager_read_group_secret member_created])

        # Confirm streaming is live before relying on the single, expensive pipeline read event.
        mock_service.wait_for_streaming_to_start(event_type: 'member_created', entity_type: 'Group') do
          group.add_member(warmup_member)
        end
      ...
      it 'streams a read audit event when the group secret is consumed in CI',
        testcase: 'https://gitlab.com/gitlab-org/gitlab/-/quality/test_cases/604116' do
        event_record = mock_service.wait_for_event(
          'secrets_manager_read_group_secret', 'Group', group.full_path, wait: 60
        )

        expect(event_record[:body]).to include(
          event_type: 'secrets_manager_read_group_secret',
          entity_type: 'Group',
          entity_path: group.full_path
        )
      end
