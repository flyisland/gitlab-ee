  ...
  RSpec.describe 'Security Risk Management', feature_category: :security_policy_management do
    describe 'Approval policy' do
      ...
      it 'requires approval when a pipeline report has findings matching the approval policy',
        ...
        quarantine: {
          issue: 'https://gitlab.com/gitlab-org/quality/test-failure-issues/-/issues/42458',
          type: :flaky
        } do
        ...
        Page::MergeRequest::Show.perform do |mr|
          ...
          # Guard against transient re-renders of the MR widget that can flip
          # the merge button text after the policy waiter exits.
          # See: https://gitlab.com/gitlab-org/quality/test-failure-issues/-/work_items/42458
          expect { mr.auto_mergeable? }.to eventually_be_truthy.within(max_duration: 30, sleep_interval: 2)
        end
      end

      it 'does not block merge when approval policy does not apply for pipeline security findings',
        testcase: 'https://gitlab.com/gitlab-org/gitlab/-/quality/test_cases/423412',
        quarantine: {
          issue: 'https://gitlab.com/gitlab-org/quality/test-failure-issues/-/issues/42458',
          type: :flaky
        } do
      ...
      def wait_for_merge_request_to_be_blocked_by_policy(merge_request)
        ...
        QA::Support::Waiter.wait_until(max_duration: 120, sleep_interval: 2, reload_page: merge_request,
          ...
          Page::MergeRequest::Show.perform do |mr|
            ...
            # Only check approval conditions - merge_blocked? is unreliable because
            # the MR may show "Set to auto-merge" (not blocked) while still requiring
            # policy approvals. The approval conditions are sufficient to confirm
            # the policy is enforced.
            mr.approvals_required_from.include?(scan_result_policy_name) && !mr.approved?
      ...
      def wait_for_merge_request_to_be_mergeable(merge_request)
        ...
        QA::Support::Waiter.wait_until(max_duration: 90, sleep_interval: 2, reload_page: merge_request,
          ...
          Page::MergeRequest::Show.perform do |mr|
            mr.approval_status_loaded? && mr.mergeable?
          end
        end
      end
