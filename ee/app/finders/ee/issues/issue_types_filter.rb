...
module EE
  module Issues # rubocop:disable Gitlab/BoundedContexts -- FOSS finder is not bounded to a context
    module IssueTypesFilter
      ...
      override :by_work_item_type_ids
      def by_work_item_type_ids(issues)
        return super unless work_item_type_ids_include_epic?
        return super if epic_type_allowed?

        filtered_ids = work_item_type_ids.reject { |id| id == epic_type_id }
        return issues.model.none if filtered_ids.empty?

        issues.with_work_item_type_ids(filtered_ids)
      end

      def work_item_type_ids_include_epic?
        work_item_type_ids.present? && work_item_type_ids.include?(epic_type_id)
      end

      def epic_type_id
        ::WorkItems::TypesFramework::Provider.new(parent).find_by_base_type(:epic)&.id
      end
      strong_memoize_attr :epic_type_id
