...
module Security
  class ApprovalGroupsFinder
    ...
    def global_groups(include_inaccessible)
      ...
      include_inaccessible ? groups_by_ids_or_paths : accessible_groups_to_user
    end

    def accessible_groups_to_user
      Group.by_id(groups_by_ids_or_paths.accessible_to_user(user))
    end

    # rubocop: enable Layout/LineLength

    # "Global" only means instance-wide within the container's organization: a policy in one
    # organization must never resolve group approvers from another organization. When there's no
    # container (instance-level policy, not tied to any organization), no boundary applies.
    def groups_by_ids_or_paths
      relation = Group.by_ids_or_paths(group_ids, group_paths)
      return relation unless container

      relation.in_organization(container.root_ancestor.organization)
    end
