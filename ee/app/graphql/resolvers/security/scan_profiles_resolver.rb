    ...
    class ScanProfilesResolver < BaseResolver
      ...
      def default_scan_profiles
        ::Security::DefaultScanProfilesHelper.default_scan_profiles.tap do |scan_profiles|
          unless Feature.enabled?(:security_scan_profiles_sast, root_ancestor)
            scan_profiles.reject! { |p| p.scan_type == 'sast' }
          end

          unless Feature.enabled?(:security_scan_profiles_dependency_scanning, root_ancestor)
            scan_profiles.reject! { |p| p.scan_type == 'dependency_scanning' }
          end
