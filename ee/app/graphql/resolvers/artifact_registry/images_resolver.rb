...
module Resolvers
  module ArtifactRegistry
    class ImagesResolver < BaseResolver
      include ::ArtifactRegistry::PaginatesLists
      include ::ArtifactRegistry::ResolvesOnRepository

      # Bounds aliased re-selection: a second `images` selection in one operation raises rather
      ...
      type ::Types::ArtifactRegistry::ImageType.connection_type, null: true
      ...
      def resolve_artifact_registry(first: nil, last: nil, before: nil, after: nil)
        ...
        # The client rejects a non-image format before issuing a request, so a package
        ...
        page = artifact_registry_client.images(
        ...
        # Wrapped like the packages connection, so the manifests connection mounted on the image
        # type reaches the repository and organization through its parent element.
