    ...
    module Types
      class Update < BaseMutation
        ...
        def resolve(full_path: nil, **args)
          ...
          authorize_archive_for_granular_token!(namespace) if args.key?(:archive)
        ...
        # `archive` is optional, so its permission cannot be declared through `additional_scopes:`,
        # which is always enforced. See https://gitlab.com/gitlab-org/gitlab/-/issues/627165.
        def authorize_archive_for_granular_token!(namespace)
          response = ::Authz::Tokens::AuthorizeGranularScopesService.new(
            boundaries: [::Authz::Boundary.for(namespace)],
            permissions: [:archive_work_item_type],
          ...
          raise ::Gitlab::Graphql::Errors::ArgumentError, response.message
        end
      end
    end
  end
