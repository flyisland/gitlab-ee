    ...
    module Settings
      class Update < BaseMutation
        ...
        description 'Updates work item settings for a root group or organization. ' \
          ...
          'Only available when the `work_item_configurable_types` feature flag is enabled.'
        ...
        def resolve(full_path: nil, **args)
          ...
          raise_resource_not_available_error! unless feature_flag_enabled?(container)
        ...
        def feature_flag_enabled?(container)
          if container.is_a?(::Organizations::Organization)
            ::Feature.enabled?(:work_item_configurable_types, :instance)
          else
            ::Feature.enabled?(:work_item_configurable_types, container.root_ancestor)
          end
        end
      end
    end
