...
# rubocop:disable Graphql/AuthorizeTypes -- Authorization is enforced on the parent
# `Types::Ai::DuoWorkflows::WorkflowType` via `authorize :read_duo_workflow`.
# Audit events are always scoped to a single workflow whose access has already
# been gated by the parent type's policy.
module Types
  module AuditEvents
