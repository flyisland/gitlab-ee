...
# `SessionArtifactsFinder#allowed?` which gates on `:read_compliance_dashboard`.
module Types
  module Ai
    module DuoWorkflows
      class SessionArtifactType < ::Types::BaseObject
        ...
        def web_path
          ...
          BatchLoader::GraphQL.for({ project_id: project_id, workflow_id: workflow_id_value })
            .batch(key: 'ch-session-artifact-web-path') do |pairs, loader|
              batch_load_web_paths(pairs, loader)
            end
        end
        ...
        def batch_load_web_paths(pairs, loader)
          ...
          pairs.each do |pair|
            ...
            loader.call(
              pair,
              "#{::Gitlab::Routing.url_helpers.project_automate_agent_sessions_path(project)}/#{pair[:workflow_id]}"
            )
          end
        end
      end
