...
module Security
  module DependencyFirewall
    class EnforcementService
      ...
      # Single line (the warn path renders it in an HTTP header), and no counts or thresholds:
      # package clients may not be allowed to read the policy configuration. Everything
      # interpolated is config-derived; a rule deriving `details` externally must sanitize it.
      def build_violation_message(result)
        ...
        severities = exceeded.pluck(:severity).join(', ') # rubocop:disable CodeReuse/ActiveRecord, Database/AvoidUsingPluckWithoutLimit -- Array#pluck on a bounded in-memory array

        "#{base}: vulnerability count #{'threshold'.pluralize(exceeded.size)} exceeded (#{severities})"
      end
