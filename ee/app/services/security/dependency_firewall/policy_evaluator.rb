...
module Security
  module DependencyFirewall
    class PolicyEvaluator
      ...
      def evaluate(name, purl_type:, version: nil, licenses: [], vulnerabilities: [])
      ...
      def evaluate_policies(package, licenses, policies, vulnerabilities)
        policies.map do |policy|
          ...
          if bypass_settings.user_bypassed?(@current_user)
            ...
            matching_rules = policy.rules.filter_map do |rule|
              result = rule.evaluate(package, metadata: { licenses: licenses, vulnerabilities: vulnerabilities })
      ...
      def build_result(action, reason, matched_rule: nil)
