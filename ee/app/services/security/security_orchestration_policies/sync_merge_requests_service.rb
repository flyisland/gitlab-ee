  ...
  module SecurityOrchestrationPolicies
    class SyncMergeRequestsService
      ...
      def sync_any_merge_request_approval_rules(merge_request)
        ...
        jid = ::Security::ScanResultPolicies::SyncAnyMergeRequestApprovalRulesWorker.perform_async(merge_request.id)
        start_merge_request_worker_policy_sync(merge_request.id) if jid
      end
      ...
      def sync_preexisting_state_approval_rules(merge_request)
        ...
        jid = ::Security::ScanResultPolicies::SyncPreexistingStatesApprovalRulesWorker.perform_async(merge_request.id)
        start_merge_request_worker_policy_sync(merge_request.id) if jid
      end

      def notify_for_policy_violations(merge_request)
        jid = ::Security::UnenforceablePolicyRulesNotificationWorker.perform_async(
          merge_request.id,
