  ...
  module Ingestion
    module Tasks
      ...
      class IngestRemediations < AbstractTask
        ...
        def after_ingest
          if Feature.enabled?(:move_remediation_uploads_outside_transaction, project)
            schedule_upload_remediations
          else
            upload_remediations
          end
        ...
        def schedule_upload_remediations
          remediations_to_upload = new_remediations.to_a
          report_remediations_by_checksum = new_report_remediations.index_by(&:checksum)

          remediation_diffs_by_id = remediations_to_upload.each_with_object({}) do |remediation, hash|
            report_remediation = report_remediations_by_checksum[remediation.checksum]
            hash[remediation.id.to_s] = report_remediation.diff
          end

          return if remediation_diffs_by_id.empty?

          SecApplicationRecord.current_transaction.after_commit do
            ::Vulnerabilities::UploadRemediationsWorker.perform_async(remediation_diffs_by_id)
          end
        end
        ...
        def update_vulnerability_reads
          ...
          SecApplicationRecord.current_transaction.after_commit do
            ::Vulnerabilities::BulkEsOperationService.new(unfound_reads).execute
            ::Vulnerabilities::BulkEsOperationService.new(found_reads).execute
          end
        end
