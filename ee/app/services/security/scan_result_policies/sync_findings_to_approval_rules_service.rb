    ...
    class SyncFindingsToApprovalRulesService
      ...
      def update_required_approvals_for_scan_finding
        ...
        merge_requests_targeting_pipeline_ref.each do |merge_request|
          ...
          jid = Security::ScanResultPolicies::SyncMergeRequestApprovalsWorker.perform_async(
            head_pipeline.id,
            merge_request.id)
      ...
      def update_approvals(merge_request)
        jid = Security::ScanResultPolicies::SyncMergeRequestApprovalsWorker.perform_async(pipeline.id, merge_request.id)
        start_merge_request_worker_policy_sync(merge_request.id) if jid
      end
