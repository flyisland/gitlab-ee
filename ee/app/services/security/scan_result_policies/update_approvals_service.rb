    ...
    class UpdateApprovalsService
      ...
      def violates_grouped_scanners?(approval_rule)
        grouped_results = grouped_findings_evaluator(
          pipeline, approval_rule, related_source_pipeline_ids, true
        ).grouped_results || []
      ...
      def grouped_target_uuids(approval_rule)
        target_results = grouped_findings_evaluator(
          ...
          related_target_pipeline_ids(approval_rule), false
        ).grouped_results || []
      ...
      def grouped_findings_evaluator(pipeline, approval_rule, pipeline_ids, check_dismissed)
        evaluator_params = {
