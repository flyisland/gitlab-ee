...
module Security
  class OverrideUuidsService
    ...
    def execute
      ...
      findings.each_slice(BATCH_SIZE) { |batch| OverrideInBatch.execute(project, batch, existing_scanners, known_uuids, tracked_context) }
    ...
    class OverrideInBatch
      def self.execute(project, findings, scanners, known_uuids, tracked_context)
        new(project, findings, scanners, known_uuids, tracked_context).execute
      end

      def initialize(project, findings, scanners, known_uuids, tracked_context)
        @project = project
        @findings = findings
        @scanners = scanners
      ...
      attr_reader :project, :findings, :scanners, :known_uuids, :tracked_context
      ...
      def compare_with_existing_finding(existing_finding, finding)
        existing_finding.primary_identifier&.fingerprint == finding.primary_identifier_fingerprint &&
          ...
          existing_finding.project_tracked_context&.id ==
            tracked_context&.id
      end
