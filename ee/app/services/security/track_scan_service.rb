...
module Security
  class TrackScanService
    ...
    def execute
      build.unmerged_security_reports.each { |report_type, report| track_scan_event(report_type, report) }
    rescue Gitlab::Ci::Parsers::ParserError
      # Tracking is best-effort: skip on any parse failure rather than fail the worker.
      nil
    end
    ...
    def track_scan_event(report_type, report)
      ...
      # SARIF fan-out reports have no scan; scanner_external_id keeps same-type runs distinct.
      differentiator = report&.scanner_external_id.presence || report&.scan&.start_time || ""
      idempotency_key = [build.project_id, build.id, scan_type(report, report_type), differentiator].join("::")
    ...
    def data_to_track(report_type, report)
...

      {
        ...
        end_time: normalize_time(scan&.end_time),
        ...
        start_time: normalize_time(scan&.start_time),
