...
module MergeTrains
  class AddMergeRequestService < BaseService
    ...
    def select_strategy(auto_merge_service)
      if Feature.enabled?(:merge_train_api_auto_merge_fallback, @merge_request.target_project)
        select_strategy_with_fallback(auto_merge_service)
      else
        select_strategy_legacy
      end
    end

    def select_strategy_with_fallback(auto_merge_service)
