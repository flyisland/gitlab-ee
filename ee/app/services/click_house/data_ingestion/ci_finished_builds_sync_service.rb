...
module ClickHouse
  module DataIngestion
    class CiFinishedBuildsSyncService
      ...
      MODES = %i[recent backfill].freeze

      def self.enabled?
        ::Gitlab::ClickHouse.configured?
      end

      def initialize(worker_index: 0, total_workers: 1, mode: nil)
        @runtime_limiter = Gitlab::Metrics::RuntimeLimiter.new(MAX_RUNTIME)
        @worker_index = worker_index
        @total_workers = total_workers
        @mode = mode&.to_sym
        @threshold = RECENT_THRESHOLD.ago if @mode

        raise ArgumentError, "Invalid mode: #{mode}" if @mode && MODES.exclude?(@mode)
      end
      ...
      def execute
        ...
        lock_key = ["#{self.class.name.underscore}/worker", @mode, @worker_index].compact.join('/')
        in_lock(lock_key, ttl: MAX_TTL, retries: 0) do
      ...
      def keyset_iterator_scope
        ...
        scope = if @mode
                  apply_mode_filter(Ci::FinishedBuildChSyncEvent.pending.order_by_build_finished_at)
                else
                  Ci::FinishedBuildChSyncEvent.pending.order_by_build_id
                end
      ...
      def apply_mode_filter(scope)
        case @mode
        when :recent
          scope.where(build_finished_at: @threshold..) # rubocop: disable CodeReuse/ActiveRecord -- mode filter specific to this service
        when :backfill
          scope.where(build_finished_at: ...@threshold) # rubocop: disable CodeReuse/ActiveRecord -- mode filter specific to this service
        end
      end
