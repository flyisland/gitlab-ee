    ...
    module ScannerProfiles
      class UpdateService < BaseService
        ...
        def execute
          ...
          return ServiceResponse.error(message: format(_('Cannot modify %{profile_name} referenced in security policy'), profile_name: dast_scanner_profile.name)) if referenced_in_security_policy?(dast_scanner_profile)
          return ServiceResponse.error(message: format(_('Cannot modify %{profile_name} referenced in scan'), profile_name: dast_scanner_profile.name)) unless dast_scanner_profile.can_edit_profile?(current_user)
