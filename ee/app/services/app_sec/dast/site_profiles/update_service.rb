    ...
    module SiteProfiles
      class UpdateService < BaseService
        ...
        def execute(id:, **params)
          ...
          return ServiceResponse.error(message: _('Insufficient permissions')) unless allowed_to_modify_profile?

          return ServiceResponse.error(message: format(_('Cannot modify %{profile_name} referenced in security policy'), profile_name: dast_site_profile.name)) if referenced_in_security_policy?
          return ServiceResponse.error(message: format(_('Cannot modify %{profile_name} referenced in scan'), profile_name: dast_site_profile.name)) unless dast_site_profile.can_edit_profile?(current_user)
          ...
          ServiceResponse.error(message: format(_('%{model_name} not found'), model_name: e.model))
        ...
        def allowed_to_modify_profile?
          Ability.allowed?(current_user, :create_on_demand_dast_scan, dast_site_profile)
        end
