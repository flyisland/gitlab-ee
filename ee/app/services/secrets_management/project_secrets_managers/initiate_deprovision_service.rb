...
module SecretsManagement
  module ProjectSecretsManagers
    class InitiateDeprovisionService
      ...
      # root_namespace_id, parent_group_id) is a required kwarg. The
      # caller must capture those at the moment that matters (before any
      # state change) and pass them in explicitly. The service does not
      # read those off the live secrets manager or its parent records,
      # which may have moved or disappeared by the time we run.
      def initialize(
        secrets_manager,
        user = nil,
        project_id:,
      ...
      class << self
        ...
        def bulk_initiate_for_projects(projects_relation, user, old_root_namespace_id:, old_organization_id:)
          ...
          # pointing at a non-existent task row.
          task_ids = ::ApplicationRecord.transaction do
            ...
            ::SecretsManagement::ProjectSecretsManagerMaintenanceTask
              .insert_all(task_attrs, returning: %w[id])
              .rows.flatten
            # rubocop:enable CodeReuse/ActiveRecord
          end
