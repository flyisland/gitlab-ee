...
module Vulnerabilities
  module Flags
    class DismissFalsePositiveService
      ...
      def dismiss_flag_status
        if Feature.enabled?(:vulnerability_flag_status_based_gating, project.root_ancestor)
          Vulnerabilities::Flag::FALSE_POSITIVE_DETECTION_STATUSES[:dismissed]
        else
          Vulnerabilities::Flag::FALSE_POSITIVE_DETECTION_STATUSES[:not_started]
        end
      end
      ...
      def can_dismiss_flag?
        ...
        latest_flag.dismissable?(project.root_ancestor)
      end
