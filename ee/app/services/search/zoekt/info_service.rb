  ...
  module Zoekt
    class InfoService
      ...
      def log_buffer_factor
        ...
        flag_enabled = Feature.enabled?(:zoekt_dynamic_buffer_factor, Feature.current_request)
        source = flag_enabled ? 'dynamic (observed)' : Rainbow('static fallback (FF disabled)').yellow

        log("Storage buffer factor", value: "#{colored_factor} [#{source}]")
      end
