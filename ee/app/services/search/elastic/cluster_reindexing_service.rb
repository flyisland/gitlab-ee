  ...
  module Elastic
    class ClusterReindexingService
      ...
      def target_classes
        ...
        if classes.include?(::Search::Elastic::MigrationsIndexConfig)
          classes -= [::Search::Elastic::MigrationsIndexConfig] unless migrations_index_ready?
        elsif current_task.targets.blank? && migrations_index_ready?
      ...
      def migrations_index_explicitly_requested_and_ready?
        if current_task.target_classes.include?(::Search::Elastic::MigrationsIndexConfig) && !migrations_index_ready?
      ...
      def default_index_options(alias_name:, index_name:)
        ...
        # Migrations index has no IndexSetting entry, use replica count from old index
        options[:number_of_replicas] = if alias_name == elastic_helper.migrations_alias_name
      ...
      def initial!
        ...
        return false unless migrations_index_explicitly_requested_and_ready?
