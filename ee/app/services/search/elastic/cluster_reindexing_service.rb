...
module Search
  module Elastic
    class ClusterReindexingService
      ...
      def target_classes
        classes = current_task.target_classes.dup

        # Only add for full reindex (no specific targets), not for explicit requests
        # Readiness validation for explicit targets happens in target_readiness_valid?
        if current_task.targets.blank?
          if Feature.enabled?(:elasticsearch_migrations_type_class, :instance)
            classes << ::Search::Elastic::MigrationsIndexConfig if ::Search::Elastic::MigrationsIndexConfig.ready?
          elsif migrations_index_ready?
            classes << ::Search::Elastic::MigrationsIndexConfig
          end
        end

        classes
      end
      strong_memoize_attr :target_classes

      def migrations_index_ready?
        ::Elastic::DataMigrationService.migration_has_finished?(:delete_legacy_migrations_index)
      end

      def target_readiness_valid?
        if Feature.enabled?(:elasticsearch_migrations_type_class, :instance)
          current_task.target_classes.each do |klass|
            next unless klass.respond_to?(:ready?) && !klass.ready?

            # Keep backward-compatible error message for MigrationsIndexConfig
            error_msg = if klass == ::Search::Elastic::MigrationsIndexConfig
                          'MigrationsIndexConfig was explicitly requested but the delete_legacy_migrations_index ' \
                            'migration has not finished. Please wait for the migration to complete before reindexing.'
                        else
                          "#{klass.name} was explicitly requested but is not ready. " \
                            "Please wait for required migrations to complete before reindexing."
                        end

            abort_reindexing!(error_msg)
            return false
          end
        elsif current_task.target_classes.include?(::Search::Elastic::MigrationsIndexConfig) && !migrations_index_ready?
          abort_reindexing!('MigrationsIndexConfig was explicitly requested but the delete_legacy_migrations_index ' \
            'migration has not finished. Please wait for the migration to complete before reindexing.')
          return false
        end
      ...
      def default_index_options(alias_name:, index_name:)
        settings = elastic_helper.get_settings(index_name: index_name)
        options = {
        ...
        if Feature.enabled?(:elasticsearch_migrations_type_class, :instance)
          index_setting = ::Elastic::IndexSetting.for_alias_name(alias_name).first
          options[:number_of_replicas] = index_setting&.number_of_replicas || settings['number_of_replicas'].to_i
        else
          options[:number_of_replicas] = if alias_name == elastic_helper.migrations_index_name
                                           settings['number_of_replicas'].to_i
                                         else
                                           ::Elastic::IndexSetting[alias_name].number_of_replicas
                                         end
        end
      ...
      def initial_index_options
        base = { refresh_interval: '10s', number_of_replicas: 0 }
        async_durability_restricted? ? base : base.merge(translog: { durability: 'async' })
      end
      strong_memoize_attr :initial_index_options

      def initial!
        return false unless elasticsearch_indexing_enabled?
        return false unless no_pending_migrations?
        return false unless target_readiness_valid?

        ::Gitlab::CurrentSettings.update!(elasticsearch_advanced_search_pause_indexing: true)
        log_info('elasticsearch_advanced_search_pause_indexing set to true')
      ...
      def no_pending_migrations?
        return true if current_task.options[:skip_pending_migrations_check] ||
      ...
      def sufficient_storage_available?
        ...
        false
      rescue ::Gitlab::Search::Client::ConnectionError => e
        abort_reindexing!("Failed to calculate index size: #{e.message}")
        false
      end

      def current_index_size
        target_classes.sum do |klass|
      ...
      def indexing_paused!
        ...
        abort_reindexing!("Failed to create reindex target indices: #{e.message}")
        false
      end
      ...
      def load_alias_info(klass:, name_suffix:)
        ...
        elsif klass == ::Search::Elastic::MigrationsIndexConfig
          alias_name = ::Search::Elastic::MigrationsIndexConfig.alias_name
          new_index_name = elastic_helper.index_name_with_timestamp(alias_name, suffix: name_suffix)

          settings = ::Search::Elastic::MigrationsIndexConfig.settings.merge(initial_index_options)
          mappings = ::Search::Elastic::MigrationsIndexConfig.mappings

          elastic_helper.create_index(
            index_name: new_index_name,
            alias_name: alias_name,
            with_alias: false,
            settings: settings,
            mappings: mappings,
            options: { skip_if_exists: false }
          )

          { new_index_name => alias_name }
        else
      ...
      def check_subtasks_and_reindex_slices
        ...
        ::Search::Elastic::ClusterReindexingCronWorker.perform_in(1.minute)
      ...
      def finalize_reindexing
        ::Gitlab::CurrentSettings.update!(elasticsearch_advanced_search_pause_indexing: false)
      ...
      def abort_reindexing!(reason, additional_logs: {}, unpause_indexing: true)
        ...
        log_info('elasticsearch_advanced_search_pause_indexing set to false')
      end
      ...
      def async_durability_restricted?
        if ::Gitlab::CurrentSettings.elasticsearch_aws? && elastic_helper.matching_distribution?(:opensearch)
          return true
        end

        return true if elastic_helper.serverless?

        nodes = elastic_helper.nodes_settings
        return false if nodes.empty?

        nodes.any? do |_node_id, node|
          node.dig('settings', 'cluster.remote_store.index.restrict.async-durability') == 'true'
        end
      end
