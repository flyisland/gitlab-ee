      ...
      module FoundationalFlows
        class CompatibleRunnerProbe < BaseProbe
          ...
          def compatible_runner_exists?
            tagged_runners.active.preload(:runner_managers).any? do |runner|
              runner.runner_managers.any? { |manager| docker_compatible_executor?(manager) }
            end
          end
          ...
          def docker_compatible_executor?(manager)
            docker_compatible_executor_types.include?(manager.executor_type.to_s)
          end
          ...
          def docker_compatible_executor_types
            ::Ci::RunnerManager.executor_types
              .keys
              .select { |key| key.start_with?('docker') || key == 'kubernetes' }
          end
