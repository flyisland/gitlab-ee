...
module SystemNotes
  class MergeTrainService < ::SystemNotes::BaseService
    ...
    def abort(reason)
      ...
      body = "removed this merge request from the merge train because #{format_reason(reason)}"
    ...
    def abort_add_when_checks_pass(reason)
      ...
      # TODO: Abort message should be sent by the system, not a particular user.
      # See https://gitlab.com/gitlab-org/gitlab/issues/29467.
      body = "aborted automatic add to merge train because #{format_reason(reason)}"
      create_note(NoteSummary.new(noteable, project, author, body, action: 'merge'))
    end
