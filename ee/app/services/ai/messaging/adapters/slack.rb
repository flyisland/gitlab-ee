  ...
  module Messaging
    module Adapters
      class Slack < Base
        ...
        ACK_MESSAGE = 'On it! Working on your request.'

        # Rotating loading messages shown via assistant.threads.setStatus from
        # on_request_received until on_flow_started posts the acknowledgement
        # message. Slack renders them as "<AppName> <message>" and auto-rotates
        # them. If container setup outlives the setStatus 2-minute timeout the
        # indicator clears early; the on_flow_started post still arrives once the
        # agent starts.
        STATUS_LOADING_MESSAGES = [
          'is getting started…',
          'is reviewing your request…',
          'is spinning up the agent…'
        ].freeze
        ...
        # Best-effort: same rationale as deliver_result above.
        ...
        def body_with_session_footer(message, callback_context)
          ...
          if url.present?
            blocks << { type: 'context', elements: [{ type: 'mrkdwn', text: "<#{url}|View session in GitLab>" }] }
          end
        ...
        def error_text(error)
          ...
          when :namespace_not_configured
            ...
            "Set your default Duo namespace in your " \
              "GitLab preferences (#{preferences_url}) to use Duo from Slack."
          when :flow_not_enabled
            "The Duo Developer flow is not enabled for your namespace. " \
              "Ask a group owner to enable it in the Duo Agent Platform settings."
          when :service_account_error
            "Could not set up the service account for the Duo Developer flow. " \
              "Contact your group owner."
          when :workspace_project_error
            "Could not set up the workspace project. " \
              "Check that you have access to the namespace and permission to create projects."
          when :execute_workflow_failed
            "Failed to start the Duo Developer workflow. Try again."
          when :flow_failed
            "Something went wrong while running the task. Try again."
          when :no_response
            "The task completed but the agent didn't produce a response. " \
