    ...
    module Onboarding
      class RunService < Ai::Catalog::BaseService
        ...
        def execute
          ...
          active_workflow = tracker.active_workflow(initializer.event_type)
          if active_workflow
            return error('An initializer run is already in progress.', payload: { workflow_id: active_workflow.id })
          end
        ...
        def allowed?
          initializers_enabled? && Ability.allowed?(current_user, :duo_workflow, project)
        end

        def initializers_enabled?
