    ...
    module ItemConsumers
      class UpdateService
        ...
        def validation_error
          ...
          return if params[:pinned_version_prefix].nil? && nil_version_pin_enforced?
        ...
        def error_pinned_version_prefix_format
          error('pinned_version_prefix is not a valid version')
        end

        def error_version_is_not_latest
          error('pinned_version_prefix must resolve to the latest released version of the agent or flow')
        end
