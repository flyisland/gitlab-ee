    ...
    class StartWorkflowService
      ...
      def run_workload
        service = ::Ci::Workloads::RunWorkloadService.new(
          ...
          ref: @ref,
          duo_workflow_definition: resolve_workflow_definition(@workflow.workflow_definition, project)
        )
      ...
      def sandbox
        @sandbox ||= ::Gitlab::DuoWorkflow::Sandbox.new(
          ...
          unmask_env_variables: base_variables.merge(git_environment_variables).merge(token_variables).keys
        )
      end
      ...
      def base_variables
        ...
        base_variables = {
          ...
          DUO_WORKFLOW_DEFINITION: resolve_workflow_definition(
      ...
      def commands
        # Prepend sandbox setup commands and setup_script commands to the main commands
        sandbox.setup_sandbox_commands + setup_script_commands + main_workflow_commands
      end

      def main_workflow_commands
        [
          # Set origin to an oauth-based URL so the runner's insteadOf rules (which inject
          # the CI job token) do not apply. Those rules only match http://host, not http://user@host.
      ...
      def resolve_workflow_definition(definition, project)
        case definition
        when 'developer/v1'
          resolve_developer_workflow(definition, project)
        when 'fix_pipeline/v1'
          resolve_fix_pipeline_workflow(definition, project)
        else
          definition
        end
      end

      def resolve_developer_workflow(definition, project)
        if Feature.enabled?(:duo_developer_next_unstable, project) ||
            Feature.enabled?(:duo_developer_next_unstable, project.root_ancestor)
          'developer_unstable/experimental'
        elsif Feature.enabled?(:duo_developer_next_compatible, project) ||
            Feature.enabled?(:duo_developer_next_compatible, project.root_ancestor)
      ...
      def resolve_fix_pipeline_workflow(definition, project)
        if Feature.enabled?(:fix_pipeline_next, project)
          'fix_pipeline_next/v1'
        else
          definition
        end
      end
