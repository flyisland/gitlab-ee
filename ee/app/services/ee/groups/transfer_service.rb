  ...
  module Groups
    module TransferService
      ...
      def proceed_to_transfer
        ...
        # the tree.
        #
        # BEFORE super: capture the OLD root namespace id and OLD
        # organization id. They are the same for every descendant and
        # for `group` itself, since `group.root_ancestor` is shared by
        # the whole subtree. After super, both values reflect the NEW
        # parent. The deprovision worker needs the OLD ids because
        # OpenBao state lives at the OLD path.
        #
        # AFTER super: bulk-initiate deprovision per batch of groups
        # and projects. Bulk SQL keeps this cheap even for hierarchies
        # with thousands of SM-enabled descendants. Worker enqueue
        # waits until after the transfer commits so the queue never
        # holds a job that depends on uncommitted state.
        #
        ...
        super.tap do
          ...
          bulk_initiate_secrets_manager_deprovisions(old_root_namespace_id, old_organization_id)
        end
      end
      ...
      # We don't rescue or log here on purpose. The transfer has already
      # happened at this point, so the only kind of error that could
      # surface is infrastructural (DB hiccup, Redis outage, OOM).
      # Those belong in Sentry; a swallowed log line would be invisible.
      #
      # If something does fail before the maintenance task gets created,
      # the secrets manager row stays behind in `active` state pointing
      # at the OLD ids. The orphan reaper (gitlab-org/gitlab#600120)
      # backstops via path-divergence detection.
      def bulk_initiate_secrets_manager_deprovisions(old_root_namespace_id, old_organization_id)
        group.self_and_descendants.each_batch(of: GROUP_QUERY_BATCH_SIZE) do |groups|
          ::SecretsManagement::GroupSecretsManagers::InitiateDeprovisionService.bulk_initiate_for_groups(
            groups,
            current_user,
        ...
        group.all_projects.each_batch(of: PROJECT_QUERY_BATCH_SIZE) do |projects|
          ::SecretsManagement::ProjectSecretsManagers::InitiateDeprovisionService.bulk_initiate_for_projects(
            projects,
            current_user,
            old_root_namespace_id: old_root_namespace_id,
            old_organization_id: old_organization_id
