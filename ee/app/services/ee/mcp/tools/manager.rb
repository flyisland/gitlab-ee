    ...
    module Tools
      module Manager
        ...
        def get_tool(name:, version: nil)
          ...
          return get_ee_custom_tool(name, version) if EE_CUSTOM_TOOLS.key?(name) && !use_rest_api_for_mcp_tool?(name)
        ...
        def use_rest_api_for_mcp_tool?(name)
          name == 'semantic_code_search' &&
            ::Feature.enabled?(:mcp_semantic_code_search_use_rest_api, ::Feature.current_request)
        end
        ...
        def build_ee_tools
          ...
          EE_CUSTOM_TOOLS.each do |name, tool_class|
            next if use_rest_api_for_mcp_tool?(name)
