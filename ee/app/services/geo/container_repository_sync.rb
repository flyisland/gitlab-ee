      ...
      if LIST_MANIFESTS.include? manifest_parsed['mediaType']
        if buildkit_oci_incompatible_index?(manifest_parsed['manifests'])
          ...
          manifest_parsed['manifests'].each do |submanifest_ref|
            ...
            # The mediaType field in the manifest body is optional per the OCI spec
            # and is commonly omitted by buildkit. Fall back to the descriptor's
            # mediaType from the parent index (required by the OCI spec), then to
            # the OCI image manifest type, so the Content-Type sent to the registry
            # is never nil.
