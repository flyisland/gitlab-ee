...
module Ci
  module Minutes
    class TrackLiveConsumptionService
      ...
      FREE_USER_CONSUMPTION_THRESHOLD = -100
      TTL_PAID_NAMESPACE_CACHE = 10.minutes
      ...
      def consumption_threshold
        unless Feature.enabled?(:ci_reduced_consumption_threshold_for_free_users, build.project)
          return CONSUMPTION_THRESHOLD
        end

        return CONSUMPTION_THRESHOLD if paid_namespace?

        FREE_USER_CONSUMPTION_THRESHOLD
      end

      def paid_namespace?
        return false unless build.user

        Rails.cache.fetch("ci:minutes:user:#{build.user_id}:paid_namespace", expires_in: TTL_PAID_NAMESPACE_CACHE) do
          build.user.belongs_to_paid_namespace?(exclude_trials: true)
        end
      end
