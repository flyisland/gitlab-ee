...
module Projects
  class UpdateMirrorService < BaseService
    ...
    def create_branches(branches_to_create, errors)
      ...
      if Feature.enabled?(:async_mirror_branch_creation, project)
        ...
        result = ::Branches::CreateService.new(project, current_user)
          .bulk_create(branches_to_create)
    ...
    def schedule_async_batches(branches, waiter_key)
      branches.each_slice(ASYNC_BRANCH_BATCH_SIZE).with_index.sum do |batch, index|
        ...
        ::Repositories::CreateBranchWorker.bulk_perform_in_with_contexts(
          ...
          objects,
          arguments_proc: ->((name, ref)) { [project.id, current_user.id, name, ref, waiter_key] },
    ...
    # Only on first sync with the feature flag enabled. The default branch
    # is handled separately and will get a pipeline.
    def skip_ci_for_non_default_branches?
      first_sync? && Feature.enabled?(:skip_pipelines_for_mirror_first_sync_non_default_branches, project)
    end
