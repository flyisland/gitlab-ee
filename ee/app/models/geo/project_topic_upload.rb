...
# Read-only model for the project_topic_uploads upload partition table.
#
...
#   while ProjectTopicUploadReplicator handles replication on the secondary.
#
...
module Geo
  class ProjectTopicUpload < ::Upload
    include ::Geo::ReplicableModel
    include ::Geo::VerifiableModel

    self.table_name = 'project_topic_uploads'
    self.primary_key = :id

    delegate(*::Geo::VerificationState::VERIFICATION_METHODS, to: :project_topic_upload_state)

    with_replicator Geo::ProjectTopicUploadReplicator

    has_one :project_topic_upload_state,
      autosave: false,
      inverse_of: :project_topic_upload,
      class_name: 'Geo::ProjectTopicUploadState'

    scope :with_verification_state, ->(state) {
      joins(:project_topic_upload_state)
        .where(project_topic_upload_states: { verification_state: verification_state_value(state) })
    }

    scope :organization_id_in, ->(ids) { where(organization_id: ids) }

    def verification_state_object
      project_topic_upload_state
    end

    def project_topic_upload_state
      super || build_project_topic_upload_state
    end
