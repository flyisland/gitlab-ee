...
# Read-only model for the ai_vectorizable_file_uploads upload partition table.
#
...
#   while AiVectorizableFileUploadReplicator handles replication on the secondary.
#
...
module Geo
  class AiVectorizableFileUpload < ::Upload
    include ::Geo::ReplicableModel
    include ::Geo::VerifiableModel
    extend ::Gitlab::Utils::Override

    self.table_name = 'ai_vectorizable_file_uploads'
    self.primary_key = :id

    delegate(*::Geo::VerificationState::VERIFICATION_METHODS, to: :ai_vectorizable_file_upload_state)

    with_replicator Geo::AiVectorizableFileUploadReplicator

    has_one :ai_vectorizable_file_upload_state,
      autosave: false,
      inverse_of: :ai_vectorizable_file_upload,
      class_name: 'Geo::AiVectorizableFileUploadState'

    scope :with_verification_state, ->(state) {
      joins(:ai_vectorizable_file_upload_state)
        .where(ai_vectorizable_file_upload_states: { verification_state: verification_state_value(state) })
    }

    scope :project_id_in, ->(ids) { where(project_id: ids) }

    def verification_state_object
      ai_vectorizable_file_upload_state
    end

    def ai_vectorizable_file_upload_state
      super || build_ai_vectorizable_file_upload_state
    end
    ...
    class << self
      ...
      def verification_state_table_class
        Geo::AiVectorizableFileUploadState
      end
    end
  end
end
