...
# Read-only model for the design_management_action_uploads upload partition table.
#
...
#   while DesignManagementActionUploadReplicator handles replication on the secondary.
#
...
module Geo
  class DesignManagementActionUpload < ::Upload
    include ::Geo::ReplicableModel
    include ::Geo::VerifiableModel
    extend ::Gitlab::Utils::Override

    self.table_name = 'design_management_action_uploads'
    self.primary_key = :id

    delegate(*::Geo::VerificationState::VERIFICATION_METHODS, to: :design_management_action_upload_state)

    with_replicator Geo::DesignManagementActionUploadReplicator

    has_one :design_management_action_upload_state,
      autosave: false,
      inverse_of: :design_management_action_upload,
      class_name: 'Geo::DesignManagementActionUploadState'

    scope :with_verification_state, ->(state) {
      joins(:design_management_action_upload_state)
        .where(design_management_action_upload_states: { verification_state: verification_state_value(state) })
    }

    scope :namespace_id_in, ->(ids) { where(namespace_id: ids) }

    def verification_state_object
      design_management_action_upload_state
    end

    def design_management_action_upload_state
      super || build_design_management_action_upload_state
    end
    ...
    class << self
      ...
      def verification_state_table_class
        Geo::DesignManagementActionUploadState
      end
    end
  end
end
