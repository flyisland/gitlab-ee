...
# Read-only model for the organization_detail_uploads upload partition table.
#
...
#   while OrganizationDetailUploadReplicator handles replication on the secondary.
#
...
module Geo
  class OrganizationDetailUpload < ::Upload
    include ::Geo::ReplicableModel
    include ::Geo::VerifiableModel

    self.table_name = 'organization_detail_uploads'
    self.primary_key = :id

    delegate(*::Geo::VerificationState::VERIFICATION_METHODS, to: :organization_detail_upload_state)

    with_replicator Geo::OrganizationDetailUploadReplicator

    has_one :organization_detail_upload_state,
      autosave: false,
      inverse_of: :organization_detail_upload,
      class_name: 'Geo::OrganizationDetailUploadState'

    scope :with_verification_state, ->(state) {
      joins(:organization_detail_upload_state)
        .where(organization_detail_upload_states: { verification_state: verification_state_value(state) })
    }

    scope :organization_id_in, ->(ids) { where(organization_id: ids) }

    def verification_state_object
      organization_detail_upload_state
    end

    def organization_detail_upload_state
      super || build_organization_detail_upload_state
    end
    ...
    class << self
      ...
      def verification_state_table_class
        Geo::OrganizationDetailUploadState
      end
    end
  end
end
