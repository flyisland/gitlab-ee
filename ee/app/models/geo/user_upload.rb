...
# Read-only model for the user_uploads upload partition table.
#
...
#   while UserUploadReplicator handles replication on the secondary.
#
...
module Geo
  class UserUpload < ::Upload
    include ::Geo::ReplicableModel
    include ::Geo::VerifiableModel
    extend ::Gitlab::Utils::Override

    self.table_name = 'user_uploads'
    self.primary_key = :id

    delegate(*::Geo::VerificationState::VERIFICATION_METHODS, to: :user_upload_state)

    with_replicator Geo::UserUploadReplicator

    has_one :user_upload_state,
      autosave: false,
      inverse_of: :user_upload,
      class_name: 'Geo::UserUploadState'

    scope :with_verification_state, ->(state) {
      joins(:user_upload_state)
        .where(user_upload_states: { verification_state: verification_state_value(state) })
    }
    ...
    def user_upload_state
      super || build_user_upload_state
    end
