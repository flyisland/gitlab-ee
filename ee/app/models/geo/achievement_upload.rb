...
# Read-only model for the achievement_uploads upload partition table.
#
...
#   while AchievementUploadReplicator handles replication on the secondary.
#
...
module Geo
  class AchievementUpload < ::Upload
    include ::Geo::ReplicableModel
    include ::Geo::VerifiableModel
    extend ::Gitlab::Utils::Override

    self.table_name = 'achievement_uploads'
    self.primary_key = :id

    delegate(*::Geo::VerificationState::VERIFICATION_METHODS, to: :achievement_upload_state)

    with_replicator Geo::AchievementUploadReplicator

    has_one :achievement_upload_state,
      autosave: false,
      inverse_of: :achievement_upload,
      class_name: 'Geo::AchievementUploadState'

    scope :with_verification_state, ->(state) {
      joins(:achievement_upload_state)
        .where(achievement_upload_states: { verification_state: verification_state_value(state) })
    }

    scope :namespace_id_in, ->(ids) { where(namespace_id: ids) }

    def verification_state_object
      achievement_upload_state
    end

    def achievement_upload_state
      super || build_achievement_upload_state
    end
