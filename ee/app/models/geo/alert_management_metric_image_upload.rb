...
# Read-only model for the alert_management_alert_metric_image_uploads upload partition table.
#
...
#   while AlertManagementMetricImageUploadReplicator handles replication on the secondary.
#
...
module Geo
  class AlertManagementMetricImageUpload < ::Upload
    include ::Geo::ReplicableModel
    include ::Geo::VerifiableModel

    self.table_name = 'alert_management_alert_metric_image_uploads'
    self.primary_key = :id

    delegate(*::Geo::VerificationState::VERIFICATION_METHODS, to: :alert_management_metric_image_upload_state)

    with_replicator Geo::AlertManagementMetricImageUploadReplicator

    has_one :alert_management_metric_image_upload_state,
      autosave: false,
      inverse_of: :alert_management_metric_image_upload,
      class_name: 'Geo::AlertManagementMetricImageUploadState'

    scope :with_verification_state, ->(state) {
      joins(:alert_management_metric_image_upload_state)
        .where(alert_management_metric_image_upload_states: { verification_state: verification_state_value(state) })
    }

    scope :project_id_in, ->(ids) { where(project_id: ids) }

    def verification_state_object
      alert_management_metric_image_upload_state
    end

    def alert_management_metric_image_upload_state
      super || build_alert_management_metric_image_upload_state
    end
    ...
    class << self
      ...
      def verification_state_table_class
        Geo::AlertManagementMetricImageUploadState
      end
    end
  end
end
