...
# Read-only model for the snippet_uploads upload partition table.
#
...
#   while PersonalSnippetUploadReplicator handles replication on the secondary.
#
...
module Geo
  class PersonalSnippetUpload < ::Upload
    include ::Geo::ReplicableModel
    include ::Geo::VerifiableModel
    extend ::Gitlab::Utils::Override

    self.table_name = 'snippet_uploads'
    self.primary_key = :id

    delegate(*::Geo::VerificationState::VERIFICATION_METHODS, to: :personal_snippet_upload_state)

    with_replicator Geo::PersonalSnippetUploadReplicator

    has_one :personal_snippet_upload_state,
      autosave: false,
      inverse_of: :personal_snippet_upload,
      class_name: 'Geo::PersonalSnippetUploadState'

    scope :with_verification_state, ->(state) {
      joins(:personal_snippet_upload_state)
        .where(personal_snippet_upload_states: { verification_state: verification_state_value(state) })
    }

    scope :organization_id_in, ->(ids) { where(organization_id: ids) }

    def verification_state_object
      personal_snippet_upload_state
    end

    def personal_snippet_upload_state
      super || build_personal_snippet_upload_state
    end
    ...
    class << self
      ...
      def verification_state_table_class
        Geo::PersonalSnippetUploadState
      end
    end
  end
end
