...
class ApprovalState
  ...
  def approval_context
    strong_memoize(:approval_context) do
      Approvals::Context.new(merge_request)
    end
  end
  ...
  def eligible_for_approval_by?(user)
    ...
    unless committers_can_approve?
      committer_users = if Feature.enabled?(:approval_committer_emails_from_diff, merge_request.target_project)
                          merge_request.committers_to_filter_from_approvers
                        else
                          merge_request.committers(include_author_when_signed: true)
                        end
    ...
    strong_memoize(:user_defined_rules) do
      if approval_rules_overwritten? || (merge_request.merged? && user_defined_merge_request_rules.any?)
        ...
        project.visible_user_defined_rules(branch: target_branch).map do |rule|
          ApprovalWrappedRule.wrap(merge_request, rule, context: approval_context)
        end
      end
    end
  end
