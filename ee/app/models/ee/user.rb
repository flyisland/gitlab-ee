...
module EE
  ...
  module User
    ...
    # (when include_all_memberships: true and the identity_verification_paid_membership
    # flag is enabled) any non-guest membership (project or group), in a namespace
    ...
    def namespace_union_for_membership(include_all_memberships: false)
      memberships =
        if include_all_memberships
          if ::Feature.enabled?(:identity_verification_paid_membership, self)
            member_namespaces.merge(::Member.non_request.non_guests).select(Arel.sql('namespaces.traversal_ids[1]'))
          else
            reporter_developer_maintainer_owned_groups.select(Arel.sql('namespaces.traversal_ids[1]'))
          end
        else
