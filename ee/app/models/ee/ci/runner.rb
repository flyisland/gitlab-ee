...
module EE
  module Ci
    module Runner
      ...
      prepended do
        before_validation :sync_allowed_plan_ids

        validates :allowed_plan_ids, presence: true, if: -> { allowed_plan_name_uids.present? }
      ...
      def allowed_plan_names
        allowed_plan_name_uids.filter_map { |uid| ::Plan.plan_name_uids.key(uid) }
      end

      def allowed_plans=(names)
        self.allowed_plan_name_uids = ::Plan.uids_for_names(names)
      end
      ...
      override :compute_token_expiration
      def compute_token_expiration(use_explicit: true)
        return explicit_token_expires_at if use_explicit && explicit_token_expires_at.present?
      ...
      def sync_allowed_plan_ids
        return unless allowed_plan_name_uids_changed?

        self.allowed_plan_ids = ::Plan.ids_for_plan_name_uids(allowed_plan_name_uids)
      end
