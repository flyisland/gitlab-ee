...
module EE
  module Ci
    ...
    module Build
      ...
      # NOTE: This is a temporary ficticious URL to enable tracing.
      # We will remove this once we add an application setting for it (see
      # https://gitlab.com/gitlab-org/gitlab/-/issues/591941).
      OTEL_COLLECTOR_URL = 'https://otel-collector.gitlab.net/v1/traces'
      ...
      def tracing_context
...

        {
          ...
          otel_endpoints: [{ url: OTEL_COLLECTOR_URL }]
        }.tap do |context|
          next unless pipeline.parent_pipeline? # Bail out if pipeline doesn't have a parent pipeline

          bridge = pipeline.source_bridge
          context[:span_parent_id] = format('%016x', bridge.id) if bridge
        end
      end
