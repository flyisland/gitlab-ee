  ...
  module Namespace
    ...
    prepended do
      ...
      scope :with_ai_supported_plan, ->(feature = :ai_features) do
        if ::Feature.enabled?(:filter_projects_by_duo_licensed_feature, :instance)
          with_ai_supported_plan_and_credits(feature)
        else
      ...
      scope :with_ai_supported_plan_and_credits, ->(feature = :ai_features) do
        plan_names = ::GitlabSubscriptions::Features.saas_plans_with_feature(feature)
        ...
        result = if ::Gitlab::Llm::StageCheck::CREDITS_ELIGIBLE_FEATURES.include?(feature)
                   ...
                   base.where(plans: { name: plan_names }).or(
                     base.where("EXISTS (?)", credits_matcher)
                   )
                 else
                   base.where(plans: { name: plan_names })
                 end

        result.allow_cross_joins_across_databases(url: "https://gitlab.com/gitlab-org/gitlab/-/issues/419988")
      end
    ...
    def auto_duo_code_review_settings_available?
      ...
      # Duo Enterprise is always available when the add-on is active
      add_ons = [:duo_enterprise]

      # Add Duo Pro/Core/DAP self-hosted if DAP flow available
      add_ons += [:duo_pro, :duo_core, :self_hosted_dap] if duo_code_review_dap_available?
    ...
    def enabled_flow_catalog_item_ids
      own_flows = enabled_foundational_flow_records.limit(::Ai::Catalog::Item::FOUNDATIONAL_FLOWS_LIMIT)
