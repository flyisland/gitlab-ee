...
module InsightsFeature
  ...
  def insights_config
    strong_memoize(:insights_config) { insights_config_validator&.valid_entries }
  end

  def insights_config_valid?
    !!insights_config_validator&.valid?
  end
  ...
  def insights_config_validator
    strong_memoize(:insights_config_validator) do
      ...
      ::Gitlab::Insights::Validators::ConfigValidator.new(raw)
    rescue Gitlab::Config::Loader::FormatError
      nil
    end
  end
