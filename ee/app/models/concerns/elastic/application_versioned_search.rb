...
module Elastic
  module ApplicationVersionedSearch
    ...
    # Returns the object to be tracked in Elasticsearch.
    # Override in models where a different object should be tracked instead.
    def es_tracking_target
      self
    end

    def maintain_elasticsearch_create
      ::Elastic::ProcessBookkeepingService.track!(es_tracking_target)
    end
    ...
    class_methods do
      ...
      def elastic_index_dependant_association(association_name, on_change:, depends_on_finished_migration: nil)
        ...
        # Sidekiq to avoid errors occurring when the job is picked up.
