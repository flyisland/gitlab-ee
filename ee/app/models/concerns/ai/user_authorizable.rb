    ...
    included do
      ...
      def any_group_with_mcp_server_enabled?
        Rails.cache.fetch(
          ...
          root_groups = Group.by_id(gitlab_com_root_group_ids)

          if Feature.enabled?(:mcp_server_availability_setting, :instance)
            root_groups.with_mcp_server_enabled.any?
          else
      ...
      def dap_identity_verification_required?(root_namespace = nil)
      ...
      # rubocop:disable Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity -- this is complex as it is, we need a refactor soon https://gitlab.com/gitlab-org/gitlab/-/work_items/597426.
      def allowed_to_use(
        ai_feature,
        ...
        # Access through GitLab Credits
        gitlab_credits_response = check_gitlab_credits_access(unit_primitive, ai_feature)
        return gitlab_credits_response if gitlab_credits_response

        # DAP Identity Verification gate fires after all paid-access checks so only users
        # falling through to free/trial access are gated.
        return denied_response if feature_requires_identity_verification?(ai_feature, root_namespace)

        # If the user doesn't have access through Duo add-ons or Gitlab credits,
        # and the unit_primitive isn't free, they don't have access
        return denied_response unless unit_primitive.free_access?
      ...
      def feature_requires_identity_verification?(ai_feature, root_namespace)
        ...
        dap_identity_verification_required?(root_namespace)
      end
