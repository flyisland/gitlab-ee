...
module Vulnerabilities
  class Flag < ::SecApplicationRecord
    ...
    def dismissable_status?
      status.to_sym.in?(DISMISSABLE_STATUSES)
    end

    def dismissable_score?
      confidence_score.to_f > 0.0
    end

    def dismissable?(project)
      return dismissable_status? if Feature.enabled?(:vulnerability_flag_status_based_gating, project)

      dismissable_score?
    end

    def triggers_resolution_workflow?(project)
      if Feature.enabled?(:vulnerability_flag_status_based_gating, project)
        status.to_sym.in?(WORKFLOW_TRIGGER_STATUSES)
      else
        confidence_score < ::Vulnerabilities::TriggerResolutionWorkflowWorker::CONFIDENCE_THRESHOLD
      end
    end
    ...
    def should_trigger_resolution_workflow?
      ...
      return false unless triggers_resolution_workflow?(finding.project.root_ancestor)
      ...
      return false unless duo_workflow_user_available?
      ...
      unless Feature.enabled?(:vulnerability_flag_status_based_gating, finding.project.root_ancestor)
        return saved_change_to_confidence_score? || saved_change_to_origin?
      end
    ...
    def duo_workflow_user_available?
      ...
      [vulnerability&.author, project.first_human_owner].compact.any? do |user|
