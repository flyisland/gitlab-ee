...
module Vulnerabilities
  class Feedback < ::SecApplicationRecord
    ...
    scope :with_associations, -> {
      with_partition_aware_preload.includes(:pipeline, :issue, :merge_request, :author, :comment_author)
    }
    ...
    scope :all_preloaded, -> do
      with_partition_aware_preload.preload(:author, :comment_author, :project, :issue, :merge_request, :pipeline)
    end
    ...
    def self.find_or_init_for(feedback_params)
      ...
      return feedback_by_uuid.tap { |feedback| feedback.assign_attributes(feedback_params) } if feedback_by_uuid

      feedback_params.slice(:category, :feedback_type)
                     .merge(finding_uuid: nil)
                     .then { |attrs| find_or_initialize_by(attrs) }
                     .tap { |feedback| feedback.assign_attributes(feedback_params) }
    end
