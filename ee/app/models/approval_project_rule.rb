...
class ApprovalProjectRule < ApplicationRecord
  ...
  def apply_report_approver_rules_to(merge_request, &block)
    rule = Retriable.retriable(on: ActiveRecord::RecordNotUnique, tries: 2) do
  ...
  def update_report_approver_rule_for_merge_request(merge_request, &block)
    ...
    create_violation(rule, merge_request)
  ...
  def delete_approver_rules_and_violations(merge_request)
    if Feature.enabled?(:deprecate_scan_result_policies, project) && approval_policy_rule_id
