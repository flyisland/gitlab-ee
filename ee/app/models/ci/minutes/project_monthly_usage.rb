...
module Ci
  module Minutes
    ...
    class ProjectMonthlyUsage < Ci::ApplicationRecord
      ...
      scope :for_namespace_and_date, ->(namespace, date) do
        all_namespace_ids = namespace.self_and_descendant_ids.ids
        where(
          date: date,
      ...
      scope :for_namespace_monthly_usage, ->(namespace_monthly_usage) do
        for_namespace_and_date(namespace_monthly_usage.namespace, namespace_monthly_usage.date)
      end
