...
module Ci
  module Minutes
    class Quota
      ...
      def previous_amount_used
        if Feature.enabled?(:aggregate_ci_minutes_reads, namespace)
          Ci::Minutes::NamespaceMonthlyUsage.all_previous_usages(namespace).sum(:amount_used)
        else
          Ci::Minutes::NamespaceMonthlyUsage.previous_usage(namespace)&.amount_used.to_i
        end
      end
      strong_memoize_attr :previous_amount_used
