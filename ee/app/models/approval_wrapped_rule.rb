...
class ApprovalWrappedRule
  ...
  def self.wrap(merge_request, rule, context: nil)
    if rule.any_approver?
      ApprovalWrappedAnyApproverRule.new(merge_request, rule, context: context)
    elsif rule.code_owner?
      ApprovalWrappedCodeOwnerRule.new(merge_request, rule, context: context)
    else
      ApprovalWrappedRule.new(merge_request, rule, context: context)
    end
  end

  def initialize(merge_request, approval_rule, context: nil)
  ...
  def overall_approver_ids
    return context.overall_approver_ids if use_context?
    ...
    strong_memoize(:overall_approver_ids) do
      ...
      if current_approvals.is_a?(ActiveRecord::Relation) && !current_approvals.loaded?
        current_approvals.distinct.pluck(:user_id).to_set
      else
