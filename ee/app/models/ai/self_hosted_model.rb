...
module Ai
  class SelfHostedModel < ApplicationRecord
    ...
    # Sets the provider based on identifier prefix until the UI supports provider selection
    # https://gitlab.com/gitlab-org/gitlab/-/work_items/507967
    before_save :set_provider_from_identifier, if: -> { new_record? || saved_change_to_identifier? }
    ...
    def set_provider_from_identifier
      return if self[:identifier].nil?

      self.provider = if identifier.start_with?('bedrock/')
                        :bedrock
                      elsif identifier.start_with?('vertex_ai/')
                        :vertex_ai
                      else
                        :api
                      end
