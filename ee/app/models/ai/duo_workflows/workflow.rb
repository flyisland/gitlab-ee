...
module Ai
  module DuoWorkflows
    class Workflow < ::ApplicationRecord
      ...
      validates :summary, length: { maximum: 1_024 }, allow_blank: true
      ...
      class ToolCallApprovals
        ...
        def approved?(tool_name:, call_args:)
          ...
          return false if COMMAND_TOOL_NAMES.include?(tool_name) && !command_safe_for_pattern_approval?(match_target)
        ...
        def validate_command_tool_pattern!(tool_name, pattern)
          ...
          raise ArgumentError, "Wildcard-only patterns are not allowed for command tools" if pattern == '*'
