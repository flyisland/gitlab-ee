...
module Ai
  module Catalog
    class FoundationalFlow
      ...
      attribute :suppress_mention_progress_note, :boolean, default: false
      ...
      attribute :suppress_agent_session_note, :boolean, default: false
          ...
          [
            {
              ...
              resolve_noteable: ->(project:, goal:) { project.merge_requests.find_by_iid(goal) },
              triggers: []
            },
            {
      ...
      def self.available_for_group(group)
        include_beta = group.duo_beta_flows_enabled?
        include_ultimate = group.licensed_feature_available?(:ai_features)

        all.select do |flow|
          next false if flow.beta? && !include_beta
          next false if flow.ultimate_only? && !include_ultimate

          !flow.blocked_by_feature_flag?(group)
        end
      end
