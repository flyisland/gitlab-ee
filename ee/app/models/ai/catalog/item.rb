  ...
  module Catalog
    class Item < ApplicationRecord
      ...
      class << self
        ...
        def visible_to_user_with_priority_ordering(current_user)
          return public_only if current_user.nil?

          inner_query = build_inner_query_with_access_level(current_user)
          outer_relation = apply_visibility_filter(inner_query)
        ...
        def apply_visibility_filter(inner_query)
          items_table = Arel::Table.new('ai_catalog_items')
          access_level_attr = items_table[:access_level]
          public_attr = items_table[:public]

          unscoped.from("(#{inner_query.to_sql}) ai_catalog_items").where(
            access_level_attr.gteq(MINIMUM_ROLE_READ_PRIVATE_ITEMS)
              .or(public_attr.eq(true))
          )
        end
