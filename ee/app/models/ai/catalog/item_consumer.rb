    ...
    class ItemConsumer < ApplicationRecord
      ...
      class << self
        ...
        def query_with_synthesized_foundational_agents(for_container, item_consumer_attributes)
          ...
          from(foundational_agents_union_query(foundational_consumers))
        end
        ...
        def foundational_agents_union_query(foundational_consumers)
          ...
          <<~SQL
            ((#{foundational_agents_sql}) UNION ALL (SELECT #{column_names.join(', ')} FROM ai_catalog_item_consumers))
      ...
      def validate_item_allowed_by_namespace_restriction
        return if item.nil? || item.foundational? || container.nil?

        return unless root_ancestor.ai_catalog_restricted_to_group_hierarchy &&
          item.project&.root_ancestor != root_ancestor
