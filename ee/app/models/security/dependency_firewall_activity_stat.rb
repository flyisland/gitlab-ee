...
module Security
  ...
  class DependencyFirewallActivityStat < ApplicationRecord
    ...
    # Atomically increments the hourly counter for a (rule, project, outcome) bucket, creating the
    # row on first activity. The bucket is the event time truncated to the hour (UTC). rule_id is
    # nil for "allowed" outcomes that match no rule.
    # NOTE: written synchronously on the enforcement path for now; allowed events are high-volume,
    # so this is intended to be moved behind a Redis buffer + periodic flush later.
    def self.increment!(project_id:, outcome:, rule_id: nil, time: Time.current)
      upsert_all(
        ...
        on_duplicate: Arel.sql('count = dependency_firewall_activity_stats.count + 1, updated_at = now()')
      )
    end
    ...
    # Model mirror of the check_allowed_outcome_has_null_rule DB constraint (increment! relies on the DB).
