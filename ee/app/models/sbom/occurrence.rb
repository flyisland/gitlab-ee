...
module Sbom
  class Occurrence < ::SecApplicationRecord
    ...
    attr_writer :policy_dismissals, :malware_vulnerable
    ...
    scope :for_tracked_context, ->(context) do
      next all if context.nil?

      refs = Sbom::OccurrenceRef.arel_table
      exists_subquery = Sbom::OccurrenceRef
        .by_tracked_context(context.id)
        .where(refs[:sbom_occurrence_id].eq(arel_table[:id]))
        .arel.exists
