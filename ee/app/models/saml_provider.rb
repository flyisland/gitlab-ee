...
class SamlProvider < ApplicationRecord
  ...
  validates :sso_url, presence: true, addressable_url: { schemes: %w[https], ascii_only: true }, if: :enabled?
  validates :certificate_fingerprint, presence: true, certificate_fingerprint: true, if: :enabled?
  ...
  class DefaultOptions
    ...
    def issuer
      unscoped_group_canonical_url(@group_path)
    end
