  ...
  module ProjectPolicy
    ...
    prepended do
      ...
      condition(:can_commit_to_security_policy_project) do
        ...
        Ability.allowed?(@user, :push_code, security_orchestration_policy_configuration.security_policy_management_project)
      end
      ...
      MemberRole.all_customizable_project_permissions.each do |ability|
        ...
        rule { public_send(:"custom_role_enables_#{ability}") }.policy do # rubocop:disable GitlabSecurity/PublicSend -- this is not a security concern because the input is hard coded in a yaml file that we control
      ...
      rule { auditor }.policy do
        enable(*::Authz::Role.get(:auditor).direct_permissions(:project))
      ...
      # Grants modify_security_policy based on push access to the security policy management
      # project, which is a separate project from the current subject.
      rule { security_policy_project_available & can_commit_to_security_policy_project }.policy do
      ...
      rule { security_and_compliance_disabled }.prevent :read_compliance_framework
      ...
      rule { ~agent_registry_enabled }.prevent :write_ai_agents
      ...
      condition(:ai_flow_triggers_enabled, scope: :user) do
        ::Feature.enabled?(:ai_flow_triggers, @user)
      end

      rule { ~ai_flow_triggers_enabled | ~(amazon_q_enabled | duo_workflow_available) }.policy do
