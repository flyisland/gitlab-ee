...
module Search
  class NamespaceIndexIntegrityWorker
    ...
    # New batched processing constants
    BATCH_SIZE = 500
    ...
    def process_namespace_tree(namespace, cursor)
      if Feature.enabled?(:namespace_index_integrity_resumable_processing, :instance)
        process_with_batched_iteration(namespace, cursor)
      else
        process_with_recursive_scheduling(namespace)
      end
    end

    # New behavior: Batched iteration with cursor-based resumption
    def process_with_batched_iteration(namespace, cursor)
    ...
    # Legacy behavior: Recursive scheduling of child namespaces (matches current master)
    def process_with_recursive_scheduling(namespace)
      logger.info(structured_payload(message: 'enqueueing all children for namespace', namespace_id: namespace.id))
      namespace.children.each_batch do |relation|
        relation.each do |child_namespace|
          self.class.perform_in(rand(NAMESPACE_DELAY_INTERVAL), child_namespace.id)
        end
      end

      logger.info(structured_payload(message: 'enqueueing all projects for namespace', namespace_id: namespace.id))
      namespace.projects.each_batch do |relation|
        relation.each do |project|
          next unless project.should_check_index_integrity?

          ::Search::ProjectIndexIntegrityWorker.perform_in(rand(PROJECT_DELAY_INTERVAL), project.id)
        end
      end
    end
