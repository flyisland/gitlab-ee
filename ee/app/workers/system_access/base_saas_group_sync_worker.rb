...
module SystemAccess
  class BaseSaasGroupSyncWorker # rubocop:disable Scalability/IdempotentWorker
    ...
    def update_default_membership
      ...
      access_level_adjustment = calculate_adjusted_access_level(top_level_group, user, default_membership_role)
      member = top_level_group.add_member(
        user, access_level_adjustment.access_level, member_role_id: default_member_role_id
      )

      if member.persisted? && access_level_adjustment.adjusted?
        track_minimal_access_provisioning(top_level_group, user.id)
        audit_minimal_access_role_adjustment(
          top_level_group, member, default_membership_role, { saml_provider_id: top_level_group.saml_provider.id }
        )
      end
