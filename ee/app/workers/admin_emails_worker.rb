...
class AdminEmailsWorker # rubocop:disable Scalability/IdempotentWorker
  ...
  def perform(recipient_id, subject, body)
    recipient_list(recipient_id).without_bots_except_service_accounts.distinct.find_each do |user|
      next unless user.can?(:receive_notifications)

      Notify.send_admin_notification(user.id, subject, body).deliver_later
    end
  end
