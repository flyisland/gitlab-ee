...
module SecretsManagement
  # Provisions a project's secrets manager via a maintenance task. Replaces
  # the legacy `ProvisionProjectSecretsManagerWorker` (which takes
  ...
  # `ProjectSecretsManagerMaintenanceTasksCronWorker` retries it.
  class ProvisionProjectSecretsManagerTaskWorker
    ...
    defer_on_database_health_signal :gitlab_main_org, [:project_secrets_managers], 1.minute
    ...
    def perform(maintenance_task_id)
      ...
      result = ProjectSecretsManagers::ProvisionService.new(secrets_manager, user).execute
