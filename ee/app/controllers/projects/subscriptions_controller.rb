...
class Projects::SubscriptionsController < Projects::ApplicationController
  ...
  def upstream_project
    strong_memoize(:upstream_project) do
      Project.find_by_full_path(upstream_project_path_param)
    end
  end

  def project_subscription
    project.upstream_project_subscriptions.find(id_param)
  end

  def upstream_project_path_param
    params.permit(:upstream_project_path)[:upstream_project_path]
  end
  ...
  def authorize_upstream_project!
    return if can?(current_user, :subscribe_to_upstream_pipeline, upstream_project)
