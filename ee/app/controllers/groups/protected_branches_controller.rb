...
module Groups
  class ProtectedBranchesController < Groups::ApplicationController
    ...
    def load_protected_branch
      @protected_branch = group.protected_branches.find(permitted_params[:id])
    end

    def redirect_to_repository_settings
      redirect_to group_settings_repository_path(group, anchor: permitted_params[:update_section])
    end

    def permitted_params
      params.permit(:id, :update_section)
    end
    ...
    def protected_branch_params(*attrs)
      ...
      attrs << :code_owner_approval_required if group.licensed_feature_available?(:code_owner_approval_required)

      params.require(:protected_branch).permit(attrs)
    end

    def access_level_attributes
      %i[access_level id _destroy member_role_id]
    end
  end
end
