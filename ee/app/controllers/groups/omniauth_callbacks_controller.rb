...
class Groups::OmniauthCallbacksController < OmniauthCallbacksController
  ...
  def group_saml
    @unauthenticated_group = Group.find_by_full_path(permitted_params[:group_id])
  ...
  def permitted_params
    params.permit(:group_id, :RelayState, :SAMLResponse)
  end
  ...
  def safe_relay_state
    valid_gitlab_initiated_saml_request? && safe_redirect_path(permitted_params[:RelayState])
  end
  ...
  def log_saml_response
    ParameterFilters::SamlResponse.log(permitted_params[:SAMLResponse].dup)
  end
end
