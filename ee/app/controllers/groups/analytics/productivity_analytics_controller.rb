...
class Groups::Analytics::ProductivityAnalyticsController < Groups::Analytics::ApplicationController
  ...
  def show
    respond_to do |format|
      format.html
      format.json do
        metric = chart_params.fetch(:metric_type, ProductivityAnalytics::DEFAULT_TYPE)

        data = case chart_params[:chart_type]
               when 'scatterplot'
  ...
  def chart_params
    params.permit(:metric_type, :chart_type)
  end

  def paginate(merge_requests)
    merge_requests.page(pagination_params[:page]).per(pagination_params[:per_page]).tap do |paginated_data|
  ...
  def productivity_analytics
    @productivity_analytics ||= ProductivityAnalytics.new(
      merge_requests: finder.execute, sort: pagination_params[:sort]
    )
  end
