...
module Groups
  class RoadmapController < Groups::ApplicationController
    ...
    def roadmap_params
      params.permit(:layout, :state)
    end

    def persist_roadmap_layout
      return unless current_user

      roadmap_layout = roadmap_params[:layout]&.downcase
    ...
    def epics_state_in_user_preference
      ...
      state = roadmap_params[:state]

      if state.present?
        preference.roadmap_epics_state = Epic.available_states[state]
