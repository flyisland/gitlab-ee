...
class Groups::IterationsController < Groups::ApplicationController
  ...
  def show
    redirect_to group_iteration_cadence_iteration_path(iteration_cadence_id: cadence_id, id: iteration_id_param)
  end
  ...
  def edit
    redirect_to edit_group_iteration_cadence_iteration_path(iteration_cadence_id: cadence_id, id: iteration_id_param)
  end

  private

  def iteration_id_param
    params.permit(:id)[:id]
  end

  def set_iteration!
    @iteration ||= IterationsFinder
      .new(current_user, id: iteration_id_param, parent: group, include_ancestors: true)
      .execute
      .first
