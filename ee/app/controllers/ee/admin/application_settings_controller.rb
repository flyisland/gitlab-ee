  ...
  module Admin
    module ApplicationSettingsController
      ...
      prepended do
        ...
        before_action :set_hide_search_settings, only: [:analytics, :work_item, :security_and_compliance]
        ...
        def semantic_search_available
          # Semantic search is only available if:
          # - instance setting and license allows AI features (ActiveContext.semantic_search_available?)
          # - AND
          #   - instance uses a Gitlab-selected model (ModelSelector.use_gitlab_selected_model?)
          #   - OR instance allows user to select the model (ModelSelector.user_can_select_model?)
          #
          # TODO: remove the model selector checks once we remove the FF `semantic_search_user_model_selection`
          # `ModelSelector.use_gitlab_selected_model?` and `ModelSelector.user_can_select_model?`
          # are ideally inverses of each other. However, `user_can_select_model?` is still gated behind the
          # Feature Flag, so there is currently a scenario where both returns `false`
          @semantic_search_available = ::Ai::ActiveContext.semantic_search_available? &&
            (::Ai::ActiveContext::Embeddings::ModelSelector.use_gitlab_selected_model? ||
              ::Ai::ActiveContext::Embeddings::ModelSelector.user_can_select_model?)
        end
      ...
      def visible_application_setting_attributes
        ...
        if ::Feature.enabled?(:use_built_in_project_templates_enabled, :instance) &&
