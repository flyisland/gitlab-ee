  ...
  module OmniauthCallbacksController
    ...
    def perform_registration_tasks(user, provider)
      ...
      if provider.to_sym.in?(::AuthHelper.providers_for_base_controller)
        ...
        # Under unified registration we keep stored_location_for(:user) pointing at
        # the original CDot URL so CustomersPortalRedirectController can parse its
        # purchase params, but the post-verification redirect should land on the
        # interstitial. stored_location_for(:redirect) takes precedence in
        # ApplicationController#after_sign_in_path_for, so set it explicitly here
        # (the email signup flow does this in RegistrationsController#after_inactive_sign_up_path_for).
        store_location_for(:redirect, after_sign_up_path) if onboarding_status_presenter.unification_enabled?
      else
        super
      end
    end
