  ...
  module Repositories
    module GitHttpClientController
      ...
      def geo_route_helper
        @geo_route_helper ||= GeoRouteHelper.new(
          project, controller_name, action_name, params.permit(:service)[:service], synchronous_request_required?
        )
      end

      def geo_git_lfs_params
        # operation explained: https://github.com/git-lfs/git-lfs/blob/master/docs/api/batch.md#requests
        params.permit(:operation, :oid, objects: [:oid])
      end
      ...
      def geo_git_lfs_helper
        @geo_git_lfs_helper ||= GeoGitLFSHelper.new(
          project,
          geo_route_helper,
          geo_git_lfs_params[:operation],
          request.headers['User-Agent'],
          geo_git_lfs_params[:objects],
          geo_git_lfs_params[:oid]
        )
      end
