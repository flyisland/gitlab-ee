    ...
    module Settings
      module RepositoryController
        ...
        def push_rule
          ...
          # WRITE: will be handled in a followup MR.
          unless project.push_rule
            push_rule = project.create_push_rule
            project.project_setting.update(push_rule_id: push_rule.id)
          end

          # READ: even though writing and reading happen from a different table(push_rules vs project_push_rules)
          # that's ok because the trigger ensures that anything written to push_rules immediately appears in project_push_rules.
          @push_rule = ProjectPushRuleFinder.new(project).execute
        end
