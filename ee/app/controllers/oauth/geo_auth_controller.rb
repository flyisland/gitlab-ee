...
class Oauth::GeoAuthController < BaseActionController
  ...
  def auth
    ...
    redirect_to oauth.authorize_url(redirect_uri: oauth_geo_callback_url, state: permitted_params[:state])
  end
  ...
  def callback
    ...
    token = oauth.get_token(permitted_params[:code], redirect_uri: oauth_geo_callback_url)
  ...
  def logout
    token = Gitlab::Geo::Oauth::LogoutToken.new(current_user, permitted_params[:state])
  ...
  def login_state
    Gitlab::Geo::Oauth::LoginState.from_state(permitted_params[:state])
  end
