...
class OperationsController < ApplicationController
  ...
  def create
    respond_to do |format|
      format.json do
        result = add_projects(permitted_params[:project_ids])
  ...
  # Used by Operations and Environments dashboard.
  def destroy
    if remove_project(permitted_params[:project_id])
      head :ok
    else
      head :no_content
    end
  ...
  def permitted_params
    params.permit(:project_id, project_ids: [])
  end
  ...
  def load_projects
    Dashboard::Operations::ListService.new(current_user, organization: Current.organization).execute
  end

  def load_environments_projects
    Dashboard::Environments::ListService.new(current_user, organization: Current.organization).execute
  end
