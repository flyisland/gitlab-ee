  ...
  module Ci
    module BuildRunnerPresenter
      ...
      def project_secrets_manager_available?
        ::SecretsManagement::Availability.for_project?(project) &&
          secrets_manager_entitlement_permits_direct_read?(project.root_ancestor)
      end
      ...
      def group_secrets_manager_available?(group)
        strong_memoize_with(:group_secrets_manager_available, group.id) do
          ::SecretsManagement::Availability.for_group?(group) &&
            secrets_manager_entitlement_permits_direct_read?(group.root_ancestor)
        end
      end

      # Returns `true` (access permitted) when the `secrets_manager_paid_experience` flag is
      # disabled; otherwise delegates to `Entitlement#permits_direct_read?`, which is stricter
      # than the UI/API's `permits_read?` -- only the `grace` window permits CI secret reads.
      # Entitlement-driven suppression (not the `Availability` checks) also emits denial
      # telemetry; `grace` never appears as a denial reason here because it permits direct reads.
      def secrets_manager_entitlement_permits_direct_read?(root_namespace)
        return true unless ::Feature.enabled?(:secrets_manager_paid_experience, root_namespace)

        entitlement_namespace = ::SecretsManagement::Entitlement.root_namespace_for(root_namespace)
        entitlement = ::SecretsManagement::Entitlement.for(entitlement_namespace)
        return true if entitlement.permits_direct_read?
