    ...
    module BuildRunnerPresenter
      ...
      def project_secrets_manager_available?
        ::SecretsManagement::Availability.for_project?(project) &&
          secrets_manager_entitlement_permits_ci_read?(project.root_ancestor)
      end
      ...
      def group_secrets_manager_available?(group)
        strong_memoize_with(:group_secrets_manager_available, group.id) do
          ::SecretsManagement::Availability.for_group?(group) &&
            secrets_manager_entitlement_permits_ci_read?(group.root_ancestor)
        end
      end

      # Returns `true` (access permitted) when the `secrets_manager_paid_experience` flag is
      # disabled; otherwise delegates to `Entitlement#permits_ci_read?`, which is stricter than
      # the UI/API's `permits_read?` -- only the `grace` window permits CI secret reads.
      def secrets_manager_entitlement_permits_ci_read?(root_namespace)
        ...
        ::SecretsManagement::Entitlement.for(entitlement_namespace).permits_ci_read?
      end
