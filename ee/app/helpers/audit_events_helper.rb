...
module AuditEventsHelper
  ...
  def audit_events_details_drawer_enabled?(actor)
    Feature.enabled?(:audit_events_details_drawer, actor)
  end
  ...
  def audit_log_app_data(is_last_page, events, audit_event_definitions)
    {
      ...
      data.merge!({
        ...
        audit_events_details_drawer_enabled: audit_events_details_drawer_enabled?(current_user).to_s
      })
    end
  end
end
