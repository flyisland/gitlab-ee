...
module BillingPlansHelper
  ...
  DEFAULT_PREMIUM_PRICE = 29
  DEFAULT_ULTIMATE_PRICE = 99
  ...
  def free_trial_upgrade_subscription_billing_attributes(namespace, monthly_commitment_purchased: 0)
    total_seats =
...

    {
      ...
      purchaseCreditsTrackingUrl: track_cart_abandonment_gitlab_subscriptions_hand_raise_leads_path(
  ...
  def upgrade_subscription_data_attributes(namespace, plans_data, monthly_commitment_purchased: 0)
    return {} unless namespace && plans_data

    premium_plan = find_plan(plans_data, ::Plan::PREMIUM)
    ultimate_plan = find_plan(plans_data, ::Plan::ULTIMATE)

    {
      premiumPlanPurchaseLink: premium_plan ? plan_purchase_url(namespace, premium_plan) : nil,
      premiumPricePerMonth: premium_plan&.price_per_month || DEFAULT_PREMIUM_PRICE,
      ultimatePlanPurchaseLink: ultimate_plan ? plan_purchase_url(namespace, ultimate_plan) : nil,
      ultimatePricePerMonth: ultimate_plan&.price_per_month || DEFAULT_ULTIMATE_PRICE,
