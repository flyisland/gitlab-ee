...
module Users
  module IdentityVerificationHelper
    ...
    def signup_identity_verification_data(user, unified_registration: false)
      data = build_data_hash(user, path_helper: method(:signup_iv_action_path))
        .merge(unified_registration: unified_registration)

      { data: Gitlab::Json.generate(data) }
    end

    def identity_verification_data(user)
      { data: Gitlab::Json.generate(build_data_hash(user, path_helper: method(:iv_action_path))) }
    end
    ...
    def build_data_hash(user, path_helper:)
      {
