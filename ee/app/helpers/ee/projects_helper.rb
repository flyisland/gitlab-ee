...
module EE
  module ProjectsHelper
    ...
    def group_project_templates_count(group_id)
      if ::Feature.enabled?(:constrain_group_project_templates, current_user)
        ::Projects::GroupTemplatesFinder.new(current_user, group_id).execute.count
      elsif ::Feature.enabled?(:project_templates_without_min_access, current_user)
    ...
    def show_duo_otel_button?(project, current_user)
      ...
      return false unless project.repository_languages.present?
      return false unless can?(current_user, :push_code, project)
      return false unless ::Feature.enabled?(:duo_add_otel, project)
      return false unless project.duo_remote_flows_enabled
      return false unless project.duo_features_enabled
      return false unless project.licensed_ai_features_available?
