    ...
    module SettingsHelper
      ...
      def usage_billing_dashboard_data(group, plans_data)
...

        {
          ...
          gitlab_com_purchase_credits_path: subscription_portal_gitlab_com_purchase_credits_url(group.id)
        }
      end
      ...
      def duo_workflow_settings_data
        {
          ...
          duo_workflow_mcp_available: (@group.root? && current_user.can?(:admin_duo_workflow, @group) && @group.licensed_feature_available?(:ai_features)).to_s,
      ...
      def available_foundational_flows_json
        ...
        foundational_flows =
          if allow_beta_experimental_ai_features?
            ::Ai::Catalog::FoundationalFlow.all
          else
            ::Ai::Catalog::FoundationalFlow.ga
          end

        unless @group.licensed_feature_available?(:ai_features)
          foundational_flows = foundational_flows.reject(&:ultimate_only)
        end
