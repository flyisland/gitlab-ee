  ...
  class ChatComponent < ViewComponent::Base
    ...
    def scope_data
      {
        ...
        root_namespace_id: ::Gitlab::Llm::TanukiBot.root_namespace_id(container),
        resource_id: ::Gitlab::Llm::TanukiBot.resource_id,
        metadata: ::Gitlab::DuoWorkflow::Client.metadata(user, namespace: container&.root_ancestor).to_json
      }
    end
    ...
    def tanuki_bot_data
...

      {
        ...
        credits_available: tanuki_bot.credits_available?.to_s,
    ...
    def chat_feature_data
      {
        ...
        explore_ai_catalog_path: helpers.explore_ai_catalog_path,
    ...
    def billing_attributes
      return billing_attributes_for_self_managed unless saas?

      billing_attributes_for_saas(governing_namespace)
    end
    ...
    def billing_attributes_for_self_managed
      can_buy = Ability.allowed?(user, :admin_all_resources)
      is_trial = License.current&.trial?

      {
        is_trial: is_trial.to_s,
        can_buy_addon: can_buy.to_s,
        buy_addon_path: (buy_addon_path_for_self_managed(is_trial) if can_buy)
      }
    end

    def billing_attributes_for_saas(namespace)
      can_buy = !!(namespace && Ability.allowed?(user, :edit_billing, namespace))
      is_trial = !!namespace&.trial_active?

      {
        is_trial: is_trial.to_s,
        can_buy_addon: can_buy.to_s,
        buy_addon_path: (buy_addon_path_for_saas(namespace, is_trial) if can_buy)
      }
    end

    def buy_addon_path_for_self_managed(is_trial)
      return ::Gitlab::Routing.url_helpers.subscription_portal_url if is_trial

      helpers.admin_gitlab_credits_dashboard_index_path
    end

    def buy_addon_path_for_saas(namespace, is_trial)
      return ::Gitlab::Routing.url_helpers.subscription_portal_url if is_trial
