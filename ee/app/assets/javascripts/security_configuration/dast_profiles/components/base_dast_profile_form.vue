...
export default {
  ...
  mixins: [dastProfileConfiguratorMixin(), glSlotsMixin],
  props: {
    mutation: {
      type: Object,
      required: true,
  ...
  emits: ['cancel', 'error', 'open-drawer', 'submit', 'success'],
  data() {
    return {
      isLoading: false,
      showAlert: false,
...
<template>
  <gl-form novalidate @submit.prevent="onSubmit">
    ...
    <gl-alert
      ...
      :dismissible="false"
    >
      <template v-if="glSlots()['policy-profile-notice']" #default
