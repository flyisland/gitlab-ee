...
export default {
  ...
  created() {
    // Temporarily migrate localstorage value for manual collapse key to crud component collapse key
    const isHidden = localStorage.getItem(this.$options.anchorId) === '1';
    if (isHidden) {
      localStorage.setItem(`crud-collapse-${this.$options.anchorId}`, isHidden);
    }
    localStorage.removeItem(this.$options.anchorId);
  },
  methods: {
