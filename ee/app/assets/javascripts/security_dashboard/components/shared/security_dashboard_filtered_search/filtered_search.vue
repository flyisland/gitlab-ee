...
export default {
  ...
  methods: {
    buildInitialValue(token) {
      ...
      let data;
      if (urlValues) {
        data = token.token?.parseQueryParams?.(urlValues) ?? urlValues;
      } else {
        data = this.getDefaultValues(token.token);
      }
    ...
    getDefaultValues(tokenComponent) {
      return tokenComponent.defaultValues?.(this.context) ?? [];
    },
    ...
    getTokenComponent(type) {
      return this.tokens.find((token) => token.type === type)?.token;
    },
    ...
    transformQueryParams(type, data = []) {
      const token = this.getTokenComponent(type);
      if (isEqual(data, this.getDefaultValues(token))) return undefined;
