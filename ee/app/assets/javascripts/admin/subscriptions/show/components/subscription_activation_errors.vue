...
export const testIds = Object.freeze({
  ...
  EXPIRED_TRIAL_ERROR_ALERT: 'EXPIRED_TRIAL_ERROR_ALERT',
});
...
export const i18n = Object.freeze({
  ...
  EXPIRED_TRIAL_ERROR_TITLE: s__('SuperSonics|Your trial is expired'),
  EXPIRED_TRIAL_ERROR_MESSAGE: s__(
    'SuperSonics|You can %{purchaseSubscriptionLinkStart}purchase a new subscription%{purchaseSubscriptionLinkEnd} and try again. If you need further assistance, please %{supportLinkStart}contact GitLab Support%{supportLinkEnd}.',
  ),
...
export default {
  ...
  computed: {
    ...
    hasExpiredTrialError() {
      return this.error === EXPIRED_TRIAL_SERVER_ERROR;
    },
    hasInvalidCodeError() {
...
<template>
  <div v-if="hasError" :data-testid="$options.testIds.SUBSCRIPTION_ACTIVATION_ROOT">
    ...
    <gl-alert
      v-if="hasExpiredTrialError"
      variant="danger"
      :title="$options.i18n.EXPIRED_TRIAL_ERROR_TITLE"
      :dismissible="false"
      :data-testid="$options.testIds.EXPIRED_TRIAL_ERROR_ALERT"
    >
      <gl-sprintf :message="$options.i18n.EXPIRED_TRIAL_ERROR_MESSAGE">
        <template #purchaseSubscriptionLink="{ content }">
          <promo-page-link
            :path="$options.links.purchaseSubscriptionPath"
            target="_blank"
