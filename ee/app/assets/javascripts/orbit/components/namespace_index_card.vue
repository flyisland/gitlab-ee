...
export default defineComponent({
  ...
  computed: {
    ...
    relationshipCount() {
      const edgeCounts = this.graphStatus?.edge_counts;
      if (!edgeCounts) return null;

      return Object.values(edgeCounts).reduce((sum, v) => sum + (v || 0), 0);
    },
    projectIndexDisplay() {
  ...
  methods: {
    async loadStatus({ silent = false } = {}) {
      ...
      try {
        const { data } = await fetchGraphStatus(path);
        ...
        <div class="gl-flex gl-flex-1 gl-items-center gl-justify-between">
          <div class="gl-flex gl-flex-wrap gl-items-center gl-gap-4">
            ...
            <template v-if="!statusLoading">
              ...
              <span v-if="relationshipCount !== null" class="gl-text-sm gl-text-subtle">
                {{ relationshipCount.toLocaleString() }} {{ s__('Orbit|relationships') }}
              </span>
          ...
          <div class="gl-mb-4 gl-flex gl-items-center gl-gap-5">
            ...
            <div class="gl-flex gl-flex-wrap gl-gap-5 gl-text-sm gl-text-subtle">
              <template v-if="hasFilter">
                ...
                <span v-if="relationshipCount !== null">
                  {{ relationshipCount.toLocaleString() }} {{ s__('Orbit|relationships') }}
                </span>
