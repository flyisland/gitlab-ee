...
const ERROR_MSG = s__('Orbit|Failed to load code intelligence data.');
...
export default {
  ...
  methods: {
    ...
    // KG stores 0-indexed lines; GitLab blob anchors are 1-indexed.
    // startLine comes back as a string from the API, so cast before adding.
    blobUrl(filePath, startLine) {
      if (!this.projectPath || !this.currentRef || filePath == null) return null;
      const line = Number(startLine ?? 0) + 1;
    ...
    lineLabel(startLine) {
      return sprintf(LINE_LABEL_TEMPLATE, { line: Number(startLine ?? 0) + 1 });
    },
  },
};
