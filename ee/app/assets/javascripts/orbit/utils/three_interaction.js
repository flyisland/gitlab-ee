...
import { createArcCurve, createStraightLine } from './three_edges';
...
export default class GraphInteraction {
  ...
  /** Returns true if the node is on the camera-facing hemisphere (or in 2D mode). */
  isNodeVisible(node) {
    if (!node.position) return false;
    if (this.viewMode === '2d') return true;
  ...
  moveNode(node, newPos, { nodeGeometry, nodeLabelsGroup, connections }) {
    ...
    connections.forEach((conn) => {
      ...
      const curve =
        this.viewMode === '2d'
          ? createStraightLine(startPos, endPos)
          : createArcCurve(startPos, endPos);
