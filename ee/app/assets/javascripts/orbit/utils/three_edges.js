...
import { IMPULSE_COLOR } from './three_nodes';
...
const IMPULSE_COLOR_LIGHT = new THREE.Color(0.6, 0.5, 0.85);

/** Creates an animated point-sprite particle that travels along an edge curve. */
export function createImpulse(connection, renderer, darkMode = true) {
  const geometry = new THREE.BufferGeometry();
  geometry.setAttribute('position', new THREE.Float32BufferAttribute([0, 0, 0], 3));

  const impulseColor = darkMode ? IMPULSE_COLOR : IMPULSE_COLOR_LIGHT;
  const material = new THREE.ShaderMaterial({
    uniforms: {
      color: { value: impulseColor.clone() },
