// Creates the globe mesh, atmospheric glow, and decorative city-light particles.
...
const SPHERE_SEGMENTS = 64; // tessellation detail for globe geometry
const ATMOSPHERE_SCALE = 1.02; // atmosphere shell slightly larger than globe
const CITY_LIGHT_OFFSET = 1.001; // lights sit just above the globe surface
const CITY_LIGHT_MIN_SIZE = 0.015;
const CITY_LIGHT_MAX_SIZE = 0.04;

// Globe surface colors
const GLOBE_COLOR_DARK = 0x0a0a12;
const GLOBE_COLOR_LIGHT = 0xe8e8ee;

// City light warmth color channel multipliers
...
export function createGlobe(globeGroup, darkMode) {
  ...
  const earthColor = darkMode ? GLOBE_COLOR_DARK : GLOBE_COLOR_LIGHT;
  const earthMaterial = new THREE.MeshBasicMaterial({ color: earthColor });
...
/** Creates decorative point-light particles scattered across the globe surface (dark mode only). */
