...
const D3_LINK_DISTANCE = 4;
const D3_CHARGE_SMALL = -120;
const D3_CHARGE_LARGE = -200;
const D3_SMALL_GRAPH_THRESHOLD = 15;
const D3_SIMULATION_TICKS = 300;
const D3_COLLIDE_RADIUS = 1.5;

// 2D target extent: fits comfortably within camera at default zoom
const FLAT_TARGET_RADIUS = 12;
...
function normalizeToRadius(nodes, targetRadius) {
  ...
  const scale = targetRadius / maxExtent;
  for (const n of nodes) {
    if (n.position) {
      n.position.x *= scale;
      n.position.y *= scale;
...
/**
 * Positions nodes for 2D view using d3-force simulation, then normalizes
 * to fit the target camera viewport.
 * @mutates nodes — positions are set in place for performance.
 */
export function computeFlatLayout(nodes, edges) {
  ...
  const simNodes = nodes.map((n, i) => ({
    index: i,
    x: (Math.random() - 0.5) * FLAT_TARGET_RADIUS * 2,
    y: (Math.random() - 0.5) * FLAT_TARGET_RADIUS * 2,
  }));
  ...
  for (let i = 0; i < simNodes.length; i += 1) {
    nodes[i].position = { x: simNodes[i].x, y: simNodes[i].y, z: 0 }; // eslint-disable-line no-param-reassign -- mutates in-place by design
  }
