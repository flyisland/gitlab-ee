...
import GroupTemplatesGroupSelector from './components/group_templates_group_selector.vue';
...
const initGroupTemplatesGroupSelector = () => {
  const el = document.querySelector('.js-group-templates-group-selector');
  ...
  return initVueApp({
    el,
    name: 'GroupTemplatesGroupSelectorRoot',
    ...
    component: GroupTemplatesGroupSelector,
  });
};
...
export default () => {
  ...
  if (tabContent?.classList.contains('active') && tabContent.dataset.initialTemplates) {
    fetchHtmlForTabContent(tabContent);
  } else {
    navElement?.addEventListener('click', () => fetchHtmlForTabContent(tabContent), { once: true });
  }

  if (groupTabContent?.dataset.initialTemplates) {
    if (groupTabContent.classList.contains('active')) {
      fetchHtmlForTabContent(groupTabContent);
    } else {
  ...
  initGroupTemplatesGroupSelector();
  bindEvents();
};
