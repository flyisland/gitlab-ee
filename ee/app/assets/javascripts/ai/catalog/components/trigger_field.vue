...
import { mergeRequestActionLabels, pipelineStatusLabels } from 'ee/ai/duo_agents_platform/utils';
import {
...
export default {
  ...
  methods: {
    triggerName(eventType) {
      ...
      const labels = TOKEN_LABEL_BUILDERS[eventType]?.(this.flowTrigger?.filter) ?? [];
