...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      const payload = { target: { projectId: this.projectId } };

      if (this.showTriggers) {
        payload.triggerTypes = this.triggerTypes;
        if (Object.keys(this.triggerFilter).length > 0) {
          payload.triggerFilter = this.triggerFilter;
        }
      }
