...
import aiCatalogGroupUserPermissionsQuery from '../graphql/queries/ai_catalog_group_user_permissions.query.graphql';
...
export default {
  ...
  computed: {
    canAdmin() {
      return Boolean(this.item.userPermissions?.adminAiCatalogItem) && !this.isGroupNamespace;
    },
    canAdminConsumer() {
      const userPermissions = this.isProjectNamespace
        ? this.projectUserPermissions
        : this.groupUserPermissions;
      return Boolean(userPermissions?.adminAiCatalogItemConsumer);
    },
    canReport() {
      return Boolean(this.item.userPermissions?.reportAiCatalogItem && !this.item.foundational);
    },
    canHardDelete() {
    ...
    canEnable() {
      if (this.isGlobalNamespace) {
        return this.projectsIsUserMaintainer?.length > 0;
      }
      return this.canAdminConsumer;
    },
    isEnabled() {
      return this.isProjectNamespace
        ? this.item.configurationForProject?.enabled
        : this.item.configurationForGroup?.enabled;
    },
    showDisable() {
    ...
    showDropdown() {
      return this.showDisable || this.showDuplicate || this.canAdmin || this.canReport;
    },
    showEnableFromGlobal() {
      return (
        this.canUse &&
        this.isGlobalNamespace &&
        (!this.item.foundational || this.item.itemType === AI_CATALOG_TYPE_THIRD_PARTY_FLOW)
      );
    },
    showEnableButton() {
