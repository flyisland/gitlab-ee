...
export default {
  ...
  apollo: {
    items: {
      ...
      update(data) {
        return this.getNestedData(data, this.dataKey) || [];
      },
      result() {
  ...
  methods: {
    ...
    setNestedNodes(obj, path, nodes) {
      const keys = path.split('.');
      const lastKey = keys.pop();
      const target = keys.reduce((acc, key) => acc[key], obj);
      target[lastKey].nodes = nodes;
    },
    itemText(item) {
        ...
        await this.$apollo.queries.items.fetchMore({
          ...
          updateQuery: (previousResult, { fetchMoreResult }) => {
            return produce(fetchMoreResult, (draftData) => {
              const prevNodes = this.getNestedData(previousResult, this.dataKey)?.nodes || [];
              const newNodes = this.getNestedData(draftData, this.dataKey)?.nodes || [];
              this.setNestedNodes(draftData, this.dataKey, [...prevNodes, ...newNodes]);
            });
          },
        });
      } catch (error) {
