...
export default {
  ...
  computed: {
    activeVersion() {
      return resolveVersion(this.aiCatalogAgent, { isGlobalNamespace: this.isGlobalNamespace });
    },
    initialValues() {
      return {
        ...
        mcpTools: this.activeVersion.mcpTools ?? [],
        mcpServers: (this.activeVersion.mcpServers?.nodes ?? []).map((s) => s.id),
    ...
    canDuplicate() {
      return canDuplicateAiCatalogItem(this.aiCatalogAgent, {
