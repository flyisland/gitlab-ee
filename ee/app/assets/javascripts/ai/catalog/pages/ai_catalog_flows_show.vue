...
export default {
  ...
  computed: {
    showBetaBadge() {
      return !this.aiCatalogFlow.foundational;
    },
    formattedItemId() {
  ...
  methods: {
    ...
    showEnableToast({ name, href }) {
      this.setErrors();
      const message = sprintf(s__('AICatalog|Flow enabled in %{name}.'), { name });
      if (href) {
    ...
    async addFlowToTarget({ target, triggerTypes }) {
      ...
      try {
        ...
        this.showEnableToast({
          ...
          href: showViewLink
            ? `${targetData.webUrl}/-/automate/flows/${this.formattedItemId}`
            : null,
        });
      } catch (error) {
