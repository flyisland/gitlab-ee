...
export default {
  ...
  computed: {
    ...
    initialValues() {
      return {
        ...
        mcpTools: this.aiCatalogAgent.latestVersion.mcpTools ?? [],
        mcpServers: (this.aiCatalogAgent.latestVersion.mcpServers?.nodes ?? []).map((s) => s.id),
  ...
  created() {
    if (!canEditAiCatalogItem(this.aiCatalogAgent)) {
      this.$router.push({
