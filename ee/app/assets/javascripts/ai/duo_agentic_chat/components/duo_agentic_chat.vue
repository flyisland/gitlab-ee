...
import {
  ...
  DUO_WORKFLOW_ARCHIVED_CHAT_REASON,
...
import { resetThreadContent, isThreadExpired } from '../utils/thread_utils';
...
import ThreadExpiredEmptyState from './thread_expired_empty_state.vue';
...
export default {
  ...
  apollo: {
    ...
    availableModels: {
      ...
      fetchPolicy: fetchPolicies.NETWORK_ONLY,
      skip() {
        if (this.isDuoDisabled) return true;
  ...
  computed: {
      ...
      return [
        {
          ...
          matchMessage: (message) => {
            ...
            try {
              ...
              captureException(new Error(`Malformed start_flow tool response ${content}`), {
    ...
    // TODO: I believe this can be removed. We don't have non-embedded case anymore.
    dimensions() {
      if (!this.isEmbedded) {
        return {};
      }
    ...
    shouldShowActiveTrialOrSubscriptionEmptyState() {
      return this.hasTrialOrSubscription && !this.currentAgent && !this.isSelectedThreadExpired;
    },
  ...
  created() {
    if (this.isDuoDisabled) {
      this.setChatState({ isEnabled: false, reason: s__('DuoAgenticChat|GitLab Duo is disabled') });
    }
  },
  mounted() {
    if (this.isDuoDisabled) return;
  ...
  methods: {
        ...
        if (workflowData) {
          ...
          if (workflowData.messages && workflowData.messages.length > 0) {
            workflowData.messages.forEach((msg) => {
              ...
              EventsTracker.trackMessage({ message: msg });
            });
          }
    ...
    async onSendChatPrompt({ question, triggerSource = TRIGGER_SOURCE_WEB_CHAT } = {}) {
      ...
      this.startWorkflow(question, {}, contextData);
    ...
    async onThreadSelected(thread) {
      this.isSelectedThreadExpired = false;

      if (isThreadExpired(thread.lastUpdatedAt)) {
        this.isSelectedThreadExpired = true;
    ...
    async hydrateActiveWorkflow() {
      ...
      if (this.workflowId) {
        const thread = this.agenticWorkflows.find((w) => w.id === this.workflowId);
        if (thread && isThreadExpired(thread.lastUpdatedAt)) {
          this.isSelectedThreadExpired = true;
          return;
        }
      }
    ...
    async loadActiveWorkflow() {
      try {
        ...
        if (workflow?.archived) {
          this.setChatState({
            isEnabled: false,
            reason: DUO_WORKFLOW_ARCHIVED_CHAT_REASON,
          });
        }
    ...
    async onNewChat(reuseAgent) {
      ...
      // TODO: I believe we don't need this condition here just keep emit. We don't have non-embedded case anymore.
      if (this.isEmbedded) {
        this.$emit('switch-to-active-tab', DUO_CHAT_VIEWS.CHAT);
      }
    ...
    >
      ...
      <template v-if="shouldShowCustomEmptyState" #custom-empty-state>
        <duo-disabled-empty-state
          ...
          :duo-settings-path="chatConfiguration.defaultProps.duoSettingsPath"
          :namespace-type="projectId ? 'project' : 'group'"
        />
        ...
        <active-trial-or-subscription-empty-state
          ...
          @send-chat-prompt="onSendChatPrompt({ question: $event })"
        />
        <thread-expired-empty-state
          v-else-if="isSelectedThreadExpired"
          key="thread-expired-empty-state"
          @back-to-list="onBackToList"
          @new-chat="onNewChat"
        />
      </template>
