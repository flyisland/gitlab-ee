...
export default {
  ...
  computed: {
    ...
    gitlabManagedModels() {
      ...
      const formattedDefaultGitLabModel = defaultGitlabModel
        ? formatDefaultModelData(defaultGitlabModel)
        : undefined;

      if (this.isLoadBalancedDefaultModelsEnabled) {
        return validGitlabModels.map(
          ({ name, ref, modelProvider, modelDescription, costIndicator }) => {
            const isDefaultModel = ref === defaultGitlabModel?.ref;

            return {
              value: isDefaultModel ? formattedDefaultGitLabModel?.value : ref,
              text: isDefaultModel ? formattedDefaultGitLabModel?.text : name,
