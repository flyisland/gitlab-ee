 ...
 * @param {Object} vulnerabilityAttributes
 * @returns {String|null}
 */
...
const humanizeRule = (rule) => {
  ...
  const { securityPoliciesKevFilter } = window.gon?.features || {};
  const hasObjectScanners = securityPoliciesKevFilter && rule.scanners?.some((s) => isObject(s));
