...
export default {
  ...
  methods: {
    ...
    showStartDryRunAction(type) {
      return (
        this.glFeatures.securityPoliciesSpepDryRun &&
        this.isPipelineExecutionSchedulePolicy(type) &&
        !this.isGroup
      );
    },
    ...
    showTestRunBadge(type) {
      return (
        this.glFeatures.securityPoliciesSpepDryRun && this.isPipelineExecutionSchedulePolicy(type)
      );
    },
...
<template>
  <div>
    ...
    >
      <template
        #cell(status)="{
          item: { enabled, name, deprecatedProperties, policyType, yaml, testRuns, type },
        }"
      >
        <div class="gl-flex gl-flex-col gl-gap-2">
          ...
          <test-run-status-badge v-if="showTestRunBadge(type)" :test-runs="testRuns" />
        </div>
      </template>
