...
export default {
  ...
  emits: ['vulnerability-state-change'],
  data() {
    return {
  ...
  computed: {
    ...
    shouldShowSolution() {
      if (!this.glFeatures.vulnerabilityMalwareDetection) {
        return true;
      }
      return !this.vulnerability.malware;
    },
    solutionHtml() {
      return this.vulnerability.solutionHtml || null;
    },
    solutionText() {
      const { solution, remediations } = this.vulnerability;
      return solution || remediations?.[0]?.summary || null;
    },
    canDownloadPatch() {
      return (
        !this.mergeRequest &&
...
<template>
  <div data-testid="vulnerability-footer">
    <solution-card
      v-if="(solutionHtml || solutionText) && shouldShowSolution"
      class="md gl-my-6"
    ...
    <div v-else-if="discussions.length">
      <history-entry
        ...
        @comment-updated="fetchDiscussions"
      />
    </div>
  </div>
</template>
