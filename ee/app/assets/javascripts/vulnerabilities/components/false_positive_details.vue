...
export default {
  ...
  computed: {
    ...
    statusBasedGating() {
      return this.glFeatures.vulnerabilityFlagStatusBasedGating;
    },
    isPending() {
      return this.statusBasedGating && isPendingFpDetection(this.vulnerability.latestFlag);
    },
    isCompleted() {
      return this.statusBasedGating && isCompletedFpDetection(this.vulnerability.latestFlag);
    },
    isFailed() {
      return this.statusBasedGating && isFailedFpDetection(this.vulnerability.latestFlag);
    },
    canDismissFalsePositive() {
      if (this.statusBasedGating) {
        return this.vulnerability.canAdmin && this.isCompleted;
      }
...
<template>
  <div data-testid="vulnerability-false-positive-details">
    <template v-if="isPending">
      <p data-testid="false-positive-pending">
        {{
          s__(
            'Vulnerability|AI false positive scan is in progress. Results will appear here when complete.',
          )
        }}
      </p>
    </template>
    ...
    <template v-else-if="isFailed">
      <p data-testid="false-positive-failed">
        {{ s__('Vulnerability|AI false positive scan failed. Please try running the scan again.') }}
      </p>
    </template>
