...
const initVulnerabilityDetailsEnrichment = (el) => {
  ...
  const vulnerability = convertObjectPropsToCamelCase(vulnerabilityJson, {
    deep: true,
  });
  ...
  return new Vue({
    ...
    render: (h) => h(VulnerabilityDetailsEnrichment, { props: { vulnerability } }),
  });
};
...
const initVulnerabilityDetails = (el) => {
  ...
  return new Vue({
    ...
    provide: {
      ...
      duoSastFpDetectionEnabled: parseBoolean(duoSastFpDetectionEnabled),
      projectFullPath,
      defaultBranch,
      dismissalDescriptions,
    },
    render: (h) => h(Vulnerability, { props: { initialVulnerability: vulnerability } }),
  });
};
