...
export default {
  ...
  inject: ['contextConfig', 'fullPath', 'isEligibleForTrial', 'isOpenbaoHealthy', 'isReadOnly'],
  data() {
    return {
      secrets: [],
  ...
  apollo: {
    secrets: {
      ...
      skip() {
        return this.isEligibleForTrial || !this.isOpenbaoHealthy;
      },
      variables() {
        return {
...
<template>
  <div>
    ...
    <secrets-trial-empty-state v-else-if="isEligibleForTrial" />
    <gl-empty-state
    ...
    <div v-if="!isEligibleForTrial" class="gl-mt-5 gl-text-center">
