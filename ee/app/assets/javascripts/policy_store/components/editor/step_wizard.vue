...
export default {
  ...
  apollo: {
    catalogsResult: {
      ...
      update(data) {
        return toCatalogs(data?.organization?.policyStore);
      },
      error(error) {
  ...
  data() {
    ...
    return {
      ...
      leaving: false,
      currentStepIndex: 0,
      // Without an organization id the query is skipped, so the catalogs
      // start settled and failed instead of loading forever.
      catalogsResult: {
        catalogs: EMPTY_CATALOGS,
        failedCatalogs: this.organizationId ? [] : [...CATALOG_NAMES],
      },
    };
  },
  computed: {
    catalogsLoading() {
      return this.organizationId ? this.$apollo.queries.catalogsResult.loading : false;
    },
    catalogs() {
