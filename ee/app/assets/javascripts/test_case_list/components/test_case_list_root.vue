...
export default {
  ...
  apollo: {
    project: {
      ...
      variables() {
        const queryVariables = {
          ...
          ...(this.glFeatures.workItemConfigurableTypes
            ? {
                workItemTypeIds: this.testCaseTypeId,
              }
            : {
                types: [WORK_ITEM_TYPE_ENUM_TEST_CASE],
              }),
        };
      ...
      skip() {
        // Wait until we get the type case ID from workItemTypesConfigurationQuery before calling query
  ...
  computed: {
    testCaseTypeId() {
      return this.workItemTypesConfiguration.find(
        (type) => type.name === WORK_ITEM_TYPE_NAME_TEST_CASE,
      )?.id;
    },
    testCases() {
      return {
    ...
    testCaseListEmpty() {
      return !this.testCaseListLoading && !this.testCases.list.length;
    },
