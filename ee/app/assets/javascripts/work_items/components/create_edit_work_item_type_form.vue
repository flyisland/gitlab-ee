...
<template>
  ...
  >
    <gl-form @submit.prevent="handleSubmit">
      ...
      <gl-form-group class="gl-mb-4">
        <gl-form-checkbox v-model="form.enabled" data-testid="work-item-type-enabled-checkbox">
          <div class="gl-mb-2">{{ s__('WorkItem|Enabled by default') }}</div>
          <div>
            {{
              s__(
                'WorkItem|Enable this type automatically for all existing projects and any new projects.',
              )
            }}
          </div>
        </gl-form-checkbox>
