...
const LARGE_TEXT_KEYS = {
  input_prompt: s__('AgentArtifacts|Input prompt'),
  system_prompt: s__('AgentArtifacts|System prompt'),
  agent_output: s__('AgentArtifacts|Agent output'),
};
  ...
  computed: {
    ...
    blocks() {
      return Object.entries(LARGE_TEXT_KEYS)
        ...
        .map(([key, label]) => ({ key, label, value: this.details[key] }));
    },
    isEmpty() {
