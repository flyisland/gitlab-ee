    ...
    module Helpers
      module WorkItems
        module Creation
          ...
          override :extract_feature_params
          def extract_feature_params(resource_parent)
            widget_params = super

            resolve_iteration_param(widget_params[:iteration_widget])
            resolve_status_param(widget_params[:status_widget], resource_parent)

            widget_params
          end

          def resolve_iteration_param(iteration_params)
            ...
            # Global lookup is intentional, the Iteration callback validates that the
            # iteration belongs to the same group hierarchy via in_the_same_group_hierarchy?
