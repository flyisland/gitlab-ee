      ...
      module ReadOnly
        module Controller
          ...
          def admin_settings_update?
            ...
            request.path.start_with?(url_with_relative_root('api/v4/application/settings'),
              url_with_relative_root('admin/application_settings/general'))
          end

          def geo_node_update_route?
            # Calling route_hash may be expensive. Only do it if we think there's a possible match
            return false unless request.path.start_with?(url_with_relative_root('admin/geo/'))
          ...
          def geo_proxy_git_ssh_route?
            ::Gitlab::Middleware::ReadOnly::API_VERSIONS.any? do |version|
              request.path.start_with?(url_with_relative_root("api/v#{version}/geo/proxy_git_ssh"))
            end
          end
          ...
          def geo_sign_out_route?
            return unless request_path.start_with?(url_with_relative_root('users/auth/geo/sign_out'))
          ...
          def sign_in_route?
            ...
            is_regular_sign_in_route = request.path.start_with?(
              url_with_relative_root('users/sign_in'),
              url_with_relative_root('oauth/token'),
              url_with_relative_root('users/auth/geo/sign_in')
            ) && ALLOWLISTED_SIGN_IN_ROUTES[route_hash[:controller]]&.include?(route_hash[:action])
            is_sso_callback_route = request.path.start_with?(url_with_relative_root('users/auth/')) &&
          ...
          def jwt_auth_route?
            request.post? && request.path.start_with?(url_with_relative_root('jwt/auth'))
          end

          def url_with_relative_root(path)
            File.join(relative_url, path)
          end
