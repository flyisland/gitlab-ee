        ...
        module Sync
          class Groups
            ...
            def update_permissions
              ...
              if config.audit_group.present?
                sync_auditor_users
              else
                logger.debug "No `audit_group` configured for '#{provider}' provider. Skipping"
              end
            ...
            def sync_auditor_users
              logger.debug "Syncing auditor users for '#{provider}' provider"

              if Sync::AuditorUsers.execute(proxy)
                logger.debug "Finished syncing auditor users for '#{provider}' provider"
              else
                logger.debug "Error syncing auditor users for '#{provider}' provider. LDAP connection error"
              end
            end
