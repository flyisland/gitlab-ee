...
module Vulnerabilities
  module ConsistencyChecks
    ...
    # in vulnerability_merge_request_links before VAC (Vulnerabilities Across Contexts) is enabled.
    #
    ...
    #
    # Related issues:
    # - https://gitlab.com/gitlab-org/gitlab/-/issues/596475 (this check)
    ...
    class BackfillMergeRequestLinkOccurrenceIdCheck < BaseCheck
      BATCH_SIZE = 1000
      ...
      def fix!
        ...
        null_occurrence_id_relation.each_batch(of: BATCH_SIZE) do |batch|
          count = Vulnerabilities::MergeRequestLink.connection.exec_update(<<~SQL)
            UPDATE vulnerability_merge_request_links
            SET vulnerability_occurrence_id = v.finding_id
            FROM vulnerabilities v
            WHERE vulnerability_merge_request_links.id IN (#{batch.select(:id).to_sql})
              AND vulnerability_merge_request_links.vulnerability_id = v.id
              AND vulnerability_merge_request_links.vulnerability_occurrence_id IS NULL
        ...
        log('Backfilled vulnerability_occurrence_id in vulnerability_merge_request_links', count: total) if total > 0
      end
