    ...
    module GroupSaml
      class IdentityLinker < Gitlab::Auth::Saml::IdentityLinker
        ...
        def update_enterprise_user_attributes
          EnterpriseUserAttributesUpdater.new(current_user, saml_provider.group, auth_hash).execute
        end
        ...
        # When a signed-in user completes the SAML linking flow, they have
        # proven account ownership. Re-trust the identity so future SAML
        # logins succeed. This restores trust after an admin-initiated
        # extern_uid change via the API.
        def re_trust_identity
          return if identity.new_record?
          return if identity.trusted_extern_uid?

          identity.update_column(:trusted_extern_uid, true)
        end
