    ...
    module Parsers
      class Sarif
        ...
        def parse!
          ...
          add_ingestion_feedback if @added_findings < @total_results
        ...
        def add_ingestion_feedback
          if ingestion_drop_ratio > INGESTION_FAILURE_THRESHOLD
            report.add_error('Ingestion', ingestion_error_message)
          else
            report.add_warning('Ingestion', ingestion_warning_message)
          end
        end
        ...
        def ingestion_warning_message
          "#{ingestion_drop_count} of #{@total_results} result(s) were skipped during ingestion. " \
            "Common reasons: missing physicalLocation, missing ruleId, or nil UUID components. " \
