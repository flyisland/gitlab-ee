    ...
    module Replication
      class BlobDownloader
        ...
        def check_preconditions
          unless Gitlab::Geo.secondary_or_org_migration_target?
        ...
        def failure_result(bytes_downloaded: 0, primary_missing_file: false, reason: nil, extra_details: nil)
          Result.new(success: false, bytes_downloaded: bytes_downloaded, primary_missing_file: primary_missing_file,
            reason: reason, extra_details: extra_details)
        end
        ...
        def download_file(url, req_headers, temp_file)
          ...
          if checksum_mismatch?(temp_file.path)
            return failure_result(bytes_downloaded: file_size, reason: "Downloaded file checksum mismatch",
              extra_details: { primary_checksum: primary_checksum, actual_checksum: @actual_checksum })
          end
          ...
          failure_result(bytes_downloaded: file_size, reason: "Error downloading file",
            extra_details: { error: e, url: url })
        end
