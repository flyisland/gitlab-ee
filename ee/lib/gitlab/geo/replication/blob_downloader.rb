    ...
    module Replication
      class BlobDownloader
        ...
        def download_file(url, req_headers, temp_file)
          if Feature.enabled?(:geo_blob_download_with_gitlab_http, type: :ops)
            download_file_with_gitlab_http(url, req_headers, temp_file)
          else
            download_file_with_http_rb(url, req_headers, temp_file)
          end
        end
        ...
        def download_file_with_http_rb(url, req_headers, temp_file)
          file_size = 0
          ...
          unless response.status.success?
            return non_success_response_result_http_rb(response, url)
          end
        ...
        # See: https://gitlab.com/gitlab-org/gitlab/-/issues/595139
        #
        # @return [Result] Object with transfer status and information
        def download_file_with_gitlab_http(url, req_headers, temp_file)
          file_size = 0

          # Disable automatic redirect following. HTTParty passes all
          # request headers (including our Geo auth header) to the redirect
          # target, which S3 rejects. See also:
          ...
          if response.redirection?
            response = stream_from_url(response.headers['location'], temp_file)
          elsif response.success?
            response = stream_from_url(url, temp_file, headers: req_headers)
          end

          unless response.success?
            return non_success_response_result_gitlab_http(response, url)
          end
        ...
        def primary_missing_file_http_rb?(response)
        ...
        def primary_missing_file_gitlab_http?(response)
        ...
        def non_success_response_result_http_rb(response, url)
          primary_missing_file = primary_missing_file_http_rb?(response)
        ...
        def non_success_response_result_gitlab_http(response, url)
          primary_missing_file = primary_missing_file_gitlab_http?(response)
          reason = if primary_missing_file
                     "The file is missing on the Geo primary site"
                   else
                     "Non-success HTTP response status code #{response.code}"
                   end
