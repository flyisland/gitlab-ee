  ...
  module Geo
    class HealthCheck
      ...
      def perform_checks
        ...
        return 'Geo node has a database that is writable which is an indication it is not configured for replication with the primary node.' unless ApplicationRecord.database.recovery? || Gitlab::Geo.postgresql_replication_agnostic_enabled?
        ...
        return "Databases with pending migrations: #{pending_migration_databases.join(', ')}.\nYou may have to run `gitlab-rake db:migrate` on the secondary." if logical_replication_mode? && pending_migration_databases.any?
      ...
      def logical_replication_mode?
        Gitlab::Geo.postgresql_replication_agnostic_enabled? && !ApplicationRecord.database.recovery?
      end

      def pending_migration_databases
        @pending_migration_databases ||= Gitlab::Database.database_base_models.keys.map(&:to_sym).reject do |db|
          replicated_database_migration_match?(db)
        end
      end

      private

      def replicated_database_migration_match?(database_name)
        connection_pool = Gitlab::Database.database_base_models[database_name]&.connection_pool
        return true unless connection_pool

        !connection_pool.migration_context.needs_migration?
      end
