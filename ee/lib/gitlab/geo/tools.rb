  ...
  module Geo
    ...
    module Tools
      ...
      def cleanup_check
        ...
        detected = ::Geo::Tools::KnownErrors.catalog.select(&:detected?)
        ...
        if detected.empty?
          ...
          detected.each_with_index do |known_error, index|
            ...
            if known_error.match_pattern.present?
              puts "   #{known_error.affected_count} records matching '#{known_error.match_pattern}'"
            else
              puts "   #{known_error.affected_count} records affected (structural check)"
            end
