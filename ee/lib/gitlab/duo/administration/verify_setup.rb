    ...
    module Administration
      class VerifySetup
        ...
        def check_duo_code_review_availability
          ...
          consent_routing_enabled = ::Feature.enabled?(:duo_code_review_dap_routing_consent_enabled, root_ancestor)
          ...
          info("duo_code_review_dap_routing_consent_enabled flag: " \
            "#{consent_routing_enabled ? '✔ enabled' : '✗ disabled'} (namespace: #{root_ancestor.full_path})")
          ...
          print_duo_code_review_mode(dap_available, has_enterprise_addon, consent_routing_enabled, consent_given)
        ...
        def print_duo_code_review_mode(dap_available, has_enterprise_addon, consent_routing_enabled, consent_given)
          ...
          print_duo_enterprise_routing(consent_routing_enabled, consent_given)
        end

        def print_duo_enterprise_routing(consent_routing_enabled, consent_given)
          if consent_routing_enabled && consent_given
            info("Mode: DAP - Duo Enterprise seat holders are routed through Code Review Flow (consent given)")
          else
            reason = if consent_routing_enabled
                       'consent has not been given for this namespace yet'
                     else
                       'duo_code_review_dap_routing_consent_enabled is disabled for this namespace'
                     end
