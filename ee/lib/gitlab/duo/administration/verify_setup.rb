      ...
      class VerifySetup
        ...
        def service_accounts_by_catalog_item_id
          consumers = ::Ai::Catalog::ItemConsumer
            ...
            .where("project_id = ? OR group_id = ?", project.id, project.root_namespace.id)
        ...
        def validate!
          ...
          warn "Usage: bundle exec rake \"gitlab:support:duo_code_review_debug[PROJECT_PATH]\""
          exit 1
        end
        ...
        def check_project
          ...
          if project
            ...
            info("Root group: #{project.root_namespace.full_path} (ID: #{project.root_namespace.id})")
        ...
        def check_duo_code_review_availability
          ...
          if settings_available
            ...
            elsif has_enterprise_addon
              info("Mode: Classic — non-agentic Duo Enterprise code review")
            end
          else
