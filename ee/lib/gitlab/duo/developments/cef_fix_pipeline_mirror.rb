...
module Gitlab
  module Duo
    module Developments
      ...
      #   3. Synthesize CI records the file export dropped -- cross-pipeline
      #      `needs:` jobs, downstream child pipelines, parent->child link.
      #      These are read-only fixtures; nothing runs them.
      #   4. Replay prod job traces onto matching local jobs.
      #   5. Write `fix_pipeline_evaluation_pipelines.yml` (consumed via
      #      CEF_FIX_PIPELINE_URL_OVERRIDE_YAML) and verify completeness.
      #
      # The source project must be imported manually before running this
      # task (export from gitlab.com via UI / `glab project export`, then
      # import locally). The mirror only fills in CI fixtures the file
      # export format can't represent.
      class CefFixPipelineMirror
        ...
        def self.find_local_pipeline(local_project, source_branch)
          ...
          local_project.ci_pipelines.where(sha: head_sha).order(id: :desc).first ||
