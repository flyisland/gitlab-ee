    ...
    module CodeReview
      class CustomInstructionsResolver
        ...
        def fetch_all_instructions(ref)
          (fetch_instance_instructions + fetch_ancestor_groups_instructions + fetch_project_instructions(ref))
            .compact.flatten
        end
        ...
        def fetch_instance_instructions
          ...
          parse_repo_instructions(template_project, template_project.default_branch_or_main, :instance)
        end

        def fetch_ancestor_groups_instructions
          ancestor_duo_template_projects.uniq.filter_map do |duo_template_project|
            parse_repo_instructions(duo_template_project, duo_template_project.default_branch_or_main, :group)
          end
        end

        def fetch_project_instructions(ref)
          parse_repo_instructions(project, ref, :project)
        end

        def parse_repo_instructions(project, ref, type)
