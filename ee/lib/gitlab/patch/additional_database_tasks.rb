...
module Gitlab
  module Patch
    module AdditionalDatabaseTasks
      # Returns the proper path for the structure.sql file for additional databases.
      ...
      # Returns the proper path for the schema_cache.yml file for additional databases.
      module SchemaCacheDumpPath
        def default_schema_cache_path(db_dir = 'db')
          connection_db_dir = Gitlab::Database.all_database_connections[name.to_s]&.db_dir

          return super if connection_db_dir.nil? || connection_db_dir == Rails.root.join('db')

          Rails.root.join(connection_db_dir, 'schema_cache.yml').to_s
        end
      end
      ...
      def self.patch!
        ActiveRecord::DatabaseConfigurations::HashConfig.prepend(SchemaCacheDumpPath)
