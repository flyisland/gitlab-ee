  ...
  module LicenseScanning
    class PackageLicenses
      ...
      def licenses_for_component(component)
        licenses = usable_licenses(component).map do |component_license|
          ...
          license[:expression] = component_license.expression if include_expression?(component_license)
          license
        end

        # A component that declares a license is always resolved from the SBOM, never PMDB.
        # When stripping unusable licenses (see usable_licenses) leaves nothing, we fall back to
        # an unknown license instead of an empty list, which would break
        # Reports::LicenseScanning::Report#licenses.
        licenses.presence || [UNKNOWN_LICENSE]
      end

      # Expression-only licenses carry no spdx identifier or name, so when ingestion is disabled we
      # drop them from the result. The component still stays on the SBOM path (the project chose the
      # SBOM as its license source); if nothing usable remains, licenses_for_component records it as
      # unknown rather than handing it off to the PMDB lookup.
      def usable_licenses(component)
        licenses = component.licenses.to_a
        return licenses if license_expression_ingestion_enabled?

        licenses.reject { |license| license.expression.present? }
      end

      def include_expression?(component_license)
        component_license.expression.present? && license_expression_ingestion_enabled?
      end

      def license_expression_ingestion_enabled?
        Feature.enabled?(:cyclonedx_license_expression_ingestion, project)
      end
      strong_memoize_attr :license_expression_ingestion_enabled?
