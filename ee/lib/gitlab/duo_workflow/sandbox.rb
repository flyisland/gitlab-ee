...
module Gitlab
  module DuoWorkflow
    ...
    class Sandbox
      ...
      # Placed outside /tmp so it is not in the agent-writable area.
      # The agent is explicitly denied write access via denyWrite.
      ...
      def srt_config
        if (::Gitlab::Saas.feature_available?(:gitlab_com_subscriptions) &&
              Feature.enabled?(:dap_group_network_access_controls, @duo_config.project)) ||
      ...
      def combine_settings(inherited_settings)
        ...
        if allow_project_extension
          ...
          if !duo_config_include_recommended.nil? &&
              (!duo_config_include_recommended || effective_include_recommended_allowed)
            effective_include_recommended_allowed = duo_config_include_recommended
          end

          if !duo_config_allow_all_unix_sockets.nil? &&
              (!duo_config_allow_all_unix_sockets || effective_allow_all_unix_sockets)
            effective_allow_all_unix_sockets = duo_config_allow_all_unix_sockets
          end
        end
