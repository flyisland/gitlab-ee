      ...
      module Sbom
        module License
          class Common
            ...
            def parse
              # `expression` and `license` are mutually exclusive within a CycloneDX
              # `licenses[]` entry (schema `oneOf`), so prefer `expression` defensively.
              expression = data['expression']
              return parsed_expression(expression) if expression.present?
            ...
            def parsed_expression(expression)
              ::Gitlab::Ci::Reports::Sbom::License.new(expression: expression)
            end
          end
        end
      end
