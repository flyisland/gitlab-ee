...
module Audit
  class Details
    ...
    def action_text
      ...
      when :add
        text = if @details[:as]
                 _('Added %{target_name} as %{role}') % { target_name: target_name, role: @details[:as] }
               else
                 _('Added %{target_name}') % { target_name: target_name }
               end

        text += " #{_('with expiry: %{expiry}') % { expiry: expiry_text_for_add }}" if @details.has_key?(:expires_at)
        text
      when :remove
    ...
    def expiry_details_available?
      (@details.has_key?(:expiry_from) && @details.has_key?(:expiry_to)) ||
        @details.has_key?(:expires_at)
    end

    def expiry_text_for_add
      @details[:expires_at].presence || never_expires_text
    end
