    ...
    module McpHandlers
      class CallTool < Base
        ...
        def invoke
          ...
          raise ArgumentError, "Unknown tool: #{tool_name}" unless allowed_tools.include?(tool_name)
        ...
        # The set of tool names accepted by tools/call. When orbit_use_legacy_tools
        # is disabled, legacy tools (query_graph, get_graph_schema, get_graph_status)
        # are excluded so they return "Unknown tool" rather than executing.
        def allowed_tools
          return KNOWN_TOOLS + COMMAND_TOOLS if ToolCatalog.legacy_tools_enabled?(current_user)

          (KNOWN_TOOLS + COMMAND_TOOLS) - ToolCatalog::ALL_LEGACY_TOOL_NAMES
        end
