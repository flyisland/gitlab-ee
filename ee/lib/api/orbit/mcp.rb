...
module API
  module Orbit
    class Mcp < ::API::Base
      ...
      ORBIT_FEATURE_CACHE_PERIOD = 1.hour
      ORBIT_FEATURE_CACHE_KEY = 'group_with_orbit_enabled'
      ...
      before do
        ...
        forbidden! unless AccessTokenValidationService.new(access_token).include_any_scope?(
          [Gitlab::Auth::MCP_ORBIT_SCOPE, Gitlab::Auth::AI_WORKFLOW]
        )
      end
      ...
      helpers do
        ...
        def feature_available?
          ...
          Rails.cache.fetch(
            ['users', current_user.id, ORBIT_FEATURE_CACHE_KEY],
            expires_in: ORBIT_FEATURE_CACHE_PERIOD
          ) do
            current_user.authorized_groups.top_level.any? do |group|
