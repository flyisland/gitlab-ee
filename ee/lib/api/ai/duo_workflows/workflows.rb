      ...
      class Workflows < ::API::Base
        ...
        helpers do
          ...
          def start_workflow_params(workflow_id, container:)
...

            {
              ...
              workflow_metadata: Gitlab::DuoWorkflow::Client.metadata(current_user,
                namespace: container&.root_ancestor).to_json,
          ...
          def workflow_context_generation_service(container: nil)
            @workflow_context_generation_service ||= ::Ai::DuoWorkflows::WorkflowContextGenerationService.new(
          ...
          def compute_server_capabilities(dws_capabilities:, root_namespace:)
            rails_capabilities = []
            rails_capabilities << "advanced_search" if advanced_search_enabled?

            # Filter DWS capabilities based on admin/namespace settings
            dws_capabilities.delete('tool_call_approval') unless tool_approval_for_session_enabled?(root_namespace)

            (rails_capabilities + dws_capabilities).uniq
          end

          def tool_approval_for_session_enabled?(root_namespace)
            return false unless root_namespace

            root_namespace.namespace_settings&.tool_approval_for_session_enabled?
          end
        ...
        namespace :ai do
          namespace :duo_workflows do
            resources :direct_access do
              ...
              post do
                ...
                access = {
                  ...
                  workflow_metadata: Gitlab::DuoWorkflow::Client.metadata(current_user, namespace: root_namespace),
                  server_capabilities: compute_server_capabilities(
                    dws_capabilities: dws_capabilities, root_namespace: root_namespace
                  )
                }
            ...
            get :ws do
              ...
              gitlab_token = if workflow_context_generation_service.already_scoped_for_ai_workflows?(access_token)
                               parsed_oauth_token
                             else
                               gitlab_oauth_token.plaintext_token
                             end
...

              {
                DuoWorkflow: {
                  ...
                  ServerCapabilities: compute_server_capabilities(dws_capabilities: [], root_namespace: root_namespace)
                }
              }
            end
