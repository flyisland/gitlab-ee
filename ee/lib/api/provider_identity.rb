    ...
    resource :groups do
      %w[saml scim].each do |provider_type|
        resource ":id/#{provider_type}", requirements: API::NAMESPACE_OR_PROJECT_REQUIREMENTS do
          ...
          route_setting :authorization, permissions: :"read_#{provider_type}_identity", boundary_type: :group
          get ':uid', requirements: { uid: API::NO_SLASH_URL_PART_REGEX }.merge(API::NO_FORMAT_SUFFIX_REQUIREMENT) do
            group = find_group(params[:id])
          ...
          patch ':uid', requirements: { uid: API::NO_SLASH_URL_PART_REGEX }.merge(API::NO_FORMAT_SUFFIX_REQUIREMENT) do
            group = find_group(params[:id])
            identity = find_provider_identity(provider_type, params[:uid], group)
          ...
          delete ':uid', requirements: { uid: API::NO_SLASH_URL_PART_REGEX }.merge(API::NO_FORMAT_SUFFIX_REQUIREMENT) do
            group = find_group(params[:id])
            identity = find_provider_identity(provider_type, params[:uid], group)
