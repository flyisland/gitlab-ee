...
module API
  module Entities
    class GeoSite < Grape::Entity
      ...
      expose :blob_download_timeout
      expose :sync_object_storage, if: ->(geo_site, _) { geo_site.secondary_or_org_migration_target? }

      expose :web_edit_url do |geo_site|
        ::Gitlab::Routing.url_helpers.edit_admin_geo_node_url(geo_site)
      end

      expose :web_geo_replication_details_url, if: ->(geo_site) { geo_site.secondary_or_org_migration_target? },
