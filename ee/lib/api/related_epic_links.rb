  ...
  class RelatedEpicLinks < ::API::Base
    ...
    helpers do
      ...
      def can_read_link?(link)
        ...
        if source.is_a?(::WorkItem)
          ...
          can?(current_user, :read_epic, source) && can?(current_user, :read_epic, target)
        end
      end
    end
    ...
    resource :groups, requirements: API::NAMESPACE_OR_PROJECT_REQUIREMENTS do
      ...
      get ':id/related_epic_links' do
        legacy_epics = unless Feature.enabled?(:related_epic_links_from_work_items, user_group)
                         EpicsFinder.new(current_user, group_id: user_group.id).execute
                       end

        related_links = ::WorkItems::LegacyEpics::RelatedLinksFinder
          .new(user_group, current_user: current_user, params: declared_params, legacy_epics: legacy_epics).execute

        related_links = paginate(related_links)

        # Links can reference epics the user has no access to (e.g. cross-group or confidential).
        # When the flag is enabled, source/target are WorkItems; otherwise they are Epics.
