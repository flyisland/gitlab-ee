...
module API
  module Authn
    ...
    class TokenExchange < ::API::Base
      ...
      DEFAULT_EXPIRES_IN_SECONDS = 5.minutes.to_i
      ...
      namespace 'token_exchange' do
        ...
        params do
          ...
          optional :expires_in, type: Integer,
            ...
            desc: "Requested token lifetime in seconds. Defaults to #{DEFAULT_EXPIRES_IN_SECONDS}; " \
        ...
        post do
          ...
          # Only one audience is supported today; map each future audience to its own
          # Ability when added. Per-repo authorization (push/pull a given repository)
          # remains the modular service's responsibility.
          forbidden! unless can?(current_user, :access_artifact_registry_service)

          ttl = declared_params[:expires_in] || DEFAULT_EXPIRES_IN_SECONDS
