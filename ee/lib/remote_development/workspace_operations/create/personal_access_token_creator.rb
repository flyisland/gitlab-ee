      ...
      class PersonalAccessTokenCreator
        ...
        def self.create(context)
          ...
          personal_access_token = user.personal_access_tokens.build(
            ...
            expires_at: max_allowed_personal_access_token_expires_at(user),
        ...
        # @param [User] user
        # @return [Date]
        def self.max_allowed_personal_access_token_expires_at(user)
          default_expires_at = MaxHoursBeforeTermination::MAX_HOURS_BEFORE_TERMINATION.hours.from_now.to_date

          pat_max_expiry_date =
            EE::Gitlab::PersonalAccessTokens::ExpiryDateCalculator.new(user).max_expiry_date

          expires_at_date = if pat_max_expiry_date.present?
                              [default_expires_at, pat_max_expiry_date].min
                            else
                              default_expires_at
                            end

          expires_at_date.next_day - 1.second
        end
