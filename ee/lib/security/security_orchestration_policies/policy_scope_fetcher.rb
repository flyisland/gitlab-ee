  ...
  module SecurityOrchestrationPolicies
    class PolicyScopeFetcher
      ...
      def execute
        ...
        including_business_impact, excluding_business_impact = scoped_business_impact_attributes

        {
          ...
          including_business_impact_attributes: including_business_impact,
          excluding_business_impact_attributes: excluding_business_impact
        }
      end
      ...
      def scoped_business_impact_attributes
        return [[], []] unless policy_scope_security_attributes_enabled?

        included_ids = policy_scope.dig(:business_impact, :including)&.pluck(:id) || []
        excluded_ids = policy_scope.dig(:business_impact, :excluding)&.pluck(:id) || []
        ids = included_ids + excluded_ids

        return [[], []] if ids.empty?

        attributes = security_attributes(ids)

        [attributes.values_at(*included_ids).compact, attributes.values_at(*excluded_ids).compact]
      end

      def security_attributes(ids)
        ::Security::Attribute
