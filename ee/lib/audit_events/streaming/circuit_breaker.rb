...
#
# Gated behind the `audit_event_streaming_circuit_breaker` feature flag.
module AuditEvents
  module Streaming
    module CircuitBreaker
      FAILURE_THRESHOLD = 5
      FAILURE_WINDOW = 5.minutes
      OPEN_DURATION = 15.minutes
      ...
      class << self
        ...
        def trackable?(destination)
          destination.id.present? && enabled?(destination)
        end

        def enabled?(destination)
          if instance_scope?(destination)
            # rubocop:disable Gitlab/FeatureFlagWithoutActor -- instance-wide destinations have no per-actor scope
            ::Feature.enabled?(:audit_event_streaming_circuit_breaker)
            # rubocop:enable Gitlab/FeatureFlagWithoutActor
          else
            ::Feature.enabled?(:audit_event_streaming_circuit_breaker, destination.group)
          end
        end
