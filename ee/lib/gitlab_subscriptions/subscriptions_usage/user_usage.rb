...
module GitlabSubscriptions
  module SubscriptionsUsage
    class UserUsage
      ...
      def users(search_query: nil, sort: nil, username: nil, first: nil, last: nil, before: nil, after: nil)
        strong_memoize_with(:users, search_query, sort, username, first, last, before, after) do
          if username.blank? && sort.in?([:total_credits_used_asc, :total_credits_used_desc])
            ...
            users_sorted_by_credits_used({ sort: sort, first: first, last: last, before: before, after: after })
          else
      ...
      def users_sorted_by_credits_used(args)
        ...
        users = User.by_ids(user_ids).human_or_service_user.limit(MAX_USERS_PER_PAGE)
        ...
        sorted_users = user_ids.filter_map do |user_id|
          ...
          ::GitlabSubscriptions::SubscriptionsUsage::UserWithConsumption.new(user, consumers_by_user_id[user_id])
        end
