...
module GitlabSubscriptions
  class SubscriptionUsage
    ...
    def credits_used
      return unless subscription_usage[:success]

      subscription_usage.dig(:subscriptionUsage, :creditsUsed)
    end
    strong_memoize_attr :credits_used

    def daily_usage(sort: :date_asc, limit: 30)
      strong_memoize_with(:daily_usage, sort, limit) do
        validate_daily_usage_limit!(limit)
        response = subscription_usage(sort: sort, limit: limit)
    ...
    def daily_average
      ...
      subscription_usage.dig(:subscriptionUsage, :dailyAverage)
    end
    strong_memoize_attr :daily_average
    ...
    def validate_daily_usage_limit!(limit)
      client_start_date = subscription_usage_client.start_date
      client_end_date = subscription_usage_client.end_date

      max_days = (Date.parse(client_end_date.to_s) - Date.parse(client_start_date.to_s)).to_i + 1
