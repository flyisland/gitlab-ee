...
module CloudConnector
  ...
  #
  # Includes `x-gitlab-client-ip` from `Gitlab::RequestContext` when present,
  # so downstream services (e.g. Duo Workflow Service for AI audit events)
  # can attribute the originating user's IP.
  def ai_headers(user, namespace_ids: [])
    headers(user).merge(
      'x-gitlab-feature-enabled-by-namespace-ids' => namespace_ids.join(','),
      'x-gitlab-subject-type' => user&.user_type.to_s
    ).tap do |h|
      client_ip = Gitlab::RequestContext.instance.client_ip
      h['x-gitlab-client-ip'] = client_ip if client_ip.present?
    end
  end
