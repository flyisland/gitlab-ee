...
module Ci
  module MinutesHelpers
    ...
    def set_sharded_ci_minutes_used(namespace, minutes, shared_runners_duration = 0, project: nil)
      if namespace.namespace_statistics
        namespace.namespace_statistics.update!(shared_runners_seconds: minutes.minutes)
      else
        namespace.create_namespace_statistics(shared_runners_seconds: minutes.minutes)
      ...
      ::Ci::Minutes::NamespaceMonthlyUsage
        .find_or_create_current(namespace_id: namespace.id)
        .update!(amount_used: minutes / 2.0, shared_runners_duration: shared_runners_duration / 2.0)

      ::Ci::Minutes::NamespaceMonthlyUsage.new.tap do |usage|
        usage.shard_number = 2
        usage.namespace = namespace
        usage.amount_used = minutes / 2.0
        usage.shared_runners_duration = shared_runners_duration / 2.0
        usage.date = Date.current.beginning_of_month
      end.save!
