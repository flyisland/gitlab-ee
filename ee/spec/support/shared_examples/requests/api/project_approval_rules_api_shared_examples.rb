  ...
  context 'when the request is correct' do
    ...
    context 'when applies_to_all_protected_branches param is present' do
      ...
      it 'returns a list of project protected branches in the response' do
        ...
        expect(json_response['applies_to_all_protected_branches']).to be(true)
      end
    end
  ...
  shared_examples 'a user with access' do
    ...
    context 'when approval rule is from scan result policy' do
      ...
      context 'when report_type is scan_finding' do
        let_it_be(:scan_result_policy_rule, freeze: false) { create(:approval_project_rule, :scan_finding, project: project) }

        it_behaves_like 'cannot update rule'
      end

      context 'when report_type is license_scanning' do
        let_it_be(:scan_result_policy_rule, freeze: false) { create(:approval_project_rule, :license_scanning, project: project) }
    ...
    context 'when user is authorized but cannot edit the approval rule' do
      it 'returns forbidden' do
        expect_next(::ApprovalRules::UpdateService).to receive(:authorized?).and_return(false)
...
RSpec.shared_examples 'an API endpoint for deleting project approval rule' do
  ...
  shared_examples_for 'a user with access' do
    it 'destroys' do
      ...
      expect(ApprovalProjectRule.exists?(id: approval_rule.id)).to be(false)
