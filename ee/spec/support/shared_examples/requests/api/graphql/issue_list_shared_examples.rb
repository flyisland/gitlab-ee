...
RSpec.shared_examples 'graphql issue list request spec EE' do
  ...
  describe 'sorting and pagination' do
    ...
    context 'when sorting by sla due' do
      let_it_be(:sla_issue1, freeze: false) { create(:issue, :incident, project: public_project) }
      let_it_be(:sla_issue2, freeze: false) { create(:issue, :incident, project: public_project) }
      let_it_be(:sla_issue3, freeze: false) { create(:issue, :incident, project: public_project) }
      let_it_be(:sla_issue4, freeze: false) { create(:issue, :incident, project: public_project) }
      let_it_be(:sla_issue5, freeze: false) { create(:issue, :incident, project: public_project) }
  ...
  describe 'filtering' do
    ...
    context 'when filtering by iteration' do
      let_it_be(:cadence, freeze: false) do
      ...
      let_it_be(:project, freeze: false) { create(:project, group: group1) }
      let_it_be(:issue_1, freeze: false) { create(:issue, project: project, iteration: iteration) }
      let_it_be(:issue_2, freeze: false) { create(:issue, project: project, iteration: iteration) }
    ...
    context 'when filtering by epic' do
      ...
      let_it_be(:epic_a, freeze: false) { create(:epic, group: group1) }
      let_it_be(:epic_b, freeze: false) { create(:epic, group: group1) }
  ...
  describe 'blocked' do
    let_it_be(:link1, freeze: false) { create(:issue_link, source: issue_a, target: issue_b, link_type: 'blocks') }
    let_it_be(:link2, freeze: false) { create(:issue_link, source: issue_c, target: issue_b, link_type: 'blocks') }
    let_it_be(:link3, freeze: false) { create(:issue_link, source: issue_a, target: issue_d, link_type: 'blocks') }
  ...
  describe 'related_vulnerabilities' do
    ...
    it 'avoids N+1 queries', :aggregate_failures, :use_sql_query_cache, :request_store do
      post_query # warm-up
      ...
      expect { post_query }.not_to exceed_query_limit(control).allow_skip_cache_inconsistency
