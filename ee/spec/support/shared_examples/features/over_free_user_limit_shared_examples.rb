...
RSpec.shared_examples_for 'over the free user limit alert' do
  let_it_be(:new_user, freeze: false) { create(:user) }
  let_it_be(:dismiss_button, freeze: false) do
      ...
      context 'when over storage limits' do
        ...
        context 'without storage size check' do
          it 'does show alerts' do
            ...
            expect(page).to have_content("Your namespace #{group.name} has been placed in a read-only state")
          end
        end
      end
    end
