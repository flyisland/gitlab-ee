  ...
  context 'when licensed' do
    ...
    context 'when operation succeeds' do
      it 'logs an audit event' do
        ...
        expect { operation }.to change { AuditEventReader.count }.by(defined?(event_count) ? event_count : 1)
      end
      ...
      it 'logs the audit event info' do
        ...
        if defined?(event_count)
          expect(AuditEventReader.last(event_count)).to match_array(attributes.map { |attrs| have_attributes(attrs) })
        else
          expect(AuditEventReader.last).to have_attributes(attributes)
        end
      end
    ...
    it 'does not log audit event if operation fails' do
      ...
      expect { operation }.not_to change { AuditEventReader.count }
    end
...
RSpec.shared_examples 'logs the custom audit event' do
  ...
  it 'creates an event and logs to a file with the provided details' do
    freeze_time do
      ...
      expect { service.security_event }.to change { AuditEventReader.count }.by(1)
      security_event = AuditEventReader.last
