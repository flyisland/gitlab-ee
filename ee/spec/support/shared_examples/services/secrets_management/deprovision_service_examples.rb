...
RSpec.shared_examples 'a secrets manager deprovision service' do
  ...
  context 'when the secrets manager record is already gone' do
    before do
      ...
      allow(find_sm_target).to receive_messages(find_by_id: nil, find_by_group_id: nil)
    end
  ...
  # By the time the deprovision worker runs after a destroy, the parent
  # record (project or group) is already gone and the FK `ON DELETE SET
  # NULL` has nulled `project_id` / `group_id` on the SM. The service
  # must still find and destroy the SM row so it doesn't leak as a
  # zombie in `deprovisioning` state forever.
  context 'when the SM parent has been destroyed and its FK column is nil' do
    before do
      maintenance_task # eager-evaluate to capture original parent id
      secrets_manager.update_column(parent_fk_column, nil)
    end

    it 'still finds and destroys the orphaned SM row' do
      expect(result).to be_success
      expect(secrets_manager.class.find_by(id: secrets_manager.id)).to be_nil
      expect(maintenance_task.class.find_by(id: maintenance_task.id)).to be_nil
    end
