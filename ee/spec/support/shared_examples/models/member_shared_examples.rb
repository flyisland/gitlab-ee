...
RSpec.shared_examples 'member group domain validations' do |source_type|
  context 'validates group domain limitations' do
    ...
    let_it_be(:gitlab_user, freeze: false) { create(:user, email: 'test@gitlab.com') }
    let_it_be(:gmail_user, freeze: false) { create(:user, email: 'test@gmail.com') }
    let_it_be(:unconfirmed_gitlab_user, freeze: false) { create(:user, :unconfirmed, email: 'unverified@gitlab.com') }
    let_it_be(:acme_user, freeze: false) { create(:user, email: 'user@acme.com') }
    ...
    context 'when group_allowed_email_domains is turned on' do
      ...
      context 'with service account users' do
        let_it_be(:service_account, freeze: false) { create(:user, :service_account, email: "bot@example.com") }
