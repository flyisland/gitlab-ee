...
RSpec.shared_examples 'a replicator' do
  ...
  context 'Geo node status' do
    context 'on a primary site' do
      let_it_be(:model_class_factory, freeze: false) { model_class_factory_name(described_class.registry_class) }
      let_it_be(:replicables, freeze: false) { create_list(model_class_factory, 2) }
    ...
    context 'on a secondary site' do
      let_it_be(:registry_factory, freeze: false) { registry_factory_name(described_class.registry_class) }
  ...
  describe '#sync' do
    context 'when the model record has been deleted on the primary' do
      before do
        stub_current_geo_node(secondary)
        # Persist the model record and create the registry before stubbing,
        # otherwise the registry cannot be saved due to NOT NULL constraints.
        model_record.save!
        replicator.registry.save!
        allow(replicator).to receive(:model_record)
          .and_raise(ActiveRecord::RecordNotFound)
      end

      it 'does not raise an error' do
        expect { replicator.sync }.not_to raise_error
      end
      ...
      it 'marks the registry as failed' do
        ...
        expect(replicator.registry.reload).to have_attributes(
          state: described_class.registry_class::STATE_VALUES[:failed],
          last_sync_failure: expected_message
        )
      end

      it 'logs the record not found message' do
        expect(replicator).to receive(:log_info).with(
          "Couldn't find #{described_class.model} with 'id'=#{model_record.id}",
  ...
  context 'when replicator enqueues a sync event' do
    specify do
      ...
      expect(Geo::EventWorker).to receive(:perform_async).with(
        ...
        event_name,
        replicator.event_params.merge('correlation_id' => an_instance_of(String))
      )
