...
RSpec.shared_examples 'model with cvss v4 vector validation' do |attribute|
  let(:cvss_version) { '4.0' }
  let(:attack_vector) { 'N' }
  let(:attack_complexity) { 'L' }
  let(:attack_requirements) { 'N' }
  let(:privileges_required) { 'N' }
  let(:user_interaction) { 'N' }
  let(:vuln_confidentiality) { 'N' }
  let(:vuln_integrity) { 'N' }
  let(:vuln_availability) { 'N' }
  let(:sub_confidentiality) { 'N' }
  let(:sub_integrity) { 'N' }
  let(:sub_availability) { 'N' }
  ...
  context 'when validating', :aggregate_failures do
    describe 'version' do
      ...
      context 'when version param is invalid' do
        where(:cvss_version) { ['3.1', '3.0', '2.0', '???', ''] }
      ...
      context 'when given a valid cvss v3 vector' do
        let(:vector) { "CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:N/I:N/A:N" }

        it { is_expected.to be_invalid }

        it 'reports correct error' do
          expect(subject.errors[attribute]).to include('must use version 4.0')
        end
      end
    end
    ...
    describe 'parameters' do
      {
        attack_vector: %w[N A L P],
        attack_complexity: %w[L H],
        attack_requirements: %w[N P],
        privileges_required: %w[N L H],
        user_interaction: %w[N P A],
        vuln_confidentiality: %w[N L H],
        vuln_integrity: %w[N L H],
        vuln_availability: %w[N L H],
        sub_confidentiality: %w[N L H],
        sub_integrity: %w[N L H],
        sub_availability: %w[N L H]
      }.each do |parameter, valid_parameter_values|
        context "when #{parameter} is valid" do
          where(parameter) { valid_parameter_values }
      ...
      %i[CVSS AV AC AT PR UI VC VI VA SC SI SA].each do |parameter|
        context "when #{parameter} is missing" do
          let(:params) { base_params.except(parameter) }
