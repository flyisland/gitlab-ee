...
RSpec.shared_examples 'epics hierarchy finder with filtering' do
  context 'with correct params' do
    ...
    context 'with milestone' do
      ...
      let_it_be(:another_project, freeze: false) { create(:project, group: another_group) }
      let_it_be(:group_milestone, freeze: false) { create(:milestone, group: group, title: 'test') }
      let_it_be(:another_milestone, freeze: false) { create(:milestone, project: another_project, title: 'test') }
      let_it_be(:issue, freeze: false) { create(:issue, project: project, milestone: group_milestone) }
      let_it_be(:another_issue, freeze: false) do
        create(:issue, project: another_project, milestone: another_milestone)
      end

      let_it_be(:epic_issue, freeze: false) { create(:epic_issue, epic: epic2, issue: issue) }
      let_it_be(:another_epic_issue, freeze: false) { create(:epic_issue, epic: epic3, issue: another_issue) }
