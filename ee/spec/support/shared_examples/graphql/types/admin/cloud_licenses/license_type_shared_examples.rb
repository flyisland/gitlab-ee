...
RSpec.shared_examples_for 'license type fields' do
  ...
  context 'with license type fields' do
    ...
    describe 'field values' do
      let_it_be(:starts_at, freeze: false) { Date.current - 3.months }
      let_it_be(:expires_at, freeze: false) { Date.current + 9.months }
      let_it_be(:block_changes_at, freeze: false) { Date.current + 10.months }
      let_it_be(:activated_at, freeze: false) { DateTime.current - 2.months }
      let_it_be(:licensee, freeze: false) do
        {
          'Name' => 'User Example',
      ...
      let_it_be(:restrictions, freeze: false) do
        {
          id: 5,
          plan: 'ultimate',
      ...
      let_it_be(:license, freeze: false) do
