...
# maintenance tasks. Including specs must provide:
#
# - `task_factory`            -- factory name, e.g. :project_secrets_manager_maintenance_task
# - `deprovision_worker_class` -- the worker class the cron re-enqueues
# - `extra_attrs`             -- hash of additional factory attrs needed for a
#                                valid record (e.g. project_secrets_manager: ...)
RSpec.shared_examples 'a secrets manager maintenance tasks cron worker' do
  ...
  describe '#perform' do
    ...
    context 'when database is read-only' do
      ...
      it 'does not process tasks or enqueue deprovision workers' do
    ...
    context 'when one task in the batch raises an error' do
      ...
      before do
        allow(deprovision_worker_spy).to receive(:perform_async) do |task_id|
      ...
      it 'tracks the exception and continues processing the rest of the batch' do
        ...
        expect(deprovision_worker_spy).to have_received(:perform_async).with(good_task.id)
      end
    end
  end
end
