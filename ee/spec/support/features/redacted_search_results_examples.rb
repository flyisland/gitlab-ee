...
RSpec.shared_examples 'redacted search results page assertions' do |logged_in|
  context "when #{logged_in ? '' : 'not '}logged in", :elastic, :js, :disable_rate_limiter do
    before do
      ...
      stub_current_organization(public_restricted_project.organization) unless logged_in
    end
    ...
    it 'redacts private features the user does not have access to' do
      ...
      # work_items scope is not available for search within a project when
      # work_items are restricted
      if has_search_scope?('Work items')
        select_search_scope('Work items')
        # Project work_items are restricted
        expect(page).to have_no_content('The Issue')
        expect(page).to have_no_content('The Confidential work_item')
      end
      ...
      if has_search_scope?('Merge requests')
        ...
        expect(page).to have_no_content('The Merge Request')
      end
