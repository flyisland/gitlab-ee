...
module SaasRegistrationHelpers
  ...
  def fills_in_trial_welcome_form(trial_success: true)
    if trial_success
      stub_trial_success
    else
      stub_trial_failure
  ...
  def stub_trial_success
    ...
    allow_next_instance_of(GitlabSubscriptions::CreateLeadService) do |service|
      expect(service).to receive(:execute).with(trial_user: trial_lead_params).and_return(ServiceResponse.success)
    end
  ...
  def expect_to_be_on_standard_user_registration
