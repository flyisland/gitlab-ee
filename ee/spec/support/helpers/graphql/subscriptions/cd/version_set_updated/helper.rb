    ...
    module Cd
      module VersionSetUpdated
        module Helper
          ...
          def cd_version_set_updated_subscription(application, current_user)
            mock_channel = Graphql::Subscriptions::ActionCable::MockActionCable.get_mock_channel
            query = cd_version_set_updated_subscription_query(application)
          ...
          def cd_version_set_updated_subscription_query(application)
            <<~SUBSCRIPTION
              subscription {
                cdVersionSetUpdated(applicationId: "#{application.to_global_id}") {
                  id
                  name
                  status
                }
