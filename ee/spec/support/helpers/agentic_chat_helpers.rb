...
module AgenticChatHelpers
  def ensure_duo_workflow_service_running!(max_attempts: 20, interval: 5.0)
    max_attempts.times do |attempt|
      ...
      result = ::CloudConnector::StatusChecks::Probes::DuoAgentPlatformProbe.new(u).execute
  ...
  def wait_for_grpc_stream_finished!(max_attempts: 20, interval: 2.0)
    max_attempts.times do |attempt|
      begin
        find('.duo-chat-loader', wait: interval)
      rescue Capybara::ElementNotFound
        break
      end

      raise 'Timeout error: Waiting for gRPC stream finished' if attempt >= max_attempts - 1
