...
RSpec.shared_context 'for virtual registry api setup' do
  ...
  let_it_be(:expired_oauth_token) do
    create(
      :oauth_access_token,
  ...
  let_it_be(:revoked_oauth_token) do
    create(:oauth_access_token, application_id: oauth_application.id, resource_owner_id: user.id,
      scopes: [:api]).tap(&:revoke)
  end

  let_it_be(:revoked_oauth_token_plaintext) { revoked_oauth_token.plaintext_token }
  let_it_be(:wrong_scope_oauth_token) do
...
RSpec.shared_context 'for npm virtual registry api setup' do
  ...
  let_it_be(:upstream) { create(:virtual_registries_packages_npm_upstream, registries: [registry]) }
  let_it_be_with_reload(:cache_entry) do
