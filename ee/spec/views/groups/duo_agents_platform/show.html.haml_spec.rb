...
RSpec.describe 'groups/duo_agents_platform/show', feature_category: :duo_agent_platform do
  ...
  context 'when identity verification is required' do
    before do
      allow(view).to receive(:duo_agents_platform_identity_verification_required?).with(group).and_return(true)
    end
  ...
  context 'when identity verification is not required' do
    before do
      allow(view).to receive(:duo_agents_platform_identity_verification_required?).with(group).and_return(false)
    end
