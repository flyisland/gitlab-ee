...
RSpec.describe Ai::Catalog::ItemConsumersFinder, feature_category: :workflow_catalog do
  ...
  let_it_be(:parent_group, freeze: false) { create(:group, developers: user) }
  let_it_be(:parent_group_consumers, freeze: false) { create_list(:ai_catalog_item_consumer, 2, group: parent_group) }
  let_it_be(:project, freeze: false) { create(:project, group: parent_group) }
  let_it_be(:project_consumers, freeze: false) { create_list(:ai_catalog_item_consumer, 2, project: project) }
  let_it_be(:another_project, freeze: false) { create(:project, group: parent_group) }
  let_it_be(:another_project_consumer, freeze: false) { create(:ai_catalog_item_consumer, project: another_project) }
  let_it_be(:group, freeze: false) { create(:group, parent: parent_group) }
  let_it_be(:group_consumers, freeze: false) { create_list(:ai_catalog_item_consumer, 2, group: group) }
  let_it_be(:foundational_agent, freeze: false) { create(:ai_catalog_item) }
  let_it_be(:foundational_agent2, freeze: false) { create(:ai_catalog_item) }
  ...
  context 'when project_id is provided' do
    ...
    context 'when item_types is provided' do
      let_it_be(:flow, freeze: false) { create(:ai_catalog_flow) }
      let_it_be(:flow_consumer, freeze: false) { create(:ai_catalog_item_consumer, project: project, item: flow) }

      let_it_be(:third_party_flow, freeze: false) { create(:ai_catalog_third_party_flow) }
      let_it_be(:third_party_flow_consumer, freeze: false) do
    ...
    context 'when user can read foundational flows but not custom flows' do
      ...
      let_it_be(:custom_flow, freeze: false) { create(:ai_catalog_flow) }
      let_it_be(:custom_flow_consumer, freeze: false) do
  ...
  context 'when the top-level group restricts the AI Catalog to its hierarchy' do
    ...
    let_it_be(:project, freeze: false) { create(:project, group: parent_group) }
    let_it_be(:sibling_project, freeze: false) { create(:project, group: parent_group) }

    let_it_be(:project_flow, freeze: false) { create(:ai_catalog_flow, :public, project: project) }
    let_it_be(:sibling_project_flow, freeze: false) { create(:ai_catalog_flow, :public, project: sibling_project) }
    let_it_be(:external_flow, freeze: false) { create(:ai_catalog_flow, :public) }
    let_it_be(:foundational_flow, freeze: false) do
      create(:ai_catalog_flow, :public, verification_level: :gitlab_maintained)
    end

    let_it_be(:project_flow_consumer, freeze: false) do
      create(:ai_catalog_item_consumer, project: project, item: project_flow)
    end

    let_it_be(:sibling_project_flow_consumer, freeze: false) do
      create(:ai_catalog_item_consumer, project: project, item: sibling_project_flow)
    end

    let_it_be(:external_flow_consumer, freeze: false) do
  ...
  context 'when foundational_flow_reference is provided' do
    ...
    let_it_be(:flow_with_ref, freeze: false) do
    ...
    context 'when searching for beta foundational flow' do
      let_it_be(:beta_flow, freeze: false) do
        create(:ai_catalog_flow, foundational_flow_reference: 'example_beta_flow/v1')
      end

      let_it_be(:beta_flow_consumer, freeze: false) do
    ...
    context 'when searching for GA foundational flow' do
      let_it_be(:ga_flow, freeze: false) { create(:ai_catalog_flow, foundational_flow_reference: 'code_review/v1') }
      let_it_be(:ga_flow_consumer, freeze: false) { create(:ai_catalog_item_consumer, project: project, item: ga_flow) }
  ...
  context 'when configurable_for_project_id is provided' do
    let_it_be(:configurable_for_project, freeze: false) { create(:project, group: parent_group) }
    let_it_be(:public_flow_from_other_project, freeze: false) do
      create(:ai_catalog_flow, public: true, project: project)
    end

    let_it_be(:private_agent_owned_by_configurable_for_project, freeze: false) do
      create(:ai_catalog_agent, public: false, project: configurable_for_project)
    end

    let_it_be(:public_flow_consumer, freeze: false) do
      create(:ai_catalog_item_consumer, group: parent_group, item: public_flow_from_other_project)
    end

    let_it_be(:private_agent_consumer, freeze: false) do
  ...
  describe 'ordering' do
    let_it_be(:developer_project, freeze: false) { create(:project, group: parent_group, developers: user) }
    let_it_be(:reporter_project, freeze: false) { create(:project, group: parent_group, reporters: user) }
    ...
    let_it_be(:public_item, freeze: false) { create(:ai_catalog_item, public: true) }
    ...
    let_it_be(:private_item_consumer, freeze: false) do
  ...
  describe 'exclude_disabled_item_types' do
    ...
    let_it_be(:custom_flow, freeze: false) { create(:ai_catalog_flow, project: finder_project) }
    let_it_be(:foundational_flow, freeze: false) do
      create(:ai_catalog_flow, project: finder_project, foundational_flow_reference: 'code_review/v1')
    end

    let_it_be(:external_agent, freeze: false) { create(:ai_catalog_third_party_flow, project: finder_project) }
    let_it_be(:agent_item, freeze: false) { create(:ai_catalog_agent, project: finder_project) }
    let_it_be(:custom_flow_consumer, freeze: false) do
    ...
    let_it_be(:external_agent_consumer, freeze: false) do
