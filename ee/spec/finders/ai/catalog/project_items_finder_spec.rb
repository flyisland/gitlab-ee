...
RSpec.describe Ai::Catalog::ProjectItemsFinder, feature_category: :workflow_catalog do
  ...
  let_it_be(:public_flow_of_sibling_project) { create(:ai_catalog_flow, :public, project: sibling_project) }
  let_it_be(:private_flow_of_sibling_project) { create(:ai_catalog_flow, :private, project: sibling_project) }
  ...
  context 'when `all_available` param is `true`' do
    ...
    context 'when ai_catalog_items_priority_order flag is disabled' do
      before do
        stub_feature_flags(ai_catalog_items_priority_order: false)
      end

      it 'orders by id desc with no boosting for gitlab_maintained items' do
        is_expected.to eq([
          public_flow_of_sibling_project,
          foundational_flow,
          public_flow_of_other_project,
