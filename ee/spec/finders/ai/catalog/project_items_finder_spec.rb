...
RSpec.describe Ai::Catalog::ProjectItemsFinder, feature_category: :workflow_catalog do
  ...
  let_it_be(:project, freeze: false) { create(:project, developers: user, group: group) }
  let_it_be(:other_project, freeze: false) { create(:project, developers: user) }
  let_it_be(:sibling_project, freeze: false) { create(:project, group: group, developers: user) }

  let_it_be(:public_flow, freeze: false) { create(:ai_catalog_flow, :public, project: project) }
  let_it_be(:private_agent, freeze: false) { create(:ai_catalog_agent, :private, project: project) }
  let_it_be(:private_third_party_flow, freeze: false) do
    create(:ai_catalog_third_party_flow, project: project)
  end

  let_it_be(:deleted_flow, freeze: false) { create(:ai_catalog_flow, :soft_deleted, project: project) }
  let_it_be(:private_flow_of_other_project, freeze: false) do
    create(:ai_catalog_flow, :private, project: other_project)
  end

  let_it_be(:public_flow_of_other_project, freeze: false) { create(:ai_catalog_flow, :public, project: other_project) }
  let_it_be(:public_flow_of_other_org, freeze: false) do
    create(:ai_catalog_flow, :public, organization: create(:organization))
  end

  let_it_be(:deleted_public_flow_of_other_project, freeze: false) do
  ...
  let_it_be(:public_flow_of_sibling_project, freeze: false) do
    create(:ai_catalog_flow, :public, project: sibling_project)
  end

  let_it_be(:private_flow_of_sibling_project, freeze: false) do
  ...
  context 'when filtering by `search`' do
    let_it_be(:agent_with_name_match, freeze: false) do
  ...
  context 'when `all_available` param is `true`' do
    ...
    it 'also returns public items within the same org and foundational items, with gitlab_maintained at top' do
      is_expected.to eq([
        foundational_flow,
    ...
    context 'when the top-level group restricts the AI Catalog to its hierarchy' do
      ...
      it 'also returns only public items owned within the group hierarchy, and foundational items' do
