...
RSpec.describe IterationsFinder, feature_category: :team_planning do
  ...
  context 'without permissions' do
    ...
    context 'when private group is nested in public group' do
      ...
      let_it_be(:iteration_cadence_1) { create(:iterations_cadence, group: public_group, active: true, duration_in_weeks: 3, title: 'three week iterations') }
      let_it_be(:iteration_cadence_2) { create(:iterations_cadence, group: create(:group, :private, parent: public_group), active: true, duration_in_weeks: 4, title: 'four week iterations') }

      let_it_be(:root_group_iteration_public) { create(:current_iteration, iterations_cadence: iteration_cadence_1, start_date: 1.day.ago, due_date: 2.days.from_now, group: public_group) }
      let_it_be(:subgroup_iteration_private) { create(:current_iteration, :skip_future_date_validation, iterations_cadence: iteration_cadence_2, title: 'subgroup test', start_date: 2.days.ago, due_date: Date.today, updated_at: 5.days.ago) }
