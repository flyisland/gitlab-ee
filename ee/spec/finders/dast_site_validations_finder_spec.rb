...
RSpec.describe DastSiteValidationsFinder, feature_category: :dynamic_application_security_testing do
  ...
  describe '#execute' do
    it 'returns all dast_site_validation_validations most recent first' do
      expect(finder).to eq([dast_site_validation_3, dast_site_validation_2, dast_site_validation_1])
    end
    ...
    context 'filtering by most_recent' do
      ...
      it 'returns the matching dast_site_validations' do
        expect(finder).to match_array([dast_site_validation_3, dast_site_validation_2])
      end

      it 'allows the results to be easily re-ordered' do
        relation = finder.reorder(id: :asc)
    ...
    context 'filtering by project_id' do
      ...
      it 'returns the matching dast_site_validations' do
        expect(finder).to eq([dast_site_validation_2])
      end
    ...
    context 'filtering by url_base' do
      ...
      it 'returns the matching dast_site_validations' do
        expect(finder).to match_array([dast_site_validation_3, dast_site_validation_1])
      end
    end
    ...
    context 'filtering by *' do
      ...
      it 'returns the matching dast_site_validations' do
        expect(finder).to eq([dast_site_validation_3])
      end
    end
  end
end
