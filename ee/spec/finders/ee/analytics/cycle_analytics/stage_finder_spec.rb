...
RSpec.describe Analytics::CycleAnalytics::StageFinder, feature_category: :value_stream_management do
  ...
  subject(:finder) { described_class.new(parent: group, stage_id: stage_id[:id]).execute }
  ...
  context 'when looking up in-memory default stage by name exists' do
    it { expect(finder).not_to be_persisted }
    it { expect(finder.name).to eq(stage_id[:id]) }
  end
  ...
  context 'when in-memory default stage cannot be found' do
    ...
    it { expect { finder }.to raise_error(ActiveRecord::RecordNotFound) }
  end
  ...
  context 'when persisted stage exists' do
    ...
    it { expect(finder).to be_persisted }
    it { expect(finder.name).to eq(stage.name) }
  end
