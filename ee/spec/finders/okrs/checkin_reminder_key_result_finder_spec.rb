...
RSpec.describe Okrs::CheckinReminderKeyResultFinder, feature_category: :team_planning do
  ...
  describe '#execute' do
    ...
    subject(:key_results) { finder.execute }

    it 'returns an array of work items' do
      expect(key_results.map(&:class)).to match_array([WorkItem, WorkItem])

      # returns only work items in the opened state
      expect(key_results).to match_array([key_result1, key_result6])
      expect(key_results).not_to include(key_result2)

      # returns only work items of the type key_result
      expect(key_results).not_to include(sub_objective1)
      expect(key_results).not_to include(sub_objective2)
      ...
      expect(key_results).not_to include(key_result5)

      # does not include work items that are not descendents
      expect(key_results).not_to include(top_level_objective1)
      expect(key_results).not_to include(top_level_objective2)
      expect(key_results).not_to include(top_level_objective3)

      # does not return work items when last_reminder_sent_at is after the configured days in the past
      expect(key_results).not_to include(key_result4)
  ...
  describe '#frequency_reminder_date' do
    subject(:reminder_date) { finder.send(:frequency_reminder_date) }
