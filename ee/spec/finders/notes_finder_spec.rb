...
RSpec.describe NotesFinder, feature_category: :portfolio_management do
  ...
  describe '#execute' do
    ...
    context 'when notes on public epic in a public group' do
      ...
      let_it_be(:public_group, freeze: false) { epic.group }
      let_it_be(:guest_member, freeze: false) { create(:user) }
      let_it_be(:reporter_member, freeze: false) { create(:user) }
      let_it_be(:guest_group_member, freeze: false) { create(:group_member, :guest, user: guest_member, source: public_group) }
      let_it_be(:reporter_group_member, freeze: false) { create(:group_member, :reporter, user: reporter_member, source: public_group) }
      let_it_be(:internal_note, freeze: false) { create(:note_on_epic, noteable: epic, internal: true) }
