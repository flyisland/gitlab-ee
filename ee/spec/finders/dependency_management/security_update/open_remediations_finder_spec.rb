...
RSpec.describe DependencyManagement::SecurityUpdate::OpenRemediationsFinder,
  ...
  describe '#source_branches' do
    subject(:finder) { described_class.new(project: project) }

    context 'when there is no dependency management service account' do
      it 'returns an empty array' do
        expect(finder.source_branches).to eq([])
      end
    end
    ...
    context 'when there is a dependency management service account' do
      ...
      it 'returns an empty array when there are no open merge requests' do
        expect(finder.source_branches).to eq([])
      end

      it 'returns the source branches of the open remediation merge requests' do
        ...
        expect(finder.source_branches).to contain_exactly('dependency-management/rails-6.x')
      end
    end
  end
end
