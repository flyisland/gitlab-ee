...
RSpec.describe GroupSamlIdentityFinder, feature_category: :system_access do
  ...
  describe ".find_by_group_and_uid" do
    ...
    it "returns nil when no saml_provider exists" do
      ...
      expect(described_class.find_by_group_and_uid(group: group, uid: identity.extern_uid)).to be_nil
    end
  end
  ...
  describe "#find_linked" do
    ...
    it "returns nil when no saml_provider exists" do
      ...
      expect(subject.find_linked(group: group)).to be_nil
    end

    it "returns nil when group is nil" do
      expect(subject.find_linked(group: nil)).to be_nil
    end
  end
