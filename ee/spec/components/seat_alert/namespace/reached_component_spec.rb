...
RSpec.describe SeatAlert::Namespace::ReachedComponent, :aggregate_failures, :saas, feature_category: :seat_cost_management do
  let(:group) do
  ...
  it 'renders the reached title' do
  ...
  it 'renders the correct alert class for dismissal' do
    expect(component).to have_css('.js-reached-seat-count-threshold')
  end

  it 'renders the correct alert testid' do
    expect(component).to have_css("[data-testid='reached-seat-count-threshold-alert']")
  end

  it 'renders the dismiss button with correct testid' do
    expect(component).to have_css("[data-testid='reached-seat-count-threshold-alert-dismiss']")
  end

  it 'renders the correct feature ID for tracking dismissals' do
    expect(component).to have_css("[data-feature-id='reached_seat_count_threshold']")
  end

  context 'when restricted access is enabled' do
    let(:seat_control) { :block_overages }

    it 'renders the total seat count' do
      expect(component).to have_content('all 100 seats')
    end
