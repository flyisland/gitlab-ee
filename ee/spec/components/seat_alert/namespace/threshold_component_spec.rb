...
RSpec.describe SeatAlert::Namespace::ThresholdComponent, :aggregate_failures, :saas, feature_category: :seat_cost_management do
  let(:group) do
  ...
  it 'renders the correct alert class for dismissal' do
    expect(component).to have_css('.js-approaching-seat-count-threshold')
  end

  it 'renders the correct alert testid' do
    expect(component).to have_css("[data-testid='approaching-seat-count-threshold-alert']")
  end

  it 'renders the dismiss button with correct testid' do
    expect(component).to have_css("[data-testid='approaching-seat-count-threshold-alert-dismiss']")
  end

  it 'renders the correct feature ID for tracking dismissals' do
    expect(component).to have_css("[data-feature-id='approaching_seat_count_threshold']")
  end
