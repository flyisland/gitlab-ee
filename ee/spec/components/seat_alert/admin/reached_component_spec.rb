...
RSpec.describe SeatAlert::Admin::ReachedComponent, :aggregate_failures, feature_category: :seat_cost_management do
  ...
  before do
    ...
    allow(component_instance).to receive_messages(render?: true, remaining_user_count: 0, total_user_count: 100)
  end
  ...
  context 'with a single seat license' do
    before do
      allow(component_instance).to receive_messages(remaining_user_count: 0, total_user_count: 1)
    end
    ...
    context 'when restricted access is enabled' do
      ...
      it 'renders singular form for total seats' do
        expect(component).to have_content(/all 1 seat\b/)
      end
    end
