...
RSpec.describe DuoChatPanel::TrialExpiredComponent, :aggregate_failures, feature_category: :duo_chat do
  ...
  describe 'rendering' do
    ...
    it 'renders the is_trial_expired data attribute on the element' do
      is_expected.to have_css('#duo-chat-panel-empty-state[data-is-trial-expired="true"]')
    end
  end
  ...
  describe '#data', :saas_gitlab_com_subscriptions do
    ...
    context 'when source is nil' do
      ...
      it 'returns can_buy_addon as "false"' do
        is_expected.to have_css('#duo-chat-panel-empty-state[data-can-buy-addon="false"]')
      end

      it 'does not include buy_addon_path' do
        is_expected.not_to have_css('#duo-chat-panel-empty-state[data-buy-addon-path]')
      end
    end
    ...
    context 'when user can edit billing' do
      ...
      it 'returns can_buy_addon as "true"' do
        is_expected.to have_css('#duo-chat-panel-empty-state[data-can-buy-addon="true"]')
      end
      ...
      it 'returns purchase URL for Premium plan' do
        ...
        is_expected.to have_css(
          "#duo-chat-panel-empty-state[data-buy-addon-path=\"#{expected_url}\"]"
        )
      end
    end
    ...
    context 'when premium plan is not available' do
      ...
      it 'returns can_buy_addon as "true" with fallback pricing URL' do
        is_expected.to have_css('#duo-chat-panel-empty-state[data-can-buy-addon="true"]')
        is_expected.to have_css(
          "#duo-chat-panel-empty-state[data-buy-addon-path=\"#{::Gitlab::Routing.url_helpers.promo_pricing_url}\"]"
        )
      end
    end
  end
  ...
  describe '#data on self-managed' do
    ...
    it 'returns can_buy_addon as "false" even when user can edit billing' do
      is_expected.to have_css('#duo-chat-panel-empty-state[data-can-buy-addon="false"]')
    end
