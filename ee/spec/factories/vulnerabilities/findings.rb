...
FactoryBot.define do
  ...
  factory :vulnerabilities_finding, class: 'Vulnerabilities::Finding' do
    ...
    report_type { :sast }
    project_tracked_context { project.security_project_tracked_contexts.first }
    uuid do
    ...
    # new_uuid is the context_unaware UUID, so tracked_context is always nil.
    new_uuid do
      Security::VulnerabilityUUID.generate(
        report_type: report_type,
        primary_identifier_fingerprint: primary_identifier.fingerprint,
    ...
    trait :with_gss_rule_identifier do
      transient do
        gss_rule_name { 'AWS' }
        gss_rule_value { 'AWS' }
      end
      ...
      after(:build) do |finding, evaluator|
        ...
        metadata['identifiers'] << {
          'type' => Security::SecretDetection::RuleIdentifiers::GSS_TYPE,
          'name' => evaluator.gss_rule_name,
          'value' => evaluator.gss_rule_value
        }
        finding.raw_metadata = metadata.to_json
      end
    end
    ...
    ::Enums::Vulnerability.report_types.each_key do |security_report_type|
