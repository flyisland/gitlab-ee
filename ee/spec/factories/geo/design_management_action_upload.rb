...
FactoryBot.define do
  factory :geo_design_management_action_upload, class: 'Geo::DesignManagementActionUpload' do
    ...
    transient do
      parent_model { create(:design_action) } # rubocop:disable RSpec/FactoryBot/InlineAssociation -- See above
    end

    initialize_with do
      upload = create(:upload, :with_file, model: parent_model)
      Geo::DesignManagementActionUpload.find(upload.id)
    end
    ...
    # (Geo::DesignManagementActionUploadState), not on the model itself.
    trait(:verification_succeeded) do
      after(:create) do |instance|
        state = instance.design_management_action_upload_state
        state.verification_checksum = 'abc'
        state.verification_state = Geo::DesignManagementActionUpload.verification_state_value(:verification_succeeded)
        state.save!
      end
    end
    ...
    trait(:verification_failed) do
      after(:create) do |instance|
        ...
        state.verification_state = Geo::DesignManagementActionUpload.verification_state_value(:verification_failed)
        state.save!
      end
    end
