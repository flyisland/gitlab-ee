...
FactoryBot.define do
  factory :geo_achievement_upload, class: 'Geo::AchievementUpload' do
    ...
    transient do
      parent_model { create(:achievement) } # rubocop:disable RSpec/FactoryBot/InlineAssociation -- See above
    end

    initialize_with do
      upload = create(:upload, :with_file, model: parent_model)
      Geo::AchievementUpload.find(upload.id)
    end
    ...
    # (Geo::AchievementUploadState), not on the model itself.
    trait(:verification_succeeded) do
      after(:create) do |instance|
        state = instance.achievement_upload_state
        state.verification_checksum = 'abc'
        state.verification_state = Geo::AchievementUpload.verification_state_value(:verification_succeeded)
        state.save!
      end
    end
    ...
    trait(:verification_failed) do
      after(:create) do |instance|
        ...
        state.verification_state = Geo::AchievementUpload.verification_state_value(:verification_failed)
        state.save!
      end
    end
