...
FactoryBot.define do
  factory :geo_project_topic_upload, class: 'Geo::ProjectTopicUpload' do
    ...
    transient do
      parent_model { create(:topic) } # rubocop:disable RSpec/FactoryBot/InlineAssociation -- See above
    end

    initialize_with do
      upload = create(:upload, :with_file, model: parent_model)
      Geo::ProjectTopicUpload.find(upload.id)
    end
    ...
    # (Geo::ProjectTopicUploadState), not on the model itself.
    trait(:verification_succeeded) do
      after(:create) do |instance|
        state = instance.project_topic_upload_state
        state.verification_checksum = 'abc'
        state.verification_state = Geo::ProjectTopicUpload.verification_state_value(:verification_succeeded)
        state.save!
      end
    end
    ...
    trait(:verification_failed) do
      after(:create) do |instance|
        ...
        state.verification_state = Geo::ProjectTopicUpload.verification_state_value(:verification_failed)
        state.save!
      end
    end
