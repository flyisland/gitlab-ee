...
FactoryBot.define do
  factory :geo_group_upload, class: 'Geo::GroupUpload' do
    ...
    transient do
      parent_model { create(:group) } # rubocop:disable RSpec/FactoryBot/InlineAssociation -- See above
    end

    initialize_with do
      upload = create(:upload, :with_file, model: parent_model)
      Geo::GroupUpload.find(upload.id)
    end
    ...
    # (Geo::GroupUploadState), not on the model itself.
    trait(:verification_succeeded) do
      after(:create) do |instance|
        state = instance.group_upload_state
        state.verification_checksum = 'abc'
        state.verification_state = Geo::GroupUpload.verification_state_value(:verification_succeeded)
        state.save!
      end
    end
    ...
    trait(:verification_failed) do
      after(:create) do |instance|
        ...
        state.verification_state = Geo::GroupUpload.verification_state_value(:verification_failed)
        state.save!
      end
    end
