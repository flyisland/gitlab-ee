...
FactoryBot.define do
  factory :geo_personal_snippet_upload, class: 'Geo::PersonalSnippetUpload' do
    ...
    transient do
      parent_model { create(:personal_snippet) } # rubocop:disable RSpec/FactoryBot/InlineAssociation -- See above
    end

    initialize_with do
      upload = create(:upload, :with_file, model: parent_model)
      Geo::PersonalSnippetUpload.find(upload.id)
    end
    ...
    # (Geo::PersonalSnippetUploadState), not on the model itself.
    trait(:verification_succeeded) do
      after(:create) do |instance|
        state = instance.personal_snippet_upload_state
        state.verification_checksum = 'abc'
        state.verification_state = Geo::PersonalSnippetUpload.verification_state_value(:verification_succeeded)
        state.save!
      end
    end
    ...
    trait(:verification_failed) do
      after(:create) do |instance|
        ...
        state.verification_state = Geo::PersonalSnippetUpload.verification_state_value(:verification_failed)
        state.save!
      end
    end
