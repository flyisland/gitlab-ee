...
FactoryBot.define do
  factory :geo_dependency_list_export_part_upload, class: 'Geo::DependencyListExportPartUpload' do
    ...
    transient do
      parent_model { create(:dependency_list_export_part) } # rubocop:disable RSpec/FactoryBot/InlineAssociation -- See above
    end

    initialize_with do
      upload = create(:upload, :with_file, model: parent_model)
      Geo::DependencyListExportPartUpload.find(upload.id)
    end
    ...
    # (Geo::DependencyListExportPartUploadState), not on the model itself.
    trait(:verification_succeeded) do
      after(:create) do |instance|
        state = instance.dependency_list_export_part_upload_state
        state.verification_checksum = 'abc'
        state.verification_state = Geo::DependencyListExportPartUpload.verification_state_value(:verification_succeeded)
        state.save!
      end
    end
    ...
    trait(:verification_failed) do
      after(:create) do |instance|
        ...
        state.verification_state = Geo::DependencyListExportPartUpload.verification_state_value(:verification_failed)
        state.save!
      end
    end
