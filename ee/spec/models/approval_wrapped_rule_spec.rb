...
RSpec.describe ApprovalWrappedRule, feature_category: :code_review_workflow do
  ...
  describe '#approved_approvers' do
    ...
    context 'when using shared context optimization' do
      let!(:approval1) { create(:approval, merge_request: merge_request, user: approver1) }
      let(:context) { Approvals::Context.new(merge_request) }
      ...
      context 'when initialized with context' do
        subject(:wrapped_rule) { described_class.new(merge_request, rule, context: context) }

        it 'uses the context for approved_approvers calculation' do
          expect(wrapped_rule.approved_approvers).to contain_exactly(approver1)
        end

        it 'does not query approvals table when context is provided' do
          # Pre-warm caches
          wrapped_rule.approvers
          context.overall_approver_ids

          expect do
            wrapped_rule.approved_approvers
          end.not_to exceed_query_limit(0).for_model(Approval)
        end

        context 'when overall_approver_ids_optimization feature flag is disabled' do
          before do
            stub_feature_flags(overall_approver_ids_optimization: false)
          end

          it 'queries approvals table even when context is provided' do
            # Pre-warm any caches
            wrapped_rule.approvers

            expect do
              expect(wrapped_rule.approved_approvers).to contain_exactly(approver1)
            end.to exceed_query_limit(0).for_model(Approval)
          end
        end
      end
    end
  ...
  describe '#approvers' do
    ...
    before do
      allow(merge_request).to receive_messages(committers: committer_relation, committers_to_filter_from_approvers: committer_relation, committer_ids_to_filter_from_approvers: committer_relation.select(:id))
    ...
    context 'when author is also a committer' do
      before do
        allow(merge_request).to receive_messages(committers: author_relation, committers_to_filter_from_approvers: author_relation, committer_ids_to_filter_from_approvers: author_relation.select(:id))
  ...
  describe '.wrap' do
    context 'when context is provided' do
      let(:context) { instance_double(Approvals::Context) }

      it 'passes context to regular rules' do
        expect(described_class).to receive(:new).with(merge_request, rule, context: context).and_call_original

        described_class.wrap(merge_request, rule, context: context)
      end

      it 'passes context to code owner rules' do
        code_owner_rule = create(:code_owner_rule, merge_request: merge_request)

        expect(ApprovalWrappedCodeOwnerRule).to receive(:new).with(merge_request, code_owner_rule, context: context).and_call_original

        described_class.wrap(merge_request, code_owner_rule, context: context)
      end

      it 'passes context to any approver rules' do
        any_approver_rule = create(:any_approver_rule, merge_request: merge_request)

        expect(ApprovalWrappedAnyApproverRule).to receive(:new).with(merge_request, any_approver_rule, context: context).and_call_original

        described_class.wrap(merge_request, any_approver_rule, context: context)
      end
    end
  end
end
