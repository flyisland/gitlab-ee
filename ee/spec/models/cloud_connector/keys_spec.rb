...
RSpec.describe CloudConnector::Keys, :models, feature_category: :system_access do
  ...
  shared_examples 'serving valid keys' do
    ...
    context 'when there are records but the key is nil' do
      let_it_be(:key_record, freeze: false) { create(:cloud_connector_keys, secret_key: nil) }

      it { is_expected.to be_empty }
    end

    context 'when there are valid records' do
      let_it_be(:key_record, freeze: false) { create_list(:cloud_connector_keys, 2) }
  ...
  describe '.ordered_by_date' do
    ...
    let_it_be(:first_key, freeze: false) { create(:cloud_connector_keys, created_at: Time.current - 1.minute) }
    let_it_be(:second_key, freeze: false) { create(:cloud_connector_keys) }
  ...
  describe '.current' do
    ...
    context 'when there are records' do
      ...
      context 'and they are valid' do
        let_it_be(:key_record, freeze: false) { create(:cloud_connector_keys) }
  ...
  describe '.trim!' do
    ...
    context 'with a single key' do
      let_it_be(:key, freeze: false) { create(:cloud_connector_keys) }
  ...
  context 'when accessing keys stored using a different key provider' do
    let_it_be(:legacy_keys_class, freeze: false) do
