...
RSpec.describe ResourceWeightEvent, feature_category: :team_planning do
  ...
  describe '.aliased_for_timebox_report', :freeze_time do
    let_it_be(:event) { create(:resource_weight_event, issue: issue1) }
    ...
    it 'returns correct values with aliased names', :aggregate_failures do
      ...
      expect(scope.action).to be_nil
      expect(scope.created_at).to eq(event.reload.created_at)
    end
  end
  ...
  describe '.created_after' do
    ...
    let_it_be(:event1) { create(:resource_weight_event, issue: issue1, created_at: 1.day.ago) }
    let_it_be(:event2) { create(:resource_weight_event, issue: issue2, created_at: 2.days.ago) }
    let_it_be(:event3) { create(:resource_weight_event, issue: issue2, created_at: 3.days.ago) }
