...
RSpec.describe Note, feature_category: :team_planning do
  ...
  describe '.with_noteable_type' do
    ...
    let_it_be(:note2, freeze: false) { create(:note, noteable: epic, project: nil) }
  ...
  describe '.with_noteable_ids' do
    ...
    let_it_be(:note2, freeze: false) { create(:note, noteable: issue2, project: issue2.project) }
    let_it_be(:note3, freeze: false) { create(:note, noteable: issue2, project: issue2.project) }
  ...
  describe '.with_note' do
    ...
    let_it_be(:note2, freeze: false) { create(:note, noteable: issue, project: issue.project, note: 'another note') }
  ...
  describe '.distinct_on_noteable_id' do
    ...
    let_it_be(:note2, freeze: false) { create(:note, noteable: issue, project: issue.project, created_at: 1.day.ago) }
    # Making sure epic id doesn't match issue id
    let_it_be(:epic, freeze: false) do
      create(:epic, id: non_existing_record_id)
    end

    let_it_be(:note3, freeze: false) { create(:note, noteable: epic, project: nil) }
  ...
  describe '.order_by_noteable_latest_first' do
    ...
    let_it_be(:note2, freeze: false) { create(:note, noteable: issue, project: issue.project, created_at: 1.day.ago) }
    let_it_be(:note3, freeze: false) { create(:note, noteable: issue, project: issue.project, created_at: 1.day.ago) }
