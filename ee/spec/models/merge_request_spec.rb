...
RSpec.describe MergeRequest, feature_category: :code_review_workflow do
  ...
  describe '#can_resolve_with_ai?' do
    ...
    context 'when the user cannot push code to the project' do
      ...
      before do
        allow(::Ai::Catalog).to receive(:user_can_access_experimental_and_beta_features?)
          .with(guest).and_return(true)
      end

      it { is_expected.to be false }
    end

    context 'when the user cannot access experimental and beta AI features' do
      before do
        allow(::Ai::Catalog).to receive(:user_can_access_experimental_and_beta_features?)
  ...
  shared_examples_for 'security reports in child pipelines' do |report_type|
    context 'when the child pipeline has reports' do
      let_it_be(:merge_request, freeze: false) { create(:ee_merge_request, source_project: project) }
  ...
  describe '#synchronize_approval_rules_from_target_project' do
    ...
    context 'when syncing policy-linked approval rules' do
      let_it_be(:linked_policy_rule) { create(:approval_policy_rule) }
      let_it_be(:unlinked_policy_rule) { create(:approval_policy_rule) }
  ...
  describe '#sync_project_approval_rules_for_approval_policy_rules' do
    ...
    context 'when merge request is already merged' do
      let_it_be(:merge_request, freeze: false) { create(:ee_merge_request, source_project: project, state: :merged) }
  ...
  describe '#reset_required_approvals' do
    ...
    let_it_be(:license_project_rule) do
      create(:approval_project_rule, :license_scanning, :for_all_protected_branches,
        project: project, approvals_required: 2, security_orchestration_policy_configuration: policy_configuration)
    end
  ...
  context 'scopes' do
    let_it_be(:merge_request, freeze: false) { create(:ee_merge_request) }
  ...
  describe '#comparison_base_pipeline' do
    ...
    let_it_be(:base_pipeline, freeze: false) do
      create(:ci_pipeline,
        :with_test_reports,
        project: project,
        ref: merge_request.target_branch,
    ...
    context 'when the service class is `Vulnerabilities::CompareSecurityReportsService`' do
      ...
      context 'when there are merge base pipelines' do
        ...
        let_it_be(:most_recent_merge_base_pipeline, freeze: false) do
          create(:ci_pipeline,
            :success,
            project: project,
            ref: merge_request.target_branch,
  ...
  describe '#notify_approvers' do
    ...
    let_it_be(:approvers, freeze: false) { create_list(:user, 2) }
    let_it_be(:approval_rule, freeze: false) do
