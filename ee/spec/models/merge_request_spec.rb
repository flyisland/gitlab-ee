...
RSpec.describe MergeRequest, feature_category: :code_review_workflow do
  ...
  describe '#can_resolve_with_ai?' do
    ...
    context 'when the merge request can be merged' do
      before do
        allow(merge_request).to receive(:cannot_be_merged?).and_return(false)
      end
  ...
  describe '#synchronize_approval_rules_from_target_project' do
    ...
    context 'when convergent_policy_rule_sync is enabled' do
      ...
      context 'when convergent_policy_rule_sync is disabled' do
        ...
        it 'syncs all report_approver rules regardless of link status' do
          synchronize_approval_rules

          synced_names = merge_request.approval_rules.pluck(:name)
          expect(synced_names).to include('Linked rule', 'Unlinked rule')
        end
      end
    end
  end
