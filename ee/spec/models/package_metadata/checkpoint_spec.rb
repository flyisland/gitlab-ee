...
RSpec.describe PackageMetadata::Checkpoint, type: :model, feature_category: :software_composition_analysis do
  ...
  describe '.for_advisories' do
    ...
    context 'when there are no advisory checkpoints' do
      let!(:license_checkpoint) { create(:pm_checkpoint, data_type: 'licenses') }
      let!(:cve_checkpoint) { create(:pm_checkpoint, data_type: 'cve_enrichment') }

      it { is_expected.to be_empty }
    end

    context 'when there is one advisory checkpoint' do
      let!(:advisory_checkpoint) { create(:pm_checkpoint, data_type: 'advisories') }
      let!(:license_checkpoint) { create(:pm_checkpoint, data_type: 'licenses') }
    ...
    context 'when there are multiple advisory checkpoints' do
      let!(:advisory_checkpoint1) { create(:pm_checkpoint, data_type: 'advisories', purl_type: 'npm') }
      let!(:advisory_checkpoint2) { create(:pm_checkpoint, data_type: 'advisories', purl_type: 'maven') }
      let!(:license_checkpoint) { create(:pm_checkpoint, data_type: 'licenses') }
  ...
  describe '.with_purl_types' do
    let!(:npm_checkpoint) { create(:pm_checkpoint, purl_type: 'npm') }
    let!(:maven_checkpoint) { create(:pm_checkpoint, purl_type: 'maven') }
    let!(:pypi_checkpoint) { create(:pm_checkpoint, purl_type: 'pypi') }
  ...
  describe '.synced_since' do
    let!(:old_checkpoint) { create(:pm_checkpoint, sequence: 100) }
    let!(:middle_checkpoint) { create(:pm_checkpoint, sequence: 500) }
    let!(:recent_checkpoint) { create(:pm_checkpoint, sequence: 1000) }
