...
RSpec.describe GroupWiki, feature_category: :wiki do
  it_behaves_like 'wiki model' do
    ...
    describe '#after_wiki_activity' do
      context 'when a group_wiki_repository exists', :freeze_time do
        it 'updates last_repository_updated_at' do
          expect { subject.send(:after_wiki_activity) }
            .to change { wiki_container.group_wiki_repository.reload.last_repository_updated_at }
            .to(Time.current)
        end
      end

      context 'when there is no group_wiki_repository' do
        before do
          allow(wiki_container).to receive(:group_wiki_repository).and_return(nil)
        end

        it 'does nothing' do
          expect { subject.send(:after_wiki_activity) }.not_to raise_error
        end
      end
      ...
      context 'when the database is read only' do
        ...
        it 'does nothing' do
          expect { subject.send(:after_wiki_activity) }.not_to change {
            wiki_container.group_wiki_repository.reload.last_repository_updated_at
          }
        end
      end
    end
