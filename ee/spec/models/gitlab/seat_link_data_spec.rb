...
RSpec.describe Gitlab::SeatLinkData do
  ...
  describe '.to_json' do
    ...
    context 'when the instance has add-on purchases' do
      before do
        ...
        purchase = create(:gitlab_subscription_add_on_purchase, :self_managed, :secrets_manager, quantity: 3)
        create(:gitlab_subscription_user_add_on_assignment, add_on_purchase: purchase)
      end
      ...
      it 'includes the add-on metrics in the payload' do
        expect(subject.as_json).to include(
          add_on_metrics: match_array([
            ...
            { add_on_type: 'secrets_manager', purchased_seats: 3, assigned_seats: 1 }
          ])
        )
      end
    end

    context 'when the instance has gitlab_credits add-on purchase' do
      before do
        create(:gitlab_subscription_add_on_purchase, :self_managed, :gitlab_credits, quantity: 1)
      end

      it 'reports purchased_seats as 0 for gitlab_credits' do
        expect(subject.as_json).to include(
          add_on_metrics: match_array([
            { add_on_type: 'gitlab_credits', purchased_seats: 0, assigned_seats: 0 }
          ])
        )
      end
    end

    context 'when the instance has both gitlab_credits and seat-based add-ons' do
      before do
        create(:gitlab_subscription_add_on_purchase, :self_managed, :gitlab_credits, quantity: 1)
        purchase = create(:gitlab_subscription_add_on_purchase, :self_managed, :duo_enterprise, quantity: 5)
        create(:gitlab_subscription_user_add_on_assignment, add_on_purchase: purchase)
      ...
      it 'reports purchased_seats as 0 for gitlab_credits and actual quantity for seat-based add-ons' do
        expect(subject.as_json).to include(
          add_on_metrics: match_array([
            { add_on_type: 'gitlab_credits', purchased_seats: 0, assigned_seats: 0 },
            { add_on_type: 'duo_enterprise', purchased_seats: 5, assigned_seats: 1 }
          ])
        )
      end
    end
  ...
  describe '#should_sync_seats?' do
    let_it_be_with_refind(:historical_data) { create(:historical_data, recorded_at: timestamp) }
