...
RSpec.describe GitlabSubscriptions::Features, feature_category: :subscription_management do
  describe 'License -> Plan mapping' do
    ...
    it 'maps every SaaS plan name to a plan_name_uid' do
      saas_plan_names = described_class::LICENSE_PLANS_TO_SAAS_PLANS.values.flatten

      expect(::Plan.uids_for_names(saas_plan_names).size).to eq(saas_plan_names.size)
    end
  end
  ...
  describe '.minimum_plan_for' do
    subject { described_class.minimum_plan_for(feature) }

    context 'with a starter feature' do
      let(:feature) { (described_class::STARTER_FEATURES - described_class::GLOBAL_FEATURES).first }

      it { is_expected.to eq(License::STARTER_PLAN) }
    end

    context 'with a premium feature' do
      let(:feature) { (described_class::PREMIUM_FEATURES - described_class::GLOBAL_FEATURES).first }

      it { is_expected.to eq(License::PREMIUM_PLAN) }
    end

    context 'with an ultimate feature' do
      let(:feature) { (described_class::ULTIMATE_FEATURES - described_class::GLOBAL_FEATURES).first }

      it { is_expected.to eq(License::ULTIMATE_PLAN) }
    end
