...
RSpec.describe WorkItems::Type, feature_category: :team_planning do
  describe '#widgets' do
    ...
    shared_examples 'work_item_type returning only licensed widgets' do
      ...
      # Only test widgets that are actually configured on this work item type's system-defined definition
      let(:type_widget_classes) do
        work_item_type.widgets(parent).map(&:widget_class)
      end

      let(:relevant_widget_classes) { widget_classes & type_widget_classes }

      subject(:returned_widgets) { work_item_type.widgets(parent) }

      context 'when feature is available' do
        it 'returns the associated licensed widget' do
          skip('widget not available on this type') if relevant_widget_classes.empty?

          relevant_widget_classes.each do |widget|
      ...
      context 'when feature is not available' do
        ...
        it 'does not return the unlicensed widgets' do
