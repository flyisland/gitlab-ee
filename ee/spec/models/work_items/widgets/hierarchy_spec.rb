  ...
  describe '#rolled_up_counts_by_type' do
    ...
    context 'when an epic has non-converted custom type descendants delegating to Issue' do
      ...
      let_it_be(:open_custom_child) do
        create(:work_item, :issue, :opened, project: project).tap do |wi|
          wi.update_column(:work_item_type_id, non_converted_custom_type.persistable_id)
        end
      end

      let_it_be(:closed_custom_child) do
        create(:work_item, :issue, :closed, project: project).tap do |wi|
          wi.update_column(:work_item_type_id, non_converted_custom_type.persistable_id)
        end
      end
