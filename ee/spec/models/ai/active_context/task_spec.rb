...
RSpec.describe Ai::ActiveContext::Task, feature_category: :global_search do
  ...
  describe 'scopes' do
    describe '.processable' do
      let!(:pending_no_deps) { create(:ai_active_context_task, connection: connection, status: :pending) }
      let!(:completed_task) { create(:ai_active_context_task, connection: connection, status: :completed) }
      let!(:pending_with_completed_dep) do
        create(:ai_active_context_task, connection: connection, status: :pending, depends_on: completed_task)
      end

      let!(:pending_with_pending_dep) do
        create(:ai_active_context_task, connection: connection, status: :pending, depends_on: pending_no_deps)
      end

      let!(:in_progress_task) { create(:ai_active_context_task, connection: connection, status: :in_progress) }
      let!(:failed_task) { create(:ai_active_context_task, connection: connection, status: :failed) }
    ...
    describe '.with_active_connection' do
      ...
      let!(:task_active) { create(:ai_active_context_task, connection: connection) }
      let!(:task_inactive) { create(:ai_active_context_task, connection: inactive_connection) }
  ...
  describe '.current' do
    ...
    context 'when there are processable tasks' do
      let!(:older_task) { create(:ai_active_context_task, connection: connection, status: :pending) }
      let!(:newer_task) { create(:ai_active_context_task, connection: connection, status: :pending) }
    ...
    context 'when processable task has inactive connection' do
      let!(:task) { create(:ai_active_context_task, connection: inactive_connection, status: :pending) }
