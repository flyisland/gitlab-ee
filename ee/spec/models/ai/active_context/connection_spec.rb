...
RSpec.describe Ai::ActiveContext::Connection, feature_category: :global_search do
  ...
  describe '#advanced_search_adapter_name' do
    context 'when adapter is elasticsearch' do
      ...
      it 'returns :elasticsearch' do
        expect(connection.advanced_search_adapter_name).to eq(:elasticsearch)
      end
    end
    ...
    context 'when adapter is opensearch' do
      ...
      it 'returns :opensearch' do
        expect(connection.advanced_search_adapter_name).to eq(:opensearch)
      end
    end

    context 'when adapter is not in ADVANCED_SEARCH_ADAPTERS' do
      let(:connection) { build(:ai_active_context_connection, :postgresql) }

      it 'returns nil' do
        expect(connection.advanced_search_adapter_name).to be_nil
      end
    end
  end
  ...
  describe '#reload_adapter' do
    ...
    context 'when a new connection is activated' do
      ...
      let_it_be(:new_connection, freeze: false) { create(:ai_active_context_connection, :inactive) }
