...
RSpec.describe Ai::SelfHostedModel, feature_category: :"self-hosted_models" do
  ...
  describe 'validation' do
    ...
    describe '#set_provider_from_identifier' do
      where(:identifier, :expected_provider) do
        'bedrock/example-model'       | :bedrock
        'vertex_ai/example-model'     | :vertex_ai
        'custom_openai/example-model' | :api
        ''                            | :api
        nil                           | :api
      end

      with_them do
        it 'sets provider based on identifier prefix on create' do
          model = create(:ai_self_hosted_model, identifier: identifier)

          expect(model.provider.to_sym).to eq(expected_provider)
        end
      end

      it 'updates provider if identifier changes on save' do
        model = create(:ai_self_hosted_model, identifier: 'custom_openai/model')

        expect { model.update!(identifier: 'bedrock/new-model') }
          .to change { model.provider.to_sym }.from(:api).to(:bedrock)
      end
    end
