...
RSpec.describe Ai::Catalog::Item, feature_category: :workflow_catalog do
  ...
  describe 'scopes' do
    ...
    describe '.within_group_hierarchy_or_foundational' do
      ...
      let_it_be(:project_in_hierarchy) { create(:project, group: top_level_group) }
      let_it_be(:project_in_subgroup) { create(:project, group: subgroup) }
      let_it_be(:project_outside_hierarchy) { create(:project) }

      let_it_be(:item_in_top_level) { create(:ai_catalog_item, project: project_in_hierarchy) }
      ...
      context 'when given a non-Group argument' do
        it 'raises ArgumentError' do
          expect { described_class.within_group_hierarchy_or_foundational(project_in_hierarchy) }
            .to raise_error(ArgumentError, 'top_level_group must be a root group')
        end
      end
    end
    ...
    describe '.order_by_catalog_priority' do
      let_it_be(:foundational_agent) do
        create(:ai_catalog_agent, id: ::Ai::FoundationalChatAgentsDefinitions::ITEMS[2][:global_catalog_id])
      end
