...
RSpec.describe Ai::Catalog::GoalTemplates::Developer::InitMrReviewInstructions, feature_category: :duo_agent_platform do
  ...
  describe '.template' do
    ...
    it 'names the target file' do
      expect(described_class.template).to include('.gitlab/duo/mr-review-instructions.yaml')
    end

    it 'describes the instruction schema keys', :aggregate_failures do
      template = described_class.template

      expect(template).to include('name')
      expect(template).to include('fileFilters')
      expect(template).to include('instructions')
    end
