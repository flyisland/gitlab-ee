...
RSpec.describe Ai::Catalog::GoalTemplates::Developer, feature_category: :duo_agent_platform do
  ...
  describe '.resolve' do
    ...
    context 'with mention event type' do
      ...
      it 'uses the resource URL when note_id is not provided' do
        result = described_class.resolve(
        ...
        resource_url = Gitlab::UrlBuilder.build(issue)
        expect(result).to include("@#{triggered_by_username} mentioned you in a note on this issue: #{resource_url}")
      ...
      it 'includes the correct resource name for a merge request' do
        ...
        expect(result).to include("@#{triggered_by_username} mentioned you in a note on this merge request:")
      end
    ...
    context 'when user input contains format string sequences' do
      ...
      it 'does not let user input substitute resource_url in template vars' do
        ...
        # The actual resource_url in template vars should be correctly resolved
        expect(result).to include("#{resource_url}#note_42")
      end
    end
  end
