...
RSpec.describe Ai::Catalog::GoalTemplates::Developer, feature_category: :duo_agent_platform do
  ...
  describe '.resolve' do
    ...
    context 'with mention event type' do
      ...
      it 'includes the triggering username in the goal', :aggregate_failures do
        ...
        expect(result).to include("@mention @#{triggered_by_username} so they are notified")
  ...
  describe '.handler_for' do
    it 'returns Mention::Default for :mention without a delivery mode' do
      expect(described_class.handler_for(:mention, issue))
        .to eq(Ai::Catalog::GoalTemplates::Developer::Mention::Default)
    end

    it 'returns Mention::Adapter for :mention with delivery_mode: :adapter' do
      expect(described_class.handler_for(:mention, issue, { delivery_mode: :adapter }))
        .to eq(Ai::Catalog::GoalTemplates::Developer::Mention::Adapter)
    end
