...
RSpec.describe Ai::Catalog::GoalTemplates::ReadinessScore, feature_category: :duo_agent_platform do
  ...
  describe '.resolve' do
    ...
    it 'is the bare work item URL so the flow fetch step can parse it' do
      expect(goal).to eq(Gitlab::UrlBuilder.build(work_item))
    end
