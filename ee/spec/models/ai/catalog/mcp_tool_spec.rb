...
RSpec.describe Ai::Catalog::McpTool, feature_category: :workflow_catalog do
  ...
  describe '.fixed_items' do
    ...
    it 'includes Orbit Knowledge Graph tools alongside Mcp::Tools::Manager tools' do
      names = described_class.fixed_items.pluck(:name)

      expect(names).to include(*::API::Orbit::McpHandlers::CallTool::KNOWN_TOOLS)
    end
    ...
    it 'prefixes Orbit tool titles so they are discoverable by name' do
      orbit_tool_names = ::API::Orbit::McpHandlers::CallTool::KNOWN_TOOLS +
    ...
    context 'when orbit_use_legacy_tools is disabled' do
      ...
      it 'excludes the legacy Orbit tools from the catalog' do
        ...
        expect(names).not_to include(*::API::Orbit::McpHandlers::ToolCatalog::ALL_LEGACY_TOOL_NAMES)
      end

      it 'still includes the Orbit command wrapper tools' do
        names = described_class.fixed_items.pluck(:name)
