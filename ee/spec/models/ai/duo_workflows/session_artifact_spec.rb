...
RSpec.describe Ai::DuoWorkflows::SessionArtifact, feature_category: :compliance_management do
  ...
  describe '.sync_from_workflow!' do
    let_it_be(:workflow, freeze: false) { create(:duo_workflows_workflow) }
  ...
  describe 'scopes' do
    ...
    let_it_be(:artifact_in_other) do
      create(:duo_workflow_session_artifact,
        workflow: workflow_in_other,
        workflow_definition: 'software_development',
        workflow_created_at: 3.days.ago)
    end

    describe '.in_namespace' do
      it 'includes artifacts from projects directly in the group' do
        expect(described_class.in_namespace(group)).to include(artifact_in_project)
      end

      it 'includes artifacts from projects in subgroups' do
        expect(described_class.in_namespace(group)).to include(artifact_in_subgroup)
      end

      it 'excludes artifacts from projects outside the namespace' do
        expect(described_class.in_namespace(group)).not_to include(artifact_in_other)
      end

      context 'with a namespace-level artifact' do
        ...
        it 'includes artifacts whose namespace_id matches the group' do
          expect(described_class.in_namespace(group)).to include(namespace_artifact)
        end

        it 'excludes namespace-level artifacts belonging to other groups' do
          expect(described_class.in_namespace(other_group)).not_to include(namespace_artifact)
        end
      end
    end
    ...
    describe '.for_project_namespace' do
      ...
      subject(:results) { described_class.for_project_namespace(project.project_namespace) }

      it 'includes artifacts belonging to the project' do
        expect(results).to include(artifact_in_project)
      end

      it 'excludes artifacts from a project in a subgroup of the same group' do
        expect(results).not_to include(artifact_in_subgroup)
      end

      it 'excludes artifacts from a project in another group' do
        expect(results).not_to include(artifact_in_other)
      end

      it 'excludes group-namespace-level artifacts' do
        expect(results).not_to include(namespace_artifact)
      end
    end
