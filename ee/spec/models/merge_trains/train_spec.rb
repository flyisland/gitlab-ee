...
RSpec.describe MergeTrains::Train, feature_category: :merge_trains do
  ...
  let_it_be(:target_project, freeze: false) { create(:project, :repository) }
  let_it_be(:merge_request, freeze: false) { create_merge_request_on_train(project: target_project) }
  ...
  describe '#refreshable_cars' do
    context 'when unstick_stuck_merge_requests feature flag is disabled' do
      ...
      it 'returns only active cars (excludes merging)' do
        expect(train.refreshable_cars).to eq(train.all_active_cars)
      end
    end
  end

  describe '#first_active_car' do
    subject { train.first_active_car }
    ...
    context 'when unstick_stuck_merge_requests is disabled' do
      before do
        stub_feature_flags(unstick_stuck_merge_requests: false)
      end

      it 'uses first_active_car? for merge readiness' do
        car = merge_request.merge_train_car
        car.update!(pipeline: create(:ci_pipeline, :success, project: target_project))

        expect(car.merge_ready_pipeline?).to eq(true)
      end
    end

    context 'when unstick_stuck_merge_requests is enabled' do
      it 'returns the first active car' do
        expect(subject).to eq(merge_request.merge_train_car)
      end
