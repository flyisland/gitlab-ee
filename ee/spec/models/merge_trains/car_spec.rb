...
RSpec.describe MergeTrains::Car, feature_category: :merge_trains do
  ...
  describe '#all_next_active' do
    subject { car.all_next_active }
  ...
  describe '#active?' do
    ...
    context 'when unstick_stuck_merge_requests feature flag is disabled' do
      ...
      context 'when status is merging' do
        ...
        it { expect(train_car.active?).to eq(false) }
      end

      context 'when status is idle' do
        let(:train_car) { create(:merge_train_car, :idle) }

        it { expect(train_car.active?).to eq(true) }
      end
    end
  end
