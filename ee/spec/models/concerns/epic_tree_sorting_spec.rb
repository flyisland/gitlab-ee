...
RSpec.describe EpicTreeSorting do
  ...
  context 'with standard positions' do
    let_it_be_with_reload(:epic_issue1) { create(:epic_issue, epic: base_epic, issue: issue1, relative_position: 10) }
    let_it_be_with_reload(:epic_issue2) { create(:epic_issue, epic: base_epic, issue: issue2, relative_position: 500) }
    let_it_be_with_reload(:epic_issue3) { create(:epic_issue, epic: base_epic, issue: issue3, relative_position: 1002) }
    let_it_be_with_reload(:epic1) { create(:epic, parent: base_epic, group: group, relative_position: 100) }
    let_it_be_with_reload(:epic2) { create(:epic, parent: base_epic, group: group, relative_position: 1000) }
    let_it_be_with_reload(:epic3) { create(:epic, parent: base_epic, group: group, relative_position: 1001) }

    describe '#relative_siblings' do
      def siblings(obj)
        obj.class.mover.context(obj).relative_siblings.pluck(:id, :object_type)
      end
  ...
  describe '#move_sequence' do
    let_it_be_with_reload(:epic_issue1) { create(:epic_issue, epic: base_epic, issue: issue1, relative_position: 1000) }
    let_it_be_with_reload(:epic_issue2) { create(:epic_issue, epic: base_epic, issue: issue2, relative_position: 1001) }
    let_it_be_with_reload(:epic_issue3) { create(:epic_issue, epic: base_epic, issue: issue3, relative_position: 1004) }
    let_it_be_with_reload(:epic1) { create(:epic, parent: base_epic, group: group, relative_position: 1002) }
    let_it_be_with_reload(:epic2) { create(:epic, parent: base_epic, group: group, relative_position: 1003) }
    let_it_be_with_reload(:epic3) { create(:epic, parent: base_epic, group: group, relative_position: 1005) }

    def move_sequence(range)
      dx = 500
      item.class.mover.context(item).send(:move_sequence, range.first, range.last, dx)
    end
