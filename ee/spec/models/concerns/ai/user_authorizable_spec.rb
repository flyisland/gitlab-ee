...
RSpec.describe Ai::UserAuthorizable, feature_category: :ai_abstraction_layer do
  ...
  describe '#allowed_to_use' do
    ...
    let_it_be(:gitlab_add_on, freeze: false) { create(:gitlab_subscription_add_on) }
    let_it_be(:expired_gitlab_purchase, freeze: false) do
    ...
    context 'when on Gitlab.com instance', :saas do
      ...
      include_examples 'checking assigned seats' do
        ...
        context 'when namespace access rules are in place' do
          let_it_be(:namespace, freeze: false) { active_gitlab_purchase.namespace }
          let_it_be(:ns, freeze: false) { namespace }
          let_it_be(:subgroup, freeze: false) { create(:group, parent: ns) }
      ...
      context 'when GitLab Credits access requires DAP namespace settings' do
        shared_examples 'DAP credits enforcement' do
          ...
          let_it_be(:dap_namespace, freeze: false) { create(:group) }
          let_it_be(:dap_credits_purchase, freeze: false) do
          ...
          before_all do
            dap_namespace.add_developer(user)
          end
    ...
    context 'when on Self managed instance' do
      ...
      include_examples 'checking assigned seats' do
        ...
        context 'when namespace access rules are in place' do
          ...
          let_it_be(:ns, freeze: false) { create(:group) }
      ...
      context 'when GitLab Credits access requires DAP instance settings' do
        shared_examples 'SM DAP credits enforcement' do
          ...
          context 'when duo_agent_platform is disabled at the instance level' do
            ...
            before do
              ::Ai::Setting.instance.update!(duo_agent_platform_enabled: false)
            end
    ...
    context 'when feature_setting is provided' do
      let_it_be(:self_hosted_model, freeze: false) do
        create(:ai_self_hosted_model, model: :claude_3, identifier: 'claude-3-7-sonnet-20250219')
      end

      let_it_be(:feature_setting, freeze: false) do
    ...
    context 'when testing get_self_hosted_unit_primitive_name coverage' do
      ...
      shared_examples 'feature settings with unit primitive lookup' do
        let_it_be(:feature_settings, freeze: false) do
      ...
      context 'when feature_setting exists but is not self-hosted' do
        let_it_be(:non_self_hosted_feature_setting, freeze: false) do
      ...
      context 'when feature_setting is self-hosted but feature is not DAP' do
        let_it_be(:non_dap_feature_setting, freeze: false) do
          create(:ai_feature_setting, feature: :duo_chat, self_hosted_model: self_hosted_model)
        end
      ...
      context 'when feature_setting is self-hosted DAP' do
        let_it_be(:feature_names, freeze: false) do
    ...
    context 'when testing self-hosted DAP without user assignment' do
      ...
      let_it_be(:dap_feature_setting, freeze: false) do
        create(:ai_feature_setting, feature: :duo_agent_platform, self_hosted_model: self_hosted_model)
      end

      let_it_be(:dap_add_on, freeze: false) { create(:gitlab_subscription_add_on, :self_hosted_dap) }
      let_it_be(:dap_purchase, freeze: false) do
      ...
      context 'when user is not assigned to non-DAP add-on' do
        let_it_be(:product_analytics_add_on, freeze: false) do
          create(:gitlab_subscription_add_on, :product_analytics)
        end

        let_it_be(:product_analytics_purchase, freeze: false) do
          create(:gitlab_subscription_add_on_purchase, :self_managed,
    ...
    context 'with the DAP identity verification gate', :saas do
      ...
      before do
        # The gate defers every exemption (paid namespace, OSS, enterprise, risk,
        # SM) to #identity_verified?, which has its own coverage in
        # identity_verifiable_spec.rb. Here we only assert the gate's own logic:
        # flag + verification state.
        allow(dap_user).to receive(:identity_verified?).and_return(identity_verified)
      end

      describe '#dap_identity_verification_required?' do
        subject(:iv_required) { dap_user.dap_identity_verification_required?(trial_group) }

        context 'when the user is not identity-verified' do
          let(:identity_verified) { false }

          it { is_expected.to be true }

          context 'when the gate flag is disabled' do
            before do
        ...
        context 'when the user is identity-verified' do
      ...
      # The gate only scopes WHICH features it applies to; the actual decision
      # lives in #dap_identity_verification_required? (tested above). The denial
      # is blind by design, so we assert the seam here rather than a reason.
      describe '#feature_requires_identity_verification?' do
        it 'gates DAP features' do
          expect(dap_user.send(:feature_requires_identity_verification?, :duo_agent_platform, trial_group)).to be true
        end

        it 'does not gate non-DAP features even when verification would be required' do
          expect(dap_user.dap_identity_verification_required?(trial_group)).to be true
          expect(dap_user.send(:feature_requires_identity_verification?, :duo_chat, trial_group)).to be false
        end
      end

      it 'blindly denies a gated DAP feature through #allowed_to_use' do
        expect(dap_user.allowed_to_use(:duo_agent_platform, root_namespace: trial_group).allowed?).to be false
      end
    end
  end
  ...
  context 'when amazon q integration is connected with duo_enterprise addon' do
    ...
    let_it_be(:gitlab_subscription_user_add_on_assignment, freeze: false) do
  ...
  context 'when amazon q integration is connected with amazon_q addon' do
    ...
    let_it_be(:amazon_q_unit_primitive, freeze: false) do
      build(:cloud_connector_unit_primitive,
    ...
    let_it_be(:gitlab_subscription_add_on_purchase, freeze: false) do
    ...
    context 'when user is a composite identity service account' do
      ...
      it 'delegates to the scoped human user' do
        ...
        allow(scoped_user).to receive(:allowed_to_use).and_return(scoped_response)
  ...
  describe '#any_group_with_ai_available?', :saas, :use_clean_rails_redis_caching do
    ...
    context 'when member of a project only' do
      ...
      context 'with eligible group' do
        let(:group) { ultimate_group }
        let_it_be(:project, freeze: false) { create(:project, group: ultimate_group) }
      ...
      context 'with not eligible group' do
        let(:group) { bronze_group }
        let_it_be(:project, freeze: false) { create(:project, group: bronze_group) }
  ...
  describe '#any_group_with_mcp_server_enabled?', :saas, :use_clean_rails_redis_caching do
    ...
    context 'when mcp_server_availability_setting flag is on', :saas do
      let_it_be(:user, freeze: false) { create(:user) }
    ...
    context 'when mcp_server_availability_setting flag is off', :saas do
      let_it_be(:flag_off_user, freeze: false) { create(:user) }

      before do
        stub_feature_flags(mcp_server_availability_setting: false)
      end

      it 'does not call with_mcp_server_enabled scope' do
        expect(Group).not_to receive(:with_mcp_server_enabled)
        flag_off_user.any_group_with_mcp_server_enabled?
      end
    end
  end

  shared_examples 'returns IDs of namespaces with duo add-on' do
    let_it_be(:gitlab_duo_add_on, freeze: false) { create(:gitlab_subscription_add_on, add_on_type) }

    let_it_be(:expired_gitlab_duo_purchase, freeze: false) do
  ...
  describe '#duo_available_namespace_ids' do
    context 'when user has duo pro add-on' do
      it_behaves_like 'returns IDs of namespaces with duo add-on' do
        ...
        let_it_be(:add_on_type, freeze: false) { :duo_pro }
      end
    end
    ...
    context 'when user has duo enterprise add-on' do
      it_behaves_like 'returns IDs of namespaces with duo add-on' do
        ...
        let_it_be(:add_on_type, freeze: false) { :duo_enterprise }
      end
    end
    ...
    context 'when user has duo core add-on' do
      it_behaves_like 'returns IDs of namespaces with duo add-on' do
        ...
        let_it_be(:add_on_type, freeze: false) { :duo_core }
      end
    end
  end
  ...
  describe '#duo_core_ids_via_namespace_settings' do
    ...
    let_it_be(:group3, freeze: false) { create(:group) }
  ...
  describe '#eligible_for_self_managed_gitlab_duo_pro?' do
    ...
    let_it_be(:bot, freeze: false) { create(:user, :bot) }
    let_it_be(:ghost, freeze: false) { create(:user, :ghost) }
    let_it_be(:blocked_user, freeze: false) { create(:user, :blocked) }
    let_it_be(:banned_user, freeze: false) { create(:user, :banned) }
    let_it_be(:pending_approval_user, freeze: false) { create(:user, :blocked_pending_approval) }
    let_it_be(:group, freeze: false) { create(:group) }
    let_it_be(:guest_user, freeze: false) { create(:group_member, :guest, source: group).user }
  ...
  describe '#gitlab_com_root_group_ids', :use_clean_rails_redis_caching do
    ...
    let_it_be(:sub_group, freeze: false) { create(:group, parent: root_group) }
    let_it_be(:group_project, freeze: false) { create(:project, group: root_group) }
    let_it_be(:sub_group_project, freeze: false) { create(:project, group: sub_group) }
  ...
  describe '.clear_group_with_ai_available_cache', :use_clean_rails_redis_caching do
    ...
    let_it_be(:yet_another_user, freeze: false) { create(:user) }
    let_it_be(:billable_groups_user, freeze: false) { create(:user) }
  ...
  describe '.clear_group_with_mcp_server_enabled_cache', :use_clean_rails_redis_caching do
    ...
    it 'clears the MCP cache only for the given ids', :aggregate_failures do
      ...
      expect(Rails.cache.fetch(other_user_cache_key)).to eq(true)
    end
    ...
    it 'does not clear the AI cache keys', :aggregate_failures do
      ...
      expect(Rails.cache.fetch(ai_cache_key)).to eq(true)
  ...
  describe '#governing_namespace' do
    ...
    let_it_be(:default_namespace, freeze: false) { create(:group) }
  ...
  describe '#allowed_to_use_for_resource?' do
    let_it_be(:group, freeze: false) { create(:group) }
    let_it_be(:resource, freeze: false) { create(:project, group: group) }
    ...
    context 'when on Gitlab.com instance', :saas do
      shared_examples 'checking for fallback namespace' do
        let_it_be(:fallback_namespace, freeze: false) { build(:group) }
      ...
      context 'when user has both Duo Core and Duo Pro subscriptions' do
        ...
        let_it_be(:duo_pro_add_on, freeze: false) { create(:gitlab_subscription_add_on, :duo_pro) }

        let_it_be(:duo_core_purchase, freeze: false) do
          create(:gitlab_subscription_add_on_purchase, add_on: duo_core_add_on)
        end

        let_it_be(:duo_pro_purchase, freeze: false) do
  ...
  describe '#check_access_through_namespace', :use_clean_rails_redis_caching do
    ...
    context 'when on SaaS' do
      shared_examples 'checking namespace access rules', :request_store do
        context 'when namespace has group-based rules' do
          let_it_be(:subgroup, freeze: false) { create(:group, parent: namespace_to_check) }
      ...
      let_it_be(:default_namespace, freeze: false) { create(:group).tap { |ns| ns.add_guest(user) } }
      ...
      context 'when current namespace is provided' do
        let_it_be(:root_namespace, freeze: false) { create(:group) }
        ...
        context 'when user is part of current namespace' do
          ...
          it_behaves_like 'checking namespace access rules' do
            let_it_be(:namespace_to_check, freeze: false) { root_namespace }
          end
        end
        ...
        context 'when user is not part of current namespace' do
          context 'when user has default namespace' do
            ...
            it_behaves_like 'checking namespace access rules' do
              let_it_be(:namespace_to_check, freeze: false) { default_namespace }
            end
      ...
      context 'when checking multiple features for the same namespace' do
        let_it_be(:root_namespace, freeze: false) { create(:group) }
        let_it_be(:subgroup, freeze: false) { create(:group, parent: root_namespace) }
    ...
    context 'when on self-managed' do
      let_it_be(:ns, freeze: false) { create(:group).tap { |n| n.add_guest(user) } }
  ...
  describe '#duo_agent_platform_enabled_via_credits?' do
    ...
    context 'when self-managed' do
      before do
        create(:gitlab_subscription_add_on_purchase, :gitlab_credits, :active, namespace: nil)
        ::Ai::Setting.instance.update!(duo_agent_platform_enabled: dap_enabled)
      end
    ...
    context 'when SaaS', :saas do
      let_it_be(:group, freeze: false) { create(:group) }
      let_it_be(:purchase, freeze: false) do
