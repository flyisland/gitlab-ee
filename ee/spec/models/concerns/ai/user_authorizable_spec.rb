...
RSpec.describe Ai::UserAuthorizable, feature_category: :ai_abstraction_layer do
  ...
  describe '#allowed_to_use' do
    shared_examples_for 'checking assigned seats' do
      ...
      context 'when the user has a Duo Core subscription' do
        ...
        context 'when the feature requires identity verification' do
          ...
          it 'denies access' do
            expect(user.allowed_to_use(ai_feature, root_namespace: root_namespace)).not_to be_allowed
          end

          context 'when identity verification is skipped' do
            it 'allows access' do
              expect(
                user.allowed_to_use(ai_feature, root_namespace: root_namespace, skip_identity_verification: true)
              ).to be_allowed
            end
          end
        end
    ...
    context 'with the DAP identity verification gate', :saas do
      ...
      describe '#feature_requires_identity_verification?' do
        subject(:required) { dap_user.send(:feature_requires_identity_verification?, ai_feature, trial_group) }

        let(:ai_feature) { :duo_agent_platform }

        context 'for a DAP feature when the user is not identity-verified' do
        ...
        context 'for a DAP feature when the user is identity-verified' do
          let(:identity_verified) { true }

          it { is_expected.to be false }
        end

        context 'for a non-DAP feature' do
          let(:ai_feature) { :duo_chat }
  ...
  describe '#duo_agent_platform_identity_verification_required?' do
    let_it_be(:container) { create(:group) }

    subject { user.duo_agent_platform_identity_verification_required?(container) }

    before do
      allow(user).to receive(:can?).with(:read_duo_agent_platform, container).and_return(read_dap)
      allow(user).to receive(:can?).with(:duo_workflow, container).and_return(duo_workflow)
    end

    context 'when entitled to the platform but blocked from running it' do
      let(:read_dap) { true }
    ...
    context 'when able to run the platform' do
      let(:read_dap) { true }
