...
RSpec.describe SystemAccess::MicrosoftApplication, feature_category: :system_access do
  ...
  it 'only allows one record with nil namespace_id' do
    ...
    expect(conflicting_record.valid?).to be(false)
    expect(conflicting_record.errors['namespace_id']).to match_array([_('has already been taken')])
  end

  describe 'validates' do
    let_it_be_with_reload(:application) { create(:system_access_microsoft_application) }
    ...
    it { is_expected.to validate_length_of(:client_secret).is_at_most(255) }

    context 'when validating client_secret length' do
      it 'does not invalidate existing records' do
        application = create(:system_access_microsoft_application)
        application.client_secret = 'A' * 260
        application.save!(validate: false)

        expect(application.reload).to be_valid
      end
    end
