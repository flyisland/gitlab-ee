...
RSpec.describe IncidentManagement::IssuableEscalationStatus, feature_category: :incident_management do
  let_it_be_with_reload(:escalation_status) { create(:incident_management_issuable_escalation_status, :paging, :acknowledged) }
  ...
  describe '#trigger' do
    ...
    context 'without escalation policy' do
      let_it_be_with_reload(:escalation_status) { create(:incident_management_issuable_escalation_status) }
  ...
  [:acknowledge, :ignore, :resolve].each do |status_event|
    describe status_event do
      subject(:status_result) { escalation_status.send(status_event) }

      it 'does not change escalations_started_at' do
        expect { status_result }.not_to change { escalation_status.escalations_started_at }
  ...
  describe '#escalation_policy' do
    ...
    subject(:found_escalation_policy) { escalation_status.escalation_policy }
    ...
    context 'when escalation policy exists on the project' do
      it 'returns the projects first (only) escalation policy' do
        ...
        expect(found_escalation_policy).to eq(policy)
      end
    end
  end
