...
RSpec.describe IncidentManagement::OncallRotation, feature_category: :incident_management do
  ...
  describe '.validations' do
    subject(:rotation) { build(:incident_management_oncall_rotation, schedule: schedule, name: 'Test rotation') }
    ...
    context 'when the oncall rotation with the same name exists' do
      ...
      it 'has validation errors' do
        expect(rotation).to be_invalid
        expect(rotation.errors.full_messages.to_sentence).to eq('Name has already been taken')
      end
    end
    ...
    context 'with ends_at' do
      ...
      subject(:rotation) { build(:incident_management_oncall_rotation, schedule: schedule, starts_at: starts_at, ends_at: ends_at) }
      ...
      context 'with ends_at before starts_at' do
        ...
        it 'has validation errors' do
          expect(rotation).to be_invalid
          expect(rotation.errors.full_messages.to_sentence).to eq('Ends at must be after start')
        end
      end
    end
    ...
    describe 'active period start/end time' do
      context 'missing values' do
        before do
          allow(rotation).to receive(stubbed_field).and_return('08:00')
        end
      ...
      context 'hourly shifts' do
        subject(:rotation) { build(:incident_management_oncall_rotation, schedule: schedule, name: 'Test rotation', length_unit: :hours) }

        it 'raises a validation error if an active period is set' do
          rotation.active_period_start = '08:00'
          rotation.active_period_end = '17:00'

          expect(rotation.valid?).to be(false)
          expect(rotation.errors.full_messages).to include(/Restricted shift times are not available for hourly shifts/)
        end
      end
    end
  end
