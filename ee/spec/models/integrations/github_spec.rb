...
RSpec.describe Integrations::Github, feature_category: :continuous_integration do
  ...
  subject(:github_integration) { described_class.create!(integration_params) }
  ...
  describe 'default values' do
    it { expect(github_integration.pipeline_events).to be(true) }
  end

  it_behaves_like Integrations::HasAvatar

  it_behaves_like Integrations::ResetSecretFields do
    let(:integration) { github_integration }
  end
  ...
  describe "#owner" do
    it 'is determined from the repo URL' do
      expect(github_integration.owner).to eq owner
    end
  end

  describe "#repository_name" do
    it 'is determined from the repo URL' do
      expect(github_integration.repository_name).to eq repository_name
    end
  end

  describe "#api_url" do
    it 'uses github.com by default' do
      expect(github_integration.api_url).to eq "https://api.github.com"
    end
    ...
    context "with GitHub Enterprise repo URL" do
      ...
      it 'is set to the Enterprise API URL' do
        expect(github_integration.api_url).to eq "https://my.code-repo.com/api/v3"
      end
    end
  end
  ...
  describe '#help' do
    it 'links to mirroring settings' do
      expect(github_integration.help).to match(/href=.*mirroring/)
    end
  end

  describe '#properties' do
    let(:properties) { github_integration.reload.properties.symbolize_keys }

    it 'does not overwrite existing integrations' do
      github_integration.update!(integration_params.slice(:properties))

      expect(properties).to match(integration_params[:properties])
      expect(github_integration.static_context).to be_nil
    end
  ...
  describe '#execute' do
    ...
    context 'the integration is invalid' do
      it 'does not notify GitHub of a status change' do
        allow(github_integration).to receive(:invalid?).and_return(true)

        expect(Integrations::Github::StatusMessage).not_to receive(:from_pipeline_data)

        github_integration.execute(pipeline_sample_data)
      end
    end
    ...
    it 'uses StatusMessage to build message' do
      allow(github_integration).to receive(:update_status)

      expect(Integrations::Github::StatusMessage)
        .to receive(:from_pipeline_data)
        .with(project, github_integration, pipeline_sample_data)
    ...
    context 'with custom api endpoint' do
      ...
      before do
        allow(github_integration).to receive(:api_url).and_return(api_url)
      end
    ...
    context 'when an external pull request pipeline exists' do
      ...
      it 'does not send notification' do
        expect(github_integration).not_to receive(:update_status)
        expect(Integrations::Github::StatusMessage).not_to receive(:from_pipeline_data)

        expect(github_integration.execute(pipeline_sample_data)).to be_nil
      end

      it 'sends notification if the sha is not present' do
        pipeline_sample_data[:object_attributes].delete(:sha)

        expect(github_integration).to receive(:update_status)
    ...
    context 'when the pipeline is an external pull request pipeline' do
      ...
      it 'logs result of GitHub API call without the "creator" section and "avatar_url" field' do
        expect(github_integration).to receive(:notifier).twice.and_return(notifier)
        ...
        expect(github_integration).to receive(:log_info).once.with(
    ...
    context 'without a license' do
      it 'does nothing' do
        ...
        result = github_integration.execute(pipeline_sample_data)
  ...
  describe '#testable?' do
    it 'is false if there are no pipelines' do
      ...
      expect(github_integration).not_to be_testable
    end

    it 'is true if the project has a pipeline' do
      pipeline

      expect(github_integration).to be_testable
    end
  end
  ...
  describe '#test' do
    it 'mentions creator in success message' do
      ...
      allow(github_integration).to receive(:update_status).and_return(dummy_response)

      result = github_integration.test(pipeline_sample_data)
    ...
    it 'forwards failure message on error' do
      ...
      allow(github_integration).to receive(:update_status).and_raise(Octokit::Unauthorized, error_response)
