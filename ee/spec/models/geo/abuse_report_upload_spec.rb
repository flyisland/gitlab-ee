...
RSpec.describe Geo::AbuseReportUpload, feature_category: :geo_replication do
  include ::EE::GeoHelpers

  it { expect(described_class.table_name).to eq('abuse_report_uploads') }
  ...
  describe 'Geo replication' do
    describe 'associations' do
      it 'has one verification state table class' do
        is_expected
          .to have_one(:abuse_report_upload_state)
    ...
    include_examples 'a verifiable model for verification state' do
      ...
      let(:verifiable_model_record) do
        record = create(:geo_abuse_report_upload)
        described_class.verification_state_table_class.where(abuse_report_upload_id: record.id).delete_all
        record.reload
      end

      let(:unverifiable_model_record) do
        record = create(:geo_abuse_report_upload, :remote_store)
        described_class.verification_state_table_class.where(abuse_report_upload_id: record.id).delete_all
        record.reload
      end
    end
    ...
    describe 'replication/verification' do
      let_it_be(:organization_1, freeze: true) { create(:organization) }
      let_it_be(:organization_2, freeze: true) { create(:organization) }
      let_it_be(:group_1, freeze: true) { create(:group, organization: organization_1) }
      let_it_be(:group_2, freeze: true) { create(:group, organization: organization_2) }

      let!(:first_replicable_and_in_selective_sync) do
        parent_model = create(:abuse_report, organization: organization_1)
        create(:geo_abuse_report_upload, parent_model: parent_model)
      end
      ...
      let!(:third_replicable_on_object_storage_and_in_selective_sync) do
        parent_model = create(:abuse_report, organization: organization_1)
        create(:geo_abuse_report_upload, :remote_store, parent_model: parent_model)
      end

      let!(:last_replicable_and_not_in_selective_sync) do
        parent_model = create(:abuse_report, organization: organization_2)
        create(:geo_abuse_report_upload, parent_model: parent_model)
      end
