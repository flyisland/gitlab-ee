...
RSpec.describe Geo::GroupWikiRepositoryState, :geo, type: :model, feature_category: :geo_replication do
  ...
  context 'with loose foreign key on group_wiki_repository_states.group_wiki_repository_id' do
    it_behaves_like 'cleanup by a loose foreign key' do
      let_it_be(:parent) { create(:group_wiki_repository) }
      let_it_be(:model) { create(:geo_group_wiki_repository_state, group_wiki_repository: parent) }
    end
  end
end
