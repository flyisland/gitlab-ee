...
RSpec.describe Geo::ProjectRepositoryState, feature_category: :geo_replication do
  ...
  describe 'validations' do
    ...
    context 'with project_repository' do
      subject { described_class.new(project: project, project_repository: project_repository) }

      it { is_expected.to validate_uniqueness_of(:project_repository) }
    end
  end
  ...
  describe '#prevent_concurrent_inserts' do
    let(:state) { build(:geo_project_repository_state, project_repository: project_repository) }

    it 'executes advisory lock with correct parameters' do
      expected_lock_key = "project_repository_states-#{project_repository.id}"
      expected_lock_expression = "hashtext('#{expected_lock_key}')"
      expected_sql = "SELECT pg_advisory_xact_lock(#{expected_lock_expression})"

      expect(state.connection).to receive(:quote).with(expected_lock_key).and_return("'#{expected_lock_key}'")
      expect(state.connection).to receive(:execute).with(expected_sql)

      state.send(:prevent_concurrent_inserts)
    end

    it 'returns a PG::Result object' do
      result = state.send(:prevent_concurrent_inserts)

      expect(result).to be_a(PG::Result)
      expect(result.first).to eq({ 'pg_advisory_xact_lock' => '' })
    end
  end
end
