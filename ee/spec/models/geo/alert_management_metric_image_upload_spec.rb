...
RSpec.describe Geo::AlertManagementMetricImageUpload, feature_category: :geo_replication do
  include ::EE::GeoHelpers

  it { expect(described_class.table_name).to eq('alert_management_alert_metric_image_uploads') }
  ...
  describe 'Geo replication' do
    describe 'associations' do
      it 'has one verification state table class' do
        is_expected
          .to have_one(:alert_management_metric_image_upload_state)
          .class_name('Geo::AlertManagementMetricImageUploadState')
          .inverse_of(:alert_management_metric_image_upload)
          .autosave(false)
      end
    end
    ...
    include_examples 'a verifiable model for verification state' do
      ...
      let(:verifiable_model_record) do
        record = create(:geo_alert_management_metric_image_upload)
        described_class.verification_state_table_class
          .where(alert_management_metric_image_upload_id: record.id).delete_all
        record.reload
      end

      let(:unverifiable_model_record) do
        record = create(:geo_alert_management_metric_image_upload, :remote_store)
        described_class.verification_state_table_class
          .where(alert_management_metric_image_upload_id: record.id).delete_all
        record.reload
      end
    ...
    describe 'replication/verification' do
      ...
      let!(:first_replicable_and_in_selective_sync) do
        create(:geo_alert_management_metric_image_upload,
          parent_model: create(:alert_metric_image, alert: create(:alert_management_alert, project: project_1)))
      end

      let!(:second_replicable_and_in_selective_sync) do
        create(:geo_alert_management_metric_image_upload,
          parent_model: create(:alert_metric_image, alert: create(:alert_management_alert, project: project_2)))
      end

      let!(:third_replicable_on_object_storage_and_in_selective_sync) do
        create(:geo_alert_management_metric_image_upload, :remote_store,
          parent_model: create(:alert_metric_image, alert: create(:alert_management_alert, project: project_1)))
      end

      let!(:last_replicable_and_not_in_selective_sync) do
        create(:geo_alert_management_metric_image_upload,
          parent_model: create(:alert_metric_image, alert: create(:alert_management_alert, project: project_3)))
      end
