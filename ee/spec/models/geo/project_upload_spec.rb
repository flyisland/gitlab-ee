...
RSpec.describe Geo::ProjectUpload, feature_category: :geo_replication do
  include ::EE::GeoHelpers

  it { expect(described_class.table_name).to eq('project_uploads') }
  ...
  describe 'Geo replication' do
    describe 'associations' do
      it 'has one verification state table class' do
        is_expected
          .to have_one(:project_upload_state)
    ...
    include_examples 'a verifiable model for verification state' do
      ...
      let(:verifiable_model_record) do
        record = create(:geo_project_upload)
        described_class.verification_state_table_class.where(project_upload_id: record.id).delete_all
        record.reload
      end

      let(:unverifiable_model_record) do
        record = create(:geo_project_upload, :remote_store)
        described_class.verification_state_table_class.where(project_upload_id: record.id).delete_all
        record.reload
      end
    end
    ...
    describe 'replication/verification' do
      ...
      let!(:first_replicable_and_in_selective_sync) do
        create(:geo_project_upload, parent_model: project_1)
      end

      let!(:second_replicable_and_in_selective_sync) do
        create(:geo_project_upload, parent_model: project_2)
      end

      let!(:third_replicable_on_object_storage_and_in_selective_sync) do
        create(:geo_project_upload, :remote_store, parent_model: project_1)
      end

      let!(:last_replicable_and_not_in_selective_sync) do
        create(:geo_project_upload, parent_model: project_3)
      end
