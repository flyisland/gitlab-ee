...
RSpec.describe Geo::ImportExportUploadUpload, feature_category: :geo_replication do
  include ::EE::GeoHelpers

  it { expect(described_class.table_name).to eq('import_export_upload_uploads') }
  ...
  describe 'Geo replication' do
    describe 'associations' do
      it 'has one verification state table class' do
        is_expected
          .to have_one(:import_export_upload_upload_state)
    ...
    include_examples 'a verifiable model for verification state' do
      ...
      let(:verifiable_model_record) do
        record = create(:geo_import_export_upload_upload)
        described_class.verification_state_table_class.where(import_export_upload_upload_id: record.id).delete_all
        record.reload
      end

      let(:unverifiable_model_record) do
        record = create(:geo_import_export_upload_upload, :remote_store)
        described_class.verification_state_table_class.where(import_export_upload_upload_id: record.id).delete_all
        record.reload
      end
    end
    ...
    describe 'group-owned uploads' do
      ...
      let(:group_owned) do
        record = create(:geo_import_export_upload_upload, group: group)
        state_class.where(import_export_upload_upload_id: record.id).delete_all
        record.reload
      end
      ...
      describe '.create_verification_details_for' do
        it 'creates a verification state record owned by the namespace' do
          ...
          state = state_class.find_by(import_export_upload_upload_id: group_owned.id)
        ...
        it 'creates records for a batch mixing group-owned and project-owned uploads' do
          project_owned = create(:geo_import_export_upload_upload)
      ...
      describe '#save_verification_details' do
        it 'persists the verification state' do
          ...
          expect(state_class.find_by(import_export_upload_upload_id: group_owned.id).namespace_id)
            .to eq(group.id)
        end
      end
    end
    ...
    describe 'replication/verification' do
      ...
      let!(:first_replicable_and_in_selective_sync) do
        create(:geo_import_export_upload_upload,
          parent_model: create(:import_export_upload, export_file: nil, project: project_1))
      end

      let!(:second_replicable_and_in_selective_sync) do
        create(:geo_import_export_upload_upload,
          parent_model: create(:import_export_upload, export_file: nil, project: project_2))
      end

      let!(:third_replicable_on_object_storage_and_in_selective_sync) do
        create(:geo_import_export_upload_upload, :remote_store,
          parent_model: create(:import_export_upload, export_file: nil, project: project_1))
      end

      let!(:last_replicable_and_not_in_selective_sync) do
        create(:geo_import_export_upload_upload,
          parent_model: create(:import_export_upload, export_file: nil, project: project_3))
      end
    ...
    describe 'replication/verification of group-owned uploads' do
      ...
      let!(:first_replicable_and_in_selective_sync) do
        create(:geo_import_export_upload_upload, group: group_1)
      end

      let!(:second_replicable_and_in_selective_sync) do
        create(:geo_import_export_upload_upload, group: nested_group_1)
      end

      let!(:third_replicable_on_object_storage_and_in_selective_sync) do
        create(:geo_import_export_upload_upload, :remote_store, group: group_1)
      end

      let!(:last_replicable_and_not_in_selective_sync) do
        create(:geo_import_export_upload_upload, group: group_2)
      end
