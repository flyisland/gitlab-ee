    ...
    context 'when the current license is not a trial' do
      ...
      context 'when the current license is not a paid plan but has an active credits add-on' do
        ...
        before do
          ...
          allow(described_class).to receive(:current).and_return(free_license)
      ...
      context 'when the current license is not a paid plan and has no add-on' do
        ...
        before do
          allow(free_license).to receive(:paid?).and_return(false)
          allow(described_class).to receive(:current).and_return(free_license)
          allow(described_class).to receive(:has_active_gitlab_credits_add_on?).and_return(false)
        end
    ...
    context 'when the current license is a trial' do
      ...
      context 'when the prior license is also a trial' do
        ...
        before do
          ...
          allow(described_class).to receive(:recent).and_return(described_class.where(id: prior_trial.id))
          allow(described_class).to receive(:has_active_gitlab_credits_add_on?).and_return(false)
        end
      ...
      context 'when the prior license is expired' do
        ...
        before do
          allow(described_class).to receive(:current).and_return(create(:license, :ultimate_trial))
          allow(described_class).to receive(:recent).and_return(described_class.where(id: prior_license.id))
          allow(described_class).to receive(:has_active_gitlab_credits_add_on?).and_return(false)
        end
