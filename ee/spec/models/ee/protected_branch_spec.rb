...
RSpec.describe ProtectedBranch, feature_category: :source_code_management do
  ...
  describe 'squash options validation' do
    ...
    context 'when squash options exist' do
      ...
      context 'and name does not change' do
        it 'is valid' do
          expect(protected_branch).to be_valid
        end
      end

      context 'and name changes to something other than wildcard' do
        before do
          protected_branch.name = 'develop'
        end
      ...
      context 'and other attributes are changed without changing name' do
        it 'is valid' do
    ...
    context 'when adding a squash option to an existing wildcard branch via nested attributes' do
      let(:protected_branch) { create(:protected_branch, name: '*', project: project) }

      before do
        protected_branch.assign_attributes(squash_option_attributes: { squash_option: :always, project: project })
      end
