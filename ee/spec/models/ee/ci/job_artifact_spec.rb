...
RSpec.describe Ci::JobArtifact, feature_category: :job_artifacts do
  ...
  describe '#security_report' do
    ...
    context 'when the artifact is a SARIF report' do
      ...
      context 'when sarif_ingestion feature flag is enabled for the project' do
        before do
          stub_feature_flags(sarif_ingestion: job_artifact.project)
        end

        it 'returns a parsed report' do
      ...
      context 'when sarif_ingestion feature flag is disabled' do
        before do
          stub_feature_flags(sarif_ingestion: false)
        end

        it 'returns an empty report without errors' do
          expect(security_report).to be_a(::Gitlab::Ci::Reports::Security::Report)
          expect(security_report.errors).to be_empty
          expect(security_report.findings).to be_empty
        end
      end
    end
