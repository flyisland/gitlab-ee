...
RSpec.describe EE::Namespace::RootStorageStatistics, feature_category: :consumables_cost_management do
  ...
  describe 'scopes' do
    describe '.with_namespace_associations', :saas do
      subject(:statistics_list) { ::Namespace::RootStorageStatistics.with_namespace_associations }

      before do
        create_list(:namespace_with_plan, 2, :with_root_storage_statistics)
      end

      it 'preloads the namespace and limit associations' do
        statistics_list

        query = ActiveRecord::QueryRecorder.new { statistics_list.map(&:namespace).map(&:namespace_limit) }
      ...
      it 'preloads the namespace subscription and plan associations' do
        ...
        query = ActiveRecord::QueryRecorder.new { statistics_list.map(&:namespace).map(&:paid?) }
  ...
  describe '#recalculate!' do
    ...
    context 'when namespace belongs to a group' do
      ...
      let_it_be_with_reload(:project1) { create(:project, namespace: group1) }
      let_it_be_with_reload(:project2) { create(:project, namespace: group2) }
    ...
    context 'when namespace belong to a user' do
      ...
      let_it_be_with_reload(:project_stat) do
        create(:project_statistics, project: project, with_data: true, size_multiplier: 100)
      end
