...
RSpec.describe Group, feature_category: :groups_and_projects do
  ...
  describe '#sync_template_columns_to_namespace_template_setting' do
    let_it_be_with_reload(:group) { create(:group) }

    context 'when file_template_project_id changes' do
      let(:project) { create(:project, namespace: group) }

      it 'syncs a non-nil value to namespace_template_setting' do
      ...
      it 'syncs nil value to namespace_template_setting' do
        group.update_column(:file_template_project_id, project.id)
        setting = Namespaces::TemplateSetting.find_or_create_by!(namespace_id: group.id)
        setting.update_column(:file_template_project_id, project.id)
    ...
    context 'when custom_project_templates_group_id changes' do
      ...
      it 'syncs nil value to namespace_template_setting' do
        group.update_column(:custom_project_templates_group_id, subgroup.id)
        setting = Namespaces::TemplateSetting.find_or_create_by!(namespace_id: group.id)
        setting.update_column(:custom_project_templates_group_id, subgroup.id)
    ...
    context 'when neither column changes' do
      it 'does not create a namespace_template_setting' do
        group.update!(name: 'new-name')

        expect(group.reload.namespace_template_setting).to be_nil
      end
    end
  end
