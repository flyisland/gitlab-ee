...
RSpec.describe Release do
  describe '.by_namespace_id' do
    ...
    let_it_be_with_reload(:unrelated_project) { create(:project) }

    let_it_be_with_reload(:release_in_group_project) { create(:release, project: project_in_group) }
    let_it_be_with_reload(:release_in_subgroup_project_1) { create(:release, project: project_in_subgroup) }
    let_it_be(:release_in_subgroup_project_2) { create(:release, project: project_in_subgroup) }
    let_it_be_with_reload(:release_in_unrelated_project) { create(:release, project: unrelated_project) }
