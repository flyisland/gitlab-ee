  ...
  describe '#prevent_role_assignement?' do
    ...
    context 'for custom roles assignement' do
      ...
      context 'when member has a custom role with permissions the current user lacks' do
        let_it_be(:member_role) { create(:member_role, :maintainer, namespace: project.namespace, read_compliance_dashboard: true) }
        let_it_be_with_reload(:member) { create(:project_member, :maintainer, project: project, member_role: member_role) }

        it { is_expected.to be(true) }
      end

      context 'when member has a custom role with only permissions the current user also has' do
        let_it_be(:member_role) { create(:member_role, :maintainer, namespace: project.namespace, manage_merge_request_settings: true) }
        let_it_be_with_reload(:member) { create(:project_member, :maintainer, project: project, member_role: member_role) }
