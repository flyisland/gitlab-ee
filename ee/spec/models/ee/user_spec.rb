...
RSpec.describe User, feature_category: :system_access do
  ...
  describe "scopes" do
    ...
    describe '.excluding_guests_and_requests' do
      ...
      let_it_be(:requested_user, freeze: false) { create(:group_member, :reporter, :access_request).user_id }
    ...
    describe '.with_saml_provider' do
      ...
      let_it_be(:saml_provider, freeze: false) { create(:saml_provider) }
      ...
      context 'when users have a SAML identity tied to the provider' do
        let_it_be(:saml_identity, freeze: false) { create(:group_saml_identity, user: user, saml_provider: saml_provider) }
    ...
    describe '.expired_sso_session_saml_providers' do
      let_it_be(:user, freeze: false) { create(:user) }
      let_it_be(:saml_provider1, freeze: false) { create(:saml_provider) }
      let_it_be(:saml_provider2, freeze: false) { create(:saml_provider) }
      ...
      context 'for users with SAML identity' do
        let_it_be(:identity1, freeze: false) { create(:group_saml_identity, user: user) }
        let_it_be(:identity2, freeze: false) { create(:group_saml_identity, user: user) }
        let_it_be(:saml_provider1, freeze: false) { identity1.saml_provider }
        let_it_be(:saml_provider2, freeze: false) { identity2.saml_provider }
    ...
    describe '.security_policy_bots_for_projects' do
      ...
      let_it_be(:security_policy_bot_1, freeze: false) { create(:user, :security_policy_bot) }
      let_it_be(:security_policy_bot_2, freeze: false) { create(:user, :security_policy_bot) }
    ...
    describe '.security_policy_bot_users_without_project_membership' do
      let_it_be(:security_policy_bot_with_project, freeze: false) { create(:user, :security_policy_bot) }
      let_it_be(:security_policy_bot_orphaned, freeze: false) { create(:user, :security_policy_bot) }
      ...
      context 'when security policy bot has ghost user migration' do
        let_it_be(:security_policy_bot_with_migration, freeze: false) { create(:user, :security_policy_bot) }
        let_it_be(:ghost_user_migration, freeze: false) { create(:ghost_user_migration, user: security_policy_bot_with_migration) }
      ...
      context 'when security policy bot has both project membership and ghost user migration' do
        let_it_be(:security_policy_bot_both, freeze: false) { create(:user, :security_policy_bot) }
        let_it_be(:project_2, freeze: false) { create(:project) }
      ...
      context 'with various bot types' do
        let_it_be(:automation_bot, freeze: false) { create(:user, user_type: :automation_bot) }
        let_it_be(:project_bot, freeze: false) { create(:user, :project_bot) }
        let_it_be(:alert_bot, freeze: false) { create(:user, user_type: :alert_bot) }
    ...
    describe '.with_admin_role' do
      ...
      let_it_be(:user_3, freeze: false) { create(:user) }

      let_it_be(:user_role_1, freeze: false) { create(:user_member_role, user: user_1) }
      let_it_be(:user_role_2, freeze: false) { create(:user_member_role, user: user_2) }
  ...
  describe '#can_admin_all_namespaces?' do
    let_it_be(:user, freeze: false) { create(:user) }
    let_it_be(:namespace_a, freeze: false) { create(:group) }
    let_it_be(:namespace_b, freeze: false) { create(:group) }
  ...
  describe '#email_domain' do
    context 'when user email is nil' do
      let_it_be(:user, freeze: false) { build(:user, email: nil) }
    ...
    context 'when user email is empty string' do
      let_it_be(:user, freeze: false) { build(:user, email: '') }
    ...
    context 'when user email is invalid' do
      let_it_be(:user, freeze: false) { build(:user, email: 'invalid_email_format') }
    ...
    context 'when user email is valid' do
      let_it_be(:user, freeze: false) { build(:user, email: 'user-email@example.GitLab.com') }
  ...
  describe '.billable' do
    let_it_be(:bot_user, freeze: false) { create(:user, :bot) }
    let_it_be(:service_account, freeze: false) { create(:user, :service_account) }
    let_it_be(:regular_user, freeze: false) { create(:user) }
    let_it_be(:project_reporter_user, freeze: false) { create(:project_member, :reporter).user }
    let_it_be(:project_guest_user, freeze: false) { create(:project_member, :guest).user }
    let_it_be(:minimal_access_user, freeze: false) { create(:group_member, :minimal_access).user }
    let_it_be(:group, freeze: false) { create(:group) }
    let_it_be(:member_role_elevating, freeze: false) { create(:member_role, :billable, namespace: group) }
    let_it_be(:member_role_basic, freeze: false) { create(:member_role, :non_billable, namespace: group) }
    let_it_be(:guest_with_elevated_role, freeze: false) { create(:group_member, :guest, source: group, member_role: member_role_elevating).user }
    let_it_be(:guest_without_elevated_role, freeze: false) { create(:group_member, :guest, source: group, member_role: member_role_basic).user }
    let_it_be(:users_select, freeze: false) { 'SELECT "users".* FROM "users"' }
    let_it_be(:users_select_with_ignored_columns, freeze: false) { 'SELECT ("users"."\w+", )+("users"."\w+") FROM "users"' }
    let_it_be(:user_with_access_request, freeze: false) { create(:group_member, :access_request, source: group).user }
  ...
  describe '.non_billable_users_for_billable_management' do
    let_it_be(:non_billable_role, freeze: false) { create(:member_role, :non_billable, :instance) }
    let_it_be(:billable_role, freeze: false) { create(:member_role, :billable, :instance) }

    let_it_be(:billable_member, freeze: false) do
      create(:group_member, access_level: Gitlab::Access::DEVELOPER)
    end

    let_it_be(:non_billable_member, freeze: false) do
      create(:group_member, access_level: Gitlab::Access::GUEST)
    end

    let_it_be(:non_billable_minimal_access_member, freeze: false) do
    ...
    context 'when license excludes guests in active count' do
      let_it_be(:ultimate, freeze: false) { create(:license, plan: License::ULTIMATE_PLAN) }
      ...
      context 'with users in multiple groups' do
        ...
        context 'with elevation scenarios' do
          ...
          context 'with one evelated and one non elevated role' do
            let_it_be(:third_guest_member, freeze: false) do
  ...
  describe '#pending_billable_invitations' do
    let_it_be(:user, freeze: false) { described_class.new(confirmed_at: Time.zone.now, email: 'test@example.com') }
  ...
  describe '#valid_password?' do
    ...
    context 'when password authentication disabled by enterprise group' do
      let_it_be(:enterprise_group, freeze: false) { create(:group) }
      let_it_be(:saml_provider, freeze: false) { create(:saml_provider, group: enterprise_group, enabled: true, disable_password_authentication_for_enterprise_users: true) }

      let_it_be(:user, freeze: false) { create(:enterprise_user, enterprise_group: enterprise_group) }
  ...
  describe '#password_expired_if_applicable?' do
    ...
    context 'when password_automatically_set is true' do
      context 'with a SCIM identity' do
        let_it_be(:scim_identity, freeze: false) { create(:scim_identity, active: true) }
        let_it_be(:user, freeze: false) { scim_identity.user }

        it_behaves_like 'password expired not applicable'
      end

      context 'with a SAML identity' do
        let_it_be(:saml_identity, freeze: false) { create(:group_saml_identity) }
        let_it_be(:user, freeze: false) { saml_identity.user }

        it_behaves_like 'password expired not applicable'
      end

      context 'with a smartcard identity' do
        let_it_be(:smartcard_identity, freeze: false) { create(:smartcard_identity) }
        let_it_be(:user, freeze: false) { smartcard_identity.user }
  ...
  describe '#allow_passkey_authentication?' do
    ...
    context 'when GitLab.com' do
      context 'for enterprise users' do
        let_it_be(:enterprise_group, freeze: false) { create(:group) }
        let_it_be(:saml_provider, freeze: false) { create(:saml_provider, group: enterprise_group, enabled: true) }
  ...
  describe '#password_authentication_disabled_by_enterprise_group?' do
    ...
    let_it_be(:root_group_with_saml_provider, freeze: false) { create(:group) }
    let_it_be(:saml_provider, freeze: false) { create(:saml_provider, group: root_group_with_saml_provider) }
    ...
    context 'when SaaS', :saas do
      ...
      context 'when namespace is on a ultimate plan' do
        ...
        context 'when user is within project' do
          let_it_be(:group, freeze: false) { create(:group) }
          let_it_be(:namespace, freeze: false) { create(:project, namespace: group, developers: user) }
        ...
        context 'when user is within subgroup' do
          let_it_be(:group, freeze: false) { create(:group) }
          let_it_be(:namespace, freeze: false) { create(:group, parent: group, developers: user) }
  ...
  describe '#can_access_admin_area?' do
    ...
    let_it_be(:admin_role, freeze: false) { create(:member_role, :admin) }
    let_it_be(:user_member_role, freeze: false) { create(:user_member_role, member_role: admin_role, user: user_with_admin_role) }
  ...
  describe '#authorized_groups' do
    let_it_be(:user, freeze: false) { create(:user) }
    let_it_be(:private_group, freeze: false) { create(:group) }
    let_it_be(:minimal_access_group, freeze: false) { create(:group) }

    let_it_be(:project_group, freeze: false) { create(:group) }
  ...
  context 'paid namespaces', :saas do
    ...
    let_it_be(:bronze_group, freeze: false) { create(:group_with_plan, plan: :bronze_plan) }
    let_it_be(:free_group, freeze: false) { create(:group_with_plan, plan: :free_plan) }
    let_it_be(:group_without_plan, freeze: false) { create(:group) }
    ...
    describe '#belongs_to_paid_namespace?' do
      ...
      context 'when passed exclude_trials: true' do
        let_it_be(:trial_group, freeze: false) do
          create(
            :group_with_plan,
            plan: :ultimate_plan,
            trial: true,
  ...
  describe '#gitlab_employee?' do
    ...
    let_it_be(:gitlab_group, freeze: false) { create(:group, name: 'gitlab-com') }
    let_it_be(:random_group, freeze: false) { create(:group, name: 'random-group') }
  ...
  describe '#find_or_init_board_epic_preference' do
    let_it_be(:user, freeze: false) { create(:user) }
    let_it_be(:board, freeze: false) { create(:board) }
    ...
    context 'when preference already exists' do
      let_it_be(:epic_user_preference, freeze: false) { create(:epic_user_preference, board: board, epic: epic, user: user) }
  ...
  describe '.escalation_policies' do
    ...
    let_it_be(:user, freeze: false) { rule.user }
  ...
  describe '#can_read_email_enterprise_user?', :saas do
    ...
    with_them do
      ...
      context 'when group passed is not related to the user' do
        it 'returns false' do
          expect(user.can_read_email_enterprise_user?(current_user, group: unrelated_to_user_group)).to eq false
        end
      end
    end
    ...
    context 'when group passed is nil' do
      it 'returns false' do
        expect(user.can_read_email_enterprise_user?(current_user, group: nil)).to eq false
      end
    end

    context 'when current_user passed is nil' do
      it 'returns false' do
        expect(user.can_read_email_enterprise_user?(nil, group: group)).to eq false
      end
    end

    context 'when current_user and group passed are nil' do
      it 'returns false' do
        expect(user.can_read_email_enterprise_user?(nil, group: nil)).to eq false
      end
    end
  end
  ...
  describe '#can_disable_two_factor_enterprise_user?', :saas do
    ...
    with_them do
      ...
      context 'when group passed is not related to the user' do
        it 'returns false' do
          expect(user.can_disable_two_factor_enterprise_user?(current_user, group: unrelated_to_user_group)).to eq false
        end
      end
    end
    ...
    context 'when group passed is nil' do
      it 'returns false' do
        expect(user.can_disable_two_factor_enterprise_user?(current_user, group: nil)).to eq false
      end
    end

    context 'when current_user passed is nil' do
      it 'returns false' do
        expect(user.can_disable_two_factor_enterprise_user?(nil, group: group)).to eq false
      end
    end

    context 'when current_user and group passed are nil' do
      it 'returns false' do
        expect(user.can_disable_two_factor_enterprise_user?(nil, group: nil)).to eq false
      end
    end
  end
  ...
  describe 'Elastic::ApplicationVersionedSearch', :elastic, feature_category: :global_search do
    ...
    context 'when a membership is deleted' do
      let_it_be(:membership, freeze: false) { create(:group_member, :developer, source: group, user: user) }
  ...
  describe '#delete_async', :saas do
    context 'when target user is the same as deleted_by' do
      let_it_be(:user, freeze: false) { create(:user) }
      let_it_be(:delay_user_account_self_deletion_enabled, freeze: false) { true }
  ...
  describe '#lock_access!' do
    let_it_be_with_reload(:user) { create(:user) }
    let_it_be(:gitlab_admin_bot, freeze: false) { Users::Internal.in_organization(user.organization).admin_bot }
  ...
  describe 'should_use_security_policy_bot_avatar?' do
    ...
    context 'when user is a security policy bot' do
      let_it_be(:user, freeze: false) { create(:user, :security_policy_bot) }
  ...
  describe '#unlock_access!' do
    ...
    context 'when unlocked_by is specified' do
      it_behaves_like 'logs a user_access_unlocked audit event with the correct author' do
        let_it_be(:expected_author, freeze: false) { create(:user) }
  ...
  describe '#contributed_note_groups' do
    ...
    let_it_be(:group_with_wiki, freeze: false) { create(:group) }
    let_it_be(:group_with_epic, freeze: false) { create(:group) }
    let_it_be(:group_aimed_for_deletion, freeze: false) do
      create(:group).tap { |group| create(:group_deletion_schedule, group: group, deleting_user: user) }
    end

    let_it_be(:wiki_page_meta, freeze: false) { create(:wiki_page_meta, :for_wiki_page, container: group_with_wiki) }
    let_it_be(:wiki_note, freeze: false) { create(:note, author: user, project: nil, noteable: wiki_page_meta) }
    let_it_be(:epic_note, freeze: false) { create(:note, author: user, project: nil, noteable: create(:epic, group: group_with_epic)) }
  ...
  describe '#ci_available_runners' do
    ...
    let_it_be_with_refind(:user) { create(:user) }
    let_it_be(:group_a, freeze: false) { create(:group, name: "group-a") }
    let_it_be(:group_aa, freeze: false) { create(:group, parent: group_a, name: "group-aa") }
    let_it_be(:group_aaa, freeze: false) { create(:group, parent: group_aa, name: "group-aaa") }
    let_it_be(:group_ab, freeze: false) { create(:group, parent: group_a, name: "group-ab") }
    let_it_be(:group_aba, freeze: false) { create(:group, parent: group_ab, name: "group-aba") }
    let_it_be(:group_b, freeze: false) { create(:group, name: "group-b") }

    let_it_be(:group_a_runner, freeze: false) { create(:ci_runner, :group, groups: [group_a], name: "a") }
    let_it_be(:group_aa_runner, freeze: false) { create(:ci_runner, :group, groups: [group_aa], name: "aa") }
    let_it_be(:group_aaa_runner, freeze: false) { create(:ci_runner, :group, groups: [group_aaa], name: "aaa") }
    let_it_be(:group_ab_runner, freeze: false) { create(:ci_runner, :group, groups: [group_ab], name: "ab") }
    let_it_be(:group_aba_runner, freeze: false) { create(:ci_runner, :group, groups: [group_aba], name: "aba") }
    let_it_be(:group_b_runner, freeze: false) { create(:ci_runner, :group, groups: [group_b], name: "b") }

    let_it_be(:admin_runners_a, freeze: false) { create(:member_role, :guest, :admin_runners, namespace: group_a) }
    let_it_be(:admin_runners_b, freeze: false) { create(:member_role, :guest, :admin_runners, namespace: group_b) }

    let_it_be(:project_a, freeze: false) { create(:project, group: group_a, name: "a") }
    let_it_be(:project_aa, freeze: false) { create(:project, group: group_aa, name: "aa") }
    let_it_be(:project_aaa, freeze: false) { create(:project, group: group_aaa, name: "aaa") }
    let_it_be(:project_ab, freeze: false) { create(:project, group: group_ab, name: "ab") }
    let_it_be(:project_aba, freeze: false) { create(:project, group: group_aba, name: "aba") }

    let_it_be(:project_a_runner, freeze: false) { create(:ci_runner, :project, projects: [project_a], name: "a") }
    let_it_be(:project_aa_runner, freeze: false) { create(:ci_runner, :project, projects: [project_aa], name: "aa") }
    let_it_be(:project_aaa_runner, freeze: false) { create(:ci_runner, :project, projects: [project_aaa], name: "aaa") }
    let_it_be(:project_ab_runner, freeze: false) { create(:ci_runner, :project, projects: [project_ab], name: "ab") }
    let_it_be(:project_aba_runner, freeze: false) { create(:ci_runner, :project, projects: [project_aba], name: "aba") }
  ...
  context 'normalized email reuse check' do
    ...
    context 'when a saas user has a an email associated with a verified domain', :saas do
      ...
      context 'when the group is paid' do
        let_it_be(:ultimate_group, freeze: false) { create(:group_with_plan, plan: :ultimate_plan) }
        let_it_be(:ultimate_project, freeze: false) { create(:project, group: ultimate_group) }
      ...
      context 'when the root group is not paid' do
        ...
        let_it_be(:free_project, freeze: false) { create(:project, group: free_group) }
  ...
  describe '#owned_free_or_trial_groups_with_limit', :saas do
    ...
    context 'when user has owned free or trial groups' do
      let_it_be(:free_group, freeze: false) { create(:group_with_plan, plan: :free_plan, owners: user) }
      let_it_be(:trial_group, freeze: false) do
