...
RSpec.describe WorkItems::TypesFramework::SystemDefined::Type, feature_category: :team_planning do
  ...
  describe '#filterable_list_view?' do
    ...
    shared_examples "filterable list view for requirement licences types" do
      context "with the license available" do
        before do
          stub_licensed_features(requirements: true)
        end

        it "returns false for requirement type" do
          expect(requirement_type.filterable_list_view?(project)).to be false
        end

        it "returns false for test case type" do
          expect(test_case_type.filterable_list_view?(project)).to be false
        end
      end

      context "with the license not available" do
        before do
    ...
    shared_examples "filterable list view for okrs licenced types" do |expected_with_license:|
      context "when the license is available" do
        before do
          stub_licensed_features(okrs: true)
        end

        it "returns true when okrs_mvc feature flag is enabled" do
          expect(objective_type.filterable_list_view?(resource_parent)).to be expected_with_license
          expect(key_result_type.filterable_list_view?(resource_parent)).to be expected_with_license
        end

        context "when the okr_mvc feature flag is disabled" do
          before do
            stub_feature_flags(okrs_mvc: false)
          end

          it "returns false" do
            expect(objective_type.filterable_list_view?(resource_parent)).to be false
            expect(key_result_type.filterable_list_view?(resource_parent)).to be false
          end
        end
      end
    end

    shared_examples "filterable list view for epic type" do |expected_with_license:|
      context "for epics" do
        context "when the license is available" do
          ...
          it "returns #{expected_with_license} for epic type" do
            expect(epic_type.filterable_list_view?(resource_parent)).to be expected_with_license
          end
        end
        ...
        context "when the license is not available" do
          ...
          it 'returns false for epic type' do
            expect(epic_type.filterable_list_view?(resource_parent)).to be false
          end
        end
      end
    end
    ...
    context 'when resource_parent is a project' do
      ...
      it_behaves_like "filterable list view for epic type", expected_with_license: false
      it_behaves_like "filterable list view for okrs licenced types", expected_with_license: true
      it_behaves_like "filterable list view for requirement licences types"
    end

    context 'when resource_parent is a group' do
      let(:resource_parent) { build(:group) }

      it_behaves_like "filterable list view for epic type", expected_with_license: true
      it_behaves_like "filterable list view for okrs licenced types", expected_with_license: true
      it_behaves_like "filterable list view for requirement licences types"
    end
    ...
    context 'when resource_parent is nil' do
      ...
      it_behaves_like "filterable list view for epic type", expected_with_license: false
      it_behaves_like "filterable list view for okrs licenced types", expected_with_license: false
      it_behaves_like "filterable list view for requirement licences types"
    end

    context 'when resource_parent is a Namespaces::ProjectNamespace' do
      let(:resource_parent) { build(:project).project_namespace }
  ...
  describe '#filterable_board_view?' do
    ...
    let(:all_types) { [epic_type, requirement_type, test_case_type, objective_type, key_result_type] }

    shared_examples "filterable board view for all licences types" do
      it "returns false for all types" do
        all_types.each do |type|
          expect(type.filterable_board_view?(resource_parent)).to be false
        end
      end
    end
    ...
    context 'when resource_parent is a project' do
      ...
      it_behaves_like "filterable board view for all licences types"
    end
