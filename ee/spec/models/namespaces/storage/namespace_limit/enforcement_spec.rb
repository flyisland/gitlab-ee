...
RSpec.describe Namespaces::Storage::NamespaceLimit::Enforcement, :saas,
  ...
  describe '.enforce_limit?' do
    ...
    let_it_be_with_refind(:group) { create(:group_with_plan) }
    ...
    context 'when the group does not have a plan' do
      ...
      let_it_be(:plan_limit, freeze: false) { create(:plan_limits, :free_plan, enforcement_limit: 100) }
  ...
  describe '.show_pre_enforcement_alert?' do
    ...
    let_it_be(:group, freeze: false) { create(:group_with_plan, :with_root_storage_statistics, plan: :free_plan) }
    let_it_be(:namespace_limit, freeze: false) { create(:namespace_limit, namespace: group) }
