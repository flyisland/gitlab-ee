...
RSpec.describe Namespaces::Storage::NamespaceLimit::Enforcement, :saas,
  ...
  describe '.enforce_limit?' do
    ...
    context 'when the group does not have a plan' do
      ...
      let_it_be_with_reload(:plan_limit) { create(:plan_limits, :free_plan, enforcement_limit: 100) }
  ...
  describe '.show_pre_enforcement_alert?' do
    ...
    let_it_be_with_reload(:group) { create(:group_with_plan, :with_root_storage_statistics, plan: :free_plan) }
    let_it_be_with_reload(:namespace_limit) { create(:namespace_limit, namespace: group) }
