...
RSpec.describe Namespaces::Storage::RootSize, :saas, feature_category: :consumables_cost_management do
  ...
  describe '#changes_will_exceed_size_limit?' do
    ...
    context 'when the changes will exceed the size limit' do
      ...
      with_them do
        it 'returns true' do
          expect(model.changes_will_exceed_size_limit?(change_size, project)).to be(true)
        end
      end
    end
    ...
    context 'when the changes will not exceed the size limit' do
      ...
      with_them do
        it 'returns false' do
          expect(model.changes_will_exceed_size_limit?(change_size, project)).to be(false)
        end
      end
    end
    ...
    context 'when storage size limit is 0' do
      ...
      it 'returns false' do
        expect(model.changes_will_exceed_size_limit?(120.megabytes, project)).to be(false)
      end
    end
    ...
    context 'with a project fork' do
      context 'in a paid namespace' do
        ...
        with_them do
          it 'applies a cost factor for forks to the changes size' do
            ...
            expect(model.changes_will_exceed_size_limit?(100.megabytes, project_fork)).to be(false)
          end
        end
      end
      ...
      context 'in a free namespace' do
        ...
        it 'does not apply a cost factor for forks to the changes size for a private fork' do
          ...
          expect(model.changes_will_exceed_size_limit?(100.megabytes, project_fork)).to be(true)
        end
      end
    end
  end
