...
RSpec.describe Namespaces::FreeUserCap::Enforcement, :saas, feature_category: :seat_cost_management do
  let_it_be_with_refind(:namespace) { create(:group_with_plan, :private, plan: :free_plan) }
  ...
  describe '#over_limit?' do
    ...
    context 'when over the number of free users limit' do
      context 'when it is a free plan' do
        ...
        context 'when the namespace qualifies for pre-enforcement' do
          before do
            allow_next_instance_of(::Namespaces::FreeUserCap::PreEnforcement, namespace) do |instance|
              allow(instance).to receive(:qualifies?).and_return(true)
            end
          end
  ...
  describe '#seat_available?' do
    ...
    context 'when the user is a service account' do
      let(:user) { build_stubbed(:service_account) }
