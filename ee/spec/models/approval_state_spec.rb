...
RSpec.describe ApprovalState, feature_category: :code_review_workflow do
  ...
  describe '#approval_context' do
    it 'returns a memoized Approvals::Context instance initialized with merge request' do
      expect(subject.approval_context).to be_a(Approvals::Context)
      expect(subject.approval_context.merge_request).to eq(merge_request)

      first_call = subject.approval_context
      second_call = subject.approval_context
    ...
    describe '#eligible_for_approval_by?' do
      ...
      context 'when there are no regular approval rules' do
        ...
        context 'when committers are not authorized to approve their own MRs' do
          ...
          context 'when approval_committer_emails_from_diff feature flag is disabled' do
            before do
  ...
  describe '#user_defined_rules' do
    ...
    context 'when approval rules are not overwritten' do
      ...
      it 'passes approval_context to wrapped rules' do
        approval_context = subject.approval_context

        project.visible_user_defined_rules(branch: merge_request.target_branch).each do |project_rule|
          expect(ApprovalWrappedRule).to receive(:wrap)
            .with(merge_request, project_rule, context: approval_context)
            .and_call_original
        end

        subject.user_defined_rules
      end
