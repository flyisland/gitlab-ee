...
RSpec.describe ApprovalState, feature_category: :code_review_workflow do
  ...
  let_it_be(:default_approver, freeze: false) { create(:user) }
  let_it_be(:arbitrary_users, freeze: false) { create_list(:user, 2) }

  let_it_be(:group_approver1, freeze: false) { create(:user) }
  let_it_be(:group1) do
    group = create(:group)
  ...
  context 'when only a single rule is allowed' do
    ...
    describe '#eligible_for_approval_by?' do
      ...
      context 'when user is a committer' do
        let_it_be(:user, freeze: false) { create(:user, email: merge_request.commits.without_merge_commits.first.committer_email) }
