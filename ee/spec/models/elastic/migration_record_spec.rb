...
RSpec.describe Elastic::MigrationRecord, :elastic_delete_by_query, feature_category: :global_search do
  ...
  let(:record) do
    described_class.new(version: Time.now.utc.strftime('%s%6N').to_i, name: 'ExampleMigration', filename: nil)
  end
  ...
  describe '#save!' do
    it 'raises an error if the migrations index is not found' do
      index_name = 'gitlab-test-search-migrations'
      allow(helper).to receive(:migrations_index_name).and_return(index_name)
  ...
  describe '#started?' do
    before do
      helper.client.delete_by_query(
        ...
        body: { query: { term: { _id: record.version } } },
        refresh: true
      )
    end
  ...
  describe '.load_versions' do
    ...
    it 'raises an exception if no index present' do
      allow(Search::Elastic::Helper.default.client)
    ...
    it 'raises an exception when exception is raised' do
      allow(Search::Elastic::Helper.default.client).to receive(:search).and_raise(Faraday::ConnectionFailed)
