...
RSpec.describe Elastic::IndexSetting do
  ...
  describe '.for_alias_name' do
    it 'returns records matching the alias name' do
      record = create(:elastic_index_setting, alias_name: 'test_alias')
      create(:elastic_index_setting, alias_name: 'other_alias')

      expect(described_class.for_alias_name('test_alias')).to contain_exactly(record)
    end

    it 'returns empty relation when no match found' do
      expect(described_class.for_alias_name('nonexistent')).to be_empty
    end
  end
