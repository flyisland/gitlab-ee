...
RSpec.describe Ci::Build, :saas, feature_category: :continuous_integration do
  ...
  describe '#features' do
    ...
    it 'includes tracing with trace_id and otel_endpoints for top-level pipeline jobs' do
      expect(features[:tracing]).to eq(
        trace_id: format('%032x', pipeline.id),
        otel_endpoints: [{ url: 'https://otel-collector.gitlab.net/v1/traces' }]
      )
    end
    ...
    context 'when job belongs to a child pipeline' do
      ...
      it 'includes tracing with trace_id from root pipeline, span_parent_id from bridge job, and otel_endpoints' do
        expect(features[:tracing]).to eq(
          trace_id: format('%032x', pipeline.id),
          otel_endpoints: [{ url: 'https://otel-collector.gitlab.net/v1/traces' }],
          span_parent_id: format('%016x', bridge.id)
        )
      end
    end
    ...
    context 'when job belongs to a dangling child pipeline without source bridge' do
      ...
      it 'includes tracing without span_parent_id' do
        expect(features[:tracing]).to eq(
