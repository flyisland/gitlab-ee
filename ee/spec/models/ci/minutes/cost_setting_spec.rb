...
RSpec.describe Ci::Minutes::CostSetting, feature_category: :hosted_runners do
  ...
  subject(:cost_setting) { described_class.new(runner: runner) }
  ...
  describe 'validations' do
    ...
    context 'when the runner is shared' do
      specify { expect(cost_setting).to be_valid }
    end
    ...
    context 'when the runner is not shared' do
      ...
      it 'is invalid' do
        expect(cost_setting).to be_invalid
        expect(cost_setting.errors.messages[:runner]).to include('must be shared')
      end
    end
  end
end
