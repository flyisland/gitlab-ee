...
RSpec.describe Ci::Minutes::NamespaceMonthlyUsage, feature_category: :hosted_runners do
  ...
  describe '.all_previous_usages', :freeze_time do
    ...
    context 'when there are usage records for the previous month' do
      let!(:previous_month_usage_0) { create(:ci_namespace_monthly_usage, namespace: namespace, amount_used: 200, date: current_month - 2.months) }
      let!(:previous_month_usage_1) do
        create(:ci_namespace_monthly_usage, namespace: namespace, amount_used: 200, date: current_month - 2.months, shard_number: 2)
      end

      let!(:other_namespace_usage) { create(:ci_namespace_monthly_usage, amount_used: 200, date: current_month - 2.months) }
      let!(:very_old_usage) { create(:ci_namespace_monthly_usage, namespace: namespace, amount_used: 300, date: current_month - 3.months) }
    ...
    context 'when there are usage records older than the previous month' do
      let!(:old_usage_1) { create(:ci_namespace_monthly_usage, namespace: namespace, amount_used: 300, date: current_month - 3.months) }
      let!(:old_usage_2) { create(:ci_namespace_monthly_usage, namespace: namespace, amount_used: 300, date: current_month - 3.months, shard_number: 2) }
      let!(:other_namespace_usage) { create(:ci_namespace_monthly_usage, amount_used: 200, date: current_month - 3.months) }

      let!(:very_old_usage) { create(:ci_namespace_monthly_usage, namespace: namespace, amount_used: 300, date: current_month - 4.months) }
