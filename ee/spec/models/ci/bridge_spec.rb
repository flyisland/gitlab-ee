...
RSpec.describe Ci::Bridge, feature_category: :continuous_integration do
  ...
  describe 'state machine transitions' do
    ...
    context 'when bridge points towards upstream' do
      ...
      context 'when bridge points towards non-existing upstream' do
        let(:options) { { bridge_needs: { pipeline: 'not/a/project' } } }

        it 'calls the subscription service, which drops the job' do
          bridge.enqueue!
