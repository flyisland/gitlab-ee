...
RSpec.describe Ci::FinishedBuildChSyncEvent, feature_category: :fleet_visibility do
  ...
  describe 'sorting' do
    ...
    describe '.order_by_build_finished_at' do
      let_it_be(:old_build) { create(:ci_build, :success, project: project, finished_at: 3.hours.ago) }
      let_it_be(:mid_build) { create(:ci_build, :failed, project: project, finished_at: 2.hours.ago) }
      let_it_be(:new_build) { create(:ci_build, :failed, project: project, finished_at: 2.hours.ago) }
      let_it_be(:old_event) { create_ci_build_sync_event(old_build) }
      let_it_be(:mid_event) { create_ci_build_sync_event(mid_build) }
      let_it_be(:new_event) { create_ci_build_sync_event(new_build) }
