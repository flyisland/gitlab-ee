...
RSpec.describe Cd::VersionSetEntry, feature_category: :continuous_delivery do
  ...
  describe 'validations' do
    ...
    context 'when another version from the same service already exists in the version set' do
      ...
      it 'rejects a second version for the same service' do
        ...
        expect(record.errors[:service_id]).to include('already has an entry in this version set')
      end
    end
  end
