...
RSpec.describe Cd::ApplicationFlowDefinition, feature_category: :continuous_delivery do
  ...
  describe 'sharding key' do
    context 'when the application belongs to an organization' do
      let_it_be(:application) { create(:cd_application, :for_organization) }
    ...
    context 'when the application belongs to a group' do
      subject { build(:cd_application_flow_definition, application: application) }

      it { is_expected.to populate_sharding_key(:organization_id).with(application.group.organization_id) }
    end
  end
