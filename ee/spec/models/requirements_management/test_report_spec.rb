...
RSpec.describe RequirementsManagement::TestReport, feature_category: :requirements_management do
  ...
  describe 'validations' do
    ...
    context 'requirements associations' do
      subject(:test_report) { build(:test_report, requirement_issue: requirement_issue_arg) }
    ...
    context 'when requirement_issue is not of type requirement' do
      subject(:test_report) { build(:test_report, requirement_issue: create(:issue)) }

      specify do
        expect(test_report).not_to be_valid
        expect(test_report.errors.messages[:requirement_issue]).to include(/must be a `requirement`/)
      end
    end
  end
  ...
  describe '.persist_requirement_reports' do
    ...
    subject(:persist_reports) { described_class.persist_requirement_reports(build, ci_report) }
    ...
    context 'if the CI report contains no entries' do
      ...
      it 'does not create any test reports' do
        expect { persist_reports }.not_to change { RequirementsManagement::TestReport.count }
      end
    end
      ...
      context 'and the entries are valid' do
        ...
        context 'and legacy is false' do
          ...
          it 'creates test report with expected status for each open requirement' do
            expect { persist_reports }.to change { RequirementsManagement::TestReport.count }.by(2)
        ...
        context 'when legacy is true' do
          ...
          subject(:persist_reports) { described_class.persist_requirement_reports(build, ci_report, legacy: true) }
