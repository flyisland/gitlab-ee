...
RSpec.describe Analytics::CustomDashboards::DashboardVersion, feature_category: :custom_dashboards_foundation do
  ...
  describe '.for_custom_dashboards' do
    it 'returns only versions belonging to the given dashboards' do
      version = create(:dashboard_version)
      create(:dashboard_version)

      expect(described_class.for_custom_dashboards(version.custom_dashboard_id)).to contain_exactly(version)
    end
  end
