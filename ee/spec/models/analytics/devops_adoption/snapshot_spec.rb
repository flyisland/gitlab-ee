...
RSpec.describe Analytics::DevopsAdoption::Snapshot, feature_category: :devops_reports do
  ...
  describe '.latest_for_namespace_ids' do
    it 'returns for previous month finalized snapshot for the given namespace ids based on snapshot end_time' do
      travel_to(Date.new(2021, 7, 15)) do
        group1 = create(:group)
        group1_latest_snapshot = create(:devops_adoption_snapshot, namespace: group1,
          end_time: 1.month.ago.end_of_month, recorded_at: 1.day.ago)
        create(:devops_adoption_snapshot, namespace: group1, end_time: 2.months.ago.end_of_month,
  ...
  describe '.for_timespan' do
    ...
    it 'returns snapshots for given timespan', :aggregate_failures do
      ...
      expect(described_class.for_timespan(from: first_date + 1.week,
        to: first_date + 40.days)).to match_array([snapshot2])
    end
  end
  ...
  describe '.for_namespaces' do
    it 'returns all snapshots with given namespaces' do
      ...
      expect(described_class.for_namespaces([snapshot1.namespace,
        snapshot2.namespace])).to match_array([snapshot1, snapshot2])
    end
  end
