...
RSpec.describe Analytics::CycleAnalytics::IssueStageEvent, feature_category: :value_stream_management do
  ...
  let_it_be(:project, freeze: false) { create(:project, namespace: create(:group), developers: user) }
  let_it_be(:issue1, freeze: false) { create(:issue, project: project) }
  let_it_be(:issue2, freeze: false) { create(:issue, project: project) }
  let_it_be(:issue3, freeze: false) { create(:issue, project: project, assignees: [user]) }

  let_it_be(:epic1, freeze: false) { create(:epic, group: project.group) }
  let_it_be(:epic2, freeze: false) { create(:epic, group: project.group) }

  let_it_be(:epic_issue1, freeze: false) { create(:epic_issue, epic: epic1, issue: issue1) }
  let_it_be(:epic_issue2, freeze: false) { create(:epic_issue, epic: epic2, issue: issue3) }

  let_it_be(:stage_event1, freeze: false) do
  ...
  let_it_be(:stage_event2, freeze: false) do
    create(:cycle_analytics_issue_stage_event,
      author_id: nil,
      weight: nil,
      sprint_id: nil,
  ...
  let_it_be(:stage_event3, freeze: false) do
    create(:cycle_analytics_issue_stage_event,
      author_id: 4,
      weight: 5,
      sprint_id: 6,
