...
description: with missing properties
      ...
      it 'does not parse the rest of the definition' do
        ...
        expect(dashboard.views).to be_nil
      end
    end
...
views: not a list
      ...
      it 'returns schema errors instead of raising' do
        expect(dashboard.errors).to match_array([
          "property '/panels' is not of type: array",
          "property '/views' is not of type: array"
        ])
      end
    end
  ...
  describe '#views' do
    let(:dashboard) do
      config_yaml = File.open(Rails.root.join("ee/spec/fixtures/analytics/#{fixture}.yaml")).read
    ...
    subject(:views) { dashboard.views }

    context 'when the dashboard defines views' do
      let_it_be(:config_project) do
        create(:project, :with_product_analytics_dashboard_with_views, group: group)
      end

      let(:fixture) { 'dashboard_example_with_views' }

      it 'returns a view per definition, each with its own panels' do
        expect(views).to match([
          {
            title: 'Conversions',
            panels: [have_attributes(title: 'Overall Conversion Rate')]
          },
          {
            title: 'Audience',
            panels: [
              have_attributes(title: 'Visitors by month'),
              have_attributes(title: 'Visitors over time')
            ]
          }
        ])
      end

      it 'builds the panel visualizations' do
        expect(views.first[:panels].first.visualization)
          .to have_attributes(slug: 'dora_line_chart', type: 'LineChart')
      end
    end

    context 'when the dashboard does not define views' do
      let(:fixture) { 'dashboard_example_1' }
