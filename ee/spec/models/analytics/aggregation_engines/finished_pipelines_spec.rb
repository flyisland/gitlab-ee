...
RSpec.describe Analytics::AggregationEngines::FinishedPipelines, :click_house, time_travel_to: '2026-01-30',
  ...
  before do
    clickhouse_fixture(:ci_finished_pipelines, pipelines_data.map do |pipeline|
      pipeline.reverse_merge(
        path: project1.project_namespace.traversal_path,
        source: 'push',
        ref: 'main',
  ...
  describe '.prepare_base_aggregation_scope' do
    ...
    let(:pipelines_data) do
      [
        { id: 1, path: project_in_group.project_namespace.traversal_path,
          status: 'success', duration: 60, started_at: 10.days.ago },
        { id: 2, path: project_in_subgroup.project_namespace.traversal_path,
          status: 'success', duration: 60, started_at: 10.days.ago },
        { id: 3, path: other_project.project_namespace.traversal_path,
          status: 'success', duration: 60, started_at: 10.days.ago }
      ]
    end
    ...
    with_them do
      it 'returns records scoped to the provided context object' do
        expect(result.map { |r| r['id'] }).to match_array(expected_ids) # rubocop:disable Rails/Pluck -- ClickHouse result is a plain Ruby array, not an ActiveRecord relation
      end
    end
  end
