...
RSpec.describe Vulnerabilities::IssueLink, feature_category: :vulnerability_management, factory_default: :keep do
  let_it_be(:project) { create(:project) }
  let_it_be(:author) { create_default(:author) }
  ...
  describe 'associations and fields' do
    ...
    it 'provides the "related" as default link_type' do
      expect(build_stubbed(:vulnerabilities_issue_link).link_type).to eq 'related'
    end
  end
  ...
  describe 'validations' do
    ...
    describe 'only one "created" link allowed per vulnerability' do
      let_it_be(:existing_link) do
        create(:vulnerabilities_issue_link, :created,
          vulnerability: create(:vulnerability, project: project),
          issue: create(:issue, project: project))
      end
  ...
  describe '.by_link_type' do
    ...
    let_it_be(:created_issue_link) do
      create(:vulnerabilities_issue_link, :created,
        vulnerability: vulnerability, issue: create(:issue, project: project))
    end

    let_it_be(:related_issue_link) do
      create(:vulnerabilities_issue_link, :related,
        vulnerability: vulnerability, issue: create(:issue, project: project))
    end
  ...
  describe '.for_issue' do
    ...
    let_it_be(:created_issue_link) do
      create(:vulnerabilities_issue_link, :created,
        issue: issue, vulnerability: create(:vulnerability, project: project))
    end
  ...
  context 'with loose foreign key on vulnerabilities_issue_link.issue_id' do
    it_behaves_like 'cleanup by a loose foreign key' do
      let_it_be(:parent) { create(:issue, project: project) }
      let_it_be(:model) do
        create(:vulnerabilities_issue_link, issue: parent, vulnerability: create(:vulnerability, project: project))
      end
    end
  end
