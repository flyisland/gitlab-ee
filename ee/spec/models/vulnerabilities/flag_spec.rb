...
RSpec.describe Vulnerabilities::Flag, feature_category: :vulnerability_management do
  ...
  describe '#dismissable?' do
    ...
    context 'when vulnerability_flag_status_based_gating is enabled' do
      before do
        stub_feature_flags(vulnerability_flag_status_based_gating: project)
      end
      ...
      it 'returns true for detected_as_fp status' do
        ...
        expect(flag.dismissable?(project)).to be true
      end
      ...
      it 'returns false for not_started status' do
        ...
        expect(flag.dismissable?(project)).to be false
      end
    ...
    context 'when vulnerability_flag_status_based_gating is disabled' do
      before do
        stub_feature_flags(vulnerability_flag_status_based_gating: false)
      end

      it 'returns true when confidence_score is above 0' do
        flag = create(:vulnerabilities_flag, finding: finding, status: :not_started, confidence_score: 0.8, origin: 'o6')
        expect(flag.dismissable?(project)).to be true
      end

      it 'returns false when confidence_score is 0' do
        flag = create(:vulnerabilities_flag, finding: finding, status: :detected_as_fp, confidence_score: 0.0, origin: 'o7')
        expect(flag.dismissable?(project)).to be false
      end
    end
  end
  ...
  describe '#triggers_resolution_workflow?' do
    ...
    context 'when vulnerability_flag_status_based_gating is enabled' do
      ...
      it 'returns true for failed status' do
        ...
        expect(flag.triggers_resolution_workflow?(project)).to be true
      end
      ...
      it 'returns false for detected_as_fp status' do
        ...
        expect(flag.triggers_resolution_workflow?(project)).to be false
      end
    ...
    context 'when vulnerability_flag_status_based_gating is disabled' do
      ...
      it 'returns true when confidence_score is below threshold' do
        flag = create(:vulnerabilities_flag, finding: finding, status: :not_started, confidence_score: 0.5, origin: 'o12')
        expect(flag.triggers_resolution_workflow?(project)).to be true
      end

      it 'returns false when confidence_score is at threshold' do
        flag = create(:vulnerabilities_flag, finding: finding, status: :not_started, confidence_score: 0.6, origin: 'o13')
        expect(flag.triggers_resolution_workflow?(project)).to be false
      end
    end
  end
        ...
        context 'when origin is ai_sast_fp_detection' do
          ...
          context 'when confidence score is below threshold' do
            context 'when finding is eligible for resolution workflow' do
              ...
              context 'when no user has duo_workflow permission' do
                before do
                  allow(Ability).to receive(:allowed?).with(user, :duo_workflow, project).and_return(false)
                  allow(Ability).to receive(:allowed?)
                    .with(project.first_human_owner, :duo_workflow, project).and_return(false)
                end
              ...
              context 'when project has no first_human_owner' do
                before do
                  allow(project).to receive(:first_human_owner).and_return(nil)
                  allow(Ability).to receive(:allowed?).with(user, :duo_workflow, project).and_return(true)
                end

                it 'falls back to vulnerability author and triggers resolution workflow' do
                  expect(::Vulnerabilities::TriggerResolutionWorkflowWorker)
                    .to receive(:perform_async)
                          .with(anything)
              ...
              context 'when finding has no vulnerability and no first_human_owner' do
                before do
                  allow(project).to receive(:first_human_owner).and_return(nil)
                  allow(finding).to receive(:vulnerability).and_return(nil)
                  allow(Ability).to receive(:allowed?).with(nil, :duo_workflow, project).and_return(false)
                end
              ...
              context 'when only author has duo_workflow permission' do
                before do
                  allow(Ability).to receive(:allowed?).with(user, :duo_workflow, project).and_return(true)
                  allow(Ability).to receive(:allowed?)
                    .with(project.first_human_owner, :duo_workflow, project).and_return(false)
              ...
              context 'when only project first_human_owner has duo_workflow permission' do
                before do
                  allow(Ability).to receive(:allowed?).with(user, :duo_workflow, project).and_return(false)
                  allow(Ability).to receive(:allowed?)
                    .with(project.first_human_owner, :duo_workflow, project).and_return(true)
                end
              ...
              context 'when finding has no vulnerability' do
                ...
                it 'checks first_human_owner only and triggers resolution workflow' do
                  expect(::Vulnerabilities::TriggerResolutionWorkflowWorker)
                    .to receive(:perform_async)
                          .with(anything)
  ...
  describe '#should_trigger_resolution_workflow? with status-based gating' do
    ...
    let(:flag) { create(:vulnerabilities_flag, finding: finding, confidence_score: 0.5, origin: described_class::AI_SAST_FP_DETECTION_ORIGIN) }

    before do
      stub_feature_flags(vulnerability_flag_status_based_gating: true)
      project.project_setting.update!(duo_sast_vr_workflow_enabled: true)
      allow(finding).to receive(:eligible_for_resolution_workflow?).and_return(true)
      allow(Ability).to receive(:allowed?).and_call_original
      allow(Ability).to receive(:allowed?)
