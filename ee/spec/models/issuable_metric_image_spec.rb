...
RSpec.describe IssuableMetricImage, feature_category: :observability do
  ...
  describe 'scopes' do
    ...
    let_it_be(:image_1, freeze: false) { create(:issuable_metric_image, issue: issue) }
    let_it_be(:image_2, freeze: false) { create(:issuable_metric_image, issue: issue) }
  ...
  describe '#file_path' do
    ...
    context 'when file is nil' do
      before do
        allow(issuable_metric_image).to receive(:file).and_return(nil)
      end

      it { is_expected.to be_nil }
    end

    context 'when file has no upload' do
      before do
        allow(issuable_metric_image.file).to receive_messages(upload: nil, url: expected_path)
      end

      it 'returns the file url directly' do
