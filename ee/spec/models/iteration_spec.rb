...
RSpec.describe Iteration, feature_category: :team_planning do
  ...
  describe 'IID tracking during import' do
    ...
    context 'when importing is true and iid is set externally' do
      it 'does not call InternalId.track_greatest' do
        iteration = build(:iteration, group: group, iterations_cadence: cadence, skip_future_date_validation: true)
        iteration.iid = 100
        iteration.importing = true
    ...
    context 'when importing is false and iid is set externally' do
  ...
  describe '#display_text' do
    ...
    let_it_be(:iteration, freeze: false) { create(:iteration, iterations_cadence: iterations_cadence, start_date: Date.new(2022, 9, 30), due_date: Date.new(2022, 10, 4)) }
  ...
  describe '#title' do
    let_it_be(:iteration, freeze: false) { create(:iteration, title: "foobar", group: create(:group)) }
  ...
  context 'Validations' do
    ...
    describe '#dates_do_not_overlap' do
      let_it_be(:existing_iteration, freeze: false) { create(:iteration, iterations_cadence: iteration_cadence, start_date: 4.days.from_now, due_date: 1.week.from_now) }
      ...
      context 'when dates overlap' do
        ...
        context 'group' do
          ...
          context 'sub-group' do
            ...
            let_it_be(:subgroup_ic) { create(:iterations_cadence, group: subgroup) }
  ...
  describe 'callbacks', :aggregate_failures do
    ...
    describe 'after_save :update_iteration_sequences' do
      context 'when start_date or due_date is not changed after save' do
        let_it_be_with_reload(:iteration) { create(:iteration, iterations_cadence: iteration_cadence, start_date: Date.today, due_date: Date.today + 2.days) }
  ...
  it_behaves_like 'a timebox', :iteration do
    ...
    let_it_be(:cadence) { create(:iterations_cadence, group: group) }
    let_it_be(:timebox) { create(:iteration, iterations_cadence: cadence) }
