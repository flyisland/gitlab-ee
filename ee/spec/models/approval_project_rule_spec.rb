...
RSpec.describe ApprovalProjectRule, feature_category: :compliance_management do
  ...
  describe '.report_approver_without_policy_report_types' do
    ...
    let_it_be(:code_coverage_rule, freeze: false) { create(:approval_project_rule, :code_coverage) }
  ...
  describe '#applies_to_branch?' do
    ...
    context 'when rule applies to all protected branches' do
      let_it_be(:wildcard_protected_branch, freeze: false) { create(:protected_branch, name: "stable-*") }
  ...
  describe "#apply_report_approver_rules_to" do
    ...
    let_it_be(:merge_request, freeze: false) { create(:merge_request, target_branch: 'main') }
    let_it_be(:project, freeze: false) { merge_request.target_project }
    let_it_be(:user, freeze: false) { create(:user) }
    let_it_be(:group, freeze: false) { create(:group) }

    let_it_be(:approval_policy_rule, freeze: false) { create(:approval_policy_rule) }

    let_it_be(:security_orchestration_policy_configuration, freeze: false) do
      create(:security_orchestration_policy_configuration, project: project)
    end

    let_it_be(:protected_branch, freeze: false) { create(:protected_branch, project: project, name: merge_request.target_branch) }
    let_it_be(:non_matching_protected_branch, freeze: false) { create(:protected_branch, project: project, name: 'non-matching-branch') }
    ...
    describe 'retry on ActiveRecord::RecordNotUnique' do
      ...
      it 'retries once and succeeds when the unique index conflicts' do
        call_count = 0
        allow(approval_rule).to receive(:update_report_approver_rule_for_merge_request).and_wrap_original do |original, *args, &blk|
          call_count += 1
  ...
  describe 'callbacks', :request_store do
    let_it_be(:user, freeze: false) { create(:user, name: 'Batman') }
