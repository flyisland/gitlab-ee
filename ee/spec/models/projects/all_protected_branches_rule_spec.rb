...
RSpec.describe Projects::AllProtectedBranchesRule, feature_category: :source_code_management do
  ...
  describe '#any_rules?' do
    context 'when there are no approval rules and no status checks' do
      it 'returns false' do
        ...
        expect(subject.any_rules?).to be(false)
      end
    end

    context 'when there are approval rules' do
      it 'returns true' do
        expect(subject.any_rules?).to be(true)
      end
    end
  end
  ...
  describe '#default_branch?' do
    it 'returns false' do
      expect(subject.default_branch?).to be(false)
    end
  end

  describe '#matching_branches_count' do
    let_it_be(:protected_branch_1, freeze: false) { create(:protected_branch, name: "conflict-*", project: project) }
  ...
  describe '#protected?' do
    it 'returns false' do
      expect(subject.protected?).to be(false)
    end
  end
