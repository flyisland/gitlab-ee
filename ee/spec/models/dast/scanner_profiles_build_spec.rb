...
RSpec.describe Dast::ScannerProfilesBuild, :dynamic_analysis,
  ...
  describe 'validations' do
    ...
    context 'when the ci_build.project_id and dast_scanner_profile.project_id do not match' do
      ...
      subject(:scanner_profiles_build) do
        build(:dast_scanner_profiles_build,
          ci_build: ci_build, dast_scanner_profile: scanner_profile)
      end

      it 'is not valid', :aggregate_failures do
        expect(scanner_profiles_build).not_to be_valid
        expect(scanner_profiles_build.errors.full_messages)
          .to include('Ci build project_id must match dast_scanner_profile.project_id')
      end
    end
  end
