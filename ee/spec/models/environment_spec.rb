...
RSpec.describe Environment, :use_clean_rails_memory_store_caching, feature_category: :environment_management do
  ...
  describe '.deployed_to_cluster' do
    ...
    context 'when there is a deployment for the cluster' do
      ...
      let(:last_deployment) do
        create(:deployment, :success, :on_cluster, sha: environment.project.repository.commit.sha, environment: environment)
      end
    ...
    context 'when there is a non-cluster deployment' do
      ...
      before do
        create(:deployment, :success, sha: environment.project.repository.commit.sha, environment: environment)
      end
    ...
    context 'when the non-cluster deployment is latest' do
      ...
      before do
        create(:deployment, :success, sha: environment.project.repository.commit.sha, deployment_cluster: deployment_cluster, environment: environment)
        create(:deployment, :success, sha: environment.project.repository.commit.sha, environment: environment)
      end
  ...
  describe '#reactive_cache_updated' do
    ...
    context 'with a group cluster' do
      ...
      before do
        create(:deployment, :success, sha: project.repository.commit.sha, environment: environment, deployment_cluster: deployment_cluster)
      end
  ...
  describe '#find_approval_rule_for' do
    ...
    context 'with approval rules' do
      let_it_be(:approval_rule_for_qa) do
        create(:protected_environment_approval_rule, group: qa_group, protected_environment: protected_environment)
      end

      let_it_be(:approval_rule_for_security) do
        create(:protected_environment_approval_rule, group: security_group, protected_environment: protected_environment)
      end
