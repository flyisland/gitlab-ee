...
RSpec.describe SecretsManagement::GroupSecretsManager, feature_category: :secrets_management do
  ...
  describe '#full_group_namespace_path' do
    it 'returns the group path nested under root namespace for a root group' do
      root_group = create(:group)
      sm = create(:group_secrets_manager, group: root_group)

      expect(sm.full_group_namespace_path).to eq("group_#{root_group.id}/group_#{root_group.id}")
    end

    it 'returns the group path nested under root namespace for a nested group' do
      parent_group = create(:group)
      nested_group = create(:group, parent: parent_group)
      sm = create(:group_secrets_manager, group: nested_group)

      expect(sm.full_group_namespace_path).to eq("group_#{parent_group.id}/group_#{nested_group.id}")
    end

    it 'uses the root ancestor for a deeply nested group' do
      root_group = create(:group)
      subgroup_a = create(:group, parent: root_group)
      subgroup_b = create(:group, parent: subgroup_a)
      sm = create(:group_secrets_manager, group: subgroup_b)

      expect(sm.full_group_namespace_path).to eq("group_#{root_group.id}/group_#{subgroup_b.id}")
    end
  end
  ...
  describe '#root_namespace_path' do
    context 'on create' do
      ...
      let_it_be(:second_level_group) { create(:group, parent: root_group) }
      let_it_be(:third_level_group) { create(:group, parent: second_level_group) }

      it 'persists the value for a root group' do
        new_record = create(:group_secrets_manager, group: root_group)

        expect(new_record.reload.root_namespace_path).to eq("group_#{root_group.id}")
      end

      it 'persists the root ancestor id for a nested group' do
        new_record = create(:group_secrets_manager, group: second_level_group)

        expect(new_record.reload.root_namespace_path).to eq("group_#{root_group.id}")
      end

      it 'persists the root ancestor id for a deeply nested group' do
        new_record = create(:group_secrets_manager, group: third_level_group)
