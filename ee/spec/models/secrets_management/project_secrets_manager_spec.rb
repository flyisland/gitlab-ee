...
RSpec.describe SecretsManagement::ProjectSecretsManager, feature_category: :secrets_management do
  ...
  describe '#ci_auth_path' do
    ...
    context 'when project_secrets_cel_pipeline_auth feature flag is disabled' do
      before do
        stub_feature_flags(project_secrets_cel_pipeline_auth: false)
      end

      it 'returns legacy login path' do
        expect(path).to eq('some/namespace/project_1/auth/ci_auth/login')
      end
    end
  end

  describe '#use_cel_pipeline_auth?' do
    it 'returns true' do
      expect(secrets_manager.use_cel_pipeline_auth?).to be true
    end
    ...
    context 'when project_secrets_cel_pipeline_auth feature flag is disabled' do
      ...
      it 'returns false' do
        expect(secrets_manager.use_cel_pipeline_auth?).to be false
      end
    end
  end
