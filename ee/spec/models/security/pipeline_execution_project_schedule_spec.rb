...
RSpec.describe Security::PipelineExecutionProjectSchedule, feature_category: :security_policy_management do
  describe 'validations' do
    ...
    context 'when security policy is not a pipeline_execution_schedule_policy' do
      let_it_be(:security_policy, freeze: false) { create(:security_policy, :pipeline_execution_policy) }
  ...
  describe 'scopes' do
    describe '.for_project' do
      ...
      let_it_be(:schedule, freeze: false) { create(:security_pipeline_execution_project_schedule, project: project) }
      let_it_be(:other_schedule, freeze: false) { create(:security_pipeline_execution_project_schedule) }
    ...
    describe '.runnable_schedules' do
      let_it_be(:runnable_schedule, freeze: false) { create(:security_pipeline_execution_project_schedule) }
      let_it_be(:future_schedule, freeze: false) { create(:security_pipeline_execution_project_schedule) }
    ...
    describe '.ordered_by_next_run_at' do
      let_it_be(:monthly_schedule, freeze: false) { create(:security_pipeline_execution_project_schedule) }
      let_it_be(:weekly_schedule, freeze: false) { create(:security_pipeline_execution_project_schedule) }
      let_it_be(:daily_schedule, freeze: false) { create(:security_pipeline_execution_project_schedule) }
      let_it_be(:daily_schedule_2, freeze: false) { create(:security_pipeline_execution_project_schedule) }
    ...
    describe '.including_security_policy_and_project' do
      let_it_be(:schedule_1, freeze: false) { create(:security_pipeline_execution_project_schedule) }
      let_it_be(:schedule_2, freeze: false) { create(:security_pipeline_execution_project_schedule) }
    ...
    describe '.for_policy' do
      let_it_be(:policy, freeze: false) { create(:security_policy, :pipeline_execution_schedule_policy) }
      let_it_be(:schedule, freeze: false) do
        create(:security_pipeline_execution_project_schedule, security_policy: policy)
      end
  ...
  describe 'callbacks' do
    describe 'update next_run_at on create', time_travel_to: '2024-12-20 00:00:00' do
      let_it_be(:security_policy, freeze: false) { create(:security_policy, :pipeline_execution_schedule_policy) }
      ...
      it 'sets next_run_at to at least the next cron time' do
      ...
      it 'bakes random offset into next_run_at within effective_time_window' do
        save!

        base_cron_time = Time.zone.now + 1.day

        expect(schedule.next_run_at).to be >= base_cron_time
        expect(schedule.next_run_at).to be <= base_cron_time + 3600.seconds
      end

      it 'persists next_run_applied_delay' do
        save!

        expect(schedule.next_run_applied_delay).to be_a(Integer)
        expect(schedule.next_run_applied_delay).to be >= 0
        expect(schedule.next_run_applied_delay).to be <= 3600
      end
    end
  end
