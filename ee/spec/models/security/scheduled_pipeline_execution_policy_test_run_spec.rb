...
RSpec.describe Security::ScheduledPipelineExecutionPolicyTestRun, feature_category: :security_policy_management do
  ...
  describe 'validations' do
    ...
    context 'for pipeline uniqueness' do
      let_it_be(:existing_test_run) { create(:security_pipeline_execution_policy_test_run) }

      it 'does not allow duplicate pipeline_id' do
        duplicate_test_run = build(:security_pipeline_execution_policy_test_run, pipeline: existing_test_run.pipeline)

        expect(duplicate_test_run).not_to be_valid
        expect(duplicate_test_run.errors[:pipeline_id]).to include('has already been taken')
      end

      it 'allows nil pipeline_id for multiple records' do
        create(:security_pipeline_execution_policy_test_run, pipeline: nil)
        second_test_run = build(:security_pipeline_execution_policy_test_run, pipeline: nil)

        expect(second_test_run).to be_valid
      end
    end
