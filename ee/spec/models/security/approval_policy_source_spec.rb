...
RSpec.describe Security::ApprovalPolicySource, feature_category: :security_policy_management do
  ...
  describe 'linked approval merge request rules' do
    let_it_be(:merge_request) { create(:merge_request, source_project: project) }

    it 'returns rules attached to the approval_policy_rule on the new path' do
      mr_rule = create(:report_approver_rule, :scan_finding,
        merge_request: merge_request, approval_policy_rule: approval_policy_rule)
      source = build_source(approval_policy_rule: approval_policy_rule)

      expect(source.approval_merge_request_rules).to include(mr_rule)
    end

    it 'returns rules attached to the scan_result_policy_read on the legacy path' do
      stub_feature_flags(deprecate_scan_result_policies: false)
      mr_rule = create(:report_approver_rule, :scan_finding,
        merge_request: merge_request, scan_result_policy_read: scan_result_policy_read)
      source = build_source(scan_result_policy_read: scan_result_policy_read)

      expect(source.approval_merge_request_rules).to include(mr_rule)
    end

    it 'returns ApprovalMergeRequestRule.none when no source is set' do
      expect(build_source.approval_merge_request_rules).to eq(ApprovalMergeRequestRule.none)
    end
  end
