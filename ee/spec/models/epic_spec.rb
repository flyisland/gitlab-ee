...
RSpec.describe Epic, feature_category: :portfolio_management do
  ...
  let_it_be(:descendant_group, freeze: false) { create(:group, parent: group) }
  ...
  describe 'associations' do
    ...
    context 'award_emoji' do
      ...
      let_it_be(:emoji_1, freeze: false) { create(:award_emoji, :upvote, awardable: work_item, user: create(:user)) }
      let_it_be(:emoji_2, freeze: false) { create(:award_emoji, :downvote, awardable: epic, user: create(:user)) }

      it 'delegates award emoji to work item', :aggregate_failures do
      ...
      it 'reads participants from award emoji on work item' do
      ...
      describe '#find' do
        it 'delegates find to work item award emoji' do
      ...
      context 'when epic is deleted' do
        ...
        let_it_be(:emoji_1, freeze: false) { create(:award_emoji, awardable: work_item) }
        let_it_be(:emoji_2, freeze: false) { create(:award_emoji, awardable: work_item) }
        let_it_be(:emoji_3, freeze: false) { create(:award_emoji, awardable: work_item) }
        let_it_be(:emoji_4, freeze: false) { create(:award_emoji) } # Not to be deleted

        it 'deletes award emoji when work item is destroyed' do
  ...
  describe 'scopes' do
    let_it_be(:confidential_epic, freeze: false) { create(:epic, confidential: true, group: group, title: 'Foo 1') }
    let_it_be(:public_epic, freeze: false) { create(:epic, group: group, title: 'Foo 2') }
    ...
    describe 'relative position scopes' do
      let_it_be(:board, freeze: false) { create(:epic_board) }
      let_it_be(:other_board, freeze: false) { create(:epic_board) }
      let_it_be(:epic1, freeze: false) { create(:epic) }
      let_it_be(:epic2, freeze: false) { create(:epic) }
      let_it_be(:epic3, freeze: false) { create(:epic) }

      let_it_be(:position1, freeze: false) { create(:epic_board_position, epic: epic1, epic_board: board, relative_position: 20) }
      let_it_be(:position2, freeze: false) { create(:epic_board_position, epic: epic2, epic_board: board, relative_position: 10) }
      let_it_be(:position3, freeze: false) { create(:epic_board_position, epic: epic3, epic_board: board, relative_position: 20) }
      # this position should be ignored because it's for other board:
      let_it_be(:position5, freeze: false) { create(:epic_board_position, epic: confidential_epic, epic_board: other_board, relative_position: 5) }
    ...
    describe 'title sort scopes' do
      let_it_be(:epic1, freeze: false) { create(:epic, title: 'foo') }
      let_it_be(:epic2, freeze: false) { create(:epic, title: 'bar') }
      let_it_be(:epic3, freeze: false) { create(:epic, title: 'baz') }
      let_it_be(:epic4, freeze: false) { create(:epic, title: 'Baz 2') }
    ...
    describe 'from_id' do
      let_it_be(:max_id, freeze: false) { described_class.maximum(:id) }
      let_it_be(:epic1, freeze: false) { create(:epic, id: max_id + 1) }
      let_it_be(:epic2, freeze: false) { create(:epic, id: max_id + 2) }
      let_it_be(:epic3, freeze: false) { create(:epic, id: max_id + 3) }
    ...
    describe '.order_by_hierarchy_order' do
      let_it_be(:group, freeze: false) { create(:group) }
      let_it_be(:root_epic, freeze: false) { create(:epic, group: group) }
      let_it_be(:ancestor_epic, freeze: false) { create(:epic, group: group, parent: root_epic) }
      let_it_be(:parent_epic, freeze: false) { create(:epic, group: group, parent: ancestor_epic) }
      let_it_be(:child_epic, freeze: false) { create(:epic, group: group, parent: parent_epic) }
      let_it_be(:other_epic1, freeze: false) { create(:epic, group: group) }
      let_it_be(:other_epic2, freeze: false) { create(:epic, group: group) }
      ...
      context 'when hierarchy_order parameter is provided' do
        it 'returns epics ordered by hierarchy position ascending' do
          epics = described_class.where(id: [root_epic.id, ancestor_epic.id, parent_epic.id])
                                 .order_by_hierarchy_order(child_epic, hierarchy_order: :asc)

          expect(epics).to eq([parent_epic, ancestor_epic, root_epic])
        end

        it 'returns epics ordered by hierarchy position descending' do
          epics = described_class.where(id: [root_epic.id, ancestor_epic.id, parent_epic.id])
                                 .order_by_hierarchy_order(child_epic, hierarchy_order: :desc)

          expect(epics).to eq([root_epic, ancestor_epic, parent_epic])
        end

        it 'returns the original scope when child has no ancestors and hierarchy_order is provided' do
          epics = described_class.where(id: other_epic1.id)
                                 .order_by_hierarchy_order(other_epic1, hierarchy_order: :desc)
  ...
  describe 'validations' do
    ...
    context 'when parent is set' do
      ...
      context 'when epic parent is not the same as the work item parent' do
        let_it_be(:parent_epic, freeze: false) { create(:epic) }
        let_it_be(:other_epic, freeze: false) { create(:epic) }
        let_it_be(:child_epic, freeze: false) { build(:epic, parent: parent_epic) }
    ...
    describe 'children count' do
      let_it_be(:parent_epic, freeze: false) { create(:epic, group: group) }
      let_it_be(:child_epic1, freeze: false) { create(:epic, group: group, parent: parent_epic) }
      let_it_be(:child_epic2, freeze: false) { create(:epic, group: group, parent: parent_epic) }
      let_it_be(:child_issue, freeze: false) { create(:issue, project: project, title: "Child") }
      let_it_be(:epic_issue, freeze: false) { create(:epic_issue, epic: parent_epic, issue: child_issue) }
    ...
    describe 'maximum hierarchy depth' do
      let_it_be(:group, freeze: false) { create(:group) }
      let_it_be_with_reload(:level_1) { create(:epic, group: group) }
      let_it_be_with_reload(:level_2) { create(:epic, parent: level_1, group: group) }
      let_it_be_with_reload(:level_3) { create(:epic, parent: level_2, group: group) }
  ...
  describe '#max_children_count_achieved?' do
    ...
    let_it_be(:child_epic1, freeze: false) { create(:epic, group: group, parent: parent_epic) }
    let_it_be(:child_epic2, freeze: false) { create(:epic, group: group, parent: parent_epic) }
    let_it_be(:child_issue1, freeze: false) { create(:issue, project: project, title: "Child") }
    let_it_be(:epic_issue, freeze: false) { create(:epic_issue, epic: parent_epic, issue: child_issue1) }
  ...
  describe 'callbacks' do
    ...
    let_it_be(:child_epic, freeze: false) { create(:epic, group: group, parent: epic, start_date: Date.new(1998, 1, 1), end_date: Date.new(1999, 1, 1)) }
    let_it_be(:another_epic, freeze: false) { create(:epic, group: group) }
  ...
  describe 'modules' do
    ...
    describe 'IID tracking during import' do
      let_it_be(:group, freeze: true) { create(:group) }
      let(:epic) { build(:epic, group: group) }

      context 'when iid is set externally' do
        before do
          epic.iid = 100
        end

        it 'calls InternalId.track_greatest' do
          expect(InternalId).to receive(:track_greatest).and_call_original

          epic.save!
        end

        context 'and importing is true' do
          before do
            epic.importing = true
          end

          it 'does not call InternalId.track_greatest' do
  ...
  describe 'ordering' do
    let_it_be(:epic1) { create(:epic, start_date: 7.days.ago, end_date: 3.days.ago, updated_at: 3.days.ago, created_at: 7.days.ago, relative_position: 3, title: 'foo') }
    let_it_be(:epic2) { create(:epic, start_date: 3.days.ago, updated_at: 10.days.ago, created_at: 12.days.ago, relative_position: 1, title: 'bar') }
    let_it_be(:epic3) { create(:epic, end_date: 5.days.ago, updated_at: 5.days.ago, created_at: 6.days.ago, relative_position: 2, title: 'baz') }
    let_it_be(:epic4) { create(:epic, relative_position: 4, title: 'world') }
  ...
  describe '#valid_parent?' do
    context 'basic checks' do
      ...
      context 'with parent in a descendant group' do
        let_it_be(:parent_epic, freeze: false) { create(:epic, group: descendant_group) }
      ...
      context 'with parent from a different group hierarchy' do
        ...
        let_it_be(:other_epic, freeze: false) { create(:epic, group: other_group) }
    ...
    context 'when parent has ancestors and epic has children' do
      let_it_be(:root_epic, freeze: false) { create(:epic, group: group) }
      let_it_be(:parent_epic, freeze: false) { create(:epic, group: group, parent: root_epic) }
  ...
  context 'hierarchy' do
    ...
    let_it_be_with_reload(:epic2) { create(:epic, group: group, parent: epic1) }
    let_it_be_with_reload(:epic3) { create(:epic, group: group, parent: epic2) }
    let_it_be_with_reload(:epic4) { create(:epic, group: group, parent: epic3) }
    ...
    describe '#work_item_ancestors_ids_with_order' do
      it 'returns all ancestor IDs in ascending order by default' do
        ancestor_ids = epic4.work_item_ancestors_ids_with_order

        expect(ancestor_ids).to eq([epic3.issue_id, epic2.issue_id, epic1.issue_id])
      end

      it 'returns all ancestor IDs in ascending order when explicitly specified' do
        ancestor_ids = epic4.work_item_ancestors_ids_with_order(hierarchy_order: :asc)

        expect(ancestor_ids).to eq([epic3.issue_id, epic2.issue_id, epic1.issue_id])
      end

      it 'returns all ancestor IDs in descending order' do
        ancestor_ids = epic4.work_item_ancestors_ids_with_order(hierarchy_order: :desc)

        expect(ancestor_ids).to eq([epic1.issue_id, epic2.issue_id, epic3.issue_id])
      end

      it 'returns an empty array if an epic does not have any parent' do
        expect(epic1.work_item_ancestors_ids_with_order).to be_empty
      end

      it 'returns work item IDs, not epic IDs' do
        ancestor_ids = epic4.work_item_ancestors_ids_with_order

        expect(ancestor_ids).to all(be_a(Integer))
        expect(ancestor_ids).not_to include(epic3.id, epic2.id, epic1.id)
        expect(ancestor_ids).to include(epic3.issue_id, epic2.issue_id, epic1.issue_id)
      end

      context 'with different hierarchy depths' do
        it 'returns correct order for shallow hierarchy' do
          ancestor_ids = epic2.work_item_ancestors_ids_with_order

          expect(ancestor_ids).to eq([epic1.issue_id])
        end
      end
    end
  end
  ...
  it_behaves_like 'within_timeframe scope' do
    ...
    let_it_be(:group, freeze: false) { create(:group) }
    let_it_be(:resource_1, freeze: false) { create(:epic, group: group, start_date: now - 1.day, end_date: now + 1.day) }
    let_it_be(:resource_2, freeze: false) { create(:epic, group: group, start_date: now + 2.days, end_date: now + 3.days) }
    let_it_be(:resource_3, freeze: false) { create(:epic, group: group, end_date: now) }
    let_it_be(:resource_4, freeze: false) { create(:epic, group: group, start_date: now) }
  end
  ...
  describe '#issues_readable_by' do
    ...
    let_it_be(:lone_issue) { create(:issue, project: project) }
    let_it_be(:other_issue) { create(:issue, project: project2) }
  ...
  describe '#to_reference' do
    let_it_be(:group) { create(:group, path: 'group-a') }
    let_it_be(:subgroup) { create(:group) }
    let_it_be(:group_project) { create(:project, group: group) }
    let_it_be(:subgroup_project) { create(:project, group: subgroup) }
    let_it_be(:other_project) { create(:project) }
    let_it_be(:epic) { create(:epic, iid: 1, group: group) }
  ...
  describe '#children' do
    ...
    it 'returns empty relation when epic has no children' do
      expect(epic.children).to be_empty
    end

    it 'returns direct child epics' do
      child1 = create(:epic, group: group, parent: epic)
      child2 = create(:epic, group: group, parent: epic)
      child3 = create(:epic, group: group, parent: epic)

      children = epic.reload.children

      expect(children).to match_array([child1, child2, child3])
    end

    it 'does not include grandchildren epics' do
      child = create(:epic, group: group, parent: epic)
      grandchild = create(:epic, group: group, parent: child)

      children = epic.reload.children

      expect(children).to contain_exactly(child)
      expect(children).not_to include(grandchild)
    end

    it 'returns only epics, not issues' do
      child_epic = create(:epic, group: group, parent: epic)
      issue = create(:issue, project: project)
      create(:epic_issue, epic: epic, issue: issue)

      children = epic.reload.children

      expect(children).to contain_exactly(child_epic)
    end

    it 'returns correct children after parent reassignment' do
      original_parent = create(:epic, group: group)
      new_parent = create(:epic, group: group)
      child = create(:epic, group: group, parent: original_parent)

      expect(original_parent.reload.children).to include(child)
      expect(new_parent.reload.children).not_to include(child)

      child.work_item_parent_link.update!(work_item_parent: new_parent.work_item)
      child.update!(parent: new_parent)

      expect(original_parent.reload.children).not_to include(child)
      expect(new_parent.reload.children).to include(child)
    end

    it 'handles multiple children updates correctly' do
      child1 = create(:epic, group: group, parent: epic)
      child2 = create(:epic, group: group, parent: epic)

      expect(epic.reload.children.count).to eq(2)

      child1.destroy!

      expect(epic.reload.children).to contain_exactly(child2)
    end

    it 'returns children from same group' do
      child = create(:epic, group: group, parent: epic)

      expect(epic.reload.children).to include(child)
    end

    it 'returns children from descendant groups' do
      child = create(:epic, group: descendant_group, parent: epic)
  ...
  context "relative positioning" do
    let_it_be(:group, freeze: false) { create(:group) }
    let_it_be(:parent, freeze: false) { create(:epic, group: group) }

    context 'there is no parent' do
      let_it_be(:factory, freeze: false) { :epic }
      let_it_be(:default_params, freeze: false) { { group: group } }
  ...
  context 'with existing epics and related issues' do
    ...
    let_it_be(:epic2, freeze: false) { create(:epic, group: group, parent: epic1) }
    let_it_be(:epic3, freeze: false) { create(:epic, group: group, parent: epic2, state: :closed) }
    let_it_be(:epic4, freeze: false) { create(:epic, group: group) }
    let_it_be(:issue1, freeze: false) { create(:issue, weight: 2, project: project) }
    let_it_be(:issue2, freeze: false) { create(:issue, weight: 3, project: project) }
    let_it_be(:issue3, freeze: false) { create(:issue, state: :closed, project: project) }
    let_it_be(:epic_issue1, freeze: false) { create(:epic_issue, epic: epic2, issue: issue1, relative_position: 5) }
    let_it_be(:epic_issue2, freeze: false) { create(:epic_issue, epic: epic2, issue: issue2, relative_position: 2) }
    let_it_be(:epic_issue3, freeze: false) { create(:epic_issue, epic: epic3, issue: issue3) }
    ...
    describe '.issue_metadata_for_epics' do
      ...
      context 'with multiple health statuses' do
        let_it_be(:issue4, freeze: false) { create(:issue, :opened, :on_track, project: project) }
        let_it_be(:issue5, freeze: false) { create(:issue, :opened, :needs_attention, project: project) }
        let_it_be(:issue6, freeze: false) { create(:issue, :opened, :at_risk, project: project) }
        let_it_be(:issue7, freeze: false) { create(:issue, :opened, :at_risk, project: project) }
  ...
  describe '.epics_readable_by_user' do
    let_it_be(:visible_epic, freeze: false) { create(:epic) }
    let_it_be(:confidential_epic, freeze: false) { create(:epic, :confidential) }
    ...
    context 'with group hierarchy' do
      ...
      let_it_be(:base_group, freeze: false) { create(:group, :private, parent: ancestor) }
      let_it_be(:subgroup, freeze: false) { create(:group, :private, parent: base_group) }

      let_it_be(:epic1, freeze: false) { create(:epic, group: ancestor) }
      let_it_be(:epic2, freeze: false) { create(:epic, group: base_group) }
      let_it_be(:epic3, freeze: false) { create(:epic, group: subgroup) }

      let_it_be(:epics, freeze: false) { [epic1, epic2, epic3] }
  ...
  describe '#related_epics' do
    ...
    let_it_be(:user, freeze: false) { create(:user) }
    let_it_be(:public_epic, freeze: false) { create(:epic) }
    let_it_be(:confidential_epic, freeze: false) { create(:epic, :confidential) }
    let_it_be(:sub_epic, freeze: false) { create(:epic, group: create(:group, parent: epic.group)) }
  ...
  describe '#epic_link_type' do
    let_it_be(:source_epic, freeze: false) { create(:epic, group: group) }
    let_it_be(:target_epic, freeze: false) { create(:epic, group: group) }
    let_it_be(:epic_link, freeze: false) { create(:related_epic_link, link_type: ::IssuableLink::TYPE_BLOCKS, source: source_epic, target: target_epic) }
  ...
  describe '#blocked_by_epics_for' do
    ...
    let_it_be(:epic, freeze: false) { create(:epic, group: group) }
    let_it_be(:blocking_epic_1, freeze: false) { create(:epic, group: group) }
    let_it_be(:other_group_blocking_epic, freeze: false) { create(:epic, group: create(:group, :private)) }
    let_it_be(:blocking_epic_2, freeze: false) { create(:epic, group: group) }
    let_it_be(:confidential_blocking_epic, freeze: false) { create(:epic, :confidential, group: group) }
    let_it_be(:related_epic, freeze: false) { create(:epic, group: group) }
    let_it_be(:closed_blocking_epic, freeze: false) { create(:epic, group: group, state: :closed) }
  ...
  context 'order by closed_at' do
    let_it_be(:epic_a) { create(:epic, closed_at: 1.day.ago) }
    let_it_be(:epic_b) { create(:epic, closed_at: 5.days.ago) }
    let_it_be(:epic_c_nil) { create(:epic, closed_at: nil) }
    let_it_be(:epic_d) { create(:epic, closed_at: 3.days.ago) }
    let_it_be(:epic_e_nil) { create(:epic, closed_at: nil) }
  ...
  describe '#participants' do
    it_behaves_like 'issuable participants' do
      let_it_be(:issuable_parent, freeze: false) { create(:group, :public) }
  ...
  describe '#total_issue_weight_and_count' do
    ...
    let_it_be(:project, freeze: false) { create(:project, group: group) }
    let_it_be(:epic, freeze: false) { create(:epic, group: group) }
    let_it_be(:issue, freeze: false) { create(:issue, weight: 20, project: project) }
    let_it_be(:issue2, freeze: false) { create(:issue, :closed, weight: 30, project: project) }
    let_it_be(:epic_issue, freeze: false) { create(:epic_issue, epic: epic, issue: issue) }
    let_it_be(:epic_issue2, freeze: false) { create(:epic_issue, epic: epic, issue: issue2) }

    let_it_be(:subepic, freeze: false) do
      create(
  ...
  describe '#update_cached_metadata' do
    ...
    context 'when epic is already assigned to other epic' do
      let_it_be(:old_parent, freeze: false) { create(:epic, group: group) }
  ...
  describe '#linked_items_count' do
    let_it_be(:epic1, freeze: false) { create(:epic, group: group) }
    let_it_be(:epic2, freeze: false) { create(:epic, group: group) }
    let_it_be(:epic3, freeze: false) { create(:epic, group: group) }
    let_it_be(:epic4, freeze: false) { build(:epic, group: group) }
  ...
  describe '#icon_name' do
    ...
    it 'returns the epic icon name' do
      expect(epic.icon_name).to eq('work-item-epic')
    end
  end

  context 'with unified associations' do
    context 'with label_links' do
      let_it_be(:label1, freeze: false) { create(:group_label, group: group, title: 'epic-label-1') }
      let_it_be(:label2, freeze: false) { create(:group_label, group: group, title: 'epic-label-2') }
      let_it_be(:epic, freeze: false) { create(:epic, group: group) }
      let_it_be(:work_item, freeze: false) { epic.work_item }
      let_it_be(:epic_label_link, freeze: false) { create(:label_link, label: label1, target: epic) }
      let_it_be(:epic_work_item_label_link, freeze: false) { create(:label_link, label: label2, target: work_item) }
    ...
    context 'with labels' do
      let_it_be(:label1, freeze: false) { create(:group_label, group: group, title: 'epic-label-1') }
      let_it_be(:label2, freeze: false) { create(:group_label, group: group, title: 'epic-label-2') }
      let_it_be(:epic, freeze: false) { create(:epic, group: group, labels: [label1]) }
      let_it_be(:work_item, freeze: false) { epic.work_item }
    ...
    context 'with award_emoji' do
      let_it_be(:epic, freeze: false) { create(:epic, group: group) }
      let_it_be(:work_item, freeze: false) { epic.work_item }
      let_it_be(:emoji_1, freeze: false) { create(:award_emoji, :upvote, awardable: work_item) }
      let_it_be(:emoji_2, freeze: false) { create(:award_emoji, :downvote, awardable: work_item) }

      describe '#award_emoji' do
        it 'returns work_item award_emoji' do
          expect(epic.award_emoji).to match_array(work_item.award_emoji)
        end
      end

      describe '.awarded' do
        it 'finds epics where work item has emoji from user' do
          user = emoji_1.user

          expect(described_class.awarded(user, name: AwardEmoji::UPVOTE_NAME)).to include(epic)
          expect(described_class.awarded(user, name: AwardEmoji::DOWNVOTE_NAME)).not_to include(epic)
        end
      end

      describe '.not_awarded' do
        it 'excludes epics where work item has emoji from user' do
          user = emoji_1.user

          expect(described_class.not_awarded(user, name: AwardEmoji::UPVOTE_NAME)).not_to include(epic)
          expect(described_class.not_awarded(user, name: AwardEmoji::DOWNVOTE_NAME)).to include(epic)
        end
      end

      context 'for order_upvotes' do
        let_it_be(:epic_2, freeze: false) { create(:epic, group: group) }

        describe '.order_upvotes_desc' do
          it 'orders by work item upvotes descending' do
            expect(described_class.where(id: [epic.id, epic_2.id]).order_upvotes_desc).to eq([epic, epic_2])
          end
        end

        describe '.order_upvotes_asc' do
          it 'orders by work item upvotes ascending' do
            expect(described_class.where(id: [epic.id, epic_2.id]).order_upvotes_asc).to eq([epic_2, epic])
          end
        end

        describe '.order_downvotes_desc' do
          it 'orders by work item downvotes descending' do
            expect(described_class.where(id: [epic.id, epic_2.id]).order_downvotes_desc).to eq([epic, epic_2])
          end
        end
      end
    end
    ...
    context 'when work_item is nil' do
      ...
      describe '#award_emoji' do
        it 'returns empty relation' do
          expect(epic_without_work_item.award_emoji).to be_empty
        end
      end
    ...
    context 'with notes' do
      ...
      let_it_be(:note2, freeze: false) { create(:note, noteable: epic, note: 'second note on epic') }
      let_it_be(:note3, freeze: false) { create(:note, noteable: work_item, note: 'first note on epic work item') }
    ...
    context 'with events' do
      let_it_be(:epic, freeze: false) { create(:epic, group: group) }
      let_it_be(:work_item, freeze: false) { epic.work_item }
      let_it_be(:epic_event, freeze: false) { create(:event, :created, group: group, project: nil, target: epic) }

      let_it_be(:epic_work_item_event1, freeze: false) do
        create(:event, :closed, group: group, project: nil, target_id: work_item.id, target_type: 'WorkItem')
      end

      let_it_be(:epic_work_item_event2, freeze: false) do
  ...
  context 'with subscriptions' do
    let_it_be(:epic, freeze: false) { create(:epic, group: group) }
    let_it_be(:work_item, freeze: false) { epic.work_item }
    let_it_be(:work_item_subscription, freeze: false) { create(:subscription, user: user, subscribable: work_item, subscribed: true) }
