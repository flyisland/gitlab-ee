...
RSpec.describe ApprovalWrappedAnyApproverRule, feature_category: :code_review_workflow do
  ...
  describe '#approvals_approvers' do
    ...
    context 'when an author and a committer approved' do
      before do
        ...
        committer_users = User.id_in(committer.id)
        allow(merge_request).to receive_messages(
          committers: committer_users,
          committers_to_filter_from_approvers: committer_users,
          committer_ids_to_filter_from_approvers: committer_users.select(:id)
        )
      end
