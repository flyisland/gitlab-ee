...
RSpec.describe SubscriptionPresenter, :saas, feature_category: :subscription_management do
  using RSpec::Parameterized::TableSyntax

  let(:subscription) { build_stubbed(:gitlab_subscription) }
  ...
  describe '#notify_admins?' do
    subject(:notify_admins) { presenter.notify_admins? }
    ...
    with_them do
      ...
      it { expect(notify_admins).to be(expected_result) }
    end
  end
  ...
  describe '#block_changes_at' do
    subject(:block_changes_at) { presenter.block_changes_at }
    ...
    context 'when end_date is nil' do
      it 'is nil' do
        ...
        expect(block_changes_at).to be_nil
      end
    end
  end

  describe '#block_changes?' do
    subject(:block_changes) { presenter.block_changes? }
    ...
    context 'is not expired' do
      ...
      it 'is false' do
        travel_to(today) do
          expect(block_changes).to be false
        end
      end
    end
    ...
    context 'is expired' do
      ...
      context 'is past grace period' do
        ...
        it 'is true' do
          travel_to(today) do
            expect(block_changes).to be true
          end
        end
      end
    end
  ...
  describe '#will_block_changes?' do
    subject(:will_block_changes) { presenter.will_block_changes? }
    ...
    context 'when end_date does not exist' do
      it 'is false' do
        ...
        expect(will_block_changes).to be false
      end
    end
  end

  describe '#remaining_days' do
    subject(:remaining_days) { presenter.remaining_days }
    ...
    it 'is nil when end_date is nil' do
      ...
      expect(remaining_days).to be_nil
    end
