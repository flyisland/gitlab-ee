      ...
      context 'with Gitlab Secrets Manager' do
        ...
        before do
          ...
          allow(::SecretsManagement::Availability)
            .to receive_messages(enabled_for_project?: true, enabled_for_group?: true)
        end
        ...
        context 'when project level secret but SM availability is not satisfied' do
          ...
          before do
            ...
            # Here we only verify the runner gates on `Availability.enabled_for_project?`.
            allow(::SecretsManagement::Availability)
              .to receive(:enabled_for_project?).with(ci_build.project).and_return(false)
          end
        ...
        context 'when group level secret for a root group' do
          ...
          context 'when group SM availability is not satisfied' do
            before do
              # SM-availability AND combinations are covered in availability_spec.rb.
              # Here we only verify the runner gates on `Availability.enabled_for_group?`.
              allow(::SecretsManagement::Availability)
                .to receive(:enabled_for_group?).with(group).and_return(false)
            end
