...
RSpec.describe GroupMemberPresenter do
  ...
  describe '#group_sso?' do
    ...
    context 'when member does not have a user (invited member)' do
      ...
      it 'returns `false`' do
        expect(presenter.group_sso?).to be false
      end
    end
    ...
    context 'when group is a root group' do
      it 'calls through to User#group_sso?' do
        ...
        expect(presenter.group_sso?).to be true
      end
    end
  ...
  describe '#group_managed_account?' do
    context 'when member does not have a user (invited member)' do
      ...
      it 'returns `false`' do
        expect(presenter.group_managed_account?).to be false
      end
    end
    ...
    context 'when user is part of the group managed account' do
      ...
      it 'returns `true`' do
        expect(presenter.group_managed_account?).to be true
      end
    end
  ...
  describe '#can_unban?' do
    before do
      allow(presenter).to receive(:can?).with(user, :unban_group_member, group) { can_unban_group_member }
    end

    context 'when user can unban_group_member' do
      let(:can_unban_group_member) { true }

      it { expect(presenter.can_unban?).to be(true) }
    end

    context 'when user cannot unban_group_member' do
      let(:can_unban_group_member) { false }

      it { expect(presenter.can_unban?).to be(false) }
    end
  end
end
