      ...
      context 'without group provided' do
        context 'when user owns no free groups' do
          before do
            ...
            allow(user).to receive_message_chain(:owned_groups, :in_specific_plans, :limit)
        ...
        context 'when user owns exactly one free group' do
          ...
          before do
            ...
            allow(GitlabSubscriptions).to receive(:user_has_non_free_groups?).and_return(false)
            allow(user).to receive_message_chain(:owned_groups, :in_specific_plans, :limit).and_return([free_group])
          end
        ...
        context 'when user owns multiple free groups' do
          ...
          before do
            allow(GitlabSubscriptions).to receive(:user_has_non_free_groups?).and_return(false)
            allow(user).to receive_message_chain(:owned_groups, :in_specific_plans, :limit).and_return(
              [free_group_one, free_group_two]
            )
          end
        ...
        context 'when user owns free and paid groups' do
          ...
          before do
            ...
            allow(user).to receive_message_chain(:owned_groups, :in_specific_plans, :limit).and_return(free_groups)
          end
