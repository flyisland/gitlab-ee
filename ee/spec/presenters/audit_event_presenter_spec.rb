...
RSpec.describe AuditEventPresenter, feature_category: :audit_events do
  ...
  let(:audit_event) do
    create(
      :audit_events_group_audit_event,
      author_name: 'author',
  ...
  shared_context 'with a stubbed audit event' do
    let(:audit_event) do
      build_stubbed(:audit_events_group_audit_event, author_name: 'author', target_details: 'target name',
        ip_address: '10.2.1.1', details: details)
    end
  end
  ...
  context 'exposes the author' do
    ...
    context 'event authored by a user that no longer exists' do
      let(:audit_event) { build(:audit_events_group_audit_event, details: details) }
      ...
      context 'when `author_name` is included in the details and not in the author_name column' do
        let(:audit_event) { build(:audit_events_group_audit_event, author_name: nil, details: details) }
    ...
    context 'event authored by a runner registration token user' do
      ...
      let(:audit_event) do
        build(:audit_events_group_audit_event, ip_address: '127.0.0.1', target_type: ::Ci::Runner.name,
          entity_path: 'path', details: { runner_registration_token: 'abc123' })
      end
    ...
    describe '#author_removed?' do
      using RSpec::Parameterized::TableSyntax

      it 'is false for an author that exists' do
        expect(presenter.author_removed?).to be false
      end
      ...
      with_them do
        let(:audit_event) do
          build(:audit_events_group_audit_event, author_id: author_id, author_name: author_name, details: details)
        end

        it 'returns the expected result' do
          expect(presenter.author_removed?).to eq(expected)
        end
      end
    end
  end
  ...
  describe '#target' do
    include_context 'with a stubbed audit event'

    it 'delegates to the model object' do
      expect(presenter.target).to eq(audit_event.target_details)
    end
  end
  ...
  context 'exposes the object' do
    ...
    context 'when object is of type instance scope' do
      let_it_be(:audit_event) { create(:audit_events_instance_audit_event) }
    ...
    context 'when a project in a user namespace has been deleted' do
      ...
      let(:audit_event) do
        build(
          ...
          allow(event).to receive(:entity).and_return(Gitlab::Audit::NullEntity.new)
        end
      end
  ...
  context 'exposes date and action' do
    include_context 'with a stubbed audit event'
