...
RSpec.describe "User creates pipeline execution policy", :js, feature_category: :security_policy_management do
  ...
  let_it_be(:project, freeze: false) { create(:project, :repository, namespace: group, name: 'Project 1') }
  ...
  let_it_be(:protected_branch, freeze: false) { create(:protected_branch, name: 'spooky-stuff', project: project) }
  ...
  before do
    ...
    find_by_testid('policy-name-text')
  end
  ...
  it "creates an merge request for a valid policy" do
    ...
    expect(page).to have_current_path(organization_namespace_project_merge_request_path(
