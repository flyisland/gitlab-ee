...
RSpec.describe 'Project', feature_category: :source_code_management do
  ...
  describe 'Custom group-level project templates', :js do
    ...
    context 'when constrain_group_project_templates flag is disabled' do
      before do
      ...
      context 'without a namespace in the project path' do
        it 'displays empty namespace for default project URL' do
          ...
          expect(find_by_testid('select-namespace-dropdown')).to have_text(
            s_('ProjectsNew|Pick a group or namespace')
          )
        end
      end
