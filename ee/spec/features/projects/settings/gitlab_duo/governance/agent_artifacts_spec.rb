...
RSpec.describe 'Project > Settings > GitLab Duo > Governance > Agent artifacts', :js,
  feature_category: :compliance_management do
  let_it_be(:reviewer) { create(:user) }
  let_it_be(:author) { create(:user) }
  let_it_be(:group) do
    create(:group, owners: reviewer) { |g| g.namespace_settings.update!(duo_features_enabled: true) }
  end
  ...
  before do
    stub_licensed_features(ai_features: true, project_level_compliance_dashboard: true)
    stub_application_setting(gravatar_enabled: false)
    sign_in(reviewer)

    visit project_settings_gitlab_duo_governance_index_path(project, tab: 'agent-artifacts')
  end
