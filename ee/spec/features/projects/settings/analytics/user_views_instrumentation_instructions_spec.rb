...
RSpec.describe 'Project > Settings > Analytics -> Instrumentation instructions', :js, feature_category: :product_analytics do
  ...
  context 'without correct license' do
    ...
    it 'does not show instrumentation instructions' do
      expect(page).not_to have_content _('View instrumentation instructions')
    end
  end
  ...
  context 'with valid license, toggle and feature flags' do
    ...
    context 'when project is onboarded' do
      ...
      it 'shows instrumentation key' do
        fieldset = page.find('fieldset', text: _('SDK application ID'))
        expect(fieldset.find("input").value).to eq(instrumentation_key.to_s)
      end

      it 'shows collector host' do
        fieldset = page.find('fieldset', text: _('SDK host'))
        expect(fieldset.find("input").value).to eq(collector_host)
      end

      it 'shows instrumentation setup instructions' do
        expect(page).to have_button(_("View instrumentation instructions"))

        click_button _("View instrumentation instructions")

        expect(page).to have_content(
          _("1. Add the NPM package to your package.json using your preferred package manager"))
        expect(page).to have_content(_("2. Import the new package into your JS code"))
        expect(page).to have_content(_("3. Initiate the tracking"))
        expect(page).to have_content(_("Add the script to the page and assign the client SDK to window"))
      end
    end
  end
end
