...
RSpec.describe 'Project > Settings > GitLab Duo > SAST Vulnerability Resolution', :js,
  ...
  before do
    ...
    # The SAST VR row is nested under the (hidden-for-this-role) Duo enablement toggle,
    # so Duo features must be enabled for it to render.
  ...
  it 'lets the security manager enable and save the SAST VR workflow setting' do
    ...
    # The SAST VR toggle is rendered by the GitlabDuoSettings Vue component only when the
    # serialized `visibleSettings` allowlist includes the value the frontend matches against.
    # If the value the helper emits ever drifts from DUO_SAST_VR_WORKFLOW_ENABLED, finding the
    # toggle below fails.
    sast_vr_toggle = find('[data-testid="duo-sast-vr-workflow-enabled"] button')
    expect(sast_vr_toggle['aria-checked']).to eq('false')

    # No other Duo settings are exposed to the security manager (e.g. the Duo enablement toggle).
