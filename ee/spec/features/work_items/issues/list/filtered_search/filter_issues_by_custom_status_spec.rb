...
RSpec.describe 'Filter issues by custom status', :js, feature_category: :planning_views do
  ...
  context 'on project work items page' do
    let(:work_items_page_path) { project_work_items_path(project) }

    it_behaves_like 'filtering by custom status'
  end

  context 'on group work items page' do
    let(:work_items_page_path) { group_work_items_path(group) }
