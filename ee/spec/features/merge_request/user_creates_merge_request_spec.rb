...
RSpec.describe "User creates a merge request", :js, feature_category: :code_review_workflow do
  ...
  # Deliberately no sign_in/visit here: the SSO context signs in again, and doing that
  # behind an already-rendered page rotates the CSRF token under that page's in-flight
  # requests, which fails them with InvalidAuthenticityToken and resets the session.
  before do
  ...
  context 'when the user has branches in an SSO enforced group' do
    let_it_be_with_reload(:saml_provider) { create(:saml_provider, enabled: true, enforced_sso: true) }
    let_it_be_with_reload(:restricted_group) { create(:group, saml_provider: saml_provider) }
