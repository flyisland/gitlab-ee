...
RSpec.describe 'Two merge requests on a merge train', feature_category: :merge_trains do
  ...
  before do
    # Stub Ci::Partition.current to keep car.pipeline lookups under the query limit
    # while partitioned_pipeline_association_finder FF rolls out (issue #599815).
    # Remove this stub once the FF reaches 100% and is cleaned up.
    allow(Ci::Partition).to receive(:current)
      .and_return(build_stubbed(:ci_partition, id: ci_testing_partition_id))
