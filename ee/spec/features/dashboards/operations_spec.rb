...
RSpec.describe 'Dashboard operations', :js, feature_category: :incident_management do
  # `freeze: false` is required here: `user` is mutated in-example
  # (`user.update!(ops_dashboard_projects: [project])`) and is also referenced
  # by `current_organization` (`user.organization`); reloading `user` swaps the
  # instance and breaks that association linkage. Keeping it unfrozen preserves
  # the original behaviour (see gitlab-org/gitlab#602925).
