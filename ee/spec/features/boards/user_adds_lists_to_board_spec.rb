...
RSpec.describe 'User adds milestone/iterations/status lists', :js, :aggregate_failures, feature_category: :team_planning do
  ...
  let_it_be(:user, freeze: false) { create(:user, maintainer_of: project, owner_of: group) }
  ...
  let_it_be(:issue_with_assignee, freeze: false) { create(:issue, project: project, assignees: [user]) }
