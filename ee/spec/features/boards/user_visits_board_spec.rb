...
RSpec.describe 'User visits issue boards', :js, feature_category: :planning_views do
  ...
  let_it_be(:group, freeze: false) { create_default(:group, :public) }
  let_it_be(:project, freeze: false) { create_default(:project, :public, group: group) }
  let_it_be(:label, freeze: false) { create(:group_label, group: group, name: label_name) }
  let_it_be(:scoped_label, freeze: false) { create(:group_label, group: group, name: scoped_label_name) }
  ...
  shared_examples "visiting board path" do
    before do
      ...
      wait_for('board URL to reflect the expected filter params') do
        query_strings(current_url).sort == query_strings(expected_board_path).sort
      end
  ...
  def query_strings(url)
    ...
    query ? URI.decode_www_form_component(query).split('&') : []
  end
