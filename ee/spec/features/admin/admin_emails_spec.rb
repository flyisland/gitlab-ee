...
RSpec.describe 'Admin::Emails', :clean_gitlab_redis_shared_state, feature_category: :notifications do
  ...
  context 'when `send_emails_from_admin_area` feature is licensed', :js, feature_category: :system_access do
    ...
    # `have_button(disabled:)` rather than reading the native property: the submit is a GlButton,
    # which signals unavailability with `aria-disabled`. Capybara's filter knows both forms.
    def expect_submit_button_disabled(disabled)
      within('#admin-emails-form') do
        expect(page).to have_button('Send message', disabled: disabled)
      end
    end
    ...
    context 'when emails from admin area are not rate limited' do
      it 'does not show the waiting period alert'\
        ...
        expect_submit_button_disabled(false)
      end
    end
    ...
    context 'when emails from admin area are rate limited' do
      ...
      it 'shows the waiting period alert'\
        ...
        expect_submit_button_disabled(true)
      end
    end
  end
end
