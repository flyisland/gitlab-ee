...
RSpec.describe 'Group', :with_trial_types, :with_current_organization, feature_category: :groups_and_projects do
  ...
  describe 'storage pre-enforcement alert', :js do
    ...
    context 'when the group is over the notification_limit' do
      ...
      it 'dismisses the alert when clicking the close button' do
        visit group_path(group)
        ...
        expect(page).to have_no_css('.js-storage-pre-enforcement-alert')
      end

      # Persistence across page loads is asserted by seeding the dismissed callout
      # rather than clicking dismiss and reloading: the dismissal is persisted by an
      # async request while alert_handler.js removes the alert from the DOM
      # immediately, so a reload can race ahead of the request and flake.
      context 'when the user has already dismissed the alert' do
        before do
          create(
            :group_callout,
            user: user,
        ...
        it 'does not display the alert even though the group is over the notification_limit' do
          visit group_path(group)
