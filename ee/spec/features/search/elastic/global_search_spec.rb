...
RSpec.describe 'Global elastic search', :elastic, :js, :sidekiq_inline, :disable_rate_limiter,
  ...
  describe 'I search through work items and I see pagination', :js do
    before do
      stub_const('SearchService::DEFAULT_PER_PAGE', 2)
      create_list(:issue, 3, project: project, title: 'initial')
  ...
  describe 'I search through the notes and I see pagination' do
    before do
      ...
      create_list(:note, 3, noteable: issue, project: project, note: 'foo')
  ...
  describe 'I search through the wiki blobs' do
    include WikiHelpers
    # `freeze: false` is required here: `GroupWiki` is not an ActiveRecord
    # record and is mutated during indexing (`disk_path` etc.); freezing raises
    # FrozenError and reload/refind are no-ops on it. Keep as-is (see
    # gitlab-org/gitlab#602925).
    let_it_be(:group_wiki, freeze: false) { create(:group_wiki) }
