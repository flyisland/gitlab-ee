...
RSpec.describe 'Groups > Audit events', :js, feature_category: :audit_events do
  ...
  describe 'audit event filter' do
    ...
    describe 'filter by date' do
      let_it_be(:audit_event_1) { create(:audit_events_group_audit_event, group_id: group.id, created_at: 5.days.ago) }
      let_it_be(:audit_event_2) { create(:audit_events_group_audit_event, group_id: group.id, created_at: 3.days.ago) }
      let_it_be(:audit_event_3) { create(:audit_events_group_audit_event, group_id: group.id, created_at: Date.current) }
    ...
    context 'signed in as a developer' do
      ...
      describe 'filter by author' do
        let_it_be(:audit_event_1) { create(:audit_events_group_audit_event, group_id: group.id, created_at: Date.today, ip_address: '1.1.1.1', author_id: alex.id) }
        let_it_be(:audit_event_2) { create(:audit_events_group_audit_event, group_id: group.id, created_at: Date.today, ip_address: '0.0.0.0', author_id: user.id) }
