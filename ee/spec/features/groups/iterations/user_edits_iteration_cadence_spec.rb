  ...
  context 'with license' do
    ...
    context 'as authorized user', time_travel_to: '2023-05-04' do
      before do
        ...
        allow(Time.zone).to receive_messages(
          name: ActiveSupport::TimeZone["UTC"].name,
          utc_offset: ActiveSupport::TimeZone["UTC"].utc_offset
        )
      ...
      context 'when a timezone is other than UTC is used' do
        before do
          allow(Time.zone).to receive_messages(
            name: ActiveSupport::TimeZone["Hawaii"].name,
            utc_offset: ActiveSupport::TimeZone["Hawaii"].utc_offset
          )
      ...
      it 'prefills fields and allows updating values' do
        aggregate_failures do
          ...
          expect(page).to have_field('cadence-description', with: cadence.description)
          expect(page).to have_field('cadence-start-date', with: cadence.start_date)
        end
