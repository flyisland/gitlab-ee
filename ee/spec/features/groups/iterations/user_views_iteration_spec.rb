...
RSpec.describe 'User views iteration', :js, feature_category: :team_planning do
  ...
  let_it_be_with_reload(:iteration) { create(:iteration, :with_due_date, iterations_cadence: manual_cadence, title: 'Correct Iteration', description: 'iteration description', start_date: Date.today) }
  let_it_be(:sub_group_iteration) { create(:iteration, iterations_cadence: create(:iterations_cadence, group: sub_group)) }
  let_it_be_with_reload(:label1) { create(:label, project: project) }
  let_it_be(:issue) { create(:issue, project: project, iteration: iteration, labels: [label1]) }
  let_it_be_with_reload(:assigned_issue) { create(:issue, project: project, iteration: iteration, assignees: [user], labels: [label1]) }
  ...
  context 'with license', :js do
    ...
    context 'when user has edit permissions' do
      ...
      context 'when iteration cadence is automatically scheduled' do
        ...
        let_it_be_with_reload(:iteration) { create(:iteration, iterations_cadence: auto_cadence, description: 'iteration description', start_date: Date.today) }
