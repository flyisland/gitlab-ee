...
RSpec.describe 'SAML provider settings', feature_category: :system_access do
  ...
  describe 'settings' do
    ...
    describe 'frontend apps mount on the page', :js do
      ...
      it 'renders the role selector' do
  ...
  describe 'SCIM token restricted access warning' do
    before do
      sign_in(user)
    end

    context 'when restricted access is enabled' do
      let!(:saml_provider) { create(:saml_provider, group: group, enabled: true) }
      ...
      it 'shows a restricted access warning in the SCIM Token section' do
