...
RSpec.describe 'Groups > Usage quotas > Storage tab', :with_trial_types, :js, :saas, feature_category: :consumables_cost_management do
  ...
  let_it_be_with_refind(:root_storage_statistics) do
  ...
  context 'with namespace storage limit' do
    ...
    context 'with a fork of a project' do
      let_it_be(:project_fork, freeze: false) { create(:project, namespace: group) }
      let_it_be(:statistics, freeze: false) do
        create(:project_statistics, project: project_fork, repository_size: 100.megabytes)
      end
