...
RSpec.describe "Git HTTP requests (Geo)", :geo, feature_category: :geo_replication do
  ...
  let_it_be(:project_with_repo, freeze: false) { create(:project, :repository, :private) }
  let_it_be(:project_no_repo, freeze: false) { create(:project, :private) }
  let_it_be(:project_registry_with_repo, freeze: false) { create(:geo_project_repository_registry, :verification_succeeded, project: project_with_repo, last_synced_at: project_with_repo.last_repository_updated_at + 10.seconds) }

  let_it_be(:primary_url, freeze: false) { 'http://primary.example.com' }
  let_it_be(:primary_internal_url, freeze: false) { 'http://primary-internal.example.com' }
  let_it_be(:secondary_url, freeze: false) { 'http://secondary.example.com' }
  let_it_be(:primary, freeze: false) { create(:geo_node, :primary, url: primary_url, internal_url: primary_internal_url) }
  let_it_be(:secondary, freeze: false) { create(:geo_node, url: secondary_url) }
    ...
    describe 'GET info_refs' do
      context 'git pull' do
        ...
        context 'when the repository does not exist' do
          let_it_be(:project, freeze: false) { project_no_repo }
      ...
      context 'git push' do
        ...
        let_it_be(:project, freeze: false) { project_with_repo }
      ...
      context 'Batch API' do
        describe 'POST /namespace/repo.git/info/lfs/objects/batch' do
          ...
          context 'operation download' do
            ...
            context 'when the repository exists' do
              let_it_be(:project, freeze: false) { project_with_repo }
              let_it_be(:registry, freeze: false) { create(:geo_lfs_object_registry, :synced) }
              let_it_be(:lfs_object, freeze: false) { registry.lfs_object }
      ...
      context 'when proxying an SSH request from a secondary node' do
        ...
        context 'when gl_id is provided in JWT token' do
          ...
          context 'and is valid' do
            ...
            context 'and is in the username format used by SSH certificate authentication' do
              let(:geo_gl_id) { "username-#{user.username}" }
            ...
            context 'and is in the user format' do
              let(:geo_gl_id) { "user-#{user.id}" }
    ...
    describe 'GET info_refs' do
      ...
      context 'IP allowed settings' do
        ...
        it 'returns unauthorized error' do
          ...
          expect(http_response.parsed_body).to eq('Request from this IP is not allowed')
        end
