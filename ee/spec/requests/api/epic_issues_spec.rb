  ...
  describe 'GET /groups/:id/epics/:epic_iid/issues' do
    ...
    context 'when epics feature is enabled' do
      ...
      context 'when the request is correct' do
        ...
        context 'when a child has a parent link without a legacy epic_issues row' do
          let_it_be(:orphaned_issue) { create(:issue, project: project) }
          let_it_be(:orphaned_link) do
            create(:parent_link, work_item: WorkItem.find(orphaned_issue.id), work_item_parent: epic.work_item)
          end

          it 'exposes a null epic_issue_id instead of failing the whole page', :aggregate_failures do
            perform_request
            ...
            orphaned, siblings = response.parsed_body.partition { |item| item['id'] == orphaned_issue.id }

            expect(orphaned.first['epic_issue_id']).to be_nil
            expect(siblings.map { |item| item['epic_issue_id'] }).to match_array([epic_issue1.id, epic_issue2.id])
          end
        end
        ...
        context 'with private and confidential issues' do
          let_it_be(:private_project) { create(:project, :private, group: group) }
          let_it_be(:private_issue) { create(:issue, project: private_project) }
          let_it_be(:confidential_child_issue) { create(:issue, :confidential, project: project) }
          let_it_be(:epic_issue3) { create(:epic_issue, epic: epic, issue: private_issue) }
          let_it_be(:epic_issue4) { create(:epic_issue, epic: epic, issue: confidential_child_issue) }

          it 'excludes them from the response', :aggregate_failures do
            perform_request
            ...
            expect(returned_ids).not_to include(private_issue.id, confidential_child_issue.id)
          end
          ...
          context 'when the user has access to the private project' do
            ...
            it 'includes the issue from that project but still excludes confidential issues', :aggregate_failures do
              perform_request

              returned_ids = response.parsed_body.pluck('id')
              expect(returned_ids).to include(private_issue.id)
              expect(returned_ids).not_to include(confidential_child_issue.id)
            end
          end
        end
      end
  ...
  describe 'PUT /groups/:id/epics/:epic_iid/issues/:epic_issue_id' do
    ...
    context 'when epics feature is enabled' do
      ...
      context 'when the request is correct' do
        ...
        context 'when there are issues the user does not have access to' do
          let_it_be(:confidential_issue3) { create(:issue, :confidential, project: project) }
          let_it_be(:epic_issue3) do
            create(:epic_issue, epic: epic, issue: confidential_issue3, relative_position: 3)
          end

          it 'excludes the inaccessible issue from the response', :aggregate_failures do
            issue_ids = [issue.id, issue2.id]
            returned_ids = response.parsed_body.pluck('id')

            expect(response).to have_gitlab_http_status(:ok)
            expect(returned_ids).to match_array(issue_ids)
            expect(returned_ids).not_to include(confidential_issue3.id)
          end
        end
      end
    end
