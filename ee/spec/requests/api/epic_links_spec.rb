    ...
    context 'when subepics feature is enabled' do
      ...
      context 'when user is a reporter' do
        ...
        it 'returns 201 status' do
          ...
          expect(epic.reload.children).to include(child_epic)
          expect(epic.notes.last&.note).to eq("added #{child_epic.work_item.to_reference} as child item")
          expect(child_epic.notes.last&.note).to eq("added #{epic.work_item.to_reference} as parent item")
        end
