...
RSpec.describe API::EpicBoards, feature_category: :planning_views do
  let_it_be(:non_member) { create(:user) }
  let_it_be(:guest) { create(:user) }

  let_it_be_with_reload(:group) { create(:group, :private, guests: guest) }
    ...
    context 'when epics are available' do
      ...
      context 'when the request is correct' do
        ...
        it 'returns 200 status' do
          ...
          expect(json_response['collapsed']).to be(false)
        end
