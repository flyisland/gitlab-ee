...
RSpec.describe API::SamlGroupLinks, :api, feature_category: :system_access do
  ...
  let_it_be(:group_with_saml_group_links, freeze: false) { create(:group) }
  ...
  let_it_be(:saml_provider, freeze: false) { create(:saml_provider, group: group_with_saml_group_links, enabled: true) }
  let_it_be(:group_id, freeze: false) { group_with_saml_group_links.id }
    ...
    context "when license feature is available" do
      ...
      context "when owner of the group" do
        ...
        context "when group does not have any associated saml_group_links" do
          let_it_be(:group_with_no_saml_links, freeze: false) { create(:group) }
          let_it_be(:saml_provider, freeze: false) do
            create(:saml_provider, group: group_with_no_saml_links, enabled: true)
          end

          let_it_be(:group_id, freeze: false) { group_with_no_saml_links.id }
  ...
  describe "POST /groups/:id/saml_group_links" do
    let_it_be(:params, freeze: false) { { saml_group_name: "Test group", access_level: ::Gitlab::Access::GUEST } }
    ...
    context "when licensed feature is available" do
      ...
      context "when owner of the group and group is saml enabled" do
        ...
        context 'with minimal access level' do
          let_it_be(:params, freeze: false) { { saml_group_name: "Test group", access_level: ::Gitlab::Access::MINIMAL_ACCESS } }
  ...
  describe "GET /groups/:id/saml_group_links/:saml_group_name" do
    let_it_be(:saml_group_name, freeze: false) { "saml-group1" }
    ...
    context "when licensed feature is available" do
      ...
      context "when owner of the group" do
        ...
        context "when getting a provider-scoped SAML group link" do
          ...
          let_it_be(:group_with_provider_link, freeze: false) { create(:group) }
          let_it_be(:saml_provider_for_get, freeze: false) do
            create(:saml_provider, group: group_with_provider_link, enabled: true)
          end

          let_it_be(:group_id, freeze: false) { group_with_provider_link.id }
          let_it_be(:saml_group_name, freeze: false) { "provider-scoped-group" }
        ...
        context "when multiple links exist with same group name but different providers" do
          ...
          let_it_be(:group_with_multiple_links, freeze: false) { create(:group) }
          let_it_be(:saml_provider_for_get, freeze: false) do
            create(:saml_provider, group: group_with_multiple_links, enabled: true)
          end

          let_it_be(:group_id, freeze: false) { group_with_multiple_links.id }
          let_it_be(:saml_group_name, freeze: false) { "shared-group-name" }
        ...
        context "when group link contains a period" do
          let_it_be(:saml_group_name, freeze: false) { "saml-group-with.period" }
    ...
    context "when licensed feature is available" do
      ...
      context "when owner of the group" do
        ...
        context "when multiple links exist with same group name but different providers" do
          ...
          let_it_be(:group_with_multiple_delete_links, freeze: false) { create(:group) }
          let_it_be(:saml_provider_for_delete, freeze: false) do
            create(:saml_provider, group: group_with_multiple_delete_links, enabled: true)
          end

          let_it_be(:group_id, freeze: false) { group_with_multiple_delete_links.id }
          let_it_be(:saml_group_name, freeze: false) { "delete-shared-group-name" }
