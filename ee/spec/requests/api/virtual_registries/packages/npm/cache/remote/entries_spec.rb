...
RSpec.describe API::VirtualRegistries::Packages::Npm::Cache::Remote::Entries, :aggregate_failures, feature_category: :virtual_registry do
  using RSpec::Parameterized::TableSyntax
  include_context 'for npm virtual registry api setup'

  describe 'GET /api/v4/virtual_registries/packages/npm/remote/upstreams/:id/cache_entries' do
    let(:upstream_id) { upstream.id }
    let(:url) { "/virtual_registries/packages/npm/remote/upstreams/#{upstream_id}/cache_entries" }

    let_it_be(:processing_cache_entry, freeze: false) do
      create(
    ...
    shared_examples 'successful response' do
      it 'returns a successful response', :freeze_time do
        ...
        expect(Gitlab::Json.safe_parse(response.body))
          .to contain_exactly(
            cache_entry
              .as_json
    ...
    context 'with invalid upstream' do
      where(:upstream_id, :status) do
        ...
        'foo'                  | :bad_request
        # TODO: Change to :bad_request once Npm::Upstreams API is implemented.
        # The endpoint returns bad_request for invalid ids, but an empty string
        # interpolates to '/upstreams//cache_entries' which doesn't match the
        # '/upstreams/:id/cache_entries' route pattern, so the :id param is never coerced.
        # Maven works because its Upstreams API is mounted before Cache::Entries,
        # matching '/upstreams/:id' and validating the param first.
        ''                     | :not_found
      end
    ...
    it_behaves_like 'authorizing granular token permissions', :read_npm_virtual_registry_upstream_cache_entry do
  ...
  describe 'DELETE /api/v4/virtual_registries/packages/npm/remote/cache_entries/:id' do
    let(:url) { "/virtual_registries/packages/npm/remote/cache_entries/#{cache_entry.generate_id}" }
    ...
    shared_examples 'successful response' do
      it 'returns a successful response' do
        expect { api_request }.to change {
          VirtualRegistries::Packages::Npm::Cache::Remote::Entry.last.status
    ...
    it_behaves_like 'authorizing granular token permissions', :delete_npm_virtual_registry_upstream_cache_entry do
