...
RSpec.describe API::GroupHooks, :aggregate_failures, feature_category: :webhooks do
  ...
  let_it_be(:group, freeze: false) { create(:group, owners: group_admin) }
  ...
  it_behaves_like 'web-hook API endpoints', '/groups/:id' do
    ...
    context 'when group has a project' do
      ...
      let_it_be(:project, freeze: false) { create(:project, :repository, group: group, creator_id: user.id) }
  ...
  describe 'with admin_web_hook custom role' do
    ...
    let_it_be(:role, freeze: false) { create(:member_role, :guest, :admin_web_hook, namespace: group) }
    let_it_be(:membership, freeze: false) { create(:group_member, :guest, member_role: role, user: user, group: group) }
