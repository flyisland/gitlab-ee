...
RSpec.describe API::Analytics::ProjectDeploymentFrequency, feature_category: :value_stream_management do
  ...
  let(:dispatch_request_in_before) { true }
  ...
  before do
    ...
    next unless dispatch_request_in_before
  ...
  describe 'granular token authorization' do
    ...
    let(:user) { reporter }
    let(:dispatch_request_in_before) { false }

    it_behaves_like 'authorizing granular token permissions', :read_dora_metric do
      let(:request) do
        get api("/projects/#{project.id}/analytics/deployment_frequency", personal_access_token: pat),
          params: { from: Time.current, to: Time.current, interval: 'all', environment: prod.name }
      end
    end
  end
