...
RSpec.describe API::Analytics::CodeReviewAnalytics, feature_category: :value_stream_management do
  ...
  describe 'GET code_review' do
    ...
    it_behaves_like 'authorizing granular token permissions', :read_code_review_analytics do
      let(:boundary_object) { project }
      let(:user) { reporter }

      let(:request) { get api("/analytics/code_review?#{query_params.to_query}", personal_access_token: pat) }
    end
