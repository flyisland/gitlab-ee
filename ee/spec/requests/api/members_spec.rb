...
RSpec.describe API::Members, feature_category: :groups_and_projects do
  ...
  context 'group members endpoints for group with minimal access feature' do
    ...
    let_it_be(:minimal_access_member, freeze: false) { create(:group_member, :minimal_access, source: group) }
    ...
    describe 'PUT /groups/:id/members/:user_id' do
      ...
      context 'when member_role_id param is present' do
        let_it_be(:member_role, freeze: false) { create(:member_role, :guest, namespace: group) }
        let_it_be(:member, freeze: false) { create(:group_member, :guest, source: group) }
        ...
        context "when custom roles license is enabled" do
          ...
          context 'when on SaaS' do
            ...
            context 'when member_role is not associated with root group of member source' do
              let_it_be(:member_role, freeze: false) { create(:member_role, :guest, namespace: create(:group)) }
          ...
          context 'when on self-managed' do
            ...
            context 'when member_role is created on the instance-level' do
              let_it_be(:member_role, freeze: false) { create(:member_role, :guest, :instance) }
  ...
  context 'billable member endpoints' do
    ...
    let_it_be(:nested_user, freeze: false) { create(:user, name: 'Scott Anderson') }
    let_it_be(:nested_group, freeze: false) do
    ...
    describe 'GET /groups/:id/billable_members', feature_category: :seat_cost_management do
      ...
      context 'with sub group and projects' do
        ...
        let_it_be(:linked_group_user, freeze: false) { create(:user, name: 'Scott McNeil') }
        let_it_be(:linked_group, freeze: false) do
          create(:group) do |linked_group|
            linked_group.add_developer(linked_group_user)
          end
        end

        let_it_be(:project_group_link, freeze: false) { create(:project_group_link, project: project, group: linked_group) }
        ...
        context 'when sorting users' do
          let_it_be(:sort_group, freeze: false) { create :group }
          let_it_be(:last_activity_on_date, freeze: false) { Date.today - 1.day }
          let_it_be(:user1, freeze: false) { create(:user, last_activity_on: Date.today, current_sign_in_at: DateTime.now - 4.days) }
          let_it_be(:user2, freeze: false) { create(:user, last_activity_on: last_activity_on_date, current_sign_in_at: DateTime.now - 3.days) }
          let_it_be(:user3, freeze: false) { create(:user, last_activity_on: last_activity_on_date, current_sign_in_at: DateTime.now - 2.days) }
          let_it_be(:user4, freeze: false) { create(:user, last_activity_on: last_activity_on_date, current_sign_in_at: DateTime.now - 1.day) }

          let_it_be(:url, freeze: false) { "/groups/#{sort_group.id}/billable_members" }
          let_it_be(:owner, freeze: false) { user1 }
      ...
      context 'email' do
        ...
        context 'when member has a public_email' do
          let_it_be(:member_public_email, freeze: false) { create(:email, :confirmed, user: member.user, email: 'member-public-email@example.com') }
    ...
    describe 'GET /groups/:id/billable_members/:user_id/indirect' do
      ...
      let_it_be(:other_invited_group, freeze: false) { create(:group) }
      let_it_be(:developer, freeze: false) { invited_group.add_developer(create(:user)).user }
  ...
  context 'group with LDAP group link' do
    ...
    describe 'POST /groups/:id/members' do
      ...
      context 'when user to be added is a service account' do
        let_it_be(:service_account, freeze: false) { create(:service_account) }
  ...
  context 'group with pending members' do
    ...
    describe 'PUT /groups/:id/members/:member_id/approve' do
      let_it_be(:member, freeze: false) { create(:group_member, :awaiting, group: group, user: developer) }
    ...
    describe 'GET /groups/:id/pending_members' do
      ...
      context 'when the current user is authorized' do
        let_it_be(:pending_group_member, freeze: false) { create(:group_member, :awaiting, group: group) }
        let_it_be(:pending_subgroup_member, freeze: false) { create(:group_member, :awaiting, group: subgroup) }
        let_it_be(:pending_project_member, freeze: false) { create(:project_member, :awaiting, project: project) }
        let_it_be(:pending_invited_member, freeze: false) { create(:group_member, :awaiting, :invited, group: group) }
        ...
        context 'when pending member has public email visibility' do
          ...
          let_it_be(:pending_group_member, freeze: false) { create(:group_member, :awaiting, group: group, user: user_with_public_email) }
  ...
  context 'filtering project and group members' do
    ...
    context 'for group sources' do
      ...
      it_behaves_like 'filtered results' do
        let_it_be(:awaiting_member, freeze: false) { create(:group_member, :awaiting, group: group) }
        let_it_be(:active_member, freeze: false)   { create(:group_member, group: group) }
      end
    end
    ...
    context 'for project sources' do
      ...
      it_behaves_like 'filtered results' do
        let_it_be(:awaiting_member, freeze: false) { create(:project_member, :awaiting, project: project) }
        let_it_be(:active_member, freeze: false)   { create(:project_member, project: project) }
      end
    end
  end
  ...
  context 'with billable member management' do
    ...
    let_it_be(:owner, freeze: false) { create(:user) }
    let_it_be(:users, freeze: false) { create_list(:user, 1) }
