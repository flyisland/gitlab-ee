...
RSpec.describe 'Getting issues for an epic', feature_category: :portfolio_management do
  ...
  context 'when epics are enabled' do
    ...
    context 'when user has access to the issue project' do
      ...
      it 'returns relative position for each issue' do
        post_graphql(epic_query(iid: epic.iid), current_user: user)

        issues = epics_data.first['node']['issues']['edges'].pluck('node')
        expect(issues).to include(
          a_hash_including('relativePosition' => epic_issue.relative_position)
        )
      end

      # Exercises the EpicIssue type-level granular token authorization
      # (inherited from the Issue type) through the Epic.issues connection.
      it_behaves_like 'authorizing granular token permissions for GraphQL', :read_issue do
        let(:boundary_object) { project }
        let(:request) { post_graphql(epic_query(iid: epic.iid), token: { personal_access_token: pat }) }
      end

      context 'when querying createNoteEmail' do
        # Exercises the granular scope directive on the EpicIssue.createNoteEmail field.
        # The token also needs read_issue to authorize the enclosing EpicIssue type.
        let(:epic_node) do
          <<~NODE
            edges {
              node {
    ...
    context 'when issues from multiple epics are queried' do
      ...
      it 'does limited number of N+1 queries' do
        # extra queries:
        # - issue_assignees: policy checks if user is an issue assignee
        #   (see https://gitlab.com/gitlab-org/gitlab/-/issues/353375)
        # - work_item path: reading epic_issues through work_item_parent_links
        #   instead of Epic.related_issues introduces additional queries for
        #   loading work_items, parent_links, and epic_issue associations
        extra_queries_count = 6
