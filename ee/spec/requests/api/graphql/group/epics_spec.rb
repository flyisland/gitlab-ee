...
RSpec.describe 'Epics through GroupQuery', feature_category: :portfolio_management do
  ...
  describe 'granular PAT authorization' do
    ...
    it_behaves_like 'authorizing granular token permissions for GraphQL', [:read_group, :read_epic] do
      let(:user) { create(:user, developer_of: group) }
      let(:boundary_object) { group }
      let(:query) do
        graphql_query_for(:group, { full_path: group.full_path }, query_graphql_field(:epics, {}, 'nodes { id }'))
      end
  ...
  describe 'Get related epic links fields' do
    let_it_be_with_reload(:epic_a) { create(:epic, group: group) }
    let_it_be_with_reload(:epic_b) { create(:epic, group: group, created_at: 1.hour.ago) }
