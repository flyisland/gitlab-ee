...
RSpec.describe 'UserAddOnAssignmentCreate', feature_category: :seat_cost_management do
  ...
  let_it_be(:add_on, freeze: false) { create(:gitlab_subscription_add_on) }
  let_it_be(:assignee_user, freeze: false) { create(:user) }
  ...
  context 'on Gitlab.com/Saas' do
    ...
    let_it_be(:namespace, freeze: false) { create(:group) }
    let_it_be(:namespace_1, freeze: false) { create(:group) }
    ...
    it_behaves_like 'authorizing granular token permissions for GraphQL', :create_add_on_assignment do
      let(:user) { current_user }
      let(:boundary_object) { namespace }
      let(:mutation) { graphql_mutation(:user_add_on_assignment_create, input, 'errors') }
    ...
    context 'when user belongs to subgroup' do
      let_it_be(:subgroup, freeze: false) { create(:group, parent: namespace) }
    ...
    context 'when the query requests add on assignments that belong to a different namespace' do
      let_it_be(:additional_purchase, freeze: false) { create(:gitlab_subscription_add_on_purchase, add_on: add_on) }
    ...
    context 'when there are multiple add-on assignments for the user' do
      let_it_be(:additional_purchase, freeze: false) do
  ...
  context 'on self managed instances' do
    ...
    context 'with a granular token on the instance boundary', :enable_admin_mode do
    ...
    context 'when user is not elgible for add-on' do
      let_it_be(:assignee_user, freeze: false) { create(:user, :ghost) }
