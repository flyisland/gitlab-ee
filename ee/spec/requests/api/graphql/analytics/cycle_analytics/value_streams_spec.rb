...
RSpec.describe '(Project|Group).value_streams', feature_category: :value_stream_management do
  ...
  let_it_be(:current_time, freeze: false) { Time.current }
  ...
  shared_examples 'value streams query' do
    context 'when value streams are licensed' do
      let_it_be(:value_streams, freeze: false) do
        [
          create(
      ...
      context 'when current user has permissions' do
        ...
        context 'when value stream has stages' do
          ...
          context 'with associated labels' do
            let_it_be(:stage_with_label, freeze: false) do
          ...
          context 'when specifying the stage id argument' do
            ...
            context 'when requesting aggregated metrics' do
              ...
              let_it_be(:epic, freeze: false) { create(:epic, group: resource.root_ancestor) }
              let_it_be(:milestone, freeze: false) { create(:milestone, group: resource.root_ancestor) }
              let_it_be(:filter_label, freeze: false) { create(:group_label, group: resource.root_ancestor) }
              ...
              let_it_be(:merge_request3, freeze: false) do
  ...
  context 'for projects' do
    ...
    let_it_be(:resource, freeze: false) { create(:project, namespace: create(:group)) }
    let_it_be(:project, freeze: false) { resource }
    let_it_be(:namespace, freeze: false) { resource.project_namespace }
    let_it_be(:start_label, freeze: false) { create(:label, project: resource, title: 'Start Label') }
    let_it_be(:end_label, freeze: false) { create(:label, project: resource, title: 'End Label') }
    ...
    context 'when filtering for project ids' do
      let_it_be(:value_stream, freeze: false) do
        create(
          :cycle_analytics_value_stream,
          namespace: namespace,
          name: 'Custom 2'
  ...
  context 'for groups' do
    ...
    let_it_be(:project, freeze: false) { create(:project, group: resource) }
    let_it_be(:namespace, freeze: false) { resource }
    let_it_be(:start_label, freeze: false) { create(:group_label, group: resource, title: 'Start Label') }
    let_it_be(:end_label, freeze: false) { create(:group_label, group: resource, title: 'End Label') }
    let_it_be(:project_2, freeze: false) { create(:project, group: resource) }
    let_it_be(:project_3, freeze: false) { create(:project, group: resource) }
    let(:value_stream) { value_streams.last }
    let_it_be(:assignee, freeze: false) { create(:user) }
    ...
    context 'when filtering for project ids' do
      ...
      let_it_be(:merge_request4, freeze: false) do
