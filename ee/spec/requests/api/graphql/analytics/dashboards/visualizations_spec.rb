  ...
  context 'when current user is a developer' do
    ...
    context 'when clickhouse is enabled' do
      ...
      it 'returns the correct number of visualizations' do
        ...
        expect(graphql_data_at(:project, :customizable_dashboards, :nodes, 0, :panels, :nodes).count).to eq(19)
      end
      ...
      where(:node_idx, :panel_type, :panel_title) do
        ...
        5  | 'LineChart'          | 'Pipelines using GitLab Duo Agent Platform over time'
        6  | 'AiImpactTable'      | 'GitLab Duo usage metrics for the %{namespaceName} %{namespaceType}'
        ...
        9  | 'DataTable'          | 'GitLab Duo usage by user'
        10 | 'DataTable'          | 'GitLab Duo Code Review usage by user'
        11 | 'DataTable'          | 'GitLab Duo Root Cause Analysis usage by user'
        12 | 'DataTable'          | 'Flows usage by user'
        13 | 'DataTable'          | 'GitLab Duo Code Suggestions usage by user'
        14 | 'BarChart'           | 'GitLab Duo Code Suggestions acceptance by language'
        15 | 'BarChart'           | 'GitLab Duo Code Suggestions acceptance by IDE'
        16 | 'StackedColumnChart' | 'GitLab Duo Code Review requests by role'
        17 | 'AreaChart'          | 'GitLab Duo Code Review comments sentiment'
        18 | 'AreaChart'          | 'Code generation volume trends'
      end
