...
RSpec.describe '(Group|Project).analytics.agentPlatformSessions', :click_house, time_travel_to: '2026-01-30', feature_category: :code_suggestions do
  ...
  shared_examples 'agentPlatformSessions query' do
    context 'when user is not authorized' do
        ...
        <<~QUERY
          query {
            #{query_type}(fullPath: "#{query_path}") {
              analytics {
                agentPlatformSessions { aggregated { nodes { totalCount } } }
              }
            }
          }
        QUERY
      ...
      it 'returns no data' do
        ...
        expect(graphql_data.dig(query_type, 'analytics', 'agentPlatformSessions')).to be_nil
      end
    end
