...
RSpec.describe '(Group|Project).aiUsageData.all', feature_category: :code_suggestions do
  ...
  let_it_be(:subgroup, freeze: false) { create(:group, parent: group, name: 'my-subgroup') }
  let_it_be(:group_project, freeze: false) { create(:project, group: group) }
  let_it_be(:subgroup_project, freeze: false) { create(:project, group: group) }
  let_it_be(:other_group_project, freeze: false) { create(:project) }
  let_it_be(:current_user, freeze: false) { create(:user, :with_namespace) }
  let_it_be(:user_1, freeze: false) { create(:user, :with_namespace) }
  let_it_be(:user_2, freeze: false) { create(:user, :with_namespace) }
  let_it_be(:user_3, freeze: false) { create(:user, :with_namespace) }
  ...
  let_it_be(:code_suggestion_event_3, freeze: false) do
  ...
  let_it_be(:code_suggestion_event_4, freeze: false) do
    create(:ai_usage_event, event: :code_suggestion_accepted_in_ide, user: user_3,
      namespace: subgroup_project.reload.project_namespace)
  end

  let_it_be(:out_of_timeframe_event, freeze: false) do
    create(:ai_usage_event, event: :code_suggestion_accepted_in_ide, user: user_3,
  ...
  context 'when using ClickHouse data source', :click_house do
    ...
    context 'when there are duplicate events' do
      let_it_be(:duplicate_event, freeze: false) do
        create(:ai_usage_event, event: :code_suggestion_shown_in_ide, user: user_1,
          namespace: group_project.reload.project_namespace)
      end
