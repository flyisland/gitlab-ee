...
RSpec.describe 'getting a work item list for a project', feature_category: :team_planning do
  ...
  describe 'work items with features' do
    let(:features_data) { graphql_dig_at(items_data, :node, :features) }

    context 'with verification status widget' do
      let_it_be(:work_item1, freeze: false) { create(:work_item, :satisfied_status, project: project) }
      let_it_be(:work_item2, freeze: false) { create(:work_item, :failed_status, project: project) }
      let_it_be(:work_item3, freeze: false) { create(:work_item, :requirement, project: project) }
      ...
      it 'returns work items including verification status', :aggregate_failures do
        ...
        expect(graphql_dig_at(features_data, 'verificationStatus')).to include(
    ...
    context 'with legacy requirement widget' do
      let_it_be(:work_item1, freeze: false) { create(:work_item, :requirement, project: project) }
      let_it_be(:work_item2, freeze: false) { create(:work_item, :requirement, project: project) }
      let_it_be(:work_item3, freeze: false) { create(:work_item, :requirement, project: project) }
      let_it_be(:work_item3_different_project, freeze: false) { create(:work_item, :requirement, iid: work_item3.iid) }
      ...
      it 'returns work items including legacy iid', :aggregate_failures do
        ...
        expect(graphql_dig_at(features_data, 'requirementLegacy')).to include(
    ...
    context 'with progress widget' do
      let_it_be(:work_item1, freeze: false) { create(:work_item, :objective, project: project) }
      let_it_be(:progress, freeze: false) { create(:progress, work_item: work_item1) }
    ...
    context 'with test reports widget' do
      let_it_be(:requirement_work_item_1, freeze: false) { create(:work_item, :requirement, project: project) }
      let_it_be(:test_report, freeze: false) { create(:test_report, requirement_issue: requirement_work_item_1) }
    ...
    context 'with iteration widget' do
      let_it_be(:iteration_cadence, freeze: false) { create(:iterations_cadence, group: project.group) }
      let_it_be(:iteration, freeze: false) { create(:iteration, iterations_cadence: iteration_cadence) }
      let_it_be(:work_item1, freeze: false) { create(:work_item, :issue, project: project, iteration: iteration) }
    ...
    context 'with status widget' do
      ...
      let_it_be(:work_item_1, freeze: false) { create(:work_item, :task, project: project) }
      let_it_be(:work_item_2, freeze: false) { create(:work_item, :task, project: project) }
      let_it_be(:work_item_3, freeze: false) { create(:work_item, :task, project: project) }
      ...
      context 'when feature is licensed' do
        ...
        context 'with current statuses' do
          context 'with system-defined status' do
            let_it_be(:current_status, freeze: false) do
            ...
            it 'returns status data' do
              ...
              expect(graphql_dig_at(features_data, 'status')).to include(
                hash_including(
                  'status' => {
          ...
          context 'with custom lifecycle' do
            let_it_be(:work_item_type, freeze: false) { build(:work_item_system_defined_type, :task) }

            let_it_be(:lifecycle, freeze: false) do
              create(:work_item_custom_lifecycle, namespace: namespace)
            end

            let_it_be(:custom_status, freeze: false) { lifecycle.default_open_status }
      ...
      context 'when feature is unlicensed' do
        ...
        context 'with current statuses' do
          let_it_be(:current_status, freeze: false) { create(:work_item_current_status, work_item: work_item_1) }

          it 'does not return status widget' do
            post_graphql(query, current_user: current_user)

            expect(graphql_dig_at(features_data, 'status')).not_to include(
    ...
    context 'with linked items widget' do
      describe 'with ai workflow token' do
        let_it_be(:work_item1, freeze: false) { create(:work_item, project: project) }
        let_it_be(:work_item2, freeze: false) { create(:work_item, project: project) }
        let_it_be(:other_project, freeze: false) { create(:project, :repository, :public, group: group) }
        let_it_be(:other_milestone, freeze: false) { create(:milestone, project: other_project) }
        let_it_be(:related_items, freeze: false) do
          create_list(:work_item, 3, project: other_project, milestone: other_milestone)
        end

        let_it_be(:ai_workflows_oauth_token, freeze: false) do
        ...
        let(:linked_items) { graphql_dig_at(items_data, 'features', 'linkedItems', 'linkedItems', 'nodes', 'workItem') }
  ...
  describe 'sorting' do
    let_it_be(:status_1, freeze: false) { build(:work_item_system_defined_status, :to_do) }
    let_it_be(:status_2, freeze: false) { build(:work_item_system_defined_status, :in_progress) }
    let_it_be(:work_item_1, freeze: false) do
      create(:work_item, project: project, system_defined_status_id: status_1.id)
    end

    let_it_be(:work_item_2, freeze: false) do
      create(:work_item, :closed, project: project, system_defined_status_id: status_2.id)
    end
  ...
  context 'with agent plan widget' do
    let_it_be(:agent_plan_work_item1, freeze: false) { create(:work_item, project: project) }
    let_it_be(:agent_plan_work_item2, freeze: false) { create(:work_item, project: project) }
    let_it_be(:agent_plan1, freeze: false) do
      create(:work_item_agent_plan, work_item: agent_plan_work_item1, content: 'plan 1')
    end

    let_it_be(:agent_plan2, freeze: false) do
      create(:work_item_agent_plan, work_item: agent_plan_work_item2, content: 'plan 2')
    end
    ...
    context 'for content field across multiple work items' do
      let(:fields) do
        <<~GRAPHQL
          nodes {
            id
      ...
      it 'limits the field to 1 execution' do
        ...
        expect_graphql_errors_to_include(
          '"content" field can be requested only for 1 WorkItemWidgetAgentPlan(s) at a time.'
        )
      end
    end

    context 'for contentHtml field across multiple work items' do
      let(:fields) do
        <<~GRAPHQL
          nodes {
            id
      ...
      it 'limits the field to 1 execution' do
        ...
        expect_graphql_errors_to_include(
          '"contentHtml" field can be requested only for 1 WorkItemWidgetAgentPlan(s) at a time.'
        )
      end
    end

    context 'for a single work item' do
      let(:item_filter_params) { { iids: [agent_plan_work_item1.iid.to_s] } }
      ...
      it 'returns the content without errors' do
        post_graphql(query, current_user: current_user)
  ...
  context 'when work item epics are present' do
    let_it_be(:epic1, freeze: false) { create(:work_item, :epic, project: project) }
    let_it_be(:epic2, freeze: false) { create(:work_item, :epic, project: project) }
