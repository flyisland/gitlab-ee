...
RSpec.describe 'Query.project(fullPath).pushRules', feature_category: :source_code_management do
  ...
  it_behaves_like 'authorizing granular token permissions for GraphQL', [:read_project, :read_push_rule] do
    let(:user) { create(:user, developer_of: project) }
    let(:boundary_object) { project }
    let(:query) do
      graphql_query_for(:project, { full_path: project.full_path }, "pushRules { denyDeleteTag }")
    end
  ...
  describe 'pushRules.rejectUnsignedCommits' do
    ...
    with_them do
      ...
      it "returns" do
        expect(push_rules_response).to include("rejectUnsignedCommits" => expected)
      end
    end
  end
end
