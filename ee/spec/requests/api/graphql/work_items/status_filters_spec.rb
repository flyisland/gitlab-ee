...
RSpec.describe 'Status filters', feature_category: :team_planning do
  ...
  let_it_be(:project_2, freeze: false) { create(:project, group: group_2) }
  ...
  let_it_be(:task_work_item_type, freeze: false) { build(:work_item_system_defined_type, :task) }

  let_it_be(:work_item_1, freeze: false) { create(:work_item, :issue, project: project, labels: [group_label]) }
  let_it_be(:work_item_2, freeze: false) { create(:work_item, :task, project: project, labels: [group_label]) }
  let_it_be(:work_item_3, freeze: false) { create(:work_item, :task, project: project, labels: [group_label]) }
  let_it_be(:work_item_4, freeze: false) { create(:work_item, :task, project: project, labels: [group_label]) }
  let_it_be(:work_item_5, freeze: false) { create(:work_item, :issue, project: project_2) }
  ...
  shared_examples 'filtering by status' do
    context 'for work items' do
      ...
      context 'when querying group.workItems' do
        let_it_be(:resource_parent, freeze: false) { group }
      ...
      context 'when querying project.workItems' do
        let_it_be(:resource_parent, freeze: false) { project }
  ...
  context 'with system defined statuses' do
    let_it_be(:current_status_1, freeze: false) { create(:work_item_current_status, work_item: work_item_1) }
    let_it_be(:current_status_2, freeze: false) { create(:work_item_current_status, work_item: work_item_2) }
    let_it_be(:current_status_4, freeze: false) do
      create(:work_item_current_status, work_item: work_item_4, system_defined_status_id: 2)
    end

    let_it_be(:status, freeze: false) { build(:work_item_system_defined_status, :to_do) }
  ...
  context 'with custom statuses' do
    ...
    let_it_be(:lifecycle_2, freeze: false) do
    ...
    let_it_be(:status, freeze: false) { lifecycle.default_open_status }

    let_it_be(:current_status_2, freeze: false) do
    ...
    context 'with status mappings' do
      let_it_be(:old_status, freeze: false) do
      ...
      let_it_be(:new_status, freeze: false) do
        create(:work_item_custom_status, :without_conversion_mapping, namespace: group).tap do |status|
          create(:work_item_custom_lifecycle_status, lifecycle: lifecycle, status: status)
        end
      end

      let_it_be(:another_status, freeze: false) do
        create(:work_item_custom_status, :without_conversion_mapping, namespace: group).tap do |status|
          create(:work_item_custom_lifecycle_status, lifecycle: lifecycle, status: status)
        end
      end

      let_it_be(:converted_custom_status, freeze: false) do
        create(:work_item_custom_status, :in_progress, namespace: group).tap do |status|
          create(:work_item_custom_lifecycle_status, lifecycle: lifecycle, status: status)
        end
      end

      let_it_be(:work_item_issue_with_new_status, freeze: false) do
      ...
      let_it_be(:work_item_issue_with_another_status, freeze: false) do
        create(:work_item, :issue, project: project, labels: [group_label]).tap do |wi|
      ...
      let_it_be(:work_item_issue_with_system_status, freeze: false) do
        create(:work_item, :issue, project: project, labels: [group_label]).tap do |wi|
      ...
      context 'when mapping for issues exists for converted system-defined status' do
        ...
        context 'when filtering for the converted status while the status still exists for tasks' do
          let_it_be(:work_item_task_with_converted_status, freeze: false) do
            create(:work_item, :task, project: project, labels: [group_label]).tap do |wi|
