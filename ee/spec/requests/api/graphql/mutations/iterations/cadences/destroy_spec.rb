...
RSpec.describe 'Destroying an iteration cadence', feature_category: :team_planning do
  ...
  context 'when the user has permission' do
    ...
    context 'when iterations feature is enabled' do
      ...
      it_behaves_like 'authorizing granular token permissions for GraphQL', :delete_iteration_cadence do
        let(:user) { current_user }
        let(:boundary_object) { group }
        let(:mutation) do
          graphql_mutation(:iteration_cadence_destroy, { id: iteration_cadence.to_global_id.to_s }, 'errors')
        end
