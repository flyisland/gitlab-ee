  ...
  context 'when the user has permission' do
   ...
   'exist or you don\'t have permission to perform this action']
    ...
    context 'when iterations feature is enabled' do
      ...
      it_behaves_like 'authorizing granular token permissions for GraphQL', :create_iteration_cadence do
        let(:user) { current_user }
        let(:boundary_object) { group }
        let(:mutation) do
          graphql_mutation(:iteration_cadence_create,
            { group_path: group.full_path, title: 'title', automatic: false, active: true },
            'errors')
        end
