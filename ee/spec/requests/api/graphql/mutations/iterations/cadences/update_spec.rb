...
RSpec.describe 'Updating an iteration cadence', feature_category: :team_planning do
  ...
  let_it_be_with_refind(:iteration_cadence) { create(:iterations_cadence, group: group) }
  ...
  context 'when the user has permission' do
   ...
   'exist or you don\'t have permission to perform this action']
    ...
    context 'when iterations feature is enabled' do
      ...
      it_behaves_like 'authorizing granular token permissions for GraphQL', :update_iteration_cadence do
        let(:user) { current_user }
        let(:boundary_object) { group }
        let(:mutation) do
          graphql_mutation(:iteration_cadence_update, { id: iteration_cadence.to_global_id.to_s }, 'errors')
        end
      ...
      it 'updates the iteration cadence', :aggregate_failures do
        ...
        expect(iteration_cadence_hash['automatic']).to be(true)
        expect(iteration_cadence_hash['active']).to be(false)
        expect(iteration_cadence_hash['rollOver']).to be(true)
        ...
        expect(iteration_cadence.automatic).to be(true)
        expect(iteration_cadence.active).to be(false)
        expect(iteration_cadence.roll_over).to be(true)
