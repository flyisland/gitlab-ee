...
RSpec.describe 'Deleting an iteration', feature_category: :team_planning do
  ...
  let_it_be_with_refind(:manual_iteration) { create(:iteration, iterations_cadence: cadence) }
  ...
  context 'when the user has permission' do
    ...
    context 'when iterations feature is enabled' do
      ...
      it_behaves_like 'authorizing granular token permissions for GraphQL', :delete_iteration do
        let(:user) { current_user }
        let(:boundary_object) { group }
        let(:mutation) do
          graphql_mutation(:iteration_delete, { id: iteration.to_global_id.to_s }, 'errors')
        end
