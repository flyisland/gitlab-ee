  ...
  context 'when the user has permission' do
   ...
   'exist or you don\'t have permission to perform this action']
    ...
    context 'when iterations are enabled' do
      ...
      it_behaves_like 'authorizing granular token permissions for GraphQL', :create_iteration do
        let(:user) { current_user }
        let(:boundary_object) { group }
        let(:request) { post_graphql_mutation(mutation, token: { personal_access_token: pat }) }
      end

      # Covers the deprecated `CreateIteration` alias of this mutation.
      it_behaves_like 'authorizing granular token permissions for GraphQL', :create_iteration do
        let(:user) { current_user }
        let(:boundary_object) { group }
        let(:mutation) do
      ...
      context 'when title is not given' do
        ...
        it 'creates an iteration' do
          ...
          iteration_hash = mutation_response['iteration']
          aggregate_failures do
            expect(iteration_hash['title']).to be_nil
