...
RSpec.describe 'Updating an Iteration', feature_category: :team_planning do
  ...
  context 'when the user has permission' do
    ...
    context 'when iterations are enabled' do
      ...
      it_behaves_like 'authorizing granular token permissions for GraphQL', :update_iteration do
        let(:user) { current_user }
        let(:boundary_object) { group }
        let(:mutation) do
          graphql_mutation(
            :update_iteration,
            { group_path: group.full_path, id: iteration.to_global_id.to_s, description: 'updated description' },
            'errors'
          )
        end
