...
RSpec.describe 'Map a cluster agent to a group', feature_category: :workspaces do
  ...
  context 'with granular token authorization' do
    ...
    it_behaves_like 'authorizing granular token permissions for GraphQL', :create_cluster_agent_mapping do
      let(:boundary_object) { namespace }
      let(:mutation) do
        graphql_mutation(:namespace_create_remote_development_cluster_agent_mapping, all_mutation_args, 'errors')
      end
