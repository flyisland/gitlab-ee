  ...
  context 'when the user has permission' do
    ...
    context 'when security_dashboard is enabled' do
      ...
      it_behaves_like 'authorizing granular token permissions for GraphQL',
        :delete_vulnerability_external_issue_link do
        let(:user) { current_user }
        let(:boundary_object) { vulnerability_external_issue_link.vulnerability.project }
        let(:authz_mutation) do
          graphql_mutation(
            :vulnerability_external_issue_link_destroy,
            { id: vulnerability_external_issue_link.to_global_id.to_s },
            'errors'
          )
        end
