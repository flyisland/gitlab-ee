...
RSpec.describe 'Creating an External Issue Link', feature_category: :vulnerability_management do
  ...
  let_it_be_with_reload(:finding) { create(:vulnerabilities_finding, :with_pipeline, project: project, severity: :high) }
  let_it_be_with_reload(:vulnerability) { create(:vulnerability, title: 'My vulnerability', project: project, findings: [finding]) }
        ...
        context 'when issue creation succeeds' do
          ...
          context 'and saving external issue link succeeds' do
            ...
            it_behaves_like 'authorizing granular token permissions for GraphQL',
              :create_vulnerability_external_issue_link do
              let(:user) { current_user }
              let(:boundary_object) { project }
              let(:authz_mutation) do
                graphql_mutation(
                  :vulnerability_external_issue_link_create,
                  { id: vulnerability.to_global_id.to_s, link_type: 'CREATED', external_tracker: 'JIRA' },
                  'errors'
                )
              end
