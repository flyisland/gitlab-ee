...
RSpec.describe 'Unassigns scan execution policy project from a project/namespace', feature_category: :security_policy_management do
  ...
  context 'with granular token authorization' do
    ...
    it_behaves_like 'authorizing granular token permissions for GraphQL', :update_security_policy do
      ...
      let(:mutation) do
        graphql_mutation(:security_policy_project_unassign, { full_path: project.full_path }, 'errors')
      end
    ...
    it_behaves_like 'authorizing granular token permissions for GraphQL', :update_security_policy do
      ...
      let(:mutation) do
        graphql_mutation(:security_policy_project_unassign, { full_path: namespace.full_path }, 'errors')
      end
