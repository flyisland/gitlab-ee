...
RSpec.describe 'Creates and assigns scan execution policy project to a project/namespace', feature_category: :security_policy_management do
  ...
  shared_examples 'creates security policy project' do
    context 'when licensed feature is available' do
      ...
      context 'when current_user is a security_manager' do
        ...
        let(:current_user) { security_manager_user }
  ...
  context 'with granular token authorization' do
    ...
    it_behaves_like 'authorizing granular token permissions for GraphQL', :update_security_policy do
      ...
      let(:mutation) do
        graphql_mutation(:security_policy_project_create, { full_path: project.full_path }, <<-QL.strip_heredoc)
          project {
            id
            name
          }
    ...
    it_behaves_like 'authorizing granular token permissions for GraphQL', :update_security_policy do
      ...
      let(:mutation) do
        graphql_mutation(:security_policy_project_create, { full_path: namespace.full_path }, <<-QL.strip_heredoc)
          project {
            id
            name
          }
