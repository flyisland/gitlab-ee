...
RSpec.describe 'Setting the epic of an issue', feature_category: :team_planning do
  ...
  context 'when epic has to be in a different group' do
    ...
    context 'when user can create epic in new group' do
      # TODO: restore threshold after epic-work item sync
      # issue: https://gitlab.com/gitlab-org/gitlab/-/issues/438295
      # The query count during promotion is non-deterministic (142-147 observed in CI,
      # depending on cold vs. warm per-process caches), so the threshold keeps margin
      # above the observed maximum to avoid flaky failures.
      let(:queries_count_threshold) { 150 }
