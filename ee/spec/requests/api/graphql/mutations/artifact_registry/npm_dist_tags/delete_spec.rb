...
RSpec.describe 'Deleting one dist-tag of an Artifact Registry npm package',
  ...
  let(:tag_id) { 'e5f6a7b8-0000-0000-0000-000000000000' }
  ...
  # The npm segment is a literal on this route rather than a value the mutation supplies, so the
  # URL is built from the literal here too and a maven URL is never a thing the spec can express.
  let(:dist_tag_url) { "#{repository_url}/npm/tags/#{tag_id}" }
  ...
  let(:input) { { 'name' => repository_name, 'id' => tag_id } }

  let(:mutation) do
    graphql_mutation(:artifact_registry_npm_dist_tag_delete, input, <<~FIELDS)
      errors
      repository {
        name
        format
  ...
  context 'when Artifact Registry accepts the request' do
    it 'deletes the dist-tag id under the npm route, and resolves', :aggregate_failures do
      read = stub_repository_read
      deletion = stub_request(:delete, dist_tag_url).to_return(status: 202, body: '', headers: json_headers)
      ...
      expect(a_request(:delete, dist_tag_url)
        .with(headers: { 'Authorization' => "Bearer #{token}" })).to have_been_made.once
      expect(mutation_response['errors']).to be_empty
      expect(mutation_response['repository'])
        .to eq('name' => repository_name, 'format' => 'NPM', 'kind' => 'HOSTED')
    end
  end

  # Artifact Registry refuses a remote repository permanently, in its own prose. Rails answers
  # first so the message is stable and no round trip is spent on a refusal it already knows.
  context 'when the repository is remote rather than hosted' do
    let(:kind) { 'remote' }
    let(:repository_name) { 'npm-remote' }

    it 'refuses before any deletion is attempted with a distinct error',
      :aggregate_failures do
      stub_repository_read
      deletion = stub_request(:delete, dist_tag_url)

      post_graphql_mutation(mutation, current_user: current_user)

      expect(response).to have_gitlab_http_status(:success)
      expect(deletion).not_to have_been_requested
      expect(mutation_response['errors']).to include('Dist-tags cannot be deleted from remote repositories.')
      expect(mutation_response['repository']).to be_nil
      expect(graphql_errors).to be_nil
    end
  end

  # Both guards match here, and the format one wins: a maven repository has no dist-tags whether
  # it is hosted or remote, so naming the kind would imply a hosted maven repository would work.
  context 'when the repository is both remote and not npm' do
    let(:kind) { 'remote' }
    let(:format) { 'maven' }
    let(:repository_name) { 'maven-remote' }

    it 'refuses on format rather than on kind, before any deletion is attempted', :aggregate_failures do
      stub_repository_read
      deletion = stub_request(:delete, dist_tag_url)
      ...
      expect(response).to have_gitlab_http_status(:success)
      expect(deletion).not_to have_been_requested
      expect(mutation_response['errors']).to contain_exactly('Only npm repositories have dist-tags.')
      expect(mutation_response['repository']).to be_nil
      expect(graphql_errors).to be_nil
    end
  end

  context 'when the repository holds maven packages rather than npm ones' do
    let(:format) { 'maven' }
    ...
    it 'refuses before any deletion is attempted with a payload error', :aggregate_failures do
      ...
      expect(response).to have_gitlab_http_status(:success)
      expect(deletion).not_to have_been_requested
      expect(mutation_response['errors']).to include('Only npm repositories have dist-tags.')
      expect(mutation_response['repository']).to be_nil
      expect(graphql_errors).to be_nil
    end
  end

  context 'when the repository holds container images' do
    let(:format) { 'docker' }
    let(:repository_name) { 'docker-hosted' }
  ...
  context 'when the dist-tag is already gone (404)' do
    it 'reports a mutation error rather than resolving as a success', :aggregate_failures do
      stub_repository_read
      stub_request(:delete, dist_tag_url)
        .to_return(status: 404, body: { error: { code: 'not_found', message: 'dist-tag not found' } }.to_json,
          headers: json_headers)

      post_graphql_mutation(mutation, current_user: current_user)

      expect(response).to have_gitlab_http_status(:success)
      expect(mutation_response['errors']).to include('dist-tag not found')
      expect(mutation_response['repository']).to be_nil
      expect(graphql_errors).to be_nil
    end
  end
  ...
  context 'when authorization could not be evaluated (503)' do
    it 'surfaces the service-unavailable error rather than a refusal', :aggregate_failures do
      ...
      stub_repository_read
      deletion = stub_request(:delete, dist_tag_url).to_return(status: 503, body: '{}', headers: json_headers)
  ...
  context 'when Artifact Registry denies the request (403)' do
    it 'reports the refusal after the attempt rather than hiding the repository', :aggregate_failures do
      stub_repository_read
      deletion = stub_request(:delete, dist_tag_url).to_return(status: 403, body: '{}', headers: json_headers)
  ...
  context 'when the dist-tag id is blank' do
    let(:tag_id) { '' }
    ...
    it 'raises the client guard before any deletion is attempted', :aggregate_failures do
      ...
      expect_graphql_errors_to_include('tag_id is required')
    end
  end
  ...
  def mutation_response
    graphql_mutation_response(:artifact_registry_npm_dist_tag_delete)
  end
end
