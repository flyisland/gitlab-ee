...
RSpec.describe 'Updating a work item type', feature_category: :team_planning do
  ...
  context 'when archiving a work item type' do
    ...
    context 'with a granular token' do
      let(:pat) do
        create(:granular_pat, user: user, boundary: ::Authz::Boundary.for(group), permissions: assignables)
      end

      context 'when the token only grants update_work_item' do
        let(:assignables) { %w[update_work_item] }

        it 'denies archiving' do
          post_graphql_mutation(mutation, token: { personal_access_token: pat })

          expect_graphql_errors_to_include(/group permissions: \[Work Item Type: Archive\]\./)
          expect(work_item_type.reload.archived).to be false
        end
      end

      context 'when the token also grants archive_work_item_type' do
        let(:assignables) { %w[update_work_item archive_work_item_type] }
