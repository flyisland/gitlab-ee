...
RSpec.describe 'Generate a workplan for a work item', feature_category: :duo_agent_platform do
  ...
  let(:mutation) { graphql_mutation(:workItemGenerateWorkplan, input, fields) }
  let(:mutation_response) { graphql_mutation_response(:work_item_generate_workplan) }
  ...
  context 'when the service starts a workflow' do
    ...
    before do
      # Exercises the real GenerateWorkplanService; only the CI-backed workflow
  ...
  context 'when the service returns an error' do
    before do
      allow_next_instance_of(::Ai::DuoWorkflows::GenerateWorkplanService) do |service|
    ...
    it 'surfaces the error and returns no workflow', :aggregate_failures do
      ...
      expect(mutation_response['errors']).to include('Async workplan generation is not enabled for this project')
    end
  end
  ...
  it_behaves_like 'authorizing granular token permissions for GraphQL', :update_work_item do
    ...
    let(:mutation) { graphql_mutation(:workItemGenerateWorkplan, input, 'errors') }
