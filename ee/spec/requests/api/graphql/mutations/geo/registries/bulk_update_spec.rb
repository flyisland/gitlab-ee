...
RSpec.describe Mutations::Geo::Registries::BulkUpdate, feature_category: :geo_replication do
  ...
  with_them do
    ...
    context 'with granular token authorization' do
      ...
      it_behaves_like 'authorizing granular token permissions for GraphQL', :update_geo_registry do
        let(:user) { current_user }
        let(:boundary_object) { :instance }
        let(:mutation) do
          graphql_mutation(mutation_name, { registry_class: registry_class_argument, action: 'RESYNC_ALL' }, 'errors')
        end
