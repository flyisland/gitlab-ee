...
RSpec.describe Mutations::Security::Finding::SeverityOverride, feature_category: :vulnerability_management do
  ...
  describe '#resolve' do
    ...
    let_it_be(:scan, freeze: false) do
      create(
        :security_scan,
        :latest_successful,
    ...
    let_it_be(:finding_uuid, freeze: false) { security_finding.uuid }
    let_it_be(:new_severity, freeze: false) { 'HIGH' }
    let_it_be(:mutation_input, freeze: false) { { uuid: security_finding.uuid.to_s, severity: new_severity } }
    let_it_be(:error_message, freeze: false) { 'severity override failed' }
    let_it_be(:error_result, freeze: false) do
    ...
    context 'when the user has access to vulnerability management' do
      ...
      context 'when no uuid is provided' do
        let_it_be(:mutation_input, freeze: false) { { severity: new_severity } }
      ...
      context 'when no severity is provided' do
        let_it_be(:mutation_input, freeze: false) { { uuid: finding_uuid } }
      ...
      context 'when the user has access to the project' do
        let_it_be(:expected_finding, freeze: false) do
