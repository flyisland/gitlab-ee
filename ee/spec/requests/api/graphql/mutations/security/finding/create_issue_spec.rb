...
RSpec.describe 'Creating an Issue from a Security::Finding', feature_category: :vulnerability_management do
  ...
  let_it_be_with_reload(:report) { create(:ci_reports_security_report, pipeline: pipeline, type: :sast) }
  let_it_be_with_reload(:scan) do
  ...
  # Array that a before block appends to via `security_findings.push(...)`;
  # freezing it raises FrozenError and reload/refind are no-ops on an Array.
  # Keep as-is (see gitlab-org/gitlab#602925).
  let_it_be(:security_findings, freeze: false) { [] }
  ...
  context 'when the user has permission' do
    ...
    context 'with valid parameters' do
      ...
      it_behaves_like 'authorizing granular token permissions for GraphQL', :create_vulnerability_issue_link do
        before do
        ...
        let(:authz_mutation) do
          graphql_mutation(
            mutation_name,
            { project: project_gid, uuid: security_finding_uuid },
            'errors'
          )
        end
