...
RSpec.describe 'Update a compliance framework', feature_category: :compliance_management do
  ...
  context 'feature is licensed' do
    ...
    context 'with valid params' do
      it_behaves_like 'authorizing granular token permissions for GraphQL', :update_compliance_framework do
        let(:user) { current_user }
        let(:boundary_object) { namespace }
        let(:mutation) do
          graphql_mutation(:update_compliance_framework, { id: global_id_of(framework), **params }, 'errors')
        end

        let(:request) { post_graphql_mutation(mutation, token: { personal_access_token: pat }) }
      end

      it 'returns the updated framework with no errors', :aggregate_failures do
        subject
    ...
    context 'with invalid params' do
      ...
      it 'returns an array of errors and does not update the framework', :aggregate_failures do
