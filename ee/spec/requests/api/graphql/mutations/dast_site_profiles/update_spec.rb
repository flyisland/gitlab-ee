...
RSpec.describe 'Creating a DAST Site Profile', feature_category: :dynamic_application_security_testing do
  ...
  let_it_be(:dast_site_profile_id, freeze: false) { global_id_of(dast_site_profile) }

  let_it_be(:new_profile_name, freeze: false) { SecureRandom.hex }
  let_it_be(:new_target_url, freeze: false) { generate(:url) }
  ...
  it_behaves_like 'an on-demand scan mutation when user can run an on-demand scan' do
    ...
    context 'when the dast_site_profile does not exist' do
      let_it_be(:dast_site_profile_id, freeze: false) { Gitlab::GlobalId.build(nil, model_name: 'DastSiteProfile', id: non_existing_record_id) }
    ...
    context 'when wrong type of global id is passed' do
      let_it_be(:dast_site_profile_id, freeze: false) { global_id_of(project) }
