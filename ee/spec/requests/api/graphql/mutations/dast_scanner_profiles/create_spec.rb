...
RSpec.describe 'Creating a DAST Scanner Profile', feature_category: :dynamic_application_security_testing do
  ...
  it_behaves_like 'an on-demand scan mutation when user can run an on-demand scan' do
    it_behaves_like 'authorizing granular token permissions for GraphQL', :create_dast_scanner_profile do
    ...
    it 'sets default values of omitted properties' do
      ...
      aggregate_failures do
        ...
        expect(dast_scanner_profile.use_ajax_spider).to be(false)
        expect(dast_scanner_profile.show_debug_messages).to be(false)
      end
    end
