...
RSpec.describe 'getting AI catalog agent flow configuration', :with_current_organization, :aggregate_failures, feature_category: :workflow_catalog do
  ...
  let_it_be(:agent, freeze: false) { create(:ai_catalog_agent, project: project) }
  let_it_be(:agent_version, freeze: false) { create(:ai_catalog_agent_version, item: agent) }
  ...
  context 'when agent belongs to different organization' do
    ...
    let_it_be(:different_agent, freeze: false) { create(:ai_catalog_agent, organization: different_organization) }
    let_it_be(:different_agent_version, freeze: false) { create(:ai_catalog_agent_version, item: different_agent) }
