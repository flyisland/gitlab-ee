...
RSpec.describe 'Querying Duo Workflow Events', feature_category: :duo_agent_platform do
  ...
  let_it_be(:user, freeze: false) { create(:user, developer_of: [project, another_project]) }
  ...
  let_it_be(:ide_workflow, freeze: false) do
  ...
  context 'when user is logged in' do
    ...
    it 'returns user messages' do
      ...
      event_nodes.sort_by { |event_node| event_node['node']['threadTs'] }.each_with_index do |event_node, i|
        ...
        stored = checkpoints[i].reload
        expect(Gitlab::Json.safe_parse(event['checkpoint'])).to eq(stored.checkpoint)
        expect(event['compressedCheckpoint']).to eq(
          Base64.strict_encode64(Zlib::Deflate.deflate(stored.checkpoint.to_json, 6))
        )
        expect(Gitlab::Json.safe_parse(event['metadata'])).to eq(stored.metadata)
