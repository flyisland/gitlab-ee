  ...
  context 'when namespace_id is provided' do
    ...
    context 'when group is root' do
      context 'when user is an owner' do
        ...
        context 'when a month holds several shards' do
          before do
            create(:ci_namespace_monthly_usage,
              namespace: group,
              shard_number: 2,
              amount_used: 50,
              shared_runners_duration: 60,
              date: Date.new(2021, 7, 1))
          end

          it 'returns one node for each month with the shard values added together' do
            subject
