...
RSpec.describe 'groupSecretsManager userPermissions', :gitlab_secrets_manager, feature_category: :secrets_management do
  ...
  context 'when current user has a grant via Group principal' do
    # Grant via the group itself as a Group principal. The user is a member of
    # the group, so the group ID appears in their JWT `groups` claim, and
    # OpenBao unions the group policy into their effective capabilities.
    let(:current_user) { create(:user, developer_of: group) }
