...
RSpec.describe 'getting iterations', feature_category: :team_planning do
  ...
  it_behaves_like 'authorizing granular token permissions for GraphQL', [:read_group, :read_iteration_cadence] do
    let(:boundary_object) { group }
    let(:request) do
      post_graphql(
        graphql_query_for(:group, { full_path: group.full_path },
          query_graphql_field(:iteration_cadences, {}, query_graphql_field(:nodes, nil, 'id'))),
