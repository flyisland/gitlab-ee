...
RSpec.describe 'Query.project(fullPath).securityPolicies.testRuns', feature_category: :security_policy_management do
  ...
  context 'when the security_policies_spep_dry_run feature flag is disabled' do
    before do
      stub_feature_flags(security_policies_spep_dry_run: false)
      post_graphql(query, current_user: user, variables: variables)
    end

    it 'returns empty test runs' do
      expect(test_runs_data).to be_empty
    end
  end
