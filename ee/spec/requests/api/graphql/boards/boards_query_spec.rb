...
RSpec.describe 'get list of boards', feature_category: :planning_views do
  ...
  shared_examples 'a board epics query' do
    ...
    it 'returns open epics referenced by issues in the board' do
      ...
      aggregate_failures 'board epics response' do
        ...
        expect(epics.first['userPreferences']['collapsed']).to be(true)
      end
    end
  end
  ...
  describe 'for a group' do
    ...
    it_behaves_like 'authorizing granular token permissions for GraphQL',
      [:read_group, :read_issue_board, :read_epic] do
      let_it_be(:board) { create(:board, resource_parent: board_parent) }
      let_it_be(:issue_project) { create(:project, group: board_parent) }
      let_it_be(:issue) { create(:issue, project: issue_project) }
      let_it_be(:epic) { create(:epic, group: board_parent) }
      ...
      let(:query) do
        graphql_query_for(:group, { full_path: board_parent.full_path },
          query_graphql_field(:boards, {}, 'nodes { epics { nodes { id } } }'))
      end
