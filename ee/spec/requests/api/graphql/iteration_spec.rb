...
RSpec.describe 'Querying an Iteration', feature_category: :team_planning do
  ...
  context 'when the user has access to the iteration' do
    ...
    it_behaves_like 'authorizing granular token permissions for GraphQL', :read_iteration do
      let(:user) { group_member }
      let(:boundary_object) { group }
      let(:request) { post_graphql(query, token: { personal_access_token: pat }) }
    end

    context 'when `group` field is included' do
      let(:fields) { 'group { id name }' }

      it 'returns the iteration group' do
        expect(graphql_data['iteration']['group']).to include(
