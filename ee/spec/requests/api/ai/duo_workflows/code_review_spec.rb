...
RSpec.describe API::Ai::DuoWorkflows::CodeReview, :with_current_organization, feature_category: :duo_code_review do
  ...
  let_it_be(:foundational_flow, freeze: false) { Ai::Catalog::FoundationalFlow['code_review/v1'] }
  ...
  let_it_be(:oauth_app, freeze: false) { create(:doorkeeper_application) }
  let_it_be(:scopes, freeze: false) { ::Gitlab::Auth::AI_WORKFLOW_SCOPES + ['api'] + ["user:#{user.id}"] }
  ...
  describe 'POST /ai/duo_workflows/code_review/add_comments' do
    ...
    context 'when successful' do
      ...
      context 'when workflow_id is provided' do
        ...
        it 'calls CreateCommentsService with workflow_id' do
          expect(::Ai::DuoWorkflows::CodeReview::CreateCommentsService).to receive(:new).with(
            user: user,
            merge_request: merge_request,
            review_output: review_output,
    ...
    context 'when user does not have access to project' do
      ...
      let_it_be(:unauthorized_service_account, freeze: false) do
        create(:user, :service_account,
          composite_identity_enforced: true,
          organization: organization,
          provisioned_by_group: group
    ...
    describe '#verify_composite_identity!' do
      ...
      context 'when service account does not match the toplevel group service account' do
        let_it_be(:mismatched_service_account, freeze: false) do
          create(:user, :service_account, organization: organization)
        end

        let_it_be(:mismatched_token, freeze: false) do
          create(:oauth_access_token,
            organization: organization,
            application: oauth_app,
