...
RSpec.describe API::Ai::DuoWorkflows::Workflows, :with_current_organization, feature_category: :duo_agent_platform do
  ...
  describe 'POST /ai/duo_workflows/workflows' do
    ...
    context 'when requires_duo_cli_enabled param is true' do
      let(:duo_cli_params) { params.merge(requires_duo_cli_enabled: true) }
      ...
      it 'returns 403 with a message indicating Duo CLI has been disabled' do
        post api(path, user), params: duo_cli_params
      ...
      context 'when duo_cli_enabled is true' do
        before do
          ::Ai::Setting.instance.update!(duo_cli_enabled: true)
        end
      ...
      context 'when duo_cli_enabled_setting feature flag is disabled' do
        before do
          stub_feature_flags(duo_cli_enabled_setting: false)
          ::Ai::Setting.instance.update!(duo_cli_enabled: false)
        end
  ...
  describe 'POST /ai/duo_workflows/direct_access' do
    ...
    shared_context 'when tokens are generated' do
      ...
      before do
        allow(::CloudConnector).to receive(:ai_headers).with(user, namespace_ids: anything)
    ...
    context 'when workflow_definition param is passed' do
      context 'when it is chat' do
        ...
        it 'calls usage quota service for chat feature' do
          expect(::Ai::UsageQuotaService).to receive(:new)
            .with(ai_feature: :duo_chat, user: user, namespace: group)
      ...
      context 'when it is not chat' do
        ...
        it 'calls usage quota service for duo_agent_platform feature' do
          expect(::Ai::UsageQuotaService).to receive(:new)
            .with(ai_feature: :duo_agent_platform, user: user, namespace: group)
      ...
      context 'for server_capabilities' do
        context 'when DWS returns empty capabilities' do
          ...
          it 'returns only default Rails capabilities' do
            ...
            expect(response).to have_gitlab_http_status(:created)
            expect(json_response['server_capabilities']).to match_array(%w[job_trace_pagination])
          end
          ...
          context 'when advanced search is enabled' do
            ...
            it 'returns advanced_search and default Rails capabilities' do
              ...
              expect(response).to have_gitlab_http_status(:created)
              expect(json_response['server_capabilities']).to match_array(%w[job_trace_pagination advanced_search])
            end
          end
        end
        ...
        context 'when DWS returns capabilities' do
          ...
          it 'returns union of Rails and DWS capabilities' do
            ...
            expect(json_response['server_capabilities']).to match_array(
              %w[job_trace_pagination advanced_search tool_call_approval tool_call_pattern_approval approve_for_session]
            )
          end
        ...
        context 'when tool_approval_for_session is disabled' do
          ...
          it 'filters out tool_call_approval and tool_call_pattern_approval from capabilities' do
            ...
            expect(response).to have_gitlab_http_status(:created)
            expect(json_response['server_capabilities']).to match_array(%w[job_trace_pagination approve_for_session])
          end
        end
        ...
        context 'when DWS supports capabilities Rails does not' do
          ...
          it 'returns union including all DWS capabilities' do
            ...
            expect(response).to have_gitlab_http_status(:created)
            expect(json_response['server_capabilities']).to match_array(
              %w[job_trace_pagination advanced_search tool_call_approval dws_only_feature]
            )
          end
        end
      end
  ...
  describe 'GET /ai/duo_workflows/ws' do
    ...
    context 'when user is authenticated' do
      let(:mcp_enabled_tools) { ::Ai::DuoWorkflows::McpConfigService::GITLAB_ENABLED_TOOLS }
      let(:mcp_preapproved_tools) { ::Ai::DuoWorkflows::McpConfigService::GITLAB_PREAPPROVED_TOOLS }
      ...
      it 'returns the websocket configuration with proper headers' do
        ...
        expect(json_response['DuoWorkflow']['Service']['Headers']).to include(
          ...
          'x-gitlab-enabled-mcp-server-tools' => mcp_enabled_tools.join(',')
        )
        ...
        expect(mcp_servers['gitlab']['PreApprovedTools']).to eq(mcp_preapproved_tools)
      ...
      context 'when gitlab_duo_governance_settings feature flag is enabled' do
        before do
          ...
          group.namespace_settings.update!(tool_approval_for_session_availability: :default_on)
        ...
        it 'passes tool_access_policies from governance resolution to the cloud connector token' do
          expect(::CloudConnector::Tokens).to receive(:get).with(
            hash_including(
              extra_claims: hash_including(tool_access_policies: '{"allow":["create_work_item"],"deny":[]}')
            )
          ).and_return('token')
      ...
      context 'when software_development_flow_registry feature flag is enabled for root namespace' do
        before do
          stub_feature_flags(software_development_flow_registry: group)
        end
      ...
      context 'when ai_catalog_item_version_id is provided' do
        ...
        it 'resolves agent tool IDs and restricts MCP tools accordingly' do
          ...
          # PreApprovedTools should match the resolved tools (all agent tools are preapproved)
          expect(mcp_servers['gitlab']['PreApprovedTools']).to eq(mcp_servers['gitlab']['Tools'])
        end
      ...
      context 'for TimeoutHTTPRequests' do
        context 'when timeout_dap_http_requests_in_workhorse FF is enabled' do
          ...
          it 'enables HTTP request timeouts' do
            ...
            expect(response).to have_gitlab_http_status(:ok)
            expect(json_response['DuoWorkflow']['TimeoutHTTPRequests']).to eq(true)
          end
        end
        ...
        context 'when timeout_dap_http_requests_in_workhorse FF is disabled' do
          ...
          it 'sets TimeoutHTTPRequests to false' do
            ...
            expect(response).to have_gitlab_http_status(:ok)
            expect(json_response['DuoWorkflow']['TimeoutHTTPRequests']).to eq(false)
          end
        end
      end
