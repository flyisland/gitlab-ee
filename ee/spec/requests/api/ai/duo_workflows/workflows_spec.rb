    ...
    context 'when workflow is chat' do
      ...
      context 'when user has no governing namespace' do
        before do
          ...
          allow_any_instance_of(User).to receive(:governing_namespace).and_return(nil)
      ...
      context 'when ai_catalog_item_consumer_id is provided' do
        ...
        it 'returns not found when consumer does not exist' do
          ...
          expect(json_response['message']).to include(
            'Enabled agent or flow was not found for this project.'
          )
        end
    ...
    context 'when success' do
      ...
      context 'for server_capabilities' do
        context 'when DWS returns empty capabilities' do
          ...
          it 'returns empty when both have no capabilities' do
            post_with_params

            expect(response).to have_gitlab_http_status(:created)
            expect(json_response['server_capabilities']).to eq([])
          end
          ...
          context 'when advanced search is enabled' do
            ...
            it 'returns only advanced_search capability' do
              post_with_params

              expect(response).to have_gitlab_http_status(:created)
              expect(json_response['server_capabilities']).to eq(['advanced_search'])
            end
          end
        end
        ...
        context 'when DWS returns capabilities' do
          ...
          it 'returns union of Rails and DWS capabilities' do
            ...
            expect(json_response['server_capabilities']).to match_array(
              %w[advanced_search tool_call_approval approve_for_session]
            )
          end

          it 'returns DWS capabilities when Rails has none' do
            post_with_params

            expect(response).to have_gitlab_http_status(:created)
            expect(json_response['server_capabilities']).to match_array(
              %w[tool_call_approval approve_for_session]
            )
          end
        end
        ...
        context 'when tool_approval_for_session is disabled' do
          ...
          it 'filters out tool_call_approval from capabilities' do
            ...
            expect(response).to have_gitlab_http_status(:created)
            expect(json_response['server_capabilities']).to match_array(%w[approve_for_session])
          end
        end
        ...
        context 'when DWS supports capabilities Rails does not' do
          ...
          it 'returns union including all DWS capabilities' do
            ...
            expect(response).to have_gitlab_http_status(:created)
            expect(json_response['server_capabilities']).to match_array(
              %w[advanced_search tool_call_approval dws_only_feature]
            )
          end
        end
      end
  ...
  describe 'GET /ai/duo_workflows/ws' do
    ...
    context 'when user is authenticated' do
      ...
      context 'when knowledge_graph feature flag is enabled' do
        let(:orbit_tools) { ::API::Orbit::McpHandlers::CallTool::KNOWN_TOOLS }
      ...
      context 'when the request token is already scoped for AI workflows (dap_skip_already_scoped_token_creation)' do
        let_it_be(:ai_workflow_scopes) { ::Gitlab::Auth::AI_WORKFLOW_SCOPES + [::Gitlab::Auth::MCP_SCOPE] }
      ...
      context 'for ServerCapabilities' do
        context 'when advanced search is enabled for the project' do
          ...
          it 'returns advanced_search capability' do
            ...
            expect(response).to have_gitlab_http_status(:ok)
            expect(json_response['DuoWorkflow']['ServerCapabilities']).to match_array(%w[advanced_search])
          end
        end
        ...
        context 'when advanced search is disabled' do
          ...
          it 'returns empty server capabilities' do
            get api(path, user), headers: workhorse_headers, params: { project_id: project.id }
