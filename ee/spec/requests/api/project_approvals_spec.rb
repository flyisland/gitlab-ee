...
RSpec.describe API::ProjectApprovals, :aggregate_failures, feature_category: :source_code_management do
  let_it_be(:group, freeze: false)    { create(:group_with_members) }
  let_it_be(:user, freeze: false)     { create(:user) }
  let_it_be(:user2, freeze: false)    { create(:user) }
  let_it_be(:admin, freeze: false)    { create(:user, :admin) }
  let_it_be(:project, freeze: false) { create(:project, :public, :repository, creator: user, namespace: user.namespace, only_allow_merge_if_pipeline_succeeds: false) }
  let_it_be(:approver, freeze: false) { create(:user) }
  let_it_be(:auditor, freeze: false)  { create(:user, :auditor) }
  ...
  describe 'GET /projects/:id/approvals' do
    ...
    context 'when project is archived' do
      let_it_be(:archived_project, freeze: false) { create(:project, :archived, creator: user) }
