...
RSpec.describe API::Releases, feature_category: :release_orchestration do
  ...
  let_it_be_with_reload(:project) do
    create(:project, :repository, :private,
      maintainers: maintainer, reporters: reporter, guests: guest, developers: developer)
  end
  ...
  shared_context 'with a group-scoped project' do
    let(:project) { create(:project, :repository, group: group, creator: maintainer, maintainers: maintainer) }
  ...
  describe 'POST /projects/:id/releases' do
    ...
    context 'updates the AuditEvent' do
      subject(:audit_event_details) { AuditEventReader.last.details }
      ...
      it 'without milestone' do
        ...
        expect(audit_event_details).to include({
    ...
    context 'with a group milestone' do
      include_context 'with a group-scoped project'
  ...
  describe 'PUT /projects/:id/releases/:tag_name' do
    ...
    it 'creates an AuditEvent when a release is updated' do
      ...
      expect(AuditEventReader.last.details[:custom_message]).to eq("Updated release #{release.tag}")
    end
    ...
    shared_examples 'update with milestones' do
      it do
        ...
        end.to change { AuditEventReader.count }.by(2)

        release = project.releases.last

        expect(AuditEventReader.first.details[:custom_message]).to eq("Updated release #{release.tag}")
        expect(AuditEventReader.second.details[:custom_message]).to eq(milestone_message)
      end
    end
    ...
    context 'with milestones' do
      context 'no existing milestones' do
        ...
        context 'add multiple milestones' do
          let_it_be(:milestone2) { create(:milestone, project: project, title: 'v2.0') }
      ...
      context 'existing milestone' do
        let_it_be(:existing_milestone) { create(:milestone, project: project, title: 'v0.1') }
        let_it_be(:milestone) { create(:milestone, project: project, title: 'v1.0') }
