...
RSpec.describe API::ElasticsearchIndexedNamespaces, :aggregate_failures, feature_category: :global_search do
  ...
  describe 'PUT /elasticsearch_indexed_namespaces/rollout' do
    ...
    it_behaves_like 'authorizing granular token permissions', :update_elasticsearch_indexed_namespace do
      let(:boundary_object) { :instance }
      let(:user) { admin }
      let(:request) { put api(path, personal_access_token: pat), params: { plan: 'ultimate', percentage: 50 } }
    end
