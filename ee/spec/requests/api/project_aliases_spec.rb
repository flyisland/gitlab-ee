...
RSpec.describe API::ProjectAliases, :aggregate_failures, :api, feature_category: :source_code_management do
  ...
  context 'without premium license' do
    ...
    describe 'GET /project_aliases/:name' do
      let_it_be(:project_alias) { create(:project_alias) }
  ...
  context 'with premium license' do
    ...
    describe 'GET /project_aliases' do
      ...
      context 'admin' do
        let(:user) { admin }
        let_it_be(:project_alias_1) { create(:project_alias) }
        let_it_be(:project_alias_2) { create(:project_alias) }
        ...
        it_behaves_like 'authorizing granular token permissions', :read_project_alias do
    ...
    describe 'POST /project_aliases' do
      ...
      context 'admin' do
        ...
        it_behaves_like 'authorizing granular token permissions', :create_project_alias do
          let(:boundary_object) { :instance }
          let(:request) do
            post api(path, personal_access_token: pat), params: { project_id: project.id, name: 'some-alias-project' }
          end
        end
      end
    end
    ...
    describe 'DELETE /project_aliases/:name' do
      ...
      context 'admin' do
        ...
        it_behaves_like 'authorizing granular token permissions', :delete_project_alias do
          let_it_be(:alias_for_deletion) { create(:project_alias) }
          let(:boundary_object) { :instance }
          let(:request) do
            delete api("/project_aliases/#{alias_for_deletion.name}", personal_access_token: pat)
          end
        end
      end
    end
