...
RSpec.describe API::License, :aggregate_failures, :api, feature_category: :plan_provisioning do
  ...
  describe 'GET /license' do
    ...
    it_behaves_like 'authorizing granular token permissions', :read_license do
      let(:boundary_object) { :instance }
      let(:user) { admin }
      let(:request) do
        get api('/license', personal_access_token: pat)
      end
    end
    ...
    context 'when logged in as admin' do
      context 'when current license is not an offline cloud license' do
        ...
        it_behaves_like 'authorizing granular token permissions', :read_license do
          ...
          let(:user) { admin }
          let(:request) do
            get api('/license/usage_export', personal_access_token: pat)
          end
        end
  ...
  describe 'POST /license' do
    ...
    it_behaves_like 'authorizing granular token permissions', :create_license do
      let(:boundary_object) { :instance }
      let(:user) { admin }
      let(:request) do
        post api('/license', personal_access_token: pat), params: { license: gl_license.export }
      end
    end
  ...
  describe 'DELETE /license/:id' do
    ...
    it_behaves_like 'authorizing granular token permissions', :delete_license do
      let(:boundary_object) { :instance }
      let(:user) { admin }
      let(:request) do
        delete api(endpoint, personal_access_token: pat)
      end
    end
  ...
  describe 'PUT /license/:id/refresh_billable_users' do
    ...
    it_behaves_like 'authorizing granular token permissions', :refresh_license_billable_user do
      let(:boundary_object) { :instance }
      let(:user) { admin }
      let(:request) do
        put api(endpoint, personal_access_token: pat)
      end
    end
