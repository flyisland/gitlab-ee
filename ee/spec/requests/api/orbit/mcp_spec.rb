  ...
  describe 'POST /orbit/mcp' do
    ...
    context 'when knowledge_graph feature flag is enabled' do
      ...
      describe 'tools/list' do
        ...
        it 'returns legacy tools from gRPC when command tools are disabled' do
          ...
          expect(response).to have_gitlab_http_status(:ok)
          expect(json_response['result']['tools'].pluck('name'))
            .to eq(%w[query_graph get_graph_schema])
          expect(json_response['result']['tools'].first['inputSchema']).to be_present
        end

        it 'returns command tools from gRPC when command tools are enabled' do
      ...
      describe 'tools/call' do
        ...
        it 'returns result from gRPC on success for get_graph_schema' do
          ...
          allow(grpc_client).to receive(:get_graph_schema)
            .with(hash_including(expand_nodes: [], format: :llm, user: user))
        ...
        it 'extracts X-Duo-Workflow-Session-Id header and passes to Workhorse for query_graph' do
          allow(Gitlab::Workhorse).to receive(:send_orbit_query)
        ...
        it 'extracts X-Duo-Workflow-Session-Id header and passes to gRPC for get_graph_schema' do
          allow(grpc_client).to receive(:get_graph_schema)
            .with(hash_including(request_context: having_attributes(session_id: 'workflow-xyz-789')))
            .and_return({ 'nodes' => [], 'edges' => [] })
        ...
        it 'forwards User-Agent to Workhorse for query_graph' do
          allow(Gitlab::Workhorse).to receive(:send_orbit_query)
        ...
        it 'forwards User-Agent to gRPC for get_graph_schema' do
          allow(grpc_client).to receive(:get_graph_schema)
            .with(hash_including(request_context: having_attributes(user_agent: 'TestAgent/1.0')))
            .and_return({ 'nodes' => [], 'edges' => [] })
