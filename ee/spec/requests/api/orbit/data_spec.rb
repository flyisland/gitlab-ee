...
RSpec.describe API::Orbit::Data, :clean_gitlab_redis_rate_limiting, feature_category: :knowledge_graph do
  ...
  describe 'GET /orbit/schema/dsl' do
    it_behaves_like 'an Orbit schema command endpoint', '/orbit/schema/dsl', 'get_query_dsl'
  end
  ...
  describe 'GET /orbit/tools' do
    ...
    context 'when authenticated' do
      it 'returns legacy direct tools when command tools are disabled' do
        ...
        expect(json_response.pluck('name')).to eq(%w[query_graph get_graph_schema])
      end

      it 'returns command wrapper tools when command tools are enabled' do
