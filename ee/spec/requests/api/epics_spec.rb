...
RSpec.describe API::Epics, :aggregate_failures, feature_category: :portfolio_management do
  ...
  describe 'GET /groups/:id/epics' do
    ...
    context 'when the request is correct' do
      ...
      it 'includes the correct imported state' do
        expect(json_response.first['imported']).to be(false)
    ...
    context 'with multiple epics' do
      ...
      let_it_be(:label) { create(:group_label, title: 'a-test', group: group) }
      let_it_be(:label_link) { create(:label_link, label: label, target: epic2) }
    ...
    context 'filtering before a specific date' do
      let_it_be(:epic) do
        create(:epic, group: group, created_at: Date.new(2000, 1, 1), updated_at: Date.new(2000, 1, 1))
      end
    ...
    context 'filtering after a specific date' do
      let_it_be(:epic) { create(:epic, group: group, created_at: 1.week.from_now, updated_at: 1.week.from_now) }
    ...
    context 'with hierarchy params' do
      ...
      let_it_be(:subgroup2) { create(:group, parent: subgroup) }
    ...
    context 'with pagination params' do
      let_it_be(:epic1) { create(:epic, group: group, created_at: 3.days.ago) }
      let_it_be(:epic2) { create(:epic, group: group, created_at: 2.days.ago) }
      let_it_be(:epic3) { create(:epic, group: group, created_at: 1.day.ago) }
      let(:page) { 1 }
      let(:per_page) { 2 }
    ...
    context 'when epics feature is enabled' do
      ...
      context 'when the request is correct' do
        ...
        it 'create system notes for new relation' do
          ...
          expect(new_epic.notes.where(note: "added #{parent_epic.work_item.to_reference} as parent item").count).to eq(1)
          expect(parent_epic.notes.where(note: "added #{new_epic.work_item.to_reference} as child item").count).to eq(1)
        end
        ...
        it 'creates a new epic' do
          ...
          expect(epic.start_date_fixed).to be_nil
          expect(epic.start_date_is_fixed).to be(true)
          expect(epic.due_date).to eq(Date.new(2018, 7, 17))
          expect(epic.due_date_fixed).to eq(Date.new(2018, 7, 17))
          expect(epic.due_date_is_fixed).to be(true)
      ...
      context 'when the request is correct' do
        ...
        context 'with basic params' do
          ...
          it 'updates the epic' do
            ...
            expect(result.start_date_is_fixed).to be(true)
            expect(result.due_date_fixed).to be_nil
            expect(result.due_date_is_fixed).to be(true)
        ...
        context 'when updating start_date_is_fixed by itself' do
          ...
          it 'updates start_date_is_fixed' do
            ...
            expect(result.start_date_is_fixed).to be(false)
          end
        end
  ...
  describe 'DELETE /groups/:id/epics/:epic_iid' do
    ...
    context 'when epics feature is enabled' do
      ...
      context 'when the request is correct' do
        before_all { group.add_owner(user) }
