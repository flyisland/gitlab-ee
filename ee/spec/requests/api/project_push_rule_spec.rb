...
RSpec.describe API::ProjectPushRule, 'ProjectPushRule', :api, feature_category: :source_code_management do
  ...
  describe 'GET /projects/:id/push_rule' do
    ...
    context 'when the current user is a maintainer' do
        ...
        expect(json_response).to eq(
          {
            ...
            "deny_delete_tag" => false,
            "file_name_regex" => nil,
            "id" => project.project_push_rule.id,
            "max_file_size" => 100,
            "member_check" => false,
            "prevent_secrets" => true,
      ...
      context 'with read_project_push_rules FF enabled' do
        it 'returns push rule from project_push_rules table' do
          get_push_rule

          project.reload

          expect(response).to have_gitlab_http_status(:ok)
          expect(json_response['id']).to eq(project.project_push_rule.id)
        end
      end

      context 'with read_project_push_rules FF disabled' do
        before do
          stub_feature_flags(read_project_push_rules: false)
        end

        it 'returns push rule from push_rules table' do
          get_push_rule

          project.reload

          expect(response).to have_gitlab_http_status(:ok)
          expect(json_response['id']).to eq(project.push_rule.id)
        end
      end
    end
  ...
  describe 'POST /projects/:id/push_rule' do
    ...
    context 'when the current user is a maintainer' do
      ...
      it 'uses ProjectPushRuleFinder' do
        expect(ProjectPushRuleFinder).to receive(:new).with(project).and_call_original

        post api("/projects/#{project.id}/push_rule", user), params: attributes
      end
    end
    ...
    context 'when a push rule exists' do
      ...
      context 'when the current user is a maintainer' do
        ...
        it 'uses ProjectPushRuleFinder' do
          ...
          put api("/projects/#{project.id}/push_rule", user), params: attributes
        end
      end
