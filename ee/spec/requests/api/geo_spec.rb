...
RSpec.describe API::Geo, :aggregate_failures, feature_category: :geo_replication do
  ...
  describe 'POST /geo/failures' do
    let(:geo_base_request) { Gitlab::Geo::BaseRequest.new(scope: ::Gitlab::Geo::API_SCOPE) }
    let!(:upload) { create(:upload, :verification_succeeded) }
    ...
    subject(:request) { post api('/geo/failures'), params: data, headers: geo_base_request.headers }
    ...
    it 'responds with 401 with invalid auth header' do
      post api('/geo/failures'), params: data, headers: invalid_geo_auth_header

      expect(response).to have_gitlab_http_status(:unauthorized)
    end

    context 'when requesting node is not the primary' do
      before do
    ...
    context 'when the geo_self_heal_checksum_mismatch feature flag is disabled' do
      before do
        stub_feature_flags(geo_self_heal_checksum_mismatch: false)
      end

      it 'responds with 404 and does not enqueue the report worker' do
        expect(Geo::VerificationFailureReportWorker).not_to receive(:perform_async)
    ...
    context 'when the current node is the primary' do
      it 'responds with 202 and enqueues the report worker' do
        expect(Geo::VerificationFailureReportWorker).to receive(:perform_async)
          .with(secondary_node.id, [a_hash_including('error_type' => 'checksum_mismatch', 'replicable_id' => upload.id)])
      ...
      it 'accepts an unknown error_type without validating it' do
        data[:failures] = [{
          error_type: 'some_future_error_type',
          replicable_name: 'upload',
          replicable_id: upload.id,
      ...
      it 'responds with 400 when there are too many failures' do
        data[:failures] = Array.new(Geo::VerificationFailureReportWorker::MAX_FAILURES_PER_REQUEST + 1) do |i|
          { error_type: 'checksum_mismatch', replicable_name: 'upload', replicable_id: i, verification_retry_count: 1 }
        end
    ...
    context 'when rate limited' do
      before do
        allow(Gitlab::ApplicationRateLimiter).to receive(:throttled?)
          .with(:geo_proxy, scope: ['127.0.0.1']).and_return(true)
      end
  ...
  describe 'POST /geo/node_proxy/:id/graphql' do
    ...
    it_behaves_like 'authorizing granular token permissions', :read_geo_node do
      ...
      let(:request) do
        post api("/geo/node_proxy/#{secondary_node.id}/graphql", personal_access_token: pat),
          params: { input: 'test' }.to_json, headers: headers
      end
    end
  ...
  describe 'GET /geo/repositories/:gl_repository/pipeline_refs' do
    ...
    it 'returns a list of pipelines' do
      ...
      expect(response).to have_gitlab_http_status(:ok)
      # Asserts the full body: passing the options positionally bypassed the entity and leaked
      # a `with` key on Grape 2.4, and returned `null` on Grape 3.x.
      expect(json_response).to eq('pipeline_refs' => pipeline_refs)
    end
  end
end
