...
RSpec.describe API::GroupEnterpriseUsers, :aggregate_failures, feature_category: :user_management do
  ...
  let_it_be(:saml_provider, freeze: false) { create(:saml_provider, group: enterprise_group) }

  let_it_be(:subgroup, freeze: false) { create(:group, parent: enterprise_group) }

  let_it_be(:developer_of_enterprise_group, freeze: false) { create(:user, developer_of: enterprise_group) }
  let_it_be(:maintainer_of_enterprise_group, freeze: false) { create(:user, maintainer_of: enterprise_group) }
  let_it_be(:owner_of_enterprise_group, freeze: false) { create(:user, owner_of: enterprise_group) }
  let_it_be(:auditor_user, freeze: false) { create(:auditor) }

  let_it_be(:non_enterprise_user, freeze: false) { create(:user) }
  let_it_be(:enterprise_user_of_another_group, freeze: false) { create(:enterprise_user) }
  ...
  let_it_be(:blocked_enterprise_user_of_the_group, freeze: false) do
    create(:enterprise_user, :blocked, :with_namespace, enterprise_group: enterprise_group)
  end

  let_it_be(:enterprise_user_and_member_of_the_group, freeze: false) do
  ...
  shared_examples 'authentication and authorization requirements' do |allow_auditor: false|
    ...
    context 'when current_user is an instance auditor' do
      let(:current_user) { auditor_user }
  ...
  describe 'GET /groups/:id/enterprise_users' do
    ...
    include_examples 'authentication and authorization requirements', allow_auditor: true
    ...
    context 'for search parameter' do
      ...
      context 'for search by public email' do
        let_it_be(:enterprise_user_of_the_group_with_public_email, freeze: false) do
      ...
      context 'for search by private email' do
        let_it_be(:enterprise_user_of_the_group_without_public_email, freeze: false) do
    ...
    context 'for created_after parameter' do
      ...
      let_it_be(:enterprise_user_of_the_group_created_12_days_ago, freeze: false) do
        create(:enterprise_user, :with_namespace, enterprise_group: enterprise_group).tap do |user|
          user.update_column(:created_at, 12.days.ago)
        end
      end

      let_it_be(:enterprise_user_of_the_group_created_8_days_ago, freeze: false) do
        create(:enterprise_user, :with_namespace, enterprise_group: enterprise_group).tap do |user|
    ...
    context 'for two_factor parameter' do
      let_it_be(:enterprise_user_of_the_group_with_two_factor_enabled, freeze: false) do
  ...
  describe 'PATCH /groups/:id/enterprise_users/:user_id', :saas do
    ...
    context 'for email update' do
      ...
      let_it_be(:project, freeze: false) { create(:project, group: enterprise_group) }
      let_it_be(:enterprise_group_verified_domain, freeze: false) { create(:pages_domain, project: project) }

      context 'when specified email address is from a verified domain of the group' do
        let_it_be(:email, freeze: false) { "new-email@#{enterprise_group_verified_domain.domain}" }
        ...
        context 'when specified email address is already taken by another user' do
          let_it_be(:existing_user_with_specified_email, freeze: false) do
        ...
        context 'when specified email address is invalid' do
          let_it_be(:email, freeze: false) { "invalid email@#{enterprise_group_verified_domain.domain}" }
      ...
      context 'when specified email address is not from a verified domain of the group' do
        let_it_be(:email, freeze: false) { "new-email@example.com" }
