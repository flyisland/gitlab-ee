...
RSpec.describe 'User with manage_group_access_tokens custom role', feature_category: :permissions do
  ...
  describe Groups::Settings::AccessTokensController do
    let_it_be(:role, freeze: false) { create(:member_role, :guest, namespace: group, manage_group_access_tokens: true) }
    let_it_be(:member, freeze: false) { create(:group_member, :guest, member_role: role, user: user, group: group) }
    ...
    describe '#revoke' do
      let_it_be(:access_token_user, freeze: false) { create(:user, :project_bot) }
      let_it_be(:group_member, freeze: false) { create(:group_member, user: access_token_user, group: group) }
