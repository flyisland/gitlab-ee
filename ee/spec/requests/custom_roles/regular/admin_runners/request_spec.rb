...
RSpec.describe "User with admin_runners custom role", feature_category: :runner_core do
  ...
  let_it_be(:role, freeze: false) { create(:member_role, :guest, :admin_runners, namespace: project.group) }
  ...
  describe API::Ci::Runners do
    ...
    let_it_be(:group_runner, freeze: false) { create(:ci_runner, :group, description: 'Group runner', groups: [group]) }
    ...
    describe 'POST /projects/:id/runners', :request_store do
      ...
      let_it_be(:runner_in_private_project, freeze: false) do
  ...
  describe API::UserRunners do
    ...
    context 'without the admin_runners permission' do
      let_it_be(:role, freeze: false) { create(:member_role, :guest, namespace: group) }
      let_it_be(:membership, freeze: false) do
        create(:group_member, :guest, member_role: role, user: user, source: group)
      end
  ...
  describe Mutations::Ci::Runner::BulkDelete do
    ...
    let_it_be(:runners, freeze: false) { create_list(:ci_runner, 2, :project, projects: [project]) }
