...
RSpec.describe 'User with manage_protected_tags custom role', feature_category: :source_code_management do
  ...
  let_it_be(:protected_tag, freeze: false) { create(:protected_tag, project: project, name: 'v1.0.0') }

  let_it_be(:user_with_permission, freeze: false) { create(:user) }
  let_it_be(:user_without_permission, freeze: false) { create(:user) }

  let_it_be(:role_with_permission, freeze: false) do
    create(:member_role, :developer, read_code: true, namespace: project.group, manage_protected_tags: true)
  end

  let_it_be(:role_without_permission, freeze: false) do
    create(:member_role, :developer, read_code: true, namespace: project.group, manage_protected_tags: false)
  end

  let_it_be(:member_with_permission, freeze: false) do
    create(:group_member, :developer, member_role: role_with_permission, user: user_with_permission,
      source: project.group)
  end

  let_it_be(:member_without_permission, freeze: false) do
