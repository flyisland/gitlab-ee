...
RSpec.describe 'User with read_virtual_registry custom role', :api, feature_category: :virtual_registry do
  ...
  let_it_be(:role) { create(:member_role, :minimal_access, read_virtual_registry: true, namespace: group) }
  let_it_be(:membership) { create(:group_member, :minimal_access, member_role: role, user: user, source: group) }
