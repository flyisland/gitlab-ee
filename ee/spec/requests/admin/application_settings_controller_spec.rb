    ...
    describe 'semantic search settings' do
      ...
      shared_examples 'model configuration is unavailable' do
        it 'shows model configuration is unavailable' do
          ...
          expect(response.body).to have_css(
            "[data-testid='semantic-search-model-#{collection_key}']",
            text: /Embedding model configuration is unavailable - #{expected_unavailable_message}/
          )
      ...
      context 'when there is no ActiveContext connection' do
        it_behaves_like 'model configuration is unavailable' do
          let(:expected_unavailable_message) { "Connection not configured" }
        end
      end
      ...
      context 'when there is an ActiveContext connection' do
        ...
        context 'when collection record is not present' do
          it_behaves_like 'model configuration is unavailable' do
            let(:expected_unavailable_message) { "Collection record not present" }
          end
        end

        context 'when collection record is present' do
          ...
          context 'when gitlab selects the embedding model' do
            before do
              allow(::Ai::ActiveContext::Embeddings::ModelSelector).to receive_messages(
                use_gitlab_selected_model?: true,
          ...
          context 'when user selects the embedding model' do
            before do
              allow(::Ai::ActiveContext::Embeddings::ModelSelector).to receive_messages(
                use_gitlab_selected_model?: false,
