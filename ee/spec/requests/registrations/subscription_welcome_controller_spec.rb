...
RSpec.describe Registrations::SubscriptionWelcomeController, :with_current_organization, feature_category: :onboarding do
  ...
  before do
    ...
    stub_feature_flags(subscription_com_unification: unification_enabled?)
  end
  ...
  describe 'PUT update' do
    ...
    context 'with a signed in user' do
      ...
      context 'when onboarding feature is available' do
        context 'when unification is disabled' do
          ...
          context 'when it is a failed request' do
            ...
            it 'tracks failed submission event' do
              patch_update
      ...
      context 'when unification is enabled' do
        ...
        let(:unification_update_params) do
          {
            first_name: 'Test',
            last_name: 'User',
        ...
        subject(:patch_update_unification) do
          put users_sign_up_welcome_path, params: unification_update_params
          response
        end
        ...
        context 'when namespace creation is successful' do
          ...
          it 'redirects to the customers portal redirect path' do
            expect(patch_update_unification).to redirect_to(users_sign_up_customers_portal_redirect_path)
          end

          it 'sets the namespace id in the user return to session', :clean_gitlab_redis_sessions do
            stub_session(session_data: { user_return_to: new_subscriptions_path })
            patch_update_unification
        ...
        context 'when not found' do
          ...
          it 'returns not found' do
            expect(patch_update_unification).to have_gitlab_http_status(:not_found)
          end
        end
      end
    end
