  ...
  describe 'new policy button' do
    ...
    context 'when the dependency_firewall_phase2 feature flag is disabled' do
      before do
        ...
        allow(Ability).to receive(:allowed?).with(current_user, :create_security_policy, group).and_return(true)
      end
    ...
    context 'when the user cannot create security policies' do
      before do
        allow(Ability).to receive(:allowed?).with(current_user, :create_security_policy, group).and_return(false)
      end
