...
RSpec.describe Groups::Settings::GitlabDuo::ConfigurationController, type: :request, feature_category: :ai_abstraction_layer do
  ...
  describe 'GET index' do
    ...
    context 'when show_gitlab_duo_settings_menu_item? returns true', :saas do
      ...
      it 'pushes the duo_code_review_dap_routing_consent_enabled feature flag' do
        stub_feature_flags(duo_code_review_dap_routing_consent_enabled: group)

        get_index

        expect(response.body).to have_pushed_frontend_feature_flags(duoCodeReviewDapRoutingConsentEnabled: true)
      end
    end
