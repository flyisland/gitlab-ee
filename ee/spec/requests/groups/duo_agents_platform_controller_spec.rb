...
RSpec.describe 'Groups::DuoAgentsPlatform', feature_category: :workflow_catalog do
  ...
  describe 'GET /:group/-/automate' do
    ...
    context 'when ai_catalog_public_explore is disabled and user does not have access to duo_workflow' do
      before do
    ...
    context 'when ai_catalog_public_explore is enabled and user does not have access to duo_workflow' do
      before do
        allow(Ability).to receive(:allowed?).with(user, :duo_workflow, group).and_return(false)
      end
