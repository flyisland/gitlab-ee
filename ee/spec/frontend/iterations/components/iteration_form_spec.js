...
describe('Iteration Form', () => {
  ...
  const findDescription = () => wrapper.findByTestId('iteration-description-field');
  const findStartDate = () => wrapper.findComponentByTestId('start-date');
  const findStartDateInputText = () => findStartDate().findComponent(GlFormInput).element.value;
  const findDueDate = () => wrapper.findComponentByTestId('due-date');
  const findDueDateInputText = () => findDueDate().findComponent(GlFormInput).element.value;
  const findSaveButton = () => wrapper.findComponentByTestId('save-iteration');
