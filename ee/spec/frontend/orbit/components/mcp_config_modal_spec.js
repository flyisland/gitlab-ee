...
describe('McpConfigModal', () => {
  ...
  const findCliCopyButton = () => wrapper.findByTestId('copy-cli-command-btn');
  const findManualCopyButton = () => wrapper.findByTestId('copy-manual-config-btn');
  const findToolCopyButtons = () => wrapper.findAllByTestId('copy-tool-name-btn');
  const findHostListbox = () => wrapper.findComponent(GlCollapsibleListbox);
  const findManualTabButton = () => wrapper.findByTestId('manual-config-tab-btn');
  ...
  describe('CLI command copy button', () => {
    ...
    it.each([
      ...
      ['cursor', `npx -y mcp-remote \\\n   --url ${MCP_ENDPOINT} \\\n   --client cursor`],
      ['codex', `codex mcp add \\\n   --url ${MCP_ENDPOINT} orbit`],
