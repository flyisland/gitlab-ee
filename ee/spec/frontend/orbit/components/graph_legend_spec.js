...
describe('GraphLegend', () => {
  ...
  const findToggle = () => wrapper.findByTestId('toggle-legend-btn');
  ...
  describe('counts', () => {
    it('renders a badge for items with non-zero counts', () => {
      createWrapper({ items: [items[0]] });

      expect(wrapper.findComponent(GlBadge).exists()).toBe(true);
      expect(wrapper.findComponent(GlBadge).props('variant')).toBe('neutral');
    });

    it('does not render a badge when count is 0 or null', () => {
