import { createStraightLine, createArcCurve, createImpulse } from 'ee/orbit/utils/three_edges';
...
describe('three_edges shared primitives', () => {
  ...
  describe('createImpulse', () => {
    const buildConnection = () => ({
      curve: { getPoint: jest.fn(() => ({ x: 0, y: 0, z: 0 })) },
    });
    const renderer = { getPixelRatio: jest.fn(() => 2) };

    it('returns a particle with mesh, connection and direction', () => {
      const connection = buildConnection();
      const impulse = createImpulse(connection, renderer);

      expect(impulse.mesh).toBeDefined();
      expect(impulse.connection).toBe(connection);
      expect([1, -1]).toContain(impulse.direction);
      expect(typeof impulse.linearProgress).toBe('number');
      expect(impulse.baseSpeed).toBeGreaterThan(0);
    });

    it('honors darkMode by passing the renderer pixel ratio into the shader uniforms', () => {
      const connection = buildConnection();
      createImpulse(connection, renderer, true);

      expect(renderer.getPixelRatio).toHaveBeenCalled();
    });

    it('defaults to dark mode when no flag is passed', () => {
      const connection = buildConnection();

      expect(() => createImpulse(connection, renderer)).not.toThrow();
    });
  });
});
