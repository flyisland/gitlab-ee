...
describe('AgenticDuoChatView', () => {
  ...
  describe('rendering', () => {
    ...
    describe('session pills bar', () => {
      ...
      it('does not render the session pills bar when showSessionPills is false', () => {
        createComponent({ propsData: { showSessionPills: false } });

        expect(findSessionPillsBar().exists()).toBe(false);
      });

      it('renders the session pills bar when showSessionPills is true', () => {
        createComponent({ propsData: { showSessionPills: true, messages: assistantMessages } });
      ...
      it('keeps the disclaimer above the form when showSessionPills is false', () => {
        createComponent({ propsData: { showSessionPills: false, messages: assistantMessages } });

        expect(siblingIndex(findDisclaimer().element)).toBeLessThan(
          siblingIndex(findPromptForm().element),
        );
      });

      it('moves the disclaimer below the form when showSessionPills is true', () => {
        createComponent({ propsData: { showSessionPills: true, messages: assistantMessages } });
  ...
  describe('chat', () => {
    ...
    describe('submit', () => {
      ...
      it('restores focus to prompt when isLoading becomes false after sending with focus', async () => {
        ...
        // Simulate the parent's loading cycle: the watcher's restore-focus branch
        // fires on the true -> false transition, so we need to drive the prop
        // through both states.
      ...
      it('does not restore focus when isLoading becomes false if input was not focused before send', async () => {
        createComponent({
          propsData: { isLoading: true, isChatAvailable: true, messages },
          mountFn: mountExtended,
        });
      ...
      it('restores focus to prompt when isStreaming becomes false after sending with focus', async () => {
        ...
        await wrapper.setProps({ messages: [...messages, streamingMessage] });
        await wrapper.setProps({ messages: [...messages, completedMessage] });
        await nextTick();
    ...
    describe('when chat has a content-items-menu component in slot', () => {
      ...
      it.each(['/include', '/incl'])(
        ...
        async (command) => {
          findChatInputNative().element.value = command;
          await findChatInputNative().trigger('input');
          await nextTick();

          findChatInputNative().trigger('keyup', { key: 'Enter' });
          await nextTick();
          await nextTick();
      ...
      it('opens context menu when manually typing full "/include" command', async () => {
        findChatInputNative().element.value = '/include';
        await findChatInputNative().trigger('input');
        await nextTick();

        findChatInputNative().trigger('keyup', { key: '' });
        await nextTick();
      ...
      describe('when the context menu is open', () => {
        ...
        it.each(['/', '', 'how does bread?'])(
          ...
          async (newPrompt) => {
            findChatInputNative().element.value = newPrompt;
            await findChatInputNative().trigger('input');
            findChatInputNative().trigger('keyup', { key: 'ArrowRight' });
            await nextTick();
        ...
        it('passes keyboard events to component ref when the menu is open', () => {
          findChatInputNative().trigger('keyup', { key: 'ArrowDown' });
          expect(mockContextItemMenuHandleKeyUp).toHaveBeenCalledWith(
            expect.objectContaining({ key: 'ArrowDown' }),
          );

          findChatInputNative().trigger('keyup', { key: 'ArrowUp' });
          expect(mockContextItemMenuHandleKeyUp).toHaveBeenCalledWith(
            expect.objectContaining({ key: 'ArrowUp' }),
          );

          findChatInputNative().trigger('keyup', { key: 'Z' });
          expect(mockContextItemMenuHandleKeyUp).toHaveBeenCalledWith(
