...
describe('Duo Agentic Chat State Manager', () => {
  ...
  describe('events handling', () => {
    ...
    describe('@new-chat', () => {
      ...
      it('clears archived thread state when starting a new chat', async () => {
        ...
        expect(findDuoChat().vm.$scopedSlots['custom-empty-state']({})).toBeDefined();

        findDuoChat().vm.$emit('new-chat');
        await nextTick();

        expect(findDuoChat().vm.$scopedSlots['custom-empty-state']).toBeUndefined();
      });
    });
  ...
  describe('Multithreading features', () => {
    ...
    describe('@thread-selected', () => {
      ...
      it('shows archived empty state and disables chat when selecting an archived thread', async () => {
        const mockThread = { id: MOCK_WORKFLOW_ID, archived: true };
      ...
      it('does not call clearActiveWorkflow when selecting an archived thread', async () => {
        const clearActiveWorkflowSpy = jest.spyOn(wrapper.vm, 'clearActiveWorkflow');
        const mockThread = { id: MOCK_WORKFLOW_ID, archived: true };
      ...
      it('renders ThreadArchivedEmptyState in custom-empty-state slot when thread is archived', async () => {
        const mockThread = { id: MOCK_WORKFLOW_ID, archived: true };
      ...
      it('clears the workflowId when the selected thread is stalled', async () => {
        WorkflowUtils.parseWorkflowData.mockReturnValue(null);
        ...
        expect(findDuoChat().props('activeThreadId')).toBeNull();
      });
    });
    ...
    describe('@back-to-list', () => {
      ...
      it('clears archived thread state when going back to list', async () => {
        findDuoChat().vm.$emit('thread-selected', {
          id: MOCK_WORKFLOW_ID,
          archived: true,
        });
