...
describe('Duo Agentic Chat State Manager', () => {
  ...
  describe('events handling', () => {
    ...
    describe('@send-chat-prompt', () => {
      ...
      it('replaces a caller-supplied orbit_context envelope with the authoritative value when a foundational agent is selected', async () => {
        ...
        wrapper.vm.startWorkflow(MOCK_USER_MESSAGE.content, {}, [orbitContextEnvelope(false)]);
        await waitForPromises();
  ...
  describe('workflowId watcher', () => {
    ...
    it('emits session-id-changed on mount when workflowId exists', async () => {
      getSessionStorageValue.mockReset();
      getSessionStorageValue.mockReturnValueOnce({
        exists: true,
        value: { workflowId: MOCK_WORKFLOW_ID },
      });
  ...
  describe('Global state watchers', () => {
    describe('duoChatGlobalState.isAgenticChatShown', () => {
      describe('when there is a workflowId and activeThread registered in sessionStorage', () => {
        beforeEach(() => {
          getSessionStorageValue.mockReset();
          getSessionStorageValue.mockReturnValueOnce({
            exists: true,
      ...
      describe('when there is no workflowId or activeThread registered in sessionStorage', () => {
        beforeEach(() => {
          getSessionStorageValue.mockReset();
          getSessionStorageValue.mockReturnValueOnce({
            exists: false,
    ...
    describe('@thread-selected', () => {
      ...
      describe.each([
        ...
        it('renders ThreadInactiveEmptyState in custom-empty-state slot', async () => {
          ...
          const customEmptyState = findDuoChat().vm.$scopedSlots['custom-empty-state']({});
