...
describe('deduplicateMcpServers', () => {
  ...
  it('handles an agent with no latestVersion gracefully', () => {
    const agentWithoutVersion = {
      id: 'gid://gitlab/Ai::Catalog::Item/3',
      name: 'Bare Agent',
      latestVersion: null,
    };
    expect(() => deduplicateMcpServers(buildResponse(agentWithoutVersion))).not.toThrow();
    expect(deduplicateMcpServers(buildResponse(agentWithoutVersion))).toEqual([]);
  });
});
