...
describe('AiItemsIndex', () => {
  ...
  describe.each(TYPE_CONFIGS)('with $label itemType', (config) => {
    ...
    const findManagedList = () => wrapper.findByTestId(config.managedTestId);
    const findCatalogList = () => wrapper.findByTestId(config.catalogTestId);
    ...
    describe('Managed tab', () => {
      ...
      describe('search', () => {
        ...
        it('refetches with search term when search is submitted', async () => {
          findManagedList().vm.$emit('search', ['test item']);
          await nextTick();
        ...
        it('refetches with empty search when clear-search is emitted', async () => {
          findManagedList().vm.$emit('search', ['test item']);
          await nextTick();
        ...
        it('preserves search term when switching tabs', async () => {
          findManagedList().vm.$emit('search', ['persisted search']);
          await nextTick();
    ...
    describe('tracking', () => {
      describe('Managed tab', () => {
        ...
        it(`does not re-track ${TRACK_EVENT_VIEW_AI_CATALOG_PROJECT_MANAGED} when already active`, () => {
          createComponent();
          findTabs().vm.$emit('input', 1);
          const { trackEventSpy } = bindInternalEventDocument(wrapper.element);
          findAllTabs().at(1).vm.$emit('click');
      ...
      describe('Catalog tab', () => {
        ...
        it(`does not re-track ${TRACK_EVENT_VIEW_AI_CATALOG_PROJECT_CATALOG} when already active`, () => {
          createComponent();
          findTabs().vm.$emit('input', 2);
          const { trackEventSpy } = bindInternalEventDocument(wrapper.element);
          findAllTabs().at(2).vm.$emit('click');
