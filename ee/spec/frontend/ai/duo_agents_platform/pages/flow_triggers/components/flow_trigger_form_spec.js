...
describe('FlowTriggerForm', () => {
  ...
  const createWrapper = async (props = {}, provide = {}) => {
    ...
    wrapper = shallowMountExtended(FlowTriggerForm, {
      ...
      provide: {
        ...
        glFeatures: {
          mergeRequestReadyFlowTrigger: true,
        },
        ...provide,
      },
      stubs: {
  ...
  describe('Event Type Selection', () => {
    ...
    it('includes merge_request_ready when feature flag is enabled', () => {
      const options = findEventTypes().props('items');
      expect(options.map((o) => o.value)).toContain(FLOW_TRIGGER_TYPE_MERGE_REQUEST_READY.valueInt);
    });

    describe('when mergeRequestReadyFlowTrigger feature flag is disabled', () => {
      beforeEach(async () => {
        await createWrapper({}, { glFeatures: { mergeRequestReadyFlowTrigger: false } });
      });

      it('excludes merge_request_ready from event type options', () => {
        const options = findEventTypes().props('items');
        expect(options.map((o) => o.value)).not.toContain(
          FLOW_TRIGGER_TYPE_MERGE_REQUEST_READY.valueInt,
        );
      });

      it('still includes other event types', () => {
        const options = findEventTypes().props('items');
        expect(options.map((o) => o.value)).toContain(FLOW_TRIGGER_TYPES[0].valueInt);
        expect(options.map((o) => o.value)).toContain(FLOW_TRIGGER_TYPES[1].valueInt);
      });
    });
  });
