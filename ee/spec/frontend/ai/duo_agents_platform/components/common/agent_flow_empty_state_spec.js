...
describe('AgentFlowEmptyState', () => {
  ...
  describe('within retention period', () => {
    ...
    describe('session failed state', () => {
      ...
      it('does not render a failed state AgentFlowSessionMessage', () => {
  ...
  describe('with useChatBubbles enabled', () => {
    describe('when creating session', () => {
      beforeEach(() => {
        createComponent({ useChatBubbles: true, hasLogs: false, status: '' });
      });

      it('renders DuoChatConversation instead of session messages', () => {
        expect(findChatConversation().exists()).toBe(true);
        expect(findSessionMessages()).toHaveLength(0);
      });
    ...
    describe('when session is created with logs', () => {
      beforeEach(() => {
        createComponent({ useChatBubbles: true, hasLogs: true, status: 'SUCCESS' });
      });
    ...
    describe('when session failed without logs', () => {
      beforeEach(() => {
        createComponent({ useChatBubbles: true, hasLogs: false, status: 'FAILED' });
      });
    ...
    describe('when outside retention period', () => {
      beforeEach(() => {
        const oldDate = new Date();
        oldDate.setDate(oldDate.getDate() - (AGENT_PLATFORM_SESSION_RETENTION_LENGTH + 1));
      ...
      it('shows retention message instead of chat bubbles', () => {
        expect(findRetentionMessage().exists()).toBe(true);
