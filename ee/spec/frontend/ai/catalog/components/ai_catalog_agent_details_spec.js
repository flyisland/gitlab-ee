    ...
    describe('when MCP tools are present', () => {
      ...
      beforeEach(() => {
        createComponent({
          ...
          provide: {
            glFeatures: { mcpCatalogAgentTools: true },
            instanceBetaFeaturesEnabled: true,
          },
        });
      });

      it('renders MCP tools with human-readable titles', () => {
        const tokens = wrapper.findAllComponents(GlToken);
        const tokenTexts = tokens.wrappers.map((t) => t.text().replace(/\s+/g, ' ').trim());
        expect(tokenTexts).toContain('Get Mcp Server Version Experimental');
        expect(tokenTexts).toContain('Semantic Code Search Experimental');
      });

      it('renders experimental label for MCP tools as a plain span, not a badge', () => {
        const toolsField = findAllFieldsForSection(1).at(1);
        expect(toolsField.findComponent(GlBadge).exists()).toBe(false);
        expect(toolsField.text()).toContain('Experimental');
      });
    });
  });
