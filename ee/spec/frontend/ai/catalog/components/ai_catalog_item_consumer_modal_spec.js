...
describe('AiCatalogItemConsumerModal', () => {
  ...
  describe('component rendering', () => {
    ...
    describe('when the item is public', () => {
      ...
      it('does not pass the item id to the project dropdown when the feature flag is off', () => {
        expect(findProjectDropdown().props('itemId')).toBe(null);
      });

      it('passes the item id to the project dropdown when the feature flag is on', () => {
        createWrapper({
          provide: { glFeatures: { aiCatalogFilterEnabledProjects: true } },
        });
  ...
  describe('when submitting the form', () => {
    it('emits the submit event', async () => {
      ...
      expect(wrapper.emitted('submit')[0][0]).toStrictEqual({
        target: { projectId },
        triggerTypes: ['mention', 'assign', 'assign_reviewer', 'merge_request_ready'],
      });
    });
