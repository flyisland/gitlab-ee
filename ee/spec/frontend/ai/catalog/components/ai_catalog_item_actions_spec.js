...
describe('AiCatalogItemActions', () => {
  ...
  `(
    ...
    ({
      ...
      it(`shows ${isEnabled ? 'disabled "Enabled"' : 'active "Enable"'} button`, () => {
        if (!enableBtn) {
          expect(findEnableButton().exists()).toBe(false);
          return;
        }
        expect(findEnableButton().props('disabled')).toBe(isEnabled);
        expect(findEnableButton().text()).toBe(isEnabled ? 'Enabled' : 'Enable');
      });
