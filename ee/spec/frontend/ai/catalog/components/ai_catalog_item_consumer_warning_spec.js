...
describe('AiCatalogItemConsumerWarning', () => {
  ...
  describe('flow warning', () => {
    ...
    it('displays composite identity text and link', () => {
      const text =
        AI_CATALOG_ITEM_CONSUMER_WARNING_TEXTS[AI_CATALOG_TYPE_FLOW].fullWarningCompositeIdentity;
      expect(wrapper.text()).toContain(text);
      expect(findGlLink().props('href')).toBe('/help/user/duo_agent_platform/composite_identity');
      expect(findGlLink().text()).toBe('Learn more about composite identity');
    });
  ...
  describe('third-party flow warning', () => {
    ...
    it('displays composite identity text and link', () => {
      const text =
        AI_CATALOG_ITEM_CONSUMER_WARNING_TEXTS[AI_CATALOG_TYPE_THIRD_PARTY_FLOW]
          .fullWarningCompositeIdentity;
      expect(wrapper.text()).toContain(text);
      expect(findGlLink().props('href')).toBe('/help/user/duo_agent_platform/composite_identity');
      expect(findGlLink().text()).toBe('Learn more about composite identity');
    });
