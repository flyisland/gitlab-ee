      ...
      nodes: [
        {
          ...
          name: 'Test',
          nameWithNamespace: 'group-1/Test',
          __typename: 'Project',
        },
      ],
      pageInfo: {
...
describe('Flow edit page', () => {
  const mountFlowEdit = ({
    ...
    createIntegrationWrapper(AiCatalogItemEdit, {
      provide: {
        ...PROJECT_PROVIDE,
        glAbilities: { adminAiCatalogItem: true },
      },
...
describe('Flow show page', () => {
  const mountFlowShow = ({
    ...
    const { wrapper, router, apolloProvider, toast } = createIntegrationWrapper(AiCatalogItemShow, {
      ...
      stubs: {
        ...
        // The stub adds a confirm button that emits 'primary' (GlModal's primary action event)
        // so ConfirmActionModal's actionFn can be triggered via a DOM click
    ...
    beforeEach(async () => {
      ...
      ({ wrapper, toast } = mountFlowShow({
        ...
        apolloHandlers: [
          ...
          [getProjects, jest.fn().mockResolvedValue(mockGetProjectsResponse)],
        ],
      }));

      await waitForPromises(); // let getProjects resolve and populate the dropdown
      await wrapper.findByRole('button', { name: 'Enable' }).trigger('click');
      await wrapper.findByRole('option', { name: /group-1\/Test/ }).trigger('click');
  ...
  describe('In the Explore area, projects where the flow is already enabled', () => {
    it('with the filter flag on, are fetched via the filter query scoped to the current item', async () => {
      ...
      const { wrapper } = mountFlowShow({
        provide: {
          ...EXPLORE_PROVIDE,
          glFeatures: { aiCatalogFilterEnabledProjects: true },
        },
        apolloHandlers: [
          [aiCatalogFlowQuery, jest.fn().mockResolvedValue(mockAiCatalogFlowResponse)],
