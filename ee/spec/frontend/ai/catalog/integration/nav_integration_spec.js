...
// GlButton with a `to` prop renders RouterLinkStub (a component with non-element root),
// which triggers this Vue 3 warning on mount
// Suppressed this warning as it originates in @gitlab/ui
...
const mountNavTabs = async ({ glAbilities = {}, glFeatures = {} } = {}) => {
  const result = createIntegrationWrapper(AiCatalogNavTabs, {
    provide: { ...EXPLORE_PROVIDE, glAbilities, glFeatures },
...
describe('On the flows route', () => {
  describe('a user with the readAiCatalogFlow ability', () => {
    it('can see the Flows tab', async () => {
      const { wrapper } = await mountNavTabs({ glAbilities: { readAiCatalogFlow: true } });

      expect(tabTitles(wrapper)).toContain('Flows');
    });
  });

  describe('a user with the aiCatalogFlows feature flag enabled', () => {
    it('can see the Flows tab', async () => {
      const { wrapper } = await mountNavTabs({ glFeatures: { aiCatalogFlows: true } });

      expect(tabTitles(wrapper)).toContain('Flows');
    });
  });

  describe('a user without the readAiCatalogFlow ability', () => {
    it('cannot see the Flows tab', async () => {
      const { wrapper } = await mountNavTabs();

      expect(tabTitles(wrapper)).not.toContain('Flows');
    });
  });
