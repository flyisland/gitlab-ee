...
describe.each([agentTestConfig, flowTestConfig])(
  ...
  ({
    ...
    const createComponent = ({ props = {}, provide = {} } = {}) => {
      ...
      mockApollo.clients.defaultClient.watchQuery({ query: refetchQuery }).subscribe();
    ...
    describe('beta badge', () => {
      it('renders beta badge for non-foundational flows only', () => {
        createComponent({
          props: {
            aiCatalogItem: { ...aiCatalogItem, foundational: false, configurationForProject },
          },
        });

        const badge = findExperimentBadge();
        const isFlow = aiCatalogItem.itemType === AI_CATALOG_TYPE_FLOW;
        expect(badge.exists()).toBe(isFlow);
        if (badge.exists()) expect(badge.props('type')).toBe('beta');
      });

      it('does not render beta badge for foundational items', () => {
        createComponent({
          props: {
            aiCatalogItem: { ...aiCatalogItem, foundational: true, configurationForProject },
          },
        });
    ...
    describe('foundational icon', () => {
      describe('when item is foundational', () => {
        ...
        it('renders foundational icon with correct props', () => {
          expect(findFoundationalIcon().props('resourceId')).toBe(aiCatalogItem.id);
          expect(findFoundationalIcon().props('itemType')).toBe(aiCatalogItem.itemType);
        });
      });
    ...
    describe('on adding item to project', () => {
      ...
      it('calls create consumer mutation', () => {
        ...
        expect(createAiCatalogItemConsumerHandler).toHaveBeenCalledWith({
          input: { itemId: aiCatalogItem.id, target: { projectId: '1' } },
        });
      });
    ...
    describe('on adding item to group', () => {
      ...
      it('calls create consumer mutation', () => {
        ...
        expect(createAiCatalogItemConsumerHandler).toHaveBeenCalledWith({
          input: { itemId: aiCatalogItem.id, target: { groupId: '1' } },
        });
      });
