...
describe('DuoChatView', () => {
  ...
  describe('rendering', () => {
    ...
    `(
      ...
      ({ shouldAutoFocusInput, autofocus }) => {
        ...
        expect(findChatInput().attributes('autofocus')).toBe(autofocus);
      },
    );
  });
  ...
  describe('thread management', () => {
    ...
    describe('thread list navigation', () => {
      ...
      it('bubbles up go-back-to-chat event from DuoChatHeader', () => {
        createComponent({
          propsData: {
            isMultithreaded: true,
            threadList: THREADLIST,
        ...
        header.vm.$emit('go-back-to-chat');

        expect(wrapper.emitted('back-to-chat')).toHaveLength(1);
        expect(wrapper.emitted('back-to-chat')[0]).toEqual([]);
      });
