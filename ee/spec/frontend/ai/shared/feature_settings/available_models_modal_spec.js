...
describe('AvailableModelsModal', () => {
  ...
  const mockGitlabDefaultModel = mockAgenticChatFeatureSetting.defaultGitlabModel;
  ...
  const createComponent = ({ props = {} } = {}) => {
    wrapper = mountExtended(AvailableModelsModal, {
      propsData: {
        ...
        gitlabDefaultModel: mockGitlabDefaultModel,
  ...
  describe('available models list', () => {
    it('suffixes the GitLab default model name with "- Default"', () => {
      const defaultModel = mockAvailableModels.find((m) => m.ref === mockGitlabDefaultModel.ref);
      const index = mockAvailableModels.indexOf(defaultModel);

      expect(findModelNames().at(index).text()).toBe(`${defaultModel.name} - Default`);
    });

    it('renders non-default model names without a suffix', () => {
      mockAvailableModels.forEach((model, index) => {
        if (model.ref === mockGitlabDefaultModel.ref) return;
