...
import { GlForm, GlButton, GlCollapse } from '@gitlab/ui';
...
describe('AiCommonSettingsForm', () => {
  ...
  describe('collapsible content for foundational agents and flow settings', () => {
    it('expands content when duo agent platform is enabled', () => {
      createComponent({
        props: { duoAgentPlatformEnabled: true, duoAvailability: AVAILABILITY_OPTIONS.DEFAULT_ON },
      });

      expect(findCollapse().props('visible')).toBe(true);
    });

    it('collapses content when duo agent platform is disabled', () => {
      createComponent({ props: { duoAgentPlatformEnabled: false } });

      expect(findCollapse().props('visible')).toBe(false);
    });

    it('collapses content when availability is never_on even if duo agent platform is enabled', () => {
      createComponent({
