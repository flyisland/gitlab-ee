...
describe('DomainListCard', () => {
  ...
  const findMutationError = () => wrapper.findByTestId('domain-mutation-error');
  const findNoDomainEntries = () => wrapper.findByTestId('empty-state-text');
  const findEmptyAddBtn = () => wrapper.findByTestId('empty-add-domain-btn');
  const findDomainRows = () => wrapper.findAllByTestId('domain-row');
  const findHeaderAddBtn = () => wrapper.findByTestId('add-domain-btn');
  const findHeaderSearchBtn = () => wrapper.findByTestId('search-domain-btn');
  const findDomainInput = () => wrapper.findByTestId('domain-input');
  const findConfirmAddBtn = () => wrapper.findByTestId('confirm-add-domain-btn');
  const findSearchInput = () => wrapper.findByTestId('search-input');
  const findPagination = () => wrapper.findByTestId('domain-pagination');
  const findRemoveBtns = () => wrapper.findAllByTestId('remove-domain-btn');
