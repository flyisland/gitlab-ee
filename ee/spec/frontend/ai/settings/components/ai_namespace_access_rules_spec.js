...
describe('AiNamespaceAccessRules', () => {
  ...
  describe('when access rules array is empty', () => {
    ...
    it('displays the empty state message', () => {
      expect(wrapper.text()).toContain(
        'No limits applied. All eligible users have access to all AI features.',
      );
    });
  });
  ...
  describe('when initialNamespaceAccessRules is provided', () => {
    ...
    it('renders the form group with correct label', () => {
      expect(findFormGroup().text()).toContain('Limit access based on group membership');
    });

    it('renders the help text', () => {
      expect(wrapper.text()).toContain(
        'When configured, access to GitLab Duo or GitLab Duo Agent Platform can be restricted to selected groups.',
      );
    });
