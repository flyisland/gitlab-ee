...
describe('DuoAvailabilityForm', () => {
  ...
  describe('with GitLab.com', () => {
    it('displays correct subtitle', () => {
      ...
      expect(findFormGroup().attributes('labeldescription')).toContain(
        'Control whether GitLab can process your code and project data for AI-native features.',
      );
    });
    ...
    it('renders radio buttons with correct descriptions', () => {
      ...
      expect(findRadioButtonDescriptions().at(0).text()).toContain(
        'Allow GitLab to process your code and project data for AI-native features in this group. Your data is sent to GitLab Duo. Groups, subgroups, and projects can opt out individually.',
      );
      expect(findRadioButtonDescriptions().at(1).text()).toContain(
        'Block GitLab from processing your code and project data for AI-native features in this group. Your data stays private unless subgroups or projects opt in individually.',
      );
      expect(findRadioButtonDescriptions().at(2).text()).toContain(
        'Never allow GitLab to process your code and project data for AI-native features in this group. Your data is not sent to GitLab Duo.',
      );
    });
  });
  ...
  it('emits change event when radio button is selected', () => {
    ...
    expect(findFormRadioButtons().at(1).props('value')).toBe(AVAILABILITY_OPTIONS.DEFAULT_OFF);
  });
