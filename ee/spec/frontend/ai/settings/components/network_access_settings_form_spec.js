...
describe('NetworkAccessSettingsForm', () => {
  ...
  const findIncludeRecommendedDomainsCheckbox = () =>
    wrapper.findByTestId('include-recommended-allowed-domains-checkbox');
  const findAllowUnixSocketsCheckbox = () =>
    wrapper.findByTestId('allow-all-unix-sockets-checkbox');
  const findAllowProjectExtensionCheckbox = () =>
    wrapper.findByTestId('allow-project-extension-checkbox');
  ...
  it('renders the section description', () => {
    expect(wrapper.text()).toContain(
      'Control which external network resources Duo Agent Platform can access.',
    );
  });
