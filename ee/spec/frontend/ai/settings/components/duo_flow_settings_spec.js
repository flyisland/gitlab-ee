...
describe('DuoFlowSettings', () => {
  ...
  const defaultProvide = {
    ...
    glFeatures: {
      ...
      duoCodeReviewDapRoutingConsentEnabled: false,
    },
  ...
  describe('Code Review Flow consent modal', () => {
    ...
    const consentProvide = {
      ...
      glFeatures: {
        duoFoundationalFlows: true,
        duoCodeReviewDapRoutingConsentEnabled: true,
      },
    };
    ...
    describe('when consent is not required (flag disabled)', () => {
      beforeEach(async () => {
        wrapper = createWrapper(
          {
            duoRemoteFlowsAvailability: true,
      ...
      it('adds Code Review to the selection without a modal', () => {
        expect(findFoundationalFlowSelector().props('value')).toContain(CODE_REVIEW_REFERENCE);
      });
    });
