...
describe('NavigationRail', () => {
  ...
  const findAiCatalogButton = () => wrapper.findByTestId('ai-catalog-button');
  ...
  describe('when user can configure duo settings and chat is disabled', () => {
    const findDuoDisabledToggle = () => wrapper.findByTestId('duo-disabled-toggle');

    beforeEach(() => {
      createComponent({ chatDisabledReason: 'project', canConfigureDuoSettings: true });
    });
    ...
    it('emits handleTabToggle with chat on click', async () => {
    ...
    it('shows active state when panel is expanded', () => {
      ...
      expect(findDuoDisabledToggle().classes()).toContain('ai-nav-icon-active');
    });
    ...
    it('does not show active state when panel is collapsed', () => {
      ...
      expect(findDuoDisabledToggle().classes()).not.toContain('ai-nav-icon-active');
    });
  });
