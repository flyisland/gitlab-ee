...
describe('AI actions dropdown component', () => {
  ...
  const createActions = (
    handler = () => Promise.resolve("Here's the summary..."),
    extraProps = {},
  ) => [
    {
  ...
  describe('abstract actions', () => {
    ...
    it('emits `replace` on success when the action uses the replace method', async () => {
      const response = 'FooBar';
      createWrapper(createActions(() => Promise.resolve(response), { method: 'replace' }));

      await clickAction();
      await waitForPromises();

      expect(wrapper.emitted('replace')).toStrictEqual([[response]]);
    });
