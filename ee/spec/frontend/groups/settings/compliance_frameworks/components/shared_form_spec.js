...
describe('SharedForm', () => {
  ...
  const findNameGroup = () => wrapper.findComponent('[data-testid="name-input-group"]');
  const findNameInput = () => wrapper.findComponent('[data-testid="name-input"]');
  const findDescriptionGroup = () =>
    wrapper.findComponent('[data-testid="description-input-group"]');
  const findDescriptionInput = () => wrapper.findComponent('[data-testid="description-input"]');
  const findPipelineConfigurationGroup = () =>
    wrapper.findComponent('[data-testid="pipeline-configuration-input-group"]');
  const findPipelineConfigurationInput = () =>
    wrapper.findComponent('[data-testid="pipeline-configuration-input"]');
  const findColorPicker = () => wrapper.findComponent(ColorPicker);
  const findSubmitBtn = () => wrapper.find('[data-testid="submit-btn"]');
  const findCancelBtn = () => wrapper.findComponent('[data-testid="cancel-btn"]');
  ...
  describe('Fields', () => {
    it('shows the correct input and button fields', () => {
      ...
      wrapper = createComponent({ pipelineConfigurationFullPath: 'foo.yml@bar/baz' });
    ...
    it.each([true, false])(
      ...
      (enabled) => {
        jest.spyOn(Utils, 'fetchPipelineConfigurationFileExists').mockReturnValue(true);
        wrapper = createComponent(
          { pipelineConfigurationFullPath: 'foo.yml@bar/baz' },
          { pipelineConfigurationFullPathEnabled: enabled },
        );
    ...
    it('hides pipeline configuration field when pipelineConfigurationFullPath is null', () => {
      wrapper = createComponent(
        { pipelineConfigurationFullPath: null },
        { pipelineConfigurationFullPathEnabled: true, pipelineConfigurationEnabled: true },
      );

      expect(findPipelineConfigurationGroup().exists()).toBe(false);
    });

    it('hides pipeline configuration field when pipelineConfigurationFullPath is empty', () => {
      wrapper = createComponent(
    ...
    it('shows pipeline configuration field when pipelineConfigurationFullPath has a value', () => {
      jest.spyOn(Utils, 'fetchPipelineConfigurationFileExists').mockReturnValue(true);
      wrapper = createComponent(
        { pipelineConfigurationFullPath: 'foo.yml@bar/baz' },
        { pipelineConfigurationFullPathEnabled: true, pipelineConfigurationEnabled: true },
      ...
      expect(findPipelineConfigurationGroup().exists()).toBe(true);
    });

    it('keeps pipeline configuration field visible when user clears the value', async () => {
      jest.spyOn(Utils, 'fetchPipelineConfigurationFileExists').mockReturnValue(true);
      wrapper = createComponent(
        { pipelineConfigurationFullPath: 'foo.yml@bar/baz' },
        { pipelineConfigurationFullPathEnabled: true, pipelineConfigurationEnabled: true },
      ...
      await findPipelineConfigurationInput().vm.$emit('input', '');

      expect(findPipelineConfigurationGroup().exists()).toBe(true);
    });

    describe('When pipeline configuration is not enabled', () => {
      it('renders disabled pipeline configuration input when framework has existing pipeline config', () => {
        jest.spyOn(Utils, 'fetchPipelineConfigurationFileExists').mockReturnValue(true);
        wrapper = createComponent(
          { pipelineConfigurationFullPath: 'foo.yml@bar/baz' },
      ...
      it('hides disabled pipeline configuration input when framework has no existing pipeline config', () => {
  ...
  describe('Validation', () => {
    ...
    `(
      ...
      async ({ pipelineConfigurationFullPath, validity }) => {
        ...
        if (!pipelineConfigurationFullPath) {
          expect(findPipelineConfigurationGroup().exists()).toBe(false);
          return;
        }
  ...
  describe('On pipeline configuration path input', () => {
    it('updates the pipelineConfigurationFullPath value and validates the path', async () => {
      ...
      wrapper = createComponent({ pipelineConfigurationFullPath: 'initial.yml@bar/baz' });
