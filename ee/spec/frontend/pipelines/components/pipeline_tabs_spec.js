...
describe('The Pipeline Tabs', () => {
  ...
  const findJobsTab = () => wrapper.findComponentByTestId('jobs-tab');
  const findLicenseTab = () => wrapper.findByTestId('license-tab');
  const findPipelineTab = () => wrapper.findByTestId('pipeline-tab');
  const findSecurityTab = () => wrapper.findComponentByTestId('security-tab');
  ...
  const RouterViewStubTarget = { name: 'RouterViewStubTarget', template: '<div />' };
  const RouterViewWithSlotStub = stubComponent(RouterViewWithSlot, {
    data() {
      return { RouterViewStubTarget };
    },
    template: `<div><slot :Component="RouterViewStubTarget" /></div>`,
  });
    ...
    wrapper = extendedWrapper(
      shallowMount(PipelineTabs, {
        ...
        stubs: {
          BasePipelineTabs,
          RouterView: true,
          RouterViewWithSlot: RouterViewWithSlotStub,
          ...stubs,
        },
        mocks: {
          $router,
  ...
  describe('codequality badge count', () => {
    ...
    it('updates the codequality badge after a new count has been emitted', async () => {
      ...
      findCodeQualityRouterView().vm.$emit('update-badge-count', newLicenseCount);
      await nextTick();
  ...
  describe('license compliance', () => {
    ...
    it('sets the initial count and updates the license count badge after a new count has been emitted', async () => {
      ...
      findLicensesRouterView().vm.$emit('update-badge-count', newLicenseCount);
      await nextTick();
