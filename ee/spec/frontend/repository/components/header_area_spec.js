...
const mockLockUser = {
  ...
  username: 'admin',
  name: 'Admin',
  avatarUrl: 'https://www.gravatar.com/avatar/admin?s=80',
  webUrl: '/admin',
};
...
describe('HeaderArea', () => {
  ...
  describe('when rendered for tree view', () => {
    describe('HeaderLockIcon', () => {
      ...
      it('receives lock information from a LockDirectoryButton', async () => {
        ...
        expect(findHeaderLockIcon().props('lockUser')).toEqual(mockLockUser);
      });
    });
  ...
  describe('when rendered for blob view', () => {
    ...
    `('with route name $routeName', ({ routeName }) => {
      ...
      describe('HeaderLockIcon', () => {
        it('does not render HeaderLockIcon; file locks surface through BlobControls', () => {
          expect(findHeaderLockIcon().exists()).toBe(false);
