...
import SetupCiPipelineButton from 'ee_component/repository/components/header_area/setup_ci_pipeline_button.vue';
...
describe('HeaderArea', () => {
  ...
  const findSetupCiPipelineButton = () => wrapper.findComponent(SetupCiPipelineButton);
  ...
  describe('when rendered for tree view', () => {
    ...
    describe('SetupCiPipelineButton', () => {
      it('renders on project overview when accessDuoAgenticChat ability and updateVisualLanguage feature flag are enabled', () => {
        wrapper = createComponent({
          route: { name: 'projectRoot' },
          provided: {
            glAbilities: { accessDuoAgenticChat: true },
            glFeatures: { updateVisualLanguage: true },
            resourceId: 'gid://gitlab/Project/1',
          },
          stubs: { SetupCiPipelineButton },
        });

        expect(findSetupCiPipelineButton().exists()).toBe(true);
      });

      it('does not render when accessDuoAgenticChat ability is turned off', () => {
        wrapper = createComponent({
          route: { name: 'projectRoot' },
          provided: {
            glAbilities: { accessDuoAgenticChat: false },
            glFeatures: { updateVisualLanguage: true },
          },
          stubs: { SetupCiPipelineButton },
        });

        expect(findSetupCiPipelineButton().exists()).toBe(false);
      });

      it('does not render when updateVisualLanguage feature flag is turned off', () => {
        wrapper = createComponent({
          route: { name: 'projectRoot' },
          provided: {
            glAbilities: { accessDuoAgenticChat: true },
            glFeatures: { updateVisualLanguage: false },
          },
          stubs: { SetupCiPipelineButton },
        });
      ...
      it('does not render on non-overview routes even when abilities and feature flags are enabled', () => {
        wrapper = createComponent({
          route: { name: 'treePathDecoded', params: { path: '/directory' } },
          provided: {
            glAbilities: { accessDuoAgenticChat: true },
