...
/**
 * Adds the EE-only duoCreatedSession field to every Note object
 * at any level of a nested mock response.
 *
 * @param {Object} obj - The source object to process
 * @returns {Object} A new object with duoCreatedSession added to each Note
 */
const addDuoCreatedSessionToNotes = (obj) => {
  if (obj === null || obj === undefined || typeof obj !== 'object') {
    return obj;
  }

  if (Array.isArray(obj)) {
    return obj.map(addDuoCreatedSessionToNotes);
  }

  const result = {};
  Object.keys(obj).forEach((key) => {
    result[key] = addDuoCreatedSessionToNotes(obj[key]);
  });

  // eslint-disable-next-line no-underscore-dangle
  if (result.__typename === 'Note' && result.duoCreatedSession === undefined) {
...
export const mockWorkItemNotesResponse = addDuoCreatedSessionToNotes(CE.mockWorkItemNotesResponse);

export const mockWorkItemNotesByIidResponse = addDuoCreatedSessionToNotes(
  CE.mockWorkItemNotesByIidResponse,
);

export const mockMoreWorkItemNotesResponse = addDuoCreatedSessionToNotes(
  CE.mockMoreWorkItemNotesResponse,
);

export const mockWorkItemNotesResponseWithComments = (...args) =>
  addDuoCreatedSessionToNotes(CE.mockWorkItemNotesResponseWithComments(...args));

export const createWorkItemNoteResponse = (...args) =>
  addDuoCreatedSessionToNotes(CE.createWorkItemNoteResponse(...args));

export const mockToggleResolveDiscussionResponse = addDuoCreatedSessionToNotes(
  CE.mockToggleResolveDiscussionResponse,
);

export const workItemNotesCreateSubscriptionResponse = addDuoCreatedSessionToNotes(
  CE.workItemNotesCreateSubscriptionResponse,
);

export const workItemNotesUpdateSubscriptionResponse = addDuoCreatedSessionToNotes(
  CE.workItemNotesUpdateSubscriptionResponse,
);

export const workItemSystemNoteWithMetadata = addDuoCreatedSessionToNotes(
  CE.workItemSystemNoteWithMetadata,
);

export const workItemNotesWithSystemNotesWithChangedDescription = addDuoCreatedSessionToNotes(
