...
describe('MR Widget Security Reports', () => {
  ...
  const createComponent = ({
    ...
    wrapper = mountFn(MRSecurityWidget, {
      ...
      provide: {
        glFeatures: { mrSecurityWidgetGraphql: true, mrReportsTab: true },
        ...provide,
      },
      propsData: {
        ...propsData,
  ...
  describe('when mrReportsTab feature flag is disabled', () => {
    const createComponentWithFeatureFlagDisabled = (options = {}) => {
      createComponent({
        ...options,
        provide: { glFeatures: { mrReportsTab: false }, ...options.provide },
      });
    };

    const expandWidget = async () => {
      wrapper.findByRole('button', { name: 'Expand security scanning details' }).trigger('click');
      await nextTick();
      await nextTick();
    };
    ...
    it('should display the view all pipeline findings button', async () => {
      ...
      expect(findWidget().props('actionButtons')).toEqual([
        {
          href: '/gitlab-org/gitlab/-/pipelines/789/security',
