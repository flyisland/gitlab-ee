...
describe('MergeTrainFailedPipelineConfirmationDialog', () => {
  ...
  const findStartMergeTrainBtn = () => wrapper.findComponent('[data-testid="start-merge-train"]');
  ...
  it('should emit the start-merge-train event', () => {
    findStartMergeTrainBtn().vm.$emit('click');

    expect(wrapper.emitted('start-merge-train')).toHaveLength(1);
  });
