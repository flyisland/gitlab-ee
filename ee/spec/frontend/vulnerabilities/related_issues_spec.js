...
describe('Vulnerability related issues component', () => {
  ...
  describe('add related issue', () => {
    ...
    it('adds related issue with vulnerabilityLinkId populated', async () => {
      ...
      blockEmit('add-issuable-form-submit', { pendingReferences: '#1' });
      await axios.waitForAll();
    ...
    it('adds multiple issues', async () => {
      ...
      blockEmit('add-issuable-form-submit', { pendingReferences: '#1 #2 #3' });
      await axios.waitForAll();
    ...
    it('adds only issues that returns issue', async () => {
      mockAxios
        ...
        .onPost(propsData.endpoint)
        .replyOnce(HTTP_STATUS_SERVICE_UNAVAILABLE);
      blockEmit('add-issuable-form-submit', { pendingReferences: '#1 #2 #3 #4' });
      await axios.waitForAll();
  ...
  describe('related issues form', () => {
    it('resets form and hides it', async () => {
      ...
      blockEmit('add-issuable-form-cancel');
      await nextTick();
  ...
  describe('pending references', () => {
    it('adds pending references', async () => {
      ...
      blockEmit('add-issuable-form-input', { untouchedRawReferences, touchedReference });
      await nextTick();
    ...
    it('processes pending references', async () => {
      createWrapper();
      blockEmit('add-issuable-form-blur', '135 246');
      await nextTick();
    ...
    it('removes pending reference', async () => {
      ...
      blockEmit('pending-issuable-remove-request', 1);
      await nextTick();
  ...
  describe('remove related issue', () => {
    ...
    it('removes related issue', async () => {
      ...
      blockEmit('related-issue-remove-request', issue1.id);
      await axios.waitForAll();
  ...
  describe('issues-count-change event', () => {
    it('emits the count once related issues are fetched', async () => {
      mockAxios.onGet(propsData.endpoint).replyOnce(HTTP_STATUS_OK, [issue1, issue2]);
      createWrapper();
      await axios.waitForAll();

      expect(wrapper.emitted('issues-count-change')).toEqual([[2]]);
    });

    it('emits the updated count when a related issue is removed', async () => {
      mockAxios.onGet(propsData.endpoint).replyOnce(HTTP_STATUS_OK, [issue1, issue2]);
      createWrapper();
      await axios.waitForAll();
      ...
      expect(wrapper.emitted('issues-count-change')).toEqual([[2], [1]]);
    });
  });
