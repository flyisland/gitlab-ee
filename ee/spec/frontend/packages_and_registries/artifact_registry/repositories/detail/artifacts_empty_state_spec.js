...
describe('ArtifactRegistryArtifactsEmptyState', () => {
  ...
  describe('the snippets', () => {
    ...
    /* eslint-disable no-template-curly-in-string -- The snippets display shell-style placeholders. */
    it.each([
      ['MAVEN', '${env.ARTIFACT_REGISTRY_TOKEN}'],
      ['NPM', '${ARTIFACT_REGISTRY_TOKEN}'],
      ['DOCKER', '${ARTIFACT_REGISTRY_TOKEN}'],
      ['OCI', '${ARTIFACT_REGISTRY_TOKEN}'],
      /* eslint-enable no-template-curly-in-string */
    ])('leaves a %s repository a token placeholder and never a credential', (format, token) => {
      createComponent({ format });
