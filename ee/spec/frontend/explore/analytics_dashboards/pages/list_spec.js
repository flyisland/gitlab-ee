...
import NewDashboardButton from 'ee/explore/analytics_dashboards/components/new_dashboard_button.vue';
...
describe('ExploreAnalyticsDashboardsListEE', () => {
  ...
  const createComponent = ({ createCustomDashboard = true } = {}) => {
    wrapper = shallowMountExtended(ExploreAnalyticsDashboardsListEE, {
      propsData: {
        currentUserId: MOCK_CURRENT_USER_ID,
      },
      provide: {
        glAbilities: { createCustomDashboard },
      },
      stubs: {
        IndexLayout,
        PageHeading,
  ...
  describe('new dashboard button', () => {
    it('renders the button when the user can create a dashboard', () => {
      createComponent();

      expect(findNewDashboardButton().exists()).toBe(true);
    });

    it('does not render the button when the user cannot create a dashboard', () => {
      createComponent({ createCustomDashboard: false });
