...
describe('Learn GitLab Section Link', () => {
  ...
  const findActionLink = () => wrapper.findComponentByTestId('learn-gitlab-link');
  const findDisabledLink = () => wrapper.findByTestId('disabled-learn-gitlab-link');
  const findPopoverTrigger = () => wrapper.findComponentByTestId('contact-admin-popover-trigger');
