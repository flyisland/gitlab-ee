...
describe('Subscription Details Card', () => {
  ...
  describe('with data', () => {
    ...
    describe('when hasGitlabCreditsAddOn is true', () => {
      beforeEach(() => {
        createComponent({
          headerText: 'Card header title',
          subscription: { ...license.ULTIMATE, hasGitlabCreditsAddOn: true },
        });
      });

      it('appends GitLab Credits to the plan value', () => {
        const planDetail = findSubscriptionDetailsTable()
          .props('details')
          .find(({ detail }) => detail === 'plan');

        expect(planDetail.value).toBe('Ultimate with GitLab Credits');
      });
    });

    describe('when hasGitlabCreditsAddOn is false', () => {
      it('shows only the plan name', () => {
        const planDetail = findSubscriptionDetailsTable()
          .props('details')
          .find(({ detail }) => detail === 'plan');

        expect(planDetail.value).toBe('Ultimate');
      });
    });
