...
describe('SubscriptionActivationErrors', () => {
  ...
  const findExpiredTrialErrorAlert = () => wrapper.findByTestId(testIds.EXPIRED_TRIAL_ERROR_ALERT);
  ...
  describe('with error', () => {
    ...
    describe.each([
      ...
      {
        error: EXPIRED_TRIAL_SERVER_ERROR,
        title: i18n.EXPIRED_TRIAL_ERROR_TITLE,
        text: i18n.EXPIRED_TRIAL_ERROR_MESSAGE,
        helpLinks: [links.purchaseSubscriptionPath, links.supportLink],
        testId: testIds.EXPIRED_TRIAL_ERROR_ALERT,
      },
      {
      ...
      it('displays correct content', () => {
        const alert = wrapper.findComponentByTestId(testId);
      ...
      it('does not show other alerts', () => {
        ...
        expect(findExpiredTrialErrorAlert().exists()).toBe(error === EXPIRED_TRIAL_SERVER_ERROR);
