...
describe('Git abuse rate limit settings form component', () => {
  ...
  const findNumberOfReposInput = () => wrapper.findComponentByTestId('number-of-repos-input');

  const findReportingTimePeriodFormGroup = () =>
    wrapper.findByTestId('reporting-time-period-group');
  const findReportingTimePeriodInput = () =>
    wrapper.findComponentByTestId('reporting-time-period-input');

  const findAllowedUsersFormGroup = () => wrapper.findByTestId('allowed-users-group');

  const findUsersAllowlist = () => wrapper.findComponentByTestId('allowed-users');

  const findAlertedUsersFormGroup = () => wrapper.findByTestId('alerted-users-group');

  const findUsersAlertlist = () => wrapper.findComponentByTestId('alerted-users');
