...
describe('ApprovalSettings', () => {
  ...
  const findApprovalSettingsWarningIcon = () =>
    wrapper.findByTestId('approval-settings-warning-icon');
  const findCommitSettingsWarningIcon = () => wrapper.findByTestId('commit-settings-warning-icon');
  ...
  describe('with settings', () => {
    ...
    it('renders policy override warning icons for approval settings and commit settings', async () => {
      createWrapper();
      await waitForPromises();

      expect(findApprovalSettingsWarningIcon().exists()).toBe(true);
      expect(findCommitSettingsWarningIcon().exists()).toBe(true);
    });
    ...
    `('with the $testid checkbox', ({ testid, action, setting, labelKey }) => {
      ...
      beforeEach(async () => {
        ...
        createWrapper({ approvalSettingsPath: '/api/v4/projects/19/' });
        await waitForPromises();
        checkbox = wrapper.findComponentByTestId(testid);
      });
    ...
    describe('when preventMrApprovalRuleEdit is enforced by a security policy', () => {
      beforeEach(async () => {
        setupStore({
          ...settings,
      ...
      it('shows the policy tooltip on the locked checkbox', () => {
        const checkbox = wrapper.findComponentByTestId('prevent-mr-approval-rule-edit');

        expect(checkbox.props('locked')).toBe(true);
        expect(checkbox.props('lockedText')).toBe(APPROVAL_SETTINGS_I18N.enforcedByPolicy);
      });
    });
    ...
    `('with the $testid radio', ({ testid, labelKey }) => {
      ...
      beforeEach(async () => {
        ...
        createWrapper({ approvalSettingsPath: '/api/v4/projects/19/' });
        await waitForPromises();
        radio = wrapper.findComponentByTestId(testid);
      });
    ...
    `('with the $testid radio', ({ value }) => {
      ...
      beforeEach(async () => {
        ...
        createWrapper({ approvalSettingsPath: '/api/v4/projects/19/' });
        await waitForPromises();
        radios = wrapper.findComponentByTestId('when-commit-is-added-radios');
      });
    ...
    describe('locked settings', () => {
      ...
      `(
        ...
        ({ property, value, locked, testid }) => {
          ...
          expect(wrapper.findComponentByTestId(testid).props('locked')).toBe(locked);
        },
      );
    });
  });
