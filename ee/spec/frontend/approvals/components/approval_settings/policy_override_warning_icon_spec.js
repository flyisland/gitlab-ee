...
describe('PolicyOverrideWarningIcon', () => {
  ...
  describe('iconId prop', () => {
    ...
    it('uses default iconId when not provided', () => {
      ...
      expect(findIcon().attributes('id')).toBe('policy-override-warning-icon');
      expect(findPopover().props('target')).toBe('policy-override-warning-icon');
    });

    it('uses custom iconId when provided', () => {
      createComponent({ propsData: { iconId: 'custom-icon-id' } });

      expect(findIcon().attributes('id')).toBe('custom-icon-id');
      expect(findPopover().props('target')).toBe('custom-icon-id');
    });
  });

  describe('settings prop', () => {
    const policiesWithDifferentSettings = [
      {
        name: 'policy with prevent_approval_by_author',
        enabled: true,
        enforcement_type: 'enforce',
        approval_settings: { [PREVENT_APPROVAL_BY_AUTHOR]: true },
        editPath: 'link 1',
      ...
      {
        name: 'policy with remove_approvals_with_new_commit',
        enabled: true,
        enforcement_type: 'enforce',
        approval_settings: { [REMOVE_APPROVALS_WITH_NEW_COMMIT]: true },
        editPath: 'link 2',
      },
    ];

    it('filters policies to only show those matching the settings', () => {
      setupStore(policiesWithDifferentSettings);
      createComponent({ propsData: { settings: [REMOVE_APPROVALS_WITH_NEW_COMMIT] } });

      expect(findIcon().exists()).toBe(true);
      expect(findAllPolicyOverrideTexts().at(0).props('policies')).toHaveLength(1);
      expect(findAllPolicyOverrideTexts().at(0).props('policies')[0].name).toBe(
        'policy with remove_approvals_with_new_commit',
      );
    });

    it('does not render icon when no policies match the settings', () => {
      setupStore(policiesWithDifferentSettings);
      createComponent({ propsData: { settings: ['nonexistent_setting'] } });
