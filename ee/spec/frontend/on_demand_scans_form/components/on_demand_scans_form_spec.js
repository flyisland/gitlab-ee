...
describe('OnDemandScansForm', () => {
  ...
  const findNameInputComponent = () => wrapper.findComponentByTestId('dast-scan-name-input');
  const findBranchInput = () => wrapper.findComponentByTestId('dast-scan-branch-input');
  const findDescriptionInput = () => wrapper.findComponentByTestId('dast-scan-description-input');
  ...
  const findSubmitButton = () => wrapper.findComponentByTestId('on-demand-scan-submit-button');
  const findSaveButton = () => wrapper.findComponentByTestId('on-demand-scan-save-button');
  const findRunnerTagsFormGroup = () => wrapper.findByTestId('on-demand-scan-runner-tags');
  const findCancelButton = () => wrapper.findComponentByTestId('on-demand-scan-cancel-button');
  ...
  describe('when editing an existing scan', () => {
    ...
    describe('when the branch is present', () => {
      ...
      it('populates the fields with passed values', () => {
        expect(findNameInputComponent().props('value')).toBe(dastScan.name);
    ...
    `(
      ...
      ({ actionFunction, submitButtonLoading, saveButtonLoading, runAfter, redirectPath }) => {
        ...
        it('does not run any mutation if name is empty', () => {
          createShallowComponent();
          setValidFormData();
          findNameInputComponent().vm.$emit('input', '');
          actionFunction();
