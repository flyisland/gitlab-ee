...
describe('Audit Event Utils', () => {
  ...
  describe('getFormattedFormItem', () => {
    it('maps namespaceFilters to the array shape that the form components expect', () => {
    ...
    it('returns an empty namespaceFilters array when the item has none', () => {
      const itemWithoutNamespaceFilters = { ...mockHttpTypeDestination[0], namespaceFilters: [] };

      expect(getFormattedFormItem(itemWithoutNamespaceFilters).namespaceFilters).toEqual([]);
    });

    it('returns an empty namespaceFilters array when the item omits the field entirely', () => {
      const { namespaceFilters: _omit, ...itemWithoutKey } = mockHttpTypeDestination[0];

      expect(getFormattedFormItem(itemWithoutKey).namespaceFilters).toEqual([]);
    });
  });
});
