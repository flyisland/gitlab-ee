...
describe('StreamWithFilters', () => {
  ...
  const createComponent = (props) => {
    wrapper = shallowMountExtended(StreamEventTypeFilters, {
      propsData: {
  ...
  const findSelectionCount = () => wrapper.findByTestId('event-type-filter-selection-count');
  ...
  describe('selection counter', () => {
    it('is not rendered when no events are selected', () => {
      createComponent({ value: [] });

      expect(findSelectionCount().exists()).toBe(false);
    });

    it('shows "2 selected" when two events are selected', () => {
      expect(findSelectionCount().text()).toBe('2 selected');
    });

    it('shows "4 selected" when four events are selected', () => {
      createComponent({
        value: ['add_gpg_key', 'user_created', 'user_blocked', 'project_unarchived'],
      });

      expect(findSelectionCount().text()).toBe('4 selected');
    });
  });
});
