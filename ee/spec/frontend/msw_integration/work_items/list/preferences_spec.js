...
describe('Work items list - display preferences', () => {
  ...
  const findAssigneeLink = () => findByTestId('assignee-link', findIssueCard());
  const findMilestone = () => findByTestId('issuable-milestone', findIssueCard());
  const findStatusBadge = () => findByTestId('work-item-status-badge', findIssueCard());
  const findWeight = () => findByTestId('issuable-weight-content', findIssueCard());
  const findDates = () => findByTestId('issuable-due-date', findIssueCard());
  const findIteration = () => findByTestId('iteration-attribute', findIssueCard());
  const findHealthStatus = () => findByTestId('status-text', findIssueCard());
  ...
  const findBlockedIcon = () => findByTestId('relationship-blocked-by-icon', findSecondIssueCard());
  const findUpvotes = () => findByTestId('issuable-upvotes', findSecondIssueCard());
  const findComments = () => findByTestId('issuable-comments', findSecondIssueCard());

  const findChildTaskCard = () => findByGraphQLId(childTaskId, getIdFromGraphQLId);
  const findParentMetadata = () =>
    findByTestId('work-item-parent-metadata-link', findChildTaskCard());
