...
describe('Work items list - views', () => {
  ...
  describe('when the work items list page renders', () => {
    ...
    it('shows default sort order', () => {
      expect(getText(findByTestId('issuable-search-container'))).toContain('Created date');
    });

    it('displays assignee on the work item card', () => {
      expect(findByTestId('assignee-link', findIssueCard())).not.toBe(null);
    });

    it('displays milestone on the work item card', () => {
      expect(findByTestId('issuable-milestone', findIssueCard())).not.toBe(null);
    ...
    it('displays upvotes on the work item card', () => {
      expect(findByTestId('issuable-upvotes', findSecondIssueCard())).not.toBe(null);
    });

    it('displays health status on the work item card', () => {
      expect(findByTestId('status-text', findIssueCard())).not.toBe(null);
    });

    it('displays weight on the work item card', () => {
      expect(findByTestId('issuable-weight-content', findIssueCard())).not.toBe(null);
    });
