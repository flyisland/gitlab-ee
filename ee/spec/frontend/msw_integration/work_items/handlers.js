...
import { buildListFilterMatcher } from './list_filter_fixtures';
...
const OPERATION_NAME_OVERRIDES = {
  ...
  projectWorkItems: fixtures.projectWorkItems,
};

const matchListFilterFixture = buildListFilterMatcher(fixtures);

// Hand-shaped rather than generated: `ClientFieldRedactor` drops `@persist` fields as if
// they were `@client`, so `search_labels.query.graphql` redacts to nothing. Project branch
// only, which suits specs mounting with `isGroup: false`.
function searchLabelsResponse() {
  const { id: projectId, labels } = fixtures.projectLabels.data.namespace;

  return { data: { project: { id: projectId, labels, __typename: 'Project' } } };
}

function listResponse(kind, variables = {}) {
  // Throws rather than falling back when a label filter has no fixture; see `matchFixture`.
  const match = matchListFilterFixture(variables);

  const unfiltered = kind === 'slim' ? fixtures.getWorkItemsSlim : fixtures.getWorkItemsFull;

  return { data: (match ? match[kind]() : unfiltered).data };
}
...
const DYNAMIC_OPERATION_HANDLERS = {
  ...
  getWorkItemsSlim: ({ variables }) => listResponse('slim', variables),
  getWorkItemsSlimEE: ({ variables }) => listResponse('slim', variables),
  getWorkItemsFull: ({ variables }) => listResponse('full', variables),
  getWorkItemsFullEE: ({ variables }) => listResponse('full', variables),
