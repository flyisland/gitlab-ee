...
describe('FlowEditor', () => {
  ...
  const findEditor = () => wrapper.findByTestId('flow-source-editor');
  const findBackButton = () => wrapper.findByTestId('back-button');
  const findDiscardButton = () => wrapper.findByTestId('discard-flow-button');
  const findSaveButton = () => wrapper.findByTestId('save-flow-button');
  ...
  describe('saving', () => {
    ...
    it('runs the create mutation with the edited definition', async () => {
      ...
      expect(createHandler).toHaveBeenCalledWith({
        input: { applicationId: applicationGid, definition: 'stages: []\n' },
      });
    });
