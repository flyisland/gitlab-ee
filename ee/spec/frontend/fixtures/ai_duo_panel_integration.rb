...
RSpec.describe 'AI Duo Panel Integration (GraphQL fixtures)', type: :request, feature_category: :duo_chat do
  ...
  describe GraphQL::Query, 'state manager bootstrap queries' do
    ...
    it "#{base_output_path}get_gitlab_credits_available.query.graphql.json" do
      # The TanukiBot credits check requires cloud-connector wiring that's not
      # set up in the fixture-generator environment. Stub the helper so the
      # response is deterministic.
      allow_next_instance_of(::Gitlab::Llm::TanukiBot) do |bot|
        allow(bot).to receive(:credits_available?).and_return(true)
      end

      query = get_graphql_query_as_string('ai/graphql/get_gitlab_credits_available.query.graphql', ee: true)
      post_graphql(query, current_user: user)
      expect_graphql_errors_to_be_empty
    end
  end
