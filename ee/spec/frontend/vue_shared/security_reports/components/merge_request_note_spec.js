...
describe('Merge request note', () => {
  ...
  const createWrapper = ({ feedback = FEEDBACK, project = PROJECT, feedAligned } = {}) => {
    wrapper = shallowMountExtended(MergeRequestNote, {
      stubs: { GlSprintf },
      propsData: { feedback, project, feedAligned },
    });
  };
  ...
  describe('when feed-aligned', () => {
    beforeEach(() => createWrapper({ feedAligned: true }));

    it('renders as a system note so the message reads inline like the other feed rows', () => {
      expect(findEventItem().props('isSystemNote')).toBe(true);
    });

    it('lowercases the event text to follow the actor name', () => {
      expect(wrapper.text()).toMatchInterpolatedText(
        `created merge request !${FEEDBACK.merge_request_iid} at ${PROJECT.value}`,
      );
    });
