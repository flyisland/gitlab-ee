...
describe('Protected Branches Selector', () => {
  ...
  const findSelectableBranches = () =>
    findListbox()
      .props('items')
      .map((item) => item.name);
  const findAllBranchesOption = () => wrapper.findComponentByTestId('all-branches-option');
  const findAllProtectedBranchesOption = () =>
    wrapper.findComponentByTestId('all-protected-branches-option');
  ...
  describe('default rendering', () => {
    ...
    it('displays the protected branches and all branches option', async () => {
      ...
      expect(wrapper.emitted('api-error')).toStrictEqual([[{ hasErrored: false }]]);
  ...
  describe('when fetching the branch list fails', () => {
    ...
    it('emits the `api-error` event', () => {
      expect(wrapper.emitted('api-error')).toStrictEqual([[{ hasErrored: true, error }]]);
    });
