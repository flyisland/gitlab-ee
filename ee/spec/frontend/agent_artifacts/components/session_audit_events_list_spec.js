...
import getGroupSessionAuditEventsQuery from 'ee/agent_artifacts/graphql/queries/get_group_session_audit_events.query.graphql';
import waitForPromises from 'helpers/wait_for_promises';

Vue.use(VueApollo);

const MOCK_WORKFLOW_ID = 'gid://gitlab/Ai::DuoWorkflows::Workflow/1';
const MOCK_PROJECT_FULL_PATH = 'group/project';
const MOCK_GROUP_FULL_PATH = 'group';
...
const buildProjectResponse = (auditEventsArgs) => ({
  data: {
    project: {
      ...
      duoWorkflowSessionArtifacts: {
        nodes: [
          {
            id: MOCK_WORKFLOW_ID,
            auditEvents: buildAuditEvents(auditEventsArgs),
...
const buildGroupResponse = (auditEventsArgs) => ({
  data: {
    group: {
...
describe('SessionAuditEventsList', () => {
  ...
  const createComponent = ({
    ...
    const isProjectMode = Boolean(projectFullPath);
    const query = isProjectMode
      ? getProjectSessionAuditEventsQuery
      : getGroupSessionAuditEventsQuery;
    const defaultHandler = jest
      .fn()
      .mockResolvedValue(isProjectMode ? buildProjectResponse() : buildGroupResponse());
    const requestHandler = handler || defaultHandler;

    const apolloProvider = createMockApollo([[query, requestHandler]]);
  ...
  describe('query selection', () => {
    it('uses the project query and variables in project mode', async () => {
    ...
    it('uses the group query and variables in group mode', async () => {
      const handler = createComponent({
        projectFullPath: null,
        groupFullPath: MOCK_GROUP_FULL_PATH,
        handler: jest.fn().mockResolvedValue(buildGroupResponse()),
      });
      await waitForPromises();
    ...
    it('renders events resolved from the group response in group mode', async () => {
      createComponent({
        projectFullPath: null,
        groupFullPath: MOCK_GROUP_FULL_PATH,
        handler: jest.fn().mockResolvedValue(buildGroupResponse()),
