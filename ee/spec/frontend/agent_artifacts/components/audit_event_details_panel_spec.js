...
describe('AuditEventDetailsPanel', () => {
  ...
  it('renders the event name as the title', () => {
    expect(findTitle().text()).toBe('tool_execution');
  });
  ...
  it('renders the breadcrumb with three items', () => {
    ...
    expect(items[2].text).toBe('tool_execution');
  });
  ...
  it('omits the session crumb when the session name is blank', () => {
    ...
    expect(items.map((item) => item.text)).toEqual(['Agent audit events', 'tool_execution']);
  });
