    ...
    describe('popup when saving framework', () => {
      ...
      describe('on update', () => {
        it('shows popup after, with pipeline', async () => {
          ...
          const frameworkWithPipeline = () => {
            const response = createComplianceFrameworksReportResponse();
            response.data.namespace.complianceFrameworks.nodes[0].pipelineConfigurationFullPath =
              '.compliance.yml@group/project';
            return response;
          };
          wrapper = createComponent(mountExtended, {
            requestHandlers: [[getComplianceFrameworkQuery, frameworkWithPipeline]],
            routeParams: { id: 1 },
          });
          await waitForPromises();
          ...
          const mutateMock = jest.fn().mockResolvedValue({
            data: {
              updateComplianceFramework: { errors: [] },
            },
          });
          wrapper.vm.$apollo.mutate = mutateMock;

          const interjectSpy = jest.spyOn(wrapper.vm, 'interjectModal');
          await fillAndSubmitForm();
          await waitForPromises();

          expect(mutateMock).toHaveBeenCalled();
          expect(interjectSpy).toHaveBeenCalled();
        });
