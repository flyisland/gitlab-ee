  ...
  jest.fn().mockResolvedValue({
    data: {
      pipelineExecutionSchedulePolicyTestRun: {
        testRun: { id: 'gid://gitlab/Security::PolicyTestRun/1', state: 'RUNNING' },
        pipeline: { id: 'gid://gitlab/Ci::Pipeline/1', status: 'created' },
        errors: [],
        ...response,
      },
    },
    ...
    describe('actions', () => {
      ...
      describe('test run badge', () => {
        ...
        it('updates badge testRun when test-run-created fires from drawer', async () => {
          ...
          wrapper.vm.presentPolicyDrawer([policy]);
          await nextTick();

          wrapper.findComponent(DrawerWrapper).vm.$emit('test-run-created', newTestRun);
          await nextTick();
