...
describe('BranchSelection', () => {
  ...
  describe('group-level', () => {
    ...
    describe('specific branches input', () => {
      ...
      it.each`
        description                                  | inputBranches                              | expectedBranches
        ${'filters out "*" branches'}                | ${'main, *'}                               | ${['main']}
        ${'filters out duplicate branches'}          | ${'main, release, main, staging, release'} | ${['main', 'release', 'staging']}
        ${'filters out duplicates and "*" branches'} | ${'main, *, release, main, *'}             | ${['main', 'release']}
      `('$description', async ({ inputBranches, expectedBranches }) => {
        findSpecificBranchInput().vm.$emit('input', inputBranches);
        await nextTick();

        expect(wrapper.emitted().changed).toEqual([
          [expect.objectContaining({ branches: expectedBranches })],
        ]);
      });
