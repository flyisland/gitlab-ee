  ...
  describe('global compliance security policies (CSP) group', () => {
    describe('scope type selection', () => {
      ...
      it('renders project selector', () => {
        ...
        expect(findScopeProjectSelector().props('groupExceptionType')).toBe(EXCEPT_GROUPS);
        expect(findScopeProjectSelector().props('projectScopeType')).toBe(ALL_PROJECTS_IN_GROUP);
        expect(findScopeProjectSelector().props('groups')).toEqual({
          excluding: [{ id: 1 }],
        });
      });
    });
    ...
    describe('scope selection behavior', () => {
      ...
      it('handles select-group-exception-type event from ScopeProjectSelector', async () => {
        createComponent({ provide: { designatedAsCsp: true } });

        await findScopeProjectSelector().vm.$emit('select-group-exception-type', EXCEPT_GROUPS);
