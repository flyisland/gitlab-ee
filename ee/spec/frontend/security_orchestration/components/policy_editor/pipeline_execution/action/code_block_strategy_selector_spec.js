...
describe('CodeBlockStrategySelector', () => {
  ...
  describe("'schedule' strategy visibility", () => {
    it('renders schedule strategy when feature flag is enabled', () => {
      createComponent();
    ...
    it('does not render schedule strategy when feature flag is disabled', () => {
      createComponent({ provide: { glFeatures: { scheduledPipelineExecutionPolicies: false } } });
      expect(findListBox().props('items')).toEqual([
        { text: 'Inject', value: INJECT },
        { text: 'Override', value: OVERRIDE },
      ]);
  ...
  describe('beta badge', () => {
    it.each`
      ...
      ${INJECT}            | ${false}
      ${OVERRIDE}          | ${false}
      ${DEPRECATED_INJECT} | ${false}
      ${SCHEDULE}          | ${true}
    `('showBetaBadge returns $expected for $strategy', ({ strategy, expected }) => {
      createComponent();

      expect(wrapper.vm.showBetaBadge(strategy)).toBe(expected);
    });

    it('renders beta badge with neutral variant for schedule strategy', () => {
      createComponent({
        propsData: {
          strategy: SCHEDULE,
        },
      ...
      expect(findBetaBadge().props('variant')).toBe('neutral');
