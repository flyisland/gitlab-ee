...
jest.mock('lodash-es', () => ({
  ...
  uniqueId: jest.fn().mockReturnValue(ruleId),
}));
...
describe('RuleSection', () => {
  ...
  describe('security scan builder', () => {
    it('renders security scan rule builder', () => {
      const expectedRule = securityScanBuildRule();
      factory({
        propsData: { initRule: SECURITY_SCAN_RULE },
      });

      expect(findSecurityScanRule().exists()).toBe(true);

      findSecurityScanRule().vm.$emit('changed', expectedRule);
