...
describe('ProjectDastProfileSelector', () => {
  ...
  const findScannerProfileTrigger = () => wrapper.findComponentByTestId('scanner-profile-trigger');
  const findSiteProfileTrigger = () => wrapper.findComponentByTestId('site-profile-trigger');
  ...
  describe('default', () => {
    ...
    const scannerProfileNode = scannerProfilesFixtures.data.project.scannerProfiles.nodes.find(
      ({ profileName }) => profileName === 'Passive scanner',
    );
    const siteProfileNode = siteProfilesFixtures.data.project.siteProfiles.nodes.find(
      ({ profileName }) => profileName === 'Non-validated',
    );

    it.each`
      findTrigger                  | profileType     | selectedProfile          | selectedProfileName
      ${findScannerProfileTrigger} | ${SCANNER_TYPE} | ${scannerProfileNode.id} | ${scannerProfileNode.profileName}
      ${findSiteProfileTrigger}    | ${SITE_TYPE}    | ${siteProfileNode.id}    | ${siteProfileNode.profileName}
    `(
      'selects profile',
