...
describe('SignUpArkoseApp', () => {
  ...
  describe('when hasReactiveSubmitButton is true', () => {
    let mockSubmitEvent;
    ...
    beforeEach(() => {
      ...
      createComponent({ props: { hasReactiveSubmitButton: true } });
      mockSubmitEvent = {
        preventDefault: jest.fn(),
        stopPropagation: jest.fn(),
        target: { submit: jest.fn() },
    ...
    describe('when iframe is shown but token not yet present', () => {
      beforeEach(async () => {
        onShown();
        await submitForm(mockSubmitEvent);
      });
    ...
    describe('when iframe has not been shown yet', () => {
      beforeEach(async () => {
        await submitForm(mockSubmitEvent);
      });

      it('shows loading error immediately', () => {
        expect(createAlert).toHaveBeenCalledWith({
          message: 'Please wait while we prepare for verification.',
        });
      });
