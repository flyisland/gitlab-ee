...
import TEST_RUNS_EMPTY_STATE_SVG from '@gitlab/svgs/dist/illustrations/empty-state/empty-pipeline-md.svg?url';
...
describe('Project quality summary app component', () => {
  ...
  const findCodeQualityStat = (index) =>
    wrapper.findAllComponentsByTestId('code-quality-stat').at(index);
  ...
  describe('without data', () => {
    ...
    it('shows a test runs empty state', () => {
      ...
      expect(wrapper.findComponent(GlEmptyState).props('svgPath')).toBe(TEST_RUNS_EMPTY_STATE_SVG);
    });
  });
});
