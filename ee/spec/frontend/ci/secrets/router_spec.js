...
describe('Secrets router', () => {
  ...
  describe('when isEligibleForTrial is true', () => {
    it('allows navigation to the index route', async () => {
      router = createRouter(base, true);
      await router.push('/');

      expect(router.currentRoute.name).toBe(INDEX_ROUTE_NAME);
    });

    it.each`
      routeName             | path
      ${NEW_ROUTE_NAME}     | ${'/new'}
      ${DETAILS_ROUTE_NAME} | ${'/secretName/details'}
      ${EDIT_ROUTE_NAME}    | ${'/secretName/edit'}
    `('redirects $routeName back to index', async ({ path }) => {
      router = createRouter(base, true);
