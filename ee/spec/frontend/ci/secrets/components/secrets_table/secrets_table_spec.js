...
describe('SecretsTable component', () => {
  ...
  const findSecretHealthStatus = (index) =>
    wrapper.findAllByTestId('secret-health-status').at(index);
  ...
  describe.each`
    context           | secretsList               | mockSecretsResponse           | showBranch | visitEventName
    ${ENTITY_PROJECT} | ${mockProjectSecretsData} | ${mockProjectSecretsResponse} | ${true}    | ${'visit_project_secrets_manager'}
    ${ENTITY_GROUP}   | ${mockGroupSecretsData}   | ${mockGroupSecretsResponse}   | ${false}   | ${'visit_group_secrets_manager'}
  `(
    ...
    ({ context, secretsList, mockSecretsResponse, showBranch, visitEventName }) => {
      ...
      describe('when secrets are fetched', () => {
        ...
        it('renders a table of secrets', () => {
          ...
          expect(findSecretBranches().exists()).toBe(showBranch);
        });
