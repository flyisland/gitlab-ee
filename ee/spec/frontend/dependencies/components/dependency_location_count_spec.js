...
describe('Dependency Location Count component', () => {
  ...
  const createComponent = ({
    ...
    groupDependencyDropdownsGraphql = true,
    ...
    wrapper = mountFn(DependencyLocationCount, {
      ...
      provide: {
        ...
        glFeatures: {
          dependencyPaths,
          groupDependencyDropdownsGraphql,
        },
      },
      ...options,
    });
  ...
  describe('when groupDependencyDropdownsGraphql feature flag is disabled', () => {
    let mockAxios;

    const restLocationsData = {
      locations: [
        {
          location: {
            blob_path: blobPath,
    ...
    it('fetches locations via REST', async () => {
      createComponent({
        mountFn: mountExtended,
        groupDependencyDropdownsGraphql: false,
      });
      mockAxios.onGet(endpoint).reply(HTTP_STATUS_OK, restLocationsData);
    ...
    it('normalizes REST response to camelCase', async () => {
      const restDependencyPathData = {
        locations: [
          {
            location: {},
            project: { name: projectName, full_path: projectFullPath },
      ...
      mockAxios.onGet(endpoint).reply(HTTP_STATUS_OK, restDependencyPathData);
      ...
      const locations = wrapper.emitted('click-dependency-path')[0][0];

      expect(locations[0]).toEqual(
        expect.objectContaining({
          hasDependencyPaths: true,
          occurrenceId: 1,
          project: { name: projectName, fullPath: projectFullPath },
        }),
      );
    });
  });
