...
describe('PaginatedDependenciesTable component', () => {
  ...
  const factory = ({
    ...
    glFeatures = { groupDependencyVulnerabilitiesGraphql: true },
  } = {}) => {
    store = createStore();
  ...
  describe('when namespaceType is group', () => {
    ...
    describe('fetching vulnerabilities', () => {
      it('dispatches fetchVulnerabilitiesViaGraphQL', async () => {
        const item = {};
        const table = wrapper.findComponent(DependenciesTable);
  ...
  describe('when namespaceType is group and groupDependencyVulnerabilitiesGraphql is disabled', () => {
    beforeEach(() => {
      factory({
        namespaceType: 'group',
        glFeatures: { groupDependencyVulnerabilitiesGraphql: false },
      });
      jest.spyOn(store, 'dispatch').mockImplementation();
    });

    it('dispatches fetchVulnerabilities via REST', async () => {
      const item = {};
      const table = wrapper.findComponent(DependenciesTable);
