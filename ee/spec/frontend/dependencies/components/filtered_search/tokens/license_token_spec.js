...
describe('ee/dependencies/components/filtered_search/tokens/license_token.vue', () => {
  ...
  const createComponent = ({
    propsData = {},
    glFeatures = { groupDependencyDropdownsGraphql: true },
  } = {}) => {
  ...
  describe('when the component is initially rendered', () => {
    ...
    it('fetches licenses via GraphQL', () => {
      createComponent();

      expect(store.dispatch).toHaveBeenCalledWith('fetchLicensesViaGraphQL');
    });
  ...
  describe('when groupDependencyDropdownsGraphql feature flag is disabled', () => {
    it('fetches the list of licenses from the REST endpoint', () => {
      createComponent({ glFeatures: { groupDependencyDropdownsGraphql: false } });

      expect(store.dispatch).toHaveBeenCalledWith('fetchLicenses', TEST_GROUP_LICENSES_ENDPOINT);
      expect(store.dispatch).not.toHaveBeenCalledWith('fetchLicensesViaGraphQL');
    });
  });
});
