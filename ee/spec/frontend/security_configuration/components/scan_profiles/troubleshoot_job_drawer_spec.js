...
describe('TroubleshootJobDrawer', () => {
  ...
  const findDuoButton = () => wrapper.findByTestId('duo-button');
  ...
  describe('when jobData is empty and needs to be fetched', () => {
    it('renders drawer after query resolves', async () => {
      createWrapper({ jobData: {} });
      await waitForPromises();
      wrapper.vm.fetchedJobData = mockJobData;
      await Vue.nextTick();
