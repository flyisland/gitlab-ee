...
import { GlTable, GlAlert, GlLink, GlDisclosureDropdownItem, GlLoadingIcon } from '@gitlab/ui';
...
describe('ScanProfileConfiguration', () => {
  ...
  const findFailureLoadingIcon = () => wrapper.findByTestId('failure-loading-icon');
  const findApplyProfileBtn = () => wrapper.findByTestId('apply-profile-button');
  const findDisableBtn = () => wrapper.findByTestId('disable-button');
  const findPreviewBtn = () => wrapper.findByTestId('preview-button');
  ...
  describe('with securityScanProfilesStatusIndicators feature flag', () => {
    ...
    describe('status cell (scanner health)', () => {
      ...
      it('shows "Awaiting first pipeline" when profile is attached but has no matched status', async () => {
        createComponentWithFlag({ statuses: [] });
        await waitForPromises();
        expect(wrapper.text()).toContain('Awaiting first pipeline');
      });

      it('shows correct status icon for pending status', async () => {
        createComponentWithFlag({ statuses: [] });
        await waitForPromises();
    ...
    describe('drawer', () => {
      ...
      it('updates isDrawerLoading when loading-changed event is emitted', async () => {
        createComponentWithFlag({ statuses: [mockStatusWithStatus('FAILED')] });
        await waitForPromises();
        ...
        findTroubleshootDrawer().vm.$emit('loading-changed', true);
        await nextTick();
        expect(findFailureLoadingIcon().exists()).toBe(true);

        findTroubleshootDrawer().vm.$emit('loading-changed', false);
        await nextTick();
        expect(findFailureLoadingIcon().exists()).toBe(false);
      });

      it('shows loading icon in troubleshoot dropdown item when drawer is loading', async () => {
        createComponentWithFlag({ statuses: [mockStatusWithStatus('FAILED')] });
        await waitForPromises();
        ...
        expect(troubleshootItem.findComponent(GlLoadingIcon).exists()).toBe(true);
      });
    });
  });
