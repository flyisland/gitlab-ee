...
describe('ScannerDetails', () => {
  ...
  const createComponent = ({
    ...
    wrapper = mountExtended(ScannerDetails, {
      ...
      provide: {
        ...
        securityInventoryPath: `/groups/path/to/group/-/security/inventory`,
        ...provide,
      },
      mocks: {
  ...
  describe('project table row', () => {
    ...
    describe('analyzer source', () => {
      ...
      it('renders the source label when the matching analyzer has a source', async () => {
        createComponent({
          projectsResolver: createProjectsResolver({
            nodes: [projectWithSource('SCAN_EXECUTION_POLICY')],
          }),
        });

        await waitForPromises();

        expect(findAnalyzerSource().text()).toBe('Scan execution policy');
      });

      it('does not render when the matching analyzer status has no source', async () => {
        createComponent({
          projectsResolver: createProjectsResolver({
            nodes: [projectWithSource(null)],
          }),
        });
      ...
      it('does not render when there is no analyzer status for the scanner', async () => {
        createComponent({
          projectsResolver: createProjectsResolver({
            nodes: [{ ...mockProject, analyzerStatuses: [] }],
          }),
