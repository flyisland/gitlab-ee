...
describe('DastScannerProfileForm', () => {
  ...
  const findProfileNameInput = () => wrapper.findComponentByTestId('profile-name-input');
  const findSpiderTimeoutInput = () => wrapper.findComponentByTestId('spider-timeout-input');
  const findTargetTimeoutInput = () => wrapper.findComponentByTestId('target-timeout-input');
  const findScanType = () => wrapper.findComponentByTestId('scan-type-option');
  ...
  `('$title', ({ profile, title, mutation, mutationVars, mutationKind }) => {
    ...
    it('populates the fields with the data passed in via the profile prop or default values', () => {
      ...
      expect(findScanType().props().checked).toBe(profile?.scanType ?? SCAN_TYPE.PASSIVE);

      expect(findSpiderTimeoutInput().element.value).toBe(String(profile?.spiderTimeout ?? 175));
