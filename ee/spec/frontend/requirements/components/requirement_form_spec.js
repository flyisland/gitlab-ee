    ...
    describe('default behavior', () => {
      ...
      describe('keyboard events', () => {
        it('handles enter+meta event', () => {
          ...
          findIssuableBody().vm.$emit('keydown-title', enterEvent, {
            issuableTitle: 'My title',
            issuableDescription: 'My desc',
          });

          expect(wrapper.emitted('save')).toEqual([
            [{ description: 'My desc', title: 'My title' }],
          ]);
        });

        it('handles esc event', () => {
          const escEvent = new KeyboardEvent('keydown', {
            key: 'Escape',
            code: 'Escape',
          });
          jest.spyOn(escEvent, 'stopPropagation');

          findIssuableBody().vm.$emit('keydown-title', escEvent);

          expect(wrapper.emitted('drawer-close')).toEqual([[]]);
          expect(escEvent.stopPropagation).toHaveBeenCalled();
        });

        it('handles a text input event', () => {
          ...
          findIssuableBody().vm.$emit('keydown-title', event);
  ...
  describe('existing requirement', () => {
    describe('default behavior', () => {
      ...
      it('returns requirement object while in show/edit mode', () => {
        expect(findIssuableBody().props('issuable')).toBe(mockRequirementsOpen[0]);
      });
