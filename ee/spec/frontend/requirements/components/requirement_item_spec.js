...
describe('RequirementItem', () => {
  ...
  describe('methods', () => {
    describe('handleArchiveClick', () => {
      it('emits `archive-click` event on component with object containing `requirement.iid` & `state` as "ARCHIVED" as param', async () => {
        wrapper.vm.handleArchiveClick();

        await nextTick();
        expect(wrapper.emitted()).toHaveProperty('archive-click');
        expect(wrapper.emitted('archive-click')[0]).toEqual([
          {
    ...
    describe('handleReopenClick', () => {
      it('emits `reopen-click` event on component with object containing `requirement.iid` & `state` as "OPENED" as param', async () => {
        wrapperArchived.find('.requirement-reopen').findComponent(GlButton).vm.$emit('click');

        await nextTick();
        expect(wrapperArchived.emitted()).toHaveProperty('reopen-click');
        expect(wrapperArchived.emitted('reopen-click')[0]).toEqual([
          {
  ...
  describe('template', () => {
    ...
    it('renders element containing requirement created at', () => {
      ...
      expect(createdAtEl.attributes('title')).toBe(
        localeDateFormat.asDateTimeFullWithWeekday.format(requirement1.createdAt),
      );
    });
    ...
    it('renders element containing requirement updated at', () => {
      ...
      expect(updatedAtEl.attributes('title')).toBe(
        localeDateFormat.asDateTimeFullWithWeekday.format(requirement1.updatedAt),
      );
    });
