...
describe('VulnerabilitiesByIdentifierPanel', () => {
  ...
  describe('when scope is "project"', () => {
    ...
    const reportTypeFilters = { reportType: ['SAST'] };
    ...
    beforeEach(() => {
      createComponent({
        ...
        props: { filters: reportTypeFilters },
    ...
    it('uses the project-level query with correct variables', () => {
      expect(vulnerabilitiesByIdentifierHandler).toHaveBeenCalledWith({
        fullPath: mockProjectFullPath,
        reportType: reportTypeFilters.reportType,
        severity: [],
      });
    });
  ...
  describe.each(['project', 'group'])('when scope is "%s"', (scopeType) => {
    it('uses the correct query for the scope', () => {
      const query =
        scopeType === 'project'
          ? projectVulnerabilitiesByIdentifier
          : groupVulnerabilitiesByIdentifier;
