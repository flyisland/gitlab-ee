...
describe('Project Security Dashboard (new version) - Component', () => {
    ...
    wrapper = shallowMountExtended(ProjectSecurityDashboardNew, {
      provide: {
        ...
        glFeatures: {
          ...
          newSecurityDashboardPdfExport: true,
          newSecurityDashboardVulnerabilitiesByIdentifier: true,
        },
        ...provide,
      },
    });
  ...
  describe('component rendering', () => {
    ...
    it('renders the risk score panel with the correct configuration', () => {
      ...
      expect(riskScorePanel.componentProps.filters).toEqual(undefined);
  ...
  describe('PDF export button', () => {
    it('renders the PDF export button when feature flag is enabled', () => {
      createComponent();
      expect(findPdfExportButton().exists()).toBe(true);
    });

    it('does not render the PDF export button when feature flag is disabled', () => {
      createComponent({ provide: { glFeatures: { newSecurityDashboardPdfExport: false } } });
      expect(findPdfExportButton().exists()).toBe(false);
    });
  });

  describe('when the newSecurityDashboardVulnerabilitiesByIdentifier feature flag is disabled', () => {
    it('does not render the panel when feature flag is disabled', () => {
      createComponent({
        provide: { glFeatures: { newSecurityDashboardVulnerabilitiesByIdentifier: false } },
      });

      expect(findVulnerabilitiesByIdentifierPanel()).toBeUndefined();
    });
  });
});
