...
describe('Project List component', () => {
  ...
  const getFirstRemoveButton = () =>
    getFirstProjectItem().findComponent('.js-projects-list-project-remove');
  ...
  describe('project item', () => {
    ...
    it(`emits a 'project-removed' event when a project's remove button has been clicked`, () => {
      getFirstRemoveButton().vm.$emit('click');

      expect(wrapper.emitted('project-removed')).toHaveLength(1);
      expect(wrapper.emitted('project-removed')[0][0]).toEqual(projects[0]);
    });
  });
});
