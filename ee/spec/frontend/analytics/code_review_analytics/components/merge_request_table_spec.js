...
describe('MergeRequestTable component', () => {
  ...
  const findMrDetailsCol = (rowIndex) => findTableRow(rowIndex).find('th[scope="row"]');
  const findReviewTimeCol = (rowIndex) => findTableRow(rowIndex).findAll('td').at(0);
  ...
  describe('template', () => {
    ...
    it('renders the correct table headers', () => {
      ...
      const headers = findTable().findAll('thead th');
    ...
    it('renders the merge request cell as a row header', () => {
      const rowHeader = findMrDetailsCol(0);

      expect(rowHeader.exists()).toBe(true);
      expect(rowHeader.text()).toContain(mockMergeRequests[0].title);
    });
