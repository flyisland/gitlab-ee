...
describe('DevopsAdoptionTable', () => {
  ...
  describe('delete modal integration', () => {
    beforeEach(async () => {
      ...
      await wrapper.findComponentByTestId('select-namespace').vm.$emit('click');
    });

    it('re emits track-modal-open-state with the given value', () => {
      findDeleteModal().vm.$emit('track-modal-open-state', true);

      expect(wrapper.emitted('track-modal-open-state')).toStrictEqual([[true]]);
    });
  });
