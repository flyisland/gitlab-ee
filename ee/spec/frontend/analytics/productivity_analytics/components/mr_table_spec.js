...
describe('MergeRequestTable component', () => {
  ...
  describe('column-metric-change', () => {
    it('emits the metric key when item is selected from the dropdown', async () => {
      ...
      await nextTick();
      expect(wrapper.emitted()['column-metric-change'][0]).toEqual(['time_to_first_comment']);
    });
  });

  describe('page-change', () => {
    it('emits the page number when the pagination changes', () => {
      factory({ ...defaultProps, pageInfo: { total: 20, page: 1, perPage: 10 } });
