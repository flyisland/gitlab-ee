...
describe('InventoryDashboardFilteredSearchBar', () => {
  ...
  describe('onFilter method', () => {
    ...
    it('lets the page change the tokens after the bar has mounted', () => {
      createComponent();

      expect(findFilteredSearchBar().props('syncFilterAndSort')).toBe(true);
    });

    it('offers a token for any type of secret detection', () => {
      const types = findFilteredSearchBar()
        .props('tokens')
        .map(({ type }) => type);

      expect(types).toContain('SECRET_DETECTION');
    });

    it('routes the combined secret detection token into the analyzer filters', async () => {
      findFilteredSearchBar().vm.$emit('on-filter', [
        {
          id: 'token-3',
