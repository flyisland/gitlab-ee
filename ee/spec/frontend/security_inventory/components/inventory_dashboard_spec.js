...
describe('InventoryDashboard', () => {
  ...
  describe('tool coverage card', () => {
    ...
    const createComponentWithCard = () =>
      createComponent({
        provide: {
          glFeatures: { ...defaultProvide.glFeatures, securityInventoryToolCoverageCard: true },
        },
      });
    ...
    it('is not rendered when the feature flag is disabled', async () => {
      await createComponent();

      expect(findToolCoverageCard().exists()).toBe(false);
    });

    it('is rendered for the group being viewed when the feature flag is enabled', async () => {
      await createComponentWithCard();

      expect(findToolCoverageCard().props('fullPath')).toBe('group/project');
    });

    it('follows the drill-down into a subgroup', async () => {
      getLocationHash.mockReturnValue('group/project/subgroup');
      await createComponentWithCard();
    ...
    it('starts with every scanner and no status selected', async () => {
      await createComponentWithCard();
    ...
    describe('when a status is selected on the card', () => {
      beforeEach(async () => {
        await createComponentWithCard();
      });

      it('narrows the project list to that scanner and status', async () => {
      ...
      it('narrows the project list on an aggregate boolean when every scanner is included', async () => {
      ...
      it('tells the table it is showing filtered results', async () => {
        await selectStatus({ scanner: 'ALL', status: 'STALE' });

        expect(findInventoryTable().props('hasSearch')).toBe(true);
      });

      it('does not refetch when the scanner changes with no status selected', async () => {
        await selectStatus({ scanner: 'DAST', status: null });

        expect(searchResolver).not.toHaveBeenCalled();
      });

      it('refetches when the scanner changes with a status selected', async () => {
      ...
      it('clears the selection when the search bar is used', async () => {
        await selectStatus({ scanner: 'DAST', status: 'FAILED' });
    ...
    describe('when the paginator drives the project list', () => {
      let resetSpy;
        ...
        await createComponent({
          provide: {
            glFeatures: {
              ...
              securityInventoryToolCoverageCard: true,
              securityInventoryPagination: true,
            },
          },
        });
      ...
      it('reloads the first page with the selected status', async () => {
        await selectStatus({ scanner: 'ALL', status: 'STALE' });

        expect(resetSpy).toHaveBeenCalledWith(expect.objectContaining({ hasStale: true }));
      });

      it('includes the selected status when it fetches a page', async () => {
        await selectStatus({ scanner: 'DAST', status: 'FAILED' });
        searchResolver.mockClear();

        await wrapper.vm.fetchSecurityProjects({ projectsFirst: 20 });
      ...
      it('does not reload when the scanner changes with no status selected', async () => {
        await selectStatus({ scanner: 'DAST', status: null });
