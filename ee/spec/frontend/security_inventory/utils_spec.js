...
describe('aggregateAnalyzerStatuses', () => {
  ...
  it('returns an empty object when no analyzer types are requested', () => {
    expect(aggregateAnalyzerStatuses([statusFor('SAST')], [])).toEqual({});
  });

  it('sums the counts of the requested analyzer types', () => {
    const statuses = [
  ...
  it('counts a requested analyzer type with no matching status as zero', () => {
  ...
  ])('counts %s once, ignoring the already-merged %s', (primary, merged) => {
  ...
  it('reads both secret detection types from the combined SECRET_DETECTION status', () => {
  ...
  it('reports the most recent update across the aggregated statuses', () => {
    const statuses = [
  ...
  it('tolerates missing analyzer statuses', () => {
    expect(aggregateAnalyzerStatuses(undefined, ['SAST'])).toMatchObject({ success: 0 });
  });

  it('returns no counts instead of throwing when every requested type is de-duplicated away', () => {
    expect(
...
describe('toolCoverageFilterVariables', () => {
  it('filters nothing when no status is selected', () => {
  ...
  it.each`
    scanner                  | analyzerType
    ${'DAST'}                | ${'DAST'}
    ${'DEPENDENCY_SCANNING'} | ${'DEPENDENCY_SCANNING'}
    ${'SAST_IAC'}            | ${'SAST_IAC'}
  `('filters $scanner on its own status column', ({ scanner, analyzerType }) => {
    expect(toolCoverageFilterVariables({ scanner, status: 'FAILED' })).toEqual({
      securityAnalyzerFilters: [{ analyzerType, status: 'FAILED' }],
    });
  });

  it.each`
    scanner                 | analyzerType            | reason
    ${'SAST'}               | ${'SAST'}               | ${'advanced SAST is counted under SAST'}
    ${'SECRET_DETECTION'}   | ${'SECRET_DETECTION'}   | ${'both secret detection types are combined'}
    ${'CONTAINER_SCANNING'} | ${'CONTAINER_SCANNING'} | ${'registry scanning is counted under container scanning'}
  `('filters $scanner on $analyzerType, because $reason', ({ scanner, analyzerType }) => {
    expect(toolCoverageFilterVariables({ scanner, status: 'SUCCESS' })).toEqual({
      securityAnalyzerFilters: [{ analyzerType, status: 'SUCCESS' }],
    });
  });

  it('filters nothing for a scanner it does not recognise', () => {
...
describe('filterQueryVariables', () => {
  ...
  it('defaults every filter when given nothing', () => {
  ...
  it('appends the card selection to the analyzer filters from the search bar', () => {
  ...
  it('spreads the aggregate variables the card selects instead of an analyzer filter', () => {
    expect(
  ...
  it('carries the search term, the count and attribute filters, and hasSearch', () => {
    expect(
      filterQueryVariables({
        filters: {
          search: 'gitlab',
  ...
  it('sends a status filter the search bar set, and null for the rest', () => {
