  ...
  describe '#execute' do
    ...
    context 'when group can be created' do
      ...
      it 'marks the group for Duo Code Review auto enable' do
        group = execute.payload[:group]

        expect(group.namespace_settings.enable_duo_code_review_by_default).to eq('pending')
      end
      ...
      context 'when the dap_code_review_default_on_new_signups feature flag is disabled' do
        ...
        it 'does not mark the group for Duo Code Review auto enable' do
          ...
          expect(group.namespace_settings.enable_duo_code_review_by_default).to eq('never')
        end
      end
