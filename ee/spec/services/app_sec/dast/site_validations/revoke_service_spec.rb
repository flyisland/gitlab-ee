...
RSpec.describe AppSec::Dast::SiteValidations::RevokeService, feature_category: :dynamic_application_security_testing do
  ...
  subject(:service_response) { described_class.new(container: project, params: params).execute }
  ...
  describe 'execute', :clean_gitlab_redis_shared_state do
    context 'when on demand scan licensed feature is not available' do
      it 'communicates failure' do
        ...
        aggregate_failures do
          ...
          expect(service_response.message).to eq('Insufficient permissions')
        end
      end
    end
    ...
    context 'when the feature is enabled' do
      ...
      it 'deletes only dast_site_validation validations in the same project that share a common url_base', :aggregate_failures do
        ...
        expect { service_response }.to change { DastSiteValidation.count }.from(total).to(delta)
      ...
      it 'returns a count of the dast_site_validations that were deleted' do
        expect(service_response.payload[:count]).to eq(dast_site_validations.size)
      end
      ...
      context 'when the finder does not find any dast_site_validations' do
        ...
        it 'is a noop' do
          aggregate_failures do
            expect(service_response.payload[:count]).to be_zero

            expect { service_response }.not_to change { DastSiteValidation.count }
          end
        end
      end
      ...
      context 'when a param is missing' do
        ...
        it 'communicates failure' do
          aggregate_failures do
            expect(service_response.status).to eq(:error)
            expect(service_response.message).to eq('URL parameter used to search for validations is missing')
          end
        end
      end
    end
