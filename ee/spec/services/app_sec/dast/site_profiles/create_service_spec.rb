...
RSpec.describe AppSec::Dast::SiteProfiles::CreateService, feature_category: :dynamic_application_security_testing do
  ...
  describe '#execute' do
    subject(:service_response) { described_class.new(project, user).execute(**params) }

    let(:status) { service_response.status }
    let(:message) { service_response.message }
    let(:payload) { service_response.payload }
    ...
    context 'when the user can run a dast scan' do
      ...
      it 'creates a dast_site_profile' do
        expect { service_response }.to change { DastSiteProfile.count }.by(1)
      end

      it 'creates a dast_site' do
        expect { service_response }.to change { DastSite.count }.by(1)
      end
      ...
      it 'audits the creation' do
        ...
        audit_event = AuditEventReader.find_by(author_id: user.id)
      ...
      context 'when the dast_site already exists' do
        ...
        it 'does not create a new dast_site' do
          expect { service_response }.not_to change { DastSite.count }
        end
      end
      ...
      context 'when an existing dast_site_validation does not exist' do
        it 'does not create a dast_site_validation association' do
          dast_site = service_response.payload.dast_site
