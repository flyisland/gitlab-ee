...
RSpec.describe AppSec::Dast::Profiles::DestroyService do
  ...
  let_it_be_with_refind(:dast_profile) { create(:dast_profile, project: project) }
  ...
  describe '#execute' do
    ...
    context 'when the feature is enabled' do
      ...
      context 'when the user can destroy a DAST profile' do
        ...
        it 'destroys the profile and audits the deletion', :aggregate_failures do
          expect { subject }.to change { Dast::Profile.count }.by(-1)

          profile = subject.payload

          expect(subject.status).to eq(:success)
          expect(profile).to be_a(Dast::Profile)
