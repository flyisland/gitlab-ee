  ...
  describe 'execute' do
    ...
    context 'when the feature is enabled' do
      ...
      context 'when user can run an on demand scan' do
        ...
        it 'communicates success and returns a pipeline and pipeline_url', :aggregate_failures do
          expect(response.status).to eq(:success)
          expect(response.payload[:pipeline]).to be_a(Ci::Pipeline)
          expect(response.payload[:pipeline_url]).to be_a(String)
        end
