...
RSpec.describe Members::ActivateService, feature_category: :groups_and_projects do
    ...
    context 'audit events' do
      context 'when licensed' do
        ...
        it 'tracks an audit event' do
          ...
          members.each do |member|
            audit_event = AuditEvent.find_by(target_id: member.user_id)
            ...
            expect(audit_event.details[:member_id]).to eq(member.id)
          end
        end

        context 'when member is invited by email and has no user' do
          let_it_be(:invited_member) { create(:group_member, :awaiting, :invited, group: root_group) }

          it 'tracks an audit event with fallback target and target details' do
            described_class.for_invite(
              root_group, invite_email: invited_member.invite_email
            ).execute(current_user: current_user, skip_authorization: true)

            audit_event = AuditEvent.find_by(target_id: nil)

            expect(audit_event.author).to eq(current_user)
            expect(audit_event.entity).to eq(root_group)
            expect(audit_event.target_details).to eq('Activated Member')
            expect(audit_event.target_type).to be_nil
            expect(audit_event.details[:custom_message]).to eq('Changed the membership state to active')
            expect(audit_event.details[:member_id]).to eq(invited_member.id)
          end
        end
      end
  ...
  shared_examples 'when a user has memberships with invalid access levels' do
    let_it_be(:member, freeze: false) { create(:group_member, :awaiting, :developer, group: sub_group, user: user) }
  ...
  describe '.for_invite' do
    ...
    let_it_be(:expected_members, freeze: false) do
      [
  ...
  describe '#execute' do
    ...
    context 'when skipping authorization' do
      ...
      let_it_be(:members, freeze: false) { [create(:group_member, :developer, :awaiting, group: root_group, user: user)] }
    ...
    context 'when authorized' do
      ...
      context 'when there are other awaiting members' do
        ...
        context 'with .for_invite' do
          ...
          let_it_be(:invite_email) { 'test@example.com' }
          let_it_be(:member, freeze: false) { create(:group_member, :awaiting, :invited, group: root_group, invite_email: invite_email) }
        ...
        context 'with .for_users' do
          ...
          let_it_be(:member, freeze: false) { create(:group_member, :awaiting, group: root_group, user: user) }
      ...
      context 'when users are no awaiting members' do
        let_it_be(:members, freeze: false) { [create(:group_member, group: root_group, user: user)] }

        it_behaves_like 'returns an error', 'No memberships found'
      end

      context 'when the members are only invites' do
        let_it_be(:members, freeze: false) { [create(:group_member, :invited, group: root_group)] }
