...
RSpec.describe Analytics::CycleAnalytics::NamespaceAggregatorService, feature_category: :value_stream_management do
  ...
  context 'when the aggregation succeeds' do
    ...
    context 'when merge requests and issues are present for the configured VSA stages' do
      ...
      let(:mr_source_project) { create(:project, :small_repo, namespace: group) }
      let!(:merge_request) do
        create(:merge_request, :with_merged_metrics,
          project: project, source_project: mr_source_project)
      end
      ...
      before do
        create_default(:cycle_analytics_value_stream, namespace: group)
