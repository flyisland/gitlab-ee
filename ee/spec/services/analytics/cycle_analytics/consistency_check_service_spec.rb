...
RSpec.describe Analytics::CycleAnalytics::ConsistencyCheckService, :aggregate_failures, feature_category: :value_stream_management do
  ...
  context 'for issue based stage' do
    ...
    let_it_be_with_refind(:record1) { create(:issue, :closed, project: project1, created_at: 2.months.ago) }
    let_it_be(:record2) { create(:issue, :closed, project: project2, created_at: 1.month.ago) }
    let_it_be_with_refind(:record3) { create(:issue, :closed, project: project2, created_at: 1.month.ago) }
    let_it_be_with_refind(:record4) { create(:issue, project: project2, created_at: 1.month.ago) } # no closed date, so there is no end event

    let_it_be(:stage) { create(:cycle_analytics_stage, namespace: group, start_event_identifier: :issue_created, end_event_identifier: :issue_closed) }
  ...
  context 'for merge request based stage' do
    ...
    let_it_be_with_refind(:record1) { create(:merge_request, :closed_last_month, project: project1, created_at: 3.months.ago) }
    let_it_be(:record2) { create(:merge_request, :closed_last_month, project: project2, created_at: 2.months.ago) }
    let_it_be_with_refind(:record3) { create(:merge_request, :closed_last_month, project: project2, created_at: 2.months.ago) }
    let_it_be_with_refind(:record4) { create(:merge_request, :unique_branches, source_project: project2, target_project: project2, created_at: 2.months.ago) }

    let_it_be(:stage) { create(:cycle_analytics_stage, namespace: group, start_event_identifier: :merge_request_created, end_event_identifier: :merge_request_closed) }
