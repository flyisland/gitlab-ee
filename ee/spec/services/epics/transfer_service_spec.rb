...
RSpec.describe Epics::TransferService, feature_category: :portfolio_management do
  describe '#execute' do
    ...
    let_it_be(:old_group, freeze: false) { create(:group, maintainers: user) }

    subject(:service) { described_class.new(user, old_group, project) }

    context 'when old_group is present' do
      let_it_be(:project, freeze: false) { create(:project, namespace: old_group, maintainers: user) }
      let_it_be(:epic, freeze: false) { create(:epic, group: old_group, title: 'Epic 1') }
      let_it_be(:other_epic, freeze: false) { create(:epic, group: old_group, title: 'Epic 2') }
      let_it_be(:issue1_with_epic, freeze: false) { create(:issue, project: project, epic: epic) }
      let_it_be(:issue2_with_epic, freeze: false) { create(:issue, project: project, epic: epic) }
      let_it_be(:issue_with_other_epic, freeze: false) { create(:issue, project: project, epic: other_epic) }
      ...
      context 'when user can create epics in the new group' do
        ...
        context 'when epic is from an descendant group' do
          let_it_be(:old_group_subgroup, freeze: false) { create(:group, parent: old_group) }
        ...
        context 'when epic is from an ascending group' do
          let_it_be(:new_group, freeze: false) { create(:group, parent: old_group) }
    ...
    context 'when old_group is not present' do
      let_it_be(:project, freeze: false) { create(:project, namespace: create(:namespace)) }
      let_it_be(:old_group, freeze: false) { nil }
    ...
    context 'when project group is not present' do
      let_it_be(:project, freeze: false) { create(:project, group: old_group) }
