...
RSpec.describe HistoricalUserData::CsvService, feature_category: :user_management do
  ...
  let_it_be(:csv_service, freeze: false) { described_class.new(HistoricalData.all) }

  let_it_be(:datetime, freeze: false) { DateTime.parse('2020-09-04 10:00:00') }
  let_it_be(:license_email, freeze: false) { 'data@example.com' }
  let_it_be(:license_company, freeze: false) { 'Widgets Inc.' }
  let_it_be(:license_start_date, freeze: false) { datetime.to_date - 10.months }
  let_it_be(:license_end_date, freeze: false) { datetime.to_date + 2.months }

  let_it_be(:current_license, freeze: false) do
  ...
  context 'User Count Table' do
    let_it_be(:historical_datum, freeze: false) do
      create(:historical_data, recorded_at: license_start_date, active_user_count: 1)
    end

    let_it_be(:historical_datum2, freeze: false) do
