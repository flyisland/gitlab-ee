    ...
    context 'when organization is provided' do
      ...
      before do
        allow(ProjectsFinder)
          ...
          .with(
            current_user: user,
            project_ids_relation: input,
            params: { organization: organization, min_access_level: ProjectMember::DEVELOPER }
          )
          .and_return(projects_finder)
      end

      it 'passes the organization to ProjectsFinder' do
        expect(ProjectsFinder)
          .to receive(:new)
          .with(
            current_user: user,
            project_ids_relation: input,
      ...
      context 'when user has :read_all_resources permission (admin)' do
        before do
          allow(user).to receive(:can?).with(:read_all_resources).and_return(true)
          allow(ProjectsFinder)
            .to receive(:new)
            .with(
              current_user: user,
        ...
        it 'passes only the organization to ProjectsFinder, without min_access_level' do
          expect(ProjectsFinder)
            .to receive(:new)
            .with(
              current_user: user,
