    ...
    context 'when upload registry record exists' do
      context 'with file on local storage' do
        ...
        context 'when file does not exist on disk' do
          ...
          let_it_be(:registry) { create(:geo_upload_registry, file_id: upload.id) }
      ...
      context 'with file on remote storage' do
        let_it_be(:upload) { create(:upload, :issuable_upload, :object_storage, model: project) }
        let_it_be(:registry) { create(:geo_upload_registry, file_id: upload.id) }
          ...
          context 'when file does not exist on object storage' do
            context 'when GitLab managed replication is enabled' do
              ...
              context 'when object storage returns 403 Forbidden (missing ListBucket permission)' do
                before do
                  allow_next_instance_of(Fog::Storage::AWS::Files) do |files|
                    allow(files).to receive(:head)
                                      .and_raise(Excon::Error::Forbidden, 'Expected(200) <=> Actual(403 Forbidden)')
                  end
                end

                it 'does not remove the file and does not raise' do
                ...
                it 'tracks the exception' do
                  expect(Gitlab::ErrorTracking).to receive(:track_exception).with(
                    ...
                    hash_including(object_type: :upload, object_db_id: upload.id)
                  )
                ...
                it 'logs a warning' do
                  expect_to_log_a_message_with(
                    upload.id,
                    'Received Forbidden error when checking if file exists in object storage. ' \
