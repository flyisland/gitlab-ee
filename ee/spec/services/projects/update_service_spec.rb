...
RSpec.describe Projects::UpdateService, '#execute', feature_category: :groups_and_projects do
  ...
  context 'shared runners', :saas do
    ...
    let_it_be(:group, freeze: false) { create(:group_with_plan, plan: :free_plan, trial_ends_on: Date.today + 1.day) }
  ...
  context 'repository_size_limit assignment as Bytes' do
    let_it_be(:project, freeze: false) { create(:project, repository_size_limit: 0) }
  ...
  context 'updating analytics_dashboards_pointer_attributes.target_project_id param' do
    let_it_be(:user) { create :user }
    let_it_be(:group, freeze: false) { create(:group) { |g| g.add_owner(user) } }
  ...
  describe 'when updating duo_features_enabled' do
    ...
    let_it_be(:integration, freeze: false) { create(:amazon_q_integration, instance: false, project: project, auto_review_enabled: true) }
  ...
  describe 'when updating duo_sast_vr_workflow_enabled' do
    ...
    let(:opts) { { project_setting_attributes: { duo_sast_vr_workflow_enabled: true } } }
    ...
    context 'when the user is a security manager with the SAST VR ability' do
      ...
      context 'when update_sast_vr_setting_permission FF is disabled' do
        before do
        ...
        it 'silently drops the attribute' do
          ...
          expect(project.reload.project_setting.duo_sast_vr_workflow_enabled).to eq(false)
        end
      end

      context 'when enable_vulnerability_resolution FF is disabled' do
        before do
