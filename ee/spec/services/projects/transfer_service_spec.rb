  ...
  describe 'deleting compliance statuses' do
    ...
    context 'when transferring to other group' do
      ...
      context 'and project has a secret manager' do
        ...
        it 'calls the deprovision service with snapshot ids captured from the old namespace' do
          ...
          old_namespace = deprovision_project.namespace

          expect(SecretsManagement::ProjectSecretsManagers::InitiateDeprovisionService).to receive(:new).with(
            secrets_manager,
            user,
            project_id: deprovision_project.id,
