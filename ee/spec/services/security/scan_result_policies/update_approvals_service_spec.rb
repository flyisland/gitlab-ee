    ...
    context 'with atomic scanner rule criteria' do
      ...
      context 'when related pipeline ids are blank' do
        ...
        it 'excludes related_pipeline_ids from source evaluator params' do
          ...
          allow(Security::ScanResultPolicies::GroupedFindingsEvaluator).to receive(:new).and_wrap_original do |m, *args|
            source_params ||= args.last if args.last[:check_dismissed]
            m.call(*args)
          end

          execute

          expect(source_params).not_to include(:related_pipeline_ids)
        end
      end
