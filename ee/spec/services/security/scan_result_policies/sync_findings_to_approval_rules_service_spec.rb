  ...
  describe '#execute' do
    ...
    context 'when pipeline_findings is not empty' do
      ...
      context 'with merge request targeting pipeline ref' do
        ...
        let_it_be(:other_pipeline, freeze: false) do
          create(
            :ee_ci_pipeline,
            project: project,
    ...
    describe 'policy sync state tracking' do
      ...
      context 'when SyncMergeRequestApprovalsWorker is deduplicated' do
        before do
          allow(Security::ScanResultPolicies::SyncMergeRequestApprovalsWorker)
            .to receive(:perform_async).and_return(nil)
        end

        it 'does not increment the policy sync worker counter for update_approvals path' do
          expect { execute }.not_to change { state.total_merge_request_workers_count(merge_request.id) }
        end

        context 'with merge request targeting pipeline ref' do
          let_it_be(:other_merge_request, freeze: false) do
            create(:merge_request, source_project: project, target_branch: merge_request.source_branch,
              source_branch: "feature")
          end

          let_it_be(:other_pipeline, freeze: false) do
            create(:ee_ci_pipeline, project: project, ref: other_merge_request.source_branch,
              sha: project.commit(other_merge_request.source_branch).sha,
              merge_requests_as_head_pipeline: [other_merge_request])
          end

          it 'does not increment the policy sync worker counter for merge_requests_targeting_pipeline_ref path' do
            expect { execute }.not_to change { state.total_merge_request_workers_count(other_merge_request.id) }
          end
        end
      end
    end
