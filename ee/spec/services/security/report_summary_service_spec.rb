...
RSpec.describe Security::ReportSummaryService, '#execute', feature_category: :vulnerability_management do
  ...
  let_it_be_with_reload(:build_ds) { create(:ci_build, :success, name: 'dependency_scanning', pipeline: pipeline) }
  let_it_be_with_reload(:artifact_ds) { create(:ee_ci_job_artifact, :dependency_scanning, job: build_ds) }
  let_it_be_with_reload(:report_ds) { create(:ci_reports_security_report, type: :dependency_scanning) }
  let_it_be_with_reload(:scan_ds) { create(:security_scan, scan_type: :dependency_scanning, build: build_ds) }

  let_it_be_with_reload(:build_sast) { create(:ci_build, :success, name: 'sast', pipeline: pipeline) }
  let_it_be_with_reload(:artifact_sast) { create(:ee_ci_job_artifact, :sast, job: build_sast) }
  let_it_be_with_reload(:report_sast) { create(:ci_reports_security_report, type: :sast) }
  let_it_be_with_reload(:scan_sast) { create(:security_scan, scan_type: :sast, build: build_sast) }

  let_it_be_with_reload(:build_dast) { create(:ci_build, :success, name: 'dast', pipeline: pipeline) }
  let_it_be_with_reload(:artifact_dast) { create(:ee_ci_job_artifact, :dast_large_scanned_resources_field, job: build_dast) }
  let_it_be_with_reload(:report_dast) { create(:ci_reports_security_report, type: :dast) }
  let_it_be_with_reload(:scan_dast) { create(:security_scan, scan_type: :dast, build: build_dast) }

  let_it_be_with_reload(:build_cs) { create(:ci_build, :success, name: 'container_scanning', pipeline: pipeline) }
  let_it_be_with_reload(:artifact_cs) { create(:ee_ci_job_artifact, :container_scanning, job: build_cs) }
  let_it_be_with_reload(:report_cs) { create(:ci_reports_security_report, type: :container_scanning) }
  let_it_be_with_reload(:scan_cs) { create(:security_scan, scan_type: :container_scanning, build: build_cs) }
