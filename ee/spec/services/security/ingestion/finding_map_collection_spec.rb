...
RSpec.describe Security::Ingestion::FindingMapCollection, feature_category: :vulnerability_management do
  describe '#each_slice' do
    ...
    context 'when the size argument given' do
      ...
      context 'with a security finding generated by the GitLab SBOM Vulnerability Scanner' do
        let_it_be(:sbom_scanner) { create(:vulnerabilities_scanner, :sbom_scanner, project: security_scan.project) }
        let_it_be(:ds_finding) do
          create(
            :security_finding,
            uuid: '19155bf3-6a79-446e-8891-5faed482600b',
            deduplicated: true,
            scan: security_scan,
        ...
        let_it_be(:ds_report_finding) { create(:ci_reports_security_finding, uuid: ds_finding.uuid) }

        before do
          report_findings << ds_report_finding
        end

        it 'includes the security findings' do
          run_each_slice
