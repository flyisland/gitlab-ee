...
RSpec.describe Security::Ingestion::IngestReportsService, feature_category: :vulnerability_management do
  ...
  describe '#execute' do
    ...
    it 'calls MarkAsResolvedService for each scanner and scan type', :aggregate_failures do
      expect(Security::Ingestion::MarkAsResolvedService).to receive(:execute).once.with(pipeline, sast_scanner, ids_1, 'sast')
      expect(Security::Ingestion::MarkAsResolvedService).to receive(:execute).once.with(pipeline, sbom_scanner, ids_2, 'dependency_scanning')
      expect(Security::Ingestion::MarkAsResolvedService).to receive(:execute).once.with(pipeline, cs_scanner, ids_3, 'container_scanning')
