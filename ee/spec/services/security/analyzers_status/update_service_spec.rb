  ...
  describe '#execute' do
    ...
    context 'when pipeline and project are present' do
      context 'when feature flag is disabled' do
        before do
          stub_feature_flags(analyzers_status_from_security_scans: false)
        end
        ...
        context 'when project has security scan profiles' do
          context 'when sast profile exists and sast job is not in the pipeline' do
            let_it_be(:sast_profile) do
              create(:security_scan_profile, :sast, namespace: root_group, projects: [project])
            end

            let!(:dast_build) { create(:ci_build, :dast, :success, pipeline: pipeline) }
            ...
            it 'preserves existing sast status' do
              ...
              expect { execute }.not_to change { existing_sast_status.reload.status }
            end
            ...
            it 'preserves existing sast_advanced status' do
              existing_sast_advanced_status = create(:analyzer_project_status, project: project,
                analyzer_type: :sast_advanced, status: :success)

              expect { execute }.not_to change { existing_sast_advanced_status.reload.status }
            end

            it 'updates dast status from the pipeline' do
              existing_dast_status = create(:analyzer_project_status, project: project, analyzer_type: :dast,
                status: :not_configured)

              expect { execute }.to change { existing_dast_status.reload.status }.from('not_configured').to('success')
            end
          end

          context 'when dependency_scanning profile exists and dependency_scanning job is not in the pipeline' do
            let!(:dast_build) { create(:ci_build, :dast, :success, pipeline: pipeline) }
            let!(:ds_profile) do
              create(:security_scan_profile, namespace: root_group, scan_type: :dependency_scanning,
                projects: [project])
            end

            it 'preserves existing dependency_scanning status' do
              existing_ds_status = create(:analyzer_project_status, project: project,
                analyzer_type: :dependency_scanning, status: :success)

              expect { execute }.not_to change { existing_ds_status.reload.status }
            end
          end

          context 'when sast profile exists with no dast job' do
        ...
        context 'when no security jobs are found' do
          ...
          it 'preserves pipeline analyzer statuses and increments their consecutive_absence_count' do
            sast_status = create(:analyzer_project_status, project: project, analyzer_type: :sast,
              status: :success, consecutive_absence_count: 0)
          ...
          it 'creates aggregated type status from preserved pipeline-based status when no record exists' do
        ...
        context 'with consecutive absence tracking' do
          ...
          context 'when an absent analyzer has consecutive_absence_count below threshold' do
            let!(:sast_status) do
              create(:analyzer_project_status, project: project, analyzer_type: :sast,
                status: :success, consecutive_absence_count: 1)
            end
          ...
          context 'when an already-stale analyzer is absent again' do
            let!(:sast_status) do
              create(:analyzer_project_status, project: project, analyzer_type: :sast,
                status: :stale, consecutive_absence_count: 3)
            end
          ...
          context 'when a stale analyzer is ran again' do
            let!(:sast_build) { create(:ci_build, :sast, build_status, pipeline: pipeline) }
            let_it_be_with_reload(:sast_status) do
              create(:analyzer_project_status, project: project, analyzer_type: :sast,
                status: :stale, consecutive_absence_count: 3)
            end
            ...
            context 'when succeeded' do
              ...
              it 'resets consecutive_absence_count to 0 and updates status from the build' do
                expect { execute }
            ...
            context 'when failed' do
              ...
              it 'resets consecutive_absence_count to 0 and updates status from the build' do
                expect { execute }
                  .to change { sast_status.reload.status }.from('stale').to('failed')
                  .and change { sast_status.reload.consecutive_absence_count }.from(3).to(0)
              end
            end
          end
          ...
          context 'when an absent analyzer preserves its last run reference' do
            ...
            context 'when counting towards stale' do
              let!(:sast_status) do
                create(:analyzer_project_status, project: project, analyzer_type: :sast,
                  status: :success, consecutive_absence_count: 1,
                  build: previous_sast_build, last_call: 1.week.ago)
              end
          ...
          context 'when a PIPELINE_EXCLUDED_TYPE is absent from the pipeline' do
            let!(:sd_aggregated_status) do
              create(:analyzer_project_status, project: project,
                analyzer_type: :secret_detection, status: :success, consecutive_absence_count: 2)
            end

            it 'does not increment consecutive_absence_count or change status' do
              expect { execute }
                .to not_change { sd_aggregated_status.reload.consecutive_absence_count }
                .and not_change { sd_aggregated_status.reload.status }
            end
          end

          context 'when a profile-covered type is absent from the pipeline' do
            let!(:sast_profile) { create(:security_scan_profile, :sast, namespace: root_group, projects: [project]) }
            let!(:sast_status) do
              create(:analyzer_project_status, project: project,
                analyzer_type: :sast, status: :success, consecutive_absence_count: 2)
            end

            it 'does not increment consecutive_absence_count or change status' do
              expect { execute }
                .to not_change { sast_status.reload.consecutive_absence_count }
                .and not_change { sast_status.reload.status }
            end
          end
        end
        ...
        context 'with aggregated type handling' do
          ...
          context 'for container_scanning aggregated type' do
            ...
            context 'when container_scanning_pipeline_based transitions to stale' do
              let!(:container_scanning_build) { nil }
              let!(:dast_build) { create(:ci_build, :dast, :success, pipeline: pipeline) }
              ...
              let_it_be_with_reload(:cs_aggregated_status) do
                create(:analyzer_project_status, project: project,
                  analyzer_type: :container_scanning, status: :success)
              end
              ...
              context 'when container_scanning_for_registry is failed' do
                ...
                it 'keeps container_scanning aggregated status as failed' do
      ...
      context 'when feature flag is enabled' do
        before do
          stub_feature_flags(analyzers_status_from_security_scans: project)
        end
        ...
        context 'when a scan has a skipped status' do
          let!(:created_scan) do
            create(:security_scan, scan_type: :sast, pipeline: pipeline, project: project,
              latest: true, status: :created)
          end

          let!(:preparing_scan) do
            create(:security_scan, scan_type: :dast, pipeline: pipeline, project: project,
              latest: true, status: :preparing)
          end

          let!(:purged_scan) do
            create(:security_scan, scan_type: :dependency_scanning, pipeline: pipeline, project: project,
              latest: true, status: :purged)
          end
        ...
        context 'when scan_type has no corresponding analyzer group' do
          let!(:sarif_scan) do
            create(:security_scan, scan_type: :sarif, pipeline: pipeline, project: project,
        ...
        context 'when a secret_detection scan succeeds' do
        ...
        context 'when a container_scanning scan succeeds' do
          ...
          include_examples 'creates aggregated status from pipeline-based status',
            :container_scanning, :container_scanning_pipeline_based, :success
          ...
          it 'creates the pipeline-based row' do
            ...
            expect(Security::AnalyzerProjectStatus.find_by(project: project,
              analyzer_type: :container_scanning_pipeline_based))
              .to have_attributes(status: 'success', build_id: container_scanning_scan.build_id)
          end
        ...
        context 'when multiple sast scans exist with different statuses' do
          ...
          let!(:success_scan) do
            create(:security_scan, scan_type: :sast, pipeline: pipeline, project: project,
              build: success_build, latest: true, status: :succeeded)
          end

          let!(:failed_scan) do
            create(:security_scan, scan_type: :sast, pipeline: pipeline, project: project,
              build: failed_build, latest: true, status: :job_failed)
          end
        ...
        context 'with various security scans' do
          ...
          let!(:dependency_scanning_scan) do
            create(:security_scan, scan_type: :dependency_scanning, pipeline: pipeline, project: project,
          ...
          let!(:container_scanning_scan) do
            create(:security_scan, scan_type: :container_scanning, pipeline: pipeline, project: project,
              latest: true, status: :report_error)
          end
          ...
          let!(:kics_build) { create(:ci_build, :success, pipeline: pipeline, name: 'kics-iac-sast-0') }
          let!(:kics_scan) do
            create(:security_scan, scan_type: :sast, pipeline: pipeline, project: project,
              build: kics_build, latest: true, status: :succeeded)
          end
          ...
          let!(:advanced_sast_scan) do
            create(:security_scan, scan_type: :sast, pipeline: pipeline, project: project,
              build: advanced_sast_build, latest: true, status: :succeeded)
          end
