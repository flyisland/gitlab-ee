      ...
      context 'when project is licensed', :clean_gitlab_redis_shared_state do
        ...
        context 'when operation is valid' do
          ...
          context 'when a risk_severity rule is exceeded' do
            ...
            it 'names the policy and the exceeded severity on one line', :aggregate_failures do
              expect(firewall_check).to be_error
              expect(firewall_check.message).to eq(
                "Package '#{name}' violates 'deny-high-volume' policy: vulnerability count threshold exceeded (high)"
              )
              expect(firewall_check.message).not_to include("\n")
            end

            it 'reveals neither the observed count nor the policy threshold' do
              severity_fragment = firewall_check.message.split('policy: ').last

              expect(severity_fragment).not_to match(/\d/)
            end
          end
          ...
          context 'when two severities are exceeded' do
            ...
            it 'names both, critical first' do
              expect(firewall_check.message).to eq(
                "Package '#{name}' violates 'strict' policy: vulnerability count thresholds exceeded (critical, high)"
              )
            end
          end
          ...
          context 'when a warn-mode risk_severity rule is exceeded' do
            ...
            it 'renders the same message on the warning payload', :aggregate_failures do
              ...
              expect(firewall_check.payload[:message]).to eq(
                "Package '#{name}' violates 'warn-high-volume' policy: vulnerability count threshold exceeded (high)"
              )
            end
          end
