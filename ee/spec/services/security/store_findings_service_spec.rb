...
RSpec.describe Security::StoreFindingsService, feature_category: :vulnerability_management do
  ...
  describe '#execute' do
    ...
    context 'when the given security scan does not have any findings' do
      ...
      context 'when security_findings_effective_severity feature flag is enabled' do
        ...
        before do
          stub_feature_flags(security_findings_effective_severity: true)
          security_scan.findings.delete_all
        end
      ...
      context 'when security_findings_effective_severity feature flag is disabled' do
        before do
          stub_feature_flags(security_findings_effective_severity: false)
          security_scan.findings.delete_all
        end

        it 'stores scanner_reported_severity as the scanner severity' do
          store_findings

          findings = security_scan.findings.reload
          findings.each do |finding|
            report_finding = [security_finding_1, security_finding_2, security_finding_3].find do |f|
              f.uuid == finding.uuid
            end
            expect(finding.scanner_reported_severity).to eq(report_finding.severity.to_s)
          end
        end

        it 'uses scanner severity for the severity column' do
          store_findings
          ...
          findings.each do |finding|
            ...
            expect(finding.severity).to eq(report_finding.severity.to_s)
          end
        end
      end
