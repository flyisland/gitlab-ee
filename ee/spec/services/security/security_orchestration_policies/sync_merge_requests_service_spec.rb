...
RSpec.describe Security::SecurityOrchestrationPolicies::SyncMergeRequestsService, feature_category: :security_policy_management do
  ...
  shared_examples 'tracks policy sync worker counter' do |worker_class:|
    include_context 'with policy sync state'

    let(:policy_configuration) { group_policy_configuration }

    before do
      stub_csp_group(group_policy_configuration.namespace)
    end

    let(:all_tracked_workers) do
      [
        ::Security::ScanResultPolicies::SyncAnyMergeRequestApprovalRulesWorker,
        ::Security::ScanResultPolicies::SyncPreexistingStatesApprovalRulesWorker,
        ::Security::UnenforceablePolicyRulesNotificationWorker
      ]
    end

    context 'when only the worker under test is enqueued', :clean_gitlab_redis_queues do
      before do
        (all_tracked_workers - [worker_class]).each { |w| allow(w).to receive(:perform_async).and_return(nil) }
      end

      it 'increments the counter by 1 for each MR' do
        execute

        expect(state.total_merge_request_workers_count(opened_merge_request.id)).to eq(1)
        expect(state.total_merge_request_workers_count(draft_merge_request.id)).to eq(1)
      end
    end

    context 'when the worker is deduplicated' do
      before do
        all_tracked_workers.each { |w| allow(w).to receive(:perform_async).and_return(nil) }
      end

      it 'does not increment the policy sync worker counter' do
        execute

        expect(state.total_merge_request_workers_count(opened_merge_request.id)).to eq(0)
        expect(state.total_merge_request_workers_count(draft_merge_request.id)).to eq(0)
      end
    end
  end
  ...
  describe "#execute" do
    ...
    describe '#notify_for_policy_violations' do
      ...
      it_behaves_like 'tracks policy sync worker counter',
        worker_class: ::Security::UnenforceablePolicyRulesNotificationWorker
    end
    ...
    context "when scan_result_policy_read targets commits" do
      ...
      it_behaves_like 'tracks policy sync worker counter',
        worker_class: ::Security::ScanResultPolicies::SyncAnyMergeRequestApprovalRulesWorker
    end
    ...
    context 'when merge request has scan_finding rules' do
      ...
      it_behaves_like 'tracks policy sync worker counter',
        worker_class: ::Security::ScanResultPolicies::SyncPreexistingStatesApprovalRulesWorker
    end
