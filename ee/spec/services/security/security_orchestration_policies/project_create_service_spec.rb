...
RSpec.describe Security::SecurityOrchestrationPolicies::ProjectCreateService, feature_category: :security_policy_management do
  describe '#execute' do
    ...
    context 'when user is owner in the project and can create projects in the namespace' do
      ...
      before do
        ...
        group.add_developer(developer)
        project.add_owner(developer)
      end

      it 'successfully create the security policy project without errors' do
        response = service.execute

        expect(response).to include(status: :success)
        expect(project.reload.security_orchestration_policy_configuration.security_policy_management_project).to eq(response[:policy_project])
      end
    end
    ...
    context 'when user does not have permission to create project in container' do
      let(:group) { create(:group, project_creation_level: Gitlab::Access::NO_ACCESS) }
      let(:container) { group }
