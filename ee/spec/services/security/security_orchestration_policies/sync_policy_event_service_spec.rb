  ...
  describe '#execute' do
    context 'when event is ComplianceFrameworkChangedEvent' do
      ...
      shared_examples 'when policy scope does not match compliance_framework' do
        context 'when convergent_policy_rule_sync is disabled' do
          before do
            stub_feature_flags(convergent_policy_rule_sync: false)
          end
      ...
      context 'when event compliance_framework_id does not match policy scope but policy is applicable' do
        let_it_be(:unrelated_framework) { create(:compliance_framework) }
        ...
        context 'when convergent_policy_rule_sync is disabled' do
          ...
          it 'does nothing because event framework does not match policy scope' do
            expect { execute }.not_to change { Security::PolicyProjectLink.count }
          end
        end
      end
    ...
    context 'with protected branches event' do
      ...
      context 'when event is ProtectedBranchCreatedEvent' do
        ...
        context 'when there are no affected rules' do
          it 'does nothing' do
            expect(service).not_to receive(:sync_approval_policy_rules)
          end
        end
        ...
        context 'when there are affected rules' do
          ...
          before do
            stub_feature_flags(convergent_policy_rule_sync: false)
            allow(service).to receive(:approval_policy_protected_branch_ids)
              .with(rules.first).and_return([protected_branch.id])
            allow(service).to receive(:approval_policy_protected_branch_ids).with(rules.last).and_return([])
          end

          it 'updates the rules' do
            execute

            expect(service).to have_received(:sync_approval_policy_rules).with([rules.first])
          end
      ...
      context 'when event is ProtectedBranchDestroyedEvent' do
        ...
        context 'when there are affected rules' do
          ...
          it 'updates the rules' do
            ...
            expect(service).to have_received(:sync_approval_policy_rules).with(rules)
          end
    ...
    context 'when event is DefaultBranchChangedEvent' do
      ...
      context 'when convergent_policy_rule_sync is disabled' do
        ...
        it 'updates all undeleted rules' do
          ...
          expect(service).to have_received(:sync_approval_policy_rules).with(undeleted_rules)
        end
      end
      ...
      context 'when policy scope is applicable to the project' do
        ...
        context 'when convergent_policy_rule_sync is disabled' do
          ...
          it 'calls sync_approval_policy_rules with specific rules instead of converge_policy' do
            execute

            expect(service).to have_received(:sync_approval_policy_rules).with(undeleted_rules)
            expect(service).not_to have_received(:link_policy)
          end
        end
      end
      ...
      context 'when policy scope is inapplicable to the project' do
        ...
        context 'when convergent_policy_rule_sync is disabled' do
          ...
          it 'syncs rules without checking scope applicability' do
            expect { execute }.not_to change { Security::PolicyProjectLink.count }
          end
        end
      end
