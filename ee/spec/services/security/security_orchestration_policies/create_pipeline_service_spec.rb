  ...
  describe '#execute' do
    ...
    describe "#execute" do
      ...
      context "with scan pipeline actions" do
        ...
        it 'invokes Ci::CreatePipelineService with a scan_execution_policy_context_block' do
          expect_next_instance_of(Ci::CreatePipelineService, project, current_user, ref: branch) do |pipeline_service|
            expect(pipeline_service).to receive(:execute)
              .with(:security_orchestration_policy,
                content: anything,
                scan_execution_policy_context_block: a_kind_of(Proc),
                ignore_skip_ci: true)
              .and_call_original
          end
        ...
        context 'when policy_name is provided' do
          let(:policy_name) { 'My Scan Policy' }
          let(:service) do
            described_class.new(project: project, current_user: current_user, params: {
              actions: actions, branch: branch, policy_name: policy_name
            })
          end

          it 'stores the policy name and per-job variables_override in build options' do
            execute

            pipeline = project.all_pipelines.last
            build = pipeline.builds.first

            expect(build.options[:policy]).to include(name: policy_name)
            expect(build.options[:policy][:variables_override]).to include(allowed: true)
          end
        end
      end
      ...
      context "with on-demand action" do
        ...
        context "without associated DAST profile" do
          ...
          it 'sets variables_override on the DAST build' do
            execute
            build = on_demand_pipeline.builds.first

            expect(build.options[:policy]).to include(
        ...
        context 'when DAST action has custom variables' do
          let(:actions) do
            [{ scan: "dast", variables: { DAST_WEBSITE: 'https://custom.example.com' } }]
          end

          it 'sets variables_override with exceptions for the custom variables' do
            execute
            build = on_demand_pipeline.builds.first

            expect(build.options[:policy]).to include(
              variables_override: { allowed: true, exceptions: %w[DAST_WEBSITE] }
            )
          end
        end
        ...
        context 'when policy_name is provided' do
          ...
          let(:service) do
            described_class.new(project: project, current_user: current_user, params: {
              actions: actions, branch: branch, policy_name: 'DAST Schedule Policy'
            })
          end

          it 'includes the policy name in DAST build options' do
            execute
            build = on_demand_pipeline.builds.first

            expect(build.options[:policy]).to include(
              name: 'DAST Schedule Policy',
              variables_override: { allowed: true }
            )
          end
        end
      ...
      context "with scan pipeline and on-demand actions" do
        ...
        it "sets variables_override on pipeline scan builds" do
          pipeline_scan_pipeline.builds.each do |build|
            expect(build.options[:policy]).to include(
              variables_override: a_hash_including(allowed: true)
            )
          end
        end

        it "sets variables_override on on-demand DAST builds" do
          on_demand_pipeline.builds.each do |build|
            expect(build.options[:policy]).to include(
              variables_override: a_hash_including(allowed: true)
            )
          end
      ...
      describe 'secret_detection scan action' do
        ...
        context 'with a previous scan' do
          let_it_be(:last_scan_pipeline_sha) { '0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33' }
          let_it_be(:last_scan_pipeline) do
            create(
              :ci_pipeline,
              :success,
              project: project,
          ...
          context 'without a most most_recent_commit_sha' do
            ...
            before do
              ...
              allow(project.repository).to receive(:commit).with("refs/heads/#{branch}").and_return(nil)
            end

            it "invokes Ci::CreatePipelineService with scan_execution_policy_context_block but does not create pipelines" do
              ...
              expect_next_instance_of(Ci::CreatePipelineService, project, current_user, ref: branch) do |pipeline_service|
                expect(pipeline_service).to receive(:execute)
                                              .with(:security_orchestration_policy,
                                                ...
                                                scan_execution_policy_context_block: a_kind_of(Proc),
                                                ignore_skip_ci: true).and_call_original
              end
      ...
      context "when project has a compliance framework" do
        ...
        let(:compliance_project) { create(:project, :small_repo, namespace: compliance_group, name: "hippa") }
