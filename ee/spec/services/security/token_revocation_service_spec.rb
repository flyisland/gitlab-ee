...
RSpec.describe Security::TokenRevocationService, '#execute', feature_category: :security_policy_management do
  let_it_be(:revocation_token_types_url, freeze: false) { 'https://myhost.com/api/v1/token_types' }
  let_it_be(:token_revocation_url, freeze: false) { 'https://myhost.com/api/v1/revoke' }
  let_it_be(:project, freeze: false) { build(:project) }

  let_it_be(:revocable_keys, freeze: false) do
    [{
      type: 'aws_key_id',
  ...
  let_it_be(:revocable_external_token_types, freeze: false) do
  ...
  context 'when revoking a glpat token' do
    let_it_be(:glpat_token, freeze: false) { create(:personal_access_token) }
