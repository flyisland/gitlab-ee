...
RSpec.describe SecretsManagement::ProjectSecretsManagers::DeprovisionService, :gitlab_secrets_manager, feature_category: :secrets_management do
  ...
  describe '#execute', :aggregate_failures do
    ...
    context 'when project is nil' do
      ...
      let(:base_client) { instance_double(SecretsManagement::SecretsManagerClient) }
      ...
      before do
        ...
        allow(service).to receive(:base_secrets_manager_client).and_return(base_client)
        allow(base_client).to receive(:with_namespace).with("group_#{group.id}").and_return(namespaced_client)
        ...
        allow(base_client).to receive(:disable_namespace).and_return({})
      ...
      it 'uses base_secrets_manager_client.with_namespace(namespace_path) for the namespace client',
        :aggregate_failures do
        expect(base_client).to receive(:with_namespace).with("group_#{group.id}").and_return(namespaced_client)
    ...
    it 'deletes all resources related to the project secrets manager' do
      ...
      expect_jwt_role_to_exist(secrets_manager.full_project_namespace_path, secrets_manager.user_auth_mount,
        secrets_manager.user_auth_role)
      expect_jwt_cel_role_to_exist(secrets_manager.full_project_namespace_path, secrets_manager.ci_auth_mount,
        secrets_manager.ci_auth_role)
      ...
      expect_jwt_role_not_to_exist(secrets_manager.full_project_namespace_path, secrets_manager.user_auth_mount,
        secrets_manager.user_auth_role)
      expect_jwt_cel_role_not_to_exist(secrets_manager.full_project_namespace_path, secrets_manager.ci_auth_mount,
        secrets_manager.ci_auth_role)
