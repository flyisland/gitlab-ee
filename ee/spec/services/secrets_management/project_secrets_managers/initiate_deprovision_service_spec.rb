...
RSpec.describe SecretsManagement::ProjectSecretsManagers::InitiateDeprovisionService, feature_category: :secrets_management do
  ...
  describe '.bulk_initiate_for_projects' do
    ...
    context 'when active SMs exist' do
      ...
      # Race: between our `active_sms` fetch and our `insert_all`,
      # another flow (e.g. a GraphQL deprovision mutation) creates a
      # task for one of the same projects. Without `ON CONFLICT DO
      # NOTHING` the whole batch would roll back, losing the non-
      # conflicting tasks. With it, the conflicting row is silently
      # skipped and the rest commit.
      context 'when a concurrent flow inserts a conflicting task between fetch and insert' do
        before do
          # Pre-create a task for sm1's project (simulating the racing
          # flow having already inserted) AFTER bulk_call fetches but
          # BEFORE it inserts. Easiest hook: wrap insert_all to inject
          # the conflicting row just-in-time.
          original_insert = SecretsManagement::ProjectSecretsManagerMaintenanceTask.method(:insert_all)
          allow(SecretsManagement::ProjectSecretsManagerMaintenanceTask)
            .to receive(:insert_all) do |attrs, **opts|
              ...
              original_insert.call(attrs, **opts)
            end
        end

        it 'skips the conflicting task and still inserts the rest' do
          expect { bulk_call }
            .to change { SecretsManagement::ProjectSecretsManagerMaintenanceTask.count }.by(2)

          tasks = SecretsManagement::ProjectSecretsManagerMaintenanceTask
          expect(tasks.where(project_id: projects[0].id).count).to eq(1)
          expect(tasks.where(project_id: projects[1].id).count).to eq(1)
        end

        it 'only enqueues workers for the tasks it actually inserted' do
          bulk_call

          inserted_task = SecretsManagement::ProjectSecretsManagerMaintenanceTask
            .find_by(project_id: projects[1].id)
          expect(deprovision_worker_spy)
            .to have_received(:bulk_perform_async).with([[inserted_task.id]])
        end
      end
    end
