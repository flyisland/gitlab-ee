...
RSpec.describe SecretsManagement::GroupSecretsManagers::InitiateDeprovisionService, :gitlab_secrets_manager, feature_category: :secrets_management do
  ...
  describe '.bulk_initiate_for_groups' do
    ...
    context 'when active SMs exist' do
      ...
      # Race: between our `active_sm_ids` fetch and our `insert_all`,
      # another flow (e.g. a GraphQL deprovision mutation) creates a
      # task for one of the same groups. Without `ON CONFLICT DO
      ...
      context 'when a concurrent flow inserts a conflicting task between fetch and insert' do
        before do
          original_insert = SecretsManagement::GroupSecretsManagerMaintenanceTask.method(:insert_all)
          allow(SecretsManagement::GroupSecretsManagerMaintenanceTask)
        ...
        it 'skips the conflicting task and still inserts the rest' do
          ...
          expect(SecretsManagement::GroupSecretsManagerMaintenanceTask.where(group_id: groups[0].id).count).to eq(1)
          expect(SecretsManagement::GroupSecretsManagerMaintenanceTask.where(group_id: groups[1].id).count).to eq(1)
        end

        it 'only enqueues workers for the tasks it actually inserted' do
          bulk_call

          inserted_task = SecretsManagement::GroupSecretsManagerMaintenanceTask
            .find_by(group_id: groups[1].id)
          expect(deprovision_worker_spy)
