...
RSpec.describe Sbom::Ingestion::LicensesFetcher, feature_category: :dependency_management do
  ...
  describe '#fetch' do
    ...
    context 'with license_scanning_for_cyclonedx_setting feature flag' do
      let_it_be(:project) { create(:project) }
      let(:license_fetcher) { described_class.new(project, occurrence_maps) }
      let(:report_component) { component_with_license }

      where(:ff_enabled, :setting_enabled, :expect_licenses) do
        [
          [true,  false, true],
          [true,  true,  true],
          [false, false, true],
      ...
      with_them do
        before do
          stub_feature_flags(license_scanning_for_cyclonedx_setting: ff_enabled)
          project.security_setting.update!(license_scanning_for_cyclonedx_enabled: setting_enabled)
        end

        it 'returns licenses based on flag and setting state' do
          expect(licenses.present?).to eq(expect_licenses)
        end
      end

      context 'when license scanning is disabled' do
        before do
          stub_feature_flags(license_scanning_for_cyclonedx_setting: true)
      ...
      context 'when the feature flag is enabled but security_setting is nil' do
        before do
          stub_feature_flags(license_scanning_for_cyclonedx_setting: project)
