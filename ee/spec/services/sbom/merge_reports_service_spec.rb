...
RSpec.describe Sbom::MergeReportsService, :freeze_time, feature_category: :dependency_management do
  ...
  describe '#execute' do
    ...
    context 'when a license_override_applicator is provided' do
      let(:overridden_licenses) { [{ 'name' => 'Apache-2.0', 'spdx_identifier' => 'Apache-2.0' }] }
      let(:applicator) do
        instance_double(Security::LicenseOverrideApplicator).tap do |a|
          allow(a).to receive(:apply).and_return(overridden_licenses)
        end
      end

      subject(:execute) { described_class.new(reports, license_override_applicator: applicator).execute }

      it 'applies license overrides to all components' do
        licenses = execute.components.flat_map(&:licenses)

        expect(licenses).to all(eq(overridden_licenses.first))
      end
    end

    context 'when no license_override_applicator is provided' do
      it 'preserves original component licenses' do
        expect(execute.components.flat_map(&:licenses)).not_to be_empty
      end
    end
