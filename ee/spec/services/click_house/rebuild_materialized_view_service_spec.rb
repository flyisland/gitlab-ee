...
RSpec.describe ClickHouse::RebuildMaterializedViewService, :click_house, feature_category: :database do
  ...
  describe 'rename_table strategy' do
    before do
      # Allow all other execute calls to proceed normally
    ...
    context 'when not running in CI' do
      ...
      it 'uses EXCHANGE TABLES' do
        allow(connection).to receive(:execute).with(/EXCHANGE TABLES/)

        service_response

        expect(connection).to have_received(:execute).with(/EXCHANGE TABLES/)
      end
    end
    ...
    context 'when running in CI' do
      ...
      it 'uses RENAME TABLE' do
        allow(connection).to receive(:execute).with(/RENAME TABLE/)

        service_response

        expect(connection).to have_received(:execute).with(/RENAME TABLE/)
      end
    end
  end
