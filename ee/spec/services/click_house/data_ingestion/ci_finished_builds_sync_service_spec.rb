...
RSpec.describe ClickHouse::DataIngestion::CiFinishedBuildsSyncService,
    ...
    it 'processes the builds' do
      ...
      expect(execute).to have_attributes({
        payload: {
          ...
          records_inserted: 3,
          worker_index: 0, total_workers: 1, mode: nil
        }
      })
  ...
  context 'when no ClickHouse databases are configured' do
    ...
    it 'skips execution' do
      is_expected.to have_attributes({
        ...
        payload: { worker_index: 0, total_workers: 1, mode: nil }
      })
    end
  end
  ...
  context 'when exclusive lease error happens' do
    context 'when the exclusive lease is already locked for the worker' do
      ...
      it 'does nothing' do
        ...
        expect(execute).to have_attributes({
          status: :error, reason: :skipped, payload: { worker_index: 2, total_workers: 3, mode: nil }
        })
      end
    end
  end
  ...
  context 'with mode filtering' do
    let_it_be(:recent_build) { create(:ci_build, :success, finished_at: 1.day.ago) }
    let_it_be(:old_build) { create(:ci_build, :failed, finished_at: 11.days.ago) }

    before_all do
      create_sync_events(recent_build, old_build)
    end

    context 'when mode is :recent' do
      let(:service) { described_class.new(mode: :recent) }

      it 'only processes builds finished within RECENT_THRESHOLD' do
        expect { execute }.to change { ci_finished_builds_row_count }

        records = ci_finished_builds
        synced_ids = records.pluck(:id)
        expect(synced_ids).to include(recent_build.id)
        expect(synced_ids).not_to include(old_build.id)
      end

      it 'includes mode in payload' do
        is_expected.to have_attributes({
          payload: a_hash_including(mode: :recent)
        })
      end
    end

    context 'when mode is :backfill' do
      let(:service) { described_class.new(mode: :backfill) }

      it 'only processes builds finished before RECENT_THRESHOLD' do
        expect { execute }.to change { ci_finished_builds_row_count }

        records = ci_finished_builds
        synced_ids = records.pluck(:id)
        expect(synced_ids).to include(old_build.id)
        expect(synced_ids).not_to include(recent_build.id)
      end

      it 'includes mode in payload' do
        is_expected.to have_attributes({
          payload: a_hash_including(mode: :backfill)
        })
      end
    end
    ...
    context 'when mode is nil' do
      ...
      it 'processes all builds regardless of age' do
        expect { execute }.to change { ci_finished_builds_row_count }

        records = ci_finished_builds
        synced_ids = records.pluck(:id)
        expect(synced_ids).to include(recent_build.id, old_build.id)
      end
    end

    context 'when mode is invalid' do
      let(:service) { described_class.new(mode: :invalid) }

      it 'raises ArgumentError' do
        expect { service }.to raise_error(ArgumentError, 'Invalid mode: invalid')
      end
    end
  end
