...
RSpec.describe ExternalStatusChecks::UpdateService, feature_category: :groups_and_projects do
  ...
  let(:params) do
    {
      id: check.id,
      external_url: 'http://newvalue.com',
      name: 'new name',
  ...
  describe 'audit events' do
    context 'when licensed' do
      ...
      let_it_be_with_reload(:external_status_check) { create(:external_status_check, name: 'QA', project: project, protected_branches: []) }

      context 'when a branch is added', :request_store do
        context 'when a new branch is added' do
          let_it_be(:params) { { id: external_status_check.id, protected_branch_ids: [main_branch.id] } }

          it 'logs an audit event' do
            expect { execute }.to change { AuditEventReader.count }.by(1)
            expect(AuditEventReader.last.details[:custom_message]).to eq "Added protected branch main to QA status check and removed all other branches from status check"
          end
        end
        ...
        context 'when another branch is added' do
          ...
          let_it_be(:params) { { id: external_status_check.id, protected_branch_ids: [main_branch.id, master_branch.id] } }

          it 'logs an audit event' do
            expect { execute }.to change { AuditEventReader.count }.by(1)
            expect(AuditEventReader.last.details[:custom_message]).to eq "Added protected branch master to QA status check"
          end
        end
      end
      ...
      context 'when a branch is removed', :request_store do
        context 'when the only branch is removed' do
          ...
          let_it_be(:params) { { id: external_status_check.id, protected_branch_ids: [] } }

          it 'logs an audit event' do
            expect { execute }.to change { AuditEventReader.count }.by(1)
            expect(AuditEventReader.last.details[:custom_message]).to eq "Added all branches to QA status check"
          end
        end
        ...
        context 'when a branch is removed' do
          ...
          it 'logs an audit event' do
            expect { execute }.to change { AuditEventReader.count }.by(1)
            expect(AuditEventReader.last.details[:custom_message]).to eq "Removed protected branch master from QA status check"
          end
        end
      end
    end
