...
RSpec.describe Vulnerabilities::Findings::FindOrCreateFromSecurityFindingService, '#execute',
  ...
  let_it_be(:artifact_sast, freeze: false) do
  ...
  let_it_be(:scan_sast, freeze: false) do
    create(:security_scan, :latest_successful, scan_type: :sast, build: artifact_sast.job)
  end

  let_it_be(:pipeline_dast, freeze: false) { create(:ci_pipeline, project: project) }
  let_it_be(:build_dast, freeze: false) { create(:ci_build, :success, name: 'dast', pipeline: pipeline_dast) }
  let_it_be(:artifact_dast, freeze: false) { create(:ee_ci_job_artifact, :dast_with_evidence, job: build_dast) }
  let_it_be(:report_dast, freeze: false) { create(:ci_reports_security_report, pipeline: pipeline_dast, type: :dast) }
  let_it_be(:scan_dast, freeze: false) do
  ...
  context 'when there is an existing vulnerability and vulnerability finding for the security finding' do
    let_it_be(:security_finding, freeze: false) { sast_security_findings.first }
  ...
  context 'when there is no report finding for the security finding' do
    let_it_be(:security_finding, freeze: false) { create(:security_finding, scan: scan_sast) }
