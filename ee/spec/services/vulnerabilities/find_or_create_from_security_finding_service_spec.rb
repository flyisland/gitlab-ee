...
RSpec.describe Vulnerabilities::FindOrCreateFromSecurityFindingService, '#execute',
  ...
  let_it_be(:build, freeze: false) { create(:ci_build, :success, name: 'dast', pipeline: pipeline) }
  let_it_be(:artifact, freeze: false) { create(:ee_ci_job_artifact, :dast, job: build) }
  let_it_be(:report, freeze: false) { create(:ci_reports_security_report, pipeline: pipeline, type: :dast) }
  let_it_be(:scan, freeze: false) { create(:security_scan, :latest_successful, scan_type: :dast, build: artifact.job) }
  let_it_be(:security_findings, freeze: false) { insert_security_findings }
  let_it_be(:state, freeze: false) { 'dismissed' }
  ...
  shared_examples 'vulnerability state management' do
    context 'when there is an existing vulnerability for the security finding' do
      let_it_be(:security_finding, freeze: false) { create(:security_finding) }
    ...
    context 'when there is no vulnerability for the security finding' do
      let_it_be(:security_finding, freeze: false) { security_findings.last }
      let_it_be(:security_finding_uuid, freeze: false) { security_finding.uuid }
  ...
  context 'when there is a error during the vulnerability_finding creation' do
    let_it_be(:security_finding_uuid, freeze: false) { 'invalid-security-finding-uuid' }
