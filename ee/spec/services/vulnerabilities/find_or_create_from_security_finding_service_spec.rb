...
RSpec.describe Vulnerabilities::FindOrCreateFromSecurityFindingService, '#execute',
  ...
  let_it_be_with_reload(:build) { create(:ci_build, :success, name: 'dast', pipeline: pipeline) }
  let_it_be_with_reload(:artifact) { create(:ee_ci_job_artifact, :dast, job: build) }
  let_it_be_with_reload(:report) { create(:ci_reports_security_report, pipeline: pipeline, type: :dast) }
  let_it_be_with_reload(:scan) { create(:security_scan, :latest_successful, scan_type: :dast, build: artifact.job) }
        ...
        context 'when the vulnerability state is different from the requested one' do
          ...
          context 'when comment and dismissal_reason is not given' do
            it 'creates a state transition entry', :aggregate_failures do
              ...
              expect(state_transition.vulnerability_occurrence_id).to eq(vulnerability.finding.id)
            end

            context 'when the vulnerability has no associated finding' do
              before do
                allow_any_instance_of(Vulnerability).to receive(:finding).and_return(nil) # rubocop:disable RSpec/AnyInstanceOf -- needed to stub instance fetched from DB
              end

              it 'creates a state transition entry with nil vulnerability_occurrence_id' do
                expect { subject }.to change { Vulnerabilities::StateTransition.count }.from(0).to(1)
                state_transition = Vulnerabilities::StateTransition.last
                expect(state_transition.vulnerability_occurrence_id).to be_nil
              end
            end
          end
  ...
  describe 'mirroring the finding into the merge request target branch' do
    ...
    let_it_be_with_reload(:pipeline) do
      create(:ci_pipeline, :success, project: project, ref: 'feature-branch', merge_request: merge_request)
    end
    ...
    let_it_be_with_reload(:report) { create(:ci_reports_security_report, pipeline: pipeline, type: :dast) }
    let_it_be_with_reload(:scan) { create(:security_scan, :latest_successful, scan_type: :dast, build: artifact.job) }
    let_it_be_with_reload(:security_findings) { insert_security_findings }

    let(:security_finding_uuid) { security_findings.first.uuid }
    let(:report_finding) { report.findings.find { |finding| finding.uuid == security_finding_uuid } }
    ...
    it 'links the mirrored finding to the dismissed vulnerability' do
      expect(subject).to be_success

      vulnerability = subject.payload[:vulnerability]

      expect(vulnerability).to be_dismissed
      expect(vulnerability.present_on_default_branch).to be(false)
      # This is the lookup `IngestFindings` performs on the target branch: it upserts on
      # `uuid` and reads `vulnerability_id` off the row it conflicts with.
      expect(Vulnerabilities::Finding.by_uuid(target_context_uuid).pick(:vulnerability_id))
        .to eq(vulnerability.id)
    end

    it 'lets target branch ingestion promote the dismissal rather than duplicate it' do
      vulnerability = subject.payload[:vulnerability]
      mirrored_finding = Vulnerabilities::Finding.by_uuid(target_context_uuid).first

      target_pipeline = create(:ci_pipeline, project: project, ref: 'release-19-4', user: user)
      # Stands in for the state `IngestFindings` leaves behind after matching on UUID.
      finding_map = create(:finding_map, pipeline: target_pipeline, tracked_context: target_context,
        finding: mirrored_finding, vulnerability: vulnerability)

      expect { Security::Ingestion::Tasks::IngestVulnerabilities.new(target_pipeline, [finding_map]).execute }
        .to change { vulnerability.reload.present_on_default_branch }.from(false).to(true)
