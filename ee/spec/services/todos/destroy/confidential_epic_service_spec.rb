...
RSpec.describe Todos::Destroy::ConfidentialEpicService, feature_category: :team_planning do
  ...
  let_it_be(:group) { create(:group, :public, reporters: group_member, guests: guest) }
  let_it_be(:group_link) { create(:group_group_link, shared_group: group) }
  let_it_be_with_reload(:epic_1) { create(:epic, :confidential, group: group, author: author) }
