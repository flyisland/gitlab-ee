...
RSpec.describe Cd::Environments::UpdateService, feature_category: :continuous_delivery do
  ...
  subject(:result) do
    described_class.new(environment, current_user: current_user, params: params).execute
  end

  describe '#execute' do
    it 'returns a success response with the updated environment' do
      expect(result).to be_success
      expect(result.payload[:environment]).to eq(environment)
    ...
    context 'when name is blank' do
      ...
      it 'does not update the environment and returns the error' do
