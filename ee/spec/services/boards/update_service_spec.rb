...
RSpec.describe Boards::UpdateService, :services, feature_category: :planning_views do
  describe '#execute' do
    let_it_be(:parent_group) { create(:group) }
    let_it_be_with_refind(:group) { create(:group, parent: parent_group) }
    let_it_be(:user) { create(:user) }
    let_it_be_with_refind(:project) { create(:project, group: group, reporters: user) }
    ...
    context 'with project board' do
      let_it_be_with_reload(:board) { create(:board, project: project, name: 'Backend') }
