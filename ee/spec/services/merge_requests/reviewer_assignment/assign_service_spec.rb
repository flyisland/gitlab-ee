...
RSpec.describe MergeRequests::ReviewerAssignment::AssignService, feature_category: :code_review_workflow do
  ...
  subject(:service) { described_class.new(merge_request: merge_request, strategy: strategy, current_user: author) }
  ...
  describe '#execute' do
    ...
    context 'when merge request is draft' do
      let(:merge_request) do
        create(:merge_request, source_project: project, author: author, title: 'Draft: test')
      end
      ...
      it 'returns skipped response' do
        ...
        expect(result).to be_success
        expect(result.message).to eq('MR is draft')
        expect(result.payload[:skipped]).to be true
      end
    end

    context 'when merge request already has reviewers' do
      before do
        merge_request.reviewers = [reviewer1]
      end
      ...
      it 'returns skipped response' do
        ...
        expect(result).to be_success
        expect(result.message).to eq('Reviewers already assigned')
        expect(result.payload[:skipped]).to be true
      end
    end
    ...
    context 'with real strategy integration' do
      let(:real_strategy) { MergeRequests::ReviewerAssignment::CodeOwnersStrategy.new(merge_request) }
      let!(:approval_rule) do
        create(:code_owner_rule, merge_request: merge_request, users: [reviewer1, reviewer2], approvals_required: 1)
      end

      subject(:service) do
        described_class.new(merge_request: merge_request, strategy: real_strategy, current_user: author)
      end
