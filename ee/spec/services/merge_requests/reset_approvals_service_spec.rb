...
RSpec.describe MergeRequests::ResetApprovalsService, feature_category: :code_review_workflow do
  ...
  let_it_be(:project) { create(:project, :repository, namespace: group, approvals_before_merge: 1, reset_approvals_on_push: true) }
  let_it_be(:owner) { create(:user, username: 'co1', developer_of: project) }
  let_it_be(:approver) { create(:user, username: 'co2', developer_of: project) }
  let_it_be(:security) { create(:user) }
  ...
  describe "#execute" do
    ...
    context 'with selective code owner removals' do
      ...
      context 'when optimize_reset_approvals_preloading is enabled' do
        before do
          stub_feature_flags(optimize_reset_approvals_preloading: project)
        end
        ...
        it 'does not produce N+1 queries when adding additional code owner rules' do
          ...
          merge_request.approvals.delete_all # -- reset state for second run
          create(:approval, merge_request: merge_request, user: owner, patch_id_sha: patch_id_sha)
          create(:approval, merge_request: merge_request, user: approver, patch_id_sha: patch_id_sha)
      ...
      context 'when optimize_reset_approvals_preloading is disabled' do
        before do
          stub_feature_flags(optimize_reset_approvals_preloading: false)
        end
        ...
        context 'when merge request has approvals' do
          ...
          it 'does not eagerly load approvals before rule evaluation' do
            service.send(:delete_code_owner_approvals, merge_request, patch_id_sha: patch_id_sha, cause: :new_push)

            expect(service.send(:optimize_reset_approvals_preloading?)).to be(false)
          end

          it 'does not call preload_for_approval_reset' do
            expect(service).not_to receive(:preload_for_approval_reset)
