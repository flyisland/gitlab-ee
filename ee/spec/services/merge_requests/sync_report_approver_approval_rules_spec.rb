...
RSpec.describe MergeRequests::SyncReportApproverApprovalRules, feature_category: :code_review_workflow do
  ...
  let_it_be(:protected_branch, freeze: false) { create(:protected_branch, project: merge_request.target_project, name: merge_request.target_branch) }
  ...
  describe '#execute' do
    ...
    with_them do
      context 'when a project has a single approval rule for each report_type' do
        let!(:report_approval_project_rule) { create(:approval_project_rule, report_type, :for_all_protected_branches, project: merge_request.target_project, approvals_required: 2, name: default_name) }
        let_it_be(:regular_approval_project_rule) { create(:approval_project_rule, project: merge_request.target_project, approvals_required: 2) }
    ...
    context "when a project has multiple report approval rules" do
      let_it_be(:license_compliance_project_rule) { create(:approval_project_rule, :license_scanning, :for_all_protected_branches, project: merge_request.target_project) }
      let_it_be(:coverage_project_rule) { create(:approval_project_rule, :code_coverage, :for_all_protected_branches, project: merge_request.target_project) }
      ...
      context "when some of the rules have been synchronized to the merge request" do
        let_it_be(:previous_rule) { create(:report_approver_rule, :license_scanning, merge_request: merge_request) }
    ...
    describe 'Authorization' do
      let_it_be(:coverage_project_rule) { create(:approval_project_rule, :code_coverage, project: merge_request.target_project) }
