...
RSpec.describe IncidentManagement::PendingEscalations::CreateService, feature_category: :incident_management do
  ...
  context 'for alerts' do
    let_it_be(:target, freeze: false) { create(:alert_management_alert, project: project) }
    let_it_be(:escalatable, freeze: false) { target }
  ...
  context 'for incidents' do
    let_it_be(:target, freeze: false) { create(:incident, project: project) }
    let_it_be(:escalatable, freeze: false) { create(:incident_management_issuable_escalation_status, issue: target) }
