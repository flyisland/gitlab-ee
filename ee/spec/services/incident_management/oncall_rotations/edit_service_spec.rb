...
RSpec.describe IncidentManagement::OncallRotations::EditService, feature_category: :incident_management do
  ...
  describe '#execute' do
    ...
    context 'when adding a user that do not have permissions' do
      let_it_be(:another_user_with_permission) { create(:user) }
      ...
      before_all { project.add_maintainer(another_user_with_permission) }
    ...
    context 'removing one participant' do
      ...
      it 'soft-removes the participant from the rotation' do
        ...
        expect(participant_to_remove.reload.is_removed).to be(true)
        expect(participant_to_keep.reload.is_removed).to be(false)
      end
    end
