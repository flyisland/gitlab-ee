...
RSpec.describe IncidentManagement::OncallRotations::RemoveParticipantService, feature_category: :incident_management do
  ...
  let_it_be(:schedule, freeze: false) { create(:incident_management_oncall_schedule, project: project) }
  let_it_be(:rotation, freeze: false) { create(:incident_management_oncall_rotation, schedule: schedule, length: 5, length_unit: :days) }
  let_it_be(:other_participant, freeze: false) { create(:incident_management_oncall_participant, rotation: rotation) }
  let_it_be(:participant, freeze: false) { create(:incident_management_oncall_participant, rotation: rotation, user: user) }
