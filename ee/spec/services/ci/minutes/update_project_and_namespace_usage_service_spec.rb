...
RSpec.describe Ci::Minutes::UpdateProjectAndNamespaceUsageService, feature_category: :hosted_runners do
  ...
  describe '#execute', :clean_gitlab_redis_shared_state do
    ...
    context 'with the fix_minute_contention feature flag' do
      shared_examples 'writes to the single default shard' do
        it 'writes the namespace usage to the single default shard' do
          subject

          usages = Ci::Minutes::NamespaceMonthlyUsage.for_namespace(namespace)

          expect(usages.pluck(:shard_number)).to contain_exactly(1)
          expect(usages.first.amount_used).to eq(consumption_minutes)
        end
      end

      context 'when enabled for the namespace' do
        before do
          stub_feature_flags(fix_minute_contention: Group.actor_from_id(namespace.id))
        end
      ...
      context 'when enabled for a group record' do
        let_it_be(:namespace) { create(:group) }
        let_it_be(:project) { create(:project, :private, group: namespace) }

        before do
          allow(Ci::Minutes::NamespaceMonthlyUsage).to receive(:generate_shard_number).and_return(2)
          stub_feature_flags(fix_minute_contention: namespace)
        end

        it 'routes the namespace write to the sharded path' do
          subject
      ...
      context 'when enabled for a different namespace' do
        before do
          stub_feature_flags(fix_minute_contention: Group.actor_from_id(non_existing_record_id))
        end

        it_behaves_like 'updates monthly consumption'
        it_behaves_like 'writes to the single default shard'
      end
