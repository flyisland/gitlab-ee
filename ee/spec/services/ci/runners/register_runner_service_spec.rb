...
RSpec.describe ::Ci::Runners::RegisterRunnerService, '#execute', :freeze_time, feature_category: :fleet_visibility do
  let_it_be(:group, freeze: false) { create(:group, :allow_runner_registration_token) }
  let_it_be(:project, freeze: false) { create(:project, :allow_runner_registration_token, namespace: group) }
