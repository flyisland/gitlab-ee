...
RSpec.describe SystemNotes::EscalationsService, feature_category: :incident_management do
  ...
  let_it_be(:author) { Users::Internal.in_organization(project.organization).alert_bot }
  ...
  describe '#initialize' do
    context 'when container is nil' do
      subject { described_class.new(noteable: noteable, container: nil) }

      let_it_be(:noteable, freeze: false) { create(:alert_management_alert, project: project) }

      it 'sets the author to the default organization alert_bot' do
        expect(subject.author).to eq(Users::Internal.in_organization(nil).alert_bot)
      end
    end
  end
