...
RSpec.describe GitlabSubscriptions::CheckFutureRenewalService, :use_clean_rails_memory_store_caching, :saas, feature_category: :subscription_management do
  ...
  describe '#execute' do
    ...
    where(:next_term_start_date, :expected_response) do
      nil          | false
      '2024-11-01' | true
    end

    with_them do
      let(:response) { { success: true, next_term_start_date: next_term_start_date } }

      before do
        allow(Gitlab::SubscriptionPortal::Client).to receive(:subscription_next_term_start_date).and_return(response)
      end
    ...
    context 'with an unsuccessful CustomersDot query' do
      it 'assumes no future renewal' do
        allow(Gitlab::SubscriptionPortal::Client).to receive(:subscription_next_term_start_date).and_return({
          success: false
        })
    ...
    context 'when called with a sub-group' do
      ...
      it 'uses the root ancestor namespace' do
        expect(Gitlab::SubscriptionPortal::Client).to receive(:subscription_next_term_start_date).with(root_namespace.id).and_return({})
