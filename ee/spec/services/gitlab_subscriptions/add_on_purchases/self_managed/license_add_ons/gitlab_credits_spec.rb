...
RSpec.describe GitlabSubscriptions::AddOnPurchases::SelfManaged::LicenseAddOns::GitlabCredits,
  :aggregate_failures, feature_category: :"add-on_provisioning" do
  subject(:add_on_license) { described_class.new(restrictions) }

  let_it_be(:add_on) { create(:gitlab_subscription_add_on, :gitlab_credits) }
  let(:restrictions) do
  ...
  include_examples "license add-on attributes", add_on_name: "gitlab_credits"
