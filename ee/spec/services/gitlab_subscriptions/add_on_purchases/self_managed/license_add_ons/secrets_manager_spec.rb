...
RSpec.describe GitlabSubscriptions::AddOnPurchases::SelfManaged::LicenseAddOns::SecretsManager,
  :aggregate_failures, feature_category: :"add-on_provisioning" do
  subject(:add_on_license) { described_class.new(restrictions) }

  let_it_be(:add_on) { create(:gitlab_subscription_add_on, :secrets_manager) }
  let(:restrictions) do
  ...
  include_examples "license add-on attributes", add_on_name: "secrets_manager"
