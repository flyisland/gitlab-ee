...
RSpec.describe GitlabSubscriptions::AddOnPurchases::SelfManaged::ProvisionServices::FlexOffline,
  :freeze_time, feature_category: :'add-on_provisioning' do
  describe '#execute' do
    include_context 'with provision services common setup'

    let_it_be(:add_on_flex_offline) { create(:gitlab_subscription_add_on, :flex_offline) }
    ...
    context 'without Flex Offline' do
      let(:add_ons) { [] }

      it 'does not create a Flex Offline add-on purchase' do
        expect { provision_service.execute }.not_to change { GitlabSubscriptions::AddOnPurchase.count }
      end
    end

    context 'with Flex Offline' do
      let(:add_ons) { %i[flex_offline] }

      it 'creates a new Flex Offline add-on purchase' do
        expect do
        ...
        expect(GitlabSubscriptions::AddOnPurchase.first).to have_attributes(
          subscription_add_on_id: add_on_flex_offline.id,
          quantity: quantity,
    ...
    context 'with existing Flex Offline' do
      let(:add_ons) { %i[flex_offline] }

      context 'when expired' do
        let!(:existing_flex_offline) do
          create(
            :gitlab_subscription_add_on_purchase,
            add_on: add_on_flex_offline,
            quantity: quantity,
        ...
        it 'updates existing add-on purchase', :aggregate_failures do
          ...
          expect(existing_flex_offline.reload.started_at).to eq(started_at)
          expect(existing_flex_offline.expires_on).to eq(started_at + 1.year)
        end
      end

      context 'with an additional add-on purchase' do
        let(:add_ons) { %i[flex_offline self_hosted_dap] }
        ...
        it 'does not affect the Flex Offline purchase', :aggregate_failures do
          expect { provision_service.execute }.not_to change { GitlabSubscriptions::AddOnPurchase.count }
    ...
    context 'with existing Self-Hosted DAP' do
      ...
      context 'with additional purchase of Flex Offline' do
        let(:add_ons) { %i[self_hosted_dap flex_offline] }
