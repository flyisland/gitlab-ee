...
RSpec.describe Mcp::Tools::SemanticCodeSearchService, feature_category: :mcp_server do
  ...
  describe '#description' do
    ...
    it 'returns the registered version description' do
      expect(service.description).to eq(described_class.version_metadata('0.1.0')[:description])
    end
  end
    ...
    context 'with valid arguments' do
      ...
      shared_examples 'tool executed with expected response' do
        it 'initializes the code query with search term and current_user and filters with expected params' do
          ...
          expected_text = "Confidence: HIGH\n\n1. ruby/server.rb (score: 0.9523)\n[Lines 0-0]\nrequire 'webrick'"
          expect(response[:content].first[:text]).to eq(expected_text)

          structured = response[:structuredContent]
          expect(structured).to be_a(Hash)
          expect(structured[:metadata]).to eq({ count: 1, has_more: false, confidence: :high })
          ...
          expect(item).to eq(
            {
              ...
              score: 0.9523,
              snippet_ranges: [
                { start_line: 0, end_line: 0, content: "require 'webrick'", score: 0.9523 }
              ]
            }
          )
        end
      ...
      context 'when exclusion rules filter some results' do
        ...
        it 'excludes filtered files from structured data' do
          ...
          expect(response[:content].first[:text]).to include("1. ruby/server.rb")
          expect(response[:content].first[:text]).to include("[Lines 0-0]")
      ...
      context 'with grouping hits from multiple files' do
        let(:project_id) { project.id.to_s }
