...
RSpec.describe MergeRequests::CreateService, feature_category: :code_review_workflow do
  ...
  describe '#execute' do
    ...
    context 'auto reviewer assignment enqueue param' do
      before do
        stub_licensed_features(code_owners: true)
        project.project_setting.update!(reviewer_assignment_strategy: 'code_owners')
      end

      it 'passes enqueue_auto_reviewer_assignment: true to the SyncCodeOwner worker' do
        expect(::MergeRequests::SyncCodeOwnerApprovalRulesWorker).to receive(:perform_async)
          .with(kind_of(Integer), expire_unapproved_key: true, enqueue_auto_reviewer_assignment: true)
      ...
      context 'when the feature flag is disabled' do
        ...
        it 'passes enqueue_auto_reviewer_assignment: false to the SyncCodeOwner worker' do
          expect(::MergeRequests::SyncCodeOwnerApprovalRulesWorker).to receive(:perform_async)
            .with(kind_of(Integer), expire_unapproved_key: true, enqueue_auto_reviewer_assignment: false)
