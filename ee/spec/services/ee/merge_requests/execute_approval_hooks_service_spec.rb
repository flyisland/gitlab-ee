...
RSpec.describe MergeRequests::ExecuteApprovalHooksService, feature_category: :code_review_workflow do
  ...
  describe '#execute' do
    context 'with remaining approvals' do
      ...
      before do
        allow(merge_request).to receive(:approvals_left).and_return(5)
      ...
      it 'does not fire an approved webhook' do
        expect(service).not_to receive(:execute_hooks).with(merge_request, 'approved')
