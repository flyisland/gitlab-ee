...
RSpec.describe MergeRequests::BuildService, feature_category: :code_review_workflow do
  ...
  context 'project default template configured' do
    ...
    context 'issuable default templates feature available' do
      ...
      context 'when the merge request is built from a cherry-pick' do
        let(:picked_merge_request) do
        ...
        it 'prefers the picked description to the project default template' do
          expect(merge_request.description).to eq('Description of the picked merge request')
        end

        context 'when description is provided' do
          let(:description) { 'Description' }

          it "prefers the user's description to both" do
            expect(merge_request.description).to eq(description)
          end
        end
      end
