...
RSpec.describe Mcp::Tools::Manager, feature_category: :ai_agents do
  ...
  describe '#get_tool' do
    ...
    context 'with semantic_code_search and mcp_semantic_code_search_use_rest_api flag' do
      context 'when flag is disabled (default)' do
        it 'returns SemanticCodeSearchService' do
          tool = manager.get_tool(name: 'semantic_code_search')

          expect(tool).to be_a(Mcp::Tools::SemanticCodeSearchService)
        end

        it 'includes semantic_code_search in build_ee_tools' do
          ee_tools = manager.send(:build_ee_tools)

          expect(ee_tools['semantic_code_search']).to be_a(Mcp::Tools::SemanticCodeSearchService)
        end
      end
      ...
      context 'when flag is enabled' do
        ...
        before do
          stub_feature_flags(mcp_semantic_code_search_use_rest_api: true)
        ...
        it 'bypasses EE custom tool and falls through to ApiTool discovery' do
          tool = manager.get_tool(name: 'semantic_code_search')

          expect(tool).to eq(semantic_search_api_tool)
        end

        it 'excludes semantic_code_search from build_ee_tools' do
          ee_tools = manager.send(:build_ee_tools)

          expect(ee_tools).not_to have_key('semantic_code_search')
        end
      end
    end
  end
