...
RSpec.describe Boards::Issues::ListService, :services, feature_category: :portfolio_management do
  describe '#execute' do
    ...
    let_it_be(:opened_issue3, freeze: false) { create(:labeled_issue, project: project, milestone: m2, title: 'Assigned Issue', labels: [p3]) }
    ...
    context 'with assignee, milestone, iteration, label and status lists present' do
      ...
      context 'when filtering by custom work item type' do
        ...
        let_it_be(:non_converted_custom_type) do
          create(:work_item_custom_type, :with_organization, organization: group.organization, name: 'Feature')
        end

        let_it_be(:issue_with_converted_type) do
          create(:labeled_issue, project: project, labels: [development]).tap do |issue|
            issue.update!(work_item_type_id: converted_custom_type.id)
          end
        end

        let_it_be(:issue_with_non_converted_type) do
          create(:labeled_issue, project: project, labels: [development]).tap do |issue|
            issue.update!(work_item_type_id: non_converted_custom_type.id)
          end
        end
        ...
        it 'filters by converted custom work item type id' do
          params = { board_id: board.id, id: list1.id, work_item_type_ids: [converted_custom_type.id] }

          expect(described_class.new(parent, user, params).execute).to contain_exactly(issue_with_converted_type)
        end

        it 'filters by non-converted custom work item type id' do
          params = { board_id: board.id, id: list1.id, work_item_type_ids: [non_converted_custom_type.id] }

          expect(described_class.new(parent, user, params).execute).to contain_exactly(issue_with_non_converted_type)
        end

        it 'filters by multiple custom work item type ids' do
          params = {
            board_id: board.id,
            id: list1.id,
            work_item_type_ids: [converted_custom_type.id, non_converted_custom_type.id]
          }

          expect(described_class.new(parent, user, params).execute)
            .to contain_exactly(issue_with_converted_type, issue_with_non_converted_type)
        end

        it 'filters by converted_from_system_defined_type_identifier' do
          params = {
            board_id: board.id,
            id: list1.id,
            work_item_type_ids: [converted_custom_type.converted_from_system_defined_type_identifier]
          }

          expect(described_class.new(parent, user, params).execute)
            .to contain_exactly(list1_issue1, list1_issue2, list1_issue3)
        end

        it 'filters by converted_from_system_defined_type_identifier combined with custom type id' do
          params = {
            board_id: board.id,
            id: list1.id,
            work_item_type_ids: [
              converted_custom_type.converted_from_system_defined_type_identifier,
              non_converted_custom_type.id
            ]
          }

          expect(described_class.new(parent, user, params).execute)
            .to contain_exactly(list1_issue1, list1_issue2, list1_issue3, issue_with_non_converted_type)
        end

        it 'excludes converted custom work item type with negated filter' do
          params = { board_id: board.id, id: list1.id, not: { work_item_type_ids: [converted_custom_type.id] } }

          expect(described_class.new(parent, user, params).execute)
            .to contain_exactly(list1_issue1, list1_issue2, list1_issue3, issue_with_non_converted_type)
        end

        it 'excludes non-converted custom work item type with negated filter' do
          # issue_with_converted_type is filtered out by the default positive work_item_type_ids
          # filter applied by the service (based on filterable_board_view? types), because
          # its work_item_type_id stores the custom type AR pk rather than the persistable_id.
          params = { board_id: board.id, id: list1.id, not: { work_item_type_ids: [non_converted_custom_type.id] } }

          expect(described_class.new(parent, user, params).execute)
            .to contain_exactly(list1_issue1, list1_issue2, list1_issue3)
        end

        it 'excludes issues matching converted_from_system_defined_type_identifier with negated filter' do
          params = {
            board_id: board.id,
            id: list1.id,
            not: { work_item_type_ids: [converted_custom_type.converted_from_system_defined_type_identifier] }
          }

          expect(described_class.new(parent, user, params).execute)
            .to contain_exactly(issue_with_non_converted_type)
        end

        it 'excludes issues with negated filter combining converted identifier and custom type id' do
          params = {
            board_id: board.id,
            id: list1.id,
            not: {
          ...
          expect(described_class.new(parent, user, params).execute).to be_empty
        end
      end
    end
  end
