...
RSpec.describe Ci::Pipelines::HookService, feature_category: :continuous_integration do
  ...
  describe '#execute' do
    ...
    before do
      ...
      allow(pipeline).to receive_messages(source: pipeline_source, status: 'failed')
    end
    ...
    context 'when pipeline is in an intermediate state due to job retries' do
      ...
      where(:status) { %w[running pending created] }

      with_them do
        before do
          allow(pipeline).to receive_messages(source: pipeline_source, status: status)
        end

        it 'does not trigger AI Flows' do
          expect(pipeline.project).not_to receive(:execute_flow_triggers)
