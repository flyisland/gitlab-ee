...
RSpec.describe Branches::ValidateNewService, feature_category: :source_code_management do
  ...
  describe '#execute' do
    ...
    context 'when an unrelated push rule is set' do
      let_it_be_with_reload(:project_push_rules) { create(:project_push_rule, project: project) }
    ...
    context 'when a push rules with branch names are set' do
      let_it_be_with_reload(:project_push_rules) do
        create(:project_push_rule, project: project, branch_name_regex: '(feature|hotfix)\/.*')
      end
  ...
  describe '#push_rule' do
    context 'with read_project_push_rules enabled' do
      let!(:project_push_rule) { create(:project_push_rule, project: project) }

      it 'returns a ProjectPushRule' do
        expect(service.send(:push_rule)).to be_a(ProjectPushRule)
      end
    end

    context 'with read_project_push_rules disabled' do
      before do
      ...
      it 'returns a PushRule' do
        expect(service.send(:push_rule)).to be_a(PushRule)
      end
    end
  end
end
