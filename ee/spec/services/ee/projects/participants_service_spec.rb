...
RSpec.describe ::Projects::ParticipantsService, feature_category: :code_review_workflow do
  describe '#execute' do
    ...
    let_it_be(:project, freeze: false) { create(:project, :public, group: group, organization: organization) }

    let_it_be(:current_user, freeze: false) do
    ...
    let_it_be(:non_agent_service_account, freeze: false) do
      create(:user, :service_account, organization: organization, developer_of: project, username: 'non-agent')
    end

    let_it_be(:agent_service_account, freeze: false) do
