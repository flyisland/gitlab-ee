  ...
  describe '#execute' do
    context 'when a status is included' do
      ...
      context 'when moving from a closed status to an open status' do
        let_it_be_with_reload(:alert) { create(:alert_management_alert, :resolved, project: project) }
      ...
      context 'moving from an open status to closed status' do
        let_it_be(:alert, freeze: false) { create(:alert_management_alert, :triggered, project: project) }
