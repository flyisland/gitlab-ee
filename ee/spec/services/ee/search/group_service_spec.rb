...
RSpec.describe Search::GroupService, feature_category: :global_search do
  ...
  it_behaves_like 'EE search service shared examples', ::Gitlab::GroupSearchResults,
    ::Search::Elastic::GroupSearchResults do
    let(:scope) { group }
  ...
  describe '#elastic_projects', :elastic_helpers do
    let_it_be(:sub_group) { create(:group, :public, parent: group) }
    let_it_be(:sub_project) { create(:project, :public, namespace: sub_group) }

    let(:service) { described_class.new(user, group, {}) }

    subject(:elastic_projects) { service.elastic_projects }

    before do
      set_elasticsearch_migration_to(:backfill_traversal_ids_on_commits, including: false)
    end

    context 'when the traversal_ids backfill migrations have not finished' do
      it 'returns the IDs of projects in the group hierarchy' do
        expect(elastic_projects).to contain_exactly(project.id, sub_project.id)
      end
    end

    context 'when the traversal_ids backfill migrations have finished' do
      before do
        set_elasticsearch_migration_to(:backfill_traversal_ids_on_commits, including: true)
      end

      it 'returns nil and does not load the descendant project list', :aggregate_failures do
        expect(service).not_to receive(:projects)
        expect(elastic_projects).to be_nil
      end
    end
  end
  ...
  context 'when sorting', :elastic do
    ...
    context 'for group level work_items' do
      let(:scope) { 'epics' }
      let_it_be(:member, freeze: false) { create(:group_member, :owner, group: group, user: user) }
