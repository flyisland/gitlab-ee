...
RSpec.describe Issues::UpdateService, feature_category: :team_planning do
  ...
  describe 'execute' do
    ...
    context 'updating weight' do
      before_all { project.add_developer(author) }
    ...
    context "when handling status" do
      ...
      context "when the status is updated via params" do
        let_it_be_with_reload(:issue) { create(:issue, project: project) }

        let(:new_status) { build(:work_item_system_defined_status, :in_progress) }
        ...
        it 'includes the status transition in the webhook changes payload' do
          old_status = issue.status_with_fallback

          update_issue(status: new_status)

          hook_data = issue.to_hook_data(user,
            old_associations: { status: old_status },
            action: 'update')

          status_change = hook_data[:changes][:status]
          expect(status_change[:previous]).to include('name' => 'To do', 'category' => :to_do)
          expect(status_change[:current]).to include('name' => 'In progress', 'category' => :in_progress)
        end
      end
    end
    ...
    context 'refresh epic dates' do
      ...
      context 'updating weight' do
        before_all { project.add_maintainer(author) }
    ...
    context 'assigning iteration' do
      ...
      before_all { group.add_maintainer(author) }
    ...
    context 'changing issue_type' do
      ...
      context 'from issue to restricted issue types' do
        ...
        context 'without sufficient permissions' do
          ...
          before_all { project.add_guest(guest) }
    ...
    context 'assigning epic' do
      ...
      context 'when a user has permissions to assign an epic' do
        before_all { group.add_guest(author) }
        ...
        context 'when updating issue epic and milestone and assignee attributes' do
          ...
          before_all { project.add_guest(assignee_user1) }
    ...
    context 'updating escalation status' do
      ...
      before_all { group.add_developer(author) }
      ...
      context 'when updating last test report state' do
        context 'as passing' do
          it 'creates passing test report with null build_id' do
            ...
            expect(requirement.last_test_report_manually_created?).to be(true)
            expect(test_report.state).to eq('passed')
            expect(test_report.build).to be_nil
    ...
    context 'clone a group level Epic work item' do
      let_it_be(:work_item_epic, freeze: false) { create(:work_item, :epic_with_legacy_epic, :group_level, namespace: group) }
      let_it_be(:sub_group) { create(:group, parent: group) }
      let_it_be(:clone_service_class, freeze: false) { ::WorkItems::DataSync::CloneService }

      before_all do
        group.add_owner(author)
        sub_group.add_owner(author)
      end
    ...
    context 'move a group level Epic work item' do
      let_it_be(:work_item_epic, freeze: false) { create(:work_item, :epic_with_legacy_epic, :group_level, namespace: group) }
      let_it_be(:sub_group) { create(:group, parent: group) }
      let_it_be(:move_service_class, freeze: false) { ::WorkItems::DataSync::MoveService }
    ...
    context 'AI flow triggers on assignment' do
      ...
      context 'when assigning multiple users with flow triggers' do
        it 'triggers all matching flow trigger services' do
          expect(run_service_1).to receive(:execute).with(
            { input: issue.iid.to_s, event: :assign, triggered_by_username: user.username }
          )
          expect(run_service_2).to receive(:execute).with(
            { input: issue.iid.to_s, event: :assign, triggered_by_username: user.username }
          )
      ...
      context 'when the matching flow trigger is inactive' do
        before do
          ::Ai::FlowTrigger.find(flow_trigger_1.id).update!(active: false)
        end
