      ...
      context 'when licensed' do
        ...
        context 'when user is successfully rejected' do
          it 'logs an audit event', :aggregate_failures do
            ...
            expect { reject_user }.to change { AuditEventReader.count }.by(1)

            audit_event = AuditEventReader.where(author_id: current_user.id).last
        ...
        context 'when user does not have permission to reject another user' do
          ...
          it 'does not log any audit event' do
            expect { reject_user }.not_to change { AuditEventReader.count }
          end
        end
      end
    end
