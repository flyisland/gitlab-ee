...
RSpec.describe EE::NotificationService, :mailer, feature_category: :team_planning do
  ...
  context 'IncidentManagement' do
    ...
    describe '#notify_oncall_users_of_alert' do
      let_it_be(:alert, freeze: false) { create(:alert_management_alert) }
      let_it_be(:project, freeze: false) { alert.project }
    ...
    describe '#oncall_user_removed' do
      let_it_be(:schedule, freeze: false) { create(:incident_management_oncall_schedule) }
      let_it_be(:rotation, freeze: false) { create(:incident_management_oncall_rotation, schedule: schedule) }
      let_it_be(:participant, freeze: false) { create(:incident_management_oncall_participant, rotation: rotation) }
    ...
    describe '#user_escalation_rule_deleted' do
      ...
      context 'with a group' do
        ...
        let_it_be(:blocked_owner, freeze: false) { create(:user, :blocked) }
  ...
  describe '#added_as_approver' do
    ...
    let_it_be(:recipients, freeze: false) { create_list(:user, 3) }
    let_it_be(:setting, freeze: false) do
  ...
  context 'when user subscribed to service account pipeline notifications' do
    ...
    let_it_be(:u_custom_notification_enabled, freeze: false) do
    ...
    let_it_be(:u_custom_notification_disabled, freeze: false) do
  ...
  describe '#no_more_seats' do
    ...
    let_it_be(:namespace_limit, freeze: false) { create(:namespace_limit, namespace: namespace) }
    let_it_be(:recipient, freeze: false) { create(:user) }
    let_it_be(:user, freeze: false) { create(:user) }
    let_it_be(:required_members, freeze: false) { ['Patrick Jane', 'Thomas McAllister'] }
    ...
    context 'when last_seat_all_used_seats_notification_at is not nil' do
      let_it_be(:notification_at, freeze: false) { Time.current }
      ...
      context 'when last_seat_all_used_seats_notification_at is more than 1 day ago' do
        let_it_be(:notification_at, freeze: false) { 2.days.ago }
  ...
  describe '#secret_rotation_reminder_for_project' do
    let_it_be(:group, freeze: false) { create(:group) }
    let_it_be(:project, freeze: false) { create(:project, group: group) }
    let_it_be(:rotation_info, freeze: false) { create(:project_secret_rotation_info, project: project) }
  ...
  describe '#secret_rotation_reminder_for_group' do
    let_it_be(:group, freeze: false) { create(:group) }
    let_it_be(:rotation_info, freeze: false) { create(:group_secret_rotation_info, group: group) }

    let(:group_owner1) { create(:user) }
    let(:group_owner2) { create(:user) }
    let(:maintainer) { create(:user) }
