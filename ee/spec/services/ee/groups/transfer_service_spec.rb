...
RSpec.describe Groups::TransferService, '#execute', feature_category: :groups_and_projects do
  ...
  describe '#execute' do
    ...
    context 'with audit events' do
      ...
      it 'logs a group_path_updated audit event with path change details', :aggregate_failures do
        ...
        expect { transfer_service.execute(new_group) }.to change { AuditEvent.count }.by(1)

        expect(AuditEvent.order(:id).last).to have_attributes(
          author_id: user.id,
          entity_id: group.id,
          entity_type: 'Group',
  ...
  describe 'zoekt indexing', :aggregate_failures do
    ...
    context 'when application_setting zoekt_indexing_enabled is enabled', :zoekt_settings_enabled do
      ...
      context 'when project is moved in same root namespace' do
        let_it_be(:root_group, freeze: false) { create(:group, :public) }
        let_it_be(:group, freeze: false) { create(:group, :public, parent: root_group) }
        let_it_be(:new_group, freeze: false) { create(:group, :public, parent: root_group) }
  ...
  context 'with compliance frameworks' do
    let_it_be(:source_root, freeze: false) { create(:group) }
    let_it_be(:subgroup, freeze: false) { create(:group, parent: source_root) }
    let_it_be(:project_with_framework, freeze: false) { create(:project, namespace: subgroup) }
    let_it_be(:compliance_framework, freeze: false) { create(:compliance_framework, namespace: source_root) }
    let(:transfer_service) { described_class.new(subgroup, user) }
  ...
  describe 'secrets manager deprovisioning' do
    ...
    context 'when active secrets managers exist on the group and its descendants' do
      ...
      it 'bulk-initiates deprovision for each scope AFTER the transfer using OLD root and org ids' do
        captured_old_root_id = group.root_ancestor.id
        captured_old_org_id  = group.organization_id
        ...
        expect(SecretsManagement::GroupSecretsManagers::InitiateDeprovisionService)
          ...
          .with(
            ...
            user,
            old_root_namespace_id: captured_old_root_id,
            old_organization_id: captured_old_org_id
          )
          .at_least(:once)
          .and_call_original
      ...
      it 'transitions every active SM in the subtree to deprovisioning and creates one task per SM' do
        expect { transfer_service.execute(new_group) }
        ...
        expect(group_sm.reload).to be_deprovisioning
        expect(subgroup_sm.reload).to be_deprovisioning
        expect(project_sm.reload).to be_deprovisioning
      end

      it 'persists the OLD root and org ids on the created tasks' do
      ...
      it 'populates live entity ids on the created tasks' do
        transfer_service.execute(new_group)
      ...
      it 'skips SMs already in deprovisioning state in the subtree' do
        # Force one of the descendant SMs into deprovisioning before
        # transfer; the bulk initiate must leave it alone (the state
        # filter excludes non-active rows).
        ...
        expect(SecretsManagement::GroupSecretsManagerMaintenanceTask.where(group_id: subgroup.id)).to be_empty
      end
      ...
      context 'when the subtree exceeds the batch size on both scopes' do
        ...
        it 'transitions every active SM across batches and skips non-active ones' do
          ...
          # Only the 2 active SMs per scope produce tasks.
          expect { transfer_service.execute(new_group) }
            .to change { SecretsManagement::GroupSecretsManagerMaintenanceTask.count }.by(2)
            .and change { SecretsManagement::ProjectSecretsManagerMaintenanceTask.count }.by(2)

          # Originally-active SMs transitioned.
          [group_sm, third_subgroup_sm, project_sm, project_b_sm].each do |sm|
            expect(sm.reload).to be_deprovisioning
          end

          # Non-active SMs untouched: state preserved and no task created.
          expect(subgroup_sm.reload).to be_deprovisioning
          expect(fourth_subgroup_sm.reload).to be_provisioning
          expect(project_a_sm.reload).to be_deprovisioning
          expect(project_c_sm.reload).to be_provisioning
          ...
          expect(group_tasks.where(group_id: [subgroup.id, fourth_subgroup.id])).to be_empty
          expect(project_tasks.where(project_id: [project_a.id, project_c.id])).to be_empty
        end
      end

      it 'lets exceptions raised by the bulk initiate methods bubble up to Sentry' do
        ...
        # reaper (gitlab-org/gitlab#600120) backstops.
