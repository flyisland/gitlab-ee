...
RSpec.describe Groups::TransferService, '#execute', feature_category: :groups_and_projects do
  ...
  describe 'elasticsearch indexing', :aggregate_failures, :elastic do
    ...
    describe 'vulnerability indexing', :aggregate_failures do
      before do
        ...
        allow(transfer_service).to receive(:delete_work_items_with_old_routing)
    ...
    describe 'work item indexing', :aggregate_failures do
      before do
        allow(transfer_service).to receive(:delete_vulnerabilities_with_old_routing)
        allow(transfer_service).to receive(:delete_vulnerability_reads_with_old_routing)
        allow(transfer_service).to receive(:delete_work_items_with_old_routing).and_call_original
        stub_ee_application_setting(elasticsearch_indexing: true)
      end

      it 'calls delete_work_items_with_old_routing for each project' do
        project1 = create(:project, :repository, namespace: group)
        project2 = create(:project, :repository, namespace: group)

        expect(transfer_service).to receive(:delete_work_items_with_old_routing).with(project1)
        expect(transfer_service).to receive(:delete_work_items_with_old_routing).with(project2)
      ...
      context 'when a group has subgroups with projects' do
        ...
        it 'calls delete_work_items_with_old_routing for projects in subgroups' do
          expect(transfer_service).to receive(:delete_work_items_with_old_routing).with(project)
          expect(transfer_service).to receive(:delete_work_items_with_old_routing).with(subgroup_project)
  ...
  describe '#delete_work_items_with_old_routing' do
    before do
      stub_ee_application_setting(elasticsearch_indexing: true)
    end

    it 'schedules DeleteWorker with the correct task, project_id, and traversal_id' do
      ...
      allow(::Search::Elastic::DeleteWorker).to receive(:perform_async) do |args|
        received_args = args if args[:task]&.to_sym == :delete_project_work_items
      end
