...
RSpec.describe Groups::ImportExport::ExportService, feature_category: :importers do
  ...
  describe '#execute' do
    it 'exports group and descendants wiki repositories' do
      ...
      expect(File.exist?(Gitlab::ImportExport.group_wiki_repo_bundle_full_path(shared, group.id))).to be true
      expect(File.exist?(Gitlab::ImportExport.group_wiki_repo_bundle_full_path(shared, subgroup.id))).to be true
    end
  end
end
