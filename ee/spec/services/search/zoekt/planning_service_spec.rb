...
RSpec.describe Search::Zoekt::PlanningService, :freeze_time, feature_category: :global_search do
  ...
  describe 'dynamic buffer factor (zoekt_dynamic_buffer_factor feature flag)' do
    let_it_be(:group) { create(:group) }
    let_it_be(:enabled_namespace) { create(:zoekt_enabled_namespace, namespace: group) }
    ...
    context 'when the feature flag is disabled (default)' do
      before do
        stub_feature_flags(zoekt_dynamic_buffer_factor: false)
      end

      it 'uses the default buffer factor of 3×' do
        expected_storage = project.statistics.repository_size * Search::Zoekt::Index::DEFAULT_BUFFER_FACTOR
        namespace_plan = plan[:create].find { |p| p[:namespace_id] == group.id }
        expect(namespace_plan[:namespace_required_storage_bytes]).to eq(expected_storage)
      end
    end
    ...
    context 'when the feature flag is enabled' do
      ...
      before do
        stub_feature_flags(zoekt_dynamic_buffer_factor: true)
