  ...
  describe '#execute' do
    ...
    context 'when displaying the storage buffer factor' do
      ...
      context 'when the feature flag is disabled' do
        before do
          # Use explicit argument matching to override the outer broad stub
          allow(Feature).to receive(:enabled?).with(:zoekt_dynamic_buffer_factor, any_args).and_return(false)
          allow(Search::Zoekt::Index).to receive(:global_buffer_factor)
            .and_return(Search::Zoekt::Index::DEFAULT_BUFFER_FACTOR)
        end

        it 'shows the default factor and marks it as FF disabled' do
          service.execute

          expect(logger).to have_received(:info).with(/Storage buffer factor:.+3\.0+×.+static fallback \(FF disabled\)/)
        end
      end

      context 'when the feature flag is enabled and dynamic factor is below default' do
        before do
          allow(Feature).to receive(:enabled?).with(:zoekt_dynamic_buffer_factor, any_args).and_return(true)
          allow(Search::Zoekt::Index).to receive(:global_buffer_factor).and_return(0.6)
        end

        it 'shows the dynamic factor in green and marks it as dynamic' do
          service.execute

          expect(logger).to have_received(:info).with(/Storage buffer factor:.+0\.6×.+dynamic \(observed\)/)
        end
      end

      context 'when the feature flag is enabled and dynamic factor equals the default' do
        before do
          allow(Feature).to receive(:enabled?).with(:zoekt_dynamic_buffer_factor, any_args).and_return(true)
          allow(Search::Zoekt::Index).to receive(:global_buffer_factor)
            .and_return(Search::Zoekt::Index::DEFAULT_BUFFER_FACTOR)
        ...
        it 'shows the factor in yellow (at or above default) and marks it as dynamic' do
          service.execute

          expect(logger).to have_received(:info).with(/Storage buffer factor:.+3\.0+×.+dynamic \(observed\)/)
        end
      end
    end
