...
RSpec.describe Dora::AggregateMetricsService, feature_category: :value_stream_management do
  ...
  describe '#execute' do
    ...
    context 'when container is project' do
      ...
      let_it_be(:project) { create(:project, maintainers: maintainer, guests: guest) }
      ...
      context 'when interval is all' do
        ...
        it 'returns the aggregated data' do
          ...
          expect(result[:data]).to match_array([{ 'date' => nil, 'deployment_count' => 17,
                                                  metric => 17.fdiv(total_days) }])
        end
      end
      ...
      context 'when environment tiers are changed' do
        ...
        it 'returns the aggregated data' do
          expect(result[:status]).to eq(:success)
          expect(result[:data]).to match_array([{ 'date' => Date.parse('2022-02-05'), metric => 1 }])
        end
      end
    ...
    context 'when container is a group' do
      let_it_be(:maintainer) { create(:user) }
      let_it_be(:guest) { create(:user) }
      let_it_be(:group) { create(:group, maintainers: maintainer, guests: guest) }
      ...
      context 'when interval is all' do
        ...
        it 'returns the aggregated data' do
          expect(result[:status]).to eq(:success)
          expect(result[:data]).to match_array([{ 'date' => nil, 'deployment_count' => 3,
                                                  metric => 3.fdiv(total_days) }])
        end
      end
      ...
      context 'when group_project_ids parameter is given' do
        ...
        it 'returns the aggregated data' do
          expect(result[:status]).to eq(:success)
          expect(result[:data]).to match_array([{ 'date' => nil, 'deployment_count' => 1,
                                                  metric => 1.fdiv(total_days) }])
        end
      end
    end
  ...
  describe '#execute_without_authorization' do
    context 'runs the service without authorization' do
      subject(:result) { service.execute_without_authorization }
      ...
      context 'when passing a non-ultimate group' do
        ...
        it 'loads the deployment frequency metrics' do
          expect(result[:status]).to eq(:success)
          expect(result[:data]).to match_array([{ 'date' => nil, 'deployment_count' => 2,
                                                  metric => 2.fdiv(total_days) }])
        end
      end
    end
  end
