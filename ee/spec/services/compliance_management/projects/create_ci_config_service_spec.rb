...
RSpec.describe ComplianceManagement::Projects::CreateCiConfigService, feature_category: :compliance_management do
  ...
  context "when project doesn't have default branch" do
    ...
    let_it_be(:error, freeze: false) do
      { message: "Project must have default branch", status: :error, http_status: 422 }
    end
  ...
  context "when ci config is already present" do
    ...
    let_it_be(:error, freeze: false) { { message: "Ci config already present", status: :error, http_status: 422 } }

    it_behaves_like "service response is error"
  end

  context "when branch services returns error" do
    let_it_be(:error, freeze: false) { { message: "Something went wrong", status: :error } }
  ...
  context "when files create service returns error" do
    let_it_be(:error, freeze: false) { { message: "Something went wrong", status: :error, http_status: 422 } }
  ...
  context "when merge request is not valid" do
    let_it_be(:errors, freeze: false) { %w[test_error_1 test_error_2] }
    let_it_be(:error, freeze: false) { { message: "test_error_1 and test_error_2", status: :error, http_status: 422 } }
