...
RSpec.describe StatusPage::TriggerPublishService, feature_category: :incident_management do
  ...
  describe '#execute' do
    ...
    let_it_be_with_reload(:status_page_setting) do
    ...
    describe 'triggered by issue' do
      let_it_be_with_reload(:triggered_by) { create(:issue, :published, project: project) }
      ...
      context 'when reopening an issue' do
        include_examples 'trigger status page publish' do
          let_it_be(:triggered_by, freeze: false) { create(:issue, :closed, :published, project: project) }
    ...
    describe 'triggered by note' do
      ...
      context 'for issues' do
        let_it_be_with_refind(:triggered_by) do
        ...
        context 'as system note' do
          let_it_be_with_reload(:triggered_by) do
      ...
      context 'for merge requests' do
        let_it_be(:triggered_by, freeze: false) do
    ...
    describe 'triggered by award emoji' do
      ...
      context 'for notes on issues' do
        let_it_be(:awardable, freeze: false) { create(:note_on_issue, project: project) }
      ...
      context 'for issues' do
        let_it_be(:awardable, freeze: false) { create(:issue, project: project) }

        include_examples 'no trigger status page publish'
      end

      context 'for notes on merge requests' do
        let_it_be(:awardable, freeze: false) { create(:note_on_merge_request, project: project) }
    ...
    context 'with eligable triggered_by' do
      let_it_be(:triggered_by, freeze: false) { create(:issue, :published, project: project) }
