...
RSpec.describe PackageMetadata::Ingestion::DataMap, feature_category: :software_composition_analysis do
  context 'when storing packages' do
    ...
    it 'correctly stores packages by purl_type and name' do
      ...
      expect(map.get_package_id('npm', 'foo')).to be_nil
      expect(map.get_package_id('maven', 'foo')).to be_nil
  ...
  context 'when storing package versions' do
    ...
    it 'correctly stores package versions by package id and version' do
      expect(map.get_package_version_id('npm', 'foo', 'v1.0.0')).to be_nil
      expect(map.get_package_version_id('npm', 'foo', 'v1.0.1')).to be_nil
      expect(map.get_package_version_id('npm', 'bar', 'v1.0.0')).to be_nil
      expect(map.get_package_version_id('maven', 'foo', 'v1.0.0')).to be_nil
  ...
  context 'when storing licenses' do
    ...
    it 'stores the package version id' do
      expect(map.get_license_id('MIT')).to be_nil
      expect(map.get_license_id('Apache')).to be_nil
