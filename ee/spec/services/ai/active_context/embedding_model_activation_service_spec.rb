  ...
  describe '#execute!' do
    ...
    context 'when previous embedding model exists' do
      ...
      context 'when the update params are exactly the same as the current model' do
        ...
        it 'raises an InvalidError and does not create a task chain' do
    ...
    context 'when supported model_type is provided' do
      let(:model_type) { 'gitlab_managed' }
      ...
      it 'includes model_type in the metadata passed to update_metadata!' do
        ...
        expect(collection).to have_received(:update_metadata!).with(
          next_indexing_embedding_model: hash_including('model_type' => 'gitlab_managed')
        )
      end
    end
    ...
    context 'when unsupported model_type is provided' do
      ...
      it 'raises UpdateFailed and does not create a task chain' do
        expect(collection).to receive(:update_metadata!).with(
          next_indexing_embedding_model: hash_including('model_type' => 'gitlab')
        )
        expect(task_service).not_to receive(:create_chain)
    ...
    context 'when unsupported dimensions value is provided' do
      ...
      it 'raises UpdateFailed and does not create a task chain' do
        expect(collection).to receive(:update_metadata!).with(
          next_indexing_embedding_model: hash_including('dimensions' => 1000)
        )
        expect(task_service).not_to receive(:create_chain)
    ...
    context 'when unsupported model_ref is provided' do
      ...
      it 'raises InvalidError and does not create a task chain' do
        expect(task_service).not_to receive(:create_chain)

        expect { service.execute! }.to raise_error(
          described_class::InvalidError,
          "the given model_ref 'test_model_001' is not supported"
        )
      end
    end
