...
RSpec.describe ::Ai::DuoWorkflows::CreateAndStartWorkflowService, feature_category: :duo_agent_platform do
  ...
  context 'when additional_context is provided' do
    let(:additional_context) { [{ "Category" => "test", "Content" => '{"foo":"bar"}' }] }
    ...
    it 'forwards it to StartWorkflowService' do
      expect(Ai::DuoWorkflows::StartWorkflowService).to receive(:new)
        .with(workflow: anything, params: hash_including(additional_context: additional_context))
        .and_call_original
