...
RSpec.describe Ai::DuoWorkflows::McpConfigService, feature_category: :duo_agent_platform do
  ...
  let(:mcp_server_tool_names) do
    described_class::GITLAB_PREAPPROVED_TOOLS + %w[
      create_issue create_workitem_note
    ]
  end
  ...
  describe '#gitlab_enabled_tools' do
    context 'with default chat (no catalog agent)' do
      it 'returns default tools' do
        expect(service.gitlab_enabled_tools).to eq(described_class::GITLAB_PREAPPROVED_TOOLS)
      end
    end
  ...
  describe 'orbit MCP server' do
    ...
    context 'with agentic chat workflow' do
      ...
      it 'includes orbit server with tools and trusted flag' do
        ...
        expect(result[:orbit][:PreApprovedTools])
          .to eq(::API::Orbit::McpHandlers::ToolCatalog::TRUSTED_TOOL_NAMES)
    ...
    context 'when orbit_use_legacy_tools is disabled' do
      ...
      context 'with agentic chat workflow' do
        ...
        it 'excludes legacy tools from the pre-approved tools' do
          result = service.execute

          expect(result[:orbit][:PreApprovedTools])
            .to match_array(::API::Orbit::McpHandlers::ToolCatalog::COMMAND_TOOL_NAMES)
          expect(result[:orbit][:PreApprovedTools])
            .not_to include(*::API::Orbit::McpHandlers::ToolCatalog::ALL_LEGACY_TOOL_NAMES)
        end
      end

      context 'with a custom agent that has selected only legacy orbit tools' do
        let(:workflow_definition) { 'chat' }
      ...
      context 'with a custom agent that has selected legacy and command orbit tools' do
        ...
        it 'injects only the non-legacy selected tools' do
          result = service.execute
    ...
    context 'with a custom agent that has orbit MCP tools selected' do
      ...
      it 'restricts Tools and PreApprovedTools to selected orbit tools' do
        ...
        expect(result[:orbit][:Tools]).to match_array(%w[query_graph get_graph_schema])
        expect(result[:orbit][:PreApprovedTools]).to match_array(%w[query_graph get_graph_schema])
      end
    ...
    context 'with a custom agent that has mixed orbit and non-orbit MCP tools' do
      let(:workflow_definition) { 'chat' }
      let(:ai_catalog_item_version_id) { 42 }
      let(:mcp_tools) { %w[query_graph search create_workitem_note] }

      it 'includes orbit server with only orbit tools' do
        result = service.execute

        expect(result[:orbit][:Tools]).to match_array(%w[query_graph])
        expect(result[:orbit][:PreApprovedTools]).to match_array(%w[query_graph])
      end
      ...
      it 'includes gitlab server with non-orbit tools' do
        ...
        expect(result[:gitlab][:Tools]).not_to include('query_graph')
      end
    end
  ...
  describe 'constants' do
    it 'defines GITLAB_PREAPPROVED_TOOLS' do
      expect(described_class::GITLAB_PREAPPROVED_TOOLS).to eq(%w[
    ...
    it 'defines GITLAB_ENABLED_TOOLS as the union of preapproved and requiring approval' do
      expect(described_class::GITLAB_ENABLED_TOOLS).to eq(%w[
        gitlab_search search semantic_code_search
        get_merge_request get_merge_request_commits
        get_merge_request_diffs get_merge_request_pipelines
        get_pipeline_jobs get_job_log
