...
RSpec.describe ::Ai::DuoWorkflows::StartWorkflowService, :request_store, feature_category: :duo_agent_platform do
      ...
      shared_examples 'sandbox enabled behavior' do
        ...
        it 'combines setup_sandbox_commands, setup_script, main commands, CLI install, and wrapped executor commands' do
          ...
          expect(Ci::Workloads::RunWorkloadService).to have_received(:new) do |workload_definition:, **_kwargs|
            ...
            expect(commands[2..10]).to eq(shared_main_commands)
            expect(commands[11..15]).to eq(cli_install_commands)
  ...
  context 'when dap_full_clone is enabled', :aggregate_failures do
    include_context 'with Duo enabled'

    before do
      stub_feature_flags(dap_full_clone: true)
    end
    ...
    context 'with the developer flow' do
      ...
      it 'skips the runner clone and self-clones a full developer-like checkout' do
        expect(Ci::Workloads::RunWorkloadService).to receive(:new).and_wrap_original do |method, **kwargs|
          definition = kwargs[:workload_definition]

          expect(definition.variables[:GIT_STRATEGY]).to eq('none')
          expect(definition.variables).not_to have_key(:GIT_DEPTH)
          expect(definition.variables).not_to have_key(:GIT_FETCH_EXTRA_FLAGS)
          expect(definition.variables[:GIT_LFS_SKIP_SMUDGE]).to eq(1)

          # Authentication is provided by a credential.helper, not the clone URL,
          # so the token never lands in the clone URL or .git/config on disk.
          expect(definition.variables.values).to include('credential.helper')
          expect(definition.variables.values).to include(described_class::GIT_CREDENTIAL_HELPER)

          # Authenticate proactively so our self-clone stays in the authenticated
          # rate-limit bucket (the runner would do this via FF_USE_GIT_PROACTIVE_AUTH
          # on its own clone, which we skip with GIT_STRATEGY=none). Scoped to the
          # GitLab host so creds are not sent proactively to other hosts.
          expect(definition.variables.values).to include("http.#{Gitlab.config.gitlab.url}.proactiveAuth")

          script = definition.commands.join("\n")
          expect(script).to include('command -v git >/dev/null 2>&1 ||')
          # Clone into a sibling dir and move only .git so a restored cache survives.
          expect(script)
            .to include(
              'git clone --no-checkout --filter=blob:none ' \
                '"${DUO_WORKFLOW_GIT_HTTP_BASE_URL}/${GITLAB_PROJECT_PATH}.git"'
            )
          expect(script).to include('mv "${CI_PROJECT_DIR}.dap-tmp/.git" "${CI_PROJECT_DIR}/.git"')
          expect(script).to include('git checkout -f "${DUO_WORKFLOW_SOURCE_BRANCH:-$CI_DEFAULT_BRANCH}"')
          # No oauth-in-URL hack and no `git remote set-url` rewrite in full-clone mode.
          expect(script).not_to include('sed')
          expect(script).not_to include('oauth:')
      ...
      context 'without a source_branch' do
        ...
        it 'leaves DUO_WORKFLOW_SOURCE_BRANCH blank so checkout falls back to CI_DEFAULT_BRANCH' do
          expect(Ci::Workloads::RunWorkloadService).to receive(:new).and_wrap_original do |method, **kwargs|
            definition = kwargs[:workload_definition]

            expect(definition.variables[:DUO_WORKFLOW_SOURCE_BRANCH]).to be_nil
            expect(definition.commands.join("\n"))
              .to include('git checkout -f "${DUO_WORKFLOW_SOURCE_BRANCH:-$CI_DEFAULT_BRANCH}"')
    ...
    context 'with a non-developer flow' do
      it 'keeps the default shallow clone behavior' do
        expect(Ci::Workloads::RunWorkloadService).to receive(:new).and_wrap_original do |method, **kwargs|
          definition = kwargs[:workload_definition]

          expect(definition.variables).not_to have_key(:GIT_STRATEGY)
          expect(definition.commands.join("\n")).not_to include('git clone')
  ...
  context 'when dap_full_clone is disabled', :aggregate_failures do
    include_context 'with Duo enabled'

    before do
      stub_feature_flags(dap_full_clone: false)
    end

    it 'still sets proactiveAuth independently of the full_clone flag' do
      expect(Ci::Workloads::RunWorkloadService).to receive(:new).and_wrap_original do |method, **kwargs|
        definition = kwargs[:workload_definition]
