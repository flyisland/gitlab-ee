...
RSpec.describe ::Ai::DuoWorkflows::StartWorkflowService, :request_store, feature_category: :duo_agent_platform do
  ...
  context 'when shallow_clone is not set', :aggregate_failures do
    ...
    # shallow_clone / dap_git_tree_zero_option only affect git_clone_variables when
    # the runner is not doing the full clone, so exercise the legacy path here.
    before do
  ...
  context 'when dap_full_clone is enabled', :aggregate_failures do
    # dap_full_clone is a development flag, so it is enabled by default in the suite.
    include_context 'with Duo enabled'
    ...
    context 'with a non-developer flow' do
      it 'also gets the full blobless clone and HEAD re-attach (flag applies to all flows)' do
        expect(Ci::Workloads::RunWorkloadService).to receive(:new).and_wrap_original do |method, **kwargs|
          definition = kwargs[:workload_definition]
  ...
  context 'with Git LFS skip configuration', :aggregate_failures do
    ...
    # dap_git_tree_zero_option only affects GIT_FETCH_EXTRA_FLAGS on the legacy
    # (non-full-clone) path; full clone always uses --prune --quiet --filter=blob:none.
    before do
      stub_feature_flags(dap_full_clone: false)
    end
    ...
    context 'when dap_git_tree_zero_option feature flag is disabled' do
      ...
      it 'sets GIT_FETCH_EXTRA_FLAGS to --prune --quiet --filter=blob:none' do
        expect(Ci::Workloads::RunWorkloadService).to receive(:new).and_wrap_original do |method, **kwargs|
          workload_definition = kwargs[:workload_definition]
          variables = workload_definition.variables
    ...
    context 'when dap_git_tree_zero_option feature flag is enabled' do
      ...
      it 'sets GIT_FETCH_EXTRA_FLAGS to --prune --quiet --filter=tree:0' do
        expect(Ci::Workloads::RunWorkloadService).to receive(:new).and_wrap_original do |method, **kwargs|
          workload_definition = kwargs[:workload_definition]
          variables = workload_definition.variables

          expect(variables[:GIT_FETCH_EXTRA_FLAGS]).to eq('--prune --quiet --filter=tree:0')
  ...
  context 'with coding_environment declaration', :aggregate_failures do
    ...
    # dap_full_clone selects how to clone; coding_environment selects whether to clone.
    # The declaration must be honoured on both sides of the flag.
    context 'when dap_full_clone is disabled' do
      before do
        stub_feature_flags(dap_full_clone: false)
      end
      ...
      context 'when nothing declares a coding environment' do
        it 'is unaffected and uses the legacy clone variables' do
          expect(variables).not_to have_key(:GIT_STRATEGY)
          expect(variables[:GIT_LFS_SKIP_SMUDGE]).to eq(1)
          expect(script).to include('git remote set-url origin')
        end
