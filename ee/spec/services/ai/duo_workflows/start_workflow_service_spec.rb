...
RSpec.describe ::Ai::DuoWorkflows::StartWorkflowService, :request_store, feature_category: :duo_agent_platform do
  ...
  describe 'flow versioning params' do
    ...
    context 'when flow_definition is provided in params' do
      let(:params) do
        super().merge(
      ...
      it 'uses the provided params for env vars and workload definition' do
        expect(Ci::Workloads::RunWorkloadService).to receive(:new).and_wrap_original do |method, **kwargs|
          expect(kwargs[:duo_workflow_definition]).to eq('developer_unstable/experimental')
    ...
    context 'when flow versioning params are not provided' do
      it 'falls back to workflow_definition from the workflow record' do
        expect(Ci::Workloads::RunWorkloadService).to receive(:new).and_wrap_original do |method, **kwargs|
          expect(kwargs[:duo_workflow_definition]).to eq('developer/v1')
          variables = kwargs[:workload_definition].variables
          expect(variables[:DUO_WORKFLOW_DEFINITION]).to eq('developer/v1')
          expect(variables[:DUO_WORKFLOW_FLOW_CONFIG_ID]).to be_nil
          expect(variables[:DUO_WORKFLOW_FLOW_CONFIG_SCHEMA_VERSION]).to be_nil
          expect(variables[:DUO_WORKFLOW_FLOW_VERSION]).to be_nil
          method.call(**kwargs)
        end
