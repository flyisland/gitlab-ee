...
RSpec.describe ::Ai::DuoWorkflows::StartWorkflowService, :request_store, feature_category: :duo_agent_platform do
  ...
  describe 'developer workflow definition resolution' do
    let(:workflow) do
      create(:duo_workflows_workflow, user: maintainer, image: image, workflow_definition: workflow_definition,
     **container_params)
    end
    ...
    shared_examples 'returning the correct workflow definition' do
      it 'sets the correct workflow definition' do
        expect(Ci::Workloads::RunWorkloadService).to receive(:new).and_wrap_original do |method, **kwargs|
          expect(kwargs[:duo_workflow_definition]).to eq(expected_workflow_definition)
          expect(kwargs[:workload_definition].variables[:DUO_WORKFLOW_DEFINITION]).to eq(expected_workflow_definition)
          method.call(**kwargs)
        end
    ...
    context 'when workflow_definition is developer/v1' do
      let(:workflow_definition) { 'developer/v1' }

      context 'when no developer next feature flags are enabled' do
        let(:expected_workflow_definition) { 'developer/v1' }

        before do
          stub_feature_flags(duo_developer_next_unstable: false, duo_developer_next_compatible: false)
        end

        it_behaves_like 'returning the correct workflow definition'
      end

      context 'when duo_developer_next_unstable is enabled for the project' do
        let(:expected_workflow_definition) { 'developer_unstable/experimental' }
      ...
      context 'when duo_developer_next_compatible is enabled for the project' do
        let(:expected_workflow_definition) { 'developer_next/experimental' }

        before do
          stub_feature_flags(duo_developer_next_unstable: false, duo_developer_next_compatible: project)
        end

        it_behaves_like 'returning the correct workflow definition'
      end

      context 'when duo_developer_next_compatible is enabled for the root ancestor' do
        let(:expected_workflow_definition) { 'developer_next/experimental' }

        before do
          stub_feature_flags(duo_developer_next_unstable: false, duo_developer_next_compatible: project.root_ancestor)
        end

        it_behaves_like 'returning the correct workflow definition'
      end

      context 'when both duo_developer_next_unstable and duo_developer_next_compatible are enabled' do
        let(:expected_workflow_definition) { 'developer_unstable/experimental' }

        before do
          stub_feature_flags(duo_developer_next_unstable: project, duo_developer_next_compatible: project)
        end
    ...
    context 'when workflow_definition is fix_pipeline/v1' do
      let(:workflow_definition) { 'fix_pipeline/v1' }
      let(:expected_workflow_definition) { 'fix_pipeline_next/v1' }

      it_behaves_like 'returning the correct workflow definition'

      context 'when fix_pipeline_next feature flag is disabled' do
        let(:expected_workflow_definition) { 'fix_pipeline/v1' }
  ...
  context 'with setup_script configuration' do
    ...
    let(:sandbox_setup_commands) { ['mkdir -p /tmp/gitlab_duo_agent_platform'] }
      ...
      shared_examples 'sandbox enabled behavior' do
        ...
        it 'combines setup_sandbox_commands, setup_script, main commands, CLI install, and wrapped executor commands' do
          ...
          expect(Ci::Workloads::RunWorkloadService).to have_received(:new) do |workload_definition:, **_kwargs|
            ...
            expect(commands[2..9]).to eq(shared_main_commands)
            expect(commands[10..13]).to eq(cli_install_commands)
            expect(commands[14..]).to eq(wrapped_commands)
  ...
  describe 'git environment variables' do
    ...
    it 'sets git config variables for safe directory and URL rewriting' do
      expect(Ci::Workloads::RunWorkloadService)
        ...
        expect(variables[:GIT_CONFIG_VALUE_0]).to eq('${CI_PROJECT_DIR}')
