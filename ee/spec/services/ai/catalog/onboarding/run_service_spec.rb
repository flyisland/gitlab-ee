...
RSpec.describe Ai::Catalog::Onboarding::RunService, feature_category: :ai_catalog_curation do
  ...
  describe '#execute' do
    ...
    context 'when the readiness card feature flag is disabled' do
      before do
    ...
    context 'when a workflow is already active for the initializer' do
      let(:active_workflow) { build_stubbed(:duo_workflows_workflow) }
      ...
      before do
        allow_next_instance_of(Ai::Catalog::Onboarding::WorkflowTracker) do |tracker|
          allow(tracker).to receive(:active_workflow).with('init_project_context')
            .and_return(active_workflow)
        end
      end

      it 'returns an error carrying the active workflow id, without starting a new workflow', :aggregate_failures do
        expect(goal_service).not_to receive(:new)
        ...
        expect(result.payload[:workflow_id]).to eq(active_workflow.id)
      end
    end
  end
end
