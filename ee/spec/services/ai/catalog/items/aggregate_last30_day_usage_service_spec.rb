...
RSpec.describe Ai::Catalog::Items::AggregateLast30DayUsageService, feature_category: :workflow_catalog do
  ...
  describe '#execute' do
    ...
    let_it_be(:item_version) { create(:ai_catalog_item_version, item: item) }
    ...
    context 'when updating usage counts' do
      let_it_be(:other_item) { create(:ai_catalog_item) }
      let_it_be(:other_version) { create(:ai_catalog_item_version, item: other_item) }
    ...
    context 'when item is deleted' do
      let_it_be(:deleted_item) { create(:ai_catalog_item, deleted_at: 1.day.ago, project: project) }
      let_it_be(:item_version) { create(:ai_catalog_item_version, item: deleted_item) }
    ...
    context 'when workflow record is not associated with a project' do
      ...
      let_it_be(:item_version) { create(:ai_catalog_flow_version) }
