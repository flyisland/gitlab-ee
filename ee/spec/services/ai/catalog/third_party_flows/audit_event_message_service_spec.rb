...
RSpec.describe Ai::Catalog::ThirdPartyFlows::AuditEventMessageService, feature_category: :workflow_catalog do
  it_behaves_like 'Ai::Catalog::AuditEventMessageService' do
    let_it_be(:item_name, freeze: false) { 'external agent' }
    let_it_be(:event_name_prefix, freeze: false) { 'ai_catalog_third_party_flow' }
    let_it_be(:schema_version_constant, freeze: false) { Ai::Catalog::ItemVersion::THIRD_PARTY_FLOW_SCHEMA_VERSION }
    ...
    describe '#messages' do
      ...
      context 'when event_type is create_ai_catalog_third_party_flow' do
        ...
        context 'when third_party_flow is public (legacy)' do
          before do
        ...
        context 'when third_party_flow is internal' do
          before do
            item.update!(visibility: :internal)
          end

          it 'returns create messages for internal external agent' do
            expect(messages).to contain_exactly(
              "Created a new internal AI external agent",
              "Created new draft version #{version.version} of AI external agent"
            )
          end

          context 'when internal visibility flag is disabled' do
            before do
              stub_feature_flags(ai_catalog_internal_visibility: false)
            end

            it 'falls back to the legacy public property for the visibility label' do
              expect(messages).to contain_exactly(
                "Created a new private AI external agent",
                "Created new draft version #{version.version} of AI external agent"
              )
      ...
      context 'when event_type is update_ai_catalog_third_party_flow' do
        ...
        context 'when visibility changes to public (legacy)' do
          before do
            item.update!(public: true)
          end
        ...
        context 'when visibility changes to internal' do
          before do
            item.update!(visibility: :internal)
          end

          it 'returns visibility change message' do
            expect(messages).to contain_exactly('Made AI external agent internal')
          end
          ...
          context 'when internal visibility flag is disabled' do
            ...
            it 'does not consider visibility to have changed' do
              expect(messages).to contain_exactly('Updated AI external agent')
            end
          end
        end

        context 'when visibility changes to private' do
          let_it_be(:item, freeze: false) do
        ...
        context 'when visibility changes to private (legacy)' do
          let_it_be(:item, freeze: false) do
            create(:ai_catalog_third_party_flow, :public, project: project)
          end
