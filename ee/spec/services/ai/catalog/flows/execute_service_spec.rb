...
RSpec.describe Ai::Catalog::Flows::ExecuteService, :aggregate_failures, feature_category: :workflow_catalog do
  ...
  let_it_be(:flow_owner_project, freeze: false) { create(:project, group: group, developers: developer) }
  let_it_be(:flow, freeze: false) { create(:ai_catalog_flow, project: flow_owner_project, public: true) }
  let_it_be(:user_prompt) { nil }
  let_it_be(:another_project, freeze: false) { create(:project, group: group) }
  let_it_be(:item_enabled_project, freeze: false) { create(:project, :repository, group: group, developers: developer) }
  ...
  describe '#execute' do
    ...
    context 'when flow is properly executed' do
      ...
      context 'when resume_context is provided' do
        let(:existing_workflow) { instance_double(::Ai::DuoWorkflows::Workflow, id: 1) }
