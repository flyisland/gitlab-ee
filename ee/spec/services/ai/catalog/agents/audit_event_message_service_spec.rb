  ...
  it_behaves_like 'Ai::Catalog::AuditEventMessageService' do
    ...
    describe '#messages' do
      ...
      context 'when event_type is create_ai_catalog_agent' do
        ...
        context 'when agent is public (legacy)' do
          before do
        ...
        context 'when agent is internal' do
          before do
            item.update!(visibility: :internal)
          end

          it 'returns create messages for internal agent' do
            expect(messages).to contain_exactly(
              "Created a new internal AI agent with tools: [gitlab_blob_search]",
              "Created new draft version #{version.version} of AI agent"
            )
          end
          ...
          context 'when internal visibility flag is disabled' do
            ...
            it 'falls back to the legacy public property for the visibility label' do
              expect(messages).to contain_exactly(
                "Created a new private AI agent with tools: [gitlab_blob_search]",
                "Created new draft version #{version.version} of AI agent"
              )
            end
          end
      ...
      context 'when event_type is update_ai_catalog_agent' do
        ...
        context 'when visibility changes to internal' do
          ...
          it 'returns visibility change message' do
            expect(messages).to contain_exactly('Made AI agent internal')
          end
          ...
          context 'when internal visibility flag is disabled' do
            ...
            it 'does not consider visibility to have changed' do
              expect(messages).to contain_exactly('Updated AI agent')
            end
          end
        end
