...
RSpec.describe WorkItems::AggregateChildIssuesService, feature_category: :team_planning do
  ...
  let_it_be(:child_issue1, freeze: false) { create(:work_item, :issue, project: project, weight: 2, state: :opened) }
  let_it_be(:child_issue2, freeze: false) { create(:work_item, :issue, project: project, weight: 3, state: :opened) }
  ...
  describe '#execute' do
    ...
    context 'when count_health_status is true' do
      let_it_be(:child_issue4, freeze: false) { create(:work_item, :issue, project: project, state: :opened) }
      let_it_be(:child_issue5, freeze: false) { create(:work_item, :issue, project: project, state: :opened) }
      let_it_be(:child_issue6, freeze: false) { create(:work_item, :issue, project: project, state: :opened) }
