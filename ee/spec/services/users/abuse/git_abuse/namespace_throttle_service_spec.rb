...
RSpec.describe Users::Abuse::GitAbuse::NamespaceThrottleService, feature_category: :insider_threat do
  describe '.execute' do
    ...
    let_it_be_with_reload(:namespace) do
      create(
        :group,
        owners: [namespace_admin, inactive_namespace_admin],
    ...
    context 'when user is not rate-limited' do
      ...
      it 'does not ban the user' do
        ...
        expect(user.banned_from_namespace?(namespace)).to be(false)
      end
    end
    ...
    context 'when user is rate-limited' do
      ...
      it 'bans the user' do
        ...
        expect(user.banned_from_namespace?(namespace)).to be(true)
      end
      ...
      it 'logs the event', :aggregate_failures do
        ...
        expect(AuditEventReader.where(entity_type: 'Group', entity_id: namespace.id).last).to have_attributes(
          entity_type: 'Group',
