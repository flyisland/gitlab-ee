...
RSpec.describe Users::MemberRoles::AssignService, feature_category: :permissions do
  ...
  shared_examples 'assigns roles correctly' do
    ...
    context 'when current user is an admin', :enable_admin_mode do
      ...
      context 'when the provided member role is not an admin role' do
        let_it_be(:member_role, freeze: false) { create(:member_role, name: 'Standard role') }
  ...
  context 'with member admin roles' do
    let_it_be(:join_record_klass, freeze: false) { ::Users::UserMemberRole }
    let_it_be(:join_factory_klass_name, freeze: false) { :user_member_role }
    let_it_be(:admin_role_1, freeze: false) { create(:member_role, :admin, name: 'Admin role 1') }
    let_it_be(:admin_role_2, freeze: false) { create(:member_role, :admin, name: 'Admin role 2') }
  ...
  context 'with admin roles' do
    let_it_be(:join_record_klass, freeze: false) { ::Authz::UserAdminRole }
    let_it_be(:join_factory_klass_name, freeze: false) { :user_admin_role }
    let_it_be(:admin_role_1, freeze: false) { create(:admin_role, name: 'Admin role 1') }
    let_it_be(:admin_role_2, freeze: false) { create(:admin_role, name: 'Admin role 2') }
