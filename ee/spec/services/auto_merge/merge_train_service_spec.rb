...
RSpec.describe AutoMerge::MergeTrainService, feature_category: :merge_trains do
  ...
  describe '#cancel' do
    ...
    context 'when pipeline exists' do
      ...
      context 'when merge_train_car is nil and unstick_stuck_merge_requests is disabled' do
        before do
          stub_feature_flags(unstick_stuck_merge_requests: false)
          allow(merge_request).to receive(:merge_train_car).and_return(nil)
        end

        it 'does not raise an error or process the next merge request' do
          expect(MergeTrains::RefreshWorker).not_to receive(:perform_async)
    ...
    context 'when the other merge request is following the merge request' do
      ...
      context 'when unstick_stuck_merge_requests is disabled' do
        ...
        it 'uses the next merge train car to process the train' do
          expect(merge_request.merge_train_car).to receive(:next_active).and_call_original
          expect(merge_request.merge_train_car).not_to receive(:any_next)

          expect(MergeTrains::RefreshWorker).to receive(:perform_async).with(
            merge_request_2.target_project_id,
            merge_request_2.target_branch
          )
    ...
    context 'when the other merge request is following the merge request' do
      ...
      context 'when merge_train_car is nil and unstick_stuck_merge_requests is disabled' do
        ...
        it 'does not raise an error or process the next merge request' do
          ...
          expect { service_abort }.not_to raise_error
        end
      end
    end
