...
RSpec.describe DependencyManagement::SecurityUpdate::CreateMergeRequestService,
  ...
  describe '#execute' do
    ...
    context 'when untrusted input contains markdown-sensitive characters' do
      ...
      context 'when the advisory title contains markdown, references and backticks' do
        before do
          allow(vulnerability).to receive(:title).and_return(
            "Path Traversal via `%2F`\n[click](http://evil.example) @group"
          )
        end

        it 'wraps the title in a single-line code span so none of it is interpreted' do
          result = execute
          mr = result.payload[:merge_request]

          expect(mr.description)
            .to include('``Path Traversal via `%2F` [click](http://evil.example) @group``')
        end
      end

      context 'when the file path and CVE value contain backticks' do
        let(:output_json) do
          {
            'dependency_updates' => [
        ...
        before do
          allow(vulnerability.finding).to receive(:cve_value).and_return('CVE-1`a`b')
        end

        it 'wraps them in backtick-safe code spans' do
          result = execute
          mr = result.payload[:merge_request]

          expect(mr.description).to include('``a`b.lock``')
          expect(mr.description).to include('``CVE-1`a`b``')
        end
      end
    end
