...
RSpec.describe 'EE-specific user routing' do
  ...
  describe 'sign up routes', feature_category: :acquisition do
    let(:trial_first_registration_match?) { false }
    let(:trial_user_match?) { false }
    let(:invite_user_match?) { false }
    let(:subscription_user_match?) { false }

    before do
      allow_any_instance_of(Onboarding::TrialFirstRegistrationConstraint).to receive(:matches?).and_return(trial_first_registration_match?) # rubocop:disable RSpec/AnyInstanceOf -- Needed as it is not the next instance
    ...
    context 'when trial first registration constraint is satisfied' do
      let(:trial_first_registration_match?) { true }

      it 'routes to trial_registrations controller for new' do
        expect(get: '/users/sign_up/').to route_to(controller: 'trial_registrations', action: 'new')
      end
    end

    context 'when trial first registration constraint is not satisfied' do
    ...
    context 'for ordering priority' do
      ...
      where(:trial_match, :invite_match, :subscription_match, :expected_controller) do
        true  | true  | false | 'registrations/trial_welcome'
        true  | false | true  | 'registrations/trial_welcome'
        true  | true  | true  | 'registrations/trial_welcome'
        false | true  | true  | 'registrations/invite_welcome'
        false | false | true  | 'registrations/subscription_welcome'
      end
