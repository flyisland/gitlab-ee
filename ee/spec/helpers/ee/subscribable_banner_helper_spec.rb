    ...
    context 'when instance variable true' do
      ...
      context 'when gitlab_com_subscriptions saas feature available' do
        ...
        shared_examples 'subscription message' do
          it 'calls Gitlab::ExpiringSubscriptionMessage and SubscriptionPresenter if is Gitlab.com?' do
            ...
            allow(helper).to receive(:can?).with(user, :edit_billing, root_namespace).and_return(true)
      ...
      context 'when gitlab_com_subscriptions saas feature is not available' do
        ...
        before do
          stub_saas_features(gitlab_com_subscriptions: false)
          allow(License).to receive(:current).and_return(license)
          allow(helper).to receive_messages(current_user: user, signed_in?: true)
