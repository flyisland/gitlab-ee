...
RSpec.describe ::SidebarsHelper, feature_category: :navigation do
  ...
  describe '#super_sidebar_whats_new_data' do
    before do
      allow(helper).to receive(:display_whats_new?).and_return(true)
    end

    subject { helper.super_sidebar_whats_new_data }

    context 'when on SaaS and feature flag is enabled', :saas do
      before do
        stub_saas_features(gitlab_com_subscriptions: true)
      end

      it { is_expected.to include(whats_new_show_transcend_promo: true) }
    end

    context 'when on SaaS and feature flag is disabled', :saas do
      before do
        stub_saas_features(gitlab_com_subscriptions: true)
        stub_feature_flags(whats_new_transcend_promo: false)
      end

      it { is_expected.to include(whats_new_show_transcend_promo: false) }
    end
  ...
  describe '#context_switcher_links' do
    ...
    let_it_be(:current_user_mode, freeze: false) { Gitlab::Auth::CurrentUserMode.new(user) }
