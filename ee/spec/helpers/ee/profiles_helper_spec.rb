...
RSpec.describe ProfilesHelper, feature_category: :system_access do
  ...
  describe '#prevent_delete_account?' do
    ...
    context 'with self-managed instance setting' do
      using RSpec::Parameterized::TableSyntax
    ...
    context 'on SaaS with enterprise user', :saas do
      let(:enterprise_group) { instance_double(Group) }
      ...
      context 'when group prevents account deletion' do
        before do
          allow(enterprise_group).to receive(:prevent_enterprise_user_account_deletion?).and_return(true)
        end

        it { is_expected.to be(true) }
      end

      context 'when group allows account deletion' do
        before do
          allow(enterprise_group).to receive(:prevent_enterprise_user_account_deletion?).and_return(false)
        end
