...
RSpec.describe EE::AccessTokensHelper, feature_category: :system_access do
  ...
  describe '#personal_access_token_data' do
    ...
    it 'includes agentic_available when user can access agentic chat' do
      allow(user).to receive(:can?).with(:access_duo_agentic_chat).and_return(true)

      data = helper.personal_access_token_data({})

      expect(data[:access_token][:agentic_available]).to eq('true')
    end

    it 'sets agentic_available to false when user cannot access agentic chat' do
      allow(user).to receive(:can?).with(:access_duo_agentic_chat).and_return(false)

      data = helper.personal_access_token_data({})

      expect(data[:access_token][:agentic_available]).to eq('false')
    end
  end
end
