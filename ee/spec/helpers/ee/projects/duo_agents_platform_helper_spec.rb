...
RSpec.describe EE::Projects::DuoAgentsPlatformHelper, feature_category: :duo_agent_platform do
  ...
  describe '#duo_agents_platform_identity_verification_required?' do
    subject(:required) { helper.duo_agents_platform_identity_verification_required?(group) }

    before do
      allow(Ability).to receive(:allowed?).and_call_original
      allow(Ability).to receive(:allowed?).with(user, :duo_workflow, group).and_return(can_use_dap)
      allow(Ability).to receive(:allowed?).with(user, :read_duo_agent_platform, group).and_return(entitled)
    end

    context 'when entitled to the page but blocked solely by identity verification' do
      let(:entitled) { true }
      let(:can_use_dap) { false }

      it { is_expected.to be true }
    end

    context 'when the user can already use DAP (not blocked by IV)' do
      let(:entitled) { true }
      let(:can_use_dap) { true }

      it { is_expected.to be false }
    end

    context 'when the user cannot read the DAP page at all' do
      let(:entitled) { false }
      let(:can_use_dap) { false }
  ...
  describe '#duo_agents_platform_identity_verification_data' do
    ...
    before do
      allow(helper).to receive(:duo_agents_platform_identity_verification_required?).with(group).and_return(required)
    end
  ...
  describe '#tanuki_bot_for' do
    it 'creates separate instances for different containers' do
      expect(::Gitlab::Llm::TanukiBot).to receive(:new).twice.and_call_original

      helper.send(:tanuki_bot_for, project)
      helper.send(:tanuki_bot_for, group)
    end
  end

  describe '#usage_billing_forbidden?' do
    context 'when container is a Project' do
      it 'returns true when TanukiBot billing is forbidden' do
        allow_next_instance_of(::Gitlab::Llm::TanukiBot) do |instance|
      ...
      it 'returns false when TanukiBot billing is not forbidden' do
        allow_next_instance_of(::Gitlab::Llm::TanukiBot) do |instance|
