...
RSpec.describe EE::Groups::SettingsHelper, feature_category: :groups_and_projects do
  ...
  describe '#available_foundational_flows_json' do
    subject(:flows) { Gitlab::Json.safe_parse(helper.send(:available_foundational_flows_json)) }

    before do
      allow(helper).to receive(:allow_beta_experimental_ai_features?).and_return(false)
    end
    ...
    context 'when group is root and has ai_features license (Ultimate)' do
      before do
      ...
      it 'includes ultimate-only flows' do
        references = flows.pluck('reference')

        expect(references).to include('resolve_sast_vulnerability/v1')
      end
    end

    context 'when group is root but does not have ai_features license (Free/Premium)' do
      before do
        allow(group).to receive(:root?).and_return(true)
        allow(group).to receive(:licensed_feature_available?).with(:ai_features).and_return(false)
      end
  ...
  describe 'group_ai_settings_helper_data' do
        ...
        with_stubbed_translations(:de, stubbed_translations) do
          expect(settings[:available_foundational_flows]).to eq(
            Gitlab::Json.generate([
              { name: 'Test Flow in German', description: 'Test Description in German', reference: 'test_flow/v1' }
            ])
          )
        end
      end
    ...
    context 'with duo_access_through_namespaces disabled' do
      ...
      it { expect(settings).not_to have_key(:new_group_path) }
    end
    ...
    context 'when group is on free tier (no ai_features license)' do
      before do
  ...
  describe '#show_virtual_registries_setting?' do
    it 'delegates to VirtualRegistries.any_registry_available?' do
      allow(::VirtualRegistries).to receive(:any_registry_available?)
        .with(group, current_user, :admin_virtual_registry).and_return(true)

      expect(helper.show_virtual_registries_setting?(group)).to be true
    end

    it 'returns false when any_registry_available? returns false' do
      allow(::VirtualRegistries).to receive(:any_registry_available?)
        .with(group, current_user, :admin_virtual_registry).and_return(false)
  ...
  describe '#usage_billing_dashboard_data', :saas do
    ...
    it 'includes gitlab_com_purchase_credits_path' do
      is_expected.to include(
        gitlab_com_purchase_credits_path: include("gl_namespace_id=#{group.id}", 'plan_type=gitlab_credits')
      )
    end
  end
