...
RSpec.describe Admin::IdentitiesHelper, feature_category: :system_access do
  ...
  describe '#identity_cells_to_render?' do
    context 'without SCIM identies' do
      it 'returns false' do
        expect(helper.identity_cells_to_render?([], user_without_scim_identities)).to be false
      end
    end

    context 'with SCIM identities' do
      it 'returns true' do
        expect(helper.identity_cells_to_render?([], user_with_scim_identities)).to be true
      end
    end
  end
