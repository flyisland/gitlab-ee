...
RSpec.describe AuditEventsHelper, feature_category: :audit_events do
  ...
  describe '#show_streams_for_group?' do
    ...
    it 'returns false if the group is a subgroup' do
      expect(helper.show_streams_for_group?(subgroup)).to be(false)
    end
  ...
  describe '#view_only_own_group_events?' do
    ...
    context 'when the user is group admin' do
      ...
      it "returns false" do
        expect(helper.view_only_own_group_events?(group)).to be(false)
      end
    end
    ...
    context 'when the user does not have permission' do
      ...
      it "returns true" do
        expect(helper.view_only_own_group_events?(group)).to be(true)
      end
    end
  end
  ...
  describe '#view_only_own_project_events?' do
    ...
    context 'when the user is project admin' do
      ...
      it "returns false" do
        expect(helper.view_only_own_project_events?(project)).to be(false)
      end
    end
    ...
    context 'when the user does not have permission' do
      ...
      it "returns true" do
        expect(helper.view_only_own_project_events?(project)).to be(true)
      end
    end
  end
  ...
  describe '#audit_events_details_drawer_enabled?' do
    let_it_be(:project) { build_stubbed(:project) }

    it 'returns true when feature flag is enabled' do
      stub_feature_flags(audit_events_details_drawer: project)

      expect(helper.audit_events_details_drawer_enabled?(project)).to be(true)
    end

    it 'returns false when feature flag is disabled' do
      stub_feature_flags(audit_events_details_drawer: false)

      expect(helper.audit_events_details_drawer_enabled?(project)).to be(false)
    end
  end
  ...
  describe '#audit_log_app_data' do
    ...
    let_it_be(:events) { build_list(:audit_events_group_audit_event, 5, target_group: group) }
    ...
    it 'returns the correct data' do
      ...
      expect(helper.audit_log_app_data(true, events, audit_event_definitions)).to contain_exactly(
        ...
        [:audit_events_details_drawer_enabled, "false"]
      )
    end
  end
end
