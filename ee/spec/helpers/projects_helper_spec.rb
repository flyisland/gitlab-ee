...
RSpec.describe ProjectsHelper, feature_category: :shared do
      ...
      describe 'botAccessSettingsAvailable' do
        ...
        context 'when licensed' do
          ...
          context 'when experiment is not enabled' do
            before do
              allow(project.project_setting)
                .to receive(:pipeline_execution_policy_bot_access_experiment_enabled?).and_return(false)
            end
  ...
  describe '#gitlab_duo_settings_data' do
    ...
    describe 'visibleSettings' do
      ...
      context 'when the user can only update the SAST Vulnerability Resolution setting' do
        before do
        ...
        it 'restricts visible settings to the SAST VR workflow toggle' do
          expect(data[:visibleSettings]).to eq(['duoSastVrWorkflowEnabled'])
        end
      end
  ...
  describe '#product_analytics_settings_allowed?' do
    ...
    with_them do
      before do
        ...
        allow(helper).to receive(:current_user).and_return(user)
        allow(helper).to receive(:can?).with(user, :update_product_analytics_settings,
