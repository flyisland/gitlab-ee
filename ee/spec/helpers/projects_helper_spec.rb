...
RSpec.describe ProjectsHelper, feature_category: :shared do
  ...
  describe '#group_project_templates_count' do
    ...
    context 'when constrain_group_project_templates feature flag is enabled' do
      before do
        stub_feature_flags(constrain_group_project_templates: true)
      ...
      context 'when template project is pending deletion' do
        ...
        it 'does not include the project in the count' do
          result = helper.group_project_templates_count(parent_group.id)
  ...
  describe '#show_duo_otel_button?' do
    ...
    let(:ruby_language) { instance_double(RepositoryLanguage, name: 'Ruby') }

    subject { helper.show_duo_otel_button?(project, user) }
    ...
    before do
      allow(project).to receive_messages(duo_remote_flows_enabled: true, duo_features_enabled: true,
        licensed_ai_features_available?: true, repository_languages: [ruby_language])
    end
    ...
    context 'when user cannot push code' do
      ...
      subject { helper.show_duo_otel_button?(project, guest_user) }
    ...
    context 'when licensed AI features are not available' do
      before do
        allow(project).to receive(:licensed_ai_features_available?).and_return(false)
      end
