...
RSpec.describe ::Routing::PseudonymizationHelper do
  ...
  before do
    ...
    allow(helper).to receive_messages(group: group, project: project)
  end
  ...
  describe 'when url has params to mask' do
    ...
    context 'when organization_path is present' do
      let(:masked_url) { "http://localhost/o/organization#{organization.id}/-/activity" }
