...
RSpec.describe 'Elastic Ingestion Pipeline', :sidekiq_inline, feature_category: :global_search do
  ...
  context 'for legacy User reference string conversion', :elastic_delete_by_query do
    let_it_be(:indexed_user) { create(:user) }
    ...
    context 'when a legacy reference string is already in the queue' do
      it 'converts to a new-framework ref and indexes correctly' do
