...
RSpec.describe Emails::Ai::DuoWorkflows, feature_category: :duo_agent_platform do
  ...
  describe '#duo_workflow_input_required_email' do
    ...
    let(:checkpoint_data) { { 'channel_values' => { 'ui_chat_log' => ui_chat_log } } }
    let(:checkpoint) { double('Ai::DuoWorkflows::Checkpoint', checkpoint: checkpoint_data) } # rubocop:disable RSpec/VerifiedDoubles -- checkpoint column clashes with class name
    ...
    before do
      ...
      allow(workflow).to receive_message_chain(:checkpoints, :latest).and_return(checkpoint)
    end
    ...
    context 'when the workflow has no project (namespace-level flow)' do
      ...
      before do
        ...
        allow(namespace_workflow).to receive_message_chain(:checkpoints, :latest).and_return(checkpoint)
      end
