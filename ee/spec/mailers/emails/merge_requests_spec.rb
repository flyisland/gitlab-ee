...
RSpec.describe Emails::MergeRequests, feature_category: :code_review_workflow do
  ...
  let_it_be(:current_user, freeze: false) { create(:user, name: 'Jane Doe') }
  let_it_be(:reviewer, freeze: false) { create(:user) }
  let_it_be(:merge_request, freeze: false) { create(:merge_request, author: current_user, reviewers: [reviewer]) }
