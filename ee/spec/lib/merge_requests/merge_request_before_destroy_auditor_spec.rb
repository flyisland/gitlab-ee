...
RSpec.describe MergeRequests::MergeRequestBeforeDestroyAuditor, feature_category: :code_review_workflow do
  ...
  describe '#execute' do
    ...
    context 'when merge request is merged' do
      before do
        allow(merge_request).to receive_messages(merged?: true, committer_user_ids_from_diff: [4, 5])
      ...
      context 'when destroy_mr_audit_use_committer_ids_from_diff is disabled' do
        before do
          stub_feature_flags(destroy_mr_audit_use_committer_ids_from_diff: false)
          allow(merge_request).to receive_message_chain(:commits, :committer_user_ids).and_return([4, 5])
        end

        it 'fetches committer ids via commits', :aggregate_failures do
          subject.execute

          expect(merge_request).to have_received(:commits)
          expect(merge_request).not_to have_received(:committer_user_ids_from_diff)
        end
      end
