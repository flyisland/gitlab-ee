  ...
  describe '.for' do
    ...
    it 'forwards user: to Resolver, alongside the default timeout' do
      ...
      expect(described_class::Resolver).to receive(:new)
        .with(root_group, user: user, http_timeout: described_class::DEFAULT_HTTP_TIMEOUT_SECONDS)
        .and_call_original
    ...
    it 'passes the default timeout to Resolver' do
      expect(described_class::Resolver).to receive(:new)
        .with(root_group, user: nil, http_timeout: described_class::DEFAULT_HTTP_TIMEOUT_SECONDS)
        .and_call_original
    ...
    it 'passes a caller-supplied timeout to Resolver' do
      expect(described_class::Resolver).to receive(:new)
        .with(root_group, user: nil, http_timeout: 0.25)
        .and_call_original
    ...
    context 'when the kill switch is off' do
      ...
      it 'passes no default timeout to Resolver' do
        expect(described_class::Resolver).to receive(:new)
          .with(root_group, user: nil, http_timeout: nil)
          .and_call_original
