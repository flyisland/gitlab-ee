  ...
  describe 'Jira issues' do
    ...
    context 'when issue tracker is not Jira' do
      it 'does not include Jira issues menu items' do
        ...
        expect(subject.show_jira_menu_items?).to be(false)
        expect(subject.renderable_items.any? { |e| e.item_id == :jira_issue_list }).to be(false)
      end
    end
    ...
    context 'when issue tracker is Jira' do
      ...
      context 'when issues integration is enabled' do
        ...
        it 'includes Jira issues menu items' do
          expect(subject.show_jira_menu_items?).to be(true)
          expect(subject.renderable_items.any? { |e| e.item_id == :jira_issue_list }).to be(true)
          expect(subject.renderable_items.any? { |e| e.item_id == :jira_external_link }).to be(true)
        end
      end
    end
  end
    ...
    context 'when Zentao integration is enabled' do
      ...
      context 'when Issues feature is enabled' do
        ...
        it 'includes Zentao issues menu items' do
          expect(subject.show_zentao_menu_items?).to be(true)
          expect(subject.renderable_items.any? { |e| e.item_id == :zentao_issue_list }).to be(true)
        end
      end
    ...
    context 'when Zentao integration is disabled' do
      ...
      context 'when Issues feature is disabled' do
        ...
        it 'does not include Zentao issues menu items' do
          expect(subject.show_zentao_menu_items?).to be(false)
          expect(subject.renderable_items.any? { |e| e.item_id == :zentao_issue_list }).to be(false)
        end
      end
    end
  end
