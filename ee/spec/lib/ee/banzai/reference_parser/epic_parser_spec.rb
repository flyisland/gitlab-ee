...
RSpec.describe Banzai::ReferenceParser::EpicParser, feature_category: :portfolio_management do
  ...
  subject(:parser) { described_class.new(Banzai::RenderContext.new(nil, user)) }
  ...
  describe '#nodes_visible_to_user' do
    ...
    context 'when the epics feature is enabled' do
      ...
      it 'returns the nodes the user can read for valid epic nodes' do
        ...
        expect(parser.nodes_visible_to_user(user, nodes)).to match_array(expected_result)
      end
  ...
  describe '#referenced_by' do
    context 'when using an existing epics IDs' do
      it 'returns an Array of epics' do
        ...
        expect(parser.referenced_by(nodes)).to match_array(expected_result)
      end

      it 'returns an empty Array for empty list of nodes' do
        expect(parser.referenced_by([])).to be_empty
      end
    end

    context 'when epic with given ID does not exist' do
      it 'returns an empty Array' do
        expect(parser.referenced_by([link(non_existing_record_id)])).to be_empty
      end
    end
  end
  ...
  describe '#records_for_nodes' do
    it 'returns a Hash containing the epics for a list of nodes' do
      ...
      expect(parser.records_for_nodes(nodes)).to eq(expected_hash)
    end

    context 'when extended_preload is true' do
      subject(:parser) { described_class.new(Banzai::RenderContext.new(nil, user, options: { extended_preload: true })) }

      it 'preloads the corresponding work item assignees and milestones when extended_preload is true' do
        records = parser.records_for_nodes(nodes)
