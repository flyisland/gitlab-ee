...
RSpec.describe API::Entities::Member, feature_category: :groups_and_projects do
  ...
  let_it_be(:member, freeze: false) { create(:group_member, :owner, user: user, group: group) }
  ...
  context 'when member record is invited member' do
    let_it_be(:member, freeze: false) { create(:group_member, :invited, :owner, group: group) }
  ...
  context 'for group_saml_identity' do
    ...
    context 'when current user is allowed to read group saml identity' do
      ...
      context 'when member source is subgroup' do
        ...
        let_it_be(:member, freeze: false) { create(:group_member, :owner, user: user, group: subgroup) }
      ...
      context 'when member source is project' do
        ...
        let_it_be(:member, freeze: false) { create(:project_member, :owner, user: user, project: project) }
