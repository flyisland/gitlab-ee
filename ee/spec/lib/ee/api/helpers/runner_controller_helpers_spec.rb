...
RSpec.describe EE::API::Helpers::RunnerControllerHelpers, feature_category: :continuous_integration do
  ...
  let_it_be(:runner_controller_token, freeze: false) do
    create(:ci_runner_controller_token, runner_controller: runner_controller)
  end
  ...
  describe '#check_runner_controller_token!' do
    context 'when runner_controller_token is present' do
      ...
      it 'calls TrackUsageService' do
        expect_next_instance_of(Ci::RunnerControllers::TrackTokenUsageService) do |service|
      ...
      context 'when TrackTokenUsageService fails' do
        before do
          allow_next_instance_of(Ci::RunnerControllers::TrackTokenUsageService) do |service|
            allow(service).to receive(:execute).and_return(ServiceResponse.error(message: 'failure'))
          end
        end

        it 'still allows the request' do
          get '/test'
    ...
    context 'when runner_controller_token is not present' do
      ...
      it 'does not call TrackUsageService' do
        expect(Ci::RunnerControllers::TrackTokenUsageService).not_to receive(:new)
    ...
    context 'when runner_controller_token is revoked' do
      before do
        runner_controller_token.revoke!
        header Gitlab::Kas::INTERNAL_API_AGENT_REQUEST_HEADER, runner_controller_token.token
      end
    ...
    context 'when runner controller has been deleted' do
      let(:token_value) { runner_controller_token.token }

      before do
        token_value
        runner_controller.destroy!
        header Gitlab::Kas::INTERNAL_API_AGENT_REQUEST_HEADER, token_value
      end
  ...
  describe 'integration with AuthFinders' do
    ...
    describe '#runner_controller_token_from_authorization_token' do
      ...
      subject(:runner_controller_token_from_authorization_token) do
      ...
      context 'when route allows runner_controller_token' do
        ...
        context 'when valid token is provided in header' do
          ...
          context 'when the token has been revoked' do
            let(:revoked_token) { create(:ci_runner_controller_token, runner_controller: runner_controller) }
            let(:headers) do
              { Gitlab::Kas::INTERNAL_API_AGENT_REQUEST_HEADER => revoked_token.token }
            end
          ...
          context 'when the runner controller has been deleted' do
            let(:deletable_controller) { create(:ci_runner_controller) }
            let!(:deletable_token) { create(:ci_runner_controller_token, runner_controller: deletable_controller) }
            let(:headers) do
              { Gitlab::Kas::INTERNAL_API_AGENT_REQUEST_HEADER => deletable_token.token }
            end

            before do
              deletable_controller.destroy!
            end
