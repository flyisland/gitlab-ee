...
RSpec.describe Gitlab::Ci::Pipeline::Chain::Command, feature_category: :continuous_integration do
  ...
  describe '#scan_profile_eligibility_service' do
    let(:command) do
      described_class.new(
        project: project,
        source: :push,
    ...
    subject(:service) { command.scan_profile_eligibility_service }

    it 'returns a PipelineEligibilityService instance' do
      expect(service).to be_a(Security::ScanProfiles::PipelineEligibilityService)
    end

    it 'memoizes the service' do
      expect(command.scan_profile_eligibility_service).to be(service)
    end

    it 'initializes the service with correct parameters' do
      expect(Security::ScanProfiles::PipelineEligibilityService).to receive(:new).with(
        project: project,
