...
RSpec.describe Gitlab::Ci::Config, feature_category: :pipeline_composition do
  ...
  describe '#scan_profile_eligibility_service' do
    let(:scan_profile_eligibility_service) do
      instance_double(Security::ScanProfiles::PipelineEligibilityService, eligible?: false)
    end
    ...
    it 'passes scan_profile_eligibility_service to the context' do
      expect(config.context.scan_profile_eligibility_service).to eq(scan_profile_eligibility_service)
    end
  end
end
