...
RSpec.describe Gitlab::Email::Handler::ServiceDeskHandler, feature_category: :service_desk do
  ...
  context 'service desk is enabled for the project' do
    let_it_be_with_reload(:project) { create(:project, :small_repo, :private, group: group, path: 'test', service_desk_enabled: true) }
    ...
    context 'when everything is fine' do
      context 'when using issue templates' do
        context 'that are inherited' do
          ...
          let_it_be_with_reload(:project_with_instance_template) { create(:project, :custom_repo, files: { '.gitlab/issue_templates/inherited_instance_template.md' => 'instance template' }) }
          ...
          context 'from groups' do
            let_it_be_with_reload(:project_with_group_template) { create(:project, :custom_repo, files: { '.gitlab/issue_templates/inherited_group_template.md' => 'group template' }) }
