...
RSpec.describe Gitlab::Email::Handler::ServiceDeskHandler, feature_category: :service_desk do
  ...
  context 'service desk is enabled for the project' do
    let_it_be(:project, freeze: false) { create(:project, :repository, :private, group: group, path: 'test', service_desk_enabled: true) }
    ...
    context 'when everything is fine' do
      context 'when using issue templates' do
        context 'that are inherited' do
          ...
          let_it_be(:project_with_instance_template, freeze: false) { create(:project, :custom_repo, files: { '.gitlab/issue_templates/inherited_instance_template.md' => 'instance template' }) }
          ...
          context 'from groups' do
            let_it_be(:project_with_group_template, freeze: false) { create(:project, :custom_repo, files: { '.gitlab/issue_templates/inherited_group_template.md' => 'group template' }) }
        ...
        context 'that has quick actions' do
          context 'assigning ticket to epic' do
            ...
            let_it_be(:settings, freeze: false) { create(:service_desk_setting, project: project) }
