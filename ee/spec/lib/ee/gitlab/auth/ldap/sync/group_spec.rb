  ...
  describe '#update_permissions' do
    ...
    context 'with all functionality against one LDAP group type' do
      ...
      context 'when custom roles is enabled' do
        let(:member_role) { create(:member_role, :instance, :developer) }
        let(:ldap_group1) { ldap_group_entry(user_dn(user.username)) }
    ...
    context 'with block seat overages' do
      ...
      context 'with block seat overage' do
        ...
        context 'with available seats' do
          ...
          it 'does not track minimal access provisioning' do
            expect(sync_group).not_to receive(:track_minimal_access_provisioning).with(group, user.id)
        ...
        context 'without available seats' do
          ...
          it 'tracks minimal access provisioning' do
            expect(sync_group).to receive(:track_minimal_access_provisioning).with(group, user.id)
          ...
          it 'audits minimal access role adjustment' do
            ...
            audit_event = AuditEvent.find_by(target_id: user.id)
