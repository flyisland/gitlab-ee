...
RSpec.describe EE::Gitlab::ApplicationRateLimiter::LabkitAdapter::SupportedRateLimits,
  ...
  describe '.entries' do
    ...
    it "overrides only the limit/period of CE keys that EE's rate_limits overrides", :aggregate_failures do
      code_suggestions = all.fetch(:code_suggestions_api_endpoint)
      geo_proxy = all.fetch(:geo_proxy)

      expect(code_suggestions[:limit]).to eq(60)
      expect(code_suggestions[:period]).to eq(1.minute)
      expect(code_suggestions[:limiter_name]).to eq('applimiter_code_suggestions_api_endpoint')

      expect(geo_proxy[:limit]).to eq(3600)
      expect(geo_proxy[:period]).to eq(1.minute)
      expect(geo_proxy[:limiter_name]).to eq('applimiter_geo_proxy')
    end
