...
RSpec.describe Gitlab::RackAttack::Request, feature_category: :rate_limiting do
  ...
  describe '#geo_proxy_workhorse_request?' do
    ...
    context 'with a valid Workhorse JWT' do
      let(:env) do
        { 'HTTP_GITLAB_WORKHORSE_API_REQUEST' =>
          JWT.encode({ 'iss' => 'gitlab-workhorse' }, ::Gitlab::Workhorse.secret, 'HS256') }
      end

      it { is_expected.to be(true) }
    end

    context 'with no JWT header' do
    ...
    context 'with a JWT signed with the wrong key' do
      let(:env) do
        { 'HTTP_GITLAB_WORKHORSE_API_REQUEST' =>
    ...
    context 'with a JWT with an incorrect issuer' do
      let(:env) do
        { 'HTTP_GITLAB_WORKHORSE_API_REQUEST' =>
