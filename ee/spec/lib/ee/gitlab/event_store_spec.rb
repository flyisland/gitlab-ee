...
RSpec.describe Gitlab::EventStore, feature_category: :shared do
  ...
  describe 'DependencyManagement::SecurityUpdate::SchedulerWorker subscription',
    ...
    let(:subscriptions) { described_class.instance.subscriptions }
    let(:subscription) do
      subscriptions[Sbom::VulnerabilitiesCreatedEvent].find do |s|
        s.worker == DependencyManagement::SecurityUpdate::SchedulerWorker
      end
    end

    it 'subscribes to Sbom::VulnerabilitiesCreatedEvent' do
      expect(subscription).to be_present
    end

    context 'when findings contain gem packages' do
      it 'returns true' do
        event = Sbom::VulnerabilitiesCreatedEvent.new(data: {
          findings: [{ 'uuid' => SecureRandom.uuid, 'project_id' => 1, 'vulnerability_id' => 1,
                       'package_name' => 'rails', 'package_version' => '6.0.0', 'purl_type' => 'gem' }]
        })
    ...
    context 'when findings contain no gem packages' do
      it 'returns false' do
        event = Sbom::VulnerabilitiesCreatedEvent.new(data: {
          findings: [{ 'uuid' => SecureRandom.uuid, 'project_id' => 1, 'vulnerability_id' => 1,
                       'package_name' => 'lodash', 'package_version' => '4.0.0', 'purl_type' => 'npm' }]
        })
