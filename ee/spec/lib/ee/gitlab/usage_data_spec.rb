...
RSpec.describe Gitlab::UsageData, feature_category: :service_ping do
  ...
  describe '.data' do
    ...
    let_it_be_with_reload(:projects) do
      Array.new(3) do
        creator = create(:user, group_view: :security_dashboard)
        create(:project, group: group, creator: creator)
      end
    end
  ...
  describe '.components_usage_data' do
    ...
    it 'gathers components usage data' do
      ...
      expect(subject[:advanced_search]).to eq(Search::Elastic::Helper.default.server_info.except(:build_flavor))
    end
  end
