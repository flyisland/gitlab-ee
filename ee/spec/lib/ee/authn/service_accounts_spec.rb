...
RSpec.describe ::Authn::ServiceAccounts, feature_category: :system_access do
  ...
  describe '.creation_allowed_for_saas?', :saas do
    ...
    context 'when namespace is on trial' do
      ...
      context 'when allow_unlimited_service_account_for_trials is enabled' do
        it 'returns true' do
          expect(described_class.creation_allowed_for_saas?(group)).to be(true)
        end
      end

      context 'when allow_unlimited_service_account_for_trials is disabled' do
        before do
          stub_feature_flags(allow_unlimited_service_account_for_trials: false)
        end

        it 'returns true when under the limit' do
          expect(described_class.creation_allowed_for_saas?(group, 0)).to be(true)
        end

        it 'returns false when at the limit' do
          stub_const('Authn::ServiceAccounts::LIMIT_FOR_TRIAL', 2)

          expect(described_class.creation_allowed_for_saas?(group, 2)).to be(false)
        end
      end
    end
