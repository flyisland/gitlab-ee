...
RSpec.describe Sbom::PackageUrl::Normalizer, feature_category: :dependency_management do
  ...
  describe '.normalize_name' do
    it 'normalizes as the instance method does' do
      expect(described_class.normalize_name(type: 'pypi', text: 'Evil_Pkg.Name')).to eq('evil-pkg-name')
    end

    it 'accepts a symbol type' do
      expect(described_class.normalize_name(type: :golang, text: 'GitHub.com/Evil')).to eq('github.com/evil')
    end

    context 'with invalid names' do
      [nil, 123, { 'oops' => true }, []].each do |value|
        it "returns #{value.inspect} untouched rather than raising" do
          expect(described_class.normalize_name(type: 'golang', text: value)).to eq(value)
        end
      end
    end
  end
