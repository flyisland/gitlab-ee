...
RSpec.describe OmniAuth::Strategies::GroupSaml, feature_category: :system_access, type: :strategy do
  ...
  let_it_be(:idp_sso_url) { 'https://saml.example.com/adfs/ls' }
  let_it_be(:fingerprint) { 'C1:59:74:2B:E8:0C:6C:A9:41:0F:6E:83:F6:D1:52:25:45:58:89:FB' }
  let_it_be_with_reload(:saml_provider) do
    create(:saml_provider, group: group, sso_url: idp_sso_url, certificate_fingerprint: fingerprint)
  end

  let_it_be(:unconfigured_group) { create(:group, name: 'unconfigured-group') }
  ...
  describe 'callback_url option' do
    ...
    it 'calls callback_url in the original strategy' do
      ...
      allow(strategy).to receive_messages(
        callback_path: '/v1/users/auth/group_saml/callback',
        query_string: "?group_path=#{group.name}"
      )
      ...
      before do
        allow_next_instance_of(OneLogin::RubySaml::Response) do |instance|
          allow(instance).to receive_messages(
            ...
            validate_session_expiration: true,
            validate_subject_confirmation: true,
