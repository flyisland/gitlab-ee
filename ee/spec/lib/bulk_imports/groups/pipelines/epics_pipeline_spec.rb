  ...
  describe '#run', :clean_gitlab_redis_shared_state, :aggregate_failures do
    ...
    context 'when importer_user_mapping is enabled' do
      ...
      context 'when importing epics has a parent epic' do
        ...
        it 'pushes placeholder references for child epic' do
          pipeline.run

          last_epic = group.reload.epics.order(:id).last

          user_references = placeholder_user_references(::Import::SOURCE_DIRECT_TRANSFER, bulk_import.id)
            .select { |ref| ref.first == 'Epic' }

          expect(user_references).to match_array([
            ['Epic', last_epic.id, 'author_id', other_source_user.id],
            ['Epic', last_epic.id, 'last_edited_by_id', other_source_user.id],
            ['Epic', last_epic.id, 'updated_by_id', other_source_user.id],
            ['Epic', last_epic.id, 'closed_by_id', other_source_user.id]
          ])
        end
      end
