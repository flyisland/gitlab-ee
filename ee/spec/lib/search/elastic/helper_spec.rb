...
RSpec.describe Search::Elastic::Helper, :request_store, feature_category: :global_search do
  ...
  describe '#create_migrations_index' do
    ...
    context 'when creating a fresh migrations index' do
      it 'creates the index with alias pattern' do
    ...
    context 'when old migrations index exists' do
      before do
        # Create old-style direct index
        helper.client.indices.create( # rubocop:disable Rails/SaveBang -- Elasticsearch client does not use ActiveRecord bang methods
        ...
        # Add test migration record
        helper.client.index(
          index: old_index_name,
          id: '1',
          body: { name: 'test_migration', completed: true },
          refresh: true
        )
      end

      it 'returns early and lets migration handle the conversion' do
        ...
        # Should return without creating alias (migration will handle it)
        result = helper.create_migrations_index
        expect(result).to eq(helper.migrations_alias_name)

        # New alias should NOT exist yet (migration will create it)
        expect(helper.alias_exists?(name: helper.migrations_index_name)).to be false
