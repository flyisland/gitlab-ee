...
RSpec.describe Search::Elastic::Indexer, feature_category: :global_search do
  ...
  context 'for empty project', :elastic do
    ...
    before do
      set_elasticsearch_migration_to(:add_traversal_ids_to_commits, including: true)
    end
  ...
  context 'with an indexed project', :elastic do
    ...
    context 'when indexing a HEAD commit', :elastic do
      ...
      context 'when add_traversal_ids_to_commits migration has finished' do
        before do
          set_elasticsearch_migration_to(:add_traversal_ids_to_commits, including: true)
        end

        it 'runs the indexing command with --commit-traversal-ids-enabled=true' do
      ...
      context 'when add_traversal_ids_to_commits migration has not finished' do
        before do
          set_elasticsearch_migration_to(:add_traversal_ids_to_commits, including: false)
        end

        it 'runs the indexing command with --commit-traversal-ids-enabled=false' do
          expect_popen.with(
            [
              TestEnv.indexer_bin_path,
              "--timeout=#{described_class.timeout.to_i}s",
              ...
              "--schema-version-commit=2306",
              "--archived=#{project.self_or_ancestors_archived?}",
              "--commit-traversal-ids-enabled=false",
              "--traversal-ids=#{project.namespace_ancestry}",
              "#{project.repository.disk_path}.git"
            ],
            nil,
  ...
  context 'when no aws credentials available' do
    ...
    before do
      allow(Search::Elastic::Client).to receive(:aws_credential_provider).and_return(nil)
    end
