  ...
  describe '#invoke' do
    ...
    context 'when orbit_use_legacy_tools is disabled' do
      ...
      %w[query_graph get_graph_schema get_graph_status].each do |legacy_tool|
        context "when #{legacy_tool} (a legacy tool) is called" do
          ...
          it 'does not dispatch the legacy tool' do
            expect(handler).not_to receive(:dispatch_tool)
      ...
      context 'when a command tool is called' do
        let(:tool_name) { 'list_commands' }
        let(:arguments) { { 'command_names' => ['get_query_dsl'], 'format' => 'llm' } }
        ...
        it 'still allows the command tool through' do
          result = handler.invoke
    ...
    context 'when orbit_use_legacy_tools is enabled (default)' do
      context 'when a legacy tool is called' do
        let(:tool_name) { 'get_graph_schema' }
        let(:arguments) { { 'expand_nodes' => ['Pipeline'] } }
        ...
        it 'dispatches the legacy tool' do
          result = handler.invoke
