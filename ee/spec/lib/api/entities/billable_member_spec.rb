...
RSpec.describe ::API::Entities::BillableMember, feature_category: :seat_cost_management do
  ...
  let_it_be(:last_activity_on, freeze: false) { Date.today - 1.day }
  # rubocop:enable Rails/Date
  let_it_be(:current_sign_in_at, freeze: false) { DateTime.now - 2.days }
  let_it_be(:group, freeze: false) { create(:group) }
  let_it_be(:current_user, freeze: false) { create(:user) }
  let_it_be(:current_user_member, freeze: false) { create(:group_member, :owner, user: current_user, group: group) }
  let_it_be(:user, freeze: false) do
    create(:user, last_activity_on: last_activity_on, current_sign_in_at: current_sign_in_at)
  end

  let_it_be(:user_member, freeze: false) { create(:group_member, :owner, user: user, group: group) }
  ...
  describe 'email field' do
    ...
    context 'when the user has a public_email assigned' do
      let_it_be(:user_public_email, freeze: false) do
        create(:email, :confirmed, user: user, email: 'user-public-email@example.com')
      end
    ...
    context 'on SaaS', :saas do
      ...
      let_it_be(:another_group, freeze: false) { create(:group_member, :owner, user: current_user).group }
