...
RSpec.describe Sidebars::Projects::SuperSidebarMenus::DuoAgentsMenu, feature_category: :duo_agent_platform do
  ...
  context 'when ai_catalog_public_explore is disabled' do
    ...
    context 'when user is entitled to the agent platform and duo_features_enabled' do
      before do
    ...
    context 'when user is not entitled to the agent platform' do
      before do
        project.project_setting.duo_features_enabled = true
        allow(user).to receive(:can?).with(:read_duo_agent_platform, project).and_return(false)
      end
