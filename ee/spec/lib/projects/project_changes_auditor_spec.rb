    ...
    describe 'audit changes' do
      ...
      context 'when project packages enabled setting changes is updated' do
        it "logs project_packages_enabled_updated event" do
          ...
          expect { auditor_instance.execute }.to change { AuditEventReader.count }.by(2)
          expect(AuditEventReader.last(2).map { |e| e.details[:change] })
          ...
          context "with #{column}" do
            ...
            with_them do
              ...
              it 'creates an audit event' do
                expect { auditor_instance.execute }.to change { AuditEventReader.count }.by(1)
      ...
      it 'does not create an event when suggestion_commit_message change from nil to empty string' do
        ...
        expect { auditor_instance.execute }.not_to change { AuditEventReader.count }
      end
