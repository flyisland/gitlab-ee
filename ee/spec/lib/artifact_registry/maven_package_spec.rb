...
RSpec.describe ArtifactRegistry::MavenPackage, feature_category: :artifact_registry do
  let(:attributes) do
    {
  ...
  describe 'rendered readers' do
    it 'exposes the identifier, both Maven coordinates, and the parsed timestamp', :aggregate_failures do
      expect(package.id).to eq('a1b2c3d4-0000-0000-0000-000000000000')
      expect(package.group_id).to eq('com.example.tools')
      expect(package.artifact_id).to eq('payment-core')
      expect(package.last_downloaded_at).to eq(DateTime.iso8601('2026-07-03T09:15:00Z'))
    end
  end
  ...
  describe 'absent fields' do
    ...
    it 'returns nil for absent coordinates without raising', :aggregate_failures do
      expect(package.id).to eq('a1b2c3d4-0000-0000-0000-000000000000')
      expect(package.group_id).to be_nil
      expect(package.artifact_id).to be_nil
      expect(package.last_downloaded_at).to be_nil
    end
  end

  describe 'timestamp coercion' do
    context 'when last_downloaded_at is null, which the contract sends for a package never pulled' do
