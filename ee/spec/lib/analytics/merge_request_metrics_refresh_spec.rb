...
RSpec.describe Analytics::MergeRequestMetricsRefresh, feature_category: :devops_reports do
  subject(:metrics_refresher) { calculator_class.new(merge_request) }
  ...
  describe '#execute' do
    it 'updates metric via update_metric! method' do
      expect { metrics_refresher.execute }.to change { merge_request.metrics.first_comment_at }.to(be_like_time(Time.zone.now))
    end

    it 'when MR was deleted right before metrics refresh does not raise an error' do
      merge_request.destroy!

      expect { metrics_refresher.execute }.not_to raise_error
    end
    ...
    context 'when metric is already present' do
      ...
      it 'does not update metric' do
        metrics_refresher.execute

        expect(merge_request.metrics.reload.first_comment_at).to be_like_time(first_comment_at)
      end

      it 'updates metric when forced' do
        expect { metrics_refresher.execute(force: true) }.to change { merge_request.metrics.first_comment_at }.to(be_like_time(Time.zone.now))
      end
    end
  end
  ...
  describe '#execute_async' do
    it 'schedules CodeReviewMetricsWorker with params' do
      ...
      metrics_refresher.execute_async(force: true)
    end
  end
end
