...
RSpec.describe Analytics::MergeRequestMetricsCalculator, feature_category: :code_review_workflow do
  subject(:calculator) { described_class.new(merge_request) }

  let_it_be_with_reload(:merge_request) { create(:merge_request, :merged, created_at: 31.days.ago) }
  ...
  describe '#productivity_data' do
    it 'calculates productivity data' do
      ...
      expect(calculator.productivity_data).to match(expected_data)
    end
  end

  describe '#first_comment_at' do
    it 'returns first non-author comment' do
      expect(calculator.first_comment_at).to be_like_time(merge_request_note.created_at)
    end
  end
  ...
  describe '#first_approved_at' do
    it 'returns first approval creation timestamp' do
      ...
      expect(calculator.first_approved_at).to be_like_time(1.day.ago)
    end
  end
  ...
  describe '#first_reassigned_at' do
    it 'returns earliest non-author assignee creation timestamp' do
      ...
      expect(calculator.first_reassigned_at).to be_like_time(3.days.ago)
    end
  end
end
