...
RSpec.describe ::ComplianceManagement::MergeRequestApprovalSettings::Resolver, feature_category: :compliance_management do
  ...
  describe '#allow_overrides_to_approver_list_per_merge_request' do
    ...
    context 'when a security policy enforces prevent_editing_approval_rules' do
      ...
      before do
        allow(project).to receive(:policy_prevents_editing_approval_rules?).and_return(true)
      end

      context 'when the hierarchy would otherwise allow overrides' do
        before do
          stub_ee_application_setting(disable_overriding_approvers_per_merge_request: false)
          group.group_merge_request_approval_setting.update!(
            allow_overrides_to_approver_list_per_merge_request: true
          )
          project.update!(disable_overriding_approvers_per_merge_request: false)
        end

        it 'is locked and disallowed, flagged as enforced_by_policy' do
          expect(setting.value).to be false
          expect(setting.locked).to be true
          expect(setting.inherited_from).to be_nil
          expect(setting.enforced_by_policy).to be true
        end
      end

      context 'when the hierarchy already locks the setting at the instance level' do
        before do
          stub_ee_application_setting(disable_overriding_approvers_per_merge_request: true)
          group.group_merge_request_approval_setting.update!(
            allow_overrides_to_approver_list_per_merge_request: true
          )
          project.update!(disable_overriding_approvers_per_merge_request: false)
        ...
        it 'is locked and disallowed, with policy overriding the hierarchy source' do
          expect(setting.value).to be false
          expect(setting.locked).to be true
          expect(setting.inherited_from).to be_nil
          expect(setting.enforced_by_policy).to be true
