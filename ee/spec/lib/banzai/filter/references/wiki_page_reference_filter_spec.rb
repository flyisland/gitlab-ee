...
RSpec.describe Banzai::Filter::References::WikiPageReferenceFilter, feature_category: :wiki do
  ...
  let_it_be(:wiki, freeze: false) { GroupWiki.new(group, user) }
  let_it_be(:wiki_page, freeze: false) { create(:wiki_page, wiki: wiki, title: 'nested/twice/start-page') }
  let_it_be(:cross_group, freeze: false) { create(:group) }
  let_it_be(:cross_wiki, freeze: false) { GroupWiki.new(cross_group, user) }
  let_it_be(:cross_wiki_page, freeze: false) { create(:wiki_page, wiki: cross_wiki, title: 'nested/twice/start-page') }
  ...
  context 'when group level wiki page URL reference' do
    let_it_be(:wiki_page_link_reference, freeze: false)  { urls.group_wiki_url(group, wiki_page) }
  ...
  context 'on cross group [wiki_page:group/path:slug] reference' do
    let_it_be(:wiki_page_link_reference, freeze: false)  { urls.group_wiki_url(cross_group, wiki_page) }
    let_it_be(:wiki_page_url, freeze: false)             { wiki_page_link_reference }
    let_it_be(:written_reference, freeze: false)         do
      "[wiki_page:#{cross_group.full_path}:#{cross_wiki_page.slug}]"
    end
