  ...
  describe '.preprocess_references' do
    ...
    it 'calls `fetch_content` with the correct references' do
      expect(described_class).to receive(:apply_embeddings) do |args|
        expect(args[:refs]).to eq([reference_1, reference_2])

        { successful: args[:refs], failed: [], retryable: [] }
      end
    ...
    context 'with next_model_only option' do
      before do
      ...
      it 'uses only the next indexing embedding model when next_model_only is true' do
        expect(described_class).to receive(:apply_embeddings) do |args|
          expect(args[:next_model_only]).to be true
        end.and_call_original

        described_class.preprocess_references(refs, next_model_only: true)
      end

      it 'uses all indexing embedding models when next_model_only is false' do
        expect(described_class).to receive(:apply_embeddings) do |args|
          expect(args[:next_model_only]).to be false
        end.and_call_original

        described_class.preprocess_references(refs, next_model_only: false)
      end

      it 'defaults to false when next_model_only is not provided' do
        expect(described_class).to receive(:apply_embeddings) do |args|
          expect(args[:next_model_only]).to be false
        end.and_call_original
