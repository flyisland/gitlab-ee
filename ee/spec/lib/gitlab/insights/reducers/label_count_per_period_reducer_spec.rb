...
RSpec.describe Gitlab::Insights::Reducers::LabelCountPerPeriodReducer, feature_category: :value_stream_management do
  ...
  subject(:reduced_result) { reduce(issuable_relation, query[:group_by], query[:collection_labels]) }

  it 'returns issuables with only the needed fields' do
    expect(reduced_result).to eq(expected)
  end

  it 'avoids N + 1 queries' do
    control_queries = ActiveRecord::QueryRecorder.new { reduced_result }
