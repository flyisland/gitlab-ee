...
RSpec.describe Gitlab::Analytics::TypeOfWork::TasksByType, feature_category: :value_stream_management do
  ...
  let_it_be(:label_for_subgroup, freeze: false) { create(:group_label, group: group) }
  let_it_be(:other_label, freeze: false) { create(:group_label, group: other_group) }
  let_it_be(:project, freeze: false) { create(:project, group: group) }
  let_it_be(:assignee, freeze: false) { create(:user, developer_of: group) }
  ...
  shared_examples '#counts_by_labels' do
    ...
    it 'counts the records by label and date' do
      expect(label_count_for(label, label_counts)).to eq(3)
    end

    it 'counts should include subgroups' do
      expect(label_count_for(label_for_subgroup, label_counts)).to eq(1)
    end

    it 'does not include count from outside of the group' do
      label_names = label_counts.map { |r| r.label.name }
    ...
    context 'when group without any record is given' do
      ...
      it { expect(label_counts).to be_empty }
    end
    ...
    context 'when filtering by `project_ids`' do
      ...
      it { expect(label_count_for(label, label_counts)).to eq(1) }
    end
  ...
  context 'when unknown `subject` is given' do
    ...
    it 'falls back to `MergeRequestFinder`' do
      expect(label_counts.map(&:label)).to eq([label])
    end
  end
end
