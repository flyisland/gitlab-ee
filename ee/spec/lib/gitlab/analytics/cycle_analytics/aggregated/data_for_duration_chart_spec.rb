...
RSpec.describe Gitlab::Analytics::CycleAnalytics::Aggregated::DataForDurationChart do
  let_it_be(:stage, freeze: false) { create(:cycle_analytics_stage, start_event_identifier: :issue_created, end_event_identifier: :issue_closed) }
  let_it_be(:project, freeze: false) { create(:project, group: stage.namespace) }
  let_it_be(:issue_1, freeze: false) { create(:issue, project: project) }
  let_it_be(:issue_2, freeze: false) { create(:issue, project: project) }
  ...
  describe 'calculating the daily average stage duration', :aggregate_failures do
    let_it_be(:end_timestamp_1, freeze: false) { Time.zone.local(2020, 5, 6, 12, 0) }
    let_it_be(:end_timestamp_2, freeze: false) { Time.zone.local(2020, 5, 15, 10, 0) }
