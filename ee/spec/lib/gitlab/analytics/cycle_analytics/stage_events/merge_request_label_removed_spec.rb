...
RSpec.describe Gitlab::Analytics::CycleAnalytics::StageEvents::MergeRequestLabelRemoved do
  ...
  it_behaves_like 'LEFT JOIN-able value stream analytics event' do
    ...
    let_it_be(:record_with_data, freeze: false) { create(:labeled_merge_request, :unique_branches, source_project: project, labels: [label]) }
    let_it_be(:record_without_data, freeze: false) { create(:merge_request) }
