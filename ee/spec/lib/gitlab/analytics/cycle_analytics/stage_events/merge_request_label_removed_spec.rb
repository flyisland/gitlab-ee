...
RSpec.describe Gitlab::Analytics::CycleAnalytics::StageEvents::MergeRequestLabelRemoved do
  ...
  it_behaves_like 'LEFT JOIN-able value stream analytics event' do
    ...
    let_it_be_with_reload(:record_with_data) { create(:labeled_merge_request, :unique_branches, source_project: project, labels: [label]) }
    let_it_be_with_reload(:record_without_data) { create(:merge_request) }
