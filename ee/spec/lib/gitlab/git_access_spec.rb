...
RSpec.describe Gitlab::GitAccess, feature_category: :system_access do
  ...
  describe "push_rule_check" do
    ...
    describe "author email check" do
      ...
      it 'does not allow any change from Web UI with bad commit' do
        ...
        allow(bad_commit).to receive(:refs).and_return([ref_object])
        allow(project.repository).to receive_messages(commits_between: [bad_commit], new_commits: [bad_commit])
  ...
  describe 'Geo' do
    let_it_be_with_reload(:primary_node) { create(:geo_node, :primary) }
    let_it_be_with_reload(:secondary_node) { create(:geo_node) }
  ...
  describe '#check_otp_session!' do
    ...
    let_it_be_with_reload(:key) { create(:key, user: user) }
    let_it_be_with_reload(:actor) { key }
  ...
  describe '#check_sso_session!', :clean_gitlab_redis_sessions do
    ...
    let_it_be_with_reload(:identity) { create(:group_saml_identity, saml_provider: saml_provider, user: user) }
  ...
  describe '#check_valid_actor!' do
    ...
    context 'for enterprise users', :saas do
      ...
      context "when actor is the enterprise user's SSH key" do
        let_it_be_with_reload(:actor) { create(:key, user: user) }
      ...
      context 'when actor is a DeployKey tied to the enterprise user' do
        let_it_be_with_reload(:actor) { create(:deploy_key, user: user) }
