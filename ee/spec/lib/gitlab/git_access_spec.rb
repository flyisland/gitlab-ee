...
RSpec.describe Gitlab::GitAccess, feature_category: :system_access do
  ...
  describe 'Geo' do
    let_it_be(:primary_node, freeze: false) { create(:geo_node, :primary) }
    let_it_be(:secondary_node, freeze: false) { create(:geo_node) }
  ...
  describe '#check_otp_session!' do
    ...
    let_it_be(:actor, freeze: false) { key }
    ...
    context 'with an OTP session', :clean_gitlab_redis_sessions do
      ...
      context 'based on the duration set by the `git_two_factor_session_expiry` setting' do
        let_it_be(:git_two_factor_session_expiry, freeze: false) { 20 }
        let_it_be(:redis_key_expiry_at, freeze: false) { git_two_factor_session_expiry.minutes.from_now }
  ...
  describe '#check_sso_session!', :clean_gitlab_redis_sessions do
    ...
    let_it_be(:identity, freeze: false) { create(:group_saml_identity, saml_provider: saml_provider, user: user) }
  ...
  describe '#check_valid_actor!' do
    ...
    context 'for enterprise users', :saas do
      ...
      context "when actor is the enterprise user's SSH key" do
        let_it_be(:actor, freeze: false) { create(:key, user: user) }
      ...
      context 'when actor is a DeployKey tied to the enterprise user' do
        let_it_be(:actor, freeze: false) { create(:deploy_key, user: user) }
