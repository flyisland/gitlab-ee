...
RSpec.describe Gitlab::Audit::Auditor, feature_category: :audit_events do
  ...
  describe '#log_events_and_stream' do
    ...
    context 'when new audit events are empty but saved events exist' do
      before do
    ...
    context 'when neither new nor saved events exist' do
      before do
        allow(auditor).to receive_messages(log_to_new_tables: [], log_to_database: [])
      end
    ...
    context 'when stop_legacy_audit_event_writes feature flag is enabled' do
      before do
        ...
        # Keep log_to_database stubbed to return nil (simulating feature flag behavior)
        # Keep log_to_new_tables stubbed to return mock events (avoid partition issues)
        allow(auditor).to receive_messages(log_to_database: nil, log_to_new_tables: new_audit_events)
        allow(auditor).to receive(:log_to_file)
      end

      it 'does not write to the legacy AuditEvent table' do
        expect(auditor).to receive(:log_to_database).and_return(nil)

        auditor.log_events_and_stream(events)
      end

      it 'calls log_to_new_tables with original events when saved_events is nil' do
        expect(auditor).to receive(:log_to_new_tables) do |passed_events, _name|
          # When legacy writes are skipped, original events should be passed
          expect(passed_events).to eq(events)
          new_audit_events
        end
      ...
      it 'streams events from new tables' do
        expect(auditor).to receive(:send_to_stream).with(new_audit_events)

        auditor.log_events_and_stream(events)
      end

      it 'calls log_to_new_tables which would call ComplianceViolationScheduler' do
        # ComplianceViolationScheduler is called inside log_to_new_tables (in EE::Gitlab::Audit::Logging)
        # Since we stub log_to_new_tables, we verify it's called instead
        expect(auditor).to receive(:log_to_new_tables).and_return(new_audit_events)
