...
RSpec.describe ::Gitlab::LicenseScanning::SbomScanner, feature_category: :software_composition_analysis do
  ...
  describe ".latest_pipeline" do
    ...
    context 'when the pipeline does not contain an sbom report' do
      let_it_be_with_reload(:pipeline) { create(:ee_ci_pipeline, :with_metrics_report, project: project) }
  ...
  describe "#report" do
    ...
    context "when the pipeline is not nil" do
      context "when the pipeline contains an sbom report" do
        let_it_be_with_reload(:pipeline) { create(:ee_ci_pipeline, :with_cyclonedx_report, project: project) }
        ...
        context "when the sbom report contains a component with licenses" do
          ...
          # Regression coverage for custom/proprietary license names entered in Merge Request
          # Approval Policies (for example `LicenseRef-*` or an internal license name). A CycloneDX
          # license that carries only `license.name` and no SPDX id must reach the report by name so
          # the policy allow/deny list can match it, and must not be coerced to "unknown".
          it 'preserves the custom, non-SPDX license by name without coercing it to unknown', :aggregate_failures do
            license = report.licenses.find { |license| license.name == report_license }

            expect(license).to be_present
            expect(license.id).to be_nil
            expect(license.name).not_to eq('unknown')
          end
        end

        context "when the sbom report contains a component with a license expression" do
          let(:purl) { ::Sbom::PackageUrl.parse('pkg:maven/com.example/util/library@2.0.0') }
          let(:expression) { 'Apache-2.0 AND (MIT OR GPL-2.0-only)' }
          let(:component_name) { 'com.example/util/library' }

          before do
            sbom_report = pipeline.sbom_reports.reports.last
            ...
            component = create(
              ...
              purl_type: purl.type,
              purl: purl,
              licenses: [build(:ci_reports_sbom_license, :with_expression, expression: expression)]
            )

            sbom_report.add_component(component)

            allow_next_instance_of(Gitlab::Ci::Reports::Sbom::Report) do |instance|
              allow(instance).to receive(:components).and_return(sbom_report.components)
            end
          end

          it 'surfaces the expression as the LicenseScanning license name', :aggregate_failures do
            license = report.licenses.find { |l| l.name == expression }

            expect(license).not_to be_nil
            expect(license.id).to be_nil
            expect(license.dependencies).to include(
              have_attributes(name: component_name, version: purl.version)
            )
          end
        end

        context "when the package metadata licenses resolve to an expression" do
          let_it_be(:expression) { 'MIT OR Apache-2.0' }
          let_it_be(:expression_license) do
            create(:pm_license, :with_expression, spdx_expression: expression)
          end

          before_all do
            create(:pm_package, name: "byebug", purl_type: "gem",
              licenses: [[expression_license.id], "00000001", "99999999", []])
          end

          it 'reports the raw expression as an id-less license and removes the dependency from unknown',
            :aggregate_failures do
            license = report.licenses.find { |l| l.name == expression }

            expect(license).to have_attributes(id: nil, name: expression, url: nil)
            expect(license.dependencies).to include(
              have_attributes(name: 'byebug', version: '10.0.0'))

            unknown_license = report.licenses.find { |l| l.name == 'unknown' }
            expect(unknown_license.dependencies).not_to include(
              have_attributes(name: 'byebug', version: '10.0.0'))
          end
        end

        context "when the package metadata license set holds both an identifier and an expression" do
          let_it_be(:identifier_license) { create(:pm_license, spdx_identifier: 'MIT') }
          ...
          before_all do
            create(:pm_package, name: "rspec-core", purl_type: "gem",
              licenses: [[identifier_license.id, expression_license.id], "00000001", "99999999", []])
          end

          it 'reports the expression-named license and no bare identifier license', :aggregate_failures do
            license = report.licenses.find { |l| l.name == 'MIT OR Apache-2.0' }

            expect(license).not_to be_nil
            expect(license.dependencies).to include(
              have_attributes(name: "rspec-core", version: "3.7.1"))

            expect(report.licenses.map(&:name)).not_to include('MIT', 'MIT License')
          end
        end
      end
  ...
  describe "#report memoization" do
    ...
    it 'only fetches pipeline components once across multiple calls' do
      expect(Gitlab::LicenseScanning::PipelineComponents).to receive(:new).once.and_call_original

      2.times { scanner.report }
    end
  end
    ...
    context "when the pipeline is not nil" do
      context "and the pipeline has an sbom report" do
        ...
        context "and the pipeline is running" do
          let_it_be_with_reload(:pipeline) { create(:ee_ci_pipeline, :running, project: project) }
          let_it_be(:build) do
