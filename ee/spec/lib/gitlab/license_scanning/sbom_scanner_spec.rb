...
RSpec.describe ::Gitlab::LicenseScanning::SbomScanner, feature_category: :software_composition_analysis do
  ...
  describe ".latest_pipeline" do
    ...
    context 'when the pipeline does not contain an sbom report' do
      let_it_be(:pipeline, freeze: false) { create(:ee_ci_pipeline, :with_metrics_report, project: project) }
  ...
  describe "#report" do
    ...
    context "when the pipeline is not nil" do
      context "when the pipeline contains an sbom report" do
        let_it_be(:pipeline, freeze: false) { create(:ee_ci_pipeline, :with_cyclonedx_report, project: project) }
        ...
        context "when the sbom report contains a component with a license expression" do
          ...
          context 'when cyclonedx_license_expression_ingestion is disabled' do
            before do
              stub_feature_flags(cyclonedx_license_expression_ingestion: false)
            end

            it 'falls back to an unknown license instead of building a nil-named one', :aggregate_failures do
              expect { report.licenses }.not_to raise_error
              expect(report.licenses.map(&:name)).not_to include(expression)
              expect(report.licenses.map(&:name)).not_to include(nil)
              expect(report.licenses.map(&:name)).to include('unknown')
            end
          end
        end
      end
      ...
      context "when the pipeline does not contain an sbom report" do
        let_it_be(:pipeline, freeze: false) { create(:ee_ci_pipeline, :with_license_scanning_report, project: project) }
    ...
    context "when the pipeline is not nil" do
      context "and the pipeline has an sbom report" do
        ...
        context "and the pipeline is running" do
          let_it_be(:pipeline, freeze: false) { create(:ee_ci_pipeline, :running, project: project) }
          let_it_be(:build) do
