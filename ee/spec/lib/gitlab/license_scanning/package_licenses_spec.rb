    ...
    context 'when component contains license information' do
      ...
      context 'when component\'s license is an SPDX expression' do
        ...
        context 'when cyclonedx_license_expression_ingestion is disabled' do
          ...
          it 'ignores the expression and records the component as unknown without querying PMDB' do
            # The component declared a license, so it stays on the SBOM path: PMDB is never asked
            # to resolve it (the lookup is invoked with an empty component list).
            expect(::PackageMetadata::Package).to receive(:packages_for).with(components: []).and_call_original
          ...
          context 'when the component also declares a non-expression license' do
            let(:component_licenses) do
              [
                { "expression" => 'Apache-2.0 AND (MIT OR GPL-2.0-only)' },
                { "name" => 'MIT License', "spdx_identifier" => 'MIT' }
              ]
            end

            it 'keeps the license information and drops the expression' do
              expect(fetch).to contain_exactly(
                have_attributes(name: 'beego', purl_type: 'golang', version: 'v1.10.0', licenses: [{
                  "name" => 'MIT License',
                  "spdx_identifier" => 'MIT',
        ...
        context 'when cyclonedx_license_expression_ingestion is scoped to the project' do
          it 'evaluates the FF with the project as actor' do
            allow(Feature).to receive(:enabled?).and_call_original
            expect(Feature).to receive(:enabled?)
                                 .with(:cyclonedx_license_expression_ingestion, project)
                                 .at_least(:once)
                                 .and_call_original
        ...
        context 'when the project is configured to source licenses from PMDB' do
          ...
          shared_examples 'resolves the component via package metadata' do
          ...
          context 'when cyclonedx_license_expression_ingestion is enabled' do
            it_behaves_like 'resolves the component via package metadata'
          end
