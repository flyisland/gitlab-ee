...
RSpec.describe Gitlab::WorkItems::WorkItemHierarchy, feature_category: :portfolio_management do
  ...
  describe '#base_and_ancestors' do
    subject(:work_item_hierarchy) { described_class.new(::WorkItem.where(id: issue1.id), options: options) }

    it 'includes the base and its ancestors' do
      relation = work_item_hierarchy.base_and_ancestors
    ...
    context 'when same_type option is used' do
      ...
      subject(:work_item_hierarchy) { described_class.new(::WorkItem.where(id: epic3.id), options: options) }
    ...
    it 'can find ancestors upto a certain level' do
      relation = work_item_hierarchy.base_and_ancestors(upto: epic2)
    ...
    describe 'hierarchy_order option' do
      let(:relation) do
        work_item_hierarchy.base_and_ancestors(hierarchy_order: hierarchy_order)
      end
  ...
  describe '#base_and_descendants' do
    subject(:work_item_hierarchy) { described_class.new(::WorkItem.where(id: epic2.id), options: options) }

    it 'includes the base and its descendants' do
      relation = work_item_hierarchy.base_and_descendants
    ...
    context 'when with_depth is true' do
      let(:relation) do
        work_item_hierarchy.base_and_descendants(with_depth: true)
      end
  ...
  describe 'with objective hierarchy' do
    ...
    describe '#base_and_ancestors for objectives' do
      subject(:work_item_hierarchy) { described_class.new(::WorkItem.where(id: key_result.id), options: options) }
      ...
      context 'when same_type option is used' do
        subject(:work_item_hierarchy) { described_class.new(::WorkItem.where(id: objective3.id), options: options) }
