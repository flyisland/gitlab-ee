...
RSpec.describe Gitlab::Auth::Ldap::Person, feature_category: :system_access do
  ...
  describe '#kerberos_principal' do
    ...
    subject(:person) { described_class.new(entry, 'ldapmain') }
    ...
    it 'returns the principal combining the configured UID and DC components of the distinguishedName' do
      expect(person.kerberos_principal).to eq('myName@BAR.COM')
    end
  end
  ...
  describe '#ssh_keys' do
    ...
    context 'when the SSH key is literal' do
      ...
      it 'includes the SSH key' do
        expect(person.ssh_keys).to include(ssh_key)
      end
    end
    ...
    context 'when the key is not an SSH key' do
      ...
      it 'is empty' do
        expect(person.ssh_keys).to be_empty
      end
    end
    ...
    context 'when there are multiple keys' do
      ...
      it 'includes both SSH keys' do
        expect(person.ssh_keys).to include(ssh_key)
        expect(person.ssh_keys).to include('ssh-rsa keykeykey')
        expect(person.ssh_keys).not_to include('KerberosKey:bogus')
      end
    end
    ...
    context 'when the ssh key is security key type' do
      ...
      it 'includes the ssh key' do
        ...
        expect(person.ssh_keys).to include(ssh_key_without_suffix)
      end
    end
  end
