    ...
    context 'LDAP user' do
      ...
      context 'when exists in LDAP/AD' do
        ...
        context 'user blocked in LDAP/AD' do
          ...
          it 'creates an audit event', :aggregate_failures do
            expect { access.allowed? }.to change { AuditEvents::UserAuditEvent.count }.by(1)

            audit_event = AuditEvents::UserAuditEvent.order(:id).last
            expect(audit_event.user_id).to eq(user.id)
            expect(audit_event.details[:custom_message]).to eq("Blocked user by LDAP sync")
            expect(audit_event.details[:event_name]).to eq('user_blocked_by_ldap_sync')
            expect(audit_event.details[:system_event]).to be true
            expect(audit_event.details[:reason]).to eq('is disabled in Active Directory')
            expect(audit_event.details[:extern_uid]).to eq(user.ldap_identity.extern_uid)
            expect(audit_event.details[:author_name]).to eq('(System)')
          end

          it 'creates only one audit event across repeated syncs' do
            expect { access.allowed? }.to change { AuditEvents::UserAuditEvent.count }.by(1)
            expect { described_class.new(user).allowed? }.not_to change { AuditEvents::UserAuditEvent.count }
          end
          ...
          context 'when not licensed' do
            ...
            it 'does not create an audit event' do
              expect { access.allowed? }.not_to change { AuditEvents::UserAuditEvent.count }
            end
          end
      ...
      context 'when no longer exist in LDAP/AD' do
        ...
        it 'creates an audit event', :aggregate_failures do
          ...
          expect(audit_event.details[:event_name]).to eq('user_blocked_by_ldap_sync')
          expect(audit_event.details[:system_event]).to be true
          expect(audit_event.details[:reason]).to eq('does not exist anymore')
          expect(audit_event.details[:extern_uid]).to eq(user.ldap_identity.extern_uid)
        end
