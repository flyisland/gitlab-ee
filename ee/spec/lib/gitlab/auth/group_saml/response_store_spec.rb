...
RSpec.describe Gitlab::Auth::GroupSaml::ResponseStore do
  ...
  subject(:response_store) { described_class.new(session_id) }

  describe '#set_raw' do
    it 'stores values in Redis' do
      response_store.set_raw(raw_response)
  ...
  describe '#get_raw' do
    it 'retrives a value set by set_response' do
      ...
      expect(response_store.get_raw).to eq raw_response
    end

    it 'prevents memory bloat by deleting the value' do
      response_store.set_raw(raw_response)
      response_store.get_raw

      expect(response_store.get_raw).to be_nil
    end
  end
end
