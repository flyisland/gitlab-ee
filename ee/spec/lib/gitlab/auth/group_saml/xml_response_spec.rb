...
RSpec.describe Gitlab::Auth::GroupSaml::XmlResponse do
  ...
  subject(:xml_response) { described_class.new(raw_response: raw_response, group: group) }

  it 'configures ruby-saml using configured settings' do
    expect(xml_response.saml_response.settings.idp_cert_fingerprint).to eq saml_provider.certificate_fingerprint
  end

  it 'validates xml according to SAML spec' do
    expect(xml_response.errors).to include(/Current time is on or after NotOnOrAfter condition/)
    is_expected.not_to be_valid
  end

  it 'correctly detects fingerprint mismatch' do
    expect(xml_response.errors).to include('Fingerprint mismatch')
  end
  ...
  describe 'attributes from encoded XML' do
    ...
    it 'retrieves NameID from XML' do
      expect(xml_response.name_id).to eq name_id
    end

    it 'retrieves NameID Format from XML' do
      expect(xml_response.name_id_format).to eq name_id_format
    end

    it 'provides decoded XML' do
      expect(xml_response.xml).to start_with('<?xml')
    end
  end
end
