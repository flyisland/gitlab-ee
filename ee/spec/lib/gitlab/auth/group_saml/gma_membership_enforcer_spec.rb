...
RSpec.describe Gitlab::Auth::GroupSaml::GmaMembershipEnforcer do
  ...
  subject(:enforcer) { described_class.new(project) }
  ...
  it 'allows adding a project bot to project' do
    ...
    expect(enforcer.can_add_user?(project_bot)).to be_truthy
  end

  context 'when user is group-managed' do
    it 'allows adding user to project' do
      expect(enforcer.can_add_user?(managed_user)).to be_truthy
    end
  end
  ...
  context 'when user is not group-managed' do
    it 'does not allow adding user to project' do
      ...
      expect(enforcer.can_add_user?(user)).to be_falsey
    end
  end
  ...
  context 'when the project is forked' do
    ...
    subject(:enforcer) { described_class.new(fork_project(project, managed_user_for_project)) }
    ...
    context 'when user is not group-managed' do
      it 'does not allow adding user to project' do
        expect(enforcer.can_add_user?(create(:user))).to be_falsey
      end
    end
    ...
    context 'from deleted project' do
      ...
      context 'when user is group-managed' do
        it 'allows adding user to project' do
          enforcer = described_class.new(forked_project)
          expect(enforcer.can_add_user?(managed_user)).to be_truthy
        end
      end
      ...
      context 'when user is not group-managed' do
        it 'does not allow adding user to project' do
          enforcer = described_class.new(forked_project)
          expect(enforcer.can_add_user?(create(:user))).to be_truthy
        end
      end
    end
  end
  ...
  context 'when project is forked from namespace to group' do
    ...
    subject(:enforcer) { described_class.new(forked_project) }
