...
RSpec.describe Gitlab::Auth::Smartcard::Certificate, :with_current_organization do
  ...
  shared_examples 'a new smartcard identity' do
    it 'creates smartcard identity' do
      expect { find_or_create_user }.to change { SmartcardIdentity.count }.by(1)
  ...
  shared_examples 'an existing user' do
    it 'finds existing user' do
      expect(find_or_create_user).to eql(user)
    end
  ...
  describe '#find_or_create_user' do
    subject(:find_or_create_user) { described_class.new(certificate_header, current_organization).find_or_create_user }
      ...
      context 'username generation' do
        ...
        context 'avoids conflicting namespaces' do
          ...
          it 'creates user with correct usnername' do
            expect { find_or_create_user }.to change { User.count }.from(1).to(2)
      ...
      context 'email extraction' do
        context 'when emailAddress contains embedded emailAddress= substring' do
          ...
          it 'extracts email correctly' do
            expect(find_or_create_user.email).to eql(forged_email)
          end
        end
