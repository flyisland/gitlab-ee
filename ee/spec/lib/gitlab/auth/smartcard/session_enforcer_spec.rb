...
RSpec.describe Gitlab::Auth::Smartcard::SessionEnforcer do
  subject(:session_enforcer) { described_class.new }
  ...
  describe '#update_session' do
    ...
    it 'stores the time of last sign-in in session' do
      expect { session_enforcer.update_session }.to change { session[:smartcard_signins] }
  ...
  describe '#access_restricted?' do
    ...
    subject { session_enforcer.access_restricted?(user) }
