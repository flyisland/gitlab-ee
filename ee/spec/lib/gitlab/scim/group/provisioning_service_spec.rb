...
RSpec.describe ::Gitlab::Scim::Group::ProvisioningService, :saas,
  ...
  describe '#execute' do
      ...
      context 'with BSO (Block Seat Overages) enabled' do
        ...
        context 'without available seats' do
          ...
          it 'tracks minimal access provisioning' do
            allow(service).to receive(:track_minimal_access_provisioning)

            service.execute

            expect(service).to have_received(:track_minimal_access_provisioning).with(group, user.id)
          end
        ...
        context 'with available seats' do
          ...
          it 'does not track minimal access provisioning' do
            ...
            expect(service).not_to have_received(:track_minimal_access_provisioning).with(group, user.id)
          end
        end
      end
    ...
    context 'for an existing user' do
      ...
      context 'when the user is pending deletion' do
        before do
        ...
        context 'when the user has a scim identity' do
          ...
          it 'returns an error' do
            ...
            expect(response.as_json).to match(hash_including('status' => 'conflict',
              'message' => /User is pending deletion/,
              'identity' => nil))
          end
        end
