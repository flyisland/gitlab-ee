...
RSpec.describe Gitlab::Mirror, feature_category: :source_code_management do
  ...
  describe '#configure_cron_job!' do
    ...
    describe 'without jobs already running' do
      before do
        Sidekiq::Client.via(Gitlab::Redis::Queues.sidekiq_redis) do
          update_all_mirrors_cron_job&.destroy # rubocop:disable Rails/SaveBang -- Not an ActiveRecord model
        end
      end

      it 'creates update_all_mirrors_worker' do
        expect { described_class.configure_cron_job! }
          .to change { update_all_mirrors_cron_job }.from(nil).to(Sidekiq::Cron::Job)
        expect(update_all_mirrors_cron_job.cron).to eq(cron)
      end
      ...
      describe 'when Geo is enabled' do
        it 'disables mirror cron job' do
          ...
          expect(update_all_mirrors_cron_job).to be_enabled

          allow(Gitlab::Geo).to receive_messages(connected?: true, secondary?: true)
          described_class.configure_cron_job!

          expect(update_all_mirrors_cron_job).to be_nil
        end
      end
    end
  end
  ...
  describe '#max_mirror_capacity_reached?', :clean_gitlab_redis_shared_state do
    it 'returns true if available capacity is 0' do
      ...
      expect(described_class.max_mirror_capacity_reached?).to be(true)
    end

    it 'returns false if available capacity is not 0' do
      expect(described_class).to receive(:available_capacity).and_return(1)

      expect(described_class.max_mirror_capacity_reached?).to be(false)
    end
  end
  ...
  describe '#untrack_scheduling', :clean_gitlab_redis_shared_state do
    context 'with scheduling counter above 0' do
      ...
      it 'does not decrement scheduling counter for non-existent id' do
