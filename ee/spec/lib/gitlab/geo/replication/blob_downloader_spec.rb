...
RSpec.describe Gitlab::Geo::Replication::BlobDownloader, feature_category: :geo_replication do
  ...
  describe '#execute' do
    ...
    describe 'download file' do
      ...
      shared_examples 'blob download' do
      ...
      context 'with http.rb (default)' do
        before do
          stub_feature_flags(geo_blob_download_with_gitlab_http: false)
        end

        it 'uses the http.rb code path' do
          expect(downloader).to receive(:download_file_with_http_rb).and_call_original

          stub_request(:get, downloader.resource_url).to_return(status: 200, body: 'test')
          downloader.execute
        end

        it_behaves_like 'blob download'
      end

      context 'with Gitlab::HTTP (feature flag enabled)' do
        before do
          stub_feature_flags(geo_blob_download_with_gitlab_http: true)
        end

        it 'uses the Gitlab::HTTP code path' do
          expect(downloader).to receive(:download_file_with_gitlab_http).and_call_original
