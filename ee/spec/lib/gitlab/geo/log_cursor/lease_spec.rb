...
RSpec.describe Gitlab::Geo::LogCursor::Lease, :clean_gitlab_redis_shared_state,
  ...
  describe '.try_obtain_with_ttl' do
    it 'returns zero when there is no lease' do
      result = described_class.try_obtain_with_ttl { nil }
    ...
    it 'does not log an error or info message when could not obtain lease' do
      ...
      allow(lease).to receive_messages(try_obtain_with_ttl: { ttl: 10, uuid: 'uuid' }, same_uuid?: false)
      allow(described_class).to receive(:exclusive_lease).and_return(lease)
      ...
      described_class.try_obtain_with_ttl { nil }
    end
