...
RSpec.describe Gitlab::Checks::Diffs::CodeOwnersCheck, feature_category: :source_code_management do
  ...
  let_it_be(:owner_1, freeze: false) { create(:user, username: 'owner-1') }
  let_it_be(:email_owner, freeze: false) { create(:user, username: 'owner-2') }
  let_it_be(:owner_3, freeze: false) { create(:user, username: 'owner-3') }
  let_it_be(:documentation_owner, freeze: false) { create(:user, username: 'documentation-owner') }
  let_it_be(:test_owner, freeze: false) { create(:user, username: 'test-owner') }
  let_it_be(:codeowner_content, freeze: false) do
    <<~CODEOWNERS
  ...
  let_it_be(:codeowner_blob, freeze: false) { fake_blob(path: 'CODEOWNERS', data: codeowner_content) }
