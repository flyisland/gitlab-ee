...
RSpec.describe Gitlab::Checks::DiffCheck, feature_category: :source_code_management do
  ...
  describe '#validate!' do
    ...
    context 'no push rules active' do
      let_it_be_with_reload(:push_rule) { create(:push_rule) }
    ...
    describe '#validate_code_owners?' do
      let_it_be_with_reload(:push_rule) { create(:push_rule, file_name_regex: 'READ*') }
      let_it_be(:project) { create(:project, :public, :repository, push_rule: push_rule) }
      ...
      context 'when user can not push to the branch' do
        context 'when not updated from web' do
          it 'checks if the branch requires code owner approval' do
            ...
            expect(validate_code_owners).to be(true)
          end
        end
        ...
        context 'when updated from the web' do
          ...
          it 'returns false' do
            expect(validate_code_owners).to be(false)
          end
        end
      end
    ...
    context 'file name rules' do
      ...
      context 'denylisted files check' do
        let_it_be_with_reload(:push_rule) { create(:push_rule, prevent_secrets: true) }
    ...
    context 'file lock rules' do
      ...
      let_it_be_with_refind(:project) { create(:project, :public, :repository, push_rule: push_rule, developers: owner) }

      let(:path_lock) { create(:path_lock, path: 'README', project: project) }

      shared_examples 'a locked file' do
        let_it_be(:path_lock) { create(:path_lock, path: filename, project: project, user: owner) }
