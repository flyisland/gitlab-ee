...
RSpec.describe Gitlab::CodeOwners::UsersLoader, feature_category: :source_code_management do
  ...
  describe '#members' do
    ...
    let_it_be(:blocked_developer, freeze: false) { create(:user, :blocked, developer_of: project) }
    let_it_be(:deactivated_developer, freeze: false) { create(:user, :deactivated, developer_of: project) }
    let_it_be(:non_member, freeze: false) { create(:user) }

    let_it_be(:valid_users, freeze: false) { [developer, maintainer] }
    let_it_be(:invalid_users, freeze: false) { [blocked_developer, deactivated_developer, non_member] }

    let_it_be(:loader, freeze: false) do
      names = [
        developer.username,
        maintainer.username,
    ...
    context 'when matching by usernames case insensitively' do
      let_it_be(:loader, freeze: false) { described_class.new(project, names: [developer.username.upcase]) }

      it { is_expected.to contain_exactly(developer) }
    end

    context 'when no names or emails provided' do
      let_it_be(:loader, freeze: false) { described_class.new(project, names: [], emails: []) }
