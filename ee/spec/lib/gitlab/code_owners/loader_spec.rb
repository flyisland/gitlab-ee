...
RSpec.describe Gitlab::CodeOwners::Loader, feature_category: :source_code_management do
  ...
  describe '#optional_section?', :request_store do
    let(:code_owners_file) { Gitlab::CodeOwners::File.new(codeowner_blob) }

    before do
      allow(Gitlab::CodeOwners::File).to receive(:new).and_return(code_owners_file)

      2.times { described_class.new(project, 'with-codeowners').optional_section?('codeowners') }
    end

    it 'shares the file across loader instances' do
      expect(Gitlab::CodeOwners::File).to have_received(:new).once
    end
  end
  ...
  describe '#empty_code_owners?' do
    context 'when file does not exist' do
      ...
      it 'returns true' do
        expect(loader.empty_code_owners?).to be(true)
      end
    end
    ...
    context 'when file content exists' do
      it 'returns false' do
        expect(loader.empty_code_owners?).to be(false)
      end
    end
  end
  ...
  describe '#track_file_validation' do
    subject(:track_validation) { loader.track_file_validation }
