...
RSpec.describe Gitlab::EventStore::Subscriptions::DependencyManagementSubscriptions,
  ...
  context 'for Ci::PipelineFinishedEvent' do
    ...
    context 'when the pipeline ref is not a dependency-management branch' do
      let(:pipeline) do
        create(:ci_pipeline, project: project, merge_request: merge_request, status: :failed, ref: 'master')
      end
