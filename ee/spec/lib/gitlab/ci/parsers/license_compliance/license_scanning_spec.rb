...
RSpec.describe Gitlab::Ci::Parsers::LicenseCompliance::LicenseScanning do
  describe '#parse!' do
    ...
    context 'when parsing JSON data' do
      it 'uses safe_parse to parse the JSON' do
        expect(Gitlab::Json).to receive(:safe_parse).with('{}', symbolize_names: true, object_class: Hash).and_call_original
