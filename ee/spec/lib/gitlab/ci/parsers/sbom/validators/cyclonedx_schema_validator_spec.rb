...
RSpec.describe Gitlab::Ci::Parsers::Sbom::Validators::CyclonedxSchemaValidator,
  ...
  context 'when spec version is supported' do
    where(:spec_version) { %w[1.4 1.5 1.6 1.7] }
  ...
  context 'with a real container-scanning SBOM report (specVersion 1.7)' do
    let(:report_data) do
      Gitlab::Json::SafeParser.parse(
        File.read(Rails.root.join('ee/spec/fixtures/security/cyclonedx/container_scanning_1_7.cdx.json'))
      )
    end

    it 'is valid with no errors', :aggregate_failures do
