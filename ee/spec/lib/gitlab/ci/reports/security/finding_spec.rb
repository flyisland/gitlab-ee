...
RSpec.describe Gitlab::Ci::Reports::Security::Finding, feature_category: :vulnerability_management do
  ...
  describe '#initialize' do
    subject(:finding) { described_class.new(**params) }
    ...
    context 'when both all params are given' do
      it 'initializes an instance' do
        expect { finding }.not_to raise_error
        ...
        expect(finding).to have_attributes(
          ...
          project_id: 1,
          uuid: be_truthy,
          context_unaware_uuid: be_truthy,
          details: {
            'commit' => {
              'name' => [
                {
    ...
    %i[identifiers location metadata_version name original_data report_type scanner scan].each do |attribute|
      context "when attribute #{attribute} is missing" do
        ...
        it 'raises an error' do
          expect { finding }.to raise_error(ArgumentError)
        end
      end
    end
  end
  ...
  describe "delegation" do
    subject(:finding) { create(:ci_reports_security_finding) }

    %i[file_path start_line end_line].each do |attribute|
      it "delegates attribute #{attribute} to location" do
        expect(finding.public_send(attribute)).to eq(finding.location.public_send(attribute))
      end
    end
  end
  ...
  describe '#update_location' do
    ...
    subject(:update_result) { occurrence.update_location(new_location) }
  ...
  describe '#eql?' do
    ...
    with_them do
      ...
      subject(:equality_check) { finding.eql?(other_finding) }

      context 'when the primary_identifier is nil' do
        let(:identifier) { nil }

        it 'does not raise an exception' do
          expect { equality_check }.not_to raise_error
        end
      end
  ...
  describe '#valid?' do
    ...
    context 'when the uuid is missing' do
      # project_id is a required parameter for UUID generation, if it's nil, the UUID will be nil.
      let(:project_id) { nil }
  ...
  describe '#hash' do
    ...
    context 'with vulnerability_finding_signatures enabled' do
      ...
      it 'returns the expected hash with no signatures' do
        ...
        expect(finding.hash).to eq([finding.report_type, finding.location.fingerprint, finding.primary_identifier_fingerprint].hash)
      end
      ...
      it 'returns the expected hash with signatures' do
        ...
        expect(finding.hash).to eq([finding.report_type, high_priority_signature.signature_hex, finding.primary_identifier_fingerprint].hash)
      end
    end
