...
RSpec.describe Gitlab::Timebox::SnapshotBuilder, :aggregate_failures, feature_category: :team_planning do
  ...
  shared_examples 'builds snapshots for timebox' do
    ...
    context 'when resource events exist' do
      context 'when issue is assigned to a timebox' do
        let_it_be(:issue, freeze: false) { create(:issue, project: project, title: 'issue') }
        let_it_be(:task, freeze: false) { create(:work_item, :task, project: project, title: 'task') }
      ...
      context 'when multiple issues and tasks exist' do
        let_it_be(:issue1, freeze: false) { create(:issue, project: project, title: 'issue1') }
        let_it_be(:issue2, freeze: false) { create(:issue, project: project, title: 'issue2') }
        let_it_be(:task1, freeze: false) { create(:work_item, :task, project: project, title: 'task1') }
        let_it_be(:task2, freeze: false) { create(:work_item, :task, project: project, title: 'task2') }
  ...
  context 'when timebox is milestone' do
    it_behaves_like 'builds snapshots for timebox' do
      ...
      let_it_be(:resource_timebox_event, freeze: false) { ResourceMilestoneEvent }
      let_it_be(:resource_timebox_factory, freeze: false) { resource_timebox_event.name.underscore.to_sym }
      let_it_be(:timebox, freeze: false) { create(:milestone, project: project, start_date: Date.current, due_date: Date.current + 6.days) }
      let_it_be(:next_timebox, freeze: false) { create(:milestone, project: project, start_date: Date.current + 7.days, due_date: Date.current + 13.days) }
      let_it_be(:timebox_type, freeze: false) { :milestone }
  ...
  context 'when timebox is iteration' do
    ...
    it_behaves_like 'builds snapshots for timebox' do
      # rubocop:disable Layout/LineLength
      let_it_be(:resource_timebox_event, freeze: false) { ResourceIterationEvent }
      let_it_be(:resource_timebox_factory, freeze: false) { resource_timebox_event.name.underscore.to_sym }
      let_it_be(:timebox, freeze: false) { create(:iteration, iterations_cadence: cadence, start_date: Date.current, due_date: Date.current + 6.days) }
      let_it_be(:next_timebox, freeze: false) { create(:iteration, iterations_cadence: cadence, start_date: Date.current + 7.days, due_date: Date.current + 13.days) }
      let_it_be(:timebox_type, freeze: false) { :iteration }
      # rubocop:enable Layout/LineLength
    end
  end
