...
RSpec.describe Gitlab::StatusPage::Storage, feature_category: :incident_management do
  ...
  describe '.details_url' do
    ...
    context 'with a published incident' do
      ...
      context 'when status page setting is disabled' do
        let_it_be_with_reload(:setting) { create(:status_page_setting, project: issue.project) }

        it { is_expected.to be_nil }
      end

      context 'when status page setting is enabled' do
        let_it_be_with_reload(:setting) { create(:status_page_setting, :enabled, project: issue.project) }
