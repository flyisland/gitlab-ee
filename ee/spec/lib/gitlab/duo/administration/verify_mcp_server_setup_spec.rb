...
RSpec.describe Gitlab::Duo::Administration::VerifyMcpServerSetup, :silence_stdout, feature_category: :mcp_server do
  let_it_be(:user) { create(:user, :admin, username: 'mcp_test_user') }
  ...
  describe '#check_instance_access_rules' do
    ...
    context 'when duo_access_through_namespaces is enabled and rules exist' do
    ...
    context 'when duo_access_through_namespaces is enabled and rules exist without namespace' do
      before do
        stub_feature_flags(duo_access_through_namespaces: true)
    ...
    context 'when duo_access_through_namespaces is enabled and no rules exist' do
      before do
        stub_feature_flags(duo_access_through_namespaces: true)
      end
