...
RSpec.describe Gitlab::Sitemaps::UrlExtractor, feature_category: :navigation do
  ...
  describe '.extract' do
    ...
    context 'when element is unknown' do
      ...
      it 'returns nil' do
        expect(extracted_url).to be_nil
      end
    end
  end
  ...
  describe '.extract_from_group' do
    ...
    subject(:group_urls) { described_class.extract_from_group(group) }
    ...
    it 'returns several group urls' do
      ...
      expect(group_urls).to match_array(expected_urls)
    end
  end
  ...
  describe '.extract_from_project' do
    ...
    subject(:project_urls) { described_class.extract_from_project(project) }
    ...
    it 'returns several project urls' do
      ...
      expect(project_urls).to match_array(expected_urls)
    end
    ...
    context 'when wiki access level is' do
      context 'disabled' do
        it 'does not include wiki url' do
          ...
          expect(project_urls).not_to include(project_wiki_url(project, Wiki::HOMEPAGE))
        end
      end
    ...
    context 'when snippets are disabled' do
      context 'disabled' do
        it 'does not include snippets url' do
          ...
          expect(project_urls).not_to include(project_snippets_url(project))
        end
      end
    ...
    context 'when issues are disabled' do
      context 'disabled' do
        it 'does not include issues url' do
          ...
          expect(project_urls).not_to include(project_issues_url(project))
        end
      end
    ...
    context 'when merge requests are disabled' do
      context 'disabled' do
        it 'does not include merge requests url' do
          ...
          expect(project_urls).not_to include(project_merge_requests_url(project))
        end
      end
