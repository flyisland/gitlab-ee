...
RSpec.describe Gitlab::Llm::Chain::Tools::TroubleshootJob::Executor, feature_category: :continuous_integration do
  ...
  before do
    ...
    # An authorized troubleshoot user can read the trace; authorize now enforces this.
  ...
  describe '#execute' do
    context 'when the user is authorized' do
      ...
      context 'when log is truncated' do
        ...
        context 'for internal events tracking' do
          ...
          context 'when CI job is not failed but has a trace' do
            let(:build) { create(:ci_build, :running, :trace_live, project: project) }

            it 'tracks troubleshoot_job event' do
              expect(Gitlab::InternalEvents).to receive(:track_event).with(
                'troubleshoot_job',
                user: user,
                job: build,
      ...
      context 'when the job is not failed' do
        ...
        it 'performs the troubleshooting', :aggregate_failures do
          expect(tool).to receive(:request).and_return('Troubleshooting response')
          expect(tool.execute.content).to eq('Troubleshooting response')
        end
      end
    ...
    context 'when the user has the troubleshoot entitlement but cannot read the job trace' do
      # Lacking :read_build_trace must deny even with the usage entitlement (flag-on path).
      include_context 'with stubbed LLM authorizer', allowed: true

      before do
        stub_feature_flags(dap_external_trigger_usage_billing: true)
        allow(user).to receive(:allowed_to_use?)
          .with(:troubleshoot_job, root_namespace: build.project.root_ancestor).and_return(true)
        allow(user).to receive(:can?).with(:read_build_trace, build).and_return(false)
      end

      it 'does not send the trace to the LLM and returns a not-authorized answer', :aggregate_failures do
        expect(tool).not_to receive(:request)
        expect(tool.execute.content).to include('This feature is not available with your current configuration')
      end
    end

    context 'when the flag is disabled and the user cannot read the job trace' do
      # Pins the guard on the flag-off branch independently of :troubleshoot_job_with_ai.
      include_context 'with stubbed LLM authorizer', allowed: true
      ...
      it 'does not send the trace to the LLM and returns a not-authorized answer', :aggregate_failures do
        expect(tool).not_to receive(:request)
        expect(tool.execute.content).to include("you don't have access to them, or your session has expired.")
      end
    end
    ...
    context 'when selecting prompt version' do
      ...
      before do
        ...
        allow(Gitlab::Llm::Chain::Requests::AiGateway).to receive(:new).with(
          user,
          unit_primitive_name: :troubleshoot_job,
