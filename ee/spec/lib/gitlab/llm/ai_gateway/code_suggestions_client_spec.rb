...
RSpec.describe Gitlab::Llm::AiGateway::CodeSuggestionsClient, feature_category: :code_suggestions do
  ...
  describe '#test_model_connection', :with_cloud_connector do
    let(:tested_self_hosted_model) { create(:ai_self_hosted_model, name: 'Test Model') }
    let(:endpoint) { "#{Gitlab::AiGateway.url}/v1/prompts/model_configuration%2Fcheck" }

    subject(:result) { described_class.new(user).test_model_connection(tested_self_hosted_model) }

    before do
      allow(Gitlab::AiGateway).to receive(:headers).with(
        user: user, unit_primitive_name: unit_primitive, ai_feature_name: expected_ai_feature,
        organization_id: organization_id, feature_setting: anything
      ).and_return(ai_gateway_headers)
    end

    shared_examples 'self hosted model is passed to the AI gateway' do
      it 'creates the right mock task' do
        expect(Gitlab::Llm::AiGateway::SelfHostedModels::TestedModelDetails)
          .to receive(:new)
          .with(current_user: user, self_hosted_model: tested_self_hosted_model)
          .once
          .and_call_original
      ...
      it 'tasks has the right model as assigned' do
        expect_next_instance_of(Gitlab::Llm::AiGateway::SelfHostedModels::TestedModelDetails) do |model_details|
          expect(model_details.feature_setting.self_hosted_model).to eq(tested_self_hosted_model)
        end
    ...
    context 'when there is no self-hosted model provided' do
      let(:tested_self_hosted_model) { nil }
    ...
    context 'when the AI Gateway responded with a 421 Misdirected Request' do
      ...
      it_behaves_like 'self hosted model is passed to the AI gateway'
    ...
    context 'when a model is pinned it should behave the same' do
      before do
