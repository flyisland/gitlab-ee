...
RSpec.describe Gitlab::Llm::AiGateway::Completions::ReviewMergeRequest::ResponseBodyParser, feature_category: :duo_code_review do
  ...
  describe '#summary' do
    ...
    context 'when response contains comments_summary' do
      let(:body) do
        <<~RESPONSE
        <review></review>
        <comments_summary>
        The code changes introduce a new feature with no issues found.
        </comments_summary>
        RESPONSE
      end

      it 'extracts the comments_summary content' do
        expect(parser.summary).to include('The code changes introduce a new feature')
        expect(parser.summary).not_to include('<comments_summary>')
      end
    end
    ...
    context 'when response contains both comments_summary and summary' do
      let(:body) do
        <<~RESPONSE
        <review></review>
        <summary>Old summary content.</summary>
        <comments_summary>New comments summary content.</comments_summary>
        RESPONSE
      end

      it 'prefers comments_summary over summary' do
        expect(parser.summary).to include('New comments summary content')
        expect(parser.summary).not_to include('Old summary content')
      end
    end
