...
RSpec.describe Gitlab::SubscriptionPortal::Clients::Rest, :without_license, feature_category: :subscription_management do
  ...
  describe 'request methods', :saas do
    ...
    describe '#generate_addon_trial_lead' do
      ...
      context 'when new_addon_lead_endpoint feature flag is disabled' do
        let(:route_path) { 'trials/create_addon' }

        before do
          stub_feature_flags(new_addon_lead_endpoint: false)
        end
      ...
      context 'when new_addon_lead_endpoint feature flag is enabled' do
    ...
    describe '#generate_addon_trial' do
      ...
      context 'when new_addon_trial_endpoint feature flag is disabled' do
        let(:route_path) { 'trials/create_addon' }

        before do
          stub_feature_flags(new_addon_trial_endpoint: false)
        end
      ...
      context 'when new_addon_trial_endpoint feature flag is enabled' do
        let(:route_path) { 'trials/gitlab_com/addons' }

        before do
          stub_feature_flags(new_addon_trial_endpoint: true)
        end
        ...
        it "nests in the trial_user param if needed" do
          expect(client).to receive(:http_post).with('trials/gitlab_com/addons', anything,
            { trial_user: { foo: 'bar' } })
