      ...
      context 'when called from a CDot-connected install' do
        context 'with namespace_id (gitlab.com SaaS)', :saas do
          ...
          context 'with a non-200 status from CDot' do
            ...
            [400, 401, 422, 500, 503].each do |code|
              context "with HTTP #{code}" do
                ...
                # SecretsManagement::Entitlement::Resolver#transport_failure?
                # reads `cause` to decide whether an outage may be replayed from
                # the last-known-good slot. A status raises outside any rescue,
                # so there must be no cause and the resolver must fail closed.
          ...
          context 'when the HTTP layer raises a network error' do
            ...
            # The counterpart to the no-cause example above: re-raising from
            # inside the rescue is what sets `cause`, and that is the whole
            # discriminator the last-known-good fallback turns on.
