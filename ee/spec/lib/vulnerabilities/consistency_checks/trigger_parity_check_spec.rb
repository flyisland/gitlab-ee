...
RSpec.describe Vulnerabilities::ConsistencyChecks::TriggerParityCheck, feature_category: :vulnerability_management do
  ...
  describe '#execute' do
    ...
    context 'when finding location has kubernetes_resource without agent_id' do
      ...
      before do
        finding.update_column(:location, { 'image' => 'alpine:3.7', 'kubernetes_resource' => { 'cluster_id' => '1' } })
        Vulnerabilities::Read.where(vulnerability_id: vulnerability.id).delete_all
      end
