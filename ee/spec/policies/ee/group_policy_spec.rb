...
RSpec.describe GroupPolicy, feature_category: :groups_and_projects do
  ...
  context 'when epics feature is enabled' do
    ...
    context 'when user is support bot' do
      let_it_be(:current_user, freeze: false) { create(:support_bot) }
  ...
  context 'when iterations feature is enabled' do
    ...
    context 'when project is public' do
      let_it_be(:group, freeze: false) { create(:group, :public, :owner_subgroup_creation_only) }
    ...
    context 'when group_saml is available' do
      ...
      context 'when group_saml_group_sync is not licensed' do
        context 'with an enabled SAML provider' do
          let_it_be(:saml_provider, freeze: false) { create(:saml_provider, group: group, enabled: true) }
      ...
      context 'with SSO enforcement enabled' do
        ...
        let_it_be(:saml_provider, freeze: false) { create(:saml_provider, group: group, enforced_sso: true) }
      ...
      context 'without SSO enforcement enabled' do
        ...
        let_it_be(:saml_provider, freeze: false) { create(:saml_provider, group: group, enforced_sso: false) }
  ...
  context 'when memberships locked to SAML' do
    ...
    context 'when group is not a root group' do
      ...
      let_it_be(:group, freeze: false) { create(:group, :private, parent: parent_group) }
    ...
    context 'when feature is enabled' do
      ...
      context 'with owner' do
        ...
        context 'when group has parent' do
          let_it_be(:group, freeze: false) { create(:group, :private, parent: create(:group)) }
  ...
  describe 'private nested group use the highest access level from the group and inherited permissions' do
    let_it_be(:nested_group) { create(:group, :private, parent: group) }
  ...
  context 'with group access tokens', :saas do
    ...
    context 'on GitLab.com paid', :saas do
      let_it_be(:group, freeze: false) { create(:group_with_plan, plan: :bronze_plan) }
      ...
      context 'with owner' do
        ...
        context 'when creating resource access tokens' do
          ...
          context 'when parent group has resource access token creation disabled' do
            ...
            let_it_be(:parent, freeze: false) { create(:group_with_plan, plan: :bronze_plan, namespace_settings: namespace_settings) }
  ...
  describe 'user banned from namespace' do
    ...
    context 'when user is banned' do
      ...
      context 'when inside a subgroup' do
        let_it_be(:group, freeze: false) { create(:group, :private, :nested) }
      ...
      context 'when container is a group with AI enabled' do
        ...
        context 'with customizable permissions' do
          ...
          context 'when user is an org owner' do
            let_it_be(:current_user, freeze: false) { create(:organization_owner, organization: group.organization).user }
  ...
  context 'with custom role' do
    ...
    context 'for a member role with the `admin_group_member` ability' do
      ...
      context 'with admin_service_account_member' do
        ...
        let_it_be(:group, freeze: false) { create(:group) }
        let_it_be(:role, freeze: false) do
          create(
            :member_role,
            :guest,
            namespace: group,
  ...
  describe 'model_selection_allowlist' do
    ...
    before do
      stub_saas_features(gitlab_com_subscriptions: is_saas)
      stub_feature_flags(model_selection_allowlist: false) unless ff_enabled
      allow(::Ai::Setting).to receive(:self_hosted?).and_return(with_self_hosted)
      allow(::Ai::AmazonQ).to receive(:connected?).and_return(amazon_q_enabled)
      allow(group).to receive(:has_free_or_no_subscription?).and_return(false)
    ...
    context 'when user is an owner' do
      ...
      where(:ff_enabled, :is_saas, :namespace_duo_enabled, :with_self_hosted, :amazon_q_enabled, :allowlist_accessible) do
    ...
    context 'when the feature flag is enabled per group' do
      let(:current_user) { owner }
      let_it_be(:other_group) { create(:group) }

      context 'when enabled only for a different group' do
        before do
          stub_feature_flags(model_selection_allowlist: other_group)
        end

        it { expect_disallowed(:read_model_selection_allowlist, :update_model_selection_allowlist) }
      end

      context 'when enabled for this group' do
        before do
          stub_feature_flags(model_selection_allowlist: group)
        end

        it { expect_allowed(:read_model_selection_allowlist, :update_model_selection_allowlist) }
      end
    end
  end
  ...
  shared_examples 'handling secrets manager permission' do
    # The policy condition delegates to Availability; the AND combinations
    # are exhaustively covered in availability_spec.rb. Here we stub
    # Availability and assert the policy reacts correctly.
