...
RSpec.describe GlobalPolicy, :aggregate_failures, feature_category: :shared do
  ...
  describe ':update_governing_knowledge_graph_namespace' do
    context 'when SaaS', :saas do
      ...
      context 'when user has no gitlab_credits add-on purchases' do
        it { is_expected.to be_disallowed(:update_governing_knowledge_graph_namespace) }
      end

      context 'when user has an active gitlab_credits add-on purchase' do
        let_it_be(:credits_add_on) { create(:gitlab_subscription_add_on, :gitlab_credits) }
        let_it_be(:group) { create(:group) }

        before_all do
          create(:gitlab_subscription_add_on_purchase, namespace: group, add_on: credits_add_on)
