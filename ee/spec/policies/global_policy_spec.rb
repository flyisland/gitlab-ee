...
RSpec.describe GlobalPolicy, :aggregate_failures, feature_category: :shared do
  ...
  describe 'access_artifact_registry_service' do
    context 'on GitLab.com (entitlement resolved against the user root groups)', :saas do
      where(:add_on_present, :expectation) do
        true  | be_allowed(:access_artifact_registry_service)
        false | be_disallowed(:access_artifact_registry_service)
      end

      with_them do
        before do
          allow(::GitlabSubscriptions::AddOnPurchase).to receive(:exists_for_unit_primitive?)
            .with(:access_artifact_registry, current_user).and_return(add_on_present)
        end

        it { is_expected.to expectation }
      end
    end

    context 'on a self-managed instance (entitlement resolved at instance level)' do
      where(:add_on_present, :expectation) do
        true  | be_allowed(:access_artifact_registry_service)
        false | be_disallowed(:access_artifact_registry_service)
      end
      ...
      with_them do
        before do
          ...
          allow(::GitlabSubscriptions::AddOnPurchase).to receive(:exists_for_unit_primitive?)
            .with(:access_artifact_registry, :instance).and_return(add_on_present)
        end
    ...
    context 'when there is no authenticated user' do
      ...
      it { is_expected.to be_disallowed(:access_artifact_registry_service) }
    end
  end
  ...
  describe 'manage third party agents access' do
    ...
    describe ':create_third_party_agent_direct_access_token permission' do
      where(:feature_flag, :direct_access_setting_disabled, :user_allowed, :check_policy) do
        true  | false | true  | be_allowed(:create_third_party_agent_direct_access_token)
        true  | false | false | be_disallowed(:create_third_party_agent_direct_access_token)
        true  | true  | true  | be_disallowed(:create_third_party_agent_direct_access_token)
        true  | true  | false | be_disallowed(:create_third_party_agent_direct_access_token)
        false | false | true  | be_disallowed(:create_third_party_agent_direct_access_token)
        false | false | false | be_disallowed(:create_third_party_agent_direct_access_token)
        false | true  | true  | be_disallowed(:create_third_party_agent_direct_access_token)
        false | true  | false | be_disallowed(:create_third_party_agent_direct_access_token)
      end
      ...
      context 'when user has no paid add-on but has credits with DAP enabled' do
        before do
          stub_feature_flags(agent_platform_claude_code: true)
  ...
  describe 'read_admin_knowledge_graph_settings' do
    let(:current_user) { admin }

    where(:admin_mode, :saas, :orbit_licensed, :ff_enabled, :check_policy) do
      true  | false | true  | true  | be_allowed(:read_admin_knowledge_graph_settings)
      true  | true  | true  | true  | be_disallowed(:read_admin_knowledge_graph_settings)
      true  | false | false | true  | be_disallowed(:read_admin_knowledge_graph_settings)
      true  | false | true  | false | be_disallowed(:read_admin_knowledge_graph_settings)
      false | false | true  | true  | be_disallowed(:read_admin_knowledge_graph_settings)
    end
    ...
    context 'when user is non-admin' do
      ...
      it { is_expected.to be_disallowed(:read_admin_knowledge_graph_settings) }
    end
  end
