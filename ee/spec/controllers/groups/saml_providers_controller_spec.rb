...
RSpec.describe Groups::SamlProvidersController, feature_category: :system_access do
  ...
  context 'when per group saml feature is enabled' do
    ...
    describe 'PUT #update' do
      ...
      context 'disabling SAML without URL and fingerprint' do
        subject do
          put :update, params:
            {
              group_id: group,
        ...
        it 'disables SAML and persists blank URL and fingerprint' do
          subject
          saml_provider.reload

          expect(saml_provider.enabled?).to be(false)
          expect(saml_provider.sso_url).to be_blank
          expect(saml_provider.certificate_fingerprint).to be_blank
        end
