...
RSpec.describe EE::RoutableActions::SsoEnforcementRedirect, feature_category: :system_access do
  ...
  describe '#should_redirect_to_group_saml_sso?' do
    ...
    it 'returns false for User routables' do
      ...
      expect(subject.should_redirect_to_group_saml_sso?(double, double)).to be(false)
    end
    ...
    shared_examples 'a routable with SSO enforcement redirect' do
      it 'is false when a new sso session is not needed' do
        ...
        expect(subject.should_redirect_to_group_saml_sso?(user, request)).to be false
      end
      ...
      it 'is true when a new sso session is needed' do
        ...
        expect(subject.should_redirect_to_group_saml_sso?(user, request)).to be true
      end
    end
    ...
    context 'with a project in a personal namespace' do
      subject(:sso_redirect) { described_class.new(create(:project), '/') }

      it 'returns false' do
        expect(sso_redirect.should_redirect_to_group_saml_sso?(user, double)).to be false
      end
    end
    ...
    shared_examples 'a routable blocked by IP enforcement' do
      before do
      ...
      it 'does not redirect, without needing to check the sso session' do
        expect(Ability).not_to receive(:policy_for)

        expect(subject.should_redirect_to_group_saml_sso?(user, request)).to be false
      end
    end

    context 'when IP enforcement already blocks access' do
      context 'with a group' do
        subject { described_class.new(root_group, '/') }

        it_behaves_like 'a routable blocked by IP enforcement'
      end
    ...
    context 'when IP enforcement allows access' do
      before do
        allow_next_instance_of(::Gitlab::IpRestriction::Enforcer) do |enforcer|
          allow(enforcer).to receive(:allows_current_ip?).and_return(true)
        end
      end
