...
RSpec.describe AutocompleteController, feature_category: :groups_and_projects do
  ...
  context 'GET users', feature_category: :user_profile do
    ...
    context 'project members' do
      ...
      describe 'GET #users with suggested users' do
        ...
        it 'passes only permitted params to the suggested-users UserSerializer', :aggregate_failures do
          serializer_args = []
          allow(UserSerializer).to receive(:new).and_wrap_original do |original, args|
            serializer_args << args
            original.call(args)
          end

          get(:users, params: request_common_params.merge(
            approval_rules: 'true', target_branch: 'feature', suggested: 'false', admin: 'true'
          ))

          # #users builds one serializer for project members, then a second for suggested reviewers.
          suggested_args = serializer_args.last

          expect(suggested_args[:merge_request_iid]).to eq(merge_request.iid.to_s)
          expect(suggested_args[:approval_rules]).to eq('true')
          expect(suggested_args[:target_branch]).to eq('feature')
          expect(suggested_args[:suggested]).to be(true)
          expect(suggested_args.to_h).not_to have_key('admin')
        end
      end
    end
  end
  ...
  context 'groups', feature_category: :groups_and_projects do
    ...
    it 'passes only permitted params to Autocomplete::ProjectInvitedGroupsFinder' do
      expect(Autocomplete::ProjectInvitedGroupsFinder).to receive(:new) do |_user, finder_params|
        expect(finder_params.to_h).to eq(
          'project_id' => project.id.to_s, 'search' => 'foo', 'with_project_access' => 'true', 'parent' => 'true'
        )

        instance_double(Autocomplete::ProjectInvitedGroupsFinder, execute: [])
      end

      get(:project_groups, params: {
        project_id: project.id, search: 'foo', with_project_access: true, parent: true, admin: true
      })
    end
  end
  ...
  describe 'GET group_subgroups', feature_category: :groups_and_projects do
    ...
    subject(:get_group_subgroups) { get :group_subgroups, params: params }
    ...
    it 'passes only permitted params to Autocomplete::GroupSubgroupsFinder' do
      expect(Autocomplete::GroupSubgroupsFinder).to receive(:new) do |_user, finder_params|
        expect(finder_params.to_h).to eq(
          'group_id' => group.id.to_s, 'search' => 'foo',
          'include_parent_descendants' => 'true', 'include_parent_shared_groups' => 'true'
        )

        instance_double(Autocomplete::GroupSubgroupsFinder, execute: [])
      end

      get :group_subgroups, params: {
        group_id: group.id, search: 'foo', include_parent_descendants: true,
        include_parent_shared_groups: true, admin: true
      }
    end
