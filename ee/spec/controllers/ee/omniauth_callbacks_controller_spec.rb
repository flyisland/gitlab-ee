...
RSpec.describe OmniauthCallbacksController, feature_category: :system_access do
  ...
  let_it_be(:user, freeze: false) { create(:omniauth_user, extern_uid: extern_uid, provider: provider) }
  ...
  context 'with strategies', :aggregate_failures do
    ...
    context 'when user is not registered yet' do
      ...
      context 'with trial omniauth' do
        ...
        context 'when trial_unification is enabled', :saas_onboarding do
        ...
        context 'when trial_unification is disabled' do
          ...
          it_behaves_like EE::Onboarding::Redirectable, 'trial'
        end
      end
      ...
      context 'with subscription concerns for stored location values' do
        ...
        context 'when it is a self-managed subscription under unification' do
          ...
          it 'stores the customers portal interstitial path under :redirect' do
            expect(controller)
              .to receive(:store_location_for)
              .with(:redirect, users_sign_up_customers_portal_redirect_path)
          ...
          context 'when unification is disabled' do
            before do
              allow_next_instance_of(::Onboarding::StatusPresenter) do |instance|
                allow(instance).to receive(:unification_enabled?).and_return(false)
              end
            end

            it 'does not store :redirect' do
              expect(controller).not_to receive(:store_location_for).with(:redirect, anything)
