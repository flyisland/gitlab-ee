...
RSpec.describe Projects::MirrorsController, feature_category: :source_code_management do
  ...
  describe 'forcing an update on a pull mirror' do
    ...
    context 'when sync_remote is given' do
      it 'updates the remote mirrors instead of pulling' do
        project = create(:project, :mirror, :remote_mirror)
        sign_in(project.first_owner)

        expect_next_found_instance_of(Project) do |found_project|
          expect(found_project).to receive(:update_remote_mirrors)
        end

        put :update_now, params: {
          namespace_id: project.namespace.to_param, project_id: project.to_param, sync_remote: true
        }

        expect(flash[:notice]).to eq('The remote repository is being updated...')
      end
    end
  end

  describe '#update' do
    let(:project) { create(:project, :mirror, :remote_mirror) }
    let(:first_owner) { project.first_owner }
    ...
    context 'when disabling an existing pull mirror' do
      let_it_be(:project) { create(:project, :mirror, import_url: 'https://example.com') }
