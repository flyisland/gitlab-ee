...
RSpec.describe Projects::Settings::RepositoryController, feature_category: :source_code_management do
  ...
  describe 'GET show' do
    context 'push rule' do
      ...
      # write behavior - regardless of read_project_push_rules FF
      # writes still go to push_rules table
      # writes part will be handled in https://gitlab.com/gitlab-org/gitlab/-/merge_requests/225833
      context 'when global push rule is present' do
        let_it_be(:global_push_rule) do
          create(:push_rule_sample, organization: project.organization, commit_message_regex: 'Test')
        end
        ...
        it 'creates project push rule from global push rule' do
          ...
          expect(project.reload.project_setting.push_rule).to be_a(PushRule)
      ...
      # Read behavior - depends on read_project_push_rules FF
      context 'with read_project_push_rules feature flag enabled' do
        it 'assigns a ProjectPushRule' do
          get :show, params: { namespace_id: project.namespace, project_id: project }

          expect(assigns(:push_rule)).to be_a(ProjectPushRule)
          expect(assigns(:push_rule)).to be_persisted
        end

        context 'when rendering views' do
          render_views

          it 'renders the push rule form with the correct URL' do
            get :show, params: { namespace_id: project.namespace, project_id: project }

            push_rule = assigns(:push_rule)

            expect(response.body).to include("action=\"#{project_push_rule_path(project, push_rule)}\"")
          end
        end
      end

      context 'with read_project_push_rules feature flag disabled' do
        before do
          stub_feature_flags(read_project_push_rules: false)
        end

        it 'assigns a PushRule' do
          get :show, params: { namespace_id: project.namespace, project_id: project }
