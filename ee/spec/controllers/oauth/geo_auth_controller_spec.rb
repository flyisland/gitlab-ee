...
RSpec.describe Oauth::GeoAuthController, :geo, feature_category: :geo_replication do
  ...
  describe 'GET callback' do
    ...
    context 'redirection' do
      before do
        allow_next_instance_of(Gitlab::Geo::Oauth::Session) do |instance|
          allow(instance).to receive_messages(get_token: 'token', authenticate: user.attributes)
        end
      end
    ...
    context 'non-existent remote user' do
      ...
      before do
        allow_next_instance_of(Gitlab::Geo::Oauth::Session) do |instance|
          allow(instance).to receive_messages(get_token: 'token', authenticate: nil)
        end
      end
    ...
    context 'non-existent local user' do
      ...
      before do
        allow_next_instance_of(Gitlab::Geo::Oauth::Session) do |instance|
          allow(instance).to receive_messages(get_token: 'token', authenticate: { id: non_existing_record_id })
        end
      end
