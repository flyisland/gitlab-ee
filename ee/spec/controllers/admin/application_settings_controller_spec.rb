...
RSpec.describe Admin::ApplicationSettingsController, feature_category: :settings do
  ...
  describe '#search', feature_category: :global_search do
    ...
    describe 'semantic search settings', :aggregate_failures do
      describe 'semantic search unavailable scenarios' do
        using RSpec::Parameterized::TableSyntax

        where(:semantic_search_available, :gitlab_selects_model, :user_selects_model) do
          false | false | false
          false | true  | false
          false | false | true
          true  | false | false
        ...
        with_them do
          before do
            ...
            allow(::Ai::ActiveContext).to receive_messages(
              gitlab_selects_embedding_model?: gitlab_selects_model,
              user_can_select_embedding_model?: user_selects_model
            )
          end

          it 'assigns semantic_search_available=false' do
            get :search

            expect(response).to have_gitlab_http_status(:ok)
            expect(assigns(:semantic_search_available)).to be(false)
          end
        end
      end
      ...
      context 'when semantic search is available' do
        ...
        it 'assigns semantic_search_available=true' do
          get :search

          expect(response).to have_gitlab_http_status(:ok)
          expect(assigns(:semantic_search_available)).to be(true)
        end
      end
    end
  end
