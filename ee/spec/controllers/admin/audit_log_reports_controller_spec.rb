...
RSpec.describe Admin::AuditLogReportsController, feature_category: :audit_events do
  ...
  describe 'GET index' do
    ...
    subject(:get_index) { get :index, params: params, as: :csv }
    ...
    context 'when user has access' do
      ...
      context 'when filtering by entity username' do
        let_it_be(:audited_user) { create(:user, username: 'audited-user') }

        let_it_be(:audited_user_event) do
          create(:audit_events_user_audit_event,
            target_user: audited_user,
            created_at: Time.zone.parse('2020-02-20T12:00:00Z'))
        end

        let_it_be(:unrelated_event) do
          create(:audit_events_user_audit_event,
        ...
        let(:params) do
          {
            entity_type: 'User',
            entity_username: audited_user.username,
            created_after: '2020-02-01',
            created_before: '2020-02-28'
          }
        end
        ...
        it 'exports only events for that user' do
          get_index

          # The controller streams a `CsvBuilder::Stream` enumerator. Rails 8 hands that
          # enumerator back from `response.body` instead of joining it into a String.
          csv = response.body_parts.join

          exported_event_ids = CSV.parse(csv, headers: true).map { |row| row['ID'] }

          expect(exported_event_ids).to contain_exactly(audited_user_event.id.to_s)
        end
      end
