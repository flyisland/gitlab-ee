...
RSpec.describe OperationsController, feature_category: :release_orchestration do
          ...
          context 'with environments pagination' do
            ...
            context 'pagination behaviour' do
              ...
              context 'with `per_page`' do
                it_behaves_like 'environments pagination', { per_page: 2 }, 2
              end

              context 'with `page=1`' do
                it_behaves_like 'environments pagination', { per_page: 2, page: 1 }, 2
              end

              context 'with `page=2`' do
                it_behaves_like 'environments pagination', { per_page: 2, page: 2 }, 1
              end
            end
  ...
  describe 'POST #create' do
    describe 'format json' do
      ...
      context 'without added projects' do
        let_it_be(:project_a) { create(:project, developers: user) }
        let_it_be(:project_b) { create(:project, developers: user) }
        ...
        describe 'ip restricted project' do
          ...
          let_it_be(:project_b) { create(:project, group: group, developers: user) }
  ...
  describe 'when projects belong to different organizations' do
    let_it_be(:other_organization) { create(:organization) }
    let_it_be(:project_in_current_org) { create(:project, organization: current_organization, developers: user) }
    let_it_be(:project_in_other_org) { create(:project, organization: other_organization, developers: user) }

    describe 'GET #index' do
      before do
        user.update!(ops_dashboard_projects: [project_in_current_org, project_in_other_org])
      end
      ...
      it 'returns only projects belonging to the current organization' do
        ...
        returned_ids = json_response['projects'].map { |p| p['id'] }
        expect(returned_ids).to contain_exactly(project_in_current_org.id)
        expect(returned_ids).not_to include(project_in_other_org.id)
      end
    end
    ...
    describe 'POST #create' do
      it 'only adds projects belonging to the current organization' do
        post :create, params: { project_ids: [project_in_current_org.id, project_in_other_org.id] }, format: :json

        expect(response).to have_gitlab_http_status(:ok)
        expect(json_response).to match_schema('dashboard/operations/add', dir: 'ee')
        expect(json_response['added']).to contain_exactly(project_in_current_org.id)
        expect(json_response['invalid']).to contain_exactly(project_in_other_org.id)

        user.reload
        expect(user.ops_dashboard_projects).to contain_exactly(project_in_current_org)
      end
    end
  end
