  ...
  describe '#as_json' do
    ...
    with_them do
      ...
      describe 'auto_severity_override' do
        ...
        it 'contains auto_severity_override when feature flag and license are available' do
          stub_licensed_features(security_orchestration_policies: true)
          stub_feature_flags(security_policies_severity_customize: true)

          expect(subject[:auto_severity_override]).to eq('critical')
        end

        it 'does not contain auto_severity_override when feature flag is disabled' do
          stub_licensed_features(security_orchestration_policies: true)
          stub_feature_flags(security_policies_severity_customize: false)
    ...
    context 'when a merge request link has an orphaned merge_request' do
      ...
      let_it_be(:finding) { create(:vulnerabilities_finding, vulnerability: vulnerability, project: project) }
