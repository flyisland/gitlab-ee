...
RSpec.describe Analytics::CycleAnalytics::ValueStreamErrorsSerializer, feature_category: :value_stream_management do
  let_it_be(:group) { create(:group) }
  let_it_be_with_reload(:value_stream) { create(:cycle_analytics_value_stream, name: 'name', namespace: group) }

  subject(:serialized_value_stream) { described_class.new(value_stream).as_json }
  ...
  it 'serializes error on value stream object' do
    ...
    expect(serialized_value_stream['name']).not_to be_empty
  end

  it 'does not contain stage errors' do
    expect(serialized_value_stream).not_to have_key('stages')
  end
  ...
  context 'when nested value stream stages are given' do
    ...
    it 'serializes error on value stream object' do
      ...
      stage_errors = serialized_value_stream['stages'][0]
