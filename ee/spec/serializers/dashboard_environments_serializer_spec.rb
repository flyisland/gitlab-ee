...
RSpec.describe DashboardEnvironmentsSerializer, feature_category: :continuous_delivery do
  describe '.represent' do
    ...
    it 'does not have N+1 queries when rendering multiple projects' do
      user = create(:user)

      project1 = create(:project)
      project1.add_developer(user)
      create(:environment, project: project1, state: :available)

      control = ActiveRecord::QueryRecorder.new do
        described_class.new(current_user: user).represent([project1])
      end

      project2 = create(:project)
      project2.add_developer(user)
      create(:environment, project: project2, state: :available)

      expect do
        described_class.new(current_user: user).represent([project1, project2])
