...
RSpec.describe Integrations::JiraSerializers::IssueDetailEntity do
  ...
  let_it_be(:jira_integration, freeze: false) do
    create(:jira_integration, project: project, url: 'http://jira.com', api_url: 'http://api.jira.com')
  end
  ...
  context 'without due date field' do
    let(:jira_issue) do
      double(
        summary: 'Title',
        renderedFields: {
    ...
    it 'returns nil due_date' do
      expect(subject).to include(due_date: nil)
    end
  end

  context 'without resolution date field' do
    let(:jira_issue) do
      double(
        summary: 'Title',
        renderedFields: {
    ...
    it "returns 'opened' state" do
      expect(subject).to include(state: 'opened')
    end

    it 'returns nil closed_at' do
      expect(subject).to include(closed_at: nil)
    end
  end
end
