...
RSpec.describe Ci::JobAiEntity, feature_category: :ai_abstraction_layer do
  ...
  let_it_be(:project) { create(:project, :public) } # rubocop:disable RSpec/FactoryBot/AvoidCreate -- we need to create it
  ...
  describe 'job_log authorization' do
    context 'when the user cannot read the build trace' do
      before do
        allow(Ability).to receive(:allowed?).and_call_original
        allow(Ability).to receive(:allowed?).with(user, :read_build_trace, build).and_return(false)
      end

      it 'does not expose job_log' do
        expect(basic_entity).not_to have_key(:job_log)
      end
    end

    context 'when the user can read the build trace' do
      before do
        allow(Ability).to receive(:allowed?).and_call_original
        allow(Ability).to receive(:allowed?).with(user, :read_build_trace, build).and_return(true)
      end

      it 'exposes job_log' do
        expect(basic_entity[:job_log]).to eq(build.trace.raw)
      end
    end
  end
end
