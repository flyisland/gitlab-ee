...
RSpec.describe GitlabSchema.types['Group'], feature_category: :groups_and_projects do
  ...
  describe '#knowledge_graph_enabled' do
    ...
    context 'when feature flag is enabled and namespace is enabled' do
      before do
      ...
      it 'returns true' do
        expect(result.dig('data', 'group', 'knowledgeGraphEnabled')).to be(true)
      end
    end

    context 'when feature flag is enabled but namespace is not enabled' do
      before do
        stub_feature_flags(knowledge_graph: true)
      end

      it 'returns false' do
        expect(result.dig('data', 'group', 'knowledgeGraphEnabled')).to be(false)
      end
    end
