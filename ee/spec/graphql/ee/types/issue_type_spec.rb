  ...
  context 'N+1 queries' do
    ...
    shared_examples 'avoids N+1 queries on blocked' do
      specify :request_store do
        ...
        # TODO: Remove threshold once N+1 from configurable work item types is resolved
        # Extra queries per new project namespace: custom_types (1) + root_ancestor (1) + work_item_settings (1)
        expect { GitlabSchema.execute(query, context: { current_user: user }) }
          .not_to exceed_query_limit(control).with_threshold(18)
      end
    end
