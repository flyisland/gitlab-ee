...
RSpec.describe Resolvers::EpicsResolver, feature_category: :portfolio_management do
  ...
  context "with a group" do
    ...
    let_it_be(:epic1)   { create(:epic, group: group, author: current_user, state: :closed, title: 'first created', created_at: 3.days.ago, updated_at: 2.days.ago, start_date: 10.days.ago, end_date: 10.days.from_now) }
    ...
    describe '#resolve' do
      ...
      context 'within timeframe' do
        let_it_be(:epic1) { create(:epic, group: group, author: current_user, state: :closed, start_date: "2019-08-13", end_date: "2019-08-20") }
        let_it_be(:epic2) { create(:epic, group: group, author: current_user, state: :closed, start_date: "2019-08-13", end_date: "2019-08-21") }
      ...
      it_behaves_like 'graphql query for searching issuables' do
        ...
        let_it_be(:issuable3) { create(:epic, group: group, author: current_user, title: 'third', description: 'text 2') }
        let_it_be_with_reload(:issuable4) { create(:epic, group: group, author: current_user) }
      ...
      context 'with author_username' do
        it 'filters epics by author' do
          ...
          create(:epic, group: group, author: current_user)
      ...
      context 'with label_name' do
        it 'filters epics by labels' do
          ...
          epic_1 = create(:labeled_epic, group: group, author: current_user, labels: [label_1, label_2])
          create(:labeled_epic, group: group, author: current_user, labels: [label_1])
          create(:labeled_epic, group: group, author: current_user)
      ...
      context 'with sort' do
        let_it_be(:epic3) { create(:epic, group: group, author: current_user, title: 'third', description: 'text 2', start_date: 30.days.ago, end_date: 30.days.from_now) }
        let_it_be(:epic4) { create(:epic, group: group, author: current_user, title: 'forth created', description: 'four', start_date: 40.days.ago, end_date: 40.days.from_now) }
      ...
      context 'with subgroups' do
        let_it_be(:sub_group) { create(:group, parent: group, developers: current_user) }
        let(:iids) { [epic1, epic2].map(&:iid) }
        let_it_be(:epic3) { create(:epic, group: sub_group, author: current_user, iid: epic1.iid) }
        let_it_be(:epic4) { create(:epic, group: sub_group, author: current_user, iid: epic2.iid) }
        ...
        it 'filters by milestones in subgroups' do
          subgroup_project = create(:project, group: sub_group, creator: current_user)
      ...
      context 'with partial iids' do
        ...
        let_it_be(:epic3) { create(:epic, group: group, author: current_user, iid: '1122') }
        let_it_be(:epic4) { create(:epic, group: group, author: current_user, iid: '132') }
        let_it_be(:epic5) { create(:epic, group: group, author: current_user, iid: '62') }
        let_it_be(:epic6) { create(:epic, group: other_group, author: current_user, iid: '11999') }
      ...
      context 'with `top_level_hierarchy_only` param set as `true`' do
        ...
        let_it_be(:child_epic) { create(:epic, group: group, author: current_user, title: 'child epic', parent: epic1) }
        let_it_be(:child_epic2) { create(:epic, group: group, author: current_user, title: 'child epic 2', parent: epic1) }
    ...
    context 'with negated filters' do
      ...
      let_it_be(:epic_1) { create(:labeled_epic, group: group, author: current_user, labels: [label]) }
      let_it_be(:epic_2) { create(:epic, group: group, author: author) }
      let_it_be(:epic_3) { create(:epic, group: group, author: current_user) }
