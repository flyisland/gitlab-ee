    ...
    context 'when user is present' do
      ...
      context 'when namespace_id is not provided' do
        ...
        context 'when credits are available' do
          before do
            allow(tanuki_bot_instance).to receive(:credits_available?).and_return(true)
          end
          ...
          it 'instantiates TanukiBot with nil group' do
            result

            expect(::Gitlab::Llm::TanukiBot).to have_received(:new).with(
              user: current_user,
              group: nil
            )
          end
        ...
        context 'when credits are not available' do
          before do
            allow(tanuki_bot_instance).to receive(:credits_available?).and_return(false)
          end
