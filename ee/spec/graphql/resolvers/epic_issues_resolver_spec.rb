...
RSpec.describe Resolvers::EpicIssuesResolver, feature_category: :portfolio_management do
  ...
  let_it_be(:issue6) { create(:issue, project: project1) }
  ...
  let_it_be(:epic_issue6) { create(:epic_issue, epic: epic2, issue: issue6, relative_position: nil) }
  ...
  describe '#resolve' do
    ...
    where(:epic, :user, :max_page_size, :has_next_page, :issues) do
      ...
      ref(:epic2) | ref(:developer) | 100 | false | lazy { [issue3, issue4, issue6] }
      ref(:epic2) | ref(:guest)     | 100 | false | lazy { [issue6] }
    end
    ...
    context 'when user does not have enough permissions' do
      ...
      it 'returns only public issues and does not leak private ones' do
        result = resolve_epic_issues(epic2, user, 100)

        expect(result.to_a).to eq([issue6])
      end

      it 'returns only public issues and not leak confidential ones' do
        result = resolve_epic_issues(epic1, guest, 100)

        expect(result.to_a).to match([issue1, issue5])
      end
    end
  end
