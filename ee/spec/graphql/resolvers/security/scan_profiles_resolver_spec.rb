  ...
  describe '#resolve' do
    ...
    context 'when security_scan_profiles feature is available' do
      ...
      context 'when user has permission' do
        ...
        context 'with security_scan_profiles_sast feature flag is disabled' do
          let(:group) { root_group }
          let(:args) { {} }

          before do
            stub_feature_flags(security_scan_profiles_sast: false)
          end

          it 'does not include the default sast scan profile' do
            profiles = resolve_scan_profiles

            expect(profiles.map(&:scan_type)).not_to include('sast')
            expect(profiles.map(&:scan_type)).to include('secret_detection')
          end
        end

        context 'with security_scan_profiles_dependency_scanning feature flag is disabled' do
          let(:group) { root_group }
          let(:args) { {} }

          before do
            stub_feature_flags(security_scan_profiles_dependency_scanning: false)
          end

          it 'does not include the default dependency_scanning scan profile' do
            profiles = resolve_scan_profiles

            expect(profiles.map(&:scan_type)).not_to include('dependency_scanning')
            expect(profiles.map(&:scan_type)).to include('secret_detection')
          end
        end
      end
