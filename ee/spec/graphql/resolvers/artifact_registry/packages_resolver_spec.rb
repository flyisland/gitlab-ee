...
RSpec.describe Resolvers::ArtifactRegistry::PackagesResolver, feature_category: :artifact_registry do
  ...
  let(:client) { instance_double(ArtifactRegistry::Client, packages: page) }
  ...
  # ee/spec/requests/api/graphql/organizations/artifact_registry_repository_packages_spec.rb.
  ...
  subject(:resolve_packages) do
    resolve(
      described_class,
      obj: presented_repository,
      args: args,
      # Mirrors the organization-rooted field's own mount. The field `resolve` builds inherits
      # no skip, and without one the union redactor authorizes each package node -- which has
      # no policy class.
  ...
  it 'reads the repository the field hangs off through the organization the presenter carries' do
    ...
    expect(client).to have_received(:packages)
  ...
  context 'when the repository holds npm packages' do
    ...
    it "sends the repository's own format segment rather than inferring one" do
      ...
      expect(client).to have_received(:packages)
        .with(slug: slug, repository_name: repository_name, format: 'npm', limit: schema_default_page_size)
    end
  end
  ...
  context 'with a requested page size below the maximum' do
    ...
    it 'forwards the requested size as the outbound limit' do
      ...
      expect(client).to have_received(:packages).with(hash_including(limit: 10))
    end
  end
  ...
  context 'with a requested page size above the maximum' do
    ...
    it 'caps the outbound limit at the maximum' do
      ...
      expect(client).to have_received(:packages).with(hash_including(limit: schema_default_page_size))
    end
  end
  ...
  context 'when paging forward' do
    ...
    it 'forwards the forward cursor' do
      ...
      expect(client).to have_received(:packages).with(hash_including(limit: 10, cursor: 'NEXT_CURSOR'))
    end
  end
  ...
  context 'when paging backward' do
    ...
    it 'forwards the backward cursor' do
      ...
      expect(client).to have_received(:packages).with(hash_including(limit: 5, cursor: 'PREV_CURSOR'))
    end
  end
  ...
  it 'sends no cursor on the first page' do
    ...
    expect(client).to have_received(:packages).with(hash_not_including(:cursor))
  end

  describe 'the resolved connection' do
    let(:maven_package) { ArtifactRegistry::MavenPackage.new('id' => 'a1b2c3d4-0000-0000-0000-000000000000') }
    let(:other_maven_package) { ArtifactRegistry::MavenPackage.new('id' => 'b2c3d4e5-0000-0000-0000-000000000000') }
    let(:page) do
      ArtifactRegistry::Page.new(
        nodes: [maven_package, other_maven_package],
    ...
    it 'carries the rows in order, and both Link-header cursors the client parsed',
      ...
      connection = resolve_packages

      expect(connection.nodes).to eq([maven_package, other_maven_package])
  ...
  context 'when the list read reports the repository missing' do
    ...
    it 'resolves the connection null rather than erroring' do
      expect(resolve_packages).to be_nil
    end
  end
  ...
  context 'when the repository holds images rather than packages' do
    ...
    with_them do
      it 'resolves the connection null without reaching Artifact Registry', :aggregate_failures do
        expect(resolve_packages).to be_nil
        expect(client).not_to have_received(:packages)
      end
    end
  end
