...
RSpec.describe Resolvers::Ci::CodeCoverageActivitiesResolver, feature_category: :code_testing do
  ...
  describe '#resolve' do
    ...
    let_it_be(:project_1) { create(:project, namespace: group, creator: current_user) }
    let_it_be(:project_2) { create(:project, namespace: group, creator: current_user) }

    let(:start_date) { 1.day.ago.to_date.to_s }

    context 'when group has projects with coverage' do
      let_it_be(:coverage_1) { create(:ci_daily_build_group_report_result, project: project_1, group: group) }
      let_it_be(:coverage_2) { create(:ci_daily_build_group_report_result, project: project_2, group: group) }
    ...
    context 'when coverage is included within start date' do
      let_it_be(:coverage_1) { create(:ci_daily_build_group_report_result, project: project_1, date: 1.week.ago, group: group) }
      let_it_be(:coverage_2) { create(:ci_daily_build_group_report_result, project: project_1, date: 1.week.ago, group_name: 'karma', group: group) }
    ...
    context 'when coverage is not included within start date' do
      let_it_be(:coverage_1) { create(:ci_daily_build_group_report_result, project: project_1, date: 1.week.ago) }
      let_it_be(:coverage_2) { create(:ci_daily_build_group_report_result, project: project_1, date: 2.weeks.ago) }
