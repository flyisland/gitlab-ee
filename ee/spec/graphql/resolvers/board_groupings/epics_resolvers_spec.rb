...
RSpec.describe Resolvers::BoardGroupings::EpicsResolver, feature_category: :portfolio_management do
  ...
  let_it_be(:label1, freeze: false) { create(:label, project: project, name: 'foo') }
  let_it_be(:label2, freeze: false) { create(:label, project: project, name: 'bar') }
  ...
  let_it_be(:epic1) { create(:epic, group: parent_group, title: 'Alpha') }
  let_it_be(:epic2) { create(:epic, :confidential, group: group, title: 'Gamma') }
  let_it_be(:epic3) { create(:epic, group: group, title: 'Beta') }
  ...
  shared_examples '#resolve' do
    ...
    context 'when sorting' do
      ...
      it 'sorts epics by title ascending' do
        result = resolve_board_epics(group_board, { sort: 'TITLE_ASC' })

        expect(result).to eq([epic1, epic3, epic2])
      end

      it 'sorts epics by title descending' do
        result = resolve_board_epics(group_board, { sort: 'TITLE_DESC' })

        expect(result).to eq([epic2, epic3, epic1])
      end

      it 'returns epics without a specific order when sort is not provided' do
        result = resolve_board_epics(group_board)
