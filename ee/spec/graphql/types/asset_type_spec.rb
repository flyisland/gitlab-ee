...
RSpec.describe GitlabSchema.types['AssetType'], feature_category: :release_orchestration do
  ...
  let_it_be_with_reload(:pipeline) { create(:ee_ci_pipeline, :with_api_fuzzing_report, project: project) }

  let_it_be_with_reload(:build_1) { create(:ci_build, :success, name: 'dependency_scanning', pipeline: pipeline) }
  let_it_be_with_reload(:build_2) { create(:ci_build, :success, name: 'sast', pipeline: pipeline) }
  let_it_be_with_reload(:artifact_ds) { create(:ee_ci_job_artifact, :dependency_scanning, job: build_1) }
  let_it_be_with_reload(:artifact_sast) { create(:ee_ci_job_artifact, :sast, job: build_2) }
  let_it_be_with_reload(:report_ds) do
