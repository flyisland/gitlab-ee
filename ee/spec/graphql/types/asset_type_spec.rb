...
RSpec.describe GitlabSchema.types['AssetType'], feature_category: :release_orchestration do
  ...
  let_it_be(:pipeline, freeze: false) { create(:ee_ci_pipeline, :with_api_fuzzing_report, project: project) }
  ...
  subject(:result) { GitlabSchema.execute(query, context: { current_user: user }).as_json }
  ...
  describe 'checking field contents' do
    ...
    it 'checks the contents of the assets field' do
      vulnerabilities = result.dig('data', 'project', 'pipeline', 'securityReportFindings', 'nodes')
