...
RSpec.describe Types::Ai::DuoWorkflows::WorkflowType, feature_category: :duo_agent_platform do
  describe 'field scopes' do
    ...
    it 'includes the correct scopes for the resourceIid field' do
      expect(described_class.fields['resourceIid'].instance_variable_get(:@scopes)).to include(:api, :read_api,
        :ai_features, :ai_workflows)
    end

    it 'includes the correct scopes for the resourceWebUrl field' do
      expect(described_class.fields['resourceWebUrl'].instance_variable_get(:@scopes)).to include(:api, :read_api,
        :ai_features, :ai_workflows)
    end
  end
end
