  ...
  describe '.coerce_input' do
    ...
    context 'when a JSON string is a valid JSON' do
      ...
      it 'coerces JSON string into a Hash' do
        expect(input).to eq({ "one" => { "key" => "value one" }, "two" => "value two" })
      end
    end
    ...
    context 'when a JSON string exceeds safe parsing limits' do
      let(:json_string) { "#{'{"a":' * 100}1#{'}'.* 100}" }

      it 'raises an exception for deeply nested JSON' do
        expect { input }.to raise_error(GraphQL::CoercionError).with_message(%r{Invalid JSON string})
      end
    end
  end
