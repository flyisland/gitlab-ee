...
RSpec.describe GitlabSchema.types['GroupSecretsManager'], feature_category: :secrets_management do
  include GraphqlHelpers

  it { expect(described_class).to require_graphql_authorizations(:read_group_secrets_manager_status) }

  it 'has the expected fields' do
    expect(described_class).to have_graphql_fields(:group, :status, :read_only, :user_permissions)
  end
