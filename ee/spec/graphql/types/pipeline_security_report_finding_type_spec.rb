...
RSpec.describe GitlabSchema.types['PipelineSecurityReportFinding'], feature_category: :vulnerability_management do
  ...
  let_it_be_with_reload(:project) { create(:project, id: 12346) }
  let_it_be_with_reload(:user) { create(:user) }
  let_it_be_with_reload(:pipeline) { create(:ci_pipeline, :with_sast_report, project: project) }

  let_it_be_with_reload(:sast_build) { create(:ci_build, :success, name: 'sast', pipeline: pipeline) }
  let_it_be_with_reload(:sast_artifact) { create(:ee_ci_job_artifact, :sast, job: sast_build) }
  let_it_be_with_reload(:sast_report) { create(:ci_reports_security_report, type: :sast, pipeline: pipeline) }
  let_it_be_with_reload(:sast_scan) do
    create(:security_scan, :latest_successful, scan_type: :sast, build: sast_artifact.job)
  end

  let_it_be(:sast_findings) { create_findings(sast_scan, sast_report, sast_artifact) }

  let_it_be_with_reload(:dep_scan_build) do
    create(:ci_build, :success, name: 'dependency_scanning', pipeline: pipeline)
  end

  let_it_be_with_reload(:dep_scan_artifact) do
    create(:ee_ci_job_artifact, :dependency_scanning_remediation, job: dep_scan_build)
  end

  let_it_be_with_reload(:dep_scan_report) do
    create(:ci_reports_security_report, type: :dependency_scanning, pipeline: pipeline)
  end

  let_it_be_with_reload(:dep_scan_scan) do
    create(:security_scan, :latest_successful, scan_type: :dependency_scanning, build: dep_scan_artifact.job)
  end

  # `freeze: false` is required here: this `let_it_be` subject is an Array
  # of security findings whose elements are mutated (`finding_data=`/`save!`);
  # freezing raises FrozenError and reload/refind are no-ops on an Array.
  # Keep as-is (see gitlab-org/gitlab#602925).
  let_it_be(:dep_scan_findings, freeze: false) { create_findings(dep_scan_scan, dep_scan_report, dep_scan_artifact) }
  ...
  describe 'field scopes' do
    ...
    expected_ai_workflow_fields.each do |field_name|
      it "includes :ai_workflows scope for #{field_name}" do
        expect(described_class.fields[field_name].instance_variable_get(:@scopes))
          .to include(*%i[api read_api ai_workflows])
      end
    end
  end
  ...
  describe 'vulnerability' do
    ...
    context 'when the security finding has a related vulnerability' do
      let_it_be_with_reload(:vulnerability) { create(:vulnerability, :with_issue_links, project: project) }
  ...
  describe 'issue_links' do
    ...
    context 'when the security finding has a related vulnerability' do
      let_it_be_with_reload(:issue) do
        create(:issue, description: 'Vulnerability issue description', project: project)
      end
      ...
      let_it_be_with_reload(:issue_link) do
  ...
  describe 'merge_request' do
    ...
    context 'when a merge request link exists' do
      ...
      let_it_be_with_reload(:vulnerabilities) { create_list(:vulnerability, sast_findings.count, project: project) }
      let_it_be_with_reload(:merge_requests) do
        create_list(:merge_request, sast_findings.count, :unique_branches, source_project: project)
      end

      let_it_be_with_reload(:merge_request_links) do
      ...
      let_it_be_with_reload(:vulnerability_findings) do
  ...
  describe 'dismissal data' do
    ...
    context 'when the security finding has a related dismissal state transition' do
      ...
      let_it_be_with_reload(:sast_dismissal_transition) do
  ...
  describe 'severity_overrides' do
    ...
    context 'when the security finding has a related vulnerability with severity overrides' do
      let_it_be_with_reload(:vulnerability) { create(:vulnerability, :with_severity_override, project: project) }
      let_it_be_with_reload(:vulnerability_finding) do
        create(:vulnerabilities_finding, project: project, vulnerability: vulnerability, uuid: sast_findings.first.uuid)
      end
