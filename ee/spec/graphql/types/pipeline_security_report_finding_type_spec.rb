...
RSpec.describe GitlabSchema.types['PipelineSecurityReportFinding'], feature_category: :vulnerability_management do
  ...
  let_it_be(:project, freeze: false) { create(:project, id: 12346) }
  let_it_be(:user, freeze: false) { create(:user) }
  let_it_be(:pipeline, freeze: false) { create(:ci_pipeline, :with_sast_report, project: project) }

  let_it_be(:sast_build, freeze: false) { create(:ci_build, :success, name: 'sast', pipeline: pipeline) }
  let_it_be(:sast_artifact, freeze: false) { create(:ee_ci_job_artifact, :sast, job: sast_build) }
  let_it_be(:sast_report, freeze: false) { create(:ci_reports_security_report, type: :sast, pipeline: pipeline) }
  let_it_be(:sast_scan, freeze: false) do
    create(:security_scan, :latest_successful, scan_type: :sast, build: sast_artifact.job)
  end

  let_it_be(:sast_findings, freeze: false) { create_findings(sast_scan, sast_report, sast_artifact) }

  let_it_be(:dep_scan_build, freeze: false) do
    create(:ci_build, :success, name: 'dependency_scanning', pipeline: pipeline)
  end

  let_it_be(:dep_scan_artifact, freeze: false) do
    create(:ee_ci_job_artifact, :dependency_scanning_remediation, job: dep_scan_build)
  end

  let_it_be(:dep_scan_report, freeze: false) do
    create(:ci_reports_security_report, type: :dependency_scanning, pipeline: pipeline)
  end

  let_it_be(:dep_scan_scan, freeze: false) do
  ...
  describe 'vulnerability' do
    ...
    context 'when the security finding has a related vulnerability' do
      let_it_be(:vulnerability, freeze: false) { create(:vulnerability, :with_issue_links, project: project) }
      let_it_be(:vulnerability_finding, freeze: false) do
  ...
  describe 'issue_links' do
    ...
    context 'when the security finding has a related vulnerability' do
      ...
      let_it_be(:issue_link, freeze: false) do
  ...
  describe 'merge_request' do
    ...
    context 'when a merge request link exists' do
      ...
      let_it_be(:vulnerabilities, freeze: false) { create_list(:vulnerability, sast_findings.count, project: project) }
      let_it_be(:merge_requests, freeze: false) do
        create_list(:merge_request, sast_findings.count, :unique_branches, source_project: project)
      end

      let_it_be(:merge_request_links, freeze: false) do
      ...
      let_it_be(:vulnerability_findings, freeze: false) do
  ...
  describe 'dismissal data' do
    ...
    context 'when the security finding has a related dismissal state transition' do
      let_it_be(:sast_vulnerability, freeze: false) do
        create_vulnerability_from_security_finding(project, sast_findings.first)
      end

      let_it_be(:sast_dismissal_transition, freeze: false) do
  ...
  describe 'severity_overrides' do
    ...
    context 'when the security finding has a related vulnerability with severity overrides' do
      let_it_be(:vulnerability, freeze: false) { create(:vulnerability, :with_severity_override, project: project) }
      let_it_be(:vulnerability_finding, freeze: false) do
        create(:vulnerabilities_finding, project: project, vulnerability: vulnerability, uuid: sast_findings.first.uuid)
      end
