...
RSpec.describe Mutations::Security::Finding::Dismiss, feature_category: :vulnerability_management do
  ...
  describe '#resolve' do
    ...
    let(:mutated_finding_uuid) { resolve_result[:uuid] }
    let(:mutated_finding) { resolve_result[:security_finding] }

    subject(:resolve_result) do
      mutation.resolve(uuid: finding_uuid, comment: comment, dismissal_reason: 'used_in_tests')
    end
    ...
    context 'when the user has permission to dismiss the security finding' do
      ...
      context 'when user does not have access to the project' do
        it 'raises an error' do
          expect { resolve_result }.to raise_error(Gitlab::Graphql::Errors::ResourceNotAvailable)
        end
      end

      context 'when no uuid is provided' do
        subject(:resolve_result) { mutation.resolve(comment: comment, dismissal_reason: 'used_in_tests') }

        it 'raises an error' do
          expect { resolve_result }.to raise_error(ArgumentError)
        end
      end
      ...
      context 'when the user has access to the project' do
        ...
        context 'when the dismissal is successful' do
          it 'returns the dismissed security finding uuid' do
            ...
            expect(resolve_result[:errors]).to be_empty
          end
        ...
        context 'when the dismissal fails' do
          ...
          it 'raises an error and no uuid is returned' do
            ...
            expect(resolve_result[:errors]).to match_array(['error'])
          end
