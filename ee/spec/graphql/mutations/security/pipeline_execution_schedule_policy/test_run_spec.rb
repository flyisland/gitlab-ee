...
RSpec.describe Mutations::Security::PipelineExecutionSchedulePolicy::TestRun,
  ...
  it 'has expected fields' do
    expect(described_class).to have_graphql_fields(:test_run, :pipeline, :errors, :client_mutation_id)
  end
  ...
  describe '#resolve' do
    ...
    shared_examples_for 'returns the test run and pipeline' do
      it 'returns the test run and pipeline', :aggregate_failures do
    ...
    context 'when policy configuration is inherited from group' do
      ...
      context 'when user has developer access to inherited policy management project' do
        ...
        let(:test_run) { build(:security_pipeline_execution_policy_test_run, project: group_project) }
        let(:pipeline) { test_run.pipeline }
        let(:service_result) do
          ServiceResponse.success(payload: { test_run: test_run, pipeline: pipeline })
        end
        ...
        it_behaves_like 'returns the test run and pipeline'
      end
    ...
    context 'when user has developer access to both target and policy projects' do
      ...
      context 'when the service fails' do
        ...
        it 'returns the error message and test run', :aggregate_failures do
          ...
          expect(result[:test_run]).to eq(test_run)
          expect(result[:pipeline]).to be_nil
