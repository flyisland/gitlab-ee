    ...
    context 'when on demand scan feature is enabled' do
      context 'when the user can run a dast scan' do
        ...
        it 'calls the dast_site_profile update service' do
          ...
          service_response = ServiceResponse.error(message: '')
          ...
          expect(service).to receive(:execute).with(service_params).and_return(service_response)

          result
        end

        it 'updates the dast_site_profile', :aggregate_failures do
          dast_site_profile = result[:id].find
        ...
        it 'returns the complete dast_site_profile' do
          expect(result[:dast_site_profile]).to eq(dast_site_profile)
        end
        ...
        context 'when variable creation fails' do
          it 'returns an error and the dast_site_profile' do
            ...
            mock_response = ServiceResponse.error(payload: create(:dast_site_profile), message: 'Oops')
            ...
            expect(result).to include(errors: ['Oops'])
          end
        end
      end
    end
