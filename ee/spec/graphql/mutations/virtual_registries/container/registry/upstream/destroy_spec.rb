...
RSpec.describe Mutations::VirtualRegistries::Container::Registry::Upstream::Destroy, feature_category: :virtual_registry do
  ...
  describe '#resolve' do
    ...
    context 'when the user has permissions to destroy a registry upstream' do
      before_all do
        group.add_owner(current_user)
      end

      it 'destroys a registry upstream' do
        expect do
          resolve
      ...
      it 'syncs the positions' do
        expect { resolve }.to change { registry_upstream2.reload.position }.by(-1)
      end
