...
RSpec.describe Mutations::VirtualRegistries::Packages::Maven::Registry::Upstream::Destroy,
  ...
  describe '#resolve' do
    ...
    context 'when the registry upstream does not exist' do
      let(:mutation_params) do
        {
    ...
    context 'when the user has permission to destroy a registry upstream' do
      before_all do
        group.add_maintainer(current_user)
      end

      it 'destroys the given registry upstream' do
        expect(resolver[:registry_upstream]).to be_destroyed
      end

      it 'syncs higher positions' do
        expect { resolver }.to change { registry_upstream2.reload.position }.from(2).to(1)
      end

      context 'when DestroyRegistryUpstreamService returns an error' do
        before do
          allow_next_instance_of(::VirtualRegistries::Packages::Maven::DestroyRegistryUpstreamService) do |service|
        ...
        it 'returns errors' do
          ...
          expect(resolver[:registry_upstream]).to be_nil
        end
      end
