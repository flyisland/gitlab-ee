...
RSpec.describe Mutations::Boards::Epics::Create, feature_category: :planning_views do
  ...
  subject(:mutation_result) { mutation.resolve(**epic_create_params) }
    ...
    context 'with proper permissions' do
      ...
      describe 'create epic via label list' do
        it 'creates a new epic' do
          expect { mutation_result }.to change { Epic.count }.by(1)
        end

        it 'creates and returns a new epic with that label', :aggregate_failures do
          new_epic = mutation_result[:epic]
        ...
        context 'when board not found' do
          ...
          it 'returns an error' do
            expect(mutation_result[:errors]).to include "Board not found"
          end
        end
        ...
        context 'when list not found' do
          ...
          it 'returns an error' do
            expect(mutation_result[:errors]).to include "List not found"
          end
        end
