...
RSpec.describe MergeRequests::DuoCodeReviewChatWorker, feature_category: :code_suggestions do
  ...
  describe '#perform' do
    ...
    context 'with classify_code_review_mention_intent feature flag enabled' do
      before do
        stub_feature_flags(classify_code_review_mention_intent: project)
      end
    ...
    context 'when classify_code_review_mention_intent feature flag is disabled' do
      before do
        stub_feature_flags(classify_code_review_mention_intent: false)
      end

      it 'skips intent detection' do
        expect(Gitlab::Llm::AiGateway::Completions::ClassifyCodeReviewMentionIntent).not_to receive(:new)
