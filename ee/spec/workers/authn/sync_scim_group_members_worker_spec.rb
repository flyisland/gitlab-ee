    ...
    context 'with add operation' do
      ...
      context 'with BSO (Block Seat Overages) enabled' do
        ...
        context 'without available seats' do
          ...
          it 'tracks minimal access provisioning for all groups linked to the SCIM group' do
            expect(worker).to receive(:track_minimal_access_provisioning).with(group, user1.id)
            expect(worker).to receive(:track_minimal_access_provisioning).with(another_group, user1.id)
        ...
        context 'with available seats' do
          ...
          it 'does not track minimal access provisioning for groups linked to the SCIM group' do
            expect(worker).not_to receive(:track_minimal_access_provisioning).with(group, user1.id)
            expect(worker).not_to receive(:track_minimal_access_provisioning).with(another_group, user1.id)
