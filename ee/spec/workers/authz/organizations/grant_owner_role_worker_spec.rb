...
RSpec.describe Authz::Organizations::GrantOwnerRoleWorker, feature_category: :system_access do
  ...
  describe '#perform' do
    ...
    it 'grants the organization_admin role, minting the token as the acting owner' do
      ...
      expect(client).to receive(:grant_roles) do |inputs, organization_uuid:, token:|
        ...
        expect(inputs).to contain_exactly(
          ...
          role_id: Authz::ArtifactRegistry::Roles.uuid_for(:organization_admin)
        )
    ...
    # ADR-021's Organization Administrator bootstrap tuple binds the owner to
    # the organization itself, not to an Artifact Registry namespace, so it
    # exists whether or not AR has ever been activated for this organization.
    it 'grants the role even when Artifact Registry is not activated for the organization' do
