  ...
  describe '#perform', :sidekiq_inline do
    ...
    context 'when multiple epic ids are passed' do
      ...
      context 'when save! raises ActiveRecord::RecordInvalid' do
        before do
          allow(epic).to receive(:save!).and_raise(ActiveRecord::RecordInvalid.new(epic))
          allow(::Epic).to receive(:id_in).and_return(::Epic.where(id: epic_ids))
          allow(::Epic.id_in(epic_ids)).to receive(:find_each).and_yield(epic)
        end
        ...
        it 'logs a validation error' do
          expect(Sidekiq.logger).to receive(:error).with(
            hash_including(
              'epic_id' => epic.id,
              'message' => a_string_including('skipping cache update, validation failed')
            )
          )
