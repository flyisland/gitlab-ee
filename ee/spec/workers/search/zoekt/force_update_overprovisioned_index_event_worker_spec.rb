...
RSpec.describe Search::Zoekt::ForceUpdateOverprovisionedIndexEventWorker, feature_category: :global_search do
  ...
  it_behaves_like 'an idempotent worker', :freeze_time do
    context 'when FF zoekt_batch_update_index_storage_bytes is disabled' do
      before do
        stub_feature_flags(zoekt_batch_update_index_storage_bytes: false)
      end
      ...
      it 'calls UPDATE on each zoekt index' do
        ...
        expect(update_queries.count).to eq(3),
          "Expected exactly 3 per-row UPDATE zoekt_indices, got #{update_queries.count}:\n#{update_queries.join("\n")}"
      end
