...
import { fetchIssue } from 'jh/integrations/ones/issues_show/api';
import OnesIssueSidebar from 'jh/integrations/ones/issues_show/components/sidebar/ones_issues_sidebar_root.vue';
...
export default {
  ...
  i18n: {
    defaultErrorMessage: s__(
      'JH|OnesIntegration|Failed to load ONES issue. View the issue in ONES, or reload the page.',
    ),
  },
};
</script>
...
<template>
  <div class="gl-mt-5">
    ...
    <template v-else>
      <external-issue-alert issue-tracker-name="Ones" :issue-url="issuesListPath" />
      ...
      <issuable-show :issuable="issue" :enable-edit="false" :status-icon="statusIcon">
        ...
        <template #right-sidebar-items>
          <ones-issue-sidebar :issue="issue" />
        </template>
        ...
        <template #discussion>
          ...
          >
            ...
            <template #badges>
              <gl-badge v-gl-tooltip="{ title: s__('JH|OnesIntegration|This is a ONES user.') }">
                {{ s__('JH|OnesIntegration|ONES user') }}
              </gl-badge>
            </template>
          </note>
        </template>
