  ...
  computed: {
    header() {
      const planName = this.trialActive
        ? s__('JH|BillingPlans|a trial of GitLab Ultimate')
        : s__('JH|BillingPlans|GitLab Free');

      return sprintf(s__('JH|BillingPlans|Your group is on %{planName}'), { planName });
    },
    seats() {
...
<template>
  ...
  >
    ...
    <p v-else>
      {{
        s__(
          'JH|BillingPlans|For individuals working on personal projects and open source contributions.',
        )
      }}
    </p>
    <p class="gl-mt-auto">
      ...
      <span>{{ s__('JH|BillingPlans|Seats in use') }}</span>
    </p>
    ...
    <gl-button
      ...
      >{{ s__('JH|BillingPlans|Upgrade subscription') }}</gl-button
    >
    <gl-button
      v-else
      class="gl-self-start"
      ...
      >{{ s__('JH|BillingPlans|Manage seats') }}</gl-button
    >
  </div>
</template>
