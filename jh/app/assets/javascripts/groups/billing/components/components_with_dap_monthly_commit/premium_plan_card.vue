    ...
    >
      <template #header>
        <div class="gl-flex gl-justify-between">
          <span class="gl-heading-scale-600">{{ s__('JH|BillingPlans|Premium') }}</span>

          <span v-if="highlightPremium" class="gl-self-center">
            <gl-badge variant="info">{{ s__('JH|BillingPlans|Recommended') }}</gl-badge>
          </span>
        </div>
      </template>
      ...
      <template #default>
        <p>
          {{
            s__(
              'JH|BillingPlans|For scaling organizations seeking enhanced productivity and collaboration.',
            )
          }}
        </p>
      ...
      <template #footer>
        <gl-button
          ...
          >{{ s__('JH|BillingPlans|Upgrade to Premium') }}
        </gl-button>
    ...
    <div class="gl-border gl-rounded-b-xl gl-border-t-0 gl-p-6">
      ...
      >
        <li>
          ...
          <span>{{ s__('JH|BillingPlans|GitLab Duo Agent Platform') }}</span>
        </li>

        <li>
          <gl-icon name="check" variant="info" class="gl-mr-2" />
          <span>{{ s__('JH|BillingPlans|Unlimited licensed users') }}</span>
        </li>

        <li>
          <gl-icon name="check" variant="info" class="gl-mr-2" />
          <span>{{ s__('JH|BillingPlans|10,000 compute minutes per month') }}</span>
        </li>
      </ul>
