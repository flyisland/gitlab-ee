...
import MrWidgetGeoSecondaryNode from 'ee/vue_merge_request_widget/components/states/mr_widget_secondary_geo_node.vue';
import WidgetContainer from 'ee/vue_merge_request_widget/components/widget/app.vue';
import MrWidgetMonorepo from './components/monorepo_mr_widget.vue';
...
export default {
  components: {
    ...
    BlockingMergeRequestsReport: () =>
      import('ee/vue_merge_request_widget/components/blocking_merge_requests/blocking_merge_requests_report.vue'),
  },
  ...
  mixins: [reportsMixin],
  computed: {
    shouldRenderMonorepoWidget() {
      return gon?.jh_monorepo_enabled;
    },
  },
  watch: {
  ...
  methods: {
    ...
    getServiceEndpoints(store) {
      const base = CEWidgetOptions.methods.getServiceEndpoints.call(this, store);
...
<template>
  <div v-if="!loading" id="widget-state" class="mr-state-widget gl-mt-5">
    ...
    <mr-widget-monorepo v-if="shouldRenderMonorepoWidget" />
    <mr-widget-pipeline-container
