...
import { omit, set, cloneDeep, unset } from 'lodash-es';
...
import {
  ...
  JOB_RULES_WHEN,
  i18n,
} from 'jh/ci/pipeline_editor/components/job_assistant_drawer/constants';
import {
  removeEmptyObj,
  trimFields,
  validateEmptyValue,
  validateStartIn,
} from '~/ci/pipeline_editor/components/job_assistant_drawer/utils';
...
import JobTemplate from './job_template.vue';
...
export default {
  ...
  watch: {
    ...
    isVisible(newVal) {
      if (this.glFeatures.jobTemplate && newVal === true) {
        this.showJobTemplate();
      }
    },
    stage(value) {
      this.job.stage = value;
    },
  },
  created() {
    if (!this.glFeatures.jobTemplate) {
      return;
    }
    ...
    getJobTemplateList(id)
      ...
      .catch(() => {
        createAlert({ message: i18n.TEMPLATE_LIST_ERROR });
      })
      .finally(() => {
  ...
  methods: {
    showJobTemplate() {
      this.isJobTemplateVisible = true;
    },
    hideJobTemplate() {
      this.isJobTemplateVisible = false;
    },
    closeDrawer() {
      this.clearJob();
    ...
    updateJob(key, value) {
      const jobCopy = cloneDeep(this.job);
      if (value !== undefined) {
        set(jobCopy, key, value);
      } else {
        unset(jobCopy, key);
      }
...
<template>
  ...
  >
    ...
    <gl-accordion :header-level="3">
      <job-template
        v-if="isJobTemplateVisible && glFeatures.jobTemplate"
        :job-template-list="jobTemplateList"
        :loading="loadingTemplate"
        @hide="hideJobTemplate"
      />
      <template v-else>
        <job-setup-item
    ...
    <template #footer>
      <div v-if="!isJobTemplateVisible" class="gl-flex gl-justify-end">
        <gl-button
          category="primary"
          class="gl-mr-3"
