...
RSpec.describe 'Subscription flow for user creating group and project for paid plan',
  :js, :saas_registration, :with_current_organization, feature_category: :onboarding do
  where(:case_name, :sign_up_method, :expected_opt_in) do
    [
      ['with regular sign up', -> { subscription_regular_sign_up }, nil],
      ['with sso sign up', -> { sso_subscription_sign_up }, true]
    ]
  end
  ...
  with_them do
    it 'registers the user and redirects to interstitial page' do
      ...
      expect_to_create_lead(expected_opt_in)
