...
RSpec.describe "Internal Project Pages Access", feature_category: :pages do
  ...
  let_it_be(:project, reload: true) do
    create(:project, :internal, pages_access_level: ProjectFeature::ENABLED, namespace: group)
  end
  ...
  describe "GET /projects/:id/pages_access", :saas do
    context 'with public access disabled' do
      before do
        allow(::Gitlab::Pages).to receive(:access_control_is_forced?).and_return(true)
      end

      context 'with access depends on the level' do
        where(:pages_access_level, :with_user, :admin_mode, :expected_result) do
          ...
          ProjectFeature::ENABLED    |   "admin"     | false |  403
          ...
          ProjectFeature::ENABLED    |   "user"      | false |  403
