...
RSpec.shared_examples 'validate dictionary' do |all_objects, directory_path, required_fields|
  ...
  # rubocop:disable Naming/HeredocDelimiterNaming -- EOM is the standard
...
RSpec.describe 'jh database documentation', feature_category: :database do
  context 'for views' do
    database_base_models = Gitlab::Database.database_base_models.select { |k, _| k == 'jh' }
    views = database_base_models.flat_map { |_, m| m.connection.views }.sort.uniq
    directory_path = File.join('jh', 'db', 'docs', 'views')
  ...
  context 'for tables' do
    ...
    directory_path = File.join('jh', 'db', 'docs')
