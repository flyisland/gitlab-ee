...
RSpec.describe 'Import/Export model configuration' do
  ...
  let(:all_models_hash) do
    ...
    YAML.safe_load_file(upstream_models_yml,
      aliases: true).deep_merge(YAML.safe_load_file(jh_models_yml, aliases: true)) do |_key, old_val, new_val|
  ...
  def setup_models
    model_names.index_with do |model_name|
      associations_for(relation_class_for_name(model_name)).reject { |m| m =~ /jh_.*/ } - ['project']
    end
  end
