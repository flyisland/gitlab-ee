...
RSpec.describe Gitlab::PackageMetadata::Connector::AliYun, feature_category: :software_composition_analysis do
  ...
  let(:storage) { instance_double(Aliyun::OSS::Client) }
  let(:bucket) { instance_double(Aliyun::OSS::Bucket, list_objects: all_files) }
  let(:all_files) { file_names.map { |name| instance_double(Aliyun::OSS::Object, key: name) } }
  let(:file_names) do
    [
  ...
  before do
    allow(Aliyun::OSS::Client).to receive(:new).and_return(storage)
    allow(storage).to receive(:get_bucket).with(sync_config.base_uri).and_return(bucket)
  end
  ...
  describe '.data_after(checkpoint)' do
    ...
    shared_examples_for 'a lazy file downloader' do
      let(:all_files) { [aliyun_object] }
      let(:aliyun_object) do
        instance_double(Aliyun::OSS::Object, key: "/1678352601/00000000.#{file_suffix}")
      end

      let(:content) { version_format == 'v1' ? "1\n2\n3" : "[\"1\"]\n[\"2\"]\n[\"3\"]" }
      ...
      it 'downloads the gcp file only when iterating over data_file' do
        expect(bucket).to receive(:get_object).with(aliyun_object.key).and_yield(content)
