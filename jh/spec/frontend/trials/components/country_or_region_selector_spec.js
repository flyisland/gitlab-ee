...
import { COUNTRIES, STATES, COUNTRY_WITH_STATES, STATE } from './mock_data';
...
describe('CountryOrRegionSelector', () => {
  ...
  const findFormInput = (testId) => wrapper.findComponentByTestId(testId);
  ...
  describe('selection change', () => {
    it('emits the change event properly when country is changed', async () => {
      ...
      expect(wrapper.emitted('change')[0]).toStrictEqual([
        { country: 'CN', state: 'SH', stateRequired: true },
      ]);
    });
