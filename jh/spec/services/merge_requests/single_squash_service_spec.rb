...
# Ref: spec/services/merge_requests/squash_service_spec.rb
RSpec.describe ::MergeRequests::SingleSquashService, feature_category: :source_code_management do
  let(:service) { described_class.new(merge_request: merge_request, current_user: user, commit_message: nil) }
  let_it_be_with_reload(:project) { create(:project, :repository) }
  let_it_be_with_reload(:user) { project.first_owner }
  let_it_be_with_reload(:repository) { project.repository.raw }
  let(:log_error) { "Failed to squash merge request #{merge_request.to_reference(full: true)}:" }
  let(:squash_dir_path) do
    File.join(Gitlab.config.shared.path,
      'tmp/squash', repository.gl_repository, merge_request.id.to_s)
  end
  ...
  shared_examples 'the squash succeeds' do
    it 'returns the squashed commit SHA' do
      ...
      squash_commit = repository.commit(result[:squash_sha])
      expect(squash_commit.parents.size).to eq 1
      expect(squash_commit.parents.first.id).to eq repository.commit(merge_request.target_branch).id
    end
    ...
    context 'when there is a single commit in the merge request' do
      it 'still performs the squash' do
        expect(merge_request.target_project.repository).to receive(:squash_to_target_branch_head).and_return('sha')

        service.execute
      end
    end

    context 'with squashed commit' do
      ...
      context 'if a message was provided' do
        let(:service) do
          described_class.new(merge_request: merge_request, current_user: user, commit_message: message)
        end
  ...
  describe '#execute' do
      ...
      context 'with an error in Gitaly UserSquash RPC' do
        ...
        it 'logs the error' do
          expect(service).to receive(:log_error).with(
            exception: an_instance_of(Gitlab::Git::Repository::GitError), message: 'Failed to squash merge request')
    ...
    context 'when any other exception is thrown' do
      ...
      before do
        allow(merge_request.target_project.repository).to receive(:squash_to_target_branch_head).and_raise(exception)
      end

      it 'logs the error' do
        expect(service).to receive(:log_error)
          .with(exception: exception, message: 'Failed to squash merge request')
          .and_call_original
