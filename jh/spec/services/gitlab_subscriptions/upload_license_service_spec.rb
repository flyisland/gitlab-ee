...
RSpec.describe GitlabSubscriptions::UploadLicenseService, feature_category: :plan_provisioning do
  ...
  shared_examples 'unsuccessful license upload scenarios on jh' do
    context 'with an invalid license key' do
      let(:params) { { data: 'invalid_license_key' } }
      let(:message) { _('The license key is invalid. Make sure it is exactly as you received it from GitLab Inc.') }
  ...
  context 'when license key belongs to an offline cloud license' do
    ...
    include_examples 'unsuccessful license upload scenarios on jh'
    include_examples 'successful license upload scenarios'

    it_behaves_like 'call runner to handle the provision of add-ons'
  end

  # Special case for online cloud license on JH, since we need to enable the feature flag to upload the license
  context 'when license key belongs to an online cloud licens with feature flag enabled' do
    before do
      stub_feature_flags(jh_enable_upload_cloud_license: true)
    end

    let(:license_traits) { [:online] }
    let(:attributes) { {} }
    let(:gitlab_license) { build(:gitlab_license, *license_traits, attributes) }
