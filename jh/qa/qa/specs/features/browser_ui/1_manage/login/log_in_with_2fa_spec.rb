...
module QA
  RSpec.describe 'JH Manage', :smoke, :requires_admin, only: { subdomain: :staging } do
    describe '2FA' do
      let!(:owner_user) do
        create(:user, username: "owner_user_#{SecureRandom.hex(4)}", hard_delete_on_api_removal: true)
      end
      ...
      # fictitious mobile phone number
      let(:phone) { "+86152#{Array.new(8) { rand(0..9) }.join}" }

      before do
        developer_user.add_phone(phone)
      ...
      it(
        'allows enforcing 2FA via UI and logging in using phone with 2FA',
        testcase: 'https://jihulab.com/gitlab-cn/quality/testcases/-/quality/test_cases/2'
      ) do
        enforce_two_factor_authentication_on_group(group)

        otp_settings = enable_two_factor_authentication_for_user(developer_user)
        ...
        Flow::Login.submit_2fa_code(otp_settings.fresh_otp)
      ...
      def enable_two_factor_authentication_for_user(user)
        Flow::Login.while_signed_in(as: user) do
          ...
          Page::Profile::PasswordAndAuth.perform do |two_fa_auth|
            otp_settings = QA::Support::OTP.new(two_fa_auth.otp_secret_content)

            two_fa_auth.set_pin_code(otp_settings.fresh_otp)
