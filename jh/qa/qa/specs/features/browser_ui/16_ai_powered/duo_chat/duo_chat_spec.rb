  ...
  RSpec.describe 'Ai-powered', feature_category: :duo_chat do
    describe 'Duo Chat' do
      ...
      context "when asking 'hi'" do
        context 'on Jihulab.com', :external_ai_provider,
          only: { pipeline: %i[staging staging-canary canary production] },
          quarantine: {
            issue: 'https://gitlab.com/gitlab-org/quality/test-failure-issues/-/issues/26494',
            type: :investigating
          } do
          include_examples 'Duo Chat', 'https://jihulab.com/gitlab-cn/quality/testcases/-/quality/test_cases/101'
        end

        context 'on Self-managed', :orchestrated, :ai_gateway,
          quarantine: {
          ...
          include_examples 'Duo Chat', 'https://jihulab.com/gitlab-cn/quality/testcases/-/quality/test_cases/102'
        end
      end
    end
  end
