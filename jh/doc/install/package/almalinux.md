...
title: 在 AlmaLinux 和 RHEL 兼容发行版上安装 Linux 软件包
---
...
> [!note]
> 参见[支持的平台](_index.md#supported-platforms)获取支持的发行版和架构的完整列表。
...
- 操作系统要求：
  ...
  - Oracle Linux 9
  - Oracle Linux 10
  - 任何与受支持的 Red Hat Enterprise Linux 版本兼容的发行版
- 请参阅[安装要求](../requirements.md)了解最低硬件要求。
- 在开始之前，请确保您已正确[设置 DNS](https://gitlab.cn/docs/omnibus/settings/dns)。
...
- 对于 `https://` URL，极狐GitLab 会自动[通过 Let's Encrypt 请求证书](https://gitlab.cn/docs/omnibus/settings/ssl/#enable-the-lets-encrypt-integration)，
  这需要入站 HTTP 访问和有效的主机名。您也可以使用[自己的证书](https://gitlab.cn/docs/omnibus/settings/ssl/#configure-https-manually)，
  或者直接使用 `http://`（不带 `s`）作为未加密的 URL。
- Linux 软件包和其他相关元数据文件存储在 Google Cloud Storage 中并提供服务。如果使用防火墙，您需要允许访问以下 URL 前缀：
...
1. 使用以下脚本添加极狐GitLab 仓库（您可以将脚本的 URL 粘贴到浏览器中，以查看其功能，然后再通过管道传递给 `bash`）：
...
> 如果在安装过程中未设置，您可以[稍后设置](https://gitlab.cn/docs/omnibus/settings/configuration/#configure-the-external-url-for-gitlab)。
...
极狐GitLab 安装完成后，访问您设置的 URL 并使用以下凭据登录：
...
您可以在安装前通过设置以下可选环境变量来自定义极狐GitLab 安装。**这些变量仅在首次安装时生效**，对后续的重新配置运行没有影响。对于现有安装，请使用 `/etc/gitlab/initial_root_password` 中的密码或[重置 root 密码](../../security/reset_user_password.md)。
...
如果极狐GitLab 在安装期间无法检测到有效的主机名，则不会自动运行重新配置。在这种情况下，请将任何所需的环境变量传递给您的第一个 `gitlab-ctl reconfigure` 命令。

> [!warning]
> 虽然您也可以通过设置 `gitlab_rails['initial_root_password']` 在 `/etc/gitlab/gitlab.rb` 中设置初始密码，但不建议这样做。
> 因为密码以明文形式存储，存在安全风险。如果您进行了此配置，请确保在安装后将其删除。
...
我们每月发送两次极狐GitLab 新闻，包括新功能、集成、文档以及开发团队的幕后故事。如需获取与错误和系统性能相关的关键安全更新，请注册我们的专用安全通讯。

> [!note]
> 如果您未选择加入安全通讯，您将不会收到安全警报。
