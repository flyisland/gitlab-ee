...
stage: 应用安全测试
group: 组件分析
info: 要确定与此页面关联的阶段/群组的技术写作人员，请参阅 <https://handbook.jihulab.com/handbook/product/ux/technical-writing/#assignments>
description: 学习如何配置依赖项扫描，发现项目依赖项中的漏洞，并通过实用的分步指导解决它们。
title: 教程：设置依赖项扫描
---
...
依赖项扫描可以在您开发和测试应用程序时，自动发现软件依赖项中的安全漏洞。例如，依赖项扫描能让您知道您的应用程序是否使用了已知存在漏洞的外部（开源）库。然后，您可以采取措施保护您的应用程序。

本教程向您展示如何创建一个示例脆弱应用程序，然后：

- 如何检测、分类和解决应用程序依赖项中的漏洞。
- 如何在合并请求中检测漏洞。

要设置依赖项扫描：

- [创建示例应用程序文件](#create-example-application-files)。
- [分类漏洞](#triage-the-vulnerabilities)。
- [解决高严重性漏洞](#resolve-the-high-severity-vulnerability)。
- [在合并请求中测试漏洞检测](#test-vulnerability-detection-in-a-merge-request)。

<a id="before-you-begin"></a>

## 准备工作

确保您已启用 Gitpod。Gitpod 是一个按需云开发环境。详情请参阅 [Gitpod](../integration/gitpod.md)。或者您可以使用自己的开发环境，这种情况下您需要安装 Yarn 和 Node.js。

<a id="create-example-application-files"></a>

## 创建示例应用程序文件

首先，在一个新项目中，创建用于配置流水线的文件，并添加可被扫描漏洞的依赖项。
...
1. 在 `main` 分支中创建以下文件。
   ...
   // fastify.register(require('fastify-redis'), { url: constants.redisUrl, /* 其他 Redis 选项 */ })
   ...
   使用 [Yarn 锁文件](#yarn-lock-file-content) 部分中显示的内容。
1. 前往 **构建** > **流水线**，确认最新的流水线已成功完成。

在流水线中，依赖项扫描会自动运行并检测到漏洞。

<a id="triage-the-vulnerabilities"></a>

## 分类漏洞

漏洞报告提供了有关漏洞的重要信息。您通常会根据组织的策略对漏洞进行分类。在本教程中，您将忽略中严重性漏洞，并只确认高严重性漏洞。

要分类漏洞：

1. 前往 **安全** > **漏洞报告**。
1. 通过选择每行中的复选框，选择每个中严重性漏洞。
1. 从 **设置状态** 下拉列表中选择 **忽略**。从 **忽略原因** 下拉列表中选择 **用于测试**，添加评论“用于测试”，然后选择 **更改状态**。

   中严重性漏洞将从视图中过滤掉，只剩下高严重性漏洞。
1. 选择 **高** 漏洞的描述。

   推荐的解决方案是升级 `fastify` 软件包。您通常会进一步调查，但在本教程中，您可以将此漏洞视为已确认。
1. 从 **状态** 下拉列表中选择 **确认**，然后选择 **更改状态**。

<a id="resolve-the-high-severity-vulnerability"></a>

## 解决高严重性漏洞

只剩下高严重性漏洞需要解决。从分类步骤中您知道需要升级 `fastify` 软件包。

要修复漏洞：

1. 在顶部栏中，选择 **搜索或跳转到**，然后找到您的项目。
1. 在右上角，选择 **代码** > **Gitpod**，并在新标签页中打开 Gitpod。
1. 如果系统提示，请选择 **使用极狐GitLab 继续**，然后选择 **授权**。
1. 在 **新工作区** 页面上，选择 **继续**。
1. 在 **终端** 窗格中，输入以下命令以创建新分支。

   ```plaintext
   git checkout -b update_packages main
   ```

1. 在 **终端** 窗格中，运行命令 `yarn upgrade --latest`。这会更新项目的依赖项和 `yarn.lock` 文件。
1. 在 **终端** 窗格中，运行以下命令以提交更改。这会触发一次 CI/CD 流水线。

   ```plaintext
   git add package.json yarn.lock
   git commit -m "更新软件包版本"
   git push --set-upstream origin update_packages
   ```

1. 切换回 极狐GitLab 浏览器标签页。
1. 前往 **代码** > **合并请求**，然后选择 **创建合并请求**。
1. 在 **新建合并请求** 页面上，滚动到底部并选择 **创建合并请求**。等待合并请求流水线完成。
1. 刷新页面，然后选择 **合并**。
1. 等待流水线成功完成。
1. 前往 **安全** > **漏洞报告**。
1. 选择 **高** 漏洞的描述。

   横幅确认该漏洞已在 `main` 分支中解决。您通常会通过查看 `yarn.lock` 文件中指定的 `fastify` 软件包版本来手动确认。在本教程中，您可以跳过验证步骤。
1. 在 **状态** 下拉列表中，选择 **已解决**，然后选择 **更改状态**。
1. 前往 **安全** > **漏洞报告**。

   您现在应该看到漏洞报告中未列出任何漏洞。

<a id="test-vulnerability-detection-in-a-merge-request"></a>

## 在合并请求中测试漏洞检测

现在您已经知道如何分类和解决漏洞。接下来，了解如何检测合并请求中新的潜在漏洞，添加一个已知有漏洞的依赖项。

添加新漏洞的步骤：

1. 切换到 Gitpod 标签页。如果已超时，请选择 **打开工作区**。
1. 在 **终端** 窗格中，运行以下命令以更新本地 `main` 分支：
...
1. 在 **终端** 窗格中，运行以下命令以创建新分支：

   ```plaintext
   git checkout -b add_dependency main
   ```

1. 在文件浏览器侧边栏中，选择 `package.json` 文件。
1. 将以下行添加到 `package.json` 文件的 `dependencies` 部分。

   ```json
   "axios": "0.21.0",
   ```

1. 在 **终端** 窗格中，运行以下命令安装已添加到 `package.json` 文件的依赖项。

   ```plaintext
   yarn install
   ```

1. 在 **终端** 窗格中，运行以下命令以提交更改。这会在 极狐GitLab 上触发一次 CI/CD 流水线。
...
等待合并请求流水线完成，然后刷新页面。合并请求安全小部件会警告新的潜在漏洞。这些漏洞仅存在于 `add_dependency` 分支，而不是 `main` 分支中。
...
<a id="yarn-lock-file-content"></a>

### Yarn 锁文件内容

```yaml
# 这是一个自动生成的文件，请勿直接编辑此文件。
# yarn 锁文件 v1
