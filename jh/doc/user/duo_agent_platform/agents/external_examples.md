...
- 在极狐GitLab 18.3 中引入[伴随一个功能标志](../../../administration/feature_flags/_index.md)，名为 `ai_flow_triggers`。默认启用。
- 在极狐GitLab 18.8 中[对私有化部署启用](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/218840)。
...
- `AI_FLOW_CONTEXT`：JSON 序列化的父对象，包括：
  - 在合并请求中，差异和评论（达到一定限制）
  - 在议题或史诗中，评论（达到一定限制）
- `$AI_FLOW_EVENT`：触发事件的类型（例如 `mention`）
- `$AI_FLOW_INPUT`：用户在合并请求、议题或史诗中作为评论输入的提示

## 与极狐GitLab 集成

以下代理与极狐GitLab 集成，并且在 JihuLab.com 上可用。
