...
有关点击演示，请参见[合规框架](https://gitlab.navattic.com/compliance).
...
- [创建一个新的合规框架](compliance_center/compliance_frameworks_report.md#create-a-new-compliance-framework)。
...
- [创建一个新的合规框架](compliance_center/compliance_projects_report.md#create-a-new-compliance-framework)。
...
- 在极狐GitLab 17.3 中[引入](https://gitlab.com/groups/gitlab-org/-/epics/13294)了分配多个合规框架。

{{< /history >}}

您可以将多个合规框架应用于项目，但不能将合规框架应用于个人命名空间中的项目。

要将合规框架应用于项目，请通过[合规项目报告](compliance_center/compliance_projects_report.md#apply-a-compliance-framework-to-projects-in-a-group)应用合规框架。
...
- 在极狐GitLab 15.6 中[引入](https://gitlab.com/gitlab-org/gitlab/-/issues/375036)。
...
要从[合规项目报告](compliance_center/compliance_projects_report.md)中设置为默认（或移除默认）：
...
要从[合规框架报告](compliance_center/compliance_frameworks_report.md)中设置为默认（或移除默认）：
...
- 在极狐GitLab 17.11 中[引入](https://gitlab.com/groups/gitlab-org/-/epics/16499)。

{{< /history >}}

下载现有合规框架作为 JSON 文件，并从 JSON 模板中上传新框架。

从[合规遵从模板](https://jihulab.com/gitlab-cn/software-supply-chain-security/compliance/engineering/compliance-adherence-templates)项目中获取一个 JSON 模板库。使用这些模板可以快速采用预定义的合规框架。
...
- 在极狐GitLab 17.11 中[引入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/186525)[用一个 flag](../../administration/feature_flags.md)命名为 `enable_standards_adherence_dashboard_v2`。默认情况下禁用。
...
每个控制都包含极狐GitLab 在计划或触发的扫描期间用于评估项目遵从性的逻辑。有关如何跟踪遵从性的更多详细信息，请参阅[合规状态报告](compliance_center/compliance_status_report.md)。
...
当[合规扫描](compliance_center/compliance_status_report.md#scan-timing-and-triggers)运行时，极狐GitLab 发送通知。用户或自动化工作流程可以从极狐GitLab 外部更新控制状态。

通过此集成，您可以与第三方工作流程工具（如 ServiceNow 或您选择的自定义工具）集成。第三方工具响应关联的状态。然后，该状态显示在[合规状态报告](compliance_center/compliance_status_report.md)中。
...
外部控制具有**异步**工作流程。每当[合规扫描](compliance_center/compliance_status_report.md#scan-timing-and-triggers)发出负载时。
