...
description: 部署一个支持最多 50,000 名用户或 1000 RPS 的极狐GitLab 私有化部署实例，涵盖高可用性、数据库复制和基础设施规格规划。
title: '参考架构：最高 1000 RPS 或 50,000 名用户'
---
...
本页介绍极狐GitLab 参考架构，该架构旨在应对 1000 请求每秒（RPS）的峰值负载，即基于真实数据，最多 50,000 名用户（包括手动和自动用户）的典型峰值负载。
...
- **高可用性**：是（[Praefect](#configure-praefect-postgresql) 需要第三方 PostgreSQL 解决方案来实现 HA）
- **云原生混合替代方案**：[是](#cloud-native-hybrid-reference-architecture-with-helm-charts-alternative)
- **不确定使用哪种参考架构**？[点击此处了解更多信息](_index.md#deciding-which-architecture-to-start-with)

| 服务                                  | 节点 | 配置           | GCP 示例<sup>1</sup> | AWS 示例<sup>1</sup> | Azure 示例<sup>1</sup> |
|------------------------------------------|-------|-------------------------|------------------|---------------|-----------|
| 外部负载均衡器<sup>4</sup>       | 1     | 16 vCPU, 14.4 GB 内存 | `n1-highcpu-16`  | `c5.4xlarge`  | `F16s v2` |
| Consul<sup>2</sup>                       | 3     | 2 vCPU, 1.8 GB 内存   | `n1-highcpu-2`   | `c5.large`    | `F2s v2`  |
| PostgreSQL<sup>2</sup>                   | 3     | 32 vCPU, 120 GB 内存  | `n1-standard-32` | `m5.8xlarge`  | `D32s v3` |
| PgBouncer<sup>2</sup>                    | 3     | 2 vCPU, 1.8 GB 内存   | `n1-highcpu-2`   | `c5.large`    | `F2s v2`  |
| 内部负载均衡器<sup>4</sup>       | 1     | 16 vCPU, 14.4 GB 内存 | `n1-highcpu-16`  | `c5.4xlarge`  | `F16s v2` |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 64 vCPU, 240 GB 内存  | `n1-standard-64` | `m5.16xlarge` | `D64s v3` |
...
| GitLab Rails<sup>8</sup>                 | 12    | 32 vCPU, 28.8 GB 内存 | `n1-highcpu-32`  | `c5.9xlarge`  | `F32s v2` |
...
3. 可以选择在信誉良好的第三方外部 PaaS Redis 解决方案上运行。有关更多信息，请参阅[提供您自己的 Redis 实例](#provide-your-own-redis-instances)。
   - Redis 主要是单线程的，增加 CPU 核心数不会带来显著收益。对于此规模的架构，强烈建议按指定方式配置独立的缓存和持久化实例，以实现最佳性能。
...
6. Gitaly 集群 (Praefect)提供了容错优势，但也带来了额外的设置和管理复杂性。
   在部署 Gitaly 集群 (Praefect)之前，请先查看现有的[技术限制和注意事项](../gitaly/praefect/_index.md#before-deploying-gitaly-cluster-praefect)。如果您想要分片 Gitaly，请使用上表中为 `Gitaly` 列出的相同规格。
...
8. 可以放置在自动扩缩组（ASG）中，因为该组件不存储任何[有状态数据](_index.md#autoscaling-of-stateful-nodes)。
   但是，通常更倾向于使用[云原生混合设置](#cloud-native-hybrid-reference-architecture-with-helm-charts-alternative)，因为某些组件
...
1000 RPS / 50k 用户参考架构旨在适应大多数常见工作流。GitLab 定期针对以下端点吞吐量目标进行冒烟和性能测试：
...
- 大量的[额外工作负载](_index.md#additional-workloads)
...
要设置极狐GitLab 及其组件以支持最多 1000 RPS 或 50,000 名用户：
...
1. [配置 Redis](#configure-redis)，用于存储会话数据、临时
   缓存信息和后台作业队列。
...
1. [配置主 GitLab Rails 应用](#configure-gitlab-rails)
   以运行 Puma、Workhorse、GitLab Shell，并处理所有前端
   请求（包括 UI、API 和基于 HTTP/SSH 的 Git）。
1. [配置 Prometheus](#configure-prometheus) 以监控您的极狐GitLab
   环境。
1. [配置对象存储](#configure-the-object-storage)
   用于共享数据对象。
1. [配置高级搜索](#configure-advanced-search)（可选）以实现更快、
   更高级的跨整个极狐GitLab 实例的代码搜索。
...
确保外部负载均衡器利用内置监控端点检查服务状态，并仅将流量路由到正常工作的服务。[就绪检查](../monitoring/health_check.md) 都需要在被检查的节点上进行[额外配置](../monitoring/ip_allowlist.md)，否则外部负载均衡器将无法连接。
...
- (*1*)：[Web 终端](../../ci/environments/_index.md#web-terminals-deprecated)支持要求
  ...
  传递 `Connection` 和 `Upgrade` 逐跳请求头。有关
  更多详细信息，请参阅 [Web 终端](../integration/terminal.md)集成指南。
- (*2*)：对端口 443 使用 HTTPS 协议时，您必须向负载均衡器添加 SSL
  证书。如果您希望在极狐GitLab 应用服务器上终止 SSL，请改用 TCP 协议。

如果您将 GitLab Pages 与自定义域名支持一起使用，则需要一些额外的端口配置。
GitLab Pages 需要单独的虚拟 IP 地址。配置 DNS 以将
`pages_external_url` 从 `/etc/gitlab/gitlab.rb` 指向新的虚拟 IP 地址。有关
更多信息，请参阅 [GitLab Pages 文档](../pages/_index.md)。
...
- (*2*)：GitLab Pages 的端口 443 应始终使用 TCP 协议。用户可以
  使用自定义 SSL 配置自定义域名，如果 SSL 在负载均衡器处终止，则无法实现这一点。

<a id="alternate-ssh-port"></a>

#### 备用 SSH 端口

某些组织有禁止开放 SSH 端口 22 的策略。在这种情况下，配置一个允许用户在端口 443 上使用 SSH 的备用 SSH 主机名可能会有所帮助。备用 SSH 主机名将需要一个新的虚拟 IP 地址，与之前记录的其它极狐GitLab HTTP 配置相比。
...
将您的负载均衡器配置为在端口 443 上以 `TCP` 协议而非
`HTTP(S)` 协议传递连接。这将把连接原样传递到应用节点的
NGINX 服务。NGINX 将拥有 SSL 证书并监听端口 443。

有关管理 SSL 证书和配置 NGINX 的详细信息，请参阅
...
将您的负载均衡器配置为使用 `HTTP(S)` 协议而非 `TCP` 协议。然后，负载均衡器将负责管理 SSL 证书并
终止 SSL。

由于负载均衡器与极狐GitLab 之间的通信将不安全，因此需要进行一些额外的配置。有关详细信息，请参阅
...
在此场景中，负载均衡器与 NGINX 之间的流量也将是安全的。由于连接全程都是安全的，因此无需为代理 SSL 添加配置。但是，必须向极狐GitLab 添加配置以配置 SSL 证书。有关管理 SSL 证书和配置 NGINX 的详细信息，请参阅
[HTTPS 文档](https://gitlab.cn/docs/omnibus/settings/ssl/)。
...
在多节点极狐GitLab 配置中，您需要一个内部负载均衡器来路由某些已配置的内部组件的流量，
...
1. [下载并安装](../../install/package/_index.md#supported-platforms)您选择的 Linux
   软件包。请务必仅添加极狐GitLab 软件包仓库，并为您的
   所选操作系统安装极狐GitLab。选择与您当前安装相同的版本
   和类型（基础版或企业版）。
...
1. 从您配置的第一个 Linux 软件包节点复制 `/etc/gitlab/gitlab-secrets.json` 文件，并添加或替换
   此服务器上的同名文件。如果这是您正在配置的第一个 Linux 软件包节点，则可以跳过此步骤。
1. [重新配置极狐GitLab](../restart_gitlab.md#reconfigure-a-linux-package-installation) 以使更改生效。
1. 对所有其他 Consul 节点重复上述步骤，并
   确保设置了正确的 IP。

当第三个 Consul 服务器预配完成时，会选举出一个 Consul 领导者。查看 Consul 日志 `sudo gitlab-ctl tail consul` 会显示
...
在本节中，将指导您配置一个高可用的 PostgreSQL 集群以与极狐GitLab 一起使用。
...
您可以使用
[第三方外部 PostgreSQL 服务](../postgresql/external.md)，而不是使用 Linux 软件包捆绑的 PostgreSQL、PgBouncer 和 Consul 服务发现组件。
...
- 至少三个 PostgreSQL 节点。
- 至少三个 Consul 服务器节点。
- 至少三个跟踪和处理主数据库读写操作的 PgBouncer 节点。
...
首先，确保在每个节点上[安装](../../install/package/_index.md#supported-platforms)
Linux 软件包。请务必仅添加极狐GitLab 软件包仓库并为您的所选操作系统安装极狐GitLab，但不要提供 `EXTERNAL_URL` 值。
...
1. 为 PostgreSQL 用户名/密码对生成密码哈希。这假设您将使用默认
   用户名 `gitlab`（推荐）。该命令将要求输入密码
   并确认。在下一步中使用此命令输出的值
   作为 `<postgresql_password_hash>` 的值：

   ```shell
   sudo gitlab-ctl pg-password-md5 gitlab
   ```

1. 为 PgBouncer 用户名/密码对生成密码哈希。这假设您将使用默认
   用户名 `pgbouncer`（推荐）。该命令将要求输入密码
   并确认。在下一步中使用此命令输出的值
   作为 `<pgbouncer_password_hash>` 的值：

   ```shell
   sudo gitlab-ctl pg-password-md5 pgbouncer
   ```

1. 为 PostgreSQL 复制用户名/密码对生成密码哈希。这假设您将使用默认
   用户名 `gitlab_replicator`（推荐）。该命令将要求输入密码
   并确认。在下一步中使用此命令输出的值
   作为 `<postgresql_replication_password_hash>` 的值：

   ```shell
   sudo gitlab-ctl pg-password-md5 gitlab_replicator
   ```

1. 为 Consul 数据库用户名/密码对生成密码哈希。这假设您将使用默认
   用户名 `gitlab-consul`（推荐）。该命令将要求输入密码
   并确认。在下一步中使用此命令输出的值
   作为 `<consul_password_hash>` 的值：
...
PostgreSQL 在 Patroni 管理其故障转移时，将默认使用 `pg_rewind` 来处理冲突。与大多数故障转移处理方法一样，这有很小的可能导致数据丢失。
...
如果任何节点的 'State' 列未显示 "running"，请先检查
...
1. [重新配置极狐GitLab](../restart_gitlab.md#reconfigure-a-linux-package-installation) 以使更改生效。
   ...
   在下一步之后再次运行 `reconfigure` 即可解决。
1. 创建一个 `.pgpass` 文件，以便 Consul 能够
   重新加载 PgBouncer。在提示时输入两次 PgBouncer 密码：
...
在可扩展环境中，可以采用 **主** x **副本** 的 [Redis](https://redis.io/)
拓扑，并使用 [Redis Sentinel](https://redis.io/docs/latest/operate/oss_and_stack/management/sentinel/) 服务来监视并自动
启动故障转移过程。

> [!note]
>
> - Redis 集群必须以 3 个或更多节点的奇数部署。
>   这是为了确保 Redis Sentinel 可以作为法定人数的一部分进行投票。这不
>   适用于外部配置 Redis 的情况，例如云提供商服务。
> - Redis 主要是单线程的，增加 CPU 核心数不会带来显著收益。
>   对于此规模的架构，强烈建议按指定方式配置独立的缓存和持久化实例，以实现最佳性能。
>   有关更多信息，请参阅[扩展文档](_index.md#scaling-an-environment)。

如果与 Sentinel 一起使用，Redis 需要身份验证。有关更多
信息，请参阅 [Redis 安全](https://redis.io/docs/latest/operate/rc/security/)文档。我们建议结合使用 Redis 密码和严格的
防火墙规则来保护您的 Redis 服务。
强烈建议您在将 Redis 与极狐GitLab 配置之前阅读 [Redis Sentinel](https://redis.io/docs/latest/operate/oss_and_stack/management/sentinel/) 文档，
以充分理解拓扑和架构。

Redis 设置的要求如下：

1. 所有 Redis 节点必须能够相互通信，并接受通过 Redis（`6379`）和 Sentinel（`26379`）端口的传入连接（除非您
   更改默认端口）。
1. 托管极狐GitLab 应用的服务器必须能够访问
   Redis 节点。
1. 使用防火墙保护节点免受外部网络
   （互联网）的访问。

在本节中，将指导您配置两个外部 Redis 集群以与极狐GitLab 一起使用。以下 IP 将用作示例：
...
- 应为此使用信誉良好的提供商或解决方案。[Google Memorystore](https://cloud.google.com/memorystore/docs/redis/memorystore-for-redis-overview) 和 [AWS ElastiCache](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/WhatIs.html) 已知可以很好地工作。
...
本节介绍如何安装和设置新的 Redis 缓存实例。

主节点和副本 Redis 节点都需要在
`redis['password']` 中定义相同的密码。在故障转移期间的任何时候，Sentinel 都可以重新配置
节点并将其状态从主节点更改为副本节点（反之亦然）。
...
1. [下载并安装](../../install/package/_index.md#supported-platforms)您选择的 Linux
   ...
   所选操作系统安装极狐GitLab。选择与您当前安装相同的版本
   和类型（基础版或企业版）。
1. 编辑 `/etc/gitlab/gitlab.rb`，并通过将 `redis_master_node` 替换为 `redis_replica_node` 来添加与上一节中主节点相同的内容：
   ...
   1. 从您配置的第一个 Linux 软件包
      节点复制 `/etc/gitlab/gitlab-secrets.json` 文件，并添加或替换此
      服务器上的同名文件。如果这是您正在配置的第一个 Linux 软件包节点，则
      可以跳过此步骤。

   1. [重新配置极狐GitLab](../restart_gitlab.md#reconfigure-a-linux-package-installation) 以使更改
      生效。
...
> [!warning]
> Gitaly 规格基于健康状况良好的使用模式和代码仓库大小的高百分位数。
> 但是，如果您有[大型单体代码仓库](_index.md#large-monorepos)（大于数 GB）或[额外工作负载](_index.md#additional-workloads)，这些可能会显著影响环境的性能，并且可能需要进一步调整。
...
- 负载均衡：将流量均匀分配到 Praefect 节点。您可以使用
  ...
  或用于高级配置的[服务发现 DNS](../gitaly/praefect/configure.md#service-discovery)。
  有关更多信息，请参阅 [Praefect 的负载均衡](#load-balancing-for-praefect)。

本节详细介绍了如何按顺序配置推荐的标准设置。对于更高级的设置，请参阅
...
您可以使用 TCP 负载均衡器或服务发现 DNS 将流量分配到 Praefect 节点。TCP 负载均衡器推荐用于大多数设置，因为它适用于所有部署场景。

<a id="tcp-load-balancer"></a>

#### TCP 负载均衡器

传统的 TCP 负载均衡器（例如 HAProxy 或 AWS ELB）将流量分配到各个 Praefect 节点。这种方法：
...
服务发现使用 DNS 获取 Praefect 节点地址，使客户端能够将请求均匀分配到所有可用节点。这种方法：
...
如果您希望实现高可用设置，Praefect 需要第三方 PostgreSQL 数据库。
...
Linux 软件包。请务必仅为所选操作系统添加极狐GitLab 软件包仓库并安装极狐GitLab，但不要提供 `EXTERNAL_URL` 值。

1. SSH 登录到 Praefect PostgreSQL 节点。
1. 创建一个用于 Praefect PostgreSQL 用户的强密码。请记下此密码，记为 `<praefect_postgresql_password>`。
...
1. 从您配置的第一个 Linux 软件包节点复制 `/etc/gitlab/gitlab-secrets.json` 文件，并在此服务器上添加或替换
   同名文件。如果这是您正在配置的第一个 Linux 软件包节点，则可以跳过此步骤。
1. [重新配置极狐GitLab](../restart_gitlab.md#reconfigure-a-linux-package-installation) 以使更改生效。
...
PostgreSQL HA 有许多第三方解决方案。所选解决方案必须满足以下条件才能与 Praefect 配合使用：

- 所有连接使用一个在故障转移时不会更改的静态 IP。
- 必须支持 [`LISTEN`](https://www.postgresql.org/docs/16/sql-listen.html) SQL 功能。

> [!note]
> 使用第三方设置时，可以将 Praefect 的数据库与主 [极狐GitLab](#provide-your-own-postgresql-instance) 数据库放在同一台服务器上，以方便起见，
> 除非您使用 Geo，因为 Geo 需要单独的数据库实例才能正确处理复制。
...
数据库设置完成后，遵循[配置后步骤](#praefect-postgresql-post-configuration)。

<a id="praefect-postgresql-post-configuration"></a>

#### Praefect PostgreSQL 配置后步骤

在 Praefect PostgreSQL 服务器设置完成后，您必须为 Praefect 配置要使用的用户和数据库。

我们建议用户命名为 `praefect`，数据库命名为 `praefect_production`，这些都可以在 PostgreSQL 中按标准方式配置。该用户的密码与您之前配置的 `<praefect_postgresql_password>` 相同。

以下是在 Linux 软件包 PostgreSQL 设置中的操作方式：

1. SSH 登录到 Praefect PostgreSQL 节点。
1. 使用管理权限连接到 PostgreSQL 服务器。
...
Praefect 是 Gitaly 集群 (Praefect)的路由器和事务管理器，所有到 Gitaly 的连接都通过它进行。
本节详细介绍如何配置它。
...
Gitaly 集群 (Praefect)节点通过 `virtual storage` 在 Praefect 中配置。每个存储都包含
组成集群的每个 Gitaly 节点的详细信息。每个存储还会被赋予一个名称，该名称在配置的多个地方使用。在本指南中，存储的名称将是
...
1. [下载并安装](../../install/package/_index.md#supported-platforms)您选择的 Linux
   软件包。请务必仅为所选操作系统添加极狐GitLab 软件包仓库并安装极狐GitLab。
...
1. 从您配置的第一个 Linux 软件包节点复制 `/etc/gitlab/gitlab-secrets.json` 文件，并在此服务器上添加或替换
   同名文件。如果这是您正在配置的第一个 Linux 软件包节点，则可以跳过此步骤。
1. Praefect 需要运行一些数据库迁移，与主极狐GitLab 应用程序非常相似。为此，
   您应该只选择一个 Praefect 节点来运行迁移，即 _部署节点_。该节点
   必须先于其他节点进行配置，如下所示：

   1. 在 `/etc/gitlab/gitlab.rb` 文件中，将 `praefect['auto_migrate']` 设置值从 `false` 更改为 `true`

   1. 为确保数据库迁移仅在重新配置期间运行，而不是在升级时自动运行，请运行：
...
组成集群的 [Gitaly](../gitaly/_index.md) 服务器节点具有
取决于数据和负载的要求。
...
Gitaly 服务器不得暴露于公共互联网，因为 Gitaly 上的网络流量默认情况下未加密。强烈建议使用防火墙来限制对 Gitaly 服务器的访问。另一种选择是
...
- 自带证书，因为这不是自动提供的。与每个 Praefect 服务器对应的证书
  必须安装在该 Praefect 服务器上。

此外，证书或其证书颁发机构的证书必须安装在所有 Gitaly 服务器以及所有与其通信的 Praefect 客户端上，遵循
[极狐GitLab 自定义证书配置](https://gitlab.cn/docs/omnibus/settings/ssl/#install-custom-public-certificates)中描述的过程（并在下面重复）。
...
1. 在 Praefect 客户端（包括每个 Gitaly 服务器）上，将证书
   或其证书颁发机构的证书复制到 `/etc/gitlab/trusted-certs`：
...
按照建议采用对象存储时，它还需要连接到[对象存储](#configure-the-object-storage)。
...
在配置其他极狐GitLab 功能（如容器镜像仓库、SAML 或 LDAP）时，
...
1. 从您配置的第一个 Rails 节点复制 SSH 主机密钥（所有名称格式为 `/etc/ssh/ssh_host_*_key*` 的文件），并
   在此服务器上添加或替换同名文件。这可以确保您的用户在访问负载均衡的 Rails 节点时不会遇到主机不匹配错误。如果这是您正在配置的第一个 Linux 软件包节点，
   则可以跳过此步骤。
1. 为确保数据库迁移仅在重新配置期间运行，而不是在升级时自动运行，请运行：
   ...
   只有单个指定节点应处理迁移，详见
   [GitLab Rails 配置后步骤](#gitlab-rails-post-configuration)部分。
1. [重新配置极狐GitLab](../restart_gitlab.md#reconfigure-a-linux-package-installation) 以使更改生效。
...
1. 编辑 `/etc/gitlab/gitlab.rb` 并使用以下配置。
   为保持各节点链接的一致性，应用程序服务器上的 `external_url`
...
当您在 `external_url` 中指定 `https` 时，如前面的示例所示，极狐GitLab 期望 SSL 证书位于 `/etc/gitlab/ssl/` 中。如果证书不存在，NGINX 将无法启动。有关更多信息，请参阅
[HTTPS 文档](https://gitlab.cn/docs/omnibus/settings/ssl/)。
...
对于数据对象，建议使用对象存储而不是 [NFS](../nfs.md)，并且在较大的设置中通常更好，因为对象存储通常性能更高、更可靠且可扩展性更强。有关更多信息，请参阅[基础设施和服务](_index.md#infrastructure-and-services)。
...
- [存储特定形式](../object_storage.md#configure-each-object-type-to-define-its-own-storage-connection-storage-specific-form)：每个对象定义自己的
  对象存储[连接和配置](../object_storage.md#configure-the-connection-settings)。

以下示例在可用时使用合并形式。

为每种数据类型使用单独的存储桶是极狐GitLab 的推荐方法。这可以确保极狐GitLab 存储的各种类型数据之间不会发生冲突。
...
极狐GitLab Runner 以块的形式返回作业日志，即使使用合并对象存储，Linux 软件包默认也会将其临时缓存在磁盘上的 `/var/opt/gitlab/gitlab-ci/builds` 中。使用默认配置时，此目录需要在任何 GitLab Rails 和 Sidekiq 节点上通过 NFS 共享。

虽然支持通过 NFS 共享作业日志，但可以通过启用[增量日志记录](../cicd/job_logs.md#incremental-logging)来避免使用 NFS 的要求（在未部署 NFS 节点时必需）。增量日志记录使用 Redis 而不是磁盘空间来临时缓存作业日志。
...
Elasticsearch 集群的设计和要求取决于您的具体
数据。有关如何在实例旁设置 Elasticsearch
集群的推荐最佳实践，请阅读如何
...
混合安装同时利用云原生和传统计算部署的优势。这样，无状态组件可以受益于云原生工作负载管理的优势，而有状态组件则部署在使用 Linux 软件包安装的计算 VM 中，以受益于更高的持久性。

有关设置说明（包括有关在 Kubernetes 和后端组件之间同步哪些极狐GitLab 密钥的指导），请参阅 Helm charts 的[高级配置](https://gitlab.cn/docs/charts/advanced/)
文档。

> [!note]
> 这是一种高级设置。众所周知，在 Kubernetes 中运行服务
> 非常复杂。仅当您对 Kubernetes 有扎实的
> 工作知识和经验时，才推荐使用此设置。本节的其余部分
> 假设您具备这些条件。
...
|----------------------|-------------------------|-----------------|--------------|
| Webservice           | 308 vCPU<br/>385 GB 内存（请求）<br/>539 GB 内存（限制） | 11 x `n1-standard-32` | 11 x `c5.9xlarge` |
...
- [Webservice](#webservice) 和 [Sidekiq](#sidekiq) 目标节点池总量仅针对极狐GitLab 组件给出。所选 Kubernetes 提供商的系统进程需要额外资源。给出的示例已考虑到这一点。
- [支持](#supporting)目标节点池总量是概括性给出的，用于容纳支持极狐GitLab 部署的多种资源以及您可能希望根据需求进行的任何额外部署。与其他节点池类似，所选 Kubernetes 提供商的系统进程也需要资源。给出的示例已考虑到这一点。
...
| PostgreSQL<sup>2</sup>                   | 3     | 32 vCPU，120 GB 内存 | `n1-standard-32` | `m5.8xlarge`  |
| PgBouncer<sup>2</sup>                    | 3     | 2 vCPU，1.8 GB 内存  | `n1-highcpu-2`   | `c5.large`    |
| 内部负载均衡器<sup>4</sup>       | 1     | 16 vCPU，14.4 GB 内存 | `n1-highcpu-16` | `c5.4xlarge`  |
...
| Gitaly<sup>6</sup><sup>7</sup>           | 3     | 64 vCPU，240 GB 内存 | `n1-standard-64` | `m5.16xlarge` |
...
<!-- Disable ordered list rule <https://github.com/DavidAnson/markdownlint/blob/main/doc/Rules.md#md029---ordered-list-item-prefix> -->
<!-- markdownlint-disable MD029 -->
1. 机器类型示例仅供说明之用。这些类型用于[验证和测试](_index.md#validation-and-test-results)，但并非作为规定性默认值。支持切换到满足所列要求的其他机器类型，包括可用的 ARM 变体。有关更多信息，请参阅[支持的机器类型](_index.md#supported-machine-types)。
2. 可以选择在信誉良好的第三方外部 PaaS PostgreSQL 解决方案上运行。有关更多信息，请参阅[提供您自己的 PostgreSQL 实例](#provide-your-own-postgresql-instance)。
3. 可以选择在信誉良好的第三方外部 PaaS Redis 解决方案上运行。有关更多信息，请参阅[提供您自己的 Redis 实例](#provide-your-own-redis-instances)。
   - Redis 主要是单线程的，增加 CPU 核心数不会带来显著收益。对于这种规模的架构，强烈建议按指定方式设置独立的缓存和持久化实例，以获得最佳性能。
4. 可以选择在信誉良好的第三方负载均衡服务（LB PaaS）上运行。有关更多信息，请参阅[基础设施和服务](_index.md#infrastructure-and-services)。
5. 应在信誉良好的云提供商或私有化部署解决方案上运行。有关更多信息，请参阅[配置对象存储](#configure-the-object-storage)。
6. Gitaly 集群 (Praefect)提供了容错的优势，但设置和管理也更为复杂。
   在部署 Gitaly 集群 (Praefect)之前，请查看现有的[技术限制和注意事项](../gitaly/praefect/_index.md#before-deploying-gitaly-cluster-praefect)。如果您想要分片式 Gitaly，请对 `Gitaly` 使用上表中列出的相同规格。
7. Gitaly 规格基于健康状况良好的使用模式和代码仓库大小的高百分位数。
   但是，如果您有[大型单体代码仓库](_index.md#large-monorepos)（大于数 GB）或[额外工作负载](_index.md#additional-workloads)，这些可能会显著影响 Git 和 Gitaly 的性能，并且很可能需要进一步调整。
<!-- markdownlint-enable MD029 -->

> [!note]
> 对于所有涉及配置实例的 PaaS 解决方案，建议在三个不同的可用区中至少实施三个节点，以符合弹性云架构实践。
...
对于 1000 RPS 或 50,000 用户，我们建议 Puma 工作进程总数约为 308 个，因此建议至少运行
77 个 Webservice Pod。
...
还建议将 Gateway API 或 Ingress 控制器 Pod 作为 DaemonSet 部署在 Webservice 节点上。这是为了让控制器能够与其所服务的 Webservice Pod 一起动态扩展，并利用较大机器类型通常具有的更高网络带宽。

这不是严格要求。Gateway API 或 Ingress 控制器 Pod 可以按需部署，只要它们有足够的资源来处理 Web 流量即可。
...
与之前记录的标准部署类似，此处使用了 14 个 Sidekiq 工作进程的初始目标。根据您的具体工作流程，可能需要额外的工作进程。
...
支持节点池旨在容纳所有不需要在 Webservice 和 Sidekiq 池上运行的支持部署。

这些辅助部署包括与云提供商实现相关的各种部署，以及
极狐GitLab 的辅助部署，例如 [GitLab Shell](https://gitlab.cn/docs/charts/charts/gitlab/gitlab-shell/)。

要进行任何额外部署（例如容器镜像仓库、Pages 或监控），请尽可能将这些部署在支持节点池中，而不是 Webservice 或 Sidekiq 池中。支持节点池的设计可容纳多个额外部署。但是，如果您的部署不适合给定的池，您可以相应地增加节点池。相反，如果您的用例中该池过度预配，则可以相应减少。

<a id="example-config-file"></a>

### 示例配置文件

针对 1000 RPS 或 50,000 用户参考架构配置的 GitLab Helm Charts 示例[可以在 Charts 项目中找到](https://jihulab.com/gitlab-cn/charts/gitlab/-/blob/master/examples/ref/50k.yaml)。
...
按照本指南操作后，您现在应该拥有一个全新的极狐GitLab 环境，并已相应配置核心功能。

您可能希望根据需求配置极狐GitLab 的其他可选功能。有关更多信息，请参阅[安装极狐GitLab 后的步骤](../../install/next_steps.md)。
