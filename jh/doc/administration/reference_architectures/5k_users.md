...
description: 使用此参考架构为 5,000 名用户部署极狐GitLab。配置高可用组件和支持服务。
title: '参考架构：最高 100 RPS 或 5,000 名用户'
---
...
本页介绍极狐GitLab 参考架构，该架构旨在满足峰值负载为每秒 100 个请求（RPS）的场景，即基于真实数据，最多 5,000 名用户（包括手动和自动用户）的典型峰值负载。
...
- **目标负载**：API：100 RPS，Web：10 RPS，Git（拉取）：10 RPS，Git（推送）：2 RPS
...
| PostgreSQL<sup>2</sup>                    | 3     | 4 vCPU, 15 GB 内存    | `n1-standard-4` | `m5.xlarge`  | `D4s v3` |
...
| Gitaly<sup>6</sup><sup>7</sup>            | 3     | 8 vCPU, 30 GB 内存    | `n1-standard-8` | `m5.2xlarge` | `D8s v3` |
...
| GitLab Rails<sup>8</sup>                  | 3     | 16 vCPU, 14.4 GB 内存 | `n1-highcpu-16` | `c5.4xlarge` | `F16s v2`|
...
4. 建议使用信誉良好的第三方负载均衡器或服务（LB PaaS）运行，该服务可以提供高可用性能力。
   此外，大小取决于所选负载均衡器和网络带宽等其他因素。有关更多信息，请参阅[负载均衡器](_index.md#load-balancers)。
5. 应在信誉良好的云提供商或私有化部署解决方案上运行。有关更多信息，请参阅[配置对象存储](#configure-the-object-storage)。
6. Gitaly 集群 (Praefect)提供了容错的好处，但也带来了设置和管理的额外复杂性。
...
7. Gitaly 规格基于使用模式和代码仓库大小的高百分位数据，且处于良好健康状态。
...
100 RPS / 5k 用户参考架构旨在满足大多数常见工作流。GitLab 定期针对以下端点吞吐量目标进行冒烟和性能测试：
...
- 适用于云原生混合环境的云提供商等效产品，并带有 Gateway API 或 Ingress 实现

<a id="set-up-components"></a>

## 设置组件

要设置极狐GitLab 及其组件以支持最高 100 RPS 或 5,000 名用户：
...
服务器在同一 10.6.0.0/24 私有网络范围内启动，并且可以通过这些地址自由地相互连接。
...
关于使用哪个负载均衡器或其确切配置的具体细节超出了极狐GitLab 文档的范围，但请参阅[负载均衡器](_index.md)以获取有关一般要求的更多信息。本节将重点介绍为您选择的负载均衡器需要配置的具体内容。

<a id="readiness-checks"></a>

### 就绪检查

确保外部负载均衡器仅路由到带有内置监控端点的正常服务。[就绪检查](../monitoring/health_check.md)都要求在被检查的节点上进行[额外配置](../monitoring/ip_allowlist.md)，否则外部负载均衡器将无法连接。
...
- (*1*): [Web 终端](../../ci/environments/_index.md#web-terminals-deprecated)支持要求您的负载均衡器正确处理 WebSocket 连接。使用 HTTP 或 HTTPS 代理时，这意味着您的负载均衡器必须配置为传递 `Connection` 和 `Upgrade` 逐跳头。有关更多详细信息，请参阅 [web 终端](../integration/terminal.md)集成指南。
...
- (*2*): 端口 443 用于 GitLab Pages 时应始终使用 TCP 协议。用户可以配置带有自定义 SSL 的自定义域名，如果 SSL 在负载均衡器处终止，则无法实现这一点。

<a id="alternate-ssh-port"></a>

#### 备用 SSH 端口

一些组织有禁止开放 SSH 端口 22 的策略。在这种情况下，配置一个备用 SSH 主机名可能会有所帮助，该主机名允许用户在端口 443 上使用 SSH。备用 SSH 主机名将需要一个新的虚拟 IP 地址，与之前记录的其它极狐GitLab HTTP 配置相比。
...
将您的负载均衡器配置为使用 `HTTP(S)` 协议而不是 `TCP` 协议。然后，负载均衡器将负责管理 SSL 证书和终止 SSL。
...
将您的负载均衡器配置为使用 `HTTP(S)` 协议而不是 `TCP` 协议。负载均衡器将负责管理最终用户将看到的 SSL 证书。

在这种情况下，负载均衡器和 NGINX 之间的流量也是安全的。由于连接全程都是安全的，因此无需为代理 SSL 添加配置。但是，必须向极狐GitLab 添加配置以配置 SSL 证书。有关管理 SSL 证书和配置 NGINX 的详细信息，请参阅 [HTTPS 文档](https://gitlab.cn/docs/omnibus/settings/ssl/)。
...
1. [下载并安装](../../install/package/_index.md#supported-platforms)您选择的 Linux 软件包。确保只添加极狐GitLab 软件包仓库，并为您选择的操作系统安装极狐GitLab。选择与您当前安装相同的版本和类型（基础版或企业版）。
...
当第三个 Consul 服务器预配完成后，会选举出一个 Consul 领导者。查看 Consul 日志 `sudo gitlab-ctl tail consul` 会显示 `...[INFO] consul: New leader elected: ...`。
...
在本节中，将指导您配置一个与极狐GitLab 一起使用的高可用 PostgreSQL 集群。
...
使用运行[受支持的 PostgreSQL 版本](../../install/requirements.md#postgresql)的信誉良好的提供商。已知以下服务运行良好：
...
带有复制和故障转移的 PostgreSQL 集群的推荐 Linux 软件包配置需要：
...
- 启用[数据库负载均衡](../postgresql/database_load_balancing.md)。
  ...
  在每个 PostgreSQL 节点上配置本地 PgBouncer 服务。这与跟踪主节点的 PgBouncer 集群是分开的。
...
首先，确保在每个节点上[安装](../../install/package/_index.md#supported-platforms) Linux 软件包。确保只添加极狐GitLab 软件包仓库，并为您选择的操作系统安装极狐GitLab，但不要提供 `EXTERNAL_URL` 值。
...
1. 为 PostgreSQL 用户名/密码对生成密码哈希。这假设您使用默认用户名 `gitlab`（推荐）。该命令会请求输入密码并确认。在下一步中使用此命令输出的值作为 `<postgresql_password_hash>` 的值：

   ```shell
   sudo gitlab-ctl pg-password-md5 gitlab
   ```

1. 为 PgBouncer 用户名/密码对生成密码哈希。这假设您使用默认用户名 `pgbouncer`（推荐）。该命令会请求输入密码并确认。在下一步中使用此命令输出的值作为 `<pgbouncer_password_hash>` 的值：

   ```shell
   sudo gitlab-ctl pg-password-md5 pgbouncer
   ```

1. 为 PostgreSQL 复制用户名/密码对生成密码哈希。这假设您使用默认用户名 `gitlab_replicator`（推荐）。该命令会请求输入密码并确认。在下一步中使用此命令输出的值作为 `<postgresql_replication_password_hash>` 的值：

   ```shell
   sudo gitlab-ctl pg-password-md5 gitlab_replicator
   ```

1. 为 Consul 数据库用户名/密码对生成密码哈希。这假设您使用默认用户名 `gitlab-consul`（推荐）。该命令会请求输入密码并确认。在下一步中使用此命令输出的值作为 `<consul_password_hash>` 的值：
...
PostgreSQL 在 Patroni 管理其故障转移的情况下，默认使用 `pg_rewind` 来处理冲突。与大多数故障转移处理方法一样，这有很小的可能导致数据丢失。有关更多信息，请参阅各种 [Patroni 复制方法](../postgresql/replication_and_failover.md#selecting-the-appropriate-patroni-replication-method)。
...
1. 确保每个节点都在与当前主节点通信：
   ...
   如果在输入密码后出现错误 `psql: ERROR:  Auth failed`，请确保您之前使用正确的格式生成了 MD5 密码哈希。正确的格式是连接密码和用户名：`PASSWORDUSERNAME`。例如，`Sup3rS3cr3tpgbouncer` 将是生成 `pgbouncer` 用户的 MD5 密码哈希所需的文本。
1. 控制台提示符可用后，运行以下查询：
...
> - Redis 主要是单线程的，增加 CPU 核心数不会带来显著收益。此架构使用单个组合 Redis 集群（3 个节点），但为了获得最佳性能，您可以将 Redis 拆分为单独的缓存和持久实例（共 6 个节点）。如果 Redis CPU 趋于饱和，强烈建议这样做。有关更多信息，请参阅[扩展文档](_index.md#scaling-an-environment)。
...
在本节中，将指导您配置一个与极狐GitLab 一起使用的外部 Redis 集群。以下 IP 用作示例：
...
- 应为此使用信誉良好的提供商或解决方案。已知 [Google Memorystore](https://cloud.google.com/memorystore/docs/redis/memorystore-for-redis-overview) 和 [AWS ElastiCache](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/WhatIs.html) 可以正常工作。
- 特别不支持 Redis 集群模式，但支持带有高可用性的 Redis 独立模式。
...
> 但是，如果您有[大型 monorepo](_index.md#large-monorepos)（大于几 GB）或[额外工作负载](_index.md#additional-workloads)，这些可能会显著影响环境性能，可能需要进行进一步调整。
...
- 如果要改为实现分片 Gitaly，请遵循[单独的 Gitaly 文档](../gitaly/configure_gitaly.md)，而不是本节。使用相同的 Gitaly 规格。
...
传统的 TCP 负载均衡器（如 HAProxy 或 AWS ELB）将流量分配到各个 Praefect 节点。这种方法：

- 适用于所有部署场景（Omnibus、云原生混合）
- 提供简单的设置和运维管理
- 支持 TLS 和非 TLS 配置
- 由于连接可能集中在某些节点上，因此可能会遇到流量分配不均的情况
...
1. 为 Praefect PostgreSQL 用户名/密码对生成密码哈希。这假设您使用默认用户名 `praefect`（推荐）。该命令会请求输入密码 `<praefect_postgresql_password>` 并确认。在下一步中，使用此命令输出的值作为 `<praefect_postgresql_password_hash>` 的值：
...
1. 从您配置的第一个 Linux 软件包节点复制 `/etc/gitlab/gitlab-secrets.json` 文件，并在此服务器上添加或替换同名文件。如果这是您配置的第一个 Linux 软件包节点，则可以跳过此步骤。
1. [重新配置极狐GitLab](../restart_gitlab.md#reconfigure-a-linux-package-installation) 以使更改生效。
1. 遵循[配置后的操作](#praefect-postgresql-post-configuration)步骤。

<div align="right">
  <a type="button" class="btn btn-default" href="#set-up-components">
    返回以设置组件 <i class="fa fa-angle-double-up" aria-hidden="true"></i>
  </a>
</div>
...
[如前所述](#configure-praefect-postgresql)，如果目标是实现完整的高可用性，建议为 Praefect 的数据库使用第三方 PostgreSQL 解决方案。

PostgreSQL HA 有许多第三方解决方案。所选解决方案必须满足以下条件才能与 Praefect 配合使用：

- 所有连接都有一个静态 IP，在故障转移时不会改变。
...
数据库设置完成后，请遵循[配置后的操作](#praefect-postgresql-post-configuration)步骤。

<a id="praefect-postgresql-post-configuration"></a>

#### Praefect PostgreSQL 配置后的操作
...
以下是使用 Linux 软件包 PostgreSQL 设置的示例：

1. SSH 登录到 Praefect PostgreSQL 节点。
1. 使用管理访问权限连接到 PostgreSQL 服务器。此处应使用 `gitlab-psql` 用户，因为 Linux 软件包默认会添加该用户。使用 `template1` 数据库是因为它在所有 PostgreSQL 服务器上默认创建。
...
- `<praefect_internal_token>`: 用于 Gitaly 集群 (Praefect)内部的复制流量。这与 `praefect_external_token` 不同，因为必须禁止 Gitaly 客户端直接访问 Gitaly 集群 (Praefect)的内部节点；直接访问这些节点可能导致数据丢失。
- `<praefect_postgresql_password>`: 上一节中定义的 Praefect PostgreSQL 密码也是此设置的一部分。

Gitaly 集群 (Praefect)节点在 Praefect 中使用 `virtual storage` 进行配置。每个存储包含构成集群的每个 Gitaly 节点的详细信息。每个存储也被赋予一个名称，该名称用于配置的多个区域。在本指南中，存储的名称为 `default`。此外，本指南面向新安装。如果要将现有环境升级为使用 Gitaly 集群 (Praefect)，您可能需要使用不同的名称。有关更多信息，请参阅 [Praefect 文档](../gitaly/praefect/configure.md#praefect)。
...
要在每个 Praefect 节点上进行配置，请在每个节点上执行以下操作：
...
构成集群的 [Gitaly](../gitaly/_index.md) 服务器节点具有依赖于数据和负载的要求。
...
Gitaly 服务器不得暴露于公共互联网，因为 Gitaly 上的网络流量默认未加密。强烈建议使用防火墙来限制对 Gitaly 服务器的访问。另一种选择是[使用 TLS](#gitaly-cluster-praefect-tls-support)。
...
1. 将以下内容附加到每个相应服务器的 `/etc/gitlab/gitlab.rb`：
   - 在 Gitaly 节点 1 上：
...
- 自带证书，因为这不会自动提供。与每个 Praefect 服务器对应的证书必须安装在该 Praefect 服务器上。

此外，证书或签发它的 CA 证书必须按照 [极狐GitLab 自定义证书配置](https://gitlab.cn/docs/omnibus/settings/ssl/#install-custom-public-certificates)（并在下面重复）中描述的过程安装在所有 Gitaly 服务器和所有与其通信的 Praefect 客户端上。

请注意以下几点：

- 证书必须指定您用于访问 Praefect 服务器的地址。您必须将主机名或 IP 地址作为主题备用名称添加到证书中。
- 您可以同时为 Praefect 服务器配置未加密的监听地址 `listen_addr` 和加密的监听地址 `tls_listen_addr`。这允许您在必要时从未加密流量逐步过渡到加密流量。要禁用未加密的监听器，请设置 `praefect['configuration'][:listen_addr] = nil`。
...
1. 在 Praefect 客户端（包括每个 Gitaly 服务器）上，将证书或签发它的 CA 证书复制到 `/etc/gitlab/trusted-certs`：

   ```shell
   sudo cp cert.pem /etc/gitlab/trusted-certs/
   ```

1. 在 Praefect 客户端（Gitaly 服务器除外）上，按如下方式编辑 `gitlab_rails['repositories_storages']`（位于 `/etc/gitlab/gitlab.rb` 中）：
...
要在每个 Sidekiq 节点上进行配置，请在每个节点上执行以下操作：
...
1. 为确保数据库迁移仅在重新配置期间运行，而不会在升级时自动运行，请执行：
   ...
   只有指定的单个节点应处理迁移，如 [GitLab Rails 配置后的操作](#gitlab-rails-post-configuration) 部分所述。
1. [重新配置极狐GitLab](../restart_gitlab.md#reconfigure-a-linux-package-installation) 以使更改生效。
1. 验证极狐GitLab 服务正在运行：
...
1. [下载并安装](../../install/package/_index.md#supported-platforms)您选择的 Linux 软件包。请务必仅为您的操作系统添加极狐GitLab 软件包仓库并安装极狐GitLab。
1. 创建或编辑 `/etc/gitlab/gitlab.rb` 并使用以下配置。为保持各节点间链接的统一性，应用程序服务器上的 `external_url` 应指向用户访问极狐GitLab 时使用的外部 URL。这将是[外部负载均衡器](#configure-the-external-load-balancer)的 URL，它将流量路由到极狐GitLab 应用程序服务器：
...
为每种数据类型使用单独的存储桶是极狐GitLab 的推荐方法。这可以确保极狐GitLab 存储的各种类型的数据之间不会发生冲突。未来有计划[启用单个存储桶](https://gitlab.com/gitlab-org/gitlab/-/issues/292958)。
...
首先是运行在 Kubernetes 中的组件。这些组件分布在多个节点组中，但只要满足最低 CPU 和内存要求，您可以根据需要更改整体构成。

| 组件节点组 | 目标节点池总计 | GCP 示例     | AWS 示例  |
|----------------------|-------------------------|-----------------|--------------|
| Webservice           | 36 vCPU<br/>45 GB 内存（请求）<br/>63 GB 内存（限制） | 3 x `n1-standard-16` | 3 x `c5.4xlarge` |
...
- [支持](#supporting)目标节点池总计是通用的，用于容纳支持极狐GitLab 部署以及您可能根据需求进行的任何其他部署的多种资源。与其他节点池类似，所选 Kubernetes 提供商的系统进程也需要资源。给出的示例已考虑到这一点。
...
| PostgreSQL<sup>2</sup>                    | 3     | 4 vCPU, 15 GB 内存  | `n1-standard-4` | `m5.xlarge`  |
| PgBouncer<sup>3</sup>                     | 3     | 2 vCPU, 1.8 GB 内存 | `n1-highcpu-2`  | `c5.large`   |
...
| Gitaly<sup>6</sup><sup>7</sup>            | 3     | 8 vCPU, 30 GB 内存  | `n1-standard-8` | `m5.2xlarge` |
...
2. 可以选择在信誉良好的第三方外部 PaaS PostgreSQL 解决方案上运行。有关更多信息，请参阅[提供您自己的 PostgreSQL 实例](#provide-your-own-postgresql-instance)。
3. 可以选择在信誉良好的第三方外部 PaaS Redis 解决方案上运行。有关更多信息，请参阅[提供您自己的 Redis 实例](#provide-your-own-redis-instance)。
4. 建议使用信誉良好的第三方负载均衡器或服务 (LB PaaS) 运行。此外，大小取决于所选的负载均衡器和网络带宽等其他因素。有关更多信息，请参阅[负载均衡器](_index.md#load-balancers)。
5. 应在信誉良好的云提供商或私有化部署解决方案上运行。有关更多信息，请参阅[配置对象存储](#configure-the-object-storage)。
6. Gitaly 集群 (Praefect)提供了容错优势，但也带来了额外的设置和管理复杂性。在部署 Gitaly 集群 (Praefect)之前，请查看现有的[技术限制和注意事项](../gitaly/praefect/_index.md#before-deploying-gitaly-cluster-praefect)。如果您想要分片 Gitaly，请对 `Gitaly` 使用上表中列出的相同规格。
7. Gitaly 规格基于运行状况良好的使用模式和代码仓库大小的高百分位数。但是，如果您有[大型 monorepo](_index.md#large-monorepos)（大于几 GB）或[额外工作负载](_index.md#additional-workloads)，这些可能会显著影响 Git 和 Gitaly 性能，并且可能需要进行进一步调整。
<!-- markdownlint-enable MD029 -->
...
对于 100 RPS 或 5,000 个用户，我们建议 Puma 工作进程总数约为 36，因此建议至少运行 9 个 Webservice Pod。
...
这不是严格的要求。只要 Gateway API 或 Ingress 控制器 Pod 有足够的资源来处理 Web 流量，就可以按需部署。
...
支持节点池旨在容纳 Webservice 和 Sidekiq 池中不需要的所有支持性部署。

这包括与云提供商的实施相关的各种部署，以及支持极狐GitLab 的部署，例如 [GitLab Shell](https://gitlab.cn/docs/charts/charts/gitlab/gitlab-shell/)。

要进行任何其他部署，例如容器镜像仓库、Pages 或监控，请尽可能在支持节点池中部署，而不是在 Webservice 或 Sidekiq 池中。支持节点池旨在容纳多个额外部署。但是，如果您的部署不适合给定的池，您可以相应地增加节点池。相反，如果您的用例中的池过度预配，您可以相应地减少。

<a id="example-config-file"></a>

### 示例配置文件

针对 100 RPS 或 5,000 用户参考架构配置的极狐GitLab Helm Charts 示例[可以在 Charts 项目中找到](https://jihulab.com/gitlab-cn/charts/gitlab/-/blob/master/examples/ref/5k.yaml)。
