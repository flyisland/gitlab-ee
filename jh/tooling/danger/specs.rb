...
module Tooling
  module Danger
    module Specs
      ...
      EE_PREFIX = 'ee/'
      MATCH_WITH_ARRAY_REGEX = /(?<to>to\(?\s*)(?<matcher>match|eq)(?<expectation>[( ]?\[[^\]]+)/
      ...
      def add_suggestions_for_match_with_array(filename)
        ...
        added_lines.each_with_object([]) do |added_line, processed_line_numbers|
          line_number =
            find_line_number(spec_file_lines, added_line.delete_prefix('+'), exclude_indexes: processed_line_numbers)
          processed_line_numbers << line_number
          suggested_line = spec_file_lines[line_number].gsub(MATCH_WITH_ARRAY_REGEX, '\k<to>match_array\k<expectation>')
          markdown(format(SUGGEST_MR_COMMENT, suggested_line: suggested_line), file: filename, line: line_number.succ)
        end
      end
